/** El modo CLIC-CLIC: clic en vacio -> debe aparecer el rectangulo -> mover -> clic cierra. */
...
const OUT=join(__dirname,"shots","clicclic"); fs.rmSync(OUT,{recursive:true,force:true}); fs.mkdirSync(OUT,{recursive:true});
...
await new Promise(r=>srv.listen(4821,r));
...
await pag.goto(`http://localhost:4821${BASE}workspace/?t=new-blank`,{waitUntil:"networkidle2",timeout:180000});
...
// el rectangulo visible es #? -> se busca el div del drag
const rect=()=>pag.evaluate(()=>{
  const d=[...document.querySelectorAll("div")].find(x=>x.style&&x.style.position==="fixed"&&x.style.borderStyle&&(x.style.borderStyle==="solid"||x.style.borderStyle==="dashed")&&x.style.display!=="none"&&parseFloat(x.style.width||"0")>0);
  return d?{w:d.style.width,h:d.style.height,borde:d.style.borderStyle,color:d.style.borderColor}:null; });
// 1) PRIMER CLIC en vacio
await pag.mouse.click(300,250); await esp(500);
await pag.mouse.move(800,600,{steps:8}); await esp(500);
const r1=await rect();
await pag.screenshot({path:join(OUT,"01_tras_primer_clic.png")});
ok(!!r1,"tras el PRIMER clic y mover, SE VE el rectangulo",JSON.stringify(r1));
// 2) SEGUNDO CLIC cierra y selecciona
await pag.mouse.click(800,600); await esp(800);
const n=await pag.evaluate(()=>window.__hekatanSelection?.size??0);
ok(n>0,"el SEGUNDO clic cierra y selecciona",`${n} objetos`);
await pag.screenshot({path:join(OUT,"02_tras_segundo_clic.png")});
