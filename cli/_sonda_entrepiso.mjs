...
let k = 0; const foto = async (n) => { await espera(400); await pag.screenshot({ path: `cli/shots/sonda_${String(k++).padStart(2, "0")}_${n}.png` }); };
...
// sonda: corte Z ≤ 5.6 (quita la bóveda), cámara desde arriba, ¿dónde está la cara superior del entrepiso?
await corte("Z", 5.6, false);
await ev(() => { const h = [...document.querySelectorAll("div")].find((d) => d.__ctx && d.__ctx.camera); const c = h.__ctx; c.setActiveCamera(c.perspCamera); c.camera.position.set(20, 112, 34); c.camera.up.set(0, 0, 1); c.controls.target.set(20, 123, 3); c.camera.lookAt(20, 123, 3); c.controls.update(); c.render(); window.__hekatanCadState.setTool("ifcface"); });
await espera(300);
const objetivos = []; for (let y = 116; y <= 131; y += 1.5) for (const x of [17, 20, 23]) objetivos.push([x, y, 5.0]);
const pxs = await proj(objetivos);
for (let i = 0; i < objetivos.length; i++) { await pag.mouse.move(pxs[i].x, pxs[i].y); await espera(120); const r = await ev(() => ({ cara: window.__hekatanCaraIfc?.(), xyz: window.__hekatanCursorXYZ })); if (r.cara && Math.abs(r.cara.normal[2]) > 0.9) console.log(JSON.stringify(objetivos[i]), "→ tris", r.cara.tris, "plana", r.cara.plana, "punto", JSON.stringify(r.cara.punto?.map((v) => +v.toFixed(2)))); }
await foto("sonda_entrepiso");
await nav.close();
