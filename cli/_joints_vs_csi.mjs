...
for (const ar of J.areas || []) {
  ...
  const el = EL[i];
  const hc = [a.bendingXXcentro?.get(i), a.bendingYYcentro?.get(i), a.bendingXYcentro?.get(i)];
  const hj = [a.bendingXXjoint?.get(i), a.bendingYYjoint?.get(i), a.bendingXYjoint?.get(i)];
  ...
  for (const v of fe) {
    ...
    for (const [q, idx] of [[0, 4], [1, 5], [2, 6]]) {
      ...
      ej.push([c, ["M11", "M22", "M12"][q], v[idx], hj[q][pos]]);
    }
...
console.log(`tipo ${tipo} ${nom} vs ${DIR}${process.env.DKQ ? " (DKQ " + process.env.DKQ + ")" : ""}: ${nE} cascaras, ${nJ} joints · |M|max CSI ${maxM.toFixed(3)}`);
