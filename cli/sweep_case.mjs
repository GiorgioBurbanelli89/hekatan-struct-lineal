...
// ────────── caso ──────────
const [nbxA, nbyA, nFA, msA, nModesA] = process.argv.slice(2);
const p = {
  nbx: +nbxA, nby: +nbyA, nFloors: +nFA,
  ms: msA ? +msA : 0.75,
  nWalls: 1, tWall: 0.25, tSlab: 0.20, bCol: 0.40, bBeam: 0.30, hBeam: 0.50, q: 1.0,
};
const nModes = nModesA ? +nModesA : 12;
const sys = { slab: true, walls: true };
const R = { nbx: p.nbx, nby: p.nby, nF: p.nFloors, msDisplay: p.ms, nModes };

// FASE 1 — display (lo que ya está en pantalla antes de tocar Modal)
let t0 = performance.now();
const d1 = buildEdificio(p, sys);
R.t_build_display = +(performance.now() - t0).toFixed(0);
R.nodes_display = d1.nodes.length; R.dof_display = d1.nodes.length * 6; R.elems_display = d1.elements.length;
t0 = performance.now();
const def1 = deform(d1.nodes, d1.elements, d1.ni, d1.ei);
R.t_deform_display = +(performance.now() - t0).toFixed(0);
R.deform_display_ok = def1.deformations.size > 0 &&
  [...def1.deformations.values()].every(v => v.every(Number.isFinite));

// FASE 2 — rebuild malla modal (método 3 → ms=1.0, dofCap=8000)
const MS_MODAL = 1.0, DOF_CAP = 8000;
t0 = performance.now();
const d2 = buildEdificio({ ...p, ms: MS_MODAL }, sys);
R.t_build_modal = +(performance.now() - t0).toFixed(0);
R.nodes_modal = d2.nodes.length; R.dof_modal = d2.nodes.length * 6;
R.dofCapExceeded = R.dof_modal > DOF_CAP;
// buildEdificio SIEMPRE resuelve el estático de la malla modal (líneas 176-179 de testM.ts)
t0 = performance.now();
const def2 = deform(d2.nodes, d2.elements, d2.ni, d2.ei);
R.t_deform_modal = +(performance.now() - t0).toFixed(0);

if (!R.dofCapExceeded) {
  const eiMass = { ...d2.ei, densities: new Map([...d2.ei.densities].map(([k, v]) => [k, v / GRAV])) };
  t0 = performance.now();
  const out = modalAnalysis(d2.nodes, d2.elements, d2.ni, eiMass, nModes, 1);
  R.t_modal = +(performance.now() - t0).toFixed(0);
  const f = out.frequencies || [];
  R.nFreq = f.length;
  R.T = f.slice(0, 3).map(x => x > 0 ? +(1 / x).toFixed(4) : 0);
  R.freqFinite = f.every(Number.isFinite);
  ...
  t0 = performance.now();
  const nodes = d2.nodes, elements = d2.elements;
  const zl = [...new Set(elements
    .filter(e => e.length === 2 && Math.abs(nodes[e[0]][2] - nodes[e[1]][2]) < 1e-6 && nodes[e[0]][2] > 0.05)
    .map(e => +nodes[e[0]][2].toFixed(2)))].sort((a, b) => a - b);
  R.nivelesDetectados = zl.length;
  R.pisosReales = p.nFloors;
  R.nivelesOk = zl.length === p.nFloors;
  const idsAt = z => nodes.map((n, i) => [n[2], i]).filter(o => Math.abs(o[0] - z) < 0.02).map(o => o[1]);
  const loads = new Map();
  zl.forEach(z => { const ids = idsAt(z), fq = 100 / Math.max(ids.length, 1); ids.forEach(i => { const c = loads.get(i) ?? [0,0,0,0,0,0]; c[0] += fq; loads.set(i, c); }); });
  const d3 = deform(nodes, elements, { supports: d2.ni.supports, loads }, d2.ei);
  R.t_deform_derivas = +(performance.now() - t0).toFixed(0);
  R.derivas_ok = d3.deformations.size > 0 && [...d3.deformations.values()].every(v => v.every(Number.isFinite));
}
R.t_total = R.t_build_display + R.t_deform_display + R.t_build_modal + R.t_deform_modal + (R.t_modal || 0) + (R.t_deform_derivas || 0);
