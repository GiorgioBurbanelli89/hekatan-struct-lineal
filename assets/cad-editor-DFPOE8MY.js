...
import { v as y, V as b, U as q, W as K, l as Q, X as ee, Y as oe, Z as ne, _ as se, A as ie, k as te, G, w as N, $ as C, J as ae, B as z, b as F, d as E, S as re, f as le, M as de, F as ce, L as pe, z as me } from "./Text-CU8HL4cE.js";
...
function Ae(e, l) {
  ...
  for (const c of a.x.axes) for (const t of a.y.axes) {
    const s = O(e, c.position, t.position, n.elevation), i = O(e, c.position, t.position, d.elevation);
    f.push(x(e, s, i));
  }
  return f;
}
...
function Se(e) {
  e.grid.val = { x: { label: "X", axes: [] }, y: { label: "Y", axes: [] }, stories: [] }, e.nodes.val = [], e.frames.val = [], e.areas.val = [], J = 0, W = 0;
}
function Ce() {
  ...
  const d = new se(a, n.domElement);
  d.target.set(5, 5, 0), d.minDistance = 0.5, d.maxDistance = 500, d.zoomSpeed = 5, d.enableDamping = false, d.update();
  const f = new ie(16777215, 0.6), c = new te(16777215, 0.4);
  c.position.set(30, -20, 40), l.add(f, c);
  const t = new G(), s = 1, i = new N(new b(1, 0, 0), new b(0, 0, 0), s, 6710886, 0.2, 0.2), p = new N(new b(0, 1, 0), new b(0, 0, 0), s, 6710886, 0.2, 0.2), r = new N(new b(0, 0, 1), new b(0, 0, 0), s, 6710886, 0.2, 0.2), m = new C("X", "red", "transparent"), v = new C("Y", "green", "transparent"), u = new C("Z", "blue", "transparent");
  ...
  const h = new ae(100, 100, 3355443, 2236962);
  ...
  new ResizeObserver(($) => {
    for (const A of $) {
      ...
      const I = S / X, Z = o.top;
      o.left = -Z * I, o.right = Z * I, o.updateProjectionMatrix(), n.setSize(S, X), M();
    }
  }).observe(e), d.addEventListener("change", M), requestAnimationFrame(() => {
    const $ = e.clientWidth, A = e.clientHeight;
    if ($ > 0 && A > 0) {
      a.aspect = $ / A, a.updateProjectionMatrix();
      const S = $ / A;
      o.left = -10 * S, o.right = 10 * S, o.updateProjectionMatrix(), n.setSize($, A), M();
    }
  });
  const k = { scene: l, perspCamera: a, orthoCamera: o, controls: d, renderer: n, render: M };
  return k.setActiveCamera = ($) => {
    g = $, d.object = $, d.update(), M();
  }, k.getActiveCamera = () => g, { element: e, ctx: k };
}
function V(e, l, a) {
  ...
  o.name = "cad-labels", l.scene.add(a), l.scene.add(o), y.derive(() => {
    ...
    for (const i of n.stories) {
      ...
      for (const r of n.x.axes) {
        const m = r.position, v = new z().setFromPoints([new b(m, c.minY - t, p), new b(m, c.maxY + t, p)]), u = new F({ color: 4473924, transparent: true, opacity: 0.6 });
        a.add(new E(v, u));
      }
      for (const r of n.y.axes) {
        const m = r.position, v = new z().setFromPoints([new b(c.minX - t, m, p), new b(c.maxX + t, m, p)]), u = new F({ color: 4473924, transparent: true, opacity: 0.6 });
        a.add(new E(v, u));
      }
    }
    if (n.stories.length > 1) {
      ...
      for (const r of n.x.axes) for (const m of n.y.axes) {
        const v = new z().setFromPoints([new b(r.position, m.position, i), new b(r.position, m.position, p)]), u = new F({ color: 3355443, transparent: true, opacity: 0.3 });
        a.add(new E(v, u));
      }
    }
    for (const i of n.x.axes) {
      const p = new C(i.name, "#ff6644", "transparent");
    ...
    if (t && (l.scene.remove(t), P(t)), s && (l.scene.remove(s), P(s)), f && n.length > 0) {
      const i = new G();
      i.name = "cad-nodes";
      const p = new re(0.15, 8, 8), r = new le({ color: 16746496 });
      for (const m of n) {
        const v = new de(p, r);
    ...
    if (c && d.length > 0) {
      ...
      if (p.length > 0) {
        const r = new z();
        r.setAttribute("position", new ce(p, 3));
        const m = new F({ color: 52479, linewidth: 2 });
        i.add(new pe(r, m));
      }
      l.scene.add(i);
    }
...
function P(e) {
  ...
  e.dispose && e.dispose(), (_a = e.children) == null ? void 0 : _a.forEach(P);
}
function Ye(e, l) {
  ...
  function s() {
    ...
    switch (l.renderer.clippingPlanes = [], o.viewMode) {
      ...
      case "plan": {
        ...
        Me(l, new b(r.centerX, r.centerY, h), v);
        break;
      }
      case "elevationX": {
  ...
  function i() {
    ...
    l.renderer.clippingPlanes = [new me(r, p)], l.render();
  }
        ...
        for (const d of n.stories.slice(1)) {
          ...
          for (const c of n.y.axes) {
            ...
            for (let s = 0; s < n.x.axes.length - 1; s++) {
              const i = n.x.axes[s].position, p = n.x.axes[s + 1].position, r = Y(e, i, t, f), m = Y(e, p, t, f);
              r >= 0 && m >= 0 && x(e, r, m);
            }
          }
          for (const c of n.x.axes) {
            ...
            for (let s = 0; s < n.y.axes.length - 1; s++) {
              const i = n.y.axes[s].position, p = n.y.axes[s + 1].position, r = Y(e, t, i, f), m = Y(e, t, p, f);
              r >= 0 && m >= 0 && x(e, r, m);
            }
          }
        }
...
const L = ue(), { element: B, ctx: U } = Ce(), _ = Ye(L, U);
Fe(L, U);
...
H.id = "cad-info";
H.textContent = "Hekatan Struct \u2014 type cad.help() in console";
B.appendChild(H);
document.body.appendChild(j);
const ke = Pe(L, _);
window.cad = ke;
console.log("%c Hekatan Struct %c type cad.help() for commands", "background: #0088ff; color: white; padding: 2px 6px; border-radius: 3px;", "color: #888;");
