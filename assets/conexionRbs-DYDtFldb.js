import { a as ft, __tla as __tla_0 } from "./analyze-DW0aWqsq.js";
import { d as _t, __tla as __tla_1 } from "./didacticCpp-BoYi16rL.js";
import { b as bt, c as ut, i as ht } from "./menegottoPinto-9Id4uJ7l.js";
import { s as Ke, __tla as __tla_2 } from "./secantPlasticity-CBNRaGeK.js";
import { b as O, B as D, d as L, S as Me, f as ge, M as V, V as B, c as he, L as Mt, E as gt, q as Ze, e as te, T as He } from "./Text-C1TX4d8g.js";
...
let __tla = Promise.all([
  ...
  function xt(e) {
    var _a, _b;
    const n = e.nSteps ?? 10, l = e.maxIterPerStep ?? 8, f = e.softeningFactor, p = [], b = [], u = [], h = [], v = [];
    let w = -1, a = {}, s = {};
    for (let F = 1; F <= n; F++) {
      ...
      p.push(y), b.push(C), u.push(c), h.push(m.elementsYielded), v.push(m.converged), w < 0 && m.elementsYielded > 0 && (w = F - 1), a = m.deformOutputs, s = m.analyzeOutputs;
    }
    const i = b[0] > 1e-12 ? p[0] / b[0] : 0;
    return {
      lambdas: p,
      displacements: b,
      vonMisesMax: u,
  ...
  function wt(e) {
    ...
    if (f === 0) return l;
    const p = Math.max(...e.displacements, 1e-9), b = Math.max(...e.lambdas, 1e-9), u = e.displacements.map((c) => c / p * n), h = e.lambdas.map((c) => c / b * n), [v, w, a] = e.center, s = (c, g) => new B(v + c, w, a + g), i = new O({
      color: 11184810
    ...
    for (let c = 0; c < f; c++) m.push(s(u[c], h[c]));
    ...
    for (let c = 0; c < f; c++) {
      ...
      R.position.copy(s(u[c], h[c])), l.push(R);
    }
    if (e.firstYieldStep >= 0) {
      const c = e.firstYieldStep, g = s(u[c], h[c]), $ = new O({
        color: 16711680
      }), _ = n * 0.04, R = new D().setFromPoints([
      ...
      l.push(new L(R, $)), l.push(new L(me, $));
    }
    return l;
  }
  function pt(e) {
    ...
    for (let f = 0; f < e.elements.length; f++) {
      ...
      let b = 0, u = 0, h = 0;
      for (const g of p) b += e.nodes[g][0], u += e.nodes[g][1], h += e.nodes[g][2];
      b /= p.length, u /= p.length, h /= p.length;
      const v = Math.abs(b), w = Math.abs(u), a = Math.abs(h), s = Math.abs(b - (e.d_col / 2 - e.tf_col / 2)) < l, i = Math.abs(b - (-e.d_col / 2 + e.tf_col / 2)) < l, F = w < 1e-3 && v <= e.d_col / 2 && a <= e.d_col, y = b > e.x_col_face - l, S = y && Math.abs(h - (e.d_beam / 2 - e.tf_beam / 2)) < l, z = y && Math.abs(h - (-e.d_beam / 2 + e.tf_beam / 2)) < l, m = y && w < 1e-3 && a < e.d_beam / 2 - e.tf_beam / 2, M = y && b < e.x_col_face + l * 2, C = b >= e.x_rbs_start && b <= e.x_rbs_end;
      let c = "other";
  ...
  function vt(e, n, l) {
    ...
    for (const [b, u] of e.entries()) {
      ...
      w || (w = {
        vmValues: [],
        count: 0
      }, f.set(u, w)), w.vmValues.push(v), w.count++;
    }
    const p = [];
    for (const [b, u] of f.entries()) {
      if (u.vmValues.length === 0) continue;
      const h = Math.max(...u.vmValues), v = u.vmValues.reduce((i, F) => i + F, 0) / u.vmValues.length, w = h / l;
    ...
    return p.sort((b, u) => u.ratio - b.ratio), p;
  }
  function Ft(e) {
  ...
  function zt(e, n, l, f) {
    if (!f) return [];
    let p = 1 / 0, b = 1 / 0, u = 1 / 0, h = -1 / 0, v = -1 / 0, w = -1 / 0, a = 0;
    for (const [z, m] of l.entries()) {
      if (m !== f) continue;
      const M = n[z];
      for (const C of M) {
        const [c, g, $] = e[C];
        c < p && (p = c), g < b && (b = g), $ < u && (u = $), c > h && (h = c), g > v && (v = g), $ > w && (w = $);
      }
      a++;
    }
    if (a === 0) return [];
    const s = 0.015;
    p -= s, b -= s, u -= s, h += s, v += s, w += s;
    const i = new he(h - p, v - b, w - u), F = new ge({
      color: 16711748,
      transparent: true,
      opacity: 0.15,
      wireframe: false
    }), y = new V(i, F);
    y.position.set((p + h) / 2, (b + v) / 2, (u + w) / 2);
  ...
  function $t(e) {
    ...
    const f = e.size, p = Math.max(...e.theta.map((_) => Math.abs(_)), 1e-9), b = Math.max(...e.M.map((_) => Math.abs(_)), 1e-9), u = e.theta.map((_) => _ / p * (f / 2)), h = e.M.map((_) => _ / b * (f / 2)), [v, w, a] = e.center, s = (_, R) => new B(v + _, w, a + R), i = f / 2, F = new O({
      color: 6710886
    });
    ...
    for (let _ = 0; _ < l; _++) {
      const R = s(u[_], h[_]);
      m[_ * 3 + 0] = R.x, m[_ * 3 + 1] = R.y, m[_ * 3 + 2] = R.z;
    }
    const M = new D();
    ...
    return $.position.copy(s(u[0], h[0])), n.push($), {
      objects: n,
      moveCursor: (_) => {
        const R = Math.max(0, Math.min(l - 1, _));
        $.position.copy(s(u[R], h[R]));
      },
      setProgress: (_) => {
        const R = Math.max(1, Math.min(l, _ + 1));
        M.setDrawRange(0, R);
      }
    };
  }
  function Ct(e) {
    ...
    const f = e.size, p = Math.max(...e.drifts.map((m) => Math.abs(m)), 1e-9), b = e.drifts.map((m, M) => M / (l - 1) * f), u = e.drifts.map((m) => m / p * (f / 2)), [h, v, w] = e.center, a = (m, M) => new B(h + m, v, w + M), s = f / 2;
    n.push(new L(new D().setFromPoints([
      a(0, -s),
      a(f, -s),
      a(f, +s),
    ...
    return z.position.copy(a(b[0], u[0])), n.push(z), {
      objects: n,
      moveCursor: (m) => {
        const M = Math.max(0, Math.min(l - 1, m));
        z.position.copy(a(b[M], u[M]));
      },
      setProgress: (m) => {
  ...
  Et = {
    ...
    build(e, n) {
      var _a;
      const l = [], f = [], p = /* @__PURE__ */ new Map(), b = /* @__PURE__ */ new Map(), u = /* @__PURE__ */ new Map(), h = /* @__PURE__ */ new Map(), v = /* @__PURE__ */ new Map(), w = /* @__PURE__ */ new Map(), a = /* @__PURE__ */ new Map(), s = /* @__PURE__ */ new Map(), i = /* @__PURE__ */ new Map(), F = e.E_steel / 2.6, y = 7.85, S = 1e-4, z = /* @__PURE__ */ new Map(), m = (t, o, r) => {
        const d = Math.round(t / S), x = Math.round(o / S), P = Math.round(r / S), I = `${d},${x},${P}`;
        let A = z.get(I);
        ...
        const P = f.length - 1;
        p.set(P, x), b.set(P, e.E_steel), u.set(P, 0.3), h.set(P, y), v.set(P, 0), w.set(P, 0), a.set(P, 0), s.set(P, 0), i.set(P, F);
      }, C = e.story_h, c = Math.max(1, Math.round(e.mesh_density)), g = 6 * c, $ = 2 * c, _ = c + 1, R = +e.d_col / 2 - e.tf_col / 2, me = -e.d_col / 2 + e.tf_col / 2, ze = me, Je = R, oe = [];
      for (let t = 0; t <= g; t++) {
        const o = -C / 2 + t * C / g, r = [];
        for (let d = 0; d <= $; d++) {
      ...
      for (let t = 0; t <= g; t++) {
        const o = -C / 2 + t * C / g, r = [];
        for (let d = 0; d <= _; d++) {
          const x = ze + (Je - ze) * (d / _);
          r.push(m(x, 0, o));
        }
        se.push(r);
      }
      for (let t = 0; t < g; t++) for (let o = 0; o < _; o++) M(se[t][o], se[t][o + 1], se[t + 1][o + 1], se[t + 1][o], e.tw_col);
      const E = e.d_col / 2, j = e.a_rbs * e.bf_beam, W = j + e.b_rbs * e.d_beam, $e = (j + W) / 2, de = e.c_rbs * e.bf_beam, Ce = e.b_rbs * e.d_beam, xe = (4 * de * de + Ce * Ce) / (8 * de), Pe = (t) => {
      ...
      for (let t = 1; t <= Re; t++) J.push(j * t / Re);
      const Ie = 8 * c + 4;
      ...
      try {
        ...
        if (t === 2) {
          ...
          const d = r.finalAnalyzeOutputs, [x, P] = Se(d.vonMises, 90, e.Fy);
          d.colorMapRanges = {
            ...d.colorMapRanges,
            vonMises: [
              x,
      ...
      try {
        ...
        if (o) {
          ...
          const P = zt(l, f, t, (d == null ? void 0 : d.zone) ?? null);
          n.objects3D.val = [
            ...n.objects3D.val,
            ...P
          ];
      ...
      const Ne = (t) => {
        const o = Pe(t), r = E + t, d = [
          new B(r, -o / 2, +e.d_beam / 2 + 5e-3),
          new B(r, +o / 2, +e.d_beam / 2 + 5e-3),
          new B(r, +o / 2, -e.d_beam / 2 - 5e-3),
          new B(r, -o / 2, -e.d_beam / 2 - 5e-3),
          new B(r, -o / 2, +e.d_beam / 2 + 5e-3)
        ], x = new D().setFromPoints(d), P = new O({
          color: 16776960
        });
        return new L(x, P);
      };
      N.push(Ne(j)), N.push(Ne(W)), n.objects3D.val = N;
      const ae = e.classification < 0.5 ? 0.02 : e.classification < 1.5 ? 0.04 : 0.06, ct = bt(e.story_h, Math.round(e.steps_per_cycle), ae), it = e.b_rbs * e.d_beam, mt = e.d_beam / 2 / it, Ve = {
        Fy: e.Fy,
        E: e.E_steel,
        b: e.b_hard,
        R0: 15,
        cR1: 0.925,
        cR2: 0.15
      }, Xe = e.bf_beam * (1 - 2 * e.c_rbs), Te = Xe * e.tf_beam * (e.d_beam - e.tf_beam), be = e.Fy * Te, k = [], le = [];
      let Fe = ht(Ve);
      for (const t of ct) {
        const o = t * mt;
        Fe = ut(o, Fe, Ve), k.push(t), le.push(Fe.sigma * Te);
      }
      n.__rbsHistory = {
        theta: k,
        M: le,
      ...
      if (je && (clearInterval(je), window.__rbsK3Anim = null), e.animate_k3 > 0.5) {
        const t = e.animate_k3 < 1.5 ? 25 : 100, o = Math.max(...k.map((P) => Math.abs(P))), r = e.anim_amp;
        let d = 0;
      ...
      let ue = 0;
      for (let t = 0; t < k.length; t++) Math.abs(k[t] - ae) < 5e-3 && Math.abs(le[t]) > Math.abs(ue) && (ue = le[t]);
      const We = Math.abs(ue) / be;
      console.log(`[RBS AISC 358-22] Shells=${f.length}, Nodos=${l.length}
  bf_rbs=${Xe.toFixed(3)}m, Mp_rbs=${be.toFixed(1)} kN\xB7m
  M@${(ae * 100).toFixed(1)}%=${ue.toFixed(1)} kN\xB7m \xB7 Ratio=${We.toFixed(3)} \u2192 ${We >= 0.8 ? "PASA" : "FALLA"}`);
    },
    computedLabels(e, n) {
      ...
      const f = e.bf_beam * (1 - 2 * e.c_rbs), p = f * e.tf_beam * (e.d_beam - e.tf_beam), b = l.Mp, u = l.targetDrift;
      let h = 0;
      for (const a of l.M) Math.abs(a) > Math.abs(h) && (h = a);
      let v = 0;
      for (let a = 0; a < l.theta.length; a++) Math.abs(l.theta[a] - u) < 5e-3 && Math.abs(l.M[a]) > Math.abs(v) && (v = l.M[a]);
      const w = Math.abs(v) / b;
      return {
        "\u2500\u2500 Geometr\xEDa RBS (AISC 358-22 \xA75) \u2500\u2500": "",
        "Clasificaci\xF3n ensayo": l.classification,
        "Target drift \u03B8": `${(u * 100).toFixed(1)} %`,
