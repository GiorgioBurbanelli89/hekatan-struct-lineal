...
let __tla = Promise.all([
  ...
  function je(d) {
    ...
    for (const $ of c) Oe[$] !== void 0 && (X[Oe[$]] = true);
    if (/^[01]+$/.test(t) && t.length <= 6) for (let $ = 0; $ < t.length; $++) X[$] = t[$] === "1";
    return X;
  }
  Qe = function(d) {
    const t = {
      ...
      try {
        switch (z) {
          ...
          case "n": {
            ...
            !isFinite(e) || !isFinite(n) || !isFinite(a) || !isFinite(r) ? t.errors.push(`L${Y + 1}: node mal formado: ${k}`) : t.nodes.set(e, [
              n,
              a,
              r
            ]);
          ...
          case "cft": {
            const e = parseInt(o[1], 10), n = o.slice(2).map((R) => parseFloat(R)), a = n.length >= 5 && n[3] < 1 && n[4] >= 1, r = n[0], l = n[1], F = n[2], v = a ? n[3] : n[2], h = a ? 4 : 3, y = isFinite(n[h]) ? n[h] : 25e6, I = isFinite(n[h + 1]) ? n[h + 1] : 0.2, j = isFinite(n[h + 2]) ? n[h + 2] : 2.4;
          ...
          case "mass": {
            const e = parseInt(o[1], 10), n = parseFloat(o[2] ?? "0");
            Number.isFinite(e) && Number.isFinite(n) ? t.masses.set(e, (t.masses.get(e) ?? 0) + n) : t.errors.push(`L${Y + 1}: mass necesita <nudo> <toneladas>`);
            break;
          }
          case "vista": {
          ...
          case "clear":
            ...
            break;
          default:
            t.errors.push(`L${Y + 1}: comando desconocido "${z}"`);
        }
      } catch (e) {
        t.errors.push(`L${Y + 1}: error "${k}" \u2014 ${e.message}`);
      }
    }
    return t;
  };
...
`, Z = (d, t) => [
  ...
  function ss(d, t = 200, X) {
    ...
    const o = Y.map((h) => h[0]), z = Y.map((h) => h[1]), e = Math.min(...o), n = Math.max(...o), a = Math.min(...z), r = Math.max(...z), l = [
      0,
      1,
      2,
      3
    ...
    for (let h = 0; h < t; h++) for (let y = 0; y < t; y++) {
      const I = e + (n - e) * (h + 0.5) / t, j = a + (r - a) * (y + 0.5) / t;
      let R = 0, V = 0;
      for (let q = 0; q < 4; q++) {
        const U = Y[q], se = Y[(q + 1) % 4];
        (se[0] - U[0]) * (j - U[1]) - (se[1] - U[1]) * (I - U[0]) >= 0 ? R++ : V++;
      }
      if (R !== 4 && V !== 4) continue;
      let J = k[0], P = 1 / 0;
      for (const q of k) {
        const U = Y[q], se = Y[(q + 1) % 4], oe = se[0] - U[0], _ = se[1] - U[1], pe = oe * oe + _ * _, ce = Math.max(0, Math.min(1, ((I - U[0]) * oe + (j - U[1]) * _) / pe)), te = Math.hypot(I - (U[0] + ce * oe), j - (U[1] + ce * _));
        te < P && (P = te, J = q);
      }
      l[J].pts.push([
        c[0] + I * $[0] + j * D[0],
  ...
  function ts(d, t) {
    if (!(t > 0)) return;
    const X = 1e-6, c = (z) => d.nodes.get(z);
    let $ = Math.max(0, ...d.nodes.keys()) + 1, B = d.shells.reduce((z, e) => Math.max(z, e.id), 0) + 1;
    const D = (z) => {
      for (const [n, a] of d.nodes) if (K(Z(a, z)) < X) return n;
      const e = $++;
  ...
  function os(d) {
    const X = (a) => d.nodes.get(a), c = [
      ...d.nodes.keys()
    ], $ = (a, r, l) => {
      const F = Z(r, a), v = K(F), h = ie(F, 1 / v), y = [];
      for (const I of c) {
        if (l.includes(I)) continue;
        const j = Z(X(I), a), R = fe(j, h);
        R > 1e-6 && R < v - 1e-6 && K(Z(j, ie(h, R))) < 1e-4 && y.push(R / v);
      }
      return y.sort((I, j) => I - j);
    }, B = (a, r) => {
      const l = [];
    ...
    let Y = d.shells.reduce((a, r) => Math.max(a, r.id), 0) + 1;
    const k = [], o = (a, r) => {
    ...
    for (const a of d.shells) {
      if (!Pe(d, a.id) || a.pts.length !== 4 || a.pts.some((J) => !d.nodes.has(J))) {
        k.push(a);
        continue;
      }
      const r = a.pts.map(X), l = $(r[0], r[1], a.pts), F = $(r[2], r[3], a.pts).map((J) => 1 - J), v = $(r[1], r[2], a.pts), h = $(r[3], r[0], a.pts).map((J) => 1 - J);
      let y = [
        0,
        ...B(l, F),
        1
      ...
      const j = (J, P) => [
        ...
        1,
        2
      ].map((q) => (1 - J) * (1 - P) * r[0][q] + J * (1 - P) * r[1][q] + J * P * r[2][q] + (1 - J) * P * r[3][q]);
      let R = I.map((J) => y.map((P) => D(j(P, J))));
      if (R.some((J) => J.some((P) => P === void 0)) && (y.length >= I.length ? I = [
        0,
        1
      ] : y = [
        0,
        1
      ], R = I.map((J) => y.map((P) => D(j(P, J)))), R.some((J) => J.some((P) => P === void 0)))) {
        k.push(a);
        continue;
      }
      let V = true;
      for (let J = 0; J < I.length - 1; J++) for (let P = 0; P < y.length - 1; P++) {
        const q = [
          R[J][P],
          R[J][P + 1],
          R[J + 1][P + 1],
    ...
    const z = 9.80665, e = (a, r) => {
      ...
      return d.frames.filter((h) => [
        ...
        if (!I) return false;
        const j = Z(I, a), R = fe(j, v);
        return R > -1e-4 && R < F + 1e-4 && K(Z(j, ie(v, R))) < 1e-4;
      }));
    };
    for (const a of d.shells) {
      if (!Pe(d, a.id) || a.pts.length !== 4) continue;
      ...
      if (d.deckOneWay) {
        const I = Z(v[1], v[0]);
        let j = Fe(I, Z(v[3], v[0]));
        j = ie(j, 1 / K(j));
        const R = ie(I, 1 / K(I)), V = Fe(j, R), J = (d.shellAngles.get(a.id) ?? 0) * Math.PI / 180;
        h = [
          0,
          1,
          2
      ...
      const y = ss(v, 200, h);
      for (let I = 0; I < 4; I++) {
        const { pts: j, dA: R } = y[I];
        if (!j.length) continue;
        const V = v[I], J = v[(I + 1) % 4], P = n(V, J);
        if (!P.length) {
          const _ = F * R * j.length;
          e(a.pts[I], [
            0,
            0,
            _ / 2,
        ...
        const q = Z(J, V), U = K(q), se = ie(q, 1 / U), oe = j.map((_) => fe(Z(_, V), se));
        for (const _ of P) {
          const pe = X(_.nI), ce = X(_.nJ), te = fe(Z(pe, V), se), be = fe(Z(ce, V), se), Me = Math.min(te, be), le = Math.max(te, be), Q = le - Me;
          if (Q < 1e-9) continue;
          const Ce = le >= U - 1e-6, ne = ie(Z(ce, pe), 1 / Q), ee = Fe(ne, [
            0,
            0,
            1
          ]);
          let he = 0, re = 0, Ie = 0, me = 0;
          for (const de of oe) {
            if (de < Me - 1e-9 || (Ce ? de > le + 1e-9 : de >= le - 1e-9)) continue;
            let we = de - Me;
            te > be && (we = Q - we);
            const G = we / Q;
            he += 1 - 3 * G * G + 2 * G * G * G, re += Q * (G - 2 * G * G + G * G * G), Ie += 3 * G * G - 2 * G * G * G, me += Q * (-G * G + G * G * G);
          }
          const ae = F * R;
          e(_.nI, [
            0,
  ...
  ls = {
    ...
    runModal(d, t, X) {
      ...
      if (!(!c.length || !$.length)) try {
        const B = Math.max(1, parseInt(window.__hekatanCliModalModes ?? "12", 10) || 12), D = t.nodeInputs.val, Y = window.__hekatanCliSprings, k = He(c, $, D, t.elementInputs.val, B, 0, 0, 1, (D == null ? void 0 : D.diaphragms) instanceof Map && D.diaphragms.size ? D.diaphragms : void 0, Y && Y.length ? Y : void 0);
    ...
    build(d, t) {
      ...
      const $ = /* @__PURE__ */ new Map(), B = /* @__PURE__ */ new Map(), D = [], Y = Array.from(c.nodes.keys()).sort((s, f) => s - f);
      for (const s of Y) $.set(s, D.length), D.push(c.nodes.get(s));
      const k = [], o = /* @__PURE__ */ new Map(), z = /* @__PURE__ */ new Map(), e = /* @__PURE__ */ new Map(), n = /* @__PURE__ */ new Map(), a = /* @__PURE__ */ new Map(), r = /* @__PURE__ */ new Map(), l = /* @__PURE__ */ new Map(), F = /* @__PURE__ */ new Map(), v = /* @__PURE__ */ new Map(), h = /* @__PURE__ */ new Map(), y = /* @__PURE__ */ new Map(), I = /* @__PURE__ */ new Map(), j = /* @__PURE__ */ new Map(), R = /* @__PURE__ */ new Map(), V = /* @__PURE__ */ new Map(), J = /* @__PURE__ */ new Map(), P = /* @__PURE__ */ new Map(), q = /* @__PURE__ */ new Map(), U = /* @__PURE__ */ new Map(), se = /* @__PURE__ */ new Map();
      for (const s of c.frames) {
        const f = $.get(s.nI), M = $.get(s.nJ);
        if (f === void 0 || M === void 0) {
          const b = Y.length ? `IDs disponibles: ${Y.join(", ")}` : "ning\xFAn nodo definido", C = [];
        ...
        const p = s.nu ?? 0.2;
        o.set(i, s.E), z.set(i, s.E / (2 * (1 + p))), e.set(i, s.A), n.set(i, s.I), a.set(i, s.Iy ?? s.I), r.set(i, s.J ?? 0.14 * Math.pow(Math.sqrt(s.A), 4)), l.set(i, s.rho ?? 2.45), U.set(i, p), s.D !== void 0 && isFinite(s.D) && F.set(i, s.D), s.B !== void 0 && isFinite(s.B) && v.set(i, s.B);
        const m = c.frameAngles.get(s.id);
        m !== void 0 && isFinite(m) && y.set(i, m);
        const w = c.frameReleases.get(s.id);
        w && I.set(i, w);
        const u = c.frameEndOffsets.get(s.id);
        u && j.set(i, u);
        const S = c.frameLoads.get(s.id);
        S && J.set(i, S);
        const O = c.frameShearAreas.get(s.id);
        if (O && (q.set(i, O[0]), P.set(i, O[1])), s.sec || s.D !== void 0 && s.B !== void 0) {
          const b = {
            type: "general"
          };
        ...
        if (g) {
          const b = Ne(g.d, g.bf, g.tf, g.tw, g.t2b, g.tfb);
          e.set(i, b.A), a.set(i, b.Iz), n.set(i, b.Iy), r.set(i, b.J), q.set(i, b.As2), P.set(i, b.As3), F.set(i, g.d), v.set(i, Math.max(g.bf, g.t2b));
          const C = (W) => Math.round(W * 1e4) / 10;
          h.set(i, {
            type: "I",
            h: g.d,
            b: g.bf,
        ...
        if (x) {
          const b = qe(x.b, x.h, x.tf, x.tw);
          e.set(i, b.A), a.set(i, b.Iz), n.set(i, b.Iy), r.set(i, b.J), q.set(i, b.As2), P.set(i, b.As3), F.set(i, x.h), v.set(i, x.b);
          const C = (W) => Math.round(W * 1e4) / 10;
          h.set(i, {
            type: "HSS",
            h: x.h,
        ...
        if (E) {
          ...
          e.set(i, b.A), a.set(i, b.Iz), n.set(i, b.Iy), r.set(i, b.J), q.set(i, b.As2), P.set(i, b.As3), F.set(i, E.d), v.set(i, E.bf);
          const C = (W) => Math.round(W * 1e4) / 10;
          h.set(i, {
            type: "C",
            h: E.d,
        ...
        if (T) {
          ...
          e.set(i, b.A), a.set(i, b.Iz), n.set(i, b.Iy), r.set(i, b.J), q.set(i, b.As2), P.set(i, b.As3), F.set(i, T.d), v.set(i, T.t2);
          const C = (W) => Math.round(W * 1e4) / 10;
          h.set(i, {
            type: "2L",
            h: T.d,
        ...
        if (L) {
          const b = Ye(L.D, L.t, s.E, p, L.Ec, L.nuC);
          e.set(i, b.A), a.set(i, b.Iz), n.set(i, b.Iy), r.set(i, b.J), q.set(i, b.As2), P.set(i, b.As3), F.set(i, L.D), v.set(i, L.D);
          const C = L.D - 2 * L.t, W = Math.PI * C * C / 4, N = Math.PI * L.D * L.D / 4 - W;
          l.set(i, ((s.rho ?? 7.85) * N + L.rhoC * W) / b.A), h.set(i, {
            type: "CFT",
            d: L.D,
            tw: L.t,
            fillE: L.Ec,
        ...
        const A = c.frameCft.get(s.id);
        if (A) {
          const b = Ue(A.b, A.h, A.t, s.E, p, A.Ec, A.nuC, A.tw);
          e.set(i, b.A), a.set(i, b.Iz), n.set(i, b.Iy), r.set(i, b.J), q.set(i, b.As2), P.set(i, b.As3), F.set(i, A.h), v.set(i, A.b);
          const C = (A.b - 2 * A.tw) * (A.h - 2 * A.t), W = A.b * A.h - C;
          l.set(i, ((s.rho ?? 7.85) * W + A.rhoC * C) / b.A);
          const N = (H) => Math.round(H * 1e3);
          h.set(i, {
            type: "CFT",
            b: A.b,
            h: A.h,
            ...
            fillRho: A.rhoC,
            steelRho: s.rho ?? 7.85,
            name: s.sec ?? `CFT ${N(A.h)}X${N(A.b)}X${N(A.t)}${A.tw !== A.t ? `X${N(A.tw)}` : ""}`
          });
        }
      }
      if (c.meshCross) {
        const s = (w, u) => w[0] * u[0] + w[1] * u[1] + w[2] * u[2], f = [
          o,
          z,
          e,
          n,
          ...
          for (const S of f) S.has(w) && S.set(u, S.get(w));
        }, i = (w) => {
          for (let u = 0; u < D.length; u++) if (Math.hypot(D[u][0] - w[0], D[u][1] - w[1], D[u][2] - w[2]) < 1e-6) return u;
          return D.push([
            w[0],
            w[1],
            w[2]
        ...
        for (let w = 0; w < k.length; w++) {
          ...
          for (let S = w + 1; S < k.length; S++) {
            ...
            const L = D[O], A = D[g], b = D[x], C = D[E], W = [
              ...
              L[1] - b[1],
              L[2] - b[2]
            ], ue = s(W, W), ye = s(W, N), $e = s(N, N), ze = s(W, H), Te = s(N, H), Se = ue * $e - ye * ye;
            if (Se < 1e-10 * ue * $e) continue;
            const ve = (ye * Te - $e * ze) / Se, ke = (ue * Te - ye * ze) / Se;
            ...
            if (Math.hypot(Le[0] - Ae[0], Le[1] - Ae[1], Le[2] - Ae[2]) > 1e-6) continue;
            const Xe = i(Le);
            for (const ge of [
              w,
              S
      ...
      for (const s of c.shells) {
        const f = s.pts.map((m) => $.get(m));
        if (f.some((m) => m === void 0)) {
          c.errors.push(`shell ${s.id}: algun nodo inexistente`);
          continue;
        }
        const M = k.length;
        B.set(s.id, M), k.push(f), o.set(M, s.E), z.set(M, s.E / (2 * 1.2)), se.set(M, s.t), l.set(M, s.rho ?? 2.45), U.set(M, 0.2);
        const i = c.shellTypes.get(s.id);
        i !== void 0 && R.set(M, i);
        const p = c.deckSecs.get(s.id);
        if (p) {
          const m = p.tc + (p.sr > 0 ? p.hr * (p.wrt + p.wrb) / 2 / p.sr : 0);
          se.set(M, p.tc), l.set(M, ((s.rho ?? 2.45) * m + p.w / 9.80665) / p.tc), V.set(M, {
            ...p
          });
        }
      }
      ...
      for (const [s, f] of Me) if (s.size) {
        ...
        for (const [i, p] of s.entries()) {
          const m = c.frames.find((A) => A.id === i);
          if (!m) {
            c.errors.push(`frameload ${i}: no existe esa barra`);
            continue;
          }
          const w = $.get(m.nI), u = $.get(m.nJ);
          if (w === void 0 || u === void 0) continue;
          const S = D[w], O = D[u], g = [
            O[0] - S[0],
            O[1] - S[1],
            O[2] - S[2]
      ...
      for (const s of c.shells) {
        ...
        for (const [m, w] of Ce) {
          const u = [
            ...
            1,
            2
          ].map((L) => S.reduce((A, b, C) => A + b * i[C][L], 0)), x = [
            0,
            1,
            2
          ].map((L) => O.reduce((A, b, C) => A + b * i[C][L], 0)), E = [
            g[1] * x[2] - g[2] * x[1],
            g[2] * x[0] - g[0] * x[2],
            g[0] * x[1] - g[1] * x[0]
          ], T = Math.hypot(E[0], E[1], E[2]);
          for (let L = 0; L < 4; L++) p[L] += u[L] * f * T;
        }
        for (let m = 0; m < 4; m++) {
          const w = M[m], u = _.get(w) ?? [
            0,
            0,
            0,
            0,
          ...
          u[2] += p[m], _.set(w, u), le.set(w, (le.get(w) ?? 0) + p[m]);
        }
      }
      if (c.selfWeight) {
        ...
        k.forEach((i, p) => {
          ...
          if (m && !f.has(p)) {
            if (i.length === 2) {
              const w = e.get(p) ?? 0, u = D[i[0]], S = D[i[1]], O = [
                S[0] - u[0],
                S[1] - u[1],
                S[2] - u[2]
              ];
              ...
              const x = j.get(p);
              if (x) {
                const W = Math.hypot(O[0], O[1]);
                W > 1e-9 && Math.abs(Math.atan2(Math.abs(O[2]), W)) * 180 / Math.PI < 20 && (g = Math.max(g - x[0] - x[1], 0));
              }
              const E = Math.hypot(O[0], O[1], O[2]), T = -w * m * 9.80665 * c.selfWeight, L = [
                O[0] / E,
                O[1] / E,
                O[2] / E
              ], A = g * g / 12, b = [
              ...
              const w = se.get(p) ?? 0, u = i.map((g) => D[g]);
              let S = 0;
              for (let g = 1; g < 3; g++) {
                const x = [
                  u[g][0] - u[0][0],
                ...
                S += Math.hypot(T[0], T[1], T[2]) / 2;
              }
              const O = S * w * m * 9.80665 * c.selfWeight;
      ...
      if (c.edgeEtabs) {
        ...
        k.forEach((m, w) => {
          if (m.length === 3 || m.length === 4) {
            i.push(w);
            for (const u of m) M.add(u);
          }
        });
        let p = 0;
        for (const m of i) {
          const w = k[m], u = w.map((O) => D[O]), S = [
            0,
            1,
            2
          ].map((O) => [
            Math.min(...u.map((g) => g[O])),
            Math.max(...u.map((g) => g[O]))
          ]);
          for (let O = 0; O < D.length; O++) {
            if (w.includes(O)) continue;
            const g = D[O], x = s ? f * Math.max(S[0][1] - S[0][0], S[1][1] - S[1][0], S[2][1] - S[2][0]) : 1e-6;
            if (g[0] < S[0][0] - x || g[0] > S[0][1] + x || g[1] < S[1][0] - x || g[1] > S[1][1] + x || g[2] < S[2][0] - x || g[2] > S[2][1] + x) continue;
            let E = false;
            for (let L = 0; L < w.length && !E; L++) {
              const A = u[L], b = u[(L + 1) % w.length], C = [
                b[0] - A[0],
                b[1] - A[1],
                b[2] - A[2]
              ], W = C[0] * C[0] + C[1] * C[1] + C[2] * C[2];
              if (W < 1e-24) continue;
              const N = [
                g[0] - A[0],
                g[1] - A[1],
                g[2] - A[2]
              ], H = (N[0] * C[0] + N[1] * C[1] + N[2] * C[2]) / W;
              ...
              Math.hypot(ue[0], ue[1], ue[2]) <= f * Math.sqrt(W) && (E = true);
            }
            !E || !(s || k.some((L, A) => A !== m && L.includes(O))) || (ne.push({
              node: -(m + 1),
              dof: s ? -4 : -2,
              k: O
            }), p++);
      ...
      for (const s of c.solids) {
        ...
        const M = k.length;
        k.push(f), o.set(M, s.E), U.set(M, s.nu), z.set(M, s.E / (2 * (1 + s.nu))), l.set(M, s.rho), ee.push(M);
      }
      ...
      const he = /* @__PURE__ */ new Map(), re = /* @__PURE__ */ new Map(), Ie = /* @__PURE__ */ new Map(), me = /* @__PURE__ */ new Map(), ae = /* @__PURE__ */ new Map();
      for (const s of c.shells) {
        const f = B.get(s.id);
        ...
        m ? (he.set(f, m[0]), re.set(f, m[1])) : c.deckSecs.has(s.id) && (he.set(f, 1), re.set(f, 0));
      }
      if (t.elementInputs.val = {
        ...
        frameLoadsPorPatron: (() => {
          ...
          k.forEach((M, i) => {
            if (M.length === 2) {
              const p = c.frames.find((m) => $.get(m.nI) === M[0] && $.get(m.nJ) === M[1]);
              p && f.set(p.id, i);
            }
          });
        ...
        const s = o.get(ee[0]) ?? 25e6, f = U.get(ee[0]) ?? 0.2;
        ee.some((w) => Math.abs((o.get(w) ?? s) - s) > 1e-9 * s || Math.abs((U.get(w) ?? f) - f) > 1e-12) && c.errors.push("hex: hex8Solve lleva UN material; los solidos tienen E o nu distintos y se usa el del primero");
        const M = /* @__PURE__ */ new Map();
        for (const [w, u] of t.nodeInputs.val.supports ?? []) M.set(w, [
          !!u[0],
          !!u[1],
          !!u[2]
        ]);
        ...
        p.displacements.forEach(([w, u, S], O) => m.set(O, [
          w,
          u,
          S,
          0,
        ...
        if (c.areaSprings.length > 0) try {
          ...
          }, t.analyzeOutputs.val = M, console.log(`[CLI Modeler] presi\xF3n Winkler: ${i.size} shells, \u03C3 ${p.toFixed(0)}..${m.toFixed(0)} kN/m\xB2`));
        } catch (f) {
        ...
        if (ee.length > 0) try {
          ...
          for (const p of ee) {
            const m = k[p], w = m.map((O) => D[O]), u = m.flatMap((O) => {
              const g = f.get(O) ?? [
                0,
                0,
                0
            ...
            }), S = Ze(w, o.get(p) ?? 25e6, U.get(p) ?? 0.2, u, c.solidIncompatible);
            M.set(p, S.stress), i.set(p, S.vonMises);
          }
          t.analyzeOutputs.val = {
            ...t.analyzeOutputs.val ?? {},
            solidStress: M,
