...
import { l as $t, a0 as xi, a1 as ql, v as we, a2 as Wl, D as Pt, M as pt, B as Ie, F as Mt, a3 as Zl, q as bt, a4 as Kl, a5 as Gl, a6 as ya, h as wa, a7 as xa, Y as Hs, a8 as Ai, a9 as Ti, $ as qn, H as ft, L as is, b as mt, f as gt, p as Qa, c as Jl, aa as Ql, V as te, I as an, J as Bt, ab as ho, u as Hi, d as Vt, a as uo, A as el, k as Ri, t as er, s as Xn, P as tr, ac as zi, n as po, m as sr, w as Ns, X as Pn, E as ba, S as vn, ad as fo, ae as Rn, g as va, i as _a, j as ka, C as Ca, x as nr, y as ir, z as or, W as ar, G as Sa, af as bi, N as mo, O as lr, ag as rr, _ as cr } from "./Text-Br8EG2up.js";
import { P as tl } from "./tweakpane-BXg6ZhiP.js";
let Ut, Tc, Md, Lc, vd, xd, ur, Ra, rc, cc, wd, dc, xc, Ii, Cs, Oo, $d, Cd, kd, _d, Sd, bd;
let __tla = (async () => {
  class sl {
    constructor(e, t = 32) {
  ...
  @5<45<30  \u2192 esf\xE9rica 3D: 5m, azimuth 45\xB0, elevaci\xF3n 30\xB0`, Ee.style.cssText = [
    ...
    window.__hekatanRevolveSelection = (u, g, w, m = 360) => {
      ...
      const de = (Re, Xe) => {
        ...
        for (let xt = 0, St = B.length - 1; xt < B.length; St = xt++) {
          ...
          fs > Xe != Js > Xe && Re < (xn - Ot) * (Xe - fs) / (Js - fs) + Ot && (We = !We);
        }
        return We;
    ...
    const vt = new ft(), El = new pt(new vn(0.01, 12, 12), new gt({
      color: 16777215,
      transparent: true,
      opacity: 0.95
    })), Jo = new pt(new vn(0.015, 12, 12), new gt({
      color: 16498468,
      transparent: true,
      opacity: 0.2,
      depthWrite: false
        ...
        if (T.length > 0 && N[T[T.length - 1]]) {
          ...
          if (!j && !Z && window.__hekatanAxisSnap !== false) {
            ...
            for (const [xt, St] of Re) {
              const Ot = Xe(He.clone().addScaledVector(St, -Je)), fs = Xe(He.clone().addScaledVector(St, Je)), xn = fs.x - Ot.x, Js = fs.y - Ot.y, Hl = et - Ot.x, Xl = Le - Ot.y, Yl = xn * xn + Js * Js || 1;
              let wi = (Hl * xn + Xl * Js) / Yl;
              wi = Math.max(0, Math.min(1, wi));
              const pa = Math.hypot(et - (Ot.x + wi * xn), Le - (Ot.y + wi * Js));
          ...
          ms.visible = ne, ne || Vo(null), ne && (Ts(qs, L, "xy", oe), Ts(dn, L, "xz", oe), Ts(hn, L, "yz", oe), As(cs, L, "xy", oe), As(gs, L, "xz", oe), As(ys, L, "yz", oe));
    ...
    const wn = [], gi = [], Ll = () => {
      ...
      gi.push(no()), aa(u), ye(`\u21B6 Deshacer \u2014 quedan ${wn.length}`);
    }, la = () => {
      const u = gi.pop();
      if (!u) {
      ...
      wn.push(no()), aa(u), ye(`\u21B7 Rehacer \u2014 quedan ${gi.length}`);
    };
...
My \u2248 ${Ft(ms * Be)} ${Ze.forceUnit}\xB7m`), Math.abs(un) > 1e-6 && (Pe += `
Mz \u2248 ${Ft(un * Be)} ${Ze.forceUnit}\xB7m`);
              }
            }
          }
        }
  ...
  wd = function({ mesh: l, settingsObj: e, drawingObj: t, objects3D: s, solids: n }) {
...
`;
  ...
  let Ro, Oa, Li, Va, ul, Vs, pl, bc, rn, Wn, Zn, Do, vc, _o, Nn, Ba, Ha, Qs, Xa, Ya, fl, _c, Kn, ja, nn;
  Ro = globalThis;
  Oa = (l) => l;
  Li = Ro.trustedTypes;
...
\f\r"'\`<>=]|("|')|))|$)`, "g");
  ...
  vd = function({ buttons: l, clickedButton: e, author: t, sourceCode: s }) {
    const n = document.createElement("div");
    function i() {
      return cr() === "dark" ? "\u2600" : "\u263E";
    }
...
O cambi\xE1 a otro provider (Gemini/Groq/OpenRouter) que solo requiere API key.`);
  ...
  let Nc, Oc;
  Md = `Eres un asistente experto en estructuras y FEM que ayuda al usuario a generar modelos
...
`).filter((t) => t.trim() && !t.trim().startsWith("#")).length} l\xEDneas .heks)` : e.length > 90 ? e.slice(0, 87) + "\u2026" : e;
  ...
  async function Io() {
    ...
    rs.value = "", os("user", l);
    const i = tn;
    tn = [], Pi(), Hn.push(sd(l, i)), on = new AbortController(), sn.textContent = "\u25A0 Parar";
    ...
    try {
      for (let p = 0; p < Ka; p++) {
        const f = await ld(t, n, s, on.signal);
        let x = f.tool_calls ?? [];
...
`).map((s) => s.trim()).find(Boolean) ?? "").replace(/[*`$]/g, "").slice(0, 60) || "Hoja \xB7 Hekatan LISP";
  ...
  function pd() {
    ...
    i.value = en.some((V) => V.id === b) ? b : "gemini", en.some((V) => V.id === b) || ud().then((V) => {
  ...
  Oo = function(l) {
    Yi(), as || (as = pd()), as.style.display = "flex", No(), setTimeout(() => {
      try {
  ...
  $d = function() {
    ...
    }, document.body.appendChild(l), md();
  };
  function md() {
