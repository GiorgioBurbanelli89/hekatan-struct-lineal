...
import { v as y } from "./Text-BE1eWO-3.js";
import { p as Y, __tla as __tla_0 } from "./didacticCpp-Czy7NlhT.js";
import "./tweakpane-BXg6ZhiP.js";
import { a as K, __tla as __tla_1 } from "./aiAgent-BdxJjwS0.js";
Promise.all([
  (() => {
    try {
      return __tla_0;
  ...
  let D = null;
  function O(l, s, i, t, g, w, R = 50) {
    let f = 0;
    for (let o = 1; o <= R; o += 2) for (let M = 1; M <= R; M += 2) {
      const E = (o * Math.PI / l) ** 2 + (M * Math.PI / s) ** 2, F = 16 * i / (Math.PI ** 2 * o * M);
      f += F / (t * E ** 2) * Math.sin(o * Math.PI * g / l) * Math.sin(M * Math.PI * w / s);
    }
    return f;
  }
  function X() {
  ...
  function U(l, s, i) {
    const t = l.getContext("2d"), g = l.width, w = l.height;
    if (t.clearRect(0, 0, g, w), s.nodeResults.length === 0) return;
    const R = n.Lx.val, f = n.Ly.val, o = 60, M = g - 2 * o, E = w - 2 * o, F = M / R, H = E / f, C = Math.min(F, H), q = o + (M - R * C) / 2, N = o + (E - f * C) / 2;
    function P(e, a) {
      return [
        q + e * C,
        N + (f - a) * C
    ...
    let x = [], G = false;
    if (i === "w") x = s.nodeResults.map((e) => e.w);
    else if (i === "bx") x = s.nodeResults.map((e) => e.bx);
    else if (i === "by") x = s.nodeResults.map((e) => e.by);
    else {
      G = true;
      const e = i;
      x = s.elementResults.map((a) => a[e]);
    }
    ...
    for (const e of s.elementResults) {
  ...
  function j() {
    const { div: l, h2: s, h3: i, p: t, span: g, label: w, input: R, select: f, option: o, button: M, canvas: E, table: F, tr: H, td: C, th: q, tbody: N, thead: P, br: x, strong: G } = y.tags, S = E({
      width: 700,
      height: 600,
    ...
    return y.derive(() => {
      ...
      try {
        const I = performance.now(), d = X(), k = performance.now() - I;
        U(S, d, T);
        const A = a * e ** 3 / (12 * (1 - p ** 2)), $ = Q === "simply-supported" ? O(r, u, Math.abs(v), A, r / 2, u / 2) : NaN, B = Math.abs(d.centerW ?? d.maxW), V = isNaN($) ? NaN : Math.abs((B - $) / $ * 100);
...
`, c += `Qy_max  = ${d.maxQy.toExponential(4)}`, b.textContent = c;
    ...
    }, s("Placa Q4 \u2014 Mindlin-Reissner (WASM)"), t({
      style: "color:#555; font-size:13px"
    }, "4-node isoparametric plate element with Selective Reduced Integration (SRI). ", "Bending: 2\xD72 Gauss, Shear: 1\xD71 Gauss. Converges to Kirchhoff theory for thin plates."), l({
      style: "display:grid; grid-template-columns:1fr 1fr; gap:16px"
    }, l(i("Geometr\xEDa"), m("Lx (m)", n.Lx, 1, 30, 1), m("Ly (m)", n.Ly, 1, 30, 1), m("nx", n.nx, 2, 64, 2), m("ny", n.ny, 2, 64, 2), i("Material"), m("t (m)", n.thickness, 0.05, 2, 0.05), m("E (kN/m\xB2)", n.E, 1e6, 1e8, 1e6), m("\u03BD", n.nu, 0, 0.45, 0.05)), l(i("Cargas"), m("q (kN/m\xB2)", n.pressure, -50, 0, 1), i("Condiciones de Borde"), l({
      style: "margin:4px 0"
    }, z), i("Resultado"), l({
      style: "margin:4px 0"
    }, W))), x(), S, b);
