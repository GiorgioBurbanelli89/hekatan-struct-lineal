function O() {
  ...
  `;
  {
    let s = false, f = 0, g = 0, r = 0, u = 0;
    t.addEventListener("mousedown", (a) => {
      const n = a.target;
      if (!n.closest("#modal-header") || n.closest("button")) return;
      s = true;
      const l = t.getBoundingClientRect();
      f = a.clientX, g = a.clientY, r = l.left, u = l.top, t.style.bottom = "auto", t.style.right = "auto", t.style.left = `${l.left}px`, t.style.top = `${l.top}px`, a.preventDefault();
    }), document.addEventListener("mousemove", (a) => {
      if (!s) return;
      let n = r + (a.clientX - f), l = u + (a.clientY - g);
...
<div id="modal-body" style="padding:0 12px 10px 12px;">
  ...
  <div style="color:#fa0; font-size:11px; line-height:1.5">${e}</div>
    ...
    const g = ["Ux", "Uy", "Uz", "Rx", "Ry", "Rz"], r = [0, 0, 0, 0, 0, 0], u = s.frequencies.length;
    let a = -1, n = -1, l = -1, w = 0, S = 0;
    {
      const e = [0, 0, 0, 0, 0, 0];
      for (let o = 0; o < u; o++) {
        const b = ((_b = s.massParticipation) == null ? void 0 : _b[o]) || [0, 0, 0, 0, 0, 0];
        for (let p = 0; p < 6; p++) e[p] += b[p];
    ...
    const N = (() => {
      ...
      return l > 0 ? `<span style="color:#0f0" title="Masa participativa \u2265 90 % en X e Y (NEC-15 \xA76.2.2 / ASCE 7-22 \xA712.9.1.1)">\u2713 \u2265 90 % en X e Y al modo ${l} de ${u} \xB7 \u03A3Ux ${(w * 100).toFixed(1)} % \xB7 \u03A3Uy ${(S * 100).toFixed(1)} %</span>` : a > 0 && n < 0 ? `<span style="color:#fa0">\u26A0 FALTAN MODOS EN Y \u2014 \u03A3Uy=${(S * 100).toFixed(1)} % en ${u} modos (faltan ${e(S)} para el 90 % que exige NEC-15 \xA76.2.2). X cumple en el modo ${a}. Sub\xED \xABN\xB0 de modos\xBB en Settings \u25B8 \u26A1 Modal + Animaci\xF3n.</span>` : n > 0 && a < 0 ? `<span style="color:#fa0">\u26A0 FALTAN MODOS EN X \u2014 \u03A3Ux=${(w * 100).toFixed(1)} % en ${u} modos (faltan ${e(w)} para el 90 % que exige NEC-15 \xA76.2.2). Y cumple en el modo ${n}. Sub\xED \xABN\xB0 de modos\xBB en Settings \u25B8 \u26A1 Modal + Animaci\xF3n.</span>` : `<span style="color:#f44">\u2717 FALTAN MODOS EN AMBAS DIRECCIONES \u2014 \u03A3Ux=${(w * 100).toFixed(1)} % \xB7 \u03A3Uy=${(S * 100).toFixed(1)} % en ${u} modos. NEC-15 \xA76.2.2 exige \u2265 90 %: el cortante din\xE1mico sale bajo y el control Vdin/Vest no es representativo. Sub\xED \xABN\xB0 de modos\xBB en Settings \u25B8 \u26A1 Modal + Animaci\xF3n.</span>`;
    })(), C = (e, o, b, p, y) => `<button id="${e}" title="${b}" style="padding:3px 9px; font-size:10px;
        cursor:pointer; background:${p}; color:#fff; border:1px solid ${y};
  ...
  <div style="display:flex; gap:4px; margin-left:12px; flex-shrink:0;">
    ...
    c += '<div id="modal-body" style="padding:0 12px 10px 12px;">', c += `<div style="padding:2px 0 4px 0; font-weight:bold; font-size:11px; line-height:1.4; width:0; min-width:100%">${N}</div>`, c += `<table style="border-collapse:collapse; color:#0f0; font-size:10px; margin-top:2px; white-space:nowrap">
<tr style="color:#ff0; border-bottom:1px solid #ff03">
  ...
  <th style="padding:1px 4px">\u03C9 (rad/s)</th>`;
    for (const e of g) c += `<th style="padding:1px 4px">${e}</th>`;
  ...
  <th style="padding:1px 4px; color:#fff">Tipo</th></tr>`;
    ...
    if (s.frequencies.forEach((e, o) => {
      var _a2;
      const b = e > 0 ? 1 / e : 0, p = e * 2 * Math.PI, y = e >= 500, m = ((_a2 = s.massParticipation) == null ? void 0 : _a2[o]) || [0, 0, 0, 0, 0, 0];
      for (let i = 0; i < 6; i++) r[i] += m[i];
      let h = 0, d = m[0];
      for (let i = 1; i < 6; i++) m[i] > d && (d = m[i], h = i);
      const z = y ? "masa faltante (r\xEDgida)" : d < 0.05 ? "\u2014" : `${g[h]} (${(d * 100).toFixed(0)} %)`, R = h === 0 || h === 1 ? "#0f0" : h === 5 ? "#0ff" : h === 2 ? "#fa0" : "#888", E = o + 1 === a, v = o + 1 === n, F = o + 1 === l;
      c += `<tr style="border-bottom:1px solid #fff1; ${y ? "background:rgba(0,180,255,0.12);" : F ? "background:rgba(0,255,0,0.12);" : E || v ? "background:rgba(255,200,0,0.1);" : ""}">
  <td style="padding:1px 4px; text-align:center">${y ? "MF" : o + 1 + (F ? " \u2605" : "")}</td>
  <td style="padding:1px 4px; text-align:right">${y ? "r\xEDgido" : e.toFixed(4)}</td>
  <td style="padding:1px 4px; text-align:right">${y ? "\u22480" : b.toFixed(4)}</td>
  <td style="padding:1px 4px; text-align:right">${y ? "\u2014" : p.toFixed(2)}</td>`;
      for (let i = 0; i < 6; i++) {
...
<div style="margin-top:6px; font-size:10px; color:#888;">
  \u2605 = primer modo donde \u03A3Ux y \u03A3Uy \u2265 90 %  \xB7  Tipos: <span style="color:#0f0">Ux/Uy</span>=lateral \xB7 <span style="color:#0ff">Rz</span>=torsional \xB7 <span style="color:#fa0">Uz</span>=vertical (no relevante para sismo)
</div>`, c += "</div>", t.innerHTML = c, !L && !x && (L = true, t.style.width = "max-content", t.style.height = "auto"), M) {
    ...
    (_c = t.querySelector("#modal-minimize")) == null ? void 0 : _c.addEventListener("click", () => {
      M = !M;
      const e = t.querySelector("#modal-body"), o = t.querySelector("#modal-minimize");
      M ? (e.style.display = "none", o.textContent = "\u25A2", o.title = "Restaurar") : (e.style.display = "block", o.textContent = "\u25AC", o.title = "Minimizar");
      ...
      const e = [];
      e.push(`Modal Analysis	${f.title}`), e.push(N.replace(/<[^>]+>/g, "").trim()), e.push(""), e.push(["Modo", "Freq (Hz)", "Periodo (s)", "w (rad/s)", ...g, "SUx", "SUy", "SRx", "SRy", "SRz", "Tipo"].join("	"));
      ...
      s.frequencies.forEach((d, z) => {
        ...
        const T = A < 0.05 ? "\u2014" : `${g[F]} (${(A * 100).toFixed(0)}%)`, U = [String(z + 1), d.toFixed(4), R.toFixed(4), E.toFixed(2), ...v.map((i) => (i * 100).toFixed(1)), (o[0] * 100).toFixed(1), (o[1] * 100).toFixed(1), (o[3] * 100).toFixed(1), (o[4] * 100).toFixed(1), (o[5] * 100).toFixed(1), T];
        e.push(U.join("	")), b.push("<tr>" + U.map((i) => `<td>${i}</td>`).join("") + "</tr>");
      });
      const p = e.join(`
`), y = `<table border="1" cellspacing="0" cellpadding="3">
<caption>Modal Analysis \u2014 ${f.title}</caption>
<tr>${["Modo", "Freq (Hz)", "Periodo (s)", "w (rad/s)", ...g, "SUx", "SUy", "SRx", "SRy", "SRz", "Tipo"].map((d) => `<th>${d}</th>`).join("")}</tr>
${b.join(`
`)}</table>`, m = t.querySelector("#modal-copy"), h = (d) => {
      ...
      (async () => {
        var _a2;
        try {
          typeof ClipboardItem < "u" && ((_a2 = navigator.clipboard) == null ? void 0 : _a2.write) ? await navigator.clipboard.write([new ClipboardItem({ "text/plain": new Blob([p], { type: "text/plain" }), "text/html": new Blob([y], { type: "text/html" }) })]) : await navigator.clipboard.writeText(p), h(true);
        } catch {
          try {
            await navigator.clipboard.writeText(p), h(true);
          } catch {
            h(false);
          }
        }
