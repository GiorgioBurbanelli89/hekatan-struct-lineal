...
function wt(m) {
  ...
  let C;
  for (; (C = R.exec(m)) !== null; ) p.set(C[1], C[2] !== void 0 ? C[2] : C[3]);
  return p;
}
function os(m) {
  const p = m.split(/\r?\n/);
  return p.some((C) => C.trim().startsWith("TABLE:")) ? Xt(p) : Kt(p);
}
function Xt(m) {
  ...
  for (const y of p) {
    ...
    switch (v) {
      ...
      case "FRAME SECTION PROPERTIES 01 - GENERAL": {
        ...
        S && A.set(S, { material: f.get("Material") || "", shape: f.get("Shape") || "Rectangular", D: B(f.get("t3")), B: B(f.get("t2")), TF: B(f.get("tf")), TW: B(f.get("tw")), A: B(f.get("Area")), Iz: B(f.get("I33")), Iy: B(f.get("I22")), J: B(f.get("TorsConst")), As2: B(f.get("AS2")), As3: B(f.get("AS3")) });
        break;
      }
      ...
      case "JOINT COORDINATES": {
        const S = f.get("Joint");
        if (S) {
          const b = B(f.get("XorR")), I = B(f.get("Y")), l = B(f.get("Z"));
          V.set(S, [b, I, l]);
        }
        break;
      }
      ...
      case "FRAME LOADS - DISTRIBUTED": {
        const S = f.get("Frame"), b = f.get("Dir"), I = B(f.get("FOverLA"));
        if (S && b && I) {
  ...
  return Gt(C, $, H, A, Y, V, q, K, te, ie, ce, ne, oe, re, Se, Me, i, d, D, W, ee, se);
}
function Kt(m) {
  const p = { force: "KN", length: "m" };
  let R = "UX,UY,UZ,RX,RY,RZ";
  const C = /* @__PURE__ */ new Map(), $ = /* @__PURE__ */ new Map(), H = /* @__PURE__ */ new Map(), W = /* @__PURE__ */ new Map(), ee = [], A = [], Y = /* @__PURE__ */ new Map(), V = [], q = /* @__PURE__ */ new Map(), K = /* @__PURE__ */ new Map(), te = /* @__PURE__ */ new Map(), se = /* @__PURE__ */ new Map(), ie = [], ce = /* @__PURE__ */ new Map(), ne = /* @__PURE__ */ new Map();
  let oe = "", re = "";
  for (const i of m) {
    const d = i.trim();
    ...
    switch (oe) {
      ...
      case "MATERIAL": {
        ...
        if (y) re = y, C.set(y, { E: 0, nu: 0, G: 0 });
          ...
          const N = C.get(re), f = D.get("E");
          f && (N.E = B(f));
          const S = D.get("U");
          S && (N.nu = B(S)), N.G = N.E / (2 * (1 + N.nu));
          const b = D.get("M");
          b && (N.density = B(b));
        }
        break;
  ...
  return Gt(p, R, C, $, H, W, ee, A, Y, /* @__PURE__ */ new Map(), /* @__PURE__ */ new Map(), V, q, K, te, se, ie, ce, ne);
}
function Gt(m, p, R, C, $, H, W, ee, A, Y, V, q, K = /* @__PURE__ */ new Map(), te = /* @__PURE__ */ new Map(), se = /* @__PURE__ */ new Map(), ie = /* @__PURE__ */ new Map(), ce = [], ne = /* @__PURE__ */ new Map(), oe = /* @__PURE__ */ new Map(), re, Se, Me = /* @__PURE__ */ new Map()) {
  var _a, _b;
  ...
  for (let o = 0; o < v.length; o++) {
    const E = N.get(o), u = E ? C.get(E) : null, U = E ? $.get(E) : null;
    if (u || v[o].length === 2) {
      const O = u || { material: "", A: 0, Iz: 0, Iy: 0, J: 0, D: 0.3, B: 0.3, shape: "Rectangular" }, z = R.get(O.material) || n, F = z.E || n.E, _ = z.nu || 0.3, j = z.G || F / (2 * (1 + _));
      I.elasticities.set(o, F), I.shearModuli.set(o, j), I.areas.set(o, O.A || O.D * O.B), I.momentsOfInertiaZ.set(o, O.Iz || O.B * O.D ** 3 / 12), I.momentsOfInertiaY.set(o, O.Iy || O.D * O.B ** 3 / 12), I.torsionalConstants.set(o, O.J || 0), I.densities.set(o, z.density || 0), O.As2 && (I.shearAreasZ ?? (I.shearAreasZ = /* @__PURE__ */ new Map()), I.shearAreasZ.set(o, O.As2)), O.As3 && (I.shearAreasY ?? (I.shearAreasY = /* @__PURE__ */ new Map()), I.shearAreasY.set(o, O.As3));
      const k = K.get(y[o]);
      ...
      const Z = E ? re == null ? void 0 : re.get(E) : void 0;
      if (Z && Z.t > 0 && (Z.b > 0 && Z.h > 0 || (Z.D ?? 0) > 0)) {
        const le = E ? Se == null ? void 0 : Se.get(E) : void 0, he = le && ((_b = R.get(le.mat)) == null ? void 0 : _b.E) || 0;
        l.set(o, Z.D ? { type: "CFT", d: Z.D, tw: Z.t, name: E, ...he > 0 ? { fillE: he } : {} } : { type: "CFT", b: Z.b, h: Z.h, tw: Z.t, name: E, ...he > 0 ? { fillE: he } : {} });
      }
    } else if (U) {
      const O = R.get(U.material) || n, z = O.E || n.E, F = O.nu || 0.2, _ = O.G || z / (2 * (1 + F));
      I.elasticities.set(o, z), I.shearModuli.set(o, _), I.thicknesses.set(o, U.thickness), I.poissonsRatios.set(o, F), I.plateFormulations ?? (I.plateFormulations = /* @__PURE__ */ new Map()), I.plateFormulations.set(o, /thin/i.test(U.type) ? 1 : 0);
  ...
  if (b.length) {
    let o = false;
    for (const E of b) {
      const u = ne.get(N.get(E) || ""), U = u && R.get(u.material) || n, O = U.E || n.E, z = U.nu || 0.2;
      I.elasticities.set(E, O), I.poissonsRatios.set(E, z), I.shearModuli.set(E, U.G || O / (2 * (1 + z))), I.densities.set(E, U.density || 0), (u == null ? void 0 : u.incomp) && (o = true);
    }
  ...
  }
  {
    ...
    for (const [E, u] of Me) {
      const U = d.get(E);
      U !== void 0 && u.forEach((O, z) => {
        O > 0 && o.push({ node: U, dof: z, k: O });
      });
    }
  ...
  for (const [o, E] of ie) {
    ...
    const U = D[v[u][0]], O = D[v[u][1]], z = [O[0] - U[0], O[1] - U[1], O[2] - U[2]], F = Math.hypot(z[0], z[1], z[2]);
    if (F < 1e-9) continue;
    const _ = [z[0] / F, z[1] / F, z[2] / F], j = F * F / 12, k = [_[1] * E[2] - _[2] * E[1], _[2] * E[0] - _[0] * E[2], _[0] * E[1] - _[1] * E[0]], Q = (Z, le) => {
      const he = c.loads.get(Z) || [0, 0, 0, 0, 0, 0];
      for (let ue = 0; ue < 6; ue++) he[ue] += le[ue];
      c.loads.set(Z, he);
    };
    Q(v[u][0], [E[0] * F / 2, E[1] * F / 2, E[2] * F / 2, j * k[0], j * k[1], j * k[2]]), Q(v[u][1], [E[0] * F / 2, E[1] * F / 2, E[2] * F / 2, -j * k[0], -j * k[1], -j * k[2]]);
  }
  for (const o of q) {
  ...
  return { units: m, dof: p, materials: R, frameSections: C, shellSections: $, nodes: D, nodeNames: i, nodeNameToIdx: d, elements: v, elementNames: y, elementSections: N, nodeInputs: c, elementInputs: I, sectionShapes: l, info: { nNodes: D.length, nFrames: f, nShells: S, title: `SAP2000 (${f} frames, ${S} shells)` } };
}
function ns(m) {
  var _a, _b, _c, _d, _e, _f, _g, _h, _i, _j, _k, _l, _m;
  const { nodes: p, elements: R, nodeInputs: C, elementInputs: $ } = m, H = { force: "KN", length: "m" };
  ...
  const W = m.title || "Awatif Model", ee = [], A = (l) => ee.push(l), Y = () => ee.push(" ");
  A(`File ${W}.$2k was saved on m/d/yy at h:mm:ss`), Y(), A('TABLE:  "ACTIVE DEGREES OF FREEDOM"'), A("   UX=Yes   UY=Yes   UZ=Yes   RX=Yes   RY=Yes   RZ=Yes"), Y();
  const V = [], q = (l) => {
  ...
  for (const l of V) {
    const n = ((_a = $.areas) == null ? void 0 : _a.get(l)) || 0, c = ((_b = $.momentsOfInertiaZ) == null ? void 0 : _b.get(l)) || 0, o = ((_c = $.momentsOfInertiaY) == null ? void 0 : _c.get(l)) || 0, E = ((_d = $.torsionalConstants) == null ? void 0 : _d.get(l)) || 0, u = ((_e = $.elasticities) == null ? void 0 : _e.get(l)) || 0, U = q(l).key, O = ((_f = $.shearAreasZ) == null ? void 0 : _f.get(l)) ?? 0, z = ((_g = $.shearAreasY) == null ? void 0 : _g.get(l)) ?? 0, F = (_h = $.sectionShapes) == null ? void 0 : _h.get(l);
    let _;
    ...
    if (m.cftAs !== "general" && (F == null ? void 0 : F.type) === "CFT" && u > 0 && (j || F.b > 0 && F.h > 0 && F.tw > 0 && F.tw < Math.min(F.b, F.h) / 2)) {
      const Z = j ? F.d - 2 * F.tw : 0, le = j ? 0 : F.b - 2 * F.tw, he = j ? 0 : F.h - 2 * F.tw, ue = j ? Math.PI * (F.d * F.d - Z * Z) / 4 : F.b * F.h - le * he, ge = j ? Math.PI * Z * Z / 4 : le * he, Ie = (F.fillE > 0 ? F.fillE / u : Math.max(0.01, Math.min(1, (n - ue) / ge))) * u, de = 0.2, Be = F.fillRho ?? 2.4, Le = `FILL_${Math.round(Ie)}_r${Be}`;
      ie.has(Le) || ie.set(Le, { E: Ie, nu: de, G: Ie / (2 * (1 + de)), rho: Be }), _ = j ? { b: F.d, h: F.d, t: F.tw, Ec: Ie, nuC: de, matFill: Le, D: F.d } : { b: F.b, h: F.h, t: F.tw, Ec: Ie, nuC: de, matFill: Le };
    }
    const k = `A${n.toPrecision(6)}_Iz${c.toPrecision(6)}_s${O.toPrecision(6)}_${z.toPrecision(6)}${_ ? _.D ? `_SDC${_.D}x${_.t}` : `_SD${_.b}x${_.h}x${_.t}` : ""}`;
    if (!se.has(k)) {
      let Z = 0.3, le = 0.3;
      n > 0 && c > 0 && (Z = Math.sqrt(12 * c / n), le = n / Z), se.set(k, { A: n, Iz: c, Iy: o, J: E, b: le, h: Z, matKey: U, As2: O > 0 ? O : n * 5 / 6, As3: z > 0 ? z : n * 5 / 6, sd: _ });
    }
  ...
  if (se.size > 0) {
    ...
    for (const [, n] of se) {
      if (l++, n.sd) {
        A(`   SectionName=SEC${l}   Material=${n.matKey}   Shape="SD Section"   Area=${P(n.A)}   TorsConst=${P(n.J)}   I33=${P(n.Iz)}   I22=${P(n.Iy)}   I23=0   AS2=${P(n.As2)}   AS3=${P(n.As3)} _`), A("        Color=Cyan   FromFile=No   AMod=1   A2Mod=1   A3Mod=1   JMod=1   I2Mod=1   I3Mod=1   MMod=1   WMod=1");
        continue;
      }
      A(`   SectionName=SEC${l}   Material=${n.matKey}   Shape=General   t3=${P(n.h)}   t2=${P(n.b)}   Area=${P(n.A)}   TorsConst=${P(n.J)}   I33=${P(n.Iz)}   I22=${P(n.Iy)}   I23=0   AS2=${P(n.As2)}   AS3=${P(n.As3)} _`), A("        Color=Blue   FromFile=No   AMod=1   A2Mod=1   A3Mod=1   JMod=1   I2Mod=1   I3Mod=1   MMod=1   WMod=1");
    }
    Y();
  }
  ...
  if (ne.length > 0) {
    ...
    if (l.length > 0) {
      ...
      for (const { sec: c, name: o } of l) {
        const E = c.sd;
        A(`   SectionName=${o}   ShapeName=TUBO   ShapeType="User Defined"   ShapeMat=${c.matKey}   ZOrder=1   FillColor=Gray4   XCenter=0   YCenter=0   Height=${P(E.h)}   Width=${P(E.b)}   FlngThick=${P(E.t)}   WebThick=${P(E.t)}   Rotation=0 _`), A('        CoreDim="Program Determined"   BCoreMajor=0   BCoreMinor=0   DCoreMajorPositive=0   DCoreMajorNegative=0   DCoreMinorPositive=0   DCoreMinorNegative=0');
      }
      Y();
    }
    if (n.length > 0) {
      ...
      for (const { sec: c, name: o } of n) {
        const E = c.sd;
        A(`   SectionName=${o}   ShapeName=TUBO   ShapeType="User Defined"   ShapeMat=${c.matKey}   ZOrder=1   FillColor=Gray4   XCenter=0   YCenter=0   OuterDiam=${P(E.D)}   WallThick=${P(E.t)}   CoreDim="Program Determined"   BCoreMajor=0   BCoreMinor=0 _`), A("        DCoreMajorPositive=0   DCoreMajorNegative=0   DCoreMinorPositive=0   DCoreMinorNegative=0");
      }
      Y();
    }
    if (l.length > 0) {
      ...
      for (const { sec: c, name: o } of l) {
        const E = c.sd;
        A(`   SectionName=${o}   ShapeName=RELLENO   ShapeMat=${E.matFill}   ZOrder=2   FillColor=Gray4   XCenter=0   YCenter=0   Height=${P(E.h - 2 * E.t)}   Width=${P(E.b - 2 * E.t)}   Rotation=0   Reinforcing=No   CoreDim="Program Determined"   BCoreMajor=0   BCoreMinor=0 _`), A("        DCoreMajorPositive=0   DCoreMajorNegative=0   DCoreMinorPositive=0   DCoreMinorNegative=0");
      }
      Y();
    }
    if (n.length > 0) {
      ...
      for (const { sec: c, name: o } of n) {
        const E = c.sd;
        A(`   SectionName=${o}   ShapeName=RELLENO   ShapeMat=${E.matFill}   ZOrder=2   FillColor=Gray4   XCenter=0   YCenter=0   Diameter=${P(E.D - 2 * E.t)}   Reinforcing=No   CoreDim="Program Determined"   BCoreMajor=0   DCoreMajorPositive=0`);
      }
      Y();
    }
  ...
  }
  {
    ...
    if (l.length > 0) {
      ...
      for (const n of l) A(`   Frame=${n + 1}   Angle=${P($.localAngles.get(n))}   AdvanceAxes=No`);
      Y();
    }
  }
  {
    ...
    if (n.length > 0) {
      ...
      for (const c of n) {
        const o = l.get(c);
        A(`   Frame=${c + 1}   Type=User   LengthI=${P(o[0])}   LengthJ=${P(o[1])}   RigidFactor=${P(o.length > 2 ? o[2] : 0)}`);
      }
      Y();
    }
  }
  ...
  if (K.length > 0) {
    ...
    if (n.length > 0) {
      ...
      for (const c of n) {
        const o = l(c);
        A(`   Area=${c + 1}   f11=${P(o[0])}   f22=${P(o[1])}   f12=${P(o[2])}   m11=${P(o[3])}   m22=${P(o[4])}   m12=${P(o[5])}   v13=${P(o[6])}   v23=${P(o[7])}   MassMod=1   WeightMod=1`);
      }
      Y();
    }
    if (A('TABLE:  "AREA SECTION PROPERTIES"'), oe) {
      const c = re, o = ((_l = c.layers[0]) == null ? void 0 : _l.material) || "MAT_DEFAULT";
      A(`   Section=${c.name}   Material=${o}   MatAngle=0   AreaType=Shell   Type=Shell-Layered   Thickness=${P(c.totalThickness)}   BendThick=${P(c.totalThickness)}   Color=Magenta`);
    } else {
      let c = 0;
      for (const [, o] of Se) {
        c++;
        ...
        A(`   Section=SSEC${c}   Material=${o.matKey}   MatAngle=0   AreaType=Shell   Type=${E}   DrillDOF=${u}   Thickness=${P(o.t)}   BendThick=${P(o.t)}   Color=Cyan`);
      }
    }
    if (Y(), oe) {
      ...
      for (const o of c.layers) {
        ...
        A(`   Section=${c.name}   LayerName=${o.name}   Distance=${P(o.distance)}   Thickness=${P(o.thickness)}   Type=Shell   NumIntPts=${u}   Material=${o.material}   MatAngle=${P(E * 180 / Math.PI)}   MatBehave=Directional   S11Opt=Linear   S22Opt=Linear   S12Opt=Linear`);
      }
      Y();
    }
  }
  ...
  for (let l = 0; l < p.length; l++) {
    const n = p[l];
    A(`   Joint=${l + 1}   CoordSys=GLOBAL   CoordType=Cartesian   XorR=${P(n[0])}   Y=${P(n[1])}   Z=${P(n[2])}   SpecialJt=No`);
  }
  if (Y(), C.supports && C.supports.size > 0) {
    A('TABLE:  "JOINT RESTRAINT ASSIGNMENTS"');
    for (const [l, n] of C.supports) {
  ...
  }
  {
    ...
    if (l.size > 0) {
      ...
      for (const [n, c] of [...l].sort((o, E) => o[0] - E[0])) A(`   Joint=${n + 1}   CoordSys=Global   U1=${P(c[0])}   U2=${P(c[1])}   U3=${P(c[2])}   R1=${P(c[3])}   R2=${P(c[4])}   R3=${P(c[5])}`);
      Y();
    }
  }
  ...
  if ((_m = C.loads) == null ? void 0 : _m.forEach((l, n) => S.set(n, [...l])), f && f.size > 0) {
    const l = (n, c) => {
    ...
    for (const [n, c] of f) {
      ...
      if (O < 1e-9) continue;
      const z = [U[0] / O, U[1] / O, U[2] / O], F = O * O / 12, _ = [z[1] * c[2] - z[2] * c[1], z[2] * c[0] - z[0] * c[2], z[0] * c[1] - z[1] * c[0]];
      l(o[0], [c[0] * O / 2, c[1] * O / 2, c[2] * O / 2, F * _[0], F * _[1], F * _[2]]), l(o[1], [c[0] * O / 2, c[1] * O / 2, c[2] * O / 2, -F * _[0], -F * _[1], -F * _[2]]);
    }
  }
  if (S.size > 0) {
    A('TABLE:  "JOINT LOADS - FORCE"');
    for (const [l, n] of S) n.some((c) => Math.abs(c) > 1e-12) && A(`   Joint=${l + 1}   LoadPat=DEAD   CoordSys=GLOBAL   F1=${P(n[0])}   F2=${P(n[1])}   F3=${P(n[2])}   M1=${P(n[3])}   M2=${P(n[4])}   M3=${P(n[5])}`);
    Y();
  }
  const b = $.frameLoads;
  if (b && b.size > 0) {
    ...
    for (const [l, n] of b) {
      ...
      ["X", "Y", "Z"].forEach((U, O) => {
        Math.abs(n[O]) < 1e-12 || A(`   Frame=${l + 1}   LoadPat=DEAD   CoordSys=GLOBAL   Type=Force   Dir=${U}   DistType=RelDist   RelDistA=0   RelDistB=1   AbsDistA=0   AbsDistB=${P(u)}   FOverLA=${P(n[O])}   FOverLB=${P(n[O])}`);
      });
    }
    Y();
  }
  ...
  for (const [l, n] of I) A(`   Material=${l}   UnitWeight=${P(n.rho * 9.81)}   UnitMass=${P(n.rho)}   E1=${P(n.E)}   G12=${P(n.G)}   U12=${P(n.nu)}   A1=9.9E-06`);
...
function Vt(m, p, R = 0.05) {
  const C = p.map(([$, H]) => `${(+$).toFixed(4)} ${(+H).toFixed(5)}`).join("  ");
  return [`  FUNCTION "${m}"  FUNCTYPE "SPECTRUM"  DAMPRATIO ${R}  SPECTYPE "USER"  `, `  FUNCTION "${m}"  TIMEVAL "${C}"  `];
}
function qt(m) {
  const { name: p, func: R, modalCase: C = "Modal", sfX: $ = 9.81, sfY: H = 9.81 } = m, W = [`  LOADCASE "${p}"  TYPE  "Response Spectrum"  MODALCASE  "${C}"  `];
  return $ && W.push(`  LOADCASE "${p}"  ACCEL  "U1"  FUNC  "${R}"  SF  ${$}  `), H && W.push(`  LOADCASE "${p}"  ACCEL  "U2"  FUNC  "${R}"  SF  ${H}  `), W;
}
function Ft(m) {
  const { name: p = "Modal", ritz: R = false, nModes: C = 12 } = m;
  return R ? [`  LOADCASE "${p}"  TYPE  "Modal - Ritz"  INITCOND  "PRESET"  `, `  LOADCASE "${p}"  MAXMODES  ${C} MINMODES  1 `, `  LOADCASE "${p}"  LOADTYPE  "Accel"  LOADNAME  "UX"  RITZMAXCYCLES  0 `, `  LOADCASE "${p}"  LOADTYPE  "Accel"  LOADNAME  "UY"  RITZMAXCYCLES  0 `, `  LOADCASE "${p}"  LOADTYPE  "Accel"  LOADNAME  "UZ"  RITZMAXCYCLES  0 `] : [`  LOADCASE "${p}"  TYPE  "Modal - Eigen"  INITCOND  "PRESET"  `, `  LOADCASE "${p}"  MAXMODES  ${C} MINMODES  1 EIGENSHIFTFREQ  0 EIGENCUTOFF  0 EIGENTOL  1E-09 `];
}
function as(m) {
  var _a;
...
`) ? `\r
` : `
`, C = m.split(/\r?\n/), $ = p.name ?? "NEC", H = Vt($, p.points, p.dampRatio ?? 0.05), W = p.modalCase ?? "Modal", ee = qt({ name: p.caseName ?? "Sismo NEC", func: $, modalCase: W, sfX: p.sfX, sfY: p.sfY });
  let A = [];
  ...
  if (p.modal) {
    ...
    A = Ft({ name: W, ritz: !!p.modal.ritz, nModes: p.modal.nModes });
  } else Y(new RegExp(`LOADCASE\\s+"${W}"\\s+TYPE\\s+"Modal`)) || (A = Ft({ name: W }));
  return bt(C, "FUNCTIONS", H), bt(C, "LOAD CASES", [...A, ...ee]), C.join(R);
}
function bt(m, p, R) {
  const C = m.findIndex((W) => W.trim() === `$ ${p}`);
  if (C >= 0) {
    m.splice(C + 1, 0, ...R);
    return;
  }
...
function ts(m) {
  ...
  const { nodes: p, elements: R, nodeInputs: C, elementInputs: $, title: H, units: W } = m, ee = m.shellLoads ?? $.shellSurfaceLoads;
  let A;
        ...
        if (r) {
          if (s.length === 2) {
            ...
            for (let w = 0; w < 4; w++) {
              ...
              M += J[1] * X[2] - J[2] * X[1], L += J[2] * X[0] - J[0] * X[2], x += J[0] * X[1] - J[1] * X[0];
            }
            const g = Math.hypot(M, L, x) / 2, G = h * g * r * 9.80665 * K;
  ...
  const O = 3, z = 0.5, F = /* @__PURE__ */ new Map();
  p.forEach((e) => {
    const t = d(e[2]);
    F.set(t, (F.get(t) ?? 0) + 1);
  });
  const _ = /* @__PURE__ */ new Set();
  p.forEach((e) => _.add(d(e[2])));
  const j = [..._].sort((e, t) => e - t);
  let k = j.filter((e) => (F.get(e) ?? 0) >= O);
  if (k.length > 1) {
    const e = [k[0]];
  ...
  const le = (e) => {
    ...
    for (let a = 0; a < k.length; a++) if (k[a] >= t) return { story: Z.get(k[a]), dz: d(k[a] - t) };
    const s = k[k.length - 1];
    return { story: Z.get(s), dz: d(s - t) };
  };
  ...
  for (const e of _e) {
    ...
    if (a) {
      ...
      i.push(`  MATERIAL  "${r}"  FY ${d(I(G))}  FU ${d(I(w))}  FYE ${d(I(G * 1.1))}  FUE ${d(I(w * 1.1))}`);
  ...
  R.forEach((e, t) => {
    ...
    let g = (s == null ? void 0 : s.type) || "rect", G = (s == null ? void 0 : s.h) ?? 0, w = (s == null ? void 0 : s.b) ?? 0, J = (s == null ? void 0 : s.d) ?? 0;
    const X = (s == null ? void 0 : s.tf) ?? 0, ae = (s == null ? void 0 : s.tw) ?? 0;
    if (!s && G <= 0 && w <= 0 && J <= 0 && T > 0 && M > 0 && L > 0) {
      const Te = (_g2 = $.cantos) == null ? void 0 : _g2.get(t), Ge = (_h = $.anchos) == null ? void 0 : _h.get(t);
      G = Te && Te > 0 ? Te : Math.sqrt(12 * M / T), w = Ge && Ge > 0 ? Ge : T / G, (!isFinite(G) || G < Ne) && (G = Ne), (!isFinite(w) || w < Ne) && (w = Ne), g = "general";
    } else G <= 0 && w <= 0 && J <= 0 && T > 0 && (M > 0 ? (G = Math.sqrt(12 * M / T), w = T / G) : G = w = Math.sqrt(T), (!isFinite(G) || G < Ne) && (G = Ne), (!isFinite(w) || w < Ne) && (w = Ne), g = "rect");
    G <= 0 && w <= 0 && J <= 0 && (G = 0.3, w = 0.3, g = "rect");
    const je = (s == null ? void 0 : s.name) ? `NAME_${s.name}` : `${g}_${d(G)}_${d(w)}_${d(J)}_${d(X)}_${d(ae)}_${r}`;
    (s == null ? void 0 : s.name) && !ke.has(je) && ke.set(je, s.name);
    let fe = ke.get(je);
    if (!fe) {
      const Te = h ? "S" : "C";
      g === "general" ? fe = `${Te}_G${Ye.size + 1}` : g === "rect" ? fe = `${Te}_R${Math.round(w * 100)}x${Math.round(G * 100)}` : g === "circ" ? fe = `${Te}_C_D${Math.round(J * 100)}` : g === "I" ? fe = `${Te}_I${Math.round(G * 100)}x${Math.round(w * 100)}` : g === "HSS" ? fe = `${Te}_HSS${Math.round(w * 100)}x${Math.round(G * 100)}x${Math.round(ae * 1e3)}` : fe = `${Te}_Sec${Ye.size + 1}`, ke.set(je, fe);
    }
    if (Ze.set(t, fe), Ye.has(fe)) return;
    Ye.add(fe);
    const Je = lt.get(t);
    if (g === "CFT" && Je && J > 0 && ae > 0 && !(G > 0 && w > 0)) {
      i.push(`  FRAMESECTION  "${fe}"  MATERIAL "${r}"  SHAPE "Filled Steel Pipe"  D ${d(N(J))} T ${d(N(ae))} FILLMATERIAL "${Je}"`);
      return;
    }
    if (g === "CFT" && Je && G > 0 && w > 0 && ae > 0) {
      i.push(`  FRAMESECTION  "${fe}"  MATERIAL "${r}"  SHAPE "Filled Steel Tube"  D ${d(N(G))} B ${d(N(w))} TF ${d(N(ae))} TW ${d(N(ae))} FILLMATERIAL "${Je}"`);
      return;
    }
    ...
    if (Ae === "General") {
      ...
      Oe += `  D ${d(N(G))} B ${d(N(w))} AREA ${D(T * 1e6)} AS2 ${D(Te * 1e6)} AS3 ${D(Ge * 1e6)} I33 ${D(M * 1e12)} I22 ${D(L * 1e12)} TORSION ${D((x || M + L) * 1e12)} S33POS ${D(2 * M / G * 1e9)} S33NEG ${D(2 * M / G * 1e9)} S22POS ${D(2 * L / w * 1e9)} S22NEG ${D(2 * L / w * 1e9)} Z33 ${D(2 * M / G * 1e9)} Z22 ${D(2 * L / w * 1e9)} R33 ${D(Math.sqrt(M / T) * 1e3)} R22 ${D(Math.sqrt(L / T) * 1e3)} `, i.push(Oe);
      return;
    }
    G && (Oe += `  D ${d(N(G))}`), w && (Oe += `  B ${d(N(w))}`), J && !G && (Oe += `  D ${d(N(J))}`), X && (Oe += `  TF ${d(N(X))}`), ae && (Oe += `  TW ${d(N(ae))}`), i.push(Oe);
  }), i.push("");
  ...
  Ke.forEach((e, t) => {
    ...
    e.elemIndices.forEach((h, T) => {
      ...
      const [T, M] = R[h[0]], L = p[T][2] <= p[M][2] ? T : M, [x, g] = R[h[h.length - 1]], G = p[x][2] >= p[g][2] ? x : g;
      ve(pe(G).story) - ve(pe(L).story);
      let w = `C${t + 1}`;
      for (let J = 1; ; J++) {
        const X = i.length;
        ht(w, e.type, L, G, e.secName, s, h.length);
  ...
  const Yt = !!(C.diaphragms && [...C.diaphragms.values()].some((e) => e !== 0)), At = m.diaphragm ?? "auto", qe = At === "d1" || At === "auto" && Yt, Fe = /* @__PURE__ */ new Set();
  ...
  }), Pe === "manual" && C.loads && C.loads.forEach((e, t) => {
    const [s, a, r] = oe(t, e);
  ...
  Tt == null ? void 0 : Tt.forEach((e) => e.cells.forEach((t) => Mt.add(t))), R.forEach((e, t) => {
    if (e.length === 4 || e.length === 3) {
      const s = p[e[0]], a = p[e[1]], r = p[e[2]], h = [a[0] - s[0], a[1] - s[1], a[2] - s[2]], T = [r[0] - s[0], r[1] - s[1], r[2] - s[2]], M = h[1] * T[2] - h[2] * T[1], L = h[2] * T[0] - h[0] * T[2], x = h[0] * T[1] - h[1] * T[0], g = Math.sqrt(M * M + L * L + x * x), G = g > 1e-10 && Math.abs(x) / g < 0.5;
  ...
  if (Ee.length > 0) {
    ...
    Ee.forEach((t, s) => {
      ...
      if (r) {
        const L = (J) => Q.indexOf(J);
        if (new Set(M.map((J) => J.pt)).size === 4) {
          const J = Math.max(...M.map((ae) => L(ae.story))), X = M.map((ae) => J - L(ae.story));
          i.push(`  AREA "${h}"  ${T}  4  "${M[0].pt}"  "${M[1].pt}"  "${M[2].pt}"  "${M[3].pt}"  ${X.join("  ")}  `), e.push(`  AREAASSIGN  "${h}"  "${Q[J]}"  SECTION "${ze(t)}"  OBJMESHTYPE "DEFAULT"  ADDRESTRAINT "No"  CARDINALPOINT "MIDDLE"  TRANSFORMSTIFFNESSFOROFFSETS "No"  `);
          return;
        }
        const g = p[a[0]][2] <= p[a[2]][2] ? 0 : 2, G = p[a[1]][2] <= p[a[3]][2] ? 1 : 3;
        i.push(`  AREA "${h}"  ${T}  4  "${M[g].pt}"  "${M[G].pt}"  "${M[G].pt}"  "${M[g].pt}"  1  1  0  0  `);
...
function yt(m, p) {
  const R = m[p[0]], C = m[p[1]], $ = Math.abs(C[2] - R[2]), H = Math.sqrt((C[0] - R[0]) ** 2 + (C[1] - R[1]) ** 2), W = $ > H * 0.5;
