...
let __tla = (async () => {
  ...
  function Ia(l, e, t, s, n, i, o = 8) {
    ...
    for (let ie = 0; ie <= o; ie++) {
      const le = ie / o, he = 1 - 3 * le * le + 2 * le * le * le, pe = x * (le - 2 * le * le + le * le * le), Ce = 3 * le * le - 2 * le * le * le, ve = x * (-le * le + le * le * le), Ae = he * W + pe * R + Ce * V + ve * X, Ve = he * q - pe * Q + Ce * D - ve * re, ot = ue + (be - ue) * le, yt = [
        l[0] + f[0] * (le * x + ot),
        l[1] + f[1] * (le * x + ot),
        l[2] + f[2] * (le * x + ot)
      ];
    ...
    return qn((p, f) => {
      ...
      for (const D of A) {
        ...
        if (D.length === 2 && e.deformedShape.val) {
          ...
          if (ue && be && re) {
            const ae = Number.isFinite(e.deformScale.val) ? e.deformScale.val : 1, ie = ae * (Number.isFinite(e.deformScaleZ.val) ? e.deformScaleZ.val : 1), le = Ia(ue, be, re.get(D[0]), re.get(D[1]), ae, ie);
        ...
        if (D.length === 2 && !e.deformedShape.val && X && X.orig.length === S.length) {
          ...
          if (Q && re) {
            const ue = X.shape.slice(D[0] * 6, D[0] * 6 + 6), be = X.shape.slice(D[1] * 6, D[1] * 6 + 6), ae = Ia(Q, re, ue, be, X.amp, X.amp);
    ...
    return we.derive(() => {
      ...
      V.forEach((X, Q) => {
        ...
        const ie = [
          ...
          (re[1] + ue[1]) / 2,
          (re[2] + ue[2]) / 2
        ], le = Di(re, ue), he = (Ce) => (Ce.userData.secElem = X, Ce);
        if (ae.type === "CFT") {
          const Ce = d(ae.b, ae.h, ae.tw ?? ae.b * 0.05), ve = new pt(Ce.concFill, S);
          ...
          switch (ae.type) {
            case "rect":
              Ce = o(ae.b, ae.h), ve = S, Ae = A;
              break;
            case "circ":
              Ce = a(ae.d), ve = S, Ae = A;
              break;
            case "I":
              Ce = r(ae.b, ae.h, ae.tf, ae.tw), ve = P, Ae = H;
              break;
            case "HSS":
              Ce = c(ae.b, ae.h, ae.tw ?? ae.b * 0.05), ve = P, Ae = H;
              break;
            case "CFT":
              Ce = d(ae.b, ae.h, ae.tw ?? ae.b * 0.05), ve = P, Ae = H;
              break;
            case "L":
              Ce = p(ae.b ?? ae.h, ae.h, ae.t ?? ae.tw ?? 3e-3), ve = P, Ae = H;
              break;
            case "2L":
              Ce = f(ae.b ?? ae.h, ae.h, ae.t ?? ae.tw ?? 3e-3, ae.dis ?? 0.01), ve = P, Ae = H;
              break;
            case "C":
            case "coldC":
              Ce = x(ae.b, ae.h, ae.tf ?? ae.t ?? 3e-3, ae.tw ?? ae.t ?? 3e-3), ve = P, Ae = H;
              break;
            case "2C":
              Ce = b(ae.b, ae.h, ae.tf ?? 5e-3, ae.tw ?? 5e-3, ae.dis ?? 0.01), ve = P, Ae = H;
              break;
            case "T":
              Ce = C(ae.b, ae.h, ae.tf ?? 0.01, ae.tw ?? 6e-3), ve = P, Ae = H;
              break;
            case "pipe":
              Ce = _(ae.d, ae.tw ?? ae.d * 0.05), ve = P, Ae = H;
              break;
            default:
              return;
          }
        ...
        x.forEach((A, P) => {
          ...
          if (A.length === 3 || A.length === 4) {
            ...
            const X = [
              ...
              Ce[1] + X[1] * pe,
              Ce[2] + X[2] * pe
            ]), ue = Math.abs(X[2]) > 0.5, be = X[2] > 0 ? -1 : 1, ae = re(ue ? 0 : +H / 2), ie = re(ue ? be * H : -H / 2), le = (pe, Ce, ve) => Q.push(...pe, ...Ce, ...ve);
            for (const pe of [
              ae,
              ie
            ...
            for (let pe = 0; pe < U.length; pe++) {
              ...
              le(ae[pe], ie[pe], ie[Ce]), le(ae[pe], ie[Ce], ae[Ce]);
            }
            const he = new Ie();
  ...
  function Nr({ drawingObj: l, gridObj: e, scene: t, getActiveCamera: s, controls: n, gridSize: i, derivedDisplayScale: o, rendererElm: a, viewerRender: r }) {
    ...
    const ie = /* @__PURE__ */ new Map(), le = new is(new Ie(), new mt({
      color: 16498468,
      transparent: true,
      opacity: 0.35,
      depthTest: true
    ...
    const Ae = (u) => {
      ...
      le.geometry.dispose(), le.geometry = new Ie(), le.geometry.setAttribute("position", new bt(v, 3)), le.visible = m > 0 && window.__hekatanRefIfcBordes !== false;
    };
  ...
  function jr(l, e, t, s) {
    ...
    return we.derive(() => {
      ...
      for (const { idx: X, vals: Q } of b) {
        ...
        ae.normalize();
        const le = Math.abs(ae.y) < 0.99 ? new te(0, 1, 0) : new te(1, 0, 0), he = new te().crossVectors(ae, le).normalize(), pe = new te().crossVectors(ae, he).normalize(), Ce = xo + 1, ve = Br;
        for (let Ae = 0; Ae < Ce; Ae++) {
          const Ve = Ae / xo, ot = ue[0] + ae.x * ie * Ve, yt = ue[1] + ae.y * ie * Ve, Se = ue[2] + ae.z * ie * Ve, ce = Q[0] + (Q[1] - Q[0]) * Ve, _e = i.getColor(ce) ?? new $t(0, 0, 0);
...
Mz \u2248 ${Ft(un * Be)} ${Ze.forceUnit}\xB7m`);
  ...
  function oc(l, e) {
    ...
    function r() {
      ...
      for (const $e of ae) for (const Be of [
        $e.a,
        $e.b
      ]) ie = Math.min(ie, Be.u), le = Math.max(le, Be.u), he = Math.min(he, Be.v), pe = Math.max(pe, Be.v);
      const Ce = le - ie || 1, ve = pe - he || 1, Ae = 0.12 * Math.max(Ce, ve), Ve = 46, ot = Math.min((ue - 2 * Ve) / (Ce + 2 * Ae), (be - 2 * Ve) / (ve + 2 * Ae)), yt = (ue - Ce * ot) / 2, Se = (be - ve * ot) / 2, ce = ($e) => yt + ($e - ie) * ot, _e = ($e) => be - (Se + ($e - he) * ot), ge = "http://www.w3.org/2000/svg", xe = ($e, Be, Rt) => {
      ...
      const ze = ae.filter(($e) => !Ne.get($e.i)).length;
      let Me = 0;
      if (re) for (const $e of ae) {
      ...
      H.textContent = `${Ze} \xB7 ${s.plano === "XY" ? "planta" : "alzado"} ${s.plano} en ${q} = ${s.en.toFixed(2)} m`, U.textContent = R ? `${ae.length} barras en el plano \xB7 m\xE1ximo ${Ee(Me)} ${nc[R] ?? ""}` + (Ge ? " \xB7 el momento va del lado de la tracci\xF3n" : "") + (ze ? ` \xB7 ${ze} barra(s) con este resultado en otro plano (sin dibujar)` : "") + " \xB7 clic en una barra: su gr\xE1fico" : "Eleg\xED un resultado en \xABFrame results\xBB (Axial, Cortante, Momento) para ver su diagrama aqu\xED.";
    }
    const c = () => {
      try {
        r();
    ...
    function S() {
      ...
      let W = 0, V = -1;
      U.forEach((le, he) => {
        const [pe, Ce] = P[le], ve = he === 0 ? U.length > 1 && P[U[1]].includes(pe) : pe !== V, Ae = ve ? Ce : pe, Ve = ve ? pe : Ce, ot = Math.hypot(A[Ve][0] - A[Ae][0], A[Ve][1] - A[Ae][1], A[Ve][2] - A[Ae][2]);
        G.push({
          x: W,
          e: le,
          fin: ve ? 1 : 0
      ...
      const q = W, D = (le, he) => {
        const pe = H[le], Ce = pe ? pe instanceof Map ? pe.get(he.e) : pe[he.e] : null;
        return Ce ? $o(le, Ce)[he.fin] : 0;
      ...
      for (const [le, he, pe, Ce] of Q) {
        const ve = G.map((Me) => D(le, Me)), Ae = Math.max(...ve), Ve = Math.min(...ve), ot = Math.max(Math.abs(Ae), Math.abs(Ve)) || 1, yt = (Me) => ae + Me / (q || 1) * (ue - 2 * ae), Se = (Me) => ie + (Ce ? 1 : -1) * (Me / ot) * (ie - 16), ce = (Me) => Math.abs(Me) >= 100 ? Me.toFixed(1) : Math.abs(Me) >= 10 ? Me.toFixed(2) : Me.toFixed(3);
        let _e = yt(0) + "," + ie + " ";
        G.forEach((Me, Fe) => {
          _e += yt(Me.x) + "," + Se(ve[Fe]) + " ";
        }), _e += yt(q) + "," + ie;
  ...
  wd = function({ mesh: l, settingsObj: e, drawingObj: t, objects3D: s, solids: n }) {
    ...
    function pe(ce, _e) {
      ...
      if (r.left = -Me * ze, r.right = Me * ze, r.updateProjectionMatrix(), ce && be) {
        if (ae ? (ae.object = be, ae.update()) : (ae = new Sa(be, d.domElement), ae.enableDamping = true, ae.dampingFactor = 0.1, ae.screenSpacePanning = true, ae.zoomSpeed = 0.8, ae.panSpeed = 1.2, ae.rotateSpeed = 0.9, ae.touches = {
          ONE: bi.ROTATE,
          TWO: bi.DOLLY_PAN
    ...
    const Ae = (ce, _e, ge) => {
      ...
      const Fe = Ne * 0.05;
      ge.addScaledVector(ze, -ce * Fe), ge.addScaledVector(Me, _e * Fe), c.position.addScaledVector(ze, -ce * Fe), c.position.addScaledVector(Me, _e * Fe), p.update(), le();
    }, ot = (ce) => {
