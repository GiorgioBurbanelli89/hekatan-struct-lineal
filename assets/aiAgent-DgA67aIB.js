...
import { l as $t, a0 as pn, a1 as Sl, v as xe, a2 as Ml, D as Rt, M as pt, B as ze, F as Mt, a3 as $l, q as bt, a4 as Al, a5 as El, a6 as ia, h as na, a7 as oa, Y as Vs, a8 as _n, a9 as kn, $ as Vi, H as ft, L as is, b as mt, f as gt, p as Pa, c as Tl, aa as zl, V as te, I as ti, J as Bt, ab as Jn, u as Tn, d as Vt, a as Qn, A as Da, k as Mn, t as Il, s as Li, P as Fl, ac as Cn, n as eo, m as Rl, w as Ds, X as Ti, E as aa, S as gi, ad as to, ae as zi, g as la, i as ra, j as ca, C as da, x as Pl, y as Dl, z as Ll, W as Nl, G as ha, af as fn, N as so, O as Ol, ag as Vl, _ as Bl } from "./Text-Br8EG2up.js";
import { P as La } from "./tweakpane-BXg6ZhiP.js";
class Na {
  constructor(e, t = 32) {
  ...
  getColor(e) {
    e = Sl.clamp(e, this.minV, this.maxV), e = (e - this.minV) / (this.maxV - this.minV);
...
const io = { rainbow: [[0, 255], [0.2, 65535], [0.5, 65280], [0.8, 16776960], [1, 16711680]], cooltowarm: [[0, 3952322], [0.2, 10206463], [0.5, 14474460], [0.8, 16163717], [1, 11797542]], blackbody: [[0, 0], [0.2, 7864320], [0.5, 15086080], [0.8, 16776960], [1, 16777215]], grayscale: [[0, 0], [0.2, 4210752], [0.5, 8355712], [0.8, 12566463], [1, 16777215]] }, ua = [[200, 0, 200], [228, 0, 100], [255, 0, 0], [255, 64, 0], [255, 128, 0], [255, 170, 0], [255, 212, 0], [255, 255, 0], [128, 255, 0], [0, 255, 0], [0, 255, 128], [0, 255, 255], [0, 170, 255], [0, 85, 255], [0, 0, 255]], wi = ua.map(([r, e, t], s) => [s / (ua.length - 1), r, e, t]), Hl = /* @__PURE__ */ new Set(["safe", "etabs", "sap2000", "csi"]);
function bi(r) {
  return Hl.has(r);
}
const Oa = { safe: wi, etabs: wi, sap2000: wi, csi: wi, jet_r: [[0, 200, 0, 0], [0.15, 255, 80, 0], [0.32, 255, 200, 0], [0.48, 180, 255, 0], [0.6, 0, 230, 90], [0.74, 0, 220, 230], [0.88, 0, 110, 255], [1, 0, 0, 180]], jet: [[0, 0, 0, 180], [0.12, 0, 110, 255], [0.26, 0, 220, 230], [0.4, 0, 230, 90], [0.52, 180, 255, 0], [0.68, 255, 200, 0], [0.85, 255, 80, 0], [1, 200, 0, 0]], viridis: [[0, 68, 1, 84], [0.25, 59, 82, 139], [0.5, 33, 145, 140], [0.75, 94, 201, 98], [1, 253, 231, 37]] }, Ts = xe.state("safe");
typeof window < "u" && (window.__hekatanColorPalette = Ts);
const Va = xe.state("auto");
function Ba(r) {
  r = Math.max(0, Math.min(1, r));
  const e = Ts.val, t = Oa[e] ?? wi;
  if (bi(e)) {
...
function pa() {
  ...
  for (let i = 0; i < 256; i++) {
    const n = i / 255, [o, a, l] = Ba(n);
    e[i * 4 + 0] = o, e[i * 4 + 1] = a, e[i * 4 + 2] = l, e[i * 4 + 3] = 255;
  }
  const t = new Al(e, 256, 1, El), s = bi(Ts.val);
  return t.minFilter = s ? ia : na, t.magFilter = s ? ia : na, t.wrapS = oa, t.wrapT = oa, t.needsUpdate = true, t;
}
function Xl() {
  const r = Ts.val;
  if (bi(r)) {
    const s = Oa[r] ?? wi, i = s.length, n = [];
    for (let o = 0; o < i; o++) {
...
function Yl(r, e, t) {
  new Na();
  const s = pa(), i = new Ml({ uniforms: { cmap: { value: s }, ambient: { value: 0.95 } }, vertexShader: `
      #include <common>
  ...
  xe.derive(() => {
    ...
    i.uniforms.cmap.value = pa(), (_a2 = o == null ? void 0 : o.dispose) == null ? void 0 : _a2.call(o);
  });
  ...
  return n.renderOrder = -1, n.frustumCulled = false, n.userData.isShellArea = true, n.name = "__hekatan_shell_colormap", xe.derive(() => {
    ...
    const o = [], a = [], l = [];
    e.val.forEach((E, R) => {
      E.length === 3 ? (o.push(E[0], E[1], E[2]), a.push(R), l.push(0)) : E.length === 4 && (o.push(E[0], E[1], E[2]), o.push(E[0], E[2], E[3]), a.push(R, R), l.push(0, 1));
    }), n.geometry.setIndex(new $l(o, 1)), n.userData.faceToElem = a, n.userData.faceLocal = l;
    const c = t.val.filter((E) => Number.isFinite(E));
    let h, p;
    const f = Oi.val;
    if (f ? (p = f[0], h = f[1]) : [p, h] = yo(c), h === p) {
      const E = Math.max(Math.abs(h) * 1e-6, 1e-9);
      h += E, p -= E;
    }
    const x = f && f[0] > f[1] && !bi(Ts.val), b = Math.min(p, h), C = Math.max(p, h), _ = C - b, S = new Float32Array(t.val.length);
    for (let E = 0; E < t.val.length; E++) {
      const R = t.val[E];
      if (!Number.isFinite(R)) {
        S[E] = -1;
        continue;
      }
      const q = ((x ? C + b - R : R) - b) / _;
      S[E] = Math.max(0, Math.min(1, q));
    }
    n.geometry.setAttribute("scalar", new bt(S, 1));
  }), n;
}
function Ul(r, e, t) {
  const s = document.createElement("div"), i = new La({ title: "Settings", expanded: true, container: s });
  ...
  const a = () => {
    ...
    let b = false, C = 0, _ = 0, S = 0, E = 0;
    x.addEventListener("mousedown", (R) => {
      b = true, C = R.clientX, _ = R.clientY;
      const X = s.getBoundingClientRect();
      S = X.left, E = X.top, s.style.left = `${S}px`, s.style.top = `${E}px`;
    }), window.addEventListener("mousemove", (R) => {
      if (!b) return;
      const X = R.clientX - C, q = R.clientY - _, G = Math.max(0, Math.min(window.innerWidth - 40, S + X)), W = Math.max(0, Math.min(window.innerHeight - 40, E + q));
  ...
  if ((e == null ? void 0 : e.deformOutputs) || (e == null ? void 0 : e.analyzeOutputs)) {
    ...
    }), x.addBinding(r.shellResults, "val", { options: { none: "none", F11: "membraneXX", F22: "membraneYY", F12: "membraneXY", FMax: "membranePrincipalMax", FMin: "membranePrincipalMin", FVM: "vonMises", V13: "tranverseShearX", V23: "tranverseShearY", VMax: "transverseShearMax", M11: "bendingXX", M22: "bendingYY", M12: "bendingXY", MMax: "bendingPrincipalMax", MMin: "bendingPrincipalMin", "Pressure (suelo)": "pressure", Ux: "displacementX", Uy: "displacementY", Uz: "displacementZ" }, label: "Resultados de c\xE1scara" }), x.addBinding(Ts, "val", { options: { "SAFE (cimentaci\xF3n)": "safe", ETABS: "etabs", SAP2000: "sap2000", "Jet_r (rojo\u2192azul)": "jet_r", "Jet (azul\u2192rojo)": "jet", Viridis: "viridis" }, label: "\u{1F3A8} Paleta colores" }), x.addBinding(Va, "val", { options: { "todas las c\xE1scaras": "auto", "solo muros": "muros", "muros X (plano x=cte)": "murosX", "muros Y (plano y=cte)": "murosY", "solo losas": "losas" }, label: "\u{1F4D0} Rango colormap" }), x.addBinding(r.solidResults, "val", { options: { none: "none", vonMises: "vonMises", \u03C3xx: "sigmaXX", \u03C3yy: "sigmaYY", \u03C3zz: "sigmaZZ", \u03C4xy: "tauXY", \u03C4yz: "tauYZ", \u03C4xz: "tauXZ", ux: "ux", uy: "uy", uz: "uz" }, label: "Resultados de s\xF3lido" }), x.addBinding(r.deformedShape, "val", { label: "Deformada" }), x.addBinding(r.deformScale, "val", { label: "  Escala XY", min: 0.1, max: 5e3, step: 0.1 }), x.addBinding(r.deformScaleZ, "val", { label: "  Escala Z", min: 0.01, max: 10, step: 0.01 });
  }
  ...
  const f = (x, b) => {
    ...
    p = [];
    const C = (_, S) => {
      const E = Math.floor(Math.min(x[S], -50)), R = Math.ceil(Math.max(b[S], 50)), X = R - E > 400 ? 0.5 : 0.1;
      return c["pos" + _] = Math.max(E, Math.min(R, c["pos" + _])), l.addBinding(c, "pos" + _, { min: E, max: R, step: X, label: `  pos ${_} (m)` }).on("change", h);
    };
...
const Wl = new $t(16746496), Zl = new $t(52428), fa = new $t(52292), ma = new $t(3377407), ga = new $t(16763904);
function wa(r, e) {
...
function ya(r, e, t, s, i, n, o = 8) {
  ...
  _ = [_[0] / S, _[1] / S, _[2] / S];
  const E = C(f, _), R = (ne, ae) => ne[0] * ae[0] + ne[1] * ae[1] + ne[2] * ae[2], X = (ne) => {
    const ae = ne ?? [0, 0, 0];
    return [(ae[0] || 0) * i, (ae[1] || 0) * i, (ae[2] || 0) * n];
  }, q = X(t), G = X(s), W = R(q, _), U = R(G, _), K = R(q, E), D = R(G, E), P = h ? R(h, E) * i : 0, B = p ? R(p, E) * i : 0, Q = h ? R(h, _) * i : 0, re = p ? R(p, _) * i : 0, pe = R(q, f), be = R(G, f), le = [];
  for (let ne = 0; ne <= o; ne++) {
    const ae = ne / o, ue = 1 - 3 * ae * ae + 2 * ae * ae * ae, he = x * (ae - 2 * ae * ae + ae * ae * ae), Se = 3 * ae * ae - 2 * ae * ae * ae, ve = x * (-ae * ae + ae * ae * ae), Oe = ue * W + he * P + Se * U + ve * B, He = ue * K - he * Q + Se * D - ve * re, dt = pe + (be - pe) * ae, wt = [r[0] + f[0] * (ae * x + dt), r[1] + f[1] * (ae * x + dt), r[2] + f[2] * (ae * x + dt)];
    le.push([wt[0] + _[0] * Oe + E[0] * He, wt[1] + _[1] * Oe + E[1] * He, wt[2] + _[2] * Oe + E[2] * He]);
  }
  return le;
}
function Gl(r, e, t) {
  ...
  const o = new gt({ vertexColors: true, transparent: true, opacity: s.shellOpacity, side: Rt, depthWrite: false, polygonOffset: true, polygonOffsetFactor: 1, polygonOffsetUnits: 1 }), a = new pt(new ze(), o);
  ...
  return Vi((p, f) => {
    ...
    const p = e.elemFrames ? e.elemFrames.rawVal : true, f = e.elemColumns.rawVal, x = e.elemBeams.rawVal, b = e.elemZapatas ? e.elemZapatas.rawVal : true, C = e.elemLosas ? e.elemLosas.rawVal : true, _ = e.colorByType ? e.colorByType.rawVal : false, S = t.val, E = ((_e = r.elements) == null ? void 0 : _e.val) || [], R = (D) => {
      if (D.length === 2) {
        if (!p) return false;
        const P = S[D[0]], B = S[D[1]];
        return !P || !B ? true : wa(P, B) ? f : x;
      }
      if (D.length === 4) {
        const P = D.map((Q) => S[Q]).filter(Boolean);
        if (P.length < 4) return true;
        const B = (P[0][2] + P[1][2] + P[2][2] + P[3][2]) / 4;
        return mn(B) ? b : C;
      }
      if (D.length === 3) {
        const P = D.map((Q) => S[Q]).filter(Boolean);
        if (P.length < 3) return true;
        const B = (P[0][2] + P[1][2] + P[2][2]) / 3;
        return mn(B) ? b : C;
      }
      return true;
    }, X = [], q = [];
    for (const D of E) {
      ...
      if (D.length === 2 && e.deformedShape.val) {
        ...
        if (pe && be && re) {
          const le = Number.isFinite(e.deformScale.val) ? e.deformScale.val : 1, ne = le * (Number.isFinite(e.deformScaleZ.val) ? e.deformScaleZ.val : 1), ae = ya(pe, be, re.get(D[0]), re.get(D[1]), le, ne);
          for (let ue = 0; ue < ae.length - 1; ue++) X.push(...ae[ue], ...ae[ue + 1]), _ && P && (q.push(P.r, P.g, P.b), q.push(P.r, P.g, P.b));
          continue;
        }
      }
      ...
      if (D.length === 2 && !e.deformedShape.val && B && B.orig.length === S.length) {
        ...
        if (Q && re) {
          const pe = B.shape.slice(D[0] * 6, D[0] * 6 + 6), be = B.shape.slice(D[1] * 6, D[1] * 6 + 6), le = ya(Q, re, pe, be, B.amp, B.amp);
          for (let ne = 0; ne < le.length - 1; ne++) X.push(...le[ne], ...le[ne + 1]), _ && P && (q.push(P.r, P.g, P.b), q.push(P.r, P.g, P.b));
          continue;
        }
      }
      for (const Q of Jl(D)) {
        const re = S[Q[0]], pe = S[Q[1]];
        !re || !pe || (X.push(...re, ...pe), _ && P && (q.push(P.r, P.g, P.b), q.push(P.r, P.g, P.b)));
      }
    }
        ...
        if (P.length === 3) {
          ...
          if (S[B] && S[Q] && S[re] && S[pe]) {
            ...
            if (_) {
              ...
              be = mn(le) ? fa : ma;
            } else be = Kl(S[B], S[Q], S[re], S[pe]) ? l : c;
...
function no(r, e) {
  ...
  const c = new $t(t.grid).multiplyScalar(1.3), h = new $t(t.grid).multiplyScalar(0.8), p = (C, _, S, E) => {
    const R = [], X = C === "xy" ? (K, D) => [K, D, 0] : C === "xz" ? (K, D) => [K, 0, D] : (K, D) => [0, K, D], q = Math.floor(a / _);
    for (let K = -q; K <= q; K++) {
      const D = K * _, P = X(D, -a), B = X(D, a);
      R.push(...P, ...B);
    }
    for (let K = -q; K <= q; K++) {
      const D = K * _, P = X(-a, D), B = X(a, D);
      R.push(...P, ...B);
    }
    const G = new ze();
    G.setAttribute("position", new Mt(R, 3));
    const W = new mt({ color: S, transparent: true, opacity: E, depthWrite: false }), U = new is(G, W);
    return U.name = `grid-${C}-${_ === o ? "minor" : "major"}`, U;
  }, f = (C, _, S) => {
    const E = C === "xy" ? (U, K) => [U, K, 0] : C === "xz" ? (U, K) => [U, 0, K] : (U, K) => [0, U, K], R = [[-a, -a], [a, -a], [a, a], [-a, a]], X = [];
    for (const [U, K] of R) X.push(...E(U, K));
    const q = new ze();
    ...
    const E = C === "xy" ? (G, W) => [G, W, 0] : C === "xz" ? (G, W) => [G, 0, W] : (G, W) => [0, G, W], R = _ === "u" ? [...E(-a, 0), ...E(a, 0)] : [...E(0, -a), ...E(0, a)], X = new ze();
    X.setAttribute("position", new Mt(R, 3));
...
function Ql(r, e, t, s) {
  const i = new ft(), n = new Tl(0.5, 0.5, 0.5), o = new zl(0.45, 0.7, 4);
  ...
  return xe.derive(() => {
    ...
    (_b = (_a2 = r.nodeInputs) == null ? void 0 : _a2.val.supports) == null ? void 0 : _b.forEach((b, C) => {
      const _ = t.val[C];
      if (!_) return;
      const S = b ?? [], E = (S[0] ? 1 : 0) + (S[1] ? 1 : 0) + (S[2] ? 1 : 0), R = (S[3] ? 1 : 0) + (S[4] ? 1 : 0) + (S[5] ? 1 : 0);
      let X;
      E >= 3 && R >= 3 ? X = new pt(n, a) : E >= 3 && R === 0 ? X = new pt(o, l) : X = new pt(o, c), X.position.set(_[0], _[1], _[2]);
      const q = x * f();
...
function er(r, e, t, s) {
  ...
  xe.derive(() => {
    ...
    let p = h;
    if (h.length > c) {
      const C = h.map((B) => a[B][0]), _ = h.map((B) => a[B][1]), S = Math.min(...C), E = Math.max(...C), R = Math.min(..._), X = Math.max(..._), q = h.map((B) => a[B][2]), G = Math.max(1e-6, (Math.max(...q) - Math.min(...q)) / 40), W = (B) => Math.round(B / G), U = new Set(q.map(W)), K = Math.max(4, Math.floor(c / Math.max(1, U.size))), D = Math.max(2, Math.round(Math.sqrt(K))), P = /* @__PURE__ */ new Map();
      for (const B of h) {
        const Q = E - S < 1e-9 ? 0 : (a[B][0] - S) / (E - S), re = X - R < 1e-9 ? 0 : (a[B][1] - R) / (X - R), pe = Math.min(D - 1, Math.floor(Q * D)), be = Math.min(D - 1, Math.floor(re * D)), le = `${pe},${be},${W(a[B][2])}`, ne = Math.hypot(Q * D - (pe + 0.5), re * D - (be + 0.5)), ae = P.get(le);
        (!ae || ne < ae.d) && P.set(le, { i: B, d: ne });
      }
      p = [...P.values()].map((B) => B.i);
    }
    let f = 0;
    for (const C of p) {
    ...
    for (const C of p) {
      ...
      if (S) for (let E = 0; E < 3; E++) {
        const R = _[E];
        if (!(Math.abs(R) > 1e-9 * (f || 1))) continue;
        const X = new te(E === 0 ? Math.sign(R) : 0, E === 1 ? Math.sign(R) : 0, E === 2 ? Math.sign(R) : 0), q = 0.45 + 0.55 * (f ? Math.abs(R) / f : 1), G = new ti(X, new te(...S), 1, E === 2 ? 15637248 : 15022123, 0.3, 0.3);
        if (G.userData = { nudo: S, dir: X, rel: q }, i.add(G), x) {
          const W = new Bt(b(R), E === 2 ? "#f5b642" : "#ff6b5e");
...
function sr(r, e, t, s) {
  ...
  return xe.derive(() => {
    ...
    (_a2 = r.elements) == null ? void 0 : _a2.val.forEach((o, a) => {
      ...
      l.position.set(...ir(o.map((c) => t.rawVal[c]))), l.updateScale(n * s.rawVal), i.add(l);
    });
  }), xe.derive(() => {
  ...
  return n.setAttribute("color", new Mt([...p, ...p, ...f, ...f, ...x, ...x], 3)), xe.derive(() => {
    ...
    e.deformedShape.val, e.orientations.val && (i.clear(), (_a2 = r.elements) == null ? void 0 : _a2.val.forEach((b) => {
      ...
      if (b.length === 2 && (C.position.set(...po(_, S)), C.rotation.setFromRotationMatrix(xo(_, S))), b.length === 3) {
        ...
        C.position.set(...Ni([_, S, X])), C.rotation.setFromRotationMatrix(nr(_, S, X));
      }
      const R = 0.05 * e.gridSize.rawVal * 0.75 * s.rawVal;
      C.scale.set(R, R, R), i.add(C);
    }));
  }), xe.derive(() => {
...
function lr(r, e, t, s) {
  ...
  i.add(n);
  function o(G, W) {
    const U = G / 2, K = W / 2, D = new Float32Array([0, -U, -K, 0, U, -K, 0, U, K, 0, -U, -K, 0, U, K, 0, -U, K]), P = new ze();
    P.setAttribute("position", new bt(D, 3));
    const B = new Float32Array([0, -U, -K, 0, U, -K, 0, U, K, 0, -U, K, 0, -U, -K]), Q = new ze();
    return Q.setAttribute("position", new bt(B, 3)), { fill: P, outline: Q };
  }
  function a(G, W = 24) {
    ...
    for (let Q = 0; Q <= W; Q++) {
      ...
      P[Q * 3] = 0, P[Q * 3 + 1] = U * Math.cos(re), P[Q * 3 + 2] = U * Math.sin(re);
    }
    const B = new ze();
    return B.setAttribute("position", new bt(P, 3)), { fill: D, outline: B };
  }
  function l(G, W, U, K) {
    const D = U ?? W * 0.08, P = K ?? G * 0.07, B = G / 2, Q = W / 2, re = Q - D, pe = P / 2, be = [];
  ...
  function c(G, W, U) {
    const K = G / 2, D = W / 2, P = K - U, B = D - U, Q = [];
    function re(ne, ae, ue, he) {
      Q.push(0, ne, ae, 0, ue, ae, 0, ue, he, 0, ne, ae, 0, ue, he, 0, ne, he);
    }
    re(-K, -D, K, -B), re(-K, B, K, D), re(-K, -B, -P, B), re(P, -B, K, B);
    const pe = new ze();
    pe.setAttribute("position", new bt(new Float32Array(Q), 3));
    const be = new Float32Array([0, -K, -D, 0, K, -D, 0, K, -D, 0, K, D, 0, K, D, 0, -K, D, 0, -K, D, 0, -K, -D, 0, -P, -B, 0, P, -B, 0, P, -B, 0, P, B, 0, P, B, 0, -P, B, 0, -P, B, 0, -P, -B]), le = new ze();
    return le.setAttribute("position", new bt(be, 3)), { fill: pe, outline: le };
  }
  function h(G, W, U) {
    const K = G / 2, D = W / 2, P = K - U, B = D - U, Q = new ze(), re = new Float32Array([0, -P, -B, 0, P, -B, 0, P, B, 0, -P, -B, 0, P, B, 0, -P, B]);
    Q.setAttribute("position", new bt(re, 3));
    const pe = [];
    function be(ue, he, Se, ve) {
      pe.push(0, ue, he, 0, Se, he, 0, Se, ve, 0, ue, he, 0, Se, ve, 0, ue, ve);
    }
    be(-K, -D, K, -B), be(-K, B, K, D), be(-K, -B, -P, B), be(P, -B, K, B);
    const le = new ze();
    le.setAttribute("position", new bt(new Float32Array(pe), 3));
    const ne = new Float32Array([0, -K, -D, 0, K, -D, 0, K, -D, 0, K, D, 0, K, D, 0, -K, D, 0, -K, D, 0, -K, -D, 0, -P, -B, 0, P, -B, 0, P, -B, 0, P, B, 0, P, B, 0, -P, B, 0, -P, B, 0, -P, -B]), ae = new ze();
    return ae.setAttribute("position", new bt(ne, 3)), { concFill: Q, steelFillGeom: le, outline: ae };
  }
  function p(G, W, U) {
    const K = [], D = [[0, -G / 2, -W / 2], [0, -G / 2 + U, -W / 2], [0, -G / 2 + U, W / 2 - U], [0, G / 2, W / 2 - U], [0, G / 2, W / 2], [0, -G / 2, W / 2]], P = [0, 1, 2, 0, 2, 5, 2, 3, 4, 2, 4, 5];
    for (const pe of P) K.push(...D[pe]);
    const B = new ze();
  ...
  function f(G, W, U, K) {
    const D = K / 2, P = [], B = [[0, -G - D, -W / 2], [0, -U - D, -W / 2], [0, -U - D, W / 2 - U], [0, -D, W / 2 - U], [0, -D, W / 2], [0, -G - D, W / 2]], Q = [[0, D, -W / 2], [0, D + U, -W / 2], [0, D + U, W / 2 - U], [0, G + D, W / 2 - U], [0, G + D, W / 2], [0, D, W / 2]], re = [0, 1, 2, 0, 2, 5, 2, 3, 4, 2, 4, 5];
    for (const ne of re) P.push(...B[ne]);
    for (const ne of re) P.push(...Q[ne]);
    const pe = new ze();
    pe.setAttribute("position", new bt(new Float32Array(P), 3));
    const be = [];
  ...
  function x(G, W, U, K) {
    const D = W / 2, P = G, B = [[0, -P, -D], [0, -P, -D + U], [0, -K, -D + U], [0, -K, D - U], [0, -P, D - U], [0, -P, D], [0, 0, D], [0, 0, -D]], Q = [0, 1, 7, 1, 6, 7, 1, 2, 6, 2, 5, 6, 2, 3, 5, 3, 4, 5], re = [];
  ...
  function b(G, W, U, K, D) {
    const P = W / 2, B = D / 2, Q = [], re = [[0, -G, -P], [0, -G, -P + U], [0, -B - K, -P + U], [0, -B - K, P - U], [0, -G, P - U], [0, -G, P], [0, -B, P], [0, -B, -P]], pe = re.map((ue) => [ue[0], -ue[1], ue[2]]), be = [0, 1, 7, 1, 6, 7, 1, 2, 6, 2, 5, 6, 2, 3, 5, 3, 4, 5];
  ...
  function C(G, W, U, K) {
    const D = G / 2, P = W / 2, B = K / 2, Q = [[0, -B, -P], [0, B, -P], [0, B, P - U], [0, D, P - U], [0, D, P], [0, -D, P], [0, -D, P - U], [0, -B, P - U]], re = [0, 1, 7, 1, 2, 7, 6, 7, 5, 2, 3, 4, 2, 4, 5, 2, 5, 7], pe = [];
  ...
  function _(G, W, U = 24) {
    ...
    for (let pe = 0; pe < U; pe++) {
      ...
      P.push(0, K * ne, K * ae, 0, K * ue, K * he, 0, D * ue, D * he), P.push(0, K * ne, K * ae, 0, D * ue, D * he, 0, D * ne, D * ae);
    }
    const B = new ze();
    B.setAttribute("position", new bt(new Float32Array(P), 3));
    const Q = [];
    for (let pe = 0; pe < U; pe++) {
  ...
  const S = new gt({ color: 52479, transparent: true, opacity: 0.35, side: Rt, depthWrite: false }), E = new mt({ color: 52479 }), R = new gt({ color: 16750848, transparent: true, opacity: 0.4, side: Rt, depthWrite: false }), X = new mt({ color: 16750848 });
  function q(G, W) {
  ...
  return xe.derive(() => {
    ...
    if (!U || !K) return;
    const D = K.sectionShapes, P = e.secFloor.rawVal;
    U.forEach((B, Q) => {
      ...
      if (P >= 0) {
        ...
        if (Math.floor(he / Se + 0.01) !== P) return;
      }
      ...
      if (le.type === "CFT") {
        ...
        const ve = new pt(he.steelFillGeom, R);
        ...
        switch (le.type) {
          case "rect":
            he = o(le.b, le.h), Se = S, ve = E;
            break;
          case "circ":
            he = a(le.d), Se = S, ve = E;
            break;
          case "I":
            he = l(le.b, le.h, le.tf, le.tw), Se = R, ve = X;
            break;
          case "HSS":
            he = c(le.b, le.h, le.tw ?? le.b * 0.05), Se = R, ve = X;
            break;
          case "CFT":
            he = h(le.b, le.h, le.tw ?? le.b * 0.05), Se = R, ve = X;
            break;
          case "L":
            he = p(le.b ?? le.h, le.h, le.t ?? le.tw ?? 3e-3), Se = R, ve = X;
            break;
          case "2L":
            he = f(le.b ?? le.h, le.h, le.t ?? le.tw ?? 3e-3, le.dis ?? 0.01), Se = R, ve = X;
            break;
          case "C":
          case "coldC":
            he = x(le.b, le.h, le.tf ?? le.t ?? 3e-3, le.tw ?? le.t ?? 3e-3), Se = R, ve = X;
            break;
          case "2C":
            he = b(le.b, le.h, le.tf ?? 5e-3, le.tw ?? 5e-3, le.dis ?? 0.01), Se = R, ve = X;
            break;
          case "T":
            he = C(le.b, le.h, le.tf ?? 0.01, le.tw ?? 6e-3), Se = R, ve = X;
            break;
          case "pipe":
            he = _(le.d, le.tw ?? le.d * 0.05), Se = R, ve = X;
            break;
          default:
            return;
        }
...
function hr(r, e, t) {
  const s = new ft();
  s.name = "extrusion";
  const i = new Qn({ color: 8369151, transparent: true, opacity: 0.92, side: Rt }), n = new Qn({ color: 12623968, transparent: true, opacity: 0.85, side: Rt }), o = new Qn({ color: 11583173, transparent: true, opacity: 0.85, side: Rt }), a = new ft();
  a.add(new Da(16777215, 0.55));
  ...
  return xe.derive(() => {
    ...
    for (const E of [...s.children]) E !== a && (s.remove(E), (_c2 = (_b = E.geometry) == null ? void 0 : _b.dispose) == null ? void 0 : _c2.call(_b));
    ...
    try {
      x.forEach((E, R) => {
        var _a3, _b2, _c3;
        if (E.length === 2) {
          let X = rr(C.get(R)), q = true;
          if (X || (X = cr((_a3 = b.areas) == null ? void 0 : _a3.get(R), (_b2 = b.momentsOfInertiaY) == null ? void 0 : _b2.get(R), (_c3 = b.momentsOfInertiaZ) == null ? void 0 : _c3.get(R)), q = false), !X) return;
          const G = f[E[0]], W = f[E[1]];
          if (!G || !W) return;
          const U = Math.hypot(W[0] - G[0], W[1] - G[1], W[2] - G[2]);
          if (U < 1e-9) return;
          const K = new Il(dr(X), { depth: U, bevelEnabled: false, curveSegments: 4 });
        ...
        if (E.length === 3 || E.length === 4) {
          const X = _.get(R);
          if (!X || X <= 0) return;
          const q = E.map((he) => f[he]).filter(Boolean);
          if (q.length < 3) return;
          const G = [q[1][0] - q[0][0], q[1][1] - q[0][1], q[1][2] - q[0][2]], W = [q[2][0] - q[0][0], q[2][1] - q[0][1], q[2][2] - q[0][2]], U = G[1] * W[2] - G[2] * W[1], K = G[2] * W[0] - G[0] * W[2], D = G[0] * W[1] - G[1] * W[0], P = Math.hypot(U, K, D);
          if (P < 1e-12) return;
          const B = [U / P, K / P, D / P], Q = [], re = (he) => q.map((Se) => [Se[0] + B[0] * he, Se[1] + B[1] * he, Se[2] + B[2] * he]), pe = Math.abs(B[2]) > 0.5, be = B[2] > 0 ? -1 : 1, le = re(pe ? 0 : +X / 2), ne = re(pe ? be * X : -X / 2), ae = (he, Se, ve) => Q.push(...he, ...Se, ...ve);
      ...
      S = String((E == null ? void 0 : E.message) ?? E);
    }
...
function Ha(r, e, t = 0) {
  ...
  if (Math.abs(t) > 1e-12) {
    const f = t * Math.PI / 180, x = Math.cos(f), b = Math.sin(f), C = h.map((S, E) => x * S + b * p[E]), _ = p.map((S, E) => -b * h[E] + x * S);
    h = C, p = _;
  }
...
class gn extends ft {
  constructor(e, t, s, i, n, o, a) {
    ...
    const p = new Cn(l), f = new gt({ color: o[1] > 0 ? 24435 : 11411474, side: Rt });
...
class oo extends ft {
  constructor(e, t, s, i, n, o, a) {
    ...
    if (this.text = new Bt(`${n[0].toFixed(4)}`), this.text2 = new Bt(`${(n[1] * -1).toFixed(4)}`), this.normalizedResult = o, this.textPosition = po(e, t), this.text2Position = po(t, e), this.text.position.set(...this.textPosition), this.text2.position.set(...this.text2Position), this.text.rotation.setFromRotationMatrix(i), this.text2.rotation.setFromRotationMatrix(i), this.add(this.text, this.text2), c) {
      ...
      const S = new Cn(h), E = new Cn(p), R = new gt({ color: o[0] > 0 ? 24435 : 11411474, side: Rt }), X = new gt({ color: -o[1] > 0 ? 24435 : 11411474, side: Rt });
      this.mesh = new pt(S, R), this.mesh2 = new pt(E, X), this.mesh.position.set(...e), this.mesh2.position.set(...e), this.mesh.rotation.setFromRotationMatrix(i), this.mesh2.rotation.setFromRotationMatrix(i), a && this.mesh.rotateX(Math.PI / 2), a && this.mesh2.rotateX(Math.PI / 2), this.add(this.mesh, this.mesh2);
    } else {
      ...
      const x = new Cn(h), b = new gt({ color: o[0] > 0 ? 24435 : 11411474, side: Rt });
...
var Ya = ((r) => (r.normals = "normals", r.shearsY = "shearsY", r.shearsZ = "shearsZ", r.torsions = "torsions", r.bendingsY = "bendingsY", r.bendingsZ = "bendingsZ", r))(Ya || {});
function ur(r, e, t, s) {
  const i = () => {
    const a = t.rawVal;
  ...
  return xe.derive(() => {
    ...
    const a = Ya[e.frameResults.rawVal];
    (_b = (_a2 = r.analyzeOutputs) == null ? void 0 : _a2.rawVal[a]) == null ? void 0 : _b.forEach((l, c) => {
      ...
      if (!p || !f) return;
      const x = new te(...f).distanceTo(new te(...p)), b = pr((_b2 = r.analyzeOutputs) == null ? void 0 : _b2.rawVal[a]), C = ((_f = (_e = (_d = (_c2 = r.elementInputs) == null ? void 0 : _c2.rawVal) == null ? void 0 : _d.localAngles) == null ? void 0 : _e.get) == null ? void 0 : _f.call(_e, c)) ?? 0, _ = Ha(p, f, C), S = Xa(a, _), E = new te(..._.e1), R = new te(...S), X = new Tn().makeBasis(E, R, E.clone().cross(R)), [q, G] = fo(a, l), W = o[a] === oo ? [q, -G] : [q, G], U = W.map((D) => D / (b === 0 ? 1 : b)), K = new o[a](p, f, x, X, W, U, false);
...
function mr(r, e, t, s) {
  ...
  return xe.derive(() => {
    ...
    (_b = (_a2 = r.deformOutputs) == null ? void 0 : _a2.val[n]) == null ? void 0 : _b.forEach((a, l) => {
      const c = new fr(t.rawVal[l], n, a ?? [0, 0, 0, 0, 0, 0]);
...
function gr({ drawingObj: r, gridObj: e, scene: t, getActiveCamera: s, controls: i, gridSize: n, derivedDisplayScale: o, rendererElm: a, viewerRender: l }) {
  var _a2;
  const c = new eo(), h = new Rl(), p = (u) => {
    const g = a.getBoundingClientRect(), y = u.clientX - g.left, m = u.clientY - g.top, v = g.width || 1, A = g.height || 1;
    if (!!window.__hekatanSplitMode) {
      const M = v / 2;
      if (y >= M) return h.x = (y - M) / M * 2 - 1, h.y = -(m / A) * 2 + 1, window.__hekatanSplitCamera ?? s();
      h.x = y / M * 2 - 1;
    } else h.x = y / v * 2 - 1;
    return h.y = -(m / A) * 2 + 1, s();
  }, f = new pt(new Ds(1e4, 1e4), new gt({ side: Rt, transparent: true, opacity: 0, depthWrite: false }));
  f.visible = true, f.frustumCulled = false, t.add(f);
  const x = (u, g, y) => {
    const m = new pt(new Ds(1e4, 1e4), new gt({ side: Rt, transparent: true, opacity: 0, depthWrite: false }));
    return m.rotation.set(u, g, y), m.visible = false, m.frustumCulled = false, t.add(m), m;
  }, b = x(Math.PI / 2, 0, 0), C = x(0, Math.PI / 2, 0);
  let _ = false, S = null, E = null, R = null;
  ...
  const q = (u, g, y) => Math.round(u * 1e3) + "," + Math.round(g * 1e3) + "," + Math.round(y * 1e3), G = (u) => {
    ...
    for (let y = 0; y + 0 < u.length / 6; y++) {
      ...
      for (const v of [q(u[m], u[m + 1], u[m + 2]), q(u[m + 3], u[m + 4], u[m + 5])]) {
        const A = g.get(v);
        A ? A.push(y) : g.set(v, [y]);
      }
    }
    return g;
  }, W = (u) => {
    const { S: g, adj: y } = u, m = (V, H) => new te(g[6 * V + 3 * H], g[6 * V + 3 * H + 1], g[6 * V + 3 * H + 2]), v = /* @__PURE__ */ new Set([u.s]), A = (V, H) => {
      const ee = [];
      let Z = V, J = H;
      for (let se = 0; se < 3e3; se++) {
        const oe = (y.get(q(J.x, J.y, J.z)) || []).filter((je) => !v.has(je));
        if (oe.length !== 1) break;
        const ie = oe[0], de = m(ie, 0), me = m(ie, 1), ke = de.distanceTo(J) < me.distanceTo(J) ? me : de, Ee = J.clone().sub(Z).normalize(), nt = ke.clone().sub(J).normalize();
        if (Ee.dot(nt) < Math.cos(35 * Math.PI / 180)) break;
        v.add(ie), ee.push(ke), Z = J, J = ke;
      }
      return ee;
    }, k = m(u.s, 0), M = m(u.s, 1), $ = A(k, M), I = A(M, k), T = [...I.reverse(), k, M, ...$], N = I.length;
    ...
    const m = String(window.__hekatanArcModo ?? "angulo"), v = m === "x" ? 0 : m === "y" ? 1 : m === "z" ? 2 : -1, A = [0];
    for (let N = 1; N < u.length; N++) A.push(A[N - 1] + u[N].distanceTo(u[N - 1]));
    const k = (N, F) => {
    ...
    for (let N = 0; N <= y; N++) {
      const F = T ? k((L) => L.getComponent(v), $ + (I - $) * N / y) : k((L, Y) => A[Y], A[A.length - 1] * N / y);
      M.push([F.x, F.y, F.z]);
    }
    return M[0] = u[0].toArray(), M[y] = u[u.length - 1].toArray(), M;
  };
  window.__hekatanCadenaIfc = () => (R || []).map((u) => [u.x, u.y, u.z]);
  const D = /* @__PURE__ */ new Map(), P = (u) => {
    const g = D.get(u.id);
    if (g) return g;
    const y = u.geometry.getAttribute("position"), m = y ? Math.floor(y.count / 3) : 0, v = new Float64Array(m * 9), A = new Float64Array(m * 3), k = new Int32Array(m * 3).fill(-1);
    if (y) {
      u.updateMatrixWorld();
      const $ = new te();
      ...
      for (let Y = 0; Y < m; Y++) {
        const j = 3 * Y;
        I.set(v[3 * (j + 1)] - v[3 * j], v[3 * (j + 1) + 1] - v[3 * j + 1], v[3 * (j + 1) + 2] - v[3 * j + 2]), T.set(v[3 * (j + 2)] - v[3 * j], v[3 * (j + 2) + 1] - v[3 * j + 1], v[3 * (j + 2) + 2] - v[3 * j + 2]), N.crossVectors(I, T).normalize(), A[3 * Y] = N.x, A[3 * Y + 1] = N.y, A[3 * Y + 2] = N.z;
        for (let O = 0; O < 3; O++) {
    ...
    const M = { V: v, N: A, vec: k, n: m };
    return D.set(u.id, M), M;
  }, B = new pt(new ze(), new gt({ color: 3718648, transparent: true, opacity: 0.35, depthTest: false, side: Rt }));
  B.name = "ref-ifc-cara", B.renderOrder = 999, B.frustumCulled = false, B.visible = false, t.add(B);
  let Q = null;
  const re = (u, g) => {
    const y = Math.cos(12 * Math.PI / 180), m = Math.cos(80 * Math.PI / 180), v = [u.N[3 * g], u.N[3 * g + 1], u.N[3 * g + 2]], A = new Uint8Array(u.n), k = [], M = [g];
    for (A[g] = 1; M.length && k.length < 4e4; ) {
      const $ = M.pop();
      k.push($);
      for (let I = 0; I < 3; I++) {
        const T = u.vec[3 * $ + I];
    ...
    const m = P(u), v = re(m, g), A = new Float32Array(v.length * 9), k = new te();
    let M = true;
    v.forEach(($, I) => {
      for (let T = 0; T < 9; T++) A[9 * I + T] = m.V[9 * $ + T];
    ...
    B.geometry.dispose(), B.geometry = new ze(), B.geometry.setAttribute("position", new bt(A, 3)), B.material.color.set(M ? 3718648 : 16096779), B.visible = true, Q = { m: u, t0: g, tris: v, normal: k, plana: M, punto: y.clone() };
  }, be = (u, g) => {
    const y = new Uint8Array(u.n);
    for (const T of g) y[T] = 1;
    const m = (T) => Math.round(u.V[3 * T] * 1e3) + "," + Math.round(u.V[3 * T + 1] * 1e3) + "," + Math.round(u.V[3 * T + 2] * 1e3), v = /* @__PURE__ */ new Map(), A = /* @__PURE__ */ new Map();
    for (const T of g) for (let N = 0; N < 3; N++) {
      const F = u.vec[3 * T + N];
      if (F >= 0 && y[F]) continue;
      const L = 3 * T + N, Y = 3 * T + (N + 1) % 3, j = m(L), O = m(Y);
      A.set(j, new te(u.V[3 * L], u.V[3 * L + 1], u.V[3 * L + 2])), A.set(O, new te(u.V[3 * Y], u.V[3 * Y + 1], u.V[3 * Y + 2])), (v.get(j) || v.set(j, []).get(j)).push(O), (v.get(O) || v.set(O, []).get(O)).push(j);
    }
  ...
  window.__hekatanRaycast = (u, g, y, m = 2) => {
    const v = new te(u[0], u[1], u[2]), A = new te(g[0], g[1], g[2]).normalize();
    let k = null;
    for (const M of [1, -1]) {
      const I = new eo(v, A.clone().multiplyScalar(M), 0, m).intersectObjects(y, false);
      I.length && (k == null || I[0].distance < k) && (k = I[0].distance);
    }
    return k;
  }, window.__hekatanCaraIfc = () => Q ? { tris: Q.tris.length, plana: Q.plana, normal: Q.normal.toArray(), punto: Q.punto.toArray(), contorno: be(P(Q.m), Q.tris).map((u) => [u.x, u.y, u.z]) } : null;
  ...
  const ue = 1, he = (u, g, y) => Math.floor(u / ue) + "," + Math.floor(g / ue) + "," + Math.floor(y / ue), Se = (u) => {
    ...
    const A = { segs: new Float32Array(m), celdas: v };
    return ne.set(u.id, A), A;
  };
  let ve = "";
  const Oe = (u) => {
    ...
    let A = 0;
    for (const k of y) v.set(k, A), A += k.length;
  ...
  const He = (u, g) => {
    ...
    if (!y) return null;
    const m = Se(u), v = m.segs, A = Math.floor(g.x / ue), k = Math.floor(g.y / ue), M = Math.floor(g.z / ue), $ = /* @__PURE__ */ new Set();
    let I = Fs, T = null, N = Fs, F = null, L = -1;
    const Y = new te(), j = new te();
    for (let O = -1; O <= 1; O++) for (let z = -1; z <= 1; z++) for (let V = -1; V <= 1; V++) {
      const H = m.celdas.get(A + O + "," + (k + z) + "," + (M + V));
      if (H) for (const ee of H) {
        ...
        let Ee = ((y.x - J.x) * de + (y.y - J.y) * me) / ke;
        Ee = Math.max(0, Math.min(1, Ee));
        const nt = Math.hypot(y.x - (J.x + Ee * de), y.y - (J.y + Ee * me));
        nt < N && (N = nt, F = Y.clone().lerp(j, Ee), L = ee);
      }
    }
    return L >= 0 && (m.adj || (m.adj = G(m.segs)), E = { S: m.segs, adj: m.adj, s: L }), T ? { tipo: "ifcVert", punto: T } : F ? { tipo: "ifcEdge", punto: F } : null;
  }, dt = () => {
    var _a3, _b, _c2;
    ...
    if (t.traverse((A) => {
      var _a4;
      ((_a4 = A.userData) == null ? void 0 : _a4.refIfc) && A.isMesh && u.push(A);
    }), !u.length) return ae.visible = false, ve = "", null;
    Oe(u);
    const g = c.intersectObjects(u, false).filter((A) => {
      const k = A.object.material;
      return (k && k.clippingPlanes || []).every(($) => $.distanceToPoint(A.point) >= 0);
    });
    ...
    if (m && m.object === y.object && m.distance - y.distance <= 1.2) {
      const A = y.point.clone().add(m.point).multiplyScalar(0.5);
      return S = { tipo: "ifcAxis" }, [{ ...y, point: A }];
    }
    return S = { tipo: "ifc" }, [y];
  };
  ...
  const _e = () => {
    ...
    t.traverse((A) => {
      var _a3;
      ((_a3 = A.userData) == null ? void 0 : _a3.refIfc) && A.isMesh && y.push(A);
    });
    const m = JSON.stringify(g) + "|" + y.map((A) => A.id).join(",");
    if (m === wt) return Ce;
    wt = m;
    const v = [];
    if (g.length && y.length) {
      const A = [new te(), new te(), new te()];
      for (const k of y) {
        const M = k.geometry.getAttribute("position");
        if (M) {
          k.updateMatrixWorld();
          for (let $ = 0; $ + 2 < M.count; $ += 3) {
            for (let I = 0; I < 3; I++) A[I].fromBufferAttribute(M, $ + I).applyMatrix4(k.matrixWorld);
            for (const [I, T] of g) {
              const N = [A[0].getComponent(I) - T, A[1].getComponent(I) - T, A[2].getComponent(I) - T], F = [];
              for (let L = 0; L < 3; L++) {
                const Y = A[L], j = A[(L + 1) % 3], O = N[L], z = N[(L + 1) % 3];
  ...
  const Le = (u, g) => {
    ...
    for (let $ = 0; $ + 5 < y.length; $ += 6) {
      ...
      j < m && (m = j, v = k.clone().lerp(M, Y), A = $ / 6);
    }
    return A >= 0 && (ye !== y && (ge = G(y), ye = y), E = { S: y, adj: ge, s: A }), v;
  };
  let Te = null;
  ...
  const Me = () => {
    ...
    if (b.visible = !!window.__hekatanGridPlaneXZ, C.visible = !!window.__hekatanGridPlaneYZ, window.__hekatanOrthoRaycast === true && as.visible) {
      const m = c.intersectObjects([as, fs, ms], false);
      if (m.length > 0) return m;
    }
    const y = [f];
    return b.visible && y.push(b), C.visible && y.push(C), As.visible && ri.length > 0 && y.push(...ri), c.intersectObjects(y, false);
  ...
  Ae.id = "hk-rubber-label", Ae.type = "text", Ae.spellcheck = false, Ae.title = `Sintaxis estilo AutoCAD:
  5         \u2192 5m en direcci\xF3n del cursor (DDE)
  5,3,2     \u2192 coordenada absoluta (X,Y,Z)
  @5,3,2    \u2192 relativa al \xFAltimo punto
  5<45      \u2192 polar 2D: 5m a 45\xB0 desde origen
  @5<45     \u2192 polar relativa: 5m a 45\xB0 del \xFAltimo punto
  @5<45<30  \u2192 esf\xE9rica 3D: 5m, azimuth 45\xB0, elevaci\xF3n 30\xB0`, Ae.style.cssText = ["position:fixed", "z-index:99996", "padding:3px 8px", "background:rgba(15,23,42,0.92)", "color:#22d3ee", "border:1.5px solid #22d3ee", "border-radius:4px", "font-family:Consolas,monospace", "font-size:13px", "font-weight:bold", "transform:translate(-50%,-50%)", "white-space:nowrap", "outline:none", "width:80px", "text-align:center", "display:none", "pointer-events:none"].join(";") + ";", document.body.appendChild(Ae);
  ...
  const Ve = new te(), Pt = (u, g, y, m, v, A) => {
    const k = m - u, M = v - g, $ = A - y, I = Math.hypot(k, M, $);
    if (I < 0.01) {
      Ae.style.display = "none";
      return;
    }
    Ye = [u, g, y], Ze = [k / I, M / I, $ / I], Ve.set((u + m) / 2, (g + v) / 2, (y + A) / 2), Ve.project(s());
    const T = a.getBoundingClientRect(), N = T.left + (Ve.x * 0.5 + 0.5) * T.width, F = T.top + (-Ve.y * 0.5 + 0.5) * T.height;
    Ae.style.left = N + "px", Ae.style.top = F + "px", Ae.style.display = "block";
    const L = new te(u, g, y).project(s()), Y = new te(m, v, A).project(s()), j = T.left + (L.x * 0.5 + 0.5) * T.width, O = T.top + (-L.y * 0.5 + 0.5) * T.height, z = T.left + (Y.x * 0.5 + 0.5) * T.width, V = T.top + (-Y.y * 0.5 + 0.5) * T.height;
    ...
    if (H < 0 && (H += 360), tt.textContent = `${Math.round(H) % 360}\xB0`, tt.style.left = z + "px", tt.style.top = V + 34 + "px", tt.style.display = "block", !$e) {
      if (Ae.value = `${I.toFixed(2)} m`, document.activeElement !== Ae) {
        const ee = document.activeElement;
        ee && (ee.tagName === "INPUT" || ee.tagName === "TEXTAREA") && ee !== Ae || Ae.focus({ preventScroll: true });
      }
      try {
        Ae.select();
      } catch {
    ...
    Ae.style.display = "none", tt.style.display = "none", Ye = null, Ze = null, $e = false, document.activeElement === Ae && Ae.blur();
  }, Et = (u) => {
    ...
    if (g === "offset") {
      Zs = u, we(`\u21C9 DESFASE distancia ${u} m \u2014 designe la l\xEDnea y luego el lado.`), Ae.blur();
      try {
    ...
    if (g === "col" || g === "wall" || g === "extp" || g === "extl") {
      It = u, we(`\u{1F4D0} Altura ${u}m memorizada \u2014 hac\xE9 el click para crear ${{ col: "columna", wall: "pared", extp: "extrusi\xF3n punto\u2192l\xEDnea", extl: "extrusi\xF3n l\xEDnea\u2192\xE1rea" }[g]}.`), Ae.blur();
      return;
    }
    ...
    const A = Ye[0] + y * u, k = Ye[1] + m * u, M = Ye[2] + v * u;
    window.__hekatanPushUndo && window.__hekatanPushUndo(), r.points.val = [...r.points.rawVal, [A, k, M]];
    const $ = r.polylines.rawVal, I = $.length ? $[$.length - 1] : [];
    r.polylines.val = [...$.slice(0, -1), [...I, r.points.rawVal.length - 1]], Ae.blur();
    try {
    ...
    if (y && (g = g.slice(1)), g.includes("<")) {
      const v = g.split("<").map((A) => parseFloat(A.trim()));
      if (v.some(isNaN)) return null;
      if (v.length === 2) {
        const [A, k] = v;
        return y ? { kind: "relPolar", L: A, ang: k } : { kind: "absPolar", L: A, ang: k };
      }
      if (v.length === 3 && y) {
        const [A, k, M] = v;
        return { kind: "relSpherical", L: A, az: k, el: M };
      }
      return null;
    }
    if (g.includes(",")) {
      ...
      if (v.some(isNaN)) return null;
      const [A, k, M = 0] = v;
      return y ? { kind: "relCart", dx: A, dy: k, dz: M } : { kind: "absCart", x: A, y: k, z: M };
    }
    ...
    r.polylines.val = [...g.slice(0, -1), [...y, r.points.rawVal.length - 1]], Ye = u, Ae.blur();
    try {
  ...
  window.__hekatanTypeCoord = (u) => {
    ...
    const g = jt(u);
    if (!g) return false;
    if (g.kind === "length") return Et(g.L), true;
    const y = at(g);
    if (!y) return false;
    Jo(new te(y[0], y[1], y[2]), null), Ye = y, Ae.blur();
    try {
      (_a3 = window.__hekatanCadRefreshPrompt) == null ? void 0 : _a3.call(window);
    } catch {
    }
    return true;
  }, Ae.addEventListener("keydown", (u) => {
    var _a3, _b, _c2;
    if (u.key === "Enter") {
      ...
      const y = jt(Ae.value);
      if (!y) return;
      if ($e = false, y.kind === "length") Et(y.L), we(`\u270F DDE ${y.L}m aplicado en direcci\xF3n actual`);
      else {
        const m = at(y);
        if (!m) return;
        Qe(m);
    ...
    if (u.key === "Escape") {
      u.preventDefault(), $e = false, Ae.blur();
      return;
    }
    const g = u.key.toLowerCase();
    if (g === "x" || g === "y" || g === "z") {
      u.preventDefault(), setTimeout(() => {
        if (!$e && Ae.style.display === "block") try {
          Ae.select();
        } catch {
        }
      }, 0);
    ...
    if (!Ye || !Ze || document.activeElement === Ae) return;
    const g = document.activeElement;
    g && (g.tagName === "INPUT" || g.tagName === "TEXTAREA") || /^[0-9.\-]$/.test(u.key) && (Ae.value = u.key, Ae.focus(), Ae.setSelectionRange(1, 1), u.preventDefault());
  });
  ...
  const Pe = new Vt(new ze().setFromPoints([new te(0, 0, 0), new te(0, 0, 0)]), new Ti({ color: 2282478, dashSize: 0.2, gapSize: 0.1, transparent: true, opacity: 0.85, linewidth: 2 }));
  Pe.frustumCulled = false, Pe.visible = false, Pe.name = "rubberBand", t.add(Pe), window.__hekatanRubberBand = Pe;
  ...
  const Wt = (u) => {
    ...
    if (!y) return null;
    const A = a.getBoundingClientRect(), k = (F) => {
      const L = new te(F[0], F[1], F[2]).project(g);
      return [A.left + (L.x * 0.5 + 0.5) * A.width, A.top + (-L.y * 0.5 + 0.5) * A.height];
    ...
    const u = s(), g = Ke[0], y = Ke[1] ?? Ke[0], v = new te((g[0] + y[0]) / 2, (g[1] + y[1]) / 2, (g[2] + y[2]) / 2).clone().project(u), A = a.getBoundingClientRect();
    it.style.left = A.left + (v.x * 0.5 + 0.5) * A.width + "px", it.style.top = A.top + (-v.y * 0.5 + 0.5) * A.height - 14 + "px", it.style.display = "block";
  };
  ...
  const _t = new pt(new ze(), new gt({ color: 16096779, transparent: true, opacity: 0.35, side: Rt, depthWrite: false }));
  ...
  const ks = (u) => {
    ...
    for (const O of $) for (const z of m.get(O)) if (!(z < O)) {
      for (const V of m.get(z)) if (V !== O) for (const H of m.get(V)) {
        if (H === O || H === z || !A(H, O) || A(O, V) || A(z, H)) continue;
    ...
    return Y;
  }, Lt = new ft(), Ht = new pt(new Ds(1, 1), new gt({ color: 2282478, transparent: true, opacity: 0.08, side: Rt, depthWrite: false })), Cs = new is(new aa(new Ds(1, 1)), new mt({ color: 2282478, transparent: true, opacity: 0.85 })), Bs = new is(new ze(), new mt({ color: 2282478, transparent: true, opacity: 0.3 })), qi = (u, g) => {
    const y = [], m = Math.ceil(u / g);
    for (let v = -m; v <= m; v++) {
      const A = v * g;
      y.push(-u, A, 0, u, A, 0), y.push(A, -u, 0, A, u, 0);
    }
  ...
  }, Us = _i(3462041), oi = _i(16724804), ai = _i(6333946), ps = new ft();
  ps.frustumCulled = false, ps.visible = false, t.add(ps), ps.add(Us, oi, ai);
  const li = (u) => {
    const g = new Ds(1, 1), y = new gt({ color: u, transparent: true, opacity: 0.06, side: Rt, depthWrite: false }), m = new pt(g, y);
    return m.frustumCulled = false, m.renderOrder = 996, m;
  }, as = li(3462041), fs = li(16724804), ms = li(6333946);
  ps.add(as, fs, ms);
  const Ms = (u, g, y, m) => {
  ...
  ws.id = "hk-refplane-badge", ws.style.cssText = ["position:fixed", "pointer-events:none", "z-index:99997", "padding:3px 10px", "border-radius:4px", "font-family:Consolas,monospace", "font-size:12px", "font-weight:bold", "transform:translate(20px,40px)", "white-space:nowrap", "display:none"].join(";") + ";", document.body.appendChild(ws), window.__hekatanSetOrthoPlanes = (u) => {
    var _a3;
    if (window.__hekatanShowOrthoPlanes = u, ps.visible = u, u) {
      const g = window.__hekatanOrthoAnchor, y = ((_a3 = r.polylines) == null ? void 0 : _a3.rawVal) ?? [], m = y[y.length - 1] ?? [], v = r.points.rawVal ?? [], A = g && g.length === 3 ? g : m.length > 0 && v[m[m.length - 1]] ? v[m[m.length - 1]] : [0, 0, 0], k = window.__hekatanOrthoExt ?? 8;
      $s(Us, A, "xy", k), $s(oi, A, "xz", k), $s(ai, A, "yz", k), Ms(as, A, "xy", k), Ms(fs, A, "xz", k), Ms(ms, A, "yz", k), as.material.opacity = 0.05, fs.material.opacity = 0.05, ms.material.opacity = 0.05;
    } else {
    ...
    var _a3;
    if (window.__hekatanOrthoExt = u, !ps.visible) {
      l();
      return;
    }
    const g = window.__hekatanOrthoAnchor, y = ((_a3 = r.polylines) == null ? void 0 : _a3.rawVal) ?? [], m = y[y.length - 1] ?? [], v = r.points.rawVal ?? [], A = g && g.length === 3 ? g : m.length > 0 && v[m[m.length - 1]] ? v[m[m.length - 1]] : [0, 0, 0];
    $s(Us, A, "xy", u), $s(oi, A, "xz", u), $s(ai, A, "yz", u), Ms(as, A, "xy", u), Ms(fs, A, "xz", u), Ms(ms, A, "yz", u), l();
  };
  const Mo = (u) => {
    if (as.material.opacity = u === "xy" ? 0.09 : 0.025, fs.material.opacity = u === "xz" ? 0.09 : 0.025, ms.material.opacity = u === "yz" ? 0.09 : 0.025, u) {
      ...
      ws.style.background = v.bg, ws.style.color = v.text, ws.textContent = `\u25A6 Plano ${u.toUpperCase()}`, ws.style.display = "block";
  ...
  window.addEventListener("keydown", (u) => {
    ...
    const g = document.activeElement;
    if (g && (g.tagName === "INPUT" || g.tagName === "TEXTAREA") && g !== Ae) return;
    ...
    if (y === "x" || y === "y" || y === "z") zt = zt === y ? null : y, $o(), u.preventDefault();
    else if (u.key === "Escape") {
      const v = document.activeElement;
      v && (v.tagName === "INPUT" || v.tagName === "TEXTAREA") && v.blur(), Wo(), u.preventDefault();
  ...
  const Wi = new te(), Zi = new te(), Ao = new te(), el = (u) => {
    if (!zt) return null;
    const g = u[0], y = u[1], m = u[2];
    return zt === "x" ? (Wi.set(g - 1e4, y, m), Zi.set(g + 1e4, y, m)) : zt === "y" ? (Wi.set(g, y - 1e4, m), Zi.set(g, y + 1e4, m)) : (Wi.set(g, y, m - 1e4), Zi.set(g, y, m + 1e4)), c.ray.distanceSqToSegment(Wi, Zi, null, Ao), Ao;
  };
  window.__hekatanProjectOnAxis = el;
  ...
  const ys = new Vt(new ze().setFromPoints([new te(), new te()]), new mt({ color: 16766720, transparent: true, opacity: 0.95, depthTest: false }));
  ys.renderOrder = 997, ys.frustumCulled = false, ys.visible = false, t.add(ys);
  const ls = new pt(new gi(0.02, 12, 12), new gt({ color: 16766720, transparent: true, opacity: 0.9, depthTest: false }));
  ls.renderOrder = 998, ls.visible = false, t.add(ls);
  const Ki = (u) => {
    const g = s();
    ...
    ls.visible && ls.scale.setScalar(Ki(ls.position));
  }, xs = new ft();
  xs.frustumCulled = false, t.add(xs);
  const Gi = 2282478;
  let bs = null;
  const tl = (u, g, y, m) => {
    ...
    var _a3, _b, _c2, _d, _e2, _f, _g, _h;
    for (; xs.children.length; ) {
      const k = xs.children.pop();
    ...
    for (const k of Ue) {
      ...
      if (M === "pt") {
        ...
        T.position.set(I[0], I[1], I[2]), T.renderOrder = 999, T.__isSelectionPt = true, xs.add(T);
      } else if (M === "seg") {
        ...
        N.renderOrder = 999, xs.add(N);
      }
    }
    const v = window.__hekatanUpdateSelectionPtScale;
    v && v();
    const A = window.__hekatanRefreshPropsPane;
    A && A();
    try {
  ...
  window.__hekatanRefreshSelection = rs, window.__hekatanSelectIds = (u) => {
    var _a3;
    Ue.clear();
  ...
  const ki = (u, g, y, m, v, A, k, M, $) => {
    const I = k - m, T = M - v, N = $ - A, F = I * I + T * T + N * N;
    if (F < 1e-12) return Math.hypot(u - m, g - v, y - A);
    let L = ((u - m) * I + (g - v) * T + (y - A) * N) / F;
    L = Math.max(0, Math.min(1, L));
    const Y = m + L * I, j = v + L * T, O = A + L * N;
    return Math.hypot(u - Y, g - j, y - O);
  }, Rn = (u, g, y, m) => {
    if (!r.polylines) return null;
    const v = r.polylines.rawVal, A = r.points.rawVal;
    let k = -1, M = -1, $ = m;
    for (let I = 0; I < v.length; I++) {
      const T = v[I];
      for (let N = 0; N < T.length - 1; N++) {
        const F = A[T[N]], L = A[T[N + 1]];
        if (!F || !L) continue;
    ...
    const v = window.__hekatanDrawingAuxLines, A = (v == null ? void 0 : v.rawVal) ?? (v == null ? void 0 : v.val) ?? v ?? [];
    let k = -1, M = m;
    for (let $ = 0; $ < A.length; $++) {
      const I = A[$];
      if (!I || I.length !== 6) continue;
    ...
    const v = ((_b = (_a3 = r.areas) == null ? void 0 : _a3.rawVal) == null ? void 0 : _b.includes(u)) ?? false, A = [];
    if (v || g < 0 || g >= y.length - 1) for (const k of y) {
      const M = m[k];
      M && A.push(new te(M[0], M[1], M[2]));
    }
    else {
      const k = m[y[g]], M = m[y[g + 1]];
      k && A.push(new te(k[0], k[1], k[2])), M && A.push(new te(M[0], M[1], M[2]));
    }
    Qt.geometry.setFromPoints(A), Qt.visible = true;
  }, Ji = (u) => {
    var _a3;
    if (!r.polylines) return;
    ...
    const v = r.points.rawVal, A = /* @__PURE__ */ new Map(), k = [];
    for (let $ = 0; $ < v.length; $++) m.has($) && (A.set($, k.length), k.push(v[$]));
    const M = y.map(($) => $.map((I) => A.get(I)).filter((I) => I !== void 0));
    r.points.val = k, r.polylines.val = M, r.areas && (r.areas.val = r.areas.rawVal.filter(($) => $ !== u).map(($) => $ > u ? $ - 1 : $)), Qt.visible = false, gs = -1, zs = -1;
    try {
      (_a3 = window.__hekatanRebuild) == null ? void 0 : _a3.call(window);
    } catch {
    }
    ...
    let A;
    g === 0 ? A = [v.slice(1)] : g === v.length - 2 ? A = [v.slice(0, -1)] : A = [v.slice(0, g + 1), v.slice(g + 1)];
    const k = [...y.slice(0, u), ...A, ...y.slice(u + 1)], M = /* @__PURE__ */ new Set();
  ...
  const Ro = () => {
    var _a3;
    const u = r.points.rawVal, g = ((_a3 = r.polylines) == null ? void 0 : _a3.rawVal) ?? [], y = `${u.length}|${g.length}|${g.reduce((v, A) => v + A.length, 0)}`;
    if (y === Fo) return Io;
    Fo = y;
    const m = [];
    for (const v of g) {
      const A = v.length;
      if (A < 6 || v[0] !== v[A - 1]) continue;
      const k = v.slice(0, A - 1).map((T) => u[T]).filter(Boolean);
  ...
  window.__hekatanDrawCircle = (u, g, y, m, v = window.__hekatanArcSegs ?? 12, A = "xy") => {
    ...
    var _a3;
    const v = Math.max(4, Math.round(m)), A = new te(...u), k = new te(...g), M = new te(...y), $ = new te().subVectors(k, A), I = new te().subVectors(M, A), T = new te().crossVectors($, I), N = 2 * T.lengthSq();
    let F;
    if (N < 1e-12) F = new te().addVectors(A, M).multiplyScalar(0.5);
    else {
      const ke = I.clone().multiplyScalar($.lengthSq()).sub($.clone().multiplyScalar(I.lengthSq())), Ee = new te().crossVectors(ke, T);
      F = A.clone().add(Ee.divideScalar(N));
    }
    const L = A.distanceTo(F), Y = T.lengthSq() > 1e-12 ? T.clone().normalize() : new te(0, 1, 0), j = new te().subVectors(A, F).normalize(), O = new te().crossVectors(Y, j).normalize(), z = (ke) => {
      const Ee = new te().subVectors(ke, F);
      return Math.atan2(Ee.dot(O), Ee.dot(j));
    }, V = (ke) => {
      let Ee = ke;
      for (; Ee < 0; ) Ee += 2 * Math.PI;
      for (; Ee >= 2 * Math.PI; ) Ee -= 2 * Math.PI;
      return Ee;
    }, H = V(z(k)), ee = V(z(M)), Z = H <= ee ? ee : ee - 2 * Math.PI, J = r.points.rawVal.length, se = [], oe = (ke) => {
      const Ee = j.clone().multiplyScalar(Math.cos(ke)).add(O.clone().multiplyScalar(Math.sin(ke)));
      return F.clone().add(Ee.multiplyScalar(L));
    }, ie = String(window.__hekatanArcModo ?? "angulo"), de = ie === "x" ? 0 : ie === "y" ? 1 : ie === "z" ? 2 : -1;
    let me = false;
    if (de >= 0) {
      const ke = u[de], Ee = y[de], nt = 512;
      let je = Math.abs(Ee - ke) > 1e-9, et = ke;
      for (let De = 1; De <= nt && je; De++) {
        const Je = oe(Z * De / nt).getComponent(de);
        (Je - et) * (Ee - ke) < -1e-9 && (je = false), et = Je;
      }
      if (je) {
        me = true;
        for (let De = 0; De <= v; De++) {
          const Je = ke + (Ee - ke) * De / v;
          let Be = 0, Re = Z;
          for (let We = 0; We < 60; We++) {
            const xt = (Be + Re) / 2;
            (oe(xt).getComponent(de) - Je) * (Ee - ke) < 0 ? Be = xt : Re = xt;
          }
          const Xe = oe((Be + Re) / 2);
    ...
    if (r.points.val = [...r.points.rawVal, ...se], r.polylines) {
      const ke = se.map((nt, je) => J + je), Ee = r.polylines.rawVal;
      r.polylines.val = [...Ee.slice(0, -1), ke, []];
    }
  }, window.__hekatanDrawPolinomio = (u, g = window.__hekatanArcSegs ?? 12) => {
    var _a3, _b, _c2, _d;
    const y = u.length;
    if (y < 2) return { ok: false, msg: "faltan puntos" };
    const m = Math.max(y - 1, Math.round(g)), v = (Z) => Math.max(...u.map((J) => J[Z])) - Math.min(...u.map((J) => J[Z])), A = [v(0), v(1), v(2)], k = String(((_c2 = (_b = (_a3 = window.__hekatanCadState) == null ? void 0 : _a3.get) == null ? void 0 : _b.call(_a3)) == null ? void 0 : _c2.workPlane) ?? ""), M = k === "xy" ? 2 : k === "xz" ? 1 : k === "yz" ? 0 : -1, $ = M >= 0 && A[M] < 1e-6 ? M : A[2] <= A[0] && A[2] <= A[1] ? 2 : A[1] <= A[0] ? 1 : 0, I = $ === 2 ? "xy" : $ === 1 ? "xz" : "yz", T = [0, 1, 2].filter((Z) => Z !== $), [N, F] = A[T[0]] >= A[T[1]] ? T : [T[1], T[0]], L = u.map((Z) => Z[N]), Y = u.map((Z) => Z[F]);
    ...
    const j = (Z) => {
      ...
      for (let ie = 0; ie < Z; ie++) {
        ...
        for (let me = ie + 1; me < Z; me++) {
          ...
          for (let Ee = ie; Ee < Z; Ee++) J[me][Ee] -= ke * J[ie][Ee];
          se[me] -= ke * se[ie];
        }
      }
  ...
  const Po = () => {
    var _a3, _b;
    const u = r.points.rawVal, g = ((_a3 = r.polylines) == null ? void 0 : _a3.rawVal) ?? [], y = new Set(((_b = r.areas) == null ? void 0 : _b.rawVal) ?? []), m = window.__hekatanDrawingAuxLines, v = (m == null ? void 0 : m.rawVal) ?? (m == null ? void 0 : m.val) ?? [], A = [], k = [], M = /* @__PURE__ */ new Set(), $ = (I) => [u[I][0], u[I][1], u[I][2]];
    return [...Ue].forEach((I) => {
      const T = I.split(":");
      if (T[0] === "aux") {
        const F = v[+T[1]];
        F && F.length === 6 && (A.push([[F[0], F[1], F[2]], [F[3], F[4], F[5]]]), k.push(I));
        return;
      }
      ...
      if (!(N < 0 || !g[N] || y.has(N))) if (T[0] === "poly") {
        if (M.has(N)) return;
        M.add(N);
        for (let F = 0; F + 1 < g[N].length; F++) A.push([$(g[N][F]), $(g[N][F + 1])]);
      } else {
        const F = g[N][+T[2]], L = g[N][+T[2] + 1];
        F != null && L != null && !M.has(N) && A.push([$(F), $(L)]);
      }
    }), { segs: A, auxIds: k };
  }, Ci = (u, g) => Math.abs(u[0] - g[0]) < 1e-6 && Math.abs(u[1] - g[1]) < 1e-6 && Math.abs(u[2] - g[2]) < 1e-6, nl = (u) => {
    ...
    for (let m = 0; m < u.length; m++) {
      ...
      for (; A; ) {
        ...
        for (let M = 0; M < u.length; M++) {
          ...
          Ci($, T) ? (v.push(I), g[M] = true, A = true) : Ci(I, T) ? (v.push($), g[M] = true, A = true) : Ci(I, N) ? (v.unshift($), g[M] = true, A = true) : Ci($, N) && (v.unshift(I), g[M] = true, A = true);
        }
      }
  ...
  window.__hekatanRevolveSelection = (u, g, y, m = 360) => {
    var _a3, _b, _c2;
    const v = Math.max(3, Math.round(y || 16)), A = Math.abs(m - 360) < 1e-9, k = v, M = A ? v : v + 1, { segs: $, auxIds: I } = Po();
    ...
    if (A && v % 2) return { anillos: 0, areas: 0, polo: false, guias: 0, msg: "con el perfil tocando el eje, los sectores tienen que ser PARES (cometas en el polo)" };
    At();
    ...
    const Y = [...((_b = r.areas) == null ? void 0 : _b.rawVal) ?? []], j = /* @__PURE__ */ new Map(), O = (se) => se.map((oe) => Math.round(oe * 1e4)).join(","), z = (se) => Math.hypot(se[0] - u, se[1] - g) < 1e-6, V = (se) => {
      ...
      let ie = j.get(oe);
      if (ie) return ie;
      if (z(se)) return ie = [Pn(F, se)], j.set(oe, ie), ie;
      const de = Math.hypot(se[0] - u, se[1] - g), me = Math.atan2(se[1] - g, se[0] - u);
      ie = [];
      for (let ke = 0; ke < M; ke++) {
        const Ee = me + m * Math.PI / 180 * ke / v;
        ie.push(Pn(F, ke === 0 ? se : [u + de * Math.cos(Ee), g + de * Math.sin(Ee), se[2]]));
      }
    ...
    for (const [se, oe] of $) {
      ...
      if (!(ie.length === 1 && de.length === 1)) {
        if (ie.length === 1 || de.length === 1) {
          ...
          for (let Ee = 0; Ee + 2 <= k; Ee += 2) Z([me, ke[Ee % M], ke[(Ee + 1) % M], ke[(Ee + 2) % M]]);
          continue;
        }
    ...
    var _a3, _b, _c2;
    const { segs: y, auxIds: m } = Po(), v = nl(y), A = (de) => de.pts.every((me) => Math.abs(me[2] - de.pts[0][2]) < 1e-6), k = v.find((de) => de.cerrada && A(de)), M = v.find((de) => !de.cerrada && de.pts.length >= 2 && !A(de));
    ...
    const F = N > 0 ? 1 : -1, L = (de) => {
      const me = $[(de - 1 + I) % I], ke = $[de], Ee = $[(de + 1) % I], nt = [ke[0] - me[0], ke[1] - me[1]], je = [Ee[0] - ke[0], Ee[1] - ke[1]], et = Math.hypot(nt[0], nt[1]) || 1, De = Math.hypot(je[0], je[1]) || 1, Je = [F * nt[1] / et, -F * nt[0] / et], Be = [F * je[1] / De, -F * je[0] / De], Re = 1 + (Je[0] * Be[0] + Je[1] * Be[1]);
      return [(Je[0] + Be[0]) / Math.max(Re, 1e-6), (Je[1] + Be[1]) / Math.max(Re, 1e-6)];
    }, Y = $.map((de, me) => L(me)), j = T[0];
    let O = [0, 0], z = 0;
    for (const de of T) {
      const me = de[0] - j[0], ke = de[1] - j[1], Ee = Math.hypot(me, ke);
      Ee > z && (z = Ee, O = [me / Ee, ke / Ee]);
    }
    if (z < 1e-9) {
      const de = j[0] - u, me = j[1] - g, ke = Math.hypot(de, me) || 1;
      O = [de / ke, me / ke];
    }
    O[0] * (j[0] - u) + O[1] * (j[1] - g) < 0 && (O = [-O[0], -O[1]]), At();
    ...
    const J = [...((_b = r.areas) == null ? void 0 : _b.rawVal) ?? []], se = T.map((de) => {
      ...
      return $.map((Ee, nt) => Pn(ee, [Ee[0] + Y[nt][0] * me, Ee[1] + Y[nt][1] * me, ke]));
    });
    let oe = 0;
    ...
    const A = Math.min(u[0], g[0]), k = Math.max(u[0], g[0]), M = Math.min(u[1], g[1]), $ = Math.max(u[1], g[1]), I = (u[2] + g[2]) / 2, T = k - A, N = $ - M, F = Math.min(y, T / 2 - 0.01, N / 2 - 0.01);
    if (F <= 0) return;
    const L = r.points.rawVal.length, Y = [], j = [], O = (z, V) => {
      Y.push([z, V, I]), j.push(L + Y.length - 1);
    };
    for (let z = 0; z <= v; z++) O(A + F + (T - 2 * F) * z / v, M);
    for (let z = 1; z <= m; z++) {
    ...
    for (let z = 1; z <= m; z++) {
      const V = Math.PI / 2 + Math.PI / 2 * z / m;
      O(A + F + F * Math.cos(V), $ - F + F * Math.sin(V));
    }
    for (let z = 1; z <= v; z++) O(A, $ - F - (N - 2 * F) * z / v);
    for (let z = 1; z < m; z++) {
      const V = Math.PI + Math.PI / 2 * z / m;
      O(A + F + F * Math.cos(V), M + F + F * Math.sin(V));
    }
    if (j.push(L), en()) {
      tn(Y, true);
      return;
    ...
    const y = r.points.rawVal.length, m = u[0], v = u[1], A = u[2], k = g[0], M = g[1], $ = g[2];
    let I;
    if (Math.abs(A - $) < 1e-6 ? I = [[m, v, A], [k, v, A], [k, M, A], [m, M, A]] : Math.abs(v - M) < 1e-6 ? I = [[m, v, A], [k, v, A], [k, v, $], [m, v, $]] : I = [[m, v, A], [m, M, A], [m, M, $], [m, v, $]], r.points.val = [...r.points.rawVal, ...I], r.polylines) {
    ...
    let I;
    if (_ && r.gridTarget) {
      const T = r.gridTarget.rawVal, N = new zi(...T.rotation), F = new te(1, 0, 0).applyEuler(N), L = new te(0, 1, 0).applyEuler(N), Y = new te(...T.position), j = new te(m, v, A), O = new te(k, M, $), z = j.clone().sub(Y).dot(F), V = j.clone().sub(Y).dot(L), H = O.clone().sub(Y).dot(F), ee = O.clone().sub(Y).dot(L), Z = (J, se) => Y.clone().addScaledVector(F, J).addScaledVector(L, se).toArray();
      I = [Z(z, V), Z(H, V), Z(H, ee), Z(z, ee)];
    } else Math.abs(A - $) < 1e-6 ? I = [[m, v, A], [k, v, A], [k, M, A], [m, M, A]] : Math.abs(v - M) < 1e-6 ? I = [[m, v, A], [k, v, A], [k, v, $], [m, v, $]] : I = [[m, v, A], [m, M, A], [m, M, $], [m, v, $]];
    ...
    const A = u.map((O) => O.map((z) => m.get(z) ?? z)), k = /* @__PURE__ */ new Map(), M = (O, z) => {
      O !== z && ((k.get(O) ?? k.set(O, /* @__PURE__ */ new Set()).get(O)).add(z), (k.get(z) ?? k.set(z, /* @__PURE__ */ new Set()).get(z)).add(O));
    };
    for (const O of A) for (let z = 0; z + 1 < O.length; z++) M(O[z], O[z + 1]);
    const $ = (O, z) => {
      var _a4;
    ...
    const F = [...((_b = r.areas) == null ? void 0 : _b.rawVal) ?? []], L = new Set(F.map((O) => [...new Set(A[O] ?? [])].sort((z, V) => z - V).join("-"))), Y = [...A];
    let j = 0;
    for (const O of T) {
    ...
    for (let Re = 0; Re < y; Re++) {
      const Xe = u[Re], We = u[(Re + 1) % y];
      m += (Xe[1] - We[1]) * (Xe[2] + We[2]), v += (Xe[2] - We[2]) * (Xe[0] + We[0]), A += (Xe[0] - We[0]) * (Xe[1] + We[1]);
    }
    const k = Math.hypot(m, v, A) || 1;
    m /= k, v /= k, A /= k;
    let M = u[1][0] - u[0][0], $ = u[1][1] - u[0][1], I = u[1][2] - u[0][2];
    const T = Math.hypot(M, $, I) || 1;
    M /= T, $ /= T, I /= T;
    let N = v * I - A * $, F = A * M - m * I, L = m * $ - v * M;
    const Y = Math.hypot(N, F, L) || 1;
    N /= Y, F /= Y, L /= Y;
    const j = u[0], O = (Re) => [(Re[0] - j[0]) * M + (Re[1] - j[1]) * $ + (Re[2] - j[2]) * I, (Re[0] - j[0]) * N + (Re[1] - j[1]) * F + (Re[2] - j[2]) * L], z = (Re, Xe) => [j[0] + Re * M + Xe * N, j[1] + Re * $ + Xe * F, j[2] + Re * I + Xe * L], V = u.map(O);
    let H = 1 / 0, ee = -1 / 0, Z = 1 / 0, J = -1 / 0;
    for (const [Re, Xe] of V) Re < H && (H = Re), Re > ee && (ee = Re), Xe < Z && (Z = Xe), Xe > J && (J = Xe);
    ...
    const de = (Re, Xe) => {
      ...
      for (let xt = 0, St = V.length - 1; xt < V.length; St = xt++) {
        const [Ot, hs] = V[xt], [fi, Ks] = V[St];
        hs > Xe != Ks > Xe && Re < (fi - Ot) * (Xe - hs) / (Ks - hs) + Ot && (We = !We);
      }
      return We;
    }, me = Math.max(1, Math.round(se / ie)), ke = Math.max(1, Math.round(oe / ie)), Ee = se / me, nt = oe / ke, je = /* @__PURE__ */ new Map(), et = [], De = r.points.rawVal.length, Je = (Re, Xe) => {
      const We = Re + "," + Xe, xt = je.get(We);
      if (xt !== void 0) return xt;
      const St = De + et.length;
      return et.push(z(H + Re * Ee, Z + Xe * nt)), je.set(We, St), St;
    }, Be = [];
    for (let Re = 0; Re < me; Re++) for (let Xe = 0; Xe < ke; Xe++) {
      if (!de(H + (Re + 0.5) * Ee, Z + (Xe + 0.5) * nt)) continue;
      const We = Je(Re, Xe), xt = Je(Re + 1, Xe), St = Je(Re + 1, Xe + 1), Ot = Je(Re, Xe + 1);
      Be.push([We, xt, St, Ot]);
    }
    ...
    if (window.__hekatanPushUndo && window.__hekatanPushUndo(), r.points.val = [...r.points.rawVal, ...et], r.polylines && r.areas) {
      let Re = r.polylines.rawVal.slice();
      Re.length && Re[Re.length - 1].length === 0 && (Re = Re.slice(0, -1));
      const Xe = [];
      for (const We of Be) Xe.push(Re.length), Re.push([We[0], We[1], We[2], We[3], We[0]]);
      Re.push([]), r.polylines.val = Re, r.areas.val = [...r.areas.rawVal, ...Xe];
    }
    try {
      (_a3 = window.__hekatanRebuild) == null ? void 0 : _a3.call(window);
    } catch {
  ...
  window.__hekatanFinalizePolyArea = sn, window.__hekatanSetInclinedPlaneFrom3 = (u, g, y) => {
    var _a3;
    const m = new te(u[0], u[1], u[2]), v = new te(g[0], g[1], g[2]), A = new te(y[0], y[1], y[2]), k = new te().subVectors(v, m).cross(new te().subVectors(A, m));
    ...
    const I = new te().addVectors(m, v).add(A).multiplyScalar(1 / 3), T = Math.max(m.distanceTo(v), m.distanceTo(A), v.distanceTo(A)) * 2.2 + 4, N = T / 2;
    Ht.geometry.dispose(), Ht.geometry = new Ds(T, T), Cs.geometry.dispose(), Cs.geometry = new aa(new Ds(T, T)), qi(N, 1), Lt.position.copy(I), Lt.quaternion.copy(M), Lt.scale.set(1, 1, 1), Lt.visible = true;
    try {
      (_a3 = window.__hekatanRefreshStatus) == null ? void 0 : _a3.call(window);
    } catch {
    }
  ...
  const cs = new ft();
  cs.visible = false, t.add(cs), window.__hekatanShowAxes = (u, g, y = 12, m = 2) => {
    var _a3, _b;
    for (; cs.children.length; ) {
      const T = cs.children.pop();
      (_a3 = T.geometry) == null ? void 0 : _a3.dispose(), (_b = T.material) == null ? void 0 : _b.dispose();
    }
    if (!u.length || !g.length) return;
    const v = Math.min(...g) - m, A = Math.max(...g) + m, k = Math.min(...u) - m, M = Math.max(...u) + m, $ = "ABCDEFGHIJKLMNOPQRSTUVWXYZ", I = (T, N, F, L, Y) => {
      ...
      const z = new la(j), V = new ra({ map: z, transparent: true }), H = new ca(V);
      return H.position.set(N, F, L), H.scale.set(1.2, 0.6, 1), H;
    };
    u.forEach((T, N) => {
      const F = N < $.length ? $[N] : `X${N}`, L = new ze().setFromPoints([new te(T, v, 0), new te(T, A, 0), new te(T, v, 0), new te(T, v, y)]), Y = new Ti({ color: 6333946, dashSize: 0.3, gapSize: 0.15, transparent: true, opacity: 0.6 }), j = new is(L, Y);
      j.computeLineDistances(), cs.add(j), cs.add(I(F, T, v - 0.5, 0, "#60a5fa")), cs.add(I(F, T, A + 0.5, 0, "#60a5fa"));
    }), g.forEach((T, N) => {
      ...
      j.computeLineDistances(), cs.add(j), cs.add(I(F, k - 0.5, T, 0, "#fb7185")), cs.add(I(F, M + 0.5, T, 0, "#fb7185"));
    }), cs.visible = true, l();
  ...
  window.__hekatanShowRefPlanes = (u = [0, 3, 6, 9, 12], g = 20, y = 0, m = 0) => {
    ...
    for (; As.children.length; ) {
      const A = As.children.pop();
      (_a3 = A.geometry) == null ? void 0 : _a3.dispose(), (_b = A.material) == null ? void 0 : _b.dispose();
    }
    ri.forEach((A) => {
      t.remove(A), A.geometry.dispose(), A.material.dispose();
    }), ri = [];
    const v = [6333946, 3462041, 16498468, 16478597, 12616956, 2282478];
    u.forEach((A, k) => {
      const M = v[k % v.length], $ = g / 2, I = [new te(y - $, m - $, A), new te(y + $, m - $, A), new te(y + $, m + $, A), new te(y - $, m + $, A), new te(y - $, m - $, A)], T = new ze().setFromPoints(I), N = new mt({ color: M, transparent: true, opacity: 0.55 });
      As.add(new Vt(T, N));
      const F = document.createElement("canvas");
      F.width = 128, F.height = 32;
      const L = F.getContext("2d");
      L.fillStyle = `#${M.toString(16).padStart(6, "0")}`, L.font = "bold 18px sans-serif", L.fillText(`Z = ${A} m`, 4, 22);
      const Y = new la(F), j = new ra({ map: Y, transparent: true }), O = new ca(j);
      O.position.set(y - $ - 1.5, m - $ - 1.5, A), O.scale.set(2.5, 0.6, 1), As.add(O);
      const z = new Ds(1e4, 1e4), V = new gt({ visible: false, side: Rt }), H = new pt(z, V);
      H.position.set(0, 0, A), H.frustumCulled = false, H.userData = { refPlaneZ: A }, t.add(H), ri.push(H);
    }), As.visible = true, l();
  }, window.__hekatanHideRefPlanes = () => {
    As.visible = false, ri.forEach((u) => {
      u.visible = false;
    }), l();
  };
  const Si = new ft();
  ...
  const ol = () => {
    ...
    for (const y of g) {
      ...
      const m = new ze().setFromPoints([new te(y[0], y[1], y[2]), new te(y[3], y[4], y[5])]), v = new Ti({ color: 2282478, dashSize: 0.3, gapSize: 0.15, transparent: true, opacity: 0.8 }), A = new Vt(m, v);
      A.computeLineDistances(), Si.add(A);
    }
  };
  xe.derive(() => {
    const u = window.__hekatanDrawingAuxLines;
    (u == null ? void 0 : u.val) && (u.val, ol(), l());
  });
  const ci = new ft();
  ...
  xe.derive(() => {
    ...
    (u == null ? void 0 : u.val) !== void 0 && (u.val, Lo(), l());
  }), i.addEventListener("change", () => {
    ci.children.forEach((u) => {
      u.scale.setScalar(Ki(u.position));
    });
  }), window.__hekatanRenderAuxPoints = Lo;
  const vt = new ft(), al = new pt(new gi(0.01, 12, 12), new gt({ color: 16777215, transparent: true, opacity: 0.95 })), No = new pt(new gi(0.015, 12, 12), new gt({ color: 16498468, transparent: true, opacity: 0.2, depthWrite: false }));
  No.visible = false, vt.add(al, No);
  ...
  const Oo = () => {
    xs.children.length !== 0 && xs.children.forEach((u) => {
      if (!u.__isSelectionPt) return;
      const g = u;
      g.scale.setScalar(Ki(g.position) * 1.8);
    });
  };
  window.__hekatanUpdateSelectionPtScale = Oo, i.addEventListener("change", () => {
    var _a3;
    Mi(), ls.visible && Eo(), (_a3 = window.__hekatanUpdateOsnapScale) == null ? void 0 : _a3.call(window), Oo();
    ...
    if ((!y.length || ((_c2 = (_b = (_a3 = window.__hekatanCadState) == null ? void 0 : _a3.get) == null ? void 0 : _b.call(_a3)) == null ? void 0 : _c2.tool) !== "fillarea") && _t.visible && (_t.visible = false), y.length) {
      ...
      else if (!v && (Te = Le(u.clientX, u.clientY))) A = true, m.copy(Te), ln("ifcSec", m.x, m.y, m.z), cn("ifcSec", u.clientX, u.clientY), vt.position.copy(m), vt.visible = true;
      else if (S && !v) A = true, ln(S.tipo, m.x, m.y, m.z), cn(S.tipo, u.clientX, u.clientY), vt.position.copy(m), vt.visible = true;
      else {
        dl(), rn();
      ...
      Mi(), U(E && !M && (A || S) ? W(E) : null), Xt = { p: m.clone(), x: u.clientX, y: u.clientY };
      const $ = ((_j = (_i2 = (_h = window.__hekatanCadState) == null ? void 0 : _h.get) == null ? void 0 : _i2.call(_h)) == null ? void 0 : _j.tool) ?? "select";
      if ($ === "select" || !$) {
        const F = (window.__hekatanSnap2D ?? 0.5) * 1.5, L = tl(m.x, m.y, m.z, F), Y = Rn(m.x, m.y, m.z, F), j = To(m.x, m.y, m.z, F);
        if (L >= 0) {
          const H = r.points.rawVal[L];
          ls.position.set(H[0], H[1], H[2]), ls.visible = true, Eo(), ys.visible = false, bs = { kind: "pt", a: L };
        } else if (Y) {
          const H = r.points.rawVal, ee = r.polylines.rawVal[Y.polyIdx], Z = H[ee[Y.segIdx]], J = H[ee[Y.segIdx + 1]];
          ys.geometry.setFromPoints([new te(Z[0], Z[1], Z[2]), new te(J[0], J[1], J[2])]), ys.visible = true, ls.visible = false, bs = ((_l = (_k = r.areas) == null ? void 0 : _k.rawVal) == null ? void 0 : _l.includes(Y.polyIdx)) ?? false ? { kind: "poly", a: Y.polyIdx } : { kind: "seg", a: Y.polyIdx, b: Y.segIdx };
        } else if (j >= 0) {
          const ee = (((_m = window.__hekatanDrawingAuxLines) == null ? void 0 : _m.rawVal) ?? [])[j];
          ee && (ys.geometry.setFromPoints([new te(ee[0], ee[1], ee[2]), new te(ee[3], ee[4], ee[5])]), ys.visible = true, ls.visible = false, bs = { kind: "aux", a: j });
        } else ys.visible = false, ls.visible = false, bs = null;
        Fe.style.left = u.clientX + "px", Fe.style.top = u.clientY + "px", Fe.style.display = "block";
        let O = m;
        if ((bs == null ? void 0 : bs.kind) === "pt") {
          const H = r.points.rawVal[bs.a];
          H && (O = new te(H[0], H[1], H[2]));
        }
        const z = `X=${O.x.toFixed(2)} Y=${O.y.toFixed(2)} Z=${O.z.toFixed(2)}`;
        if (window.__hekatanCursorXYZ = [O.x, O.y, O.z], bs) {
          const H = { pt: "nodo", seg: "segmento", poly: "\xE1rea", aux: "l\xEDnea aux" };
          Fe.textContent = `${z}  \xB7  \u{1F5B1} Click \u2192 ${H[bs.kind]}`;
        } else Fe.textContent = z;
        const V = document.getElementById("hk-coord-fixed");
        V && (V.textContent = z), Xt = { p: O.clone(), x: u.clientX, y: u.clientY }, Pe.visible = false, Zt.visible = false, Kt.visible = false, l();
        return;
      }
      if ($ === "delete" || $ === "trim" || $ === "extend" || $ === "offset") {
        const F = (window.__hekatanSnap2D ?? 0.5) * 1.5, L = Rn(m.x, m.y, m.z, F), Y = To(m.x, m.y, m.z, F);
        let j = false;
        if (Y >= 0) if (!L) j = true;
        else {
        ...
        j ? (Is = Y, gs = -1, zs = -1, sl(Y)) : L ? (gs = L.polyIdx, zs = L.segIdx, Is = -1, il(L.polyIdx, L.segIdx)) : (gs = -1, zs = -1, Is = -1, Qt.visible = false), Pe.visible = false, Zt.visible = false, Kt.visible = false, ot(), Fe.style.left = u.clientX + "px", Fe.style.top = u.clientY + "px", Fe.style.display = "block";
        ...
        V && (V.textContent = O), l();
        return;
      } else Qt.visible = false, gs = -1, Is = -1;
      Fe.style.left = u.clientX + "px", Fe.style.top = u.clientY + "px", Fe.style.display = "block";
      ...
      if (T.length > 0 && N[T[T.length - 1]]) {
        ...
        if (!Y && !j && window.__hekatanAxisSnap !== false) {
          const je = a.getBoundingClientRect(), et = u.clientX, De = u.clientY, Je = ((_q = settings.gridSize) == null ? void 0 : _q.rawVal) ?? 10, Be = new te(L[0], L[1], L[2]), Re = [["x", new te(1, 0, 0)], ["y", new te(0, 1, 0)], ["z", new te(0, 0, 1)]], Xe = (xt) => {
            const St = xt.clone().project(g);
            return { x: (St.x * 0.5 + 0.5) * je.width + je.left, y: (-St.y * 0.5 + 0.5) * je.height + je.top };
          };
          let We = null;
          for (const [xt, St] of Re) {
            const Ot = Xe(Be.clone().addScaledVector(St, -Je)), hs = Xe(Be.clone().addScaledVector(St, Je)), fi = hs.x - Ot.x, Ks = hs.y - Ot.y, bl = et - Ot.x, vl = De - Ot.y, _l2 = fi * fi + Ks * Ks || 1;
            let un = (bl * fi + vl * Ks) / _l2;
            un = Math.max(0, Math.min(1, un));
            const Qo = Math.hypot(et - (Ot.x + un * fi), De - (Ot.y + un * Ks));
            if (We === null || Qo < We.dpx) {
              const Kn = c.ray, ea = Be.clone().sub(Kn.origin), Gn = St.dot(Kn.direction), ta = St.dot(ea), kl = Kn.direction.dot(ea), sa = 1 - Gn * Gn, Cl = Math.abs(sa) < 1e-6 ? -ta : (Gn * kl - ta) / sa;
              We = { axis: xt, dpx: Qo, pt: Be.clone().addScaledVector(St, Cl) };
            }
          }
        ...
        if (!Y && !j && O) {
          ...
          let Be = null;
          Je === as ? Be = "xy" : Je === fs ? Be = "xz" : Je === ms && (Be = "yz"), Be === "xy" ? Y = je >= et ? "x" : "y" : Be === "xz" ? Y = je >= De ? "x" : "z" : Be === "yz" ? Y = et >= De ? "y" : "z" : Y = je >= et && je >= De ? "x" : et >= De ? "y" : "z";
        }
        const z = window.__hekatanPolarTrack !== false;
        if (!Y && !j && z) {
          const je = m.x - L[0], et = m.y - L[1], De = m.z - L[2], Je = Math.hypot(je, et, De);
          if (Je > 1e-3) {
            const Re = Math.tan(6 * Math.PI / 180) * Je, Xe = Math.hypot(et, De), We = Math.hypot(je, De), xt = Math.hypot(je, et), St = [["x", Xe], ["y", We], ["z", xt]];
            St.sort((Ot, hs) => Ot[1] - hs[1]), St[0][1] <= Re && (Y = St[0][0]);
          }
        }
        if (Y) {
          const je = L[0], et = L[1], De = L[2];
          Y === "x" ? m.set(m.x, et, De) : Y === "y" ? m.set(je, m.y, De) : m.set(je, et, m.z);
          const Je = !!zt, Re = { x: "#ff3344", y: "#34d399", z: "#60a5fa" }[Y];
          ...
          let We = null;
          Xe === as ? We = "xy" : Xe === fs ? We = "xz" : Xe === ms && (We = "yz");
        ...
        if (!v && !j && window.__hekatanTrack !== false && window.__hekatanOsnapOn !== false) {
          ...
          let Je = 1 / 0;
          for (const Be of je) if (!(Math.abs(Be[0] - L[0]) < 1e-9 && Math.abs(Be[1] - L[1]) < 1e-9 && Math.abs(Be[2] - L[2]) < 1e-9)) for (const Re of et) {
            const Xe = new te(Re === "x" ? Be[0] : m.x, Re === "y" ? Be[1] : m.y, Re === "z" ? Be[2] : m.z), We = Ws(Xe.x, Xe.y, Xe.z);
            if (!We) continue;
            const xt = Math.hypot(We.x - De.x, We.y - De.y);
            xt < Fs && xt < Je && (Je = xt, V = { q: Be, eje: Re });
          }
        }
        ...
        se && (se.textContent = Z), Pe.geometry.setFromPoints([new te(L[0], L[1], L[2]), new te(m.x, m.y, m.z)]), (_u = Pe.computeLineDistances) == null ? void 0 : _u.call(Pe), Pe.visible = true, Pt(L[0], L[1], L[2], m.x, m.y, m.z);
        const oe = window.__hekatanOrthoExt ?? 8, ie = window.__hekatanShowOrthoPlanes !== false;
        ps.visible = ie, ie || Mo(null), ie && ($s(Us, L, "xy", oe), $s(oi, L, "xz", oe), $s(ai, L, "yz", oe), Ms(as, L, "xy", oe), Ms(fs, L, "xz", oe), Ms(ms, L, "yz", oe));
        const de = ie ? c.intersectObjects([as, fs, ms], false) : [];
        let me = null;
        if (de.length > 0) {
          const je = de[0].object;
          je === as ? me = "xy" : je === fs ? me = "xz" : je === ms && (me = "yz");
        }
        Mo(me), me && (ws.style.left = u.clientX + "px", ws.style.top = u.clientY + "px"), Hs.geometry.setFromPoints([new te(L[0] - oe, L[1], L[2]), new te(L[0] + oe, L[1], L[2])]), (_v = Hs.computeLineDistances) == null ? void 0 : _v.call(Hs), Xs.geometry.setFromPoints([new te(L[0], L[1] - oe, L[2]), new te(L[0], L[1] + oe, L[2])]), (_w = Xs.computeLineDistances) == null ? void 0 : _w.call(Xs), Ys.geometry.setFromPoints([new te(L[0], L[1], L[2] - oe), new te(L[0], L[1], L[2] + oe)]), (_x = Ys.computeLineDistances) == null ? void 0 : _x.call(Ys), Zt.visible = true;
        const ke = Hs.material, Ee = Xs.material, nt = Ys.material;
        Hs.visible = Y === "x", Xs.visible = Y === "y", Ys.visible = Y === "z", ke.opacity = 0.95, Ee.opacity = 0.95, nt.opacity = 0.95;
      } else {
        const F = `X=${m.x.toFixed(2)} Y=${m.y.toFixed(2)} Z=${m.z.toFixed(2)}`;
        Fe.textContent = F;
        const L = document.getElementById("hk-coord-fixed");
        if (L && (L.textContent = F), Pe.visible = false, Zt.visible = false, (/* @__PURE__ */ new Set(["line", "polyline", "area", "node", "column", "wall", "rect", "circle", "arc", "polyline-multi", "axis", "chaflan"])).has($)) {
          if (Ye = null, Ze = null, Ae.style.left = u.clientX + 20 + "px", Ae.style.top = u.clientY - 28 + "px", Ae.style.display = "block", !$e) {
            Ae.value = `${m.x.toFixed(2)},${m.y.toFixed(2)},${m.z.toFixed(2)}`;
            const j = document.activeElement;
            !(j && (j.tagName === "INPUT" || j.tagName === "TEXTAREA") && j !== Ae) && document.activeElement !== Ae && Ae.focus({ preventScroll: true });
            try {
              Ae.select();
            } catch {
            }
      ...
      l();
    } else rn(), Fe.style.display = "none", vt.visible = false, Pe.visible = false, Zt.visible = false, ot(), l();
  }), xe.derive(() => {
    var _a3;
    ...
    wr(e, { position: new te(...r.gridTarget.val.position), quaternion: u.clone().multiply(g) }, l);
    {
      const m = r.gridTarget.val.position[2], v = Math.abs(u.x - Math.sin(Math.PI / 4)) < 1e-3;
      for (const A of vi) t.remove(A), Fn(A);
      if (vi.length = 0, v) {
        const A = ((_a3 = r.points) == null ? void 0 : _a3.rawVal) ?? [], k = /* @__PURE__ */ new Set([0]);
        for (const $ of A) k.add(+$[2].toFixed(3));
  ...
  const Vn = (u, g, y, m, v) => {
    ...
    var _a3, _b, _c2, _d;
    const A = Math.min(u, y), k = Math.max(u, y), M = Math.min(g, m), $ = Math.max(g, m), I = y < u, T = a.getBoundingClientRect(), N = s();
    N.updateMatrixWorld();
    const F = (J) => {
      const se = new te(J[0], J[1], J[2]);
      return se.project(N), { x: T.left + (se.x * 0.5 + 0.5) * T.width, y: T.top + (-se.y * 0.5 + 0.5) * T.height };
    }, L = (J) => J.x >= A && J.x <= k && J.y >= M && J.y <= $, Y = (J, se) => !(J.x < A && se.x < A || J.x > k && se.x > k || J.y < M && se.y < M || J.y > $ && se.y > $);
    v || Ue.clear();
    let j = 0;
    ...
    rs(), we(j === 0 && !I ? "\u{1F535} Window (izq\u2192der) \u2014 0: solo coge lo que quede ENTERO dentro. De DERECHA a IZQUIERDA (\u{1F7E2} captura) coge todo lo que toque." : `${I ? "\u{1F7E2} Crossing" : "\u{1F535} Window"} \u2014 ${j} item(s) ${v ? "agregados a" : "\u2192"} selecci\xF3n (total ${Ue.size})`), Gt.style.display = "none";
  }, on = () => {
  ...
  const Bn = () => {
    ...
    const u = [...Ue], g = ((_a3 = r.points) == null ? void 0 : _a3.rawVal) ?? [], y = ((_b = r.polylines) == null ? void 0 : _b.rawVal) ?? [], m = ((_c2 = r.areas) == null ? void 0 : _c2.rawVal) ?? [], v = window.__hekatanDrawingAuxLines, A = (v == null ? void 0 : v.rawVal) ?? [], k = /* @__PURE__ */ new Set(), M = /* @__PURE__ */ new Set(), $ = /* @__PURE__ */ new Map(), I = /* @__PURE__ */ new Set();
    for (const Y of u) {
      const [j, ...O] = Y.split(":");
      if (j === "pt") k.add(+O[0]);
  ...
  let Ai = null;
  try {
    const u = localStorage.getItem(Bo);
    u && (Ai = JSON.parse(u));
  } catch {
  }
  es.style.cssText = ["position:fixed", Ai ? `left:${Ai.left}px` : "left:14px", Ai ? `top:${Ai.top}px` : "top:200px", "transform:none", "width:min(300px, calc(100vw - 32px))", "max-height:calc(100vh - 260px)", "overflow-y:auto", "z-index:201", "box-shadow:0 6px 24px rgba(0,0,0,0.45)", "border-radius:6px", "display:none"].join(";") + ";", document.body.appendChild(es);
  const ll = () => {
    ...
    let g = false, y = 0, m = 0, v = 0, A = 0;
    u.addEventListener("mousedown", (k) => {
      g = true, y = k.clientX, m = k.clientY;
      const M = es.getBoundingClientRect();
      v = M.left, A = M.top, es.style.transform = "none", es.style.left = `${v}px`, es.style.top = `${A}px`, k.preventDefault();
    }), window.addEventListener("mousemove", (k) => {
      if (!g) return;
      const M = k.clientX - y, $ = k.clientY - m, I = Math.max(0, Math.min(window.innerWidth - 80, v + M)), T = Math.max(0, Math.min(window.innerHeight - 40, A + $));
      es.style.left = `${I}px`, es.style.top = `${T}px`;
    }), window.addEventListener("mouseup", () => {
      if (g) {
        g = false;
        try {
          localStorage.setItem(Bo, JSON.stringify({ left: parseFloat(es.style.left), top: parseFloat(es.style.top) }));
        } catch {
        }
      }
    });
  ...
  const Nt = (u, g, y, m) => {
    ...
    const y = u.filter((N) => N.startsWith("seg:")), m = u.filter((N) => N.startsWith("poly:")), v = u.filter((N) => N.startsWith("aux:")), A = g.length > 0, k = y.length > 0, M = m.length > 0, $ = !A && !k && !M, I = [];
    ...
    yt = new La({ container: es, title: T });
    {
    ...
    yt.addButton({ title: "\u2715 Cerrar (limpia selecci\xF3n)" }).on("click", () => {
      Ue.clear(), rs();
    }), es.style.display = "block", ll();
  };
  window.__hekatanRefreshPropsPane = rl;
  ...
  a.addEventListener("pointerdown", (u) => {
    ...
    if (u.button === 2) {
      ...
      if (g) {
        if (ts ? on() : window.dispatchEvent(new KeyboardEvent("keydown", { key: "Escape", bubbles: true })), Ue.size > 0 && (Ue.clear(), rs()), r.polylines) {
          const A = r.polylines.rawVal;
          (A[A.length - 1] ?? []).length > 0 && (r.polylines.val = [...A, []]);
        }
    ...
    var _a3, _b, _c2;
    const g = ((_c2 = (_b = (_a3 = window.__hekatanCadState) == null ? void 0 : _a3.get) == null ? void 0 : _b.call(_a3)) == null ? void 0 : _c2.tool) ?? "select";
    g !== "select" && g !== "none" && g || u.button === 0 && (window.__hekatanBloquearVentana || u.pointerType !== "touch" && (ds = null, $i = false));
  }), a.addEventListener("pointermove", (u) => {
    if (ts && u.buttons === 0) {
      const A = u.clientX < ts.x;
      Vn(ts.x, ts.y, u.clientX, u.clientY, A);
      return;
    }
    if (!ds) return;
    const g = u.clientX - ds.x, y = u.clientY - ds.y, m = Math.hypot(g, y);
    if (!$i && m < 8) return;
    $i = true;
    const v = u.clientX < ds.x;
    Vn(ds.x, ds.y, u.clientX, u.clientY, v);
  }), a.addEventListener("pointerup", (u) => {
    if (!ds) return;
    if (!$i) {
      ds = null;
    ...
    Vo(ds.x, ds.y, u.clientX, u.clientY, g), ds = null, $i = false;
  ...
  const Ho = { end: 16724804, mid: 16498468, node: 6333946, cen: 3462041, per: 12616956, nea: 16744118, int: 16746496, ori: 16777215, grid: 2282478, track: 16761856, ifc: 16096779, ifcAxis: 16639626, ifcSec: 16486972, ifcEdge: 16498468, ifcVert: 16724804 }, ln = (u, g, y, m) => {
    var _a3, _b, _c2, _d;
    for (; ns.children.length; ) {
      const k = ns.children.pop();
      (_b = (_a3 = k.geometry) == null ? void 0 : _a3.dispose) == null ? void 0 : _b.call(_a3), (_d = (_c2 = k.material) == null ? void 0 : _c2.dispose) == null ? void 0 : _d.call(_c2);
    }
    const v = Ho[u] ?? 16777215, A = new ze().setFromPoints([new te(-1, -1, 0), new te(1, -1, 0), new te(1, -1, 0), new te(1, 1, 0), new te(1, 1, 0), new te(-1, 1, 0), new te(-1, 1, 0), new te(-1, -1, 0)]);
    ns.add(new is(A, new mt({ color: v, linewidth: 2 }))), ns.position.set(g, y, m), ns.visible = true, Xn();
  };
  let Hn = 4;
  const Xn = () => {
  ...
  const rn = () => {
    ns.visible = false;
  }, cl = { ori: "Origen (0,0,0)", grid: "Cruce de rejilla", end: "Punto final", track: "Alineado con un nudo", node: "Nudo", mid: "Punto medio", cen: "Centro", int: "Intersecci\xF3n", per: "Perpendicular", nea: "Cercano", ifc: "Referencia IFC \xB7 cara", ifcAxis: "Referencia IFC \xB7 eje", ifcSec: "Secci\xF3n IFC (corte)", ifcEdge: "Borde IFC", ifcVert: "V\xE9rtice IFC" }, vs = document.createElement("div");
  vs.id = "hk-osnap-etiqueta", vs.style.cssText = ["position:fixed", "z-index:99995", "display:none", "pointer-events:none", "padding:2px 7px", "border-radius:4px", "white-space:nowrap", "background:rgba(15,23,42,0.92)", "border:1px solid rgba(148,163,184,.45)", "color:#e2e8f0", "font:12px Consolas,monospace"].join(";") + ";", document.body.appendChild(vs);
  const cn = (u, g, y) => {
    const m = cl[u];
    if (!m) {
    ...
    vs.textContent = m, vs.style.color = "#" + (Ho[u] ?? 16777215).toString(16).padStart(6, "0"), vs.style.left = g + 18 + "px", vs.style.top = y - 26 + "px", vs.style.display = "block";
  }, dl = () => {
    vs.style.display = "none";
  }, js = new te(), Ws = (u, g, y) => {
    const m = s();
  ...
  const hl = (u, g, y, m, v) => {
    ...
    const A = window.__hekatanOsnap, k = r.points.rawVal, M = ((_a3 = r.polylines) == null ? void 0 : _a3.rawVal) ?? [];
    let $ = null;
    ...
    if (A.ori !== false && N("ori", 0, 0, 0), A.grid !== false && window.__hekatanSnapEnabled === true) {
    ...
    (A.node || A.end) && k.forEach((z) => {
      A.node && N("node", z[0], z[1], z[2]);
    });
    for (const z of M) if (!(z.length < 2)) for (let V = 0; V < z.length - 1; V++) {
      const H = k[z[V]], ee = k[z[V + 1]];
      if (!(!H || !ee) && (A.end && (N("end", H[0], H[1], H[2]), N("end", ee[0], ee[1], ee[2])), A.mid && N("mid", (H[0] + ee[0]) / 2, (H[1] + ee[1]) / 2, (H[2] + ee[2]) / 2), A.nea || A.per)) {
        ...
        A.nea && N("nea", de, me, ke), A.per && N("per", de, me, ke);
      }
    }
    if (A.cen) {
    ...
    if (A.int) {
      ...
      for (let V = 0; V < z.length; V++) for (let H = V + 1; H < z.length; H++) {
        const [ee, Z] = z[V], [J, se] = z[H], oe = [Z[0] - ee[0], Z[1] - ee[1], Z[2] - ee[2]], ie = [se[0] - J[0], se[1] - J[1], se[2] - J[2]], de = [ee[0] - J[0], ee[1] - J[1], ee[2] - J[2]], me = oe[0] * oe[0] + oe[1] * oe[1] + oe[2] * oe[2], ke = oe[0] * ie[0] + oe[1] * ie[1] + oe[2] * ie[2], Ee = ie[0] * ie[0] + ie[1] * ie[1] + ie[2] * ie[2], nt = oe[0] * de[0] + oe[1] * de[1] + oe[2] * de[2], je = ie[0] * de[0] + ie[1] * de[1] + ie[2] * de[2], et = me * Ee - ke * ke;
        if (et < 1e-12) continue;
        const De = (ke * je - Ee * nt) / et, Je = (me * je - ke * nt) / et;
        if (De < -1e-6 || De > 1 + 1e-6 || Je < -1e-6 || Je > 1 + 1e-6) continue;
        const Be = [ee[0] + De * oe[0], ee[1] + De * oe[1], ee[2] + De * oe[2]], Re = [J[0] + Je * ie[0], J[1] + Je * ie[1], J[2] + Je * ie[2]];
        if (Math.hypot(Be[0] - Re[0], Be[1] - Re[1], Be[2] - Re[2]) > 1e-4) continue;
    ...
    for (const z of O) {
      ...
      if (A.end && (N("end", V[0], V[1], V[2]), N("end", H[0], H[1], H[2])), A.mid && N("mid", (V[0] + H[0]) / 2, (V[1] + H[1]) / 2, (V[2] + H[2]) / 2), A.nea || A.per) {
        ...
        A.nea && N("nea", ie, de, me), A.per && N("per", ie, de, me);
      }
    }
    return $ ? { type: $.type, x: $.x, y: $.y, z: $.z } : null;
  }, ui = new ft();
  ui.frustumCulled = false, t.add(ui);
  const Xo = new mt({ color: 15123555, transparent: true, opacity: 1, depthTest: false });
  let Yo = 0;
  const Uo = () => {
    var _a3, _b;
  ...
  window.__hekatanDestello = (u) => {
    ...
    for (const v of u || []) {
      ...
      if (A[0] === "pt") {
        ...
        const I = y[+A[1]] || [], T = g[I[+A[2]]], N = g[I[+A[2] + 1]];
        T && N && (k = [T, N]);
      } else A[0] === "poly" && (k = (y[+A[1]] || []).map((T) => g[T]).filter(Boolean));
      if (k.length < 2) continue;
      const M = new ze().setFromPoints(k.map((I) => new te(I[0], I[1], I[2]))), $ = new Vt(M, Xo);
      $.renderOrder = 1200, ui.add($);
    }
    if (!ui.children.length) return;
    Yo = performance.now() + 900;
    const m = () => {
      const v = Yo - performance.now();
      if (v <= 0) {
        Uo(), l();
        return;
      }
      Xo.opacity = Math.min(1, v / 900) * 0.95, l(), requestAnimationFrame(m);
    };
  ...
  }), window.__hekatanOsnapCompute = hl, window.__hekatanOsnapShow = ln, window.__hekatanOsnapHide = rn;
  ...
  const Ei = () => {
    ...
    Rs.style.bottom = Math.round(y + 8) + "px";
  };
  Ei(), window.addEventListener("resize", Ei), setTimeout(Ei, 500), setTimeout(Ei, 2e3), setTimeout(Ei, 5e3);
  const ul = () => {
    var _a3, _b, _c2;
    const u = [];
    ...
    var _a3, _b, _c2, _d;
    const u = ((_c2 = (_b = (_a3 = window.__hekatanCadState) == null ? void 0 : _a3.get) == null ? void 0 : _b.call(_a3)) == null ? void 0 : _c2.tool) ?? "select", g = ((_d = r.polylines) == null ? void 0 : _d.rawVal) ?? [], y = g.length ? g[g.length - 1] : [], m = qe.length, v = (A, k = []) => ({ txt: A, ops: k });
    switch (u) {
      case "line":
  ...
  const pi = [], dn = [], ml = () => {
    ...
    var _a3, _b;
    return { p: JSON.parse(JSON.stringify(r.points.rawVal ?? [])), l: JSON.parse(JSON.stringify(((_a3 = r.polylines) == null ? void 0 : _a3.rawVal) ?? [])), a: JSON.parse(JSON.stringify(((_b = r.areas) == null ? void 0 : _b.rawVal) ?? [])), x: ml() };
  }, qo = (u) => {
    var _a3;
    ...
    qe = [], Pe.visible = false, Zt.visible = false, ot();
    try {
      (_a3 = window.__hekatanRebuild) == null ? void 0 : _a3.call(window);
    } catch {
    }
    ...
    dn.push(Un()), qo(u), we(`\u21B6 Deshacer \u2014 quedan ${pi.length}`);
  }, jo = () => {
    const u = dn.pop();
    if (!u) {
    ...
    pi.push(Un()), qo(u), we(`\u21B7 Rehacer \u2014 quedan ${dn.length}`);
  };
  window.__hekatanPushUndo = At, window.__hekatanUndo = hn, window.__hekatanRedo = jo, document.addEventListener("keydown", (u) => {
    var _a3;
    const g = u.key.toLowerCase();
    if (!((u.ctrlKey || u.metaKey) && (g === "y" || g === "z" && u.shiftKey))) return;
    const m = u.target;
    m && (m.tagName === "INPUT" || m.tagName === "TEXTAREA") && m.type !== "checkbox" && m.type !== "range" && (((_a3 = m.value) == null ? void 0 : _a3.length) ?? 0) > 0 && m.__hkSucio || (u.preventDefault(), u.stopPropagation(), jo());
    ...
    if (g === "c" || g === "cerrar" || g === "close") {
      ...
      At(), r.polylines.val = [...m.slice(0, -1), [...v, v[0]], []];
      try {
        (_d = window.__hekatanRebuild) == null ? void 0 : _d.call(window);
      } catch {
      }
    ...
    if (g === "u" || g === "deshacer" || g === "undo") {
      ...
      At();
      const A = v[v.length - 1], k = v.slice(0, -1), M = m.some((T, N) => N !== m.length - 1 && T.includes(A)) || k.includes(A);
      let $ = r.points.rawVal, I = [...m.slice(0, -1), k];
      if (!M && A === $.length - 1 && ($ = $.slice(0, -1), r.points.val = $), r.polylines.val = I, k.length) {
        const T = $[k[k.length - 1]];
        T && (Ye = [T[0], T[1], T[2]]);
      } else Ye = null, Pe.visible = false;
      try {
  ...
  const qn = () => {
    qe = [], Ut = null, Yn(), zt = null, $o(), Pe.visible = false, Zt.visible = false, ot(), we("\u23F9 Dibujo finalizado \u2014 click para empezar otra serie"), l(), ss();
  };
  ...
  window.__hekatanEscapeCancel = Wo;
  const Zo = () => {
    var _a3;
    ...
    return Ue.forEach((y) => {
      if (y.startsWith("pt:")) g.add(+y.slice(3));
        ...
        const m = y.split(":"), v = u[+m[1]] || [], A = v[+m[2]], k = v[+m[2] + 1];
        A != null && g.add(A), k != null && g.add(k);
      }
    }), g;
  }, Ko = (u, g, y) => {
    var _a3;
    ...
    if (!m.size) return 0;
    At();
    const v = r.points.rawVal.map((A, k) => m.has(k) ? [A[0] + u, A[1] + g, A[2] + y] : A);
    r.points.val = v;
    try {
      (_a3 = window.__hekatanRebuild) == null ? void 0 : _a3.call(window);
    } catch {
  ...
  const Go = (u, g) => {
    ...
    qe = [], Pe.visible = false;
    let A = 0;
    u === "move" ? A = Ko(v[0], v[1], v[2]) : (A = Zo().size, (_c2 = window.__hekatanReplicateSelection) == null ? void 0 : _c2.call(window, v[0], v[1], v[2], 1)), we(`\u2713 ${u === "move" ? "Movidos" : "Copiados"} ${A} nudo${A === 1 ? "" : "s"} \u2014 \u0394 (${v[0].toFixed(2)}, ${v[1].toFixed(2)}, ${v[2].toFixed(2)}) m.`), u === "move" && (Ue.clear(), rs()), (_e2 = (_d = window.__hekatanCadState) == null ? void 0 : _d.setTool) == null ? void 0 : _e2.call(_d, "select"), ss();
  };
  ...
  const gl = () => {
  ...
  }, Ps = (u, g) => Math.hypot(u[0] - g[0], u[1] - g[1], u[2] - g[2]), jn = (u, g, y, m, v, A) => {
    ...
    if (!v && (j < -1e-6 || j > 1 + 1e-6) || !A && (O < -1e-6 || O > 1 + 1e-6)) return null;
    const z = [u[0] + j * k[0], u[1] + j * k[1], u[2] + j * k[2]], V = [y[0] + O * M[0], y[1] + O * M[1], y[2] + O * M[2]];
    return Ps(z, V) > 1e-4 ? null : z;
  }, wl = (u) => {
    var _a3;
    return (((_a3 = r.polylines) == null ? void 0 : _a3.rawVal) ?? []).reduce((g, y) => g + y.filter((m) => m === u).length, 0);
  }, yl = { offset: "DESFASE", trim: "RECORTAR", extend: "ALARGAR" }, xl = (u, g) => {
    var _a3, _b;
    if (!r.polylines) return;
    const y = r.polylines.rawVal, m = r.points.rawVal, v = yl[u];
    if (!Ut) {
      if (gs < 0) {
        we(`${v}: pase el cursor por una l\xEDnea (se pone roja) y haga clic.`);
        return;
      }
      Ut = { poly: gs, seg: Math.max(0, zs) }, we(u === "offset" ? `DESFASE l\xEDnea #${Ut.poly + 1} designada \u2014 clic en el lado hacia el que va la copia${Zs > 0 ? ` (${Zs} m)` : ""}.` : u === "trim" ? "RECORTAR contorno designado \u2014 clic en el trozo de l\xEDnea a quitar." : "ALARGAR contorno designado \u2014 clic en la l\xEDnea a alargar, cerca del extremo libre."), ss();
      return;
    }
    if (u === "offset") {
      ...
      const z = O.length > 2 && O[0] === O[O.length - 1], V = gl(), H = [];
      for (let De = 0; De < O.length - 1; De++) {
        const Je = m[O[De]], Be = m[O[De + 1]], Re = [Be[0] - Je[0], Be[1] - Je[1], Be[2] - Je[2]], Xe = Math.hypot(Re[0], Re[1], Re[2]) || 1, We = Re[0] / Xe, xt = Re[1] / Xe, St = Re[2] / Xe, Ot = [V[1] * St - V[2] * xt, V[2] * We - V[0] * St, V[0] * xt - V[1] * We], hs = Math.hypot(Ot[0], Ot[1], Ot[2]) || 1;
        H.push({ a: Je, b: Be, n: [Ot[0] / hs, Ot[1] / hs, Ot[2] / hs] });
      }
      let ee = 0, Z = 1 / 0;
      H.forEach((De, Je) => {
      ...
      for (let De = 0; De < Ee; De++) !z && De === 0 ? ke.push(ie[0].a) : !z && De === de ? ke.push(ie[de - 1].b) : ke.push(me(De));
      At();
      const nt = m.length;
      r.points.val = [...m, ...ke];
    ...
    let A = gs, k = Math.max(0, zs);
    if (A < 0 || A === Ut.poly && k === Ut.seg) {
      let O = (window.__hekatanSnap2D ?? 0.5) * 1.5;
      if (A = -1, y.forEach((z, V) => {
    ...
    const M = y[Ut.poly], $ = m[M[Ut.seg]], I = m[M[Ut.seg + 1]], T = y[A], N = T[k], F = T[k + 1];
    if (!$ || !I || N == null || F == null) {
    ...
    if (u === "trim") {
      ...
      r.polylines.val = y.map((H, ee) => ee === A ? z : H);
      const V = Ps(g, L) < Ps(g, Y);
      zo(A, V ? k : k + 1), we(`\u2713 Recortado en (${j[0].toFixed(2)}, ${j[1].toFixed(2)}, ${j[2].toFixed(2)}). Designe otro trozo o Esc.`);
    } else {
      ...
      if (At(), wl(V) > 1) {
        ...
        r.points.val = [...m, j];
        const Z = T.slice();
        Z[z] = ee, r.polylines.val = y.map((J, se) => se === A ? Z : J);
  ...
  window.__hekatanSelectionSize = () => Ue.size, window.__hekatanSelectLast = () => {
    ...
    return Ue.clear(), g >= 0 && Ue.add(`poly:${g}`), rs(), we(g >= 0 ? "SELECCI\xD3N 1 objeto (el \xFAltimo dibujado) \xB7 Esc suelta" : "No hay ning\xFAn objeto dibujado todav\xEDa."), Ue.size;
    ...
    return u.forEach((m, v) => {
      !m || m.length < 2 || (Ue.add(`poly:${v}`), m.forEach((A) => y.add(A)));
    }), g.forEach((m, v) => {
      y.has(v) || Ue.add(`pt:${v}`);
    }), rs(), we(`SELECCI\xD3N ${Ue.size} objetos (todo el modelo) \xB7 Esc suelta`), Ue.size;
  }, window.__hekatanReplicateSelection = (u, g, y, m, v = 0) => {
    var _a3, _b, _c2, _d;
    m = Math.max(1, Math.round(m || 1)), v = Math.max(0, Math.round(v || 0));
    const A = [...Ue], k = r.points.rawVal, M = ((_a3 = r.polylines) == null ? void 0 : _a3.rawVal) ?? [], $ = new Set(((_b = r.areas) == null ? void 0 : _b.rawVal) ?? []), I = /* @__PURE__ */ new Set(), T = /* @__PURE__ */ new Set(), N = [];
    if (A.forEach((O) => {
      if (O.startsWith("pt:")) {
        const z = +O.slice(3);
        k[z] && I.add(z);
    ...
    const v = [...Ue], A = r.points.rawVal, k = ((_a3 = r.polylines) == null ? void 0 : _a3.rawVal) ?? [], M = new Set(((_b = r.areas) == null ? void 0 : _b.rawVal) ?? []), $ = /* @__PURE__ */ new Set(), I = [], T = /* @__PURE__ */ new Set();
    ...
    const L = [...((_c2 = r.areas) == null ? void 0 : _c2.rawVal) ?? []], Y = /* @__PURE__ */ new Map(), j = (V, H) => {
      ...
      let Z = Y.get(ee);
      if (Z == null) {
        const J = [A[V][0] + u * H, A[V][1] + g * H, A[V][2] + y * H];
    ...
    const m = g.losa !== false, v = g.vigaBorde !== false, A = g.lados === "afuera" ? "afuera" : "ambos", k = r.points.rawVal, M = ((_a3 = r.polylines) == null ? void 0 : _a3.rawVal) ?? [], $ = [];
    if ([...Ue].forEach((j) => {
      if (j.startsWith("seg:")) {
    ...
    I /= Math.max(1, k.length), T /= Math.max(1, k.length), At();
    const N = [...k];
    let F = M.slice();
    F.length && F[F.length - 1].length === 0 && (F = F.slice(0, -1));
    ...
    for (const [j, O] of $) {
      ...
      const de = A === "ambos" ? [1, -1] : [1];
      for (const me of de) {
        const ke = J * y * me, Ee = se * y * me, nt = N.length;
        N.push([z[0] + ke, z[1] + Ee, z[2]]);
        const je = N.length;
        N.push([V[0] + ke, V[1] + Ee, V[2]]), F.push([j, nt]), F.push([O, je]), v && F.push([nt, je]), m && (L.push(F.length), F.push([j, O, je, nt, j])), Y++;
      }
    }
    if (!Y) return 0;
    F.push([]), r.points.val = N, r.polylines && (r.polylines.val = F), r.areas && (r.areas.val = L);
    ...
    if (!m.length) return;
    if (!y) {
      const A = g.position.distanceTo(i.target) || 1, k = m[0].distance ?? g.position.distanceTo(m[0].point), M = m[0].point;
      if (!isFinite(M.x) || !isFinite(M.y) || !isFinite(M.z) || k > Math.max(A * 12, 300)) {
    ...
    {
      const A = ((_a3 = r.polylines) == null ? void 0 : _a3.rawVal) ?? [], k = A[A.length - 1] ?? [], M = r.points.rawVal ?? [];
      if (k.length > 0) {
    ...
    if (Xt && Math.abs(u.clientX - Xt.x) <= 3 && Math.abs(u.clientY - Xt.y) <= 3) v = Xt.p.clone();
      ...
      const A = Nn(v), k = (_b = window.__hekatanOsnapCompute) == null ? void 0 : _b.call(window, v.x, v.y, v.z, A, { x: u.clientX, y: u.clientY });
  ...
  const Jo = (u, g) => {
    ...
    if (y === "select" || y === "none" || !y) {
      if (bs) {
        ts && on();
        const { kind: k, a: M, b: $ } = bs, I = $ !== void 0 ? `${k}:${M}:${$}` : `${k}:${M}`;
        !!g && (g.ctrlKey || g.metaKey || g.shiftKey) || Ue.clear(), Ue.has(I) ? Ue.delete(I) : Ue.add(I), rs(), we(`\u2713 Seleccionados ${Ue.size} elemento(s) \u2014 Ctrl+Click para multi-selecci\xF3n`);
      } else {
        const k = !!g && (g.ctrlKey || g.metaKey || g.shiftKey), M = (g == null ? void 0 : g.clientX) ?? 0, $ = (g == null ? void 0 : g.clientY) ?? 0;
        ts ? (Vo(ts.x, ts.y, M, $, k), ts = null) : k || (ts = { x: M, y: $ }, we("\u{1F5B1} Click 2 para cerrar el rect\xE1ngulo (\u2192 derecha=Window azul, \u2190izquierda=Crossing verde). Esc=cancelar."), Vn(M, $, M + 1, $ + 1, false));
      }
      return;
    }
    if (y === "axis") {
    ...
    if (y === "delete") {
      if (Is >= 0) {
        ...
        const k = gs, M = zs;
        ((_g = (_f = r.areas) == null ? void 0 : _f.rawVal) == null ? void 0 : _g.includes(k)) ?? false ? (Ji(k), we(`\u{1F5D1} \xC1rea #${k + 1} (shell Q4) borrada`)) : M >= 0 ? (zo(k, M), we(`\u{1F5D1} Segmento ${M + 1} de polil\xEDnea #${k + 1} borrado`)) : (Ji(k), we(`\u{1F5D1} Polil\xEDnea #${k + 1} borrada`));
    ...
    if (y === "ifcline") {
      ...
      const T = R.reduce((N, F, L) => L ? N + F.distanceTo(R[L - 1]) : 0, 0);
    ...
    if (y === "fillarea") {
      ...
      for (const oe of L) for (const ie of $.get(oe)) if (!(ie < oe)) {
        for (const de of $.get(ie)) if (de !== oe) for (const me of $.get(de)) {
          ...
          const ke = [oe, ie, de, me].slice().sort((Ee, nt) => Ee - nt).join("-");
      ...
      for (const oe of L) for (const ie of $.get(oe)) if (!(ie < oe)) for (const de of $.get(ie)) {
        ...
        const me = [oe, ie, de].slice().sort((ke, Ee) => ke - Ee).join("-");
      ...
      const Y = ((_E = (_D = (_C = window.__hekatanCadState) == null ? void 0 : _C.get) == null ? void 0 : _D.call(_C)) == null ? void 0 : _E.workPlane) ?? "xy", j = (oe) => Y === "xy" ? [oe[0], oe[1]] : Y === "xz" ? [oe[0], oe[2]] : [oe[1], oe[2]], O = j([u.x, u.y, u.z]), z = (oe, ie) => {
        ...
        for (let me = 0, ke = ie.length - 1; me < ie.length; ke = me++) {
          const Ee = ie[me][0], nt = ie[me][1], je = ie[ke][0], et = ie[ke][1];
          nt > oe[1] != et > oe[1] && oe[0] < (je - Ee) * (oe[1] - nt) / (et - nt) + Ee && (de = !de);
        }
        return de;
      }, V = (oe) => {
        let ie = 0;
    ...
    if (y === "extl") {
      const k = (window.__hekatanSnap2D ?? 0.5) * 1.5, M = Rn(u.x, u.y, u.z, k);
      if (!M) {
    ...
    let A = v.findIndex((k) => Math.abs(k[0] - m[0]) < 1e-3 && Math.abs(k[1] - m[1]) < 1e-3 && Math.abs(k[2] - m[2]) < 1e-3);
    if (A < 0 && (r.points.val = [...v, m], A = r.points.rawVal.length - 1), r.polylines && y !== "node") {
      const k = r.polylines.rawVal, M = k.length ? k[k.length - 1] : [];
      M.length && M[M.length - 1] === A ? r.polylines.val = [...k, [A]] : r.polylines.val = [...k.slice(0, -1), [...M, A]];
    }
    if (r.polylines) {
  ...
  a.addEventListener("click", () => ss()), a.addEventListener("contextmenu", (u) => {
    ...
    if (Ne.geometry.deleteAttribute("position"), y.length) {
      ...
      const v = Nn(m), A = (_b = window.__hekatanOsnapCompute) == null ? void 0 : _b.call(window, m.x, m.y, m.z, v, { x: u.clientX, y: u.clientY });
      if (A) m.set(A.x, A.y, A.z);
      else {
    ...
    if (m.length && v.length) {
      const A = new te(...r.points.rawVal[m[0].index]), k = new te(...v[0].point), M = A.sub(k), $ = (_a3 = v[0].face) == null ? void 0 : _a3.normal;
  ...
  a.addEventListener("pointermove", (u) => {
    ...
    const A = [...r.points.rawVal];
    if (Zn !== void 0) {
      let k = v[0].point;
      (u.ctrlKey || u.metaKey) && (k = new te(Math.round(k.x), Math.round(k.y), Math.round(k.z))), A[Zn] = k.toArray();
    }
    r.points.val = A;
  }), a.addEventListener("pointerup", () => {
    ...
    if (!y) return;
    const A = [...r.points.rawVal];
    if (A.splice(m[0].index, 1), r.points.val = A, !r.polylines) return;
...
function yr(r, e, t, s) {
  const i = Yl(t, r.elements, s);
  return xe.derive(() => {
    i.visible = e.shellResults.val != "none";
  }), i;
}
const xr = 6, ao = 10, br = 0.012;
function vr(r) {
...
function kr(r, e, t, s) {
  const i = new ft(), n = new Na();
  n.setColorMap("rainbow");
  const o = new $t(), a = xe.state([]);
  return xe.derive(() => {
    var _a2, _b, _c2;
    e.deformedShape.val;
    const l = t.val, c = ((_a2 = r.elements) == null ? void 0 : _a2.val) ?? [], h = vr(e.frameResults.val);
    ...
    for (const B of l) for (let Q = 0; Q < 3; Q++) S[Q] = Math.min(S[Q], B[Q]), E[Q] = Math.max(E[Q], B[Q]);
    const X = Math.max(E[0] - S[0], E[1] - S[1], E[2] - S[2], 1) * br, q = [], G = [], W = [];
    let U = 0;
    for (const { idx: B, vals: Q } of b) {
      ...
      le.normalize();
      const ae = Math.abs(le.y) < 0.99 ? new te(0, 1, 0) : new te(1, 0, 0), ue = new te().crossVectors(le, ae).normalize(), he = new te().crossVectors(le, ue).normalize(), Se = ao + 1, ve = xr;
    ...
    const D = new gt({ vertexColors: true, side: Rt }), P = new pt(K, D);
    P.frustumCulled = false, i.add(P);
...
const Sr = { kN: 1, tonf: 1 / 9.80665, kip: 1 / 4.4482216 }, Mr = { mm: 1e3, cm: 100, m: 1, in: 39.3700787402 }, $r = { "kN/m\xB2": 1, kPa: 1, MPa: 1 / 1e3, GPa: 1 / 1e6, "kgf/cm\xB2": 1 / 98.0665, "tonf/m\xB2": 1 / 9.80665, psi: 1 / 6.89476, ksi: 1 / 6894.76, "kip/ft\xB2": 1 / 47.88026 };
function Ft(r, e = 4) {
  return r == null || !isFinite(r) ? "\u2014" : r === 0 ? "0" : Math.abs(r) < 1e-3 || Math.abs(r) > 1e5 ? r.toExponential(e) : r.toFixed(e);
}
const Ar = 16755200, ba = 56831, Er = 56831, Tr = 56831, wn = 65382;
function zr(r) {
  const e = new ft();
  e.name = "__hekatan_hover", e.renderOrder = 99;
  const t = new gi(1, 16, 16), s = new gt({ color: Ar, transparent: true, opacity: 0.85, depthTest: false }), i = new pt(t, s);
  i.visible = false, i.renderOrder = 100, e.add(i);
  const n = new ze(), o = new mt({ color: ba, linewidth: 4, transparent: true, opacity: 0.9, depthTest: false }), a = new is(n, o);
  a.visible = false, a.renderOrder = 100, e.add(a);
  const l = new gt({ color: ba, transparent: true, opacity: 0.7, depthTest: false }), c = new pt(new da(1, 1, 1, 12), l);
  c.visible = false, c.renderOrder = 100, e.add(c);
  const h = new ze(), p = new gt({ color: Er, transparent: true, opacity: 0.45, side: Rt, depthTest: false }), f = new pt(h, p);
  f.visible = false, f.renderOrder = 100, e.add(f);
  const x = new ze(), b = new mt({ color: Tr, linewidth: 3, transparent: true, opacity: 0.95, depthTest: false }), C = new is(x, b);
  C.visible = false, C.renderOrder = 100, e.add(C);
  const _ = new gt({ color: wn, transparent: true, opacity: 0.95, depthTest: false }), S = new gt({ color: wn, transparent: true, opacity: 0.85, depthTest: false }), E = new da(1, 1, 1, 12), R = new gt({ color: wn, transparent: true, opacity: 0.55, side: Rt, depthTest: false }), X = new mt({ color: wn, linewidth: 4, transparent: true, opacity: 1, depthTest: false }), q = [];
  ...
  function K(Ce, ce) {
    ...
    const Ie = /* @__PURE__ */ new Map(), Ne = (at) => {
      ...
      const Fe = Qe.clone().project(_e), st = (Fe.x * 0.5 + 0.5) * ge.width, Pe = (-Fe.y * 0.5 + 0.5) * ge.height, ht = { x: st, y: Pe, z: Fe.z };
    ...
    for (let at = 0; at < Te.length; at++) {
      ...
      const Fe = Qe.x - ye, st = Qe.y - Le, Pe = Math.sqrt(Fe * Fe + st * st);
      Pe < Ye && (Ye = Pe, tt = at);
    }
    const Ze = Cr(), $e = Mr[Ze.dispUnit] ?? 1e3, Ve = Sr[Ze.forceUnit] ?? 1;
    if (tt >= 0) {
      const at = Te[tt];
      let Qe = `Nodo ${tt}
...
Rz = ${Ft(st[5] * 1e3, 3)} mrad`)), Fe.reactions) {
          const Pe = Fe.reactions.get(tt);
          Pe && (Math.abs(Pe[0]) > 1e-9 || Math.abs(Pe[1]) > 1e-9 || Math.abs(Pe[2]) > 1e-9 || Math.abs(Pe[3]) > 1e-6 || Math.abs(Pe[4]) > 1e-6 || Math.abs(Pe[5]) > 1e-6) && (Qe += `
\u2500\u2500\u2500\u2500 R reacciones \u2500\u2500\u2500\u2500`, Qe += `
Fx = ${Ft(Pe[0] * Ve)} ${Ze.forceUnit}`, Qe += `
Fy = ${Ft(Pe[1] * Ve)} ${Ze.forceUnit}`, Qe += `
Fz = ${Ft(Pe[2] * Ve)} ${Ze.forceUnit}`, (Math.abs(Pe[3]) > 1e-6 || Math.abs(Pe[4]) > 1e-6 || Math.abs(Pe[5]) > 1e-6) && (Qe += `
Mx = ${Ft(Pe[3] * Ve)} ${Ze.forceUnit}\xB7m`, Qe += `
My = ${Ft(Pe[4] * Ve)} ${Ze.forceUnit}\xB7m`, Qe += `
Mz = ${Ft(Pe[5] * Ve)} ${Ze.forceUnit}\xB7m`));
        }
      }
      return { type: "node", idx: tt, info: Qe };
    }
    const Pt = 5;
    let ot = -1, Et = Pt, jt = "frame";
    for (let at = 0; at < Me.length; at++) {
      ...
      if (!(!Qe || Qe.length < 2)) {
        if (Qe.length === 2) {
          ...
          const Pe = Ir(ye, Le, Fe.x, Fe.y, st.x, st.y);
          Pe < Et && (Et = Pe, ot = at, jt = "frame");
          ...
          if (!st) continue;
          const Pe = Math.min(...Fe.map((Ke) => Ke.x)), ht = Math.max(...Fe.map((Ke) => Ke.x)), lt = Math.min(...Fe.map((Ke) => Ke.y)), Tt = Math.max(...Fe.map((Ke) => Ke.y));
          if (ye >= Pe && ye <= ht && Le >= lt && Le <= Tt) {
            const it = Fe.reduce((Wt, Dt) => Wt + Dt.z, 0) / Fe.length * 1e-3;
            it < Et && (Et = it, ot = at, jt = "solid");
          }
        }
      }
    }
    if (ot >= 0) {
      ...
      const st = (_e2 = (_d = r.mesh) == null ? void 0 : _d.elementInputs) == null ? void 0 : _e2.rawVal, Pe = (_g = (_f = st == null ? void 0 : st.sectionInfo) == null ? void 0 : _f.get) == null ? void 0 : _g.call(_f, ot);
      if (Pe) {
        Pe.name && (Fe += `
  \u{1F4CB} ${Pe.name}`), Pe.shape && (Fe += `
  Shape: ${Pe.shape}`);
        const ht = /concrete|hormig|rect.*sólida/i.test(Pe.shape || ""), lt = ht ? 100 : 1e3, Tt = ht ? "cm" : "mm", Ke = (Wt) => {
          const Dt = Wt * lt;
          return Math.abs(Dt - Math.round(Dt)) < 0.05 ? `${Math.round(Dt)}` : `${Dt.toFixed(1)}`;
        }, it = [];
        if (Pe.D != null && it.push(`D=${Ke(Pe.D)}`), Pe.B != null && it.push(`B=${Ke(Pe.B)}`), Pe.TF != null && it.push(`TF=${Ke(Pe.TF)}`), Pe.TW != null && it.push(`TW=${Ke(Pe.TW)}`), Pe.t != null && it.push(`t=${Ke(Pe.t)}`), it.length && (Fe += `
  Dim: ${it.join(" ")} ${Tt}`), Pe.material) {
          let Wt = Pe.material;
          Pe.fillMaterial && (Wt += ` + FILL "${Pe.fillMaterial}"`), Fe += `
  Mat: ${Wt}`;
        }
      } else {
...
T \u2248 ${Ft(Us * Ve)} ${Ze.forceUnit}\xB7m`), Math.abs(ps) > 1e-6 && (Fe += `
My \u2248 ${Ft(ps * Ve)} ${Ze.forceUnit}\xB7m`), Math.abs(li) > 1e-6 && (Fe += `
Mz \u2248 ${Ft(li * Ve)} ${Ze.forceUnit}\xB7m`);
  ...
  function D(Ce, ce, _e) {
    ...
    if (Ce.type === "node") {
      ...
      if (Me) {
        ...
        const Ge = ((_b = r.derivedDisplayScale) == null ? void 0 : _b.rawVal) ?? 1, Ae = 0.021 * Ne * Ge;
        i.position.copy(Me), i.scale.setScalar(Ae), i.visible = true;
      }
    } else if (Ce.type === "frame" && ge) {
      const Me = ge[Ce.idx], Ie = U(Me[0]), Ne = U(Me[1]);
      if (Ie && Ne) {
        const Ge = Ie.clone().add(Ne).multiplyScalar(0.5), Ae = Ne.clone().sub(Ie), tt = Ae.length(), Ye = Math.max(1e-4, 3.5 * ve(Ge));
        c.position.copy(Ge);
        const Ze = new te(0, 1, 0), $e = Ze.clone().cross(Ae).normalize(), Ve = Ze.angleTo(Ae);
      ...
      for (const [Ge, Ae] of Ie) {
        const tt = U(Me[Ge]), Ye = U(Me[Ae]);
  ...
  const re = window.__hekatanHoverDebug ?? false, pe = (Ce) => {
    B && cancelAnimationFrame(B), B = requestAnimationFrame(() => {
      ...
      if (_e !== P) P = _e, D(ce, Ce.clientX, Ce.clientY);
      else if (ce) {
  ...
  let be = null;
  const le = () => {
    P = "", i.visible = false, a.visible = false, c.visible = false, f.visible = false, C.visible = false, W.style.display = "none", r.render();
  }, ne = (Ce) => {
  ...
  function Oe(Ce, ce) {
    ...
    if (Ce.type === "node") {
      ...
      if (!ye || !Le) return;
      const Te = ye.clone().add(Le).multiplyScalar(0.5), Me = Le.clone().sub(ye), Ie = Me.length(), Ne = Math.max(1e-4, 4 * ve(Te)), Ge = new pt(E, S);
      Ge.position.copy(Te);
      const Ae = new te(0, 1, 0);
      Ge.quaternion.setFromAxisAngle(Ae.clone().cross(Me).normalize(), Ae.angleTo(Me)), Ge.scale.set(Ne, Ie, Ne), Ge.renderOrder = 101, G.add(Ge);
      ...
      for (const [Ie, Ne] of ye) {
        const Ge = U(ge[Ie]), Ae = U(ge[Ne]);
        Ge && Ae && Le.push(Ge.x, Ge.y, Ge.z, Ae.x, Ae.y, Ae.z);
      }
      const Te = new ze();
...
const Rr = { normals: "Axial", torsions: "Torsi\xF3n", shearsY: "Cortante 2-2", shearsZ: "Cortante 3-3", bendingsY: "Momento 2-2", bendingsZ: "Momento 3-3" }, Pr = { normals: "kN", torsions: "kN\xB7m", shearsY: "kN", shearsZ: "kN", bendingsY: "kN\xB7m", bendingsZ: "kN\xB7m" }, Ls = 1e-3;
function Ii(r, e) {
...
function Lr(r, e) {
  ...
  const i = () => {
    var _a3, _b2;
    const E = ((_a3 = e == null ? void 0 : e.frameResults) == null ? void 0 : _a3.rawVal) ?? ((_b2 = e == null ? void 0 : e.frameResults) == null ? void 0 : _b2.val);
    return !E || E === "none" ? null : String(E).replace(/^contour:/, "");
  }, n = (E) => {
    var _a3, _b2;
    const R = ((_a3 = r.nodes) == null ? void 0 : _a3.rawVal) ?? [], X = ((_b2 = r.elements) == null ? void 0 : _b2.rawVal) ?? [], q = /* @__PURE__ */ new Set();
    for (const G of X) {
      if (G.length !== 2) continue;
      const W = R[G[0]], U = R[G[1]];
      if (!W || !U) continue;
      const K = Ii(W, E), D = Ii(U, E);
  ...
  function o(E) {
    var _a3, _b2;
    if (E == null ? void 0 : E.plano) s = { plano: E.plano, en: E.en ?? n(E.plano)[0] ?? 0 };
    else {
      ...
      X && G[X.idx] && q[G[X.idx][0]] && q[G[X.idx][1]] ? s = Dr(q[G[X.idx][0]], q[G[X.idx][1]]) : s = { plano: "XZ", en: n("XZ")[0] ?? 0 };
    }
  ...
  function a() {
    ...
    const E = t.querySelector(".hk-d2-plano"), R = t.querySelector(".hk-d2-en");
    E.addEventListener("change", () => {
      s = { plano: E.value, en: n(E.value)[0] ?? 0 }, l();
    }), R.addEventListener("change", () => {
      s.en = Number(R.value), l();
    });
    const X = (W) => {
      const U = n(s.plano), K = U.findIndex((P) => Math.abs(P - s.en) < Ls), D = Math.max(0, Math.min(U.length - 1, (K < 0 ? 0 : K) + W));
      U.length && (s.en = U[D], l());
    };
  ...
  function l() {
    ...
    const E = new Set(p && !p.hidden && f >= 0 ? b(f) : []), R = t.querySelector(".hk-d2-svg"), X = t.querySelector(".hk-d2-tit"), q = t.querySelector(".hk-d2-pie"), G = t.querySelector(".hk-d2-plano"), W = t.querySelector(".hk-d2-en");
    G.value = s.plano;
    ...
    const P = i(), B = ((_a3 = r.nodes) == null ? void 0 : _a3.rawVal) ?? [], Q = ((_b2 = r.elements) == null ? void 0 : _b2.rawVal) ?? [], re = P ? (_d = (_c2 = r.analyzeOutputs) == null ? void 0 : _c2.rawVal) == null ? void 0 : _d[P] : null;
    R.innerHTML = "";
    const pe = R.clientWidth || 880, be = R.clientHeight || 480, le = [];
    if (Q.forEach(($e, Ve) => {
      if ($e.length !== 2) return;
      const Pt = B[$e[0]], ot = B[$e[1]];
      if (!Pt || !ot) return;
      const Et = Ii(Pt, s.plano), jt = Ii(ot, s.plano);
      Math.abs(Et.fuera - s.en) < Ls && Math.abs(jt.fuera - s.en) < Ls && le.push({ i: Ve, a: Et, b: jt });
    }), !le.length) {
    ...
    const Se = ae - ne || 1, ve = he - ue || 1, Oe = 0.12 * Math.max(Se, ve), He = 46, dt = Math.min((pe - 2 * He) / (Se + 2 * Oe), (be - 2 * He) / (ve + 2 * Oe)), wt = (pe - Se * dt) / 2, Ce = (be - ve * dt) / 2, ce = ($e) => wt + ($e - ne) * dt, _e = ($e) => be - (Ce + ($e - ue) * dt), ge = "http://www.w3.org/2000/svg", ye = ($e, Ve, Pt) => {
      const ot = document.createElementNS(ge, $e);
      for (const Et in Ve) ot.setAttribute(Et, String(Ve[Et]));
      return Pt != null && (ot.textContent = Pt), R.appendChild(ot), ot;
    }, Le = /* @__PURE__ */ new Map();
    for (const $e of le) {
      const Ve = ((_h = (_g = (_f = (_e2 = r.elementInputs) == null ? void 0 : _e2.rawVal) == null ? void 0 : _f.localAngles) == null ? void 0 : _g.get) == null ? void 0 : _h.call(_g, $e.i)) ?? 0, Pt = Ii(Xa(P ?? "normals", Ha(B[Q[$e.i][0]], B[Q[$e.i][1]], Ve)), s.plano), ot = Math.hypot(Pt.u, Pt.v);
      Le.set($e.i, ot > 0.3 ? [Pt.u / ot, -Pt.v / ot] : null);
    }
    ...
    const Ie = 0.12 * Math.max(Se, ve) * dt, Ne = Me > 0 ? Ie / Me : 0, Ge = P === "bendingsY" || P === "bendingsZ", Ae = ($e) => Math.abs($e) >= 100 ? $e.toFixed(1) : Math.abs($e) >= 10 ? $e.toFixed(2) : $e.toFixed(3), tt = [];
    for (const $e of le) {
      const Ve = ce($e.a.u), Pt = _e($e.a.v), ot = ce($e.b.u), Et = _e($e.b.v), jt = Le.get($e.i), [at, Qe] = jt ?? [0, 0], Fe = re && jt ? re instanceof Map ? re.get($e.i) : re[$e.i] : null, [st, Pe] = Fe ? fo(P, Fe) : [0, 0];
      if (Fe && Ne > 0) {
        const Ke = [Ve + at * st * Ne * 1, Pt + Qe * st * Ne * 1], it = [ot + at * Pe * Ne * 1, Et + Qe * Pe * Ne * 1], _t = st + Pe >= 0 ? "#3fa7d6" : "#d9534f";
        ye("polygon", { points: `${Ve},${Pt} ${Ke[0]},${Ke[1]} ${it[0]},${it[1]} ${ot},${Et}`, fill: _t, "fill-opacity": 0.38, stroke: _t, "stroke-width": 1.2 }), tt.push({ x: Ke[0] + at * 12, y: Ke[1] + Qe * 12, t: Ae(st), peso: Math.abs(st) }), tt.push({ x: it[0] + at * 12, y: it[1] + Qe * 12, t: Ae(Pe), peso: Math.abs(Pe) });
      }
      ye("line", { x1: Ve, y1: Pt, x2: ot, y2: Et, stroke: "#e6ecf5", "stroke-width": 2.2, "stroke-linecap": "round" }), E.has($e.i) && ye("line", { x1: Ve, y1: Pt, x2: ot, y2: Et, stroke: "#e6c463", "stroke-width": 5, "stroke-linecap": "round" });
      const ht = ye("line", { x1: Ve, y1: Pt, x2: ot, y2: Et, stroke: "transparent", "stroke-width": 14, style: "cursor:pointer;pointer-events:stroke" });
    ...
    const Ze = P ? Rr[P] ?? P : "sin resultado";
    X.textContent = `${Ze} \xB7 ${s.plano === "XY" ? "planta" : "alzado"} ${s.plano} en ${K} = ${s.en.toFixed(2)} m`, q.textContent = P ? `${le.length} barras en el plano \xB7 m\xE1ximo ${Ae(Me)} ${Pr[P] ?? ""}` + (Ge ? " \xB7 el momento va del lado de la tracci\xF3n" : "") + (Te ? ` \xB7 ${Te} barra(s) con este resultado en otro plano (sin dibujar)` : "") + " \xB7 clic en una barra: su gr\xE1fico" : "Eleg\xED un resultado en \xABFrame results\xBB (Axial, Cortante, Momento) para ver su diagrama aqu\xED.";
  }
  const c = () => {
    try {
      l();
  ...
  setInterval(() => {
    var _a3, _b2;
    const E = (_a3 = r.analyzeOutputs) == null ? void 0 : _a3.rawVal, R = (_b2 = e == null ? void 0 : e.frameResults) == null ? void 0 : _b2.rawVal, X = [E, R];
    if (!(h && h[0] === E && h[1] === R)) {
      h = X, c();
      try {
        S();
  ...
  function b(E) {
    var _a3, _b2;
    const R = ((_a3 = r.nodes) == null ? void 0 : _a3.rawVal) ?? [], X = ((_b2 = r.elements) == null ? void 0 : _b2.rawVal) ?? [], q = /* @__PURE__ */ new Map();
    X.forEach((K, D) => {
      if (K.length === 2) for (const P of K) q.has(P) || q.set(P, []), q.get(P).push(D);
    });
    const G = (K) => {
      const D = R[X[K][0]], P = R[X[K][1]], B = [P[0] - D[0], P[1] - D[1], P[2] - D[2]], Q = Math.hypot(B[0], B[1], B[2]) || 1;
      return B.map((re) => re / Q);
    }, W = (K, D) => {
      const P = G(K), B = G(D);
      return Math.abs(P[0] * B[0] + P[1] * B[1] + P[2] * B[2]) > 0.9999;
    }, U = [E];
    for (const K of [0, 1]) {
      let D = E, P = X[E][K];
      for (let B = 0; B < 500; B++) {
        const Q = (q.get(P) ?? []).filter((pe) => pe !== D);
        if (Q.length !== 1 || !W(D, Q[0])) break;
        const re = Q[0];
        K === 0 ? U.unshift(re) : U.push(re), P = X[re][0] === P ? X[re][1] : X[re][0], D = re;
      }
    }
    return U;
  }
  ...
  function S() {
    ...
    }, P = E[R[q[0]][0]], B = (ae) => ae.toFixed(2);
    p.querySelector(".hk-b-tit").textContent = "L = " + K.toFixed(2) + " m \xB7 " + q.length + " tramo(s) \xB7 desde (" + B(P[0]) + ", " + B(P[1]) + ", " + B(P[2]) + ")";
...
function va(r, e = 8) {
  ...
  return setTimeout(() => {
    xe.derive(() => {
      const l = bi(Ts.val);
      a(l ? 15 : e), t.style.background = Xl(), t.classList.toggle("legend-discrete", l), Array.from({ length: o + 1 }, (h, p) => p / o).reverse().forEach((h, p) => {
        const f = n[p];
        f && (f.innerText = Nr(r.val, h).toString());
      });
    });
  }), t;
}
...
function Mc({ mesh: r, settingsObj: e, drawingObj: t, objects3D: s, solids: i }) {
  Ol.DEFAULT_UP = new te(0, 0, 1);
  const n = document.createElement("div"), o = new Pl(), a = new Dl(45, 1, 0.1, 2 * 1e6), l = new Ll(-10, 10, 10, -10, -1e3, 2e6);
  let c = a;
  const h = new Nl({ antialias: true });
  h.localClippingEnabled = true;
  const p = new ha(a, h.domElement);
  ...
  const _ = ql(e), S = xe.derive(() => Math.pow(10, _.displayScale.val / 10)), E = Or(r, _), R = () => {
    const ce = [];
    ...
    return { planes: R(), majorStep: _e, minorStep: ce };
  };
  ...
  G(), n.appendChild(Ul(_, r, i)), n.setAttribute("id", "viewer"), n.appendChild(h.domElement), h.setPixelRatio(window.devicePixelRatio);
  const W = Vs();
  ...
  let D = xa(_.gridSize.rawVal, _.flipAxes.rawVal);
  o.add(q, D), xe.derive(() => {
  ...
  function he(ce, _e) {
    ...
    if (l.left = -Me * Te, l.right = Me * Te, l.updateProjectionMatrix(), ce && be) {
      if (le ? (le.object = be, le.update()) : (le = new ha(be, h.domElement), le.enableDamping = true, le.dampingFactor = 0.1, le.screenSpacePanning = true, le.zoomSpeed = 0.8, le.panSpeed = 1.2, le.rotateSpeed = 0.9, le.touches = { ONE: fn.ROTATE, TWO: fn.DOLLY_PAN }, le.target.copy(p.target), le.addEventListener("change", ae), le.enabled = false), !ne) {
        const Ie = (Ne) => {
          if (!pe || !le) return;
          const Ge = h.domElement.getBoundingClientRect(), Ae = Ne.clientX - Ge.left, tt = Ge.width / 2, Ye = Ae >= tt;
  ...
  if (r) {
    o.add(jl(_, E, S), Gl(r, _, E), tr(_, E, S), sr(r, _, E, S), Ql(r, _, E, S), er(r, _, E, S), or(r, _, E, S), lr(r, _, E, S), hr(r, _, E), mr(r, _, E, S), ur(r, _, E, S)), window.__hekatanDiagrama2D || (Lr(r, _), h.domElement.addEventListener("dblclick", () => {
      var _a2;
    ...
    const ce = zr({ scene: o, rendererElm: h.domElement, getActiveCamera: () => c, derivedNodes: E, derivedDisplayScale: S, mesh: r, settings: _, render: ae });
    o.add(ce);
    const _e = Ur(r, _), ge = yr(r, _, E, _e), ye = va(_e);
    o.add(ge), n.appendChild(ye);
    const Le = kr(r, _, E);
    o.add(Le);
    const Te = Le.__colorMapValues, Me = va(Te);
    Me.id = "frame-legend", n.appendChild(Me), xe.derive(() => {
      var _a2;
      const Ie = _.shellResults.val != "none", Ne = (((_a2 = _.solidResults) == null ? void 0 : _a2.val) ?? "none") !== "none", Ge = Ie || Ne, Ae = _.frameResults.val.startsWith("contour:"), tt = _e.val.some((Ye) => Number.isFinite(Ye));
      ye.hidden = !Ge || !tt, ge.visible = Ge, Me.hidden = !Ae;
    });
  }
  if (i) {
    const ce = new Da(16777215, 0.5);
    o.add(ce);
  ...
  t && gr({ drawingObj: t, gridObj: q, scene: o, getActiveCamera: () => c, controls: p, gridSize: U, derivedDisplayScale: S, rendererElm: h.domElement, viewerRender: ae }), Vi((ce, _e) => {
    var _a2;
    ...
    }), q = no(_.gridSize.rawVal, { planes: R() }), o.add(q), n.style.setProperty("--hk-legend-color", _e.legendMarker), ae();
  });
...
let _a = "";
function Ua(r) {
  r !== _a && (_a = r, console.error(`[sello] ${r}`));
}
function qa(r, e, t) {
  var _a2;
  const s = (_a2 = r == null ? void 0 : r.caseId) == null ? void 0 : _a2.val, i = e == null ? void 0 : e.caseId;
  return s && i && i !== s ? (Ua(`NO pinto ${t}: los resultados son de \xAB${i}\xBB y en pantalla esta \xAB${s}\xBB. Resultado viejo sobre modelo nuevo.`), false) : true;
}
function Pi(r, e, t) {
  if (!r || r.size === 0 || e === 0) return true;
  let s = -1;
  for (const i of r.keys()) i > s && (s = i);
  return s >= e ? (Ua(`NO pinto ${t}: el resultado llega al indice ${s} y la malla en pantalla tiene ${e}. Es de otro modelo.`), false) : true;
}
function Or(r, e) {
  return xe.derive(() => {
    var _a2, _b, _c2, _d, _e;
    ...
    if (!s || t.length === 0 || !qa(r, (_e = r == null ? void 0 : r.deformOutputs) == null ? void 0 : _e.val, "la deformada") || !Pi(s, t.length, "la deformada")) return t;
...
const Oi = xe.state(null), mo = xe.state(""), Vr = xe.state("kN"), Br = xe.state("mm"), Hr = xe.state("tonf/m\xB2"), Xr = { kN: 1, tonf: 9.80665, kip: 4.4482216 }, ka = { mm: 1e3, cm: 100, m: 1, in: 39.3700787402, ft: 3.280839895 }, Yr = { "kN/m\xB2": 1, kPa: 1, MPa: 1 / 1e3, GPa: 1 / 1e6, "kgf/cm\xB2": 1 / 98.0665, "tonf/m\xB2": 1 / 9.80665, psi: 1 / 6.89476, ksi: 1 / 6894.76, "kip/ft\xB2": 1 / 47.88026 };
function Ur(r, e) {
  const t = xe.state([]);
  let s;
  return ((i) => {
    ...
    if (r.nodes.val.forEach((ye, Le) => {
      ...
      const ye = r.nodes.val, Le = /* @__PURE__ */ new Set(), Te = (Ie, Ne) => {
        ...
        return Ie.every((Ae) => {
          var _a4;
          return Math.abs((((_a4 = ye[Ae]) == null ? void 0 : _a4[Ne]) ?? NaN) - Ge) < 1e-6;
        });
      };
      for (const Ie of r.elements.val) {
        if (Ie.length !== 4) continue;
        const Ne = Te(Ie, 2), Ge = !Ne && Te(Ie, 0), Ae = !Ne && Te(Ie, 1);
        if (Ce === "losas" ? Ne : Ce === "muros" ? Ge || Ae : Ce === "murosX" ? Ge : Ce === "murosY" ? Ae : false) for (const Ze of Ie) Le.add(Ze);
      }
      const Me = [];
      for (const Ie of Le) {
        const Ne = ge[Ie];
...
class ni {
  ...
  trigger(e, t) {
    ...
    } else t instanceof Ca || (t = new Ca(this, t), this.activeEvents.push(t));
    let s, i, n;
  ...
  message(e, t) {
    ...
    let l = { self: t, action(b) {
      ...
      typeof C == "function" && (t.buttons += `<button class="w2ui-btn w2ui-eaction" data-click='["action","${b}","event"]' name="${b}">${b}</button>`), typeof C == "object" && (t.buttons += `<button class="w2ui-btn w2ui-eaction ${C.class || ""}" name="${b}" data-click='["action","${b}","event"]'
        ...
        return t.on("action.buttons", (E) => {
          E.detail.action[0].toLowerCase() + E.detail.action.substr(1).replace(/\s+/g, "") == _ && S(E);
        }), l;
      };
    ...
    return t.tmp || (t.tmp = { zIndex: x.css("z-index"), overflow: c.overflow }), t.html === "" && t.body === "" && t.buttons === "" ? o() : (t.msgIndex = d(e.box).find(".w2ui-message").length, t.msgIndex === 0 && typeof this.lock == "function" && (d(e.box).css("overflow", "hidden"), e.owner ? e.owner.lock(e.param) : this.lock(e.box)), d(e.box).find(".w2ui-message").css("z-index", 1390), x.css("z-index", 1501), p = `
      ...
      (_b2 = b != null ? isNaN(b) ? _.find(S).filter(b).get(0) : _.find(S).get(b) : _.find("[name=hidden-first]").get(0)) == null ? void 0 : _b2.focus(), d(e.box).find(".w2ui-message").find(S + ",[name=hidden-first],[name=hidden-last]").off(".keep-focus"), d(_).find(S + ",[name=hidden-first],[name=hidden-last]").on("blur.keep-focus", function(E) {
        setTimeout(() => {
          var _a4, _b3, _c2;
          var R = document.activeElement, X = 0 < d(_).find(S).filter(R).length, q = d(R).attr("name");
          !X && R && R !== document.body && ((_a4 = d(_).find(S).get(0)) == null ? void 0 : _a4.focus()), q == "hidden-last" && ((_b3 = d(_).find(S).get(0)) == null ? void 0 : _b3.focus()), q == "hidden-first" && ((_c2 = d(_).find(S).get(-1)) == null ? void 0 : _c2.focus());
        }, 1);
      });
    }, l;
  }
...
const _ut = class _ut {
  ...
  getPosition(e) {
    ...
    if (t && t.box) {
      ...
      let b = ["top", "bottom"].includes(o[0]), C = t.box.getBoundingClientRect(), _ = t.anchor.getBoundingClientRect(), S = (t.anchor == document.body && ({ x: a, y: l, width: c, height: h } = p.originalEvent, _ = { left: a - 2, top: l - 4, width: c, height: h, arrow: "none" }), p.arrowSize), E = (_.arrow == "none" && (S = 0), { top: _.top, bottom: x.height - (_.top + _.height) - +(s ? e : 0), left: _.left, right: x.width - (_.left + _.width) + (i ? e : 0) });
      C.width < 22 && (C.width = 22), C.height < 14 && (C.height = 14);
      let R, X, q, G, W = "", U = { offset: 0, class: "", style: `#${t.id} { --tip-size: ${S}px; }` }, K = { left: 0, top: 0 }, D = { posX: "", x: 0, posY: "", y: 0 };
      o.forEach((P) => {
        ["top", "bottom"].includes(P) && (!W && C.height + S / 1.893 < E[P] && (W = P), E[P] > D.y) && Object.assign(D, { posY: P, y: E[P] }), ["left", "right"].includes(P) && (!W && C.width + S / 1.893 < E[P] && (W = P), E[P] > D.x) && Object.assign(D, { posX: P, x: E[P] });
      }), W = W || (b ? D.posY : D.posX), p.autoResize && (["top", "bottom"].includes(W) && (C.height > E[W] ? (G = E[W], t.tmp.resizedY = true) : t.tmp.resizedY = false), ["left", "right"].includes(W)) && (C.width > E[W] ? (q = E[W], t.tmp.resizedX = true) : t.tmp.resizedX = false);
      var a = W;
      switch (U.class = _.arrow || "w2ui-arrow-" + a, a) {
        case "top":
          R = _.left + (_.width - (q ?? C.width)) / 2, X = _.top - (G ?? C.height) - S / 1.5 + 1;
          break;
        case "bottom":
          R = _.left + (_.width - (q ?? C.width)) / 2, X = _.top + _.height + S / 1.25 + 1;
          break;
        case "left":
          R = _.left - (q ?? C.width) - S / 1.2 - 1, X = _.top + (_.height - (G ?? C.height)) / 2;
          break;
        case "right":
          R = _.left + _.width + S / 1.2 + 1, X = _.top + (_.height - (G ?? C.height)) / 2;
      }
      b && (p.align == "left" && (K.left = _.left - R, R = _.left), p.align == "right" && (K.left = _.left + _.width - (q ?? C.width) - R, R = _.left + _.width - (q ?? C.width)), ["top", "bottom"].includes(W) && p.align.startsWith("both") && (n = p.align.split(":")[1] ?? 50, _.width >= n) && (R = _.left, q = _.width), p.align == "top" && (K.top = _.top - X, X = _.top), p.align == "bottom" && (K.top = _.top + _.height - (G ?? C.height) - X, X = _.top + _.height - (G ?? C.height)), ["left", "right"].includes(W) && p.align.startsWith("both") && (n = p.align.split(":")[1] ?? 50, _.height >= n) && (X = _.top, G = _.height));
      {
        let P;
        (["left", "right"].includes(p.align) && _.width < (q ?? C.width) || ["top", "bottom"].includes(p.align) && _.height < (G ?? C.height)) && (P = true);
        ...
        (["top", "bottom"].includes(W) || p.autoResize) && (R < l && (P = true, K.left -= R, R = l), R > h) && (P = true, K.left -= R - h, R += h - R), (["left", "right"].includes(W) || p.autoResize) && (X < c && (P = true, K.top -= X, X = c), X > s) && (P = true, K.top -= X - s, X += s - X), P && (l = b ? "left" : "top", h = b ? "width" : "height", U.offset = -K[l], c = C[h] / 2 - S, Math.abs(U.offset) > c + S && (U.class = ""), Math.abs(U.offset) > c && (U.offset = U.offset < 0 ? -c : c), U.style = w.stripSpaces(`#${t.id} .w2ui-overlay-body:after,
                            #${t.id} .w2ui-overlay-body:before {
                                --tip-size: ${S}px;
                                margin-${l}: ${U.offset}px;
                            }`));
      }
      return i = W == "top" ? -p.margin : W == "bottom" ? p.margin : 0, e = W == "left" ? -p.margin : W == "right" ? p.margin : 0, X = Math.floor(100 * (X + parseFloat(p.offsetY) + parseFloat(i))) / 100, { left: R = Math.floor(100 * (R + parseFloat(p.offsetX) + parseFloat(e))) / 100, top: X, arrow: U, adjust: K, width: q, height: G, pos: W };
    }
  }
};
...
class Wr extends ut {
  ...
  initControls(e) {
    ...
    function f(S) {
      var R = t.el, _ = S.pageX - t.x, S = S.pageY - t.y;
      let b = t.left + _, C = t.top + S;
      var _ = parseInt(R.prop("clientWidth")) / 2, S = (b < -_ && (b = -_), C < -_ && (C = -_), b > t.width - _ && (b = t.width - _), C > t.height - _ && (C = t.height - _), R.hasClass("move-x") && R.css({ left: b + "px" }), R.hasClass("move-y") && R.css({ top: C + "px" }), d(R.get(0).parentNode).attr("name")), E = parseInt(R.css("left")) + _, R = parseInt(R.css("top")) + _;
      S === "palette" && c({ s: Math.round(E / t.width * 100), v: Math.round(100 - R / t.height * 100) }), S === "rainbow" && (c({ h: Math.round(2.4 * E) }), h()), S === "alpha" && c({ a: parseFloat(Number(E / 150).toFixed(2)) });
    }
      ...
      if (o == null && (o = null), ["radio", "check"].indexOf(e.type) == -1 || Array.isArray(c.items) || c.group === false || (o = c.checked === true ? "w2ui-icon-check" : "w2ui-icon-empty"), c.hidden !== true) {
        ...
        if (typeof (x = typeof e.render == "function" ? e.render(c, e) : x) == "function" && (x = x(c, e)), o && (String(o).slice(0, 1) !== "<" && (o = `<span class="w2ui-icon ${o}"></span>`), b = `<div class="menu-icon">${o}</span></div>`), c.type !== "break" && x != null && x !== "" && String(x).substr(0, 2) != "--") {
          ...
          if (c.remove === true) S = '<span class="remove">x</span>';
            ...
            let E = [];
            typeof c.items == "function" ? E = c.items(c) : Array.isArray(c.items) && (E = c.items), S = "<span></span>", C = `
                            <div class="w2ui-sub-menu-box" style="${c.expanded ? "" : "display: none"}">
                                ${this.getMenuHTML(e, E, true, i.concat(h))}
                            </div>`;
...
class Kr extends ut {
  ...
  getMonthHTML(e, t, s) {
    ...
    for (let E = 0; E < i.length; E++) {
      var p = h == "M" && E == 5 || h != "M" && E == 6, f = h == "M" && E == 6 || h != "M" && E == 0;
      c += `<div class="w2ui-day w2ui-weekday ${p ? "w2ui-sunday" : ""} ${f ? "w2ui-saturday" : ""}">${i[E]}</div>`;
    }
    let x = `
    ...
    for (let E = 0; E < 42; E++) {
      ...
      let R = "", X = "", q, G;
      G = e.type === "datetime" ? (q = w.formatDateTime(_, e.format), w.formatDate(_, w.settings.dateFormat)) : q = w.formatDate(_, e.format), e.colored && e.colored[G] !== void 0 && (_ = e.colored[G].split("|"), X = "background-color: " + _[0] + ";", R = "color: " + _[1] + ";"), x += `<div class="w2ui-day ${this.inRange(q, e, true) ? "w2ui-date " + (G == a ? "w2ui-selected" : "") : "w2ui-blocked"} ${C.join(" ")}"
                       style="${R + X}" date="${G}" data-date="${b.getTime()}">
                            ${S}
...
let rt = new ut(), Jt = new Zr(), ja = new Wr(), Fi = new Kr();
class Gr extends ni {
  constructor(e) {
    ...
    if (1 < s.length) (s = this.get(e)) && !s.disabled && this.menuClick({ name: this.name, item: i, subItem: s, originalEvent: t });
      ...
      if (["menu", "menu-radio", "menu-check", "drop", "color", "text-color"].includes(i.type)) {
        ...
        setTimeout(() => {
          ...
          ["color", "text-color"].includes(i.type) && ja.show(w.extend({ color: i.color }, i.overlay, { anchor: h[0], name: this.name + "-drop", data: { item: i, btn: l } })).hide(c(i.id)).select((p) => {
...
class Ac extends ni {
        ...
        for (let l = this.total = 0; l < this.records.length; l++) {
          ...
          if (function c(h) {
            ...
            for (let S = 0; S < t.searchData.length; S++) {
              let E = t.searchData[S], R = t.getSearch(E.field);
              if (E != null) {
                R == null && (R = { field: E.field, type: E.type });
                let X = t.parseField(h, R.field);
                switch (f = X == null || typeof X == "object" && X.toString == o ? "" : String(X).toLowerCase(), E.value != null && (Array.isArray(E.value) ? (x = E.value[0], b = E.value[1]) : x = String(E.value).toLowerCase()), E.operator) {
                  case "=":
                  case "is":
                    t.parseField(h, R.field) == E.value ? p++ : R.type == "date" ? (C = t.parseField(h, R.field + "_") instanceof Date ? t.parseField(h, R.field + "_") : t.parseField(h, R.field), f = w.formatDate(C, "yyyy-mm-dd"), x = w.formatDate(w.isDate(x, w.settings.dateFormat, true), "yyyy-mm-dd"), f == x && p++) : R.type == "time" ? (C = t.parseField(h, R.field + "_") instanceof Date ? t.parseField(h, R.field + "_") : t.parseField(h, R.field), f = w.formatTime(C, "hh24:mi"), x = w.formatTime(x, "hh24:mi"), f == x && p++) : R.type == "datetime" && (C = t.parseField(h, R.field + "_") instanceof Date ? t.parseField(h, R.field + "_") : t.parseField(h, R.field), f = w.formatDateTime(C, "yyyy-mm-dd|hh24:mm:ss"), x = w.formatDateTime(w.isDateTime(x, w.settings.datetimeFormat, true), "yyyy-mm-dd|hh24:mm:ss"), f == x) && p++;
                    break;
                  case "between":
                    ["int", "float", "money", "currency", "percent"].indexOf(R.type) != -1 ? parseFloat(t.parseField(h, R.field)) >= parseFloat(x) && parseFloat(t.parseField(h, R.field)) <= parseFloat(b) && p++ : R.type == "date" ? (C = t.parseField(h, R.field + "_") instanceof Date ? t.parseField(h, R.field + "_") : t.parseField(h, R.field), f = w.isDate(C, w.settings.dateFormat, true), x = w.isDate(x, w.settings.dateFormat, true), (b = w.isDate(b, w.settings.dateFormat, true)) != null && (b = new Date(b.getTime() + 864e5)), f >= x && f < b && p++) : R.type == "time" ? (f = t.parseField(h, R.field + "_") instanceof Date ? t.parseField(h, R.field + "_") : t.parseField(h, R.field), x = w.isTime(x, true), b = w.isTime(b, true), x = (/* @__PURE__ */ new Date()).setHours(x.hours, x.minutes, x.seconds || 0, 0), b = (/* @__PURE__ */ new Date()).setHours(b.hours, b.minutes, b.seconds || 0, 0), f >= x && f < b && p++) : R.type == "datetime" && (f = t.parseField(h, R.field + "_") instanceof Date ? t.parseField(h, R.field + "_") : t.parseField(h, R.field), x = w.isDateTime(x, w.settings.datetimeFormat, true), b = (b = w.isDateTime(b, w.settings.datetimeFormat, true)) && new Date(b.getTime() + 864e5), f >= x) && f < b && p++;
                    break;
                  case "<=":
                    _ = true;
                  case "<":
                  case "less":
                    ["int", "float", "money", "currency", "percent"].indexOf(R.type) != -1 ? (f = parseFloat(t.parseField(h, R.field)), x = parseFloat(E.value), (f < x || _ && f === x) && p++) : R.type == "date" ? (C = t.parseField(h, R.field + "_") instanceof Date ? t.parseField(h, R.field + "_") : t.parseField(h, R.field), f = w.isDate(C, w.settings.dateFormat, true), x = w.isDate(x, w.settings.dateFormat, true), (f < x || _ && f === x) && p++) : R.type == "time" ? (C = t.parseField(h, R.field + "_") instanceof Date ? t.parseField(h, R.field + "_") : t.parseField(h, R.field), f = w.formatTime(C, "hh24:mi"), x = w.formatTime(x, "hh24:mi"), (f < x || _ && f === x) && p++) : R.type == "datetime" && (C = t.parseField(h, R.field + "_") instanceof Date ? t.parseField(h, R.field + "_") : t.parseField(h, R.field), f = w.formatDateTime(C, "yyyy-mm-dd|hh24:mm:ss"), x = w.formatDateTime(w.isDateTime(x, w.settings.datetimeFormat, true), "yyyy-mm-dd|hh24:mm:ss"), f.length == x.length) && (f < x || _ && f === x) && p++;
                    break;
                  case ">=":
                    _ = true;
                  case ">":
                  case "more":
                    ...
                    break;
                  case "in":
                    C = E.value, (C = E.svalue ? E.svalue : C).indexOf(w.isFloat(X) ? parseFloat(X) : X) === -1 && C.indexOf(f) === -1 || p++;
                    break;
                  case "not in":
                    C = E.value, (C = E.svalue ? E.svalue : C).indexOf(w.isFloat(X) ? parseFloat(X) : X) === -1 && C.indexOf(f) === -1 && p++;
                    break;
  ...
  refreshRanges() {
    if (this.ranges.length !== 0) {
      let C = function(S) {
        var E = f.last.move;
        if (E && E.type == "expand") {
          E.divX = S.screenX - E.x, E.divY = S.screenY - E.y;
          let R, X, q = S.target;
          q.tagName.toUpperCase() != "TD" && (q = d(q).closest("td")[0]), (X = d(q).attr("col") != null ? parseInt(d(q).attr("col")) : X) != null && (q = d(q).closest("tr")[0], R = f.records[d(q).attr("index")].recid, E.newRange[1].recid != R || E.newRange[1].column != X) && (S = w.clone(E.newRange), E.newRange = [{ recid: E.recid, column: E.column }, { recid: R, column: X }], b.detail && (b.detail.newRange = w.clone(E.newRange), b.detail.originalRange = w.clone(E.originalRange)), (b = f.trigger("selectionExtend", b)).isCancelled === true ? (E.newRange = S, b.detail.newRange = S) : (f.removeRange("grid-selection-expand"), f.addRange({ name: "grid-selection-expand", range: E.newRange, style: "background-color: rgba(100,100,100,0.1); border: 2px dotted rgba(100,100,100,0.5);" })));
        }
      }, _ = function(S) {
      ...
      for (let S = 0; S < this.ranges.length; S++) {
        ...
        let E = d(this.box).find("#grid_" + this.name + "_rec_" + w.escapeId(n.recid) + ' td[col="' + n.column + '"]'), R = d(this.box).find("#grid_" + this.name + "_rec_" + w.escapeId(o.recid) + ' td[col="' + o.column + '"]'), X = d(this.box).find("#grid_" + this.name + "_frec_" + w.escapeId(n.recid) + ' td[col="' + n.column + '"]'), q = d(this.box).find("#grid_" + this.name + "_frec_" + w.escapeId(o.recid) + ' td[col="' + o.column + '"]'), G = o.column;
        n.column < this.last.colStart && o.column > this.last.colStart && (E = d(this.box).find("#grid_" + this.name + "_rec_" + w.escapeId(n.recid) + ' td[col="start"]')), n.column < this.last.colEnd && o.column > this.last.colEnd && (R = d(this.box).find("#grid_" + this.name + "_rec_" + w.escapeId(o.recid) + ' td[col="end"]'), G = '"end"');
  ...
  select() {
    ...
    if (this.selectType == "row") for (let S = 0; S < s.length; S++) {
      ...
      if (o != null) {
        let E = null, R = null;
        (this.searchData.length !== 0 || o + 1 >= this.last.range_start && o + 1 <= this.last.range_end) && (E = d(this.box).find("#grid_" + this.name + "_frec_" + w.escapeId(n)), R = d(this.box).find("#grid_" + this.name + "_rec_" + w.escapeId(n))), this.selectType == "row" && t.indexes.indexOf(o) == -1 && (t.indexes.push(o), E && R && (E.addClass("w2ui-selected").find(".w2ui-col-number").addClass("w2ui-row-selected"), R.addClass("w2ui-selected").find(".w2ui-col-number").addClass("w2ui-row-selected"), E.find(".w2ui-grid-select-check").prop("checked", true)), e++);
      }
    }
    else {
      var a = {};
      for (let S = 0; S < s.length; S++) {
        ...
        else for (let E = 0; E < this.columns.length; E++) this.columns[E].hidden || a[l].push(parseInt(E));
      }
      var h, p = [];
      for (h in a) {
        ...
        if (f != null) {
          let S = null, E = null;
          f + 1 >= this.last.range_start && f + 1 <= this.last.range_end && (S = d(this.box).find("#grid_" + this.name + "_rec_" + w.escapeId(h)), E = d(this.box).find("#grid_" + this.name + "_frec_" + w.escapeId(h)));
          var x = t.columns[f] || [];
          t.indexes.indexOf(f) == -1 && t.indexes.push(f);
          for (let R = 0; R < a[h].length; R++) x.indexOf(a[h][R]) == -1 && x.push(a[h][R]);
          x.sort((R, X) => R - X);
          for (let R = 0; R < a[h].length; R++) {
            var b = a[h][R];
            p.indexOf(b) == -1 && p.push(b), S && (S.find("#grid_" + this.name + "_data_" + f + "_" + b).addClass("w2ui-selected"), S.find(".w2ui-col-number").addClass("w2ui-row-selected"), S.find(".w2ui-grid-select-check").prop("checked", true)), E && (E.find("#grid_" + this.name + "_data_" + f + "_" + b).addClass("w2ui-selected"), E.find(".w2ui-col-number").addClass("w2ui-row-selected"), E.find(".w2ui-grid-select-check").prop("checked", true)), e++;
          }
          t.columns[f] = x;
        }
      }
      ...
      if (o != null) {
        ...
        if (this.selectType == "row") t.indexes.indexOf(o) != -1 && (t.indexes.splice(t.indexes.indexOf(o), 1), a.removeClass("w2ui-selected w2ui-inactive").find(".w2ui-col-number").removeClass("w2ui-row-selected"), l.removeClass("w2ui-selected w2ui-inactive").find(".w2ui-col-number").removeClass("w2ui-row-selected"), a.length != 0 && (a[0].style.cssText = "height: " + this.recordHeight + "px; " + a.attr("custom_style"), l[0].style.cssText = "height: " + this.recordHeight + "px; " + l.attr("custom_style")), a.find(".w2ui-grid-select-check").prop("checked", false), e++);
          ...
          if (l = t.columns[o], Array.isArray(l) && l.indexOf(c) != -1) {
            ...
            for (let E = 0; E < p.length; E++) p[E].column == c && (_ = true), p[E].recid == n && (S = true);
  ...
  search(e, t) {
    ...
    if (arguments.length === 0 && h.length === 0 && (t = this.multiSearch ? (e = this.searchData, this.last.logic) : (e = this.last.field, this.last.search)), arguments.length === 0 && h.length !== 0) {
      ...
      for (let D = 0; D < this.searches.length; D++) {
        ...
        let P = x.val(), B = b.val(), Q = null, re = null;
        if (["int", "float", "money", "currency", "percent"].indexOf(p.type) != -1 && (C = x[0]._w2field, b = b[0]._w2field, C && (P = C.clean(P)), b) && (B = b.clean(B)), ["list", "enum"].indexOf(p.type) != -1 || ["in", "not in"].indexOf(f) != -1) if (P = x[0]._w2field.selected || {}, Array.isArray(P)) {
          Q = [];
          for (let pe = 0; pe < P.length; pe++) Q.push(w.isFloat(P[pe].id) ? parseFloat(P[pe].id) : String(P[pe].id).toLowerCase()), delete P[pe].hidden;
          Object.keys(P).length === 0 && (P = "");
        } else re = P.text || "", P = P.id || "";
        if (P !== "" && P != null || B != null && B !== "") {
          var C = { field: p.field, type: p.type, operator: f };
          f == "between" ? w.extend(C, { value: [P, B] }) : f == "in" && typeof P == "string" || f == "not in" && typeof P == "string" ? w.extend(C, { value: P.split(",") }) : w.extend(C, { value: P }), Q && w.extend(C, { svalue: Q }), re && w.extend(C, { text: re });
          try {
            p.type == "date" && f == "between" && (C.value[0] = P, C.value[1] = B), p.type == "date" && f == "is" && (C.value = P);
          } catch {
          }
          i.push(C), n = true;
        }
    ...
    if (typeof e == "string" && (arguments.length == 1 && (t = e, e = "all"), a = e, l = t, n = false, o = c ? "AND" : "OR", t != null)) if (e.toLowerCase() == "all") if (0 < this.searches.length) for (let D = 0; D < this.searches.length; D++) {
      ...
      if ((S.type == "text" || S.type == "alphanumeric" && w.isAlphaNumeric(t) || S.type == "int" && w.isInt(t) || S.type == "float" && w.isFloat(t) || S.type == "percent" && w.isFloat(t) || (S.type == "hex" || S.type == "color") && w.isHex(t) || S.type == "currency" && w.isMoney(t) || S.type == "money" && w.isMoney(t) || S.type == "date" && w.isDate(t) || S.type == "time" && w.isTime(t) || S.type == "datetime" && w.isDateTime(t) || S.type == "datetime" && w.isDate(t) || S.type == "enum" && w.isAlphaNumeric(t) || S.type == "list" && w.isAlphaNumeric(t)) && (_ = this.defaultOperator[this.operatorsMap[S.type]], _ = { field: S.field, type: S.type, operator: S.operator != null ? S.operator : _, value: t }, String(t).trim() != "") && i.push(_), ["int", "float", "money", "currency", "percent"].indexOf(S.type) != -1 && String(t).trim().split("-").length == 2 && (_ = String(t).trim().split("-"), E = { field: S.field, type: S.type, operator: S.operator != null ? S.operator : "between", value: [_[0], _[1]] }, i.push(E)), ["list", "enum"].indexOf(S.type) != -1) {
        var E, R = [];
        S.options == null && (S.options = {}), Array.isArray(S.options.items) || (S.options.items = []);
        for (let P = 0; P < S.options.items; P++) {
          var X = S.options.items[P];
          try {
            var q = new RegExp(t, "i");
            q.test(X) && R.push(P), X.text && q.test(X.text) && R.push(X.id);
          } catch {
          }
        }
        0 < R.length && (E = { field: S.field, type: S.type, operator: S.operator != null ? S.operator : "in", value: R }, i.push(E));
      }
    }
      ...
      if ((D = D ?? { field: e, type: "text" }).field == e && (this.last.label = D.label), t !== "") {
        let P = this.defaultOperator[this.operatorsMap[D.type]], B = t;
        if (["date", "time", "datetime"].indexOf(D.type) != -1 && (P = "is"), ["list", "enum"].indexOf(D.type) != -1 && (P = "is", W = W._w2field.get(), B = W && 0 < Object.keys(W).length ? W.id : ""), D.type == "int" && t !== "" && (P = "is", String(t).indexOf("-") != -1 && (W = t.split("-")).length == 2 && (P = "between", B = [parseInt(W[0]), parseInt(W[1])]), String(t).indexOf(",") != -1)) {
          var U = t.split(",");
          P = "in", B = [];
          for (let Q = 0; Q < U.length; Q++) B.push(U[Q]);
        }
        D.operator != null && (P = D.operator), W = { field: D.field, type: D.type, operator: P, value: B }, i.push(W);
      }
    }
    if (Array.isArray(e)) {
      let D = "AND";
      ...
      if (h && !["checkbox", "check"].includes(h.type)) {
        ...
        if (["enum", "file"].indexOf(h.type) != -1) console.log('ERROR: input types "enum" and "file" are not supported in inline editing.');
          ...
          if (a.isCancelled !== true) {
            ...
            let S, E = ["date", "time", "datetime", "color", "list", "combo"];
            i = getComputedStyle(l.find('[col="' + t + '"] > div').get(0)), o = `font-family: ${i["font-family"]}; font-size: ${i["font-size"]};`, h.type === "div" ? (x.addClass("w2ui-editable").html(w.stripSpaces(`<div id="grid_${this.name}_edit_${e}_${t}" class="w2ui-input w2ui-focus"
              ...
              W && ((_b2 = rt.get(W[0])) == null ? void 0 : _b2.selected) != null && (G = true), !this.last.inEditMode || q || !E.includes(h.type) || ((_c2 = X.detail.overlay.anchor) == null ? void 0 : _c2.id) != ((_d = this.last._edit.input) == null ? void 0 : _d.id) && h.type != "list" || (this.editChange(), this.editDone(void 0, void 0, { keyCode: G ? 13 : 0 }));
              ...
              var q, G;
              this.last.inEditMode && (q = this.last._edit.edit.type, G = d(S).data("tooltipName"), E.includes(q) && G || (this.editChange(S, c, t, X), this.editDone()));
              ...
              this.last.inEditMode && S && (S.focus(), clearTimeout(this.last.kbd_timer), (S.resize = R)(S));
            }, 50), a.finish({ input: S });
          }
        }
      }
  ...
  keydown(e) {
    ...
    if (t.keyboard === true) {
      ...
      if (i.isCancelled !== true) if (0 < d(this.box).find(".w2ui-message").length) e.keyCode == 27 && this.message();
        ...
        let X = function(U) {
          if (l && G(), !(_.length <= 0)) {
            let P = t.prevRow(b, t.selectType == "row" ? 0 : h[0].column, U);
            if ((P = R || P != null ? P : t.searchData.length == 0 || s ? 0 : t.last.searchIds[0]) != null) {
              if (R && t.multiSelect) {
                if (W()) return;
                if (t.selectType == "row") t.last.sel_ind > P && t.last.sel_ind != C ? t.unselect(t.records[C].recid) : t.select(t.records[P].recid);
                else if (t.last.sel_ind > P && t.last.sel_ind != C) {
                  P = C;
                  var K = [];
                  for (let B = 0; B < f.length; B++) K.push({ recid: t.records[P].recid, column: f[B] });
                  t.unselect(K);
                } else {
                  var D = [];
                  for (let B = 0; B < f.length; B++) D.push({ recid: t.records[P].recid, column: f[B] });
                  t.select(D);
                }
              } else t.selectNone(true), t.click({ recid: t.records[P].recid, column: f[0] }, e);
              t.scrollIntoView(P, null, true, U != 1), e.preventDefault && e.preventDefault();
            } else R || t.selectNone(true);
          }
        }, q = function(U) {
          if (l && G(), !(_.length <= 0)) {
            let P = t.nextRow(C, t.selectType == "row" ? 0 : h[0].column, U);
            if ((P = R || P != null ? P : t.searchData.length == 0 || s ? t.records.length - 1 : t.last.searchIds[t.last.searchIds.length - 1]) != null) {
              if (R && t.multiSelect) {
                if (W()) return;
                if (t.selectType == "row") t.last.sel_ind < P && t.last.sel_ind != b ? t.unselect(t.records[b].recid) : t.select(t.records[P].recid);
                else if (t.last.sel_ind < P && t.last.sel_ind != b) {
                  P = b;
                  var K = [];
                  for (let B = 0; B < f.length; B++) K.push({ recid: t.records[P].recid, column: f[B] });
                  t.unselect(K);
              ...
              t.scrollIntoView(P, null, true, U != 1), S = true;
            } else R || t.selectNone(true);
          }
        }, G = function() {
        ...
        let S = false, E = e.keyCode, R = e.shiftKey;
        switch (E) {
          case 8:
          case 46:
            ...
            (function() {
              if (l) G();
                ...
                if (t.selectType == "row") {
                  ...
                  if (B = (B == null ? void 0 : B.index) != b ? null : B == null ? void 0 : B.colIndex, R || B != null || (t.selectNone(true), B = 0), B != null) if (R && t.multiSelect) {
                    if (W()) return;
                    var K = [], D = [], P = [];
                    if (f.indexOf(t.last.sel_col) === 0 && 1 < f.length) {
                      for (let Q = 0; Q < h.length; Q++) K.indexOf(h[Q].recid) == -1 && K.push(h[Q].recid), P.push({ recid: h[Q].recid, column: f[f.length - 1] });
                      t.unselect(P), t.scrollIntoView(b, f[f.length - 1], true);
                    } else {
            ...
            (function() {
              if (l) G();
                ...
                if (t.selectType == "row") {
                  ...
                  let P = t.nextCell(b, f[f.length - 1]);
                  if (P = P.index != b ? null : P.colIndex, R || P != null || (t.selectNone(true), P = t.columns.length - 1), P != null) if (R && E == 39 && t.multiSelect) {
                    if (W()) return;
                    var U = [], K = [], D = [];
                    if (f.indexOf(t.last.sel_col) == f.length - 1 && 1 < f.length) {
                      ...
                      for (let B = 0; B < h.length; B++) U.indexOf(h[B].recid) == -1 && U.push(h[B].recid), K.push({ recid: h[B].recid, column: P });
                      t.select(K), t.scrollIntoView(b, P, true);
                    }
                  } else t.click({ recid: p, column: P }, e), t.scrollIntoView(b, P, true);
                  else R || t.selectNone(true);
                }
                S = true;
              }
        ...
        a.indexOf(E) == -1 || e.ctrlKey || e.metaKey || S || (f.length === 0 && f.push(0), S = false, setTimeout(() => {
      ...
      if (!Array.isArray(s) || !s.includes(C) && !s.includes(_.field)) {
        ...
        if (X != null && X.isConnected || (X = n.box.querySelector(`#grid_${n.name}_data_${b}_` + C), p.w2ui._update.cells[C] = X), X != null) {
          if (t) d(X).replace(n.getCellHTML(b, C, false)), X = n.box.querySelector(`#grid_${n.name}_data_${b}_` + C), p.w2ui._update.cells[C] = X;
            ...
            var S = X.children[0], { value: b, style: E, className: R } = n.getCellValue(b, C, false, true);
            if (S.innerHTML != b && (S.innerHTML = b), E != "" && X.style.cssText != E && (X.style.cssText = E), R != "") {
              let G = ["w2ui-grid-data"], W = [];
              S = R.split(" ").filter((U) => !!U), X.classList.forEach((U) => {
          ...
          if (n.columns[C].style && n.columns[C].style != X.style.cssText && (X.style.cssText = n.columns[C].style ?? ""), p.w2ui.class != null) {
            ...
            if (w.isPlainObject(p.w2ui.class) && typeof p.w2ui.class[_.field] == "string") {
              ...
              E = p.w2ui.class[_.field].split(" ").filter((W) => !!W), X.classList.forEach((W) => {
                q.includes(W) || G.push(W);
              }), X.classList.remove(...G), X.classList.add(...E);
            }
          }
            ...
            if (x == null) {
              ...
              if (!(q == null || s.records[q] && s.records[q].recid != p.recid)) {
                ...
                if (G != null) {
                  ...
                  var E, x = "ind1:" + q + ",ind2;" + G + ",col1:" + W + ",col2:" + U;
                  if (p.range != x) {
                    p.range = x;
                    for (let D = q; D <= G; D++) if (!(0 < s.last.searchIds.length && s.last.searchIds.indexOf(D) == -1)) if (s.selectType != "row") {
                      W > U && (E = W, W = U, U = E);
                      for (let P = W; P <= U; P++) s.columns[P].hidden || f.push({ recid: s.records[D].recid, column: parseInt(P) });
                    ...
                    if (s.selectType != "row") {
                      ...
                      for (let P = 0; P < f.length; P++) {
                        let B = false;
                        for (let Q = 0; Q < R.length; Q++) f[P].recid == R[Q].recid && f[P].column == R[Q].column && (B = true);
                        B || D.push({ recid: f[P].recid, column: f[P].column });
                      }
                      s.select(D), D = [];
                      for (let P = 0; P < R.length; P++) {
                        let B = false;
                        for (let Q = 0; Q < f.length; Q++) f[Q].recid == R[P].recid && f[Q].column == R[P].column && (B = true);
                        B || D.push({ recid: R[P].recid, column: R[P].column });
                      }
                      s.unselect(D);
...
`)), C.push({ kind: f.type == "text/html" ? "html" : "text", data: _ });
      ...
      return d(this.box).off("mousedown.mouseStart").on("mousedown.mouseStart", function(h) {
        if (h.which == 1 && (s.last.userSelect == "text" && (s.last.userSelect = "", d(s.box).find(".w2ui-grid-body").css("user-select", "none")), !(s.selectType == "row" && (d(h.target).parents().hasClass("w2ui-head") || d(h.target).hasClass("w2ui-head")) || s.last.move && s.last.move.type == "expand"))) {
          if (h.altKey) d(s.box).find(".w2ui-grid-body").css("user-select", "text"), s.selectNone(), s.last.move = { type: "text-select" }, s.last.userSelect = "text";
            ...
            let E = false;
            for (; S && (!S.classList || !S.classList.contains("w2ui-grid")); ) S.tagName && S.tagName.toUpperCase() == "TD" && (E = true), S.tagName && S.tagName.toUpperCase() != "TR" && E == 1 && (p.x += S.offsetLeft, p.y += S.offsetTop), S = S.parentNode;
  ...
  initToolbar() {
    if (this.toolbar.render == null) {
      ...
      var e;
      this.toolbar.items = [], this.toolbar = new Gr(w.extend({}, this.toolbar, { name: this.name + "_toolbar", owner: this })), this.show.toolbarReload && this.toolbar.items.push(w.extend({}, this.buttons.reload)), this.show.toolbarColumns && this.toolbar.items.push(w.extend({}, this.buttons.columns)), this.show.toolbarSearch && (e = `
  ...
  resizeRecords() {
    ...
    let S = 8 * String(this.total).length + 10, E = (S < 34 && (S = 34), this.lineNumberWidth != null && (S = this.lineNumberWidth), false), R = false, X = 0;
    ...
    ((_a2 = b[0]) == null ? void 0 : _a2.clientWidth) < X && (E = true), ((_b = p[0]) == null ? void 0 : _b.clientHeight) - (((_c2 = f[0]) == null ? void 0 : _c2.clientHeight) ?? 0) < (((_d = d(b).find(":scope > table")[0]) == null ? void 0 : _d.clientHeight) ?? 0) + (E ? w.scrollBarSize() : 0) && (R = true), this.fixedBody ? (s = ((_e = n[0]) == null ? void 0 : _e.clientHeight) - (this.show.header ? w.getSize(o, "height") : 0) - (this.show.toolbar ? w.getSize(a, "height") : 0) - (l.css("display") != "none" ? w.getSize(l, "height") : 0) - (this.show.footer ? w.getSize(h, "height") : 0), p.css("height", s + "px")) : (o = (s = w.getSize(f, "height") + w.getSize(d(this.box).find("#grid_" + this.name + "_records table"), "height") + (E ? w.scrollBarSize() : 0)) + (this.show.header ? w.getSize(o, "height") : 0) + (this.show.toolbar ? w.getSize(a, "height") : 0) + (l.css("display") != "none" ? w.getSize(l, "height") : 0) + (this.show.footer ? w.getSize(h, "height") : 0), n.css("height", o + "px"), p.css("height", s + "px"), i.css("height", w.getSize(n, "height") + "px"));
    let q = this.records.length;
    if (a = typeof this.url != "object" ? this.url : this.url.get, this.searchData.length == 0 || a || (q = this.last.searchIds.length), this.fixedBody || (R = false), E || R ? (f.find(":scope > table > tbody > tr:nth-child(1) td.w2ui-head-last").css("width", w.scrollBarSize() + "px").show(), b.css({ top: (0 < this.columnGroups.length && this.show.columns ? 1 : 0) + w.getSize(f, "height") + "px", "-webkit-overflow-scrolling": "touch", "overflow-x": E ? "auto" : "hidden", "overflow-y": R ? "auto" : "hidden" })) : (f.find(":scope > table > tbody > tr:nth-child(1) td.w2ui-head-last").hide(), b.css({ top: (0 < this.columnGroups.length && this.show.columns ? 1 : 0) + w.getSize(f, "height") + "px", overflow: "hidden" }), 0 < b.length && (this.last.scrollTop = 0, this.last.scrollLeft = 0)), E ? (C.css("margin-bottom", w.scrollBarSize() + "px"), _.show()) : (C.css("margin-bottom", 0), _.hide()), C.css({ overflow: "hidden", top: b.css("top") }), this.show.emptyRecords && !R) {
    ...
    let W, U;
    if (0 < p.length) {
      let ne = parseInt(p[0].clientWidth) - (R ? w.scrollBarSize() : 0) - (this.show.lineNumbers ? S : 0) - (this.reorderRows ? 26 : 0) - (this.show.selectColumn ? 26 : 0) - (this.show.expandColumn ? 26 : 0) - 1, ae = (W = ne, false);
      ...
      if (U != 100 && 0 < U) for (let ue = 0; ue < this.columns.length; ue++) {
        var P = this.columns[ue];
        P.hidden || P.sizeType == "%" && (P.sizeCorrected = Math.round(100 * parseFloat(P.size) * 100 / U) / 100 + "%");
      }
      for (let ue = 0; ue < this.columns.length; ue++) {
...
`;
  ...
  getColumnsHTML() {
    ...
    return this.show.columnHeaders && (s = 0 < this.columnGroups.length ? (o = a(true), i = function() {
      ...
      for (let _ = 0; _ < e.columnGroups.length; _++) {
        ...
        let S = 0;
        for (let E = f; E < f + x.span; E++) e.columns[E] && !e.columns[E].hidden && S++;
        if (!((S = _ == e.columnGroups.length - 1 ? 100 : S) <= 0)) {
          if (x.main === true) {
            let E = "";
            for (let X = 0; X < e.sortData.length; X++) e.sortData[X].field == b.field && ((e.sortData[X].direction || "").toLowerCase() === "asc" && (E = "w2ui-sort-up"), (e.sortData[X].direction || "").toLowerCase() === "desc") && (E = "w2ui-sort-down");
            let R = "";
            b.resizable !== false && (R = `<div class="w2ui-resizer" name="${f}"></div>`);
            var C = w.lang(typeof b.text == "function" ? b.text(b) : b.text);
            h = `<td id="grid_${e.name}_column_${f}" class="w2ui-head ${E}" col="${f}"     rowspan="2" colspan="${S}">` + R + `    <div class="w2ui-col-group w2ui-col-header ${E ? "w2ui-col-sorted" : ""}">        <div class="${E}"></div>` + (C || "&#160;") + "    </div></td>";
  ...
  scroll(e) {
    ...
    e && (R = e.target.scrollTop, e = e.target.scrollLeft, this.last.scrollTop = R, this.last.scrollLeft = e, p = d(this.box).find(`#grid_${this.name}_columns`)[0], f = d(this.box).find(`#grid_${this.name}_summary`)[0], p && (p.scrollLeft = e), f && (f.scrollLeft = e), n[0]) && (n[0].scrollTop = R), this.last.bubbleEl && (rt.hide(this.name + "-bubble"), this.last.bubbleEl = null);
    let o = null, a = null;
    if (this.disableCVS || 0 < this.columnGroups.length) o = 0, a = this.columns.length - 1;
    else {
      var l, c = i.prop("clientWidth");
      let D = 0;
      for (let P = 0; P < this.columns.length; P++) this.columns[P].frozen || this.columns[P].hidden || (l = parseInt(this.columns[P].sizeCalculated || this.columns[P].size), D + l + 30 > this.last.scrollLeft && o == null && (o = P), D + l - 30 > this.last.scrollLeft + c && a == null && (a = P), D += l);
    ...
    if (o != null && (o < 0 && (o = 0), a < 0 && (a = 0), o == a && (0 < o ? o-- : a++), o != this.last.colStart || a != this.last.colEnd)) {
      ...
      if (p < 5 && f < 5) {
        var x = h.find(`.w2ui-grid-columns #grid_${this.name}_column_start`), b = h.find(".w2ui-grid-columns .w2ui-head-last"), C = h.find(`#grid_${this.name}_records .w2ui-grid-data-spacer`), _ = h.find(`#grid_${this.name}_records .w2ui-grid-data-last`), S = h.find(`#grid_${this.name}_summary .w2ui-grid-data-spacer`), E = h.find(`#grid_${this.name}_summary .w2ui-grid-data-last`);
        ...
        if (o < this.last.colStart) for (let D = this.last.colStart - 1; D >= o; D--) this.columns[D] && (this.columns[D].frozen || this.columns[D].hidden) || (x.after(this.getColumnCellHTML(D)), C.each((P) => {
          var B = d(P).parent().attr("index");
          let Q = '<td class="w2ui-grid-data" col="' + D + '" style="height: 0px"></td>';
          B != null && (Q = this.getCellHTML(parseInt(B), D, false)), d(P).after(Q);
        }), S.each((P) => {
          var B = d(P).parent().attr("index");
          let Q = '<td class="w2ui-grid-data" col="' + D + '" style="height: 0px"></td>';
          B != null && (Q = this.getCellHTML(parseInt(B), D, true)), d(P).after(Q);
        }));
        if (a > this.last.colEnd) for (let D = this.last.colEnd + 1; D <= a; D++) this.columns[D] && (this.columns[D].frozen || this.columns[D].hidden) || (b.before(this.getColumnCellHTML(D)), _.each((P) => {
          var B = d(P).parent().attr("index");
          let Q = '<td class="w2ui-grid-data" col="' + D + '" style="height: 0px"></td>';
          B != null && (Q = this.getCellHTML(parseInt(B), D, false)), d(P).before(Q);
        }), E.each((P) => {
          var B = d(P).parent().attr("index") || -1, B = this.getCellHTML(parseInt(B), D, true);
          d(P).before(B);
        }));
        this.last.colStart = o, this.last.colEnd = a;
      } else {
        this.last.colStart = o, this.last.colEnd = a;
        var e = this.getColumnsHTML(), R = this.getRecordsHTML(), p = this.getSummaryHTML(), f = h.find(`#grid_${this.name}_columns`);
        let Q = h.find(`#grid_${this.name}_records`);
        var X = h.find(`#grid_${this.name}_frecords`);
        let re = h.find(`#grid_${this.name}_summary`);
        f.find("tbody").html(e[1]), X.html(R[0]), Q.prepend(R[1]), p != null && re.html(p[1]), setTimeout(() => {
    ...
    if (q > this.total && this.total !== -1 && (q = this.total), (q = this.searchData.length == 0 || s ? q : this.last.searchIds.length) !== 0 && i.length !== 0 && i.prop("clientHeight") !== 0) {
      ...
      let D = Math.round(i.prop("scrollTop") / this.recordHeight + 1), P = D + (Math.round(i.prop("clientHeight") / this.recordHeight) - 1);
      if (D > q && (D = q), P >= q - 1 && (P = q), d(this.box).find("#grid_" + this.name + "_footer .w2ui-footer-right").html((this.show.statusRange ? w.formatNumber(this.offset + D) + "-" + w.formatNumber(this.offset + P) + (this.total != -1 ? " " + w.lang("of") + " " + w.formatNumber(this.total) : "") : "") + (s && this.show.statusBuffered ? " (" + w.lang("buffered") + " " + w.formatNumber(q) + (0 < this.offset ? ", skip " + w.formatNumber(this.offset) : "") + ")" : "")), s || this.fixedBody && !(this.total != -1 && this.total <= this.vs_start)) {
        let Se = function() {
  ...
  getRecordHTML(e, t, s) {
    ...
    if (e == -1) {
      ...
      for (; ; ) {
        let X = 1;
        var p, f = this.columns[E];
        if (f == null) break;
        if (f.hidden) E++, 0 < R && R--;
        else if (0 < R) {
          if (E++, this.columns[E] == null) break;
          a.w2ui.colspan[this.columns[E - 1].field] = 0, R--;
        } else {
          if (a.w2ui && (b = a.w2ui.colspan, p = this.columns[E].field, b) && b[p] === 0 && delete b[p], !(E < this.last.colStart || E > this.last.colEnd) || f.frozen) {
            if (a.w2ui && typeof a.w2ui.colspan == "object") {
              var x = parseInt(a.w2ui.colspan[f.field]) || null;
              if (1 < x) {
                let q = 0;
                for (let G = E; G < E + x && !(G >= this.columns.length); G++) this.columns[G].hidden && q++;
                X = x - q, R = x - 1;
              }
            }
            var b = this.getCellHTML(e, E, s, X);
            f.frozen ? i += b : n += b;
          }
          E++;
        }
  ...
  getCellHTML(e, t, s, i) {
    ...
    if (_.indexes.indexOf(e) != -1 && (S = true), i == null && (i = ((_b = a == null ? void 0 : a.w2ui) == null ? void 0 : _b.colspan) && a.w2ui.colspan[o.field] ? a.w2ui.colspan[o.field] : 1), t === 0 && Array.isArray((_c2 = a == null ? void 0 : a.w2ui) == null ? void 0 : _c2.children)) {
      ...
      for (; K != null; ) {
        U++;
        var R = this.records[K].w2ui;
        if (R == null || R.parent_recid == null) break;
        K = this.get(R.parent_recid, true);
      }
      if (a.w2ui.parent_recid) for (let D = 0; D < U; D++) E += '<span class="w2ui-show-children w2ui-icon-empty"></span>';
      var X = 0 < a.w2ui.children.length ? a.w2ui.expanded ? "w2ui-icon-collapse" : "w2ui-icon-expand" : "w2ui-icon-empty";
      E += `<span class="w2ui-show-children ${X}"></span>`;
    }
    if (o.info === true && (o.info = {}), o.info != null) {
      ...
      typeof o.info.style == "function" ? K = o.info.style(a, { self: this, index: e, colIndex: t, summary: !!s }) : typeof o.info.style == "object" ? K = o.info.style[this.parseField(a, o.field)] || "" : typeof o.info.style == "string" && (K = o.info.style), E += `<span class="w2ui-info ${U}" style="${K}"></span>`;
    }
    let q = l, G = (x && ["checkbox", "check"].indexOf(x.type) != -1 && (b += "text-align: center;", q = `<input tabindex="-1" type="checkbox" class="w2ui-editable-checkbox"
                            data-changeInd="${s ? -(e + 1) : e}" data-colInd="${t}" ${q ? 'checked="checked"' : ""}>`, E = ""), (q = `<div style="${b}" ${function(U) {
      let K;
...
class yn extends ni {
            ...
            if (this.type === "file" && (/image/i.test(f.type) && (b = `
              ...
              rt.show({ name: C, anchor: h.get(0), html: b, hideOn: ["doc-click"], class: "" }).show((_) => {
                d(`#w2overlay-${C} img`).on("load", function(S) {
                  var E = this.clientWidth, R = this.clientHeight;
                  E < 300 & R < 300 || (R <= E && 300 < E && d(this).css("width", "300px"), E < R && 300 < R && d(this).css("height", "300px"));
                }).on("error", function(S) {
  ...
  updateOverlay(e) {
    ...
    if (this.type === "color") {
      if (d(this.el).prop("readOnly") || d(this.el).prop("disabled")) return;
      ja.show(w.extend({ name: this.el.id + "_color", anchor: this.el, transparent: t.transparent, advanced: t.advanced, color: this.el.value, liveUpdate: true }, this.options)).select((i) => {
...
*/
const vo = globalThis, Sa = (r) => r, $n = vo.trustedTypes, Ma = $n ? $n.createPolicy("lit-html", { createHTML: (r) => r }) : void 0, Wa = "$lit$", Ns = `lit$${Math.random().toFixed(9).slice(2)}$`, Za = "?" + Ns, Jr = `<${Za}>`, ii = document, Bi = () => ii.createComment(""), Hi = (r) => r === null || typeof r != "object" && typeof r != "function", _o = Array.isArray, Qr = (r) => _o(r) || typeof (r == null ? void 0 : r[Symbol.iterator]) == "function", co = `[ 	
\f\r]`, Ri = /<(?:(!--|\/[^a-zA-Z])|(\/?[a-zA-Z][^>\s]*)|(\/?$))/g, $a = /-->/g, Aa = />/g, Gs = RegExp(`>|${co}(?:([^\\s"'>=/]+)(${co}*=${co}*(?:[^ 	
\f\r"'\`<>=]|("|')|))|$)`, "g"), Ea = /'/g, Ta = /"/g, Ka = /^(?:script|style|textarea|title)$/i, ec = (r) => (e, ...t) => ({ _$litType$: r, strings: e, values: t }), Sn = ec(1), Xi = Symbol.for("lit-noChange"), qt = Symbol.for("lit-nothing"), za = /* @__PURE__ */ new WeakMap(), Qs = ii.createTreeWalker(ii, 129);
function Ga(r, e) {
...
const tc = (r, e) => {
  ...
  let i, n = e === 2 ? "<svg>" : e === 3 ? "<math>" : "", o = Ri;
  for (let a = 0; a < t; a++) {
    const l = r[a];
    let c, h, p = -1, f = 0;
    for (; f < l.length && (o.lastIndex = f, h = o.exec(l), h !== null); ) f = o.lastIndex, o === Ri ? h[1] === "!--" ? o = $a : h[1] !== void 0 ? o = Aa : h[2] !== void 0 ? (Ka.test(h[2]) && (i = RegExp("</" + h[2], "g")), o = Gs) : h[3] !== void 0 && (o = Gs) : o === Gs ? h[0] === ">" ? (o = i ?? Ri, p = -1) : h[1] === void 0 ? p = -2 : (p = o.lastIndex - h[2].length, c = h[1], o = h[3] === void 0 ? Gs : h[3] === '"' ? Ta : Ea) : o === Ta || o === Ea ? o = Gs : o === $a || o === Aa ? o = Ri : (o = Gs, i = void 0);
    const x = o === Gs && r[a + 1].startsWith("/>") ? " " : "";
    n += o === Ri ? l + Jr : p >= 0 ? (s.push(c), l.slice(0, p) + Wa + l.slice(p) + Ns + x) : l + Ns + (p === -2 ? a : x);
  }
  return [Ga(r, n + (r[t] || "<?>") + (e === 2 ? "</svg>" : e === 3 ? "</math>" : "")), s];
};
class Yi {
  constructor({ strings: e, _$litType$: t }, s) {
    ...
    this.parts = [];
    let n = 0, o = 0;
    const a = e.length - 1, l = this.parts, [c, h] = tc(e, t);
    ...
    for (; (i = Qs.nextNode()) !== null && l.length < a; ) {
      if (i.nodeType === 1) {
        if (i.hasAttributes()) for (const p of i.getAttributeNames()) if (p.endsWith(Wa)) {
          const f = h[o++], x = i.getAttribute(p).split(Ns), b = /([.?@])?(.*)/.exec(f);
          l.push({ type: 1, index: n, name: b[2], strings: x, ctor: b[1] === "." ? ic : b[1] === "?" ? nc : b[1] === "@" ? oc : zn }), i.removeAttribute(p);
      ...
      } else if (i.nodeType === 8) if (i.data === Za) l.push({ type: 2, index: n });
      else {
        let p = -1;
...
class sc {
  ...
  u(e) {
    ...
    for (; l !== void 0; ) {
      if (o === l.index) {
        let c;
        l.type === 2 ? c = new Ui(n, n.nextSibling, this, e) : l.type === 1 ? c = new l.ctor(n, l.name, l.strings, this, e) : l.type === 6 && (c = new ac(n, this, e)), this._$AV.push(c), l = s[++a];
      }
...
class Ui {
  ...
  _$AI(e, t = this) {
    e = xi(this, e, t), Hi(e) ? e === qt || e == null || e === "" ? (this._$AH !== qt && this._$AR(), this._$AH = qt) : e !== this._$AH && e !== Xi && this._(e) : e._$litType$ !== void 0 ? this.$(e) : e.nodeType !== void 0 ? this.T(e) : Qr(e) ? this.k(e) : this._(e);
  }
  O(e) {
  ...
  $(e) {
    var _a2;
    const { values: t, _$litType$: s } = e, i = typeof s == "number" ? this._$AC(e) : (s.el === void 0 && (s.el = Yi.createElement(Ga(s.h, s.h[0]), this.options)), s);
    if (((_a2 = this._$AH) == null ? void 0 : _a2._$AD) === i) this._$AH.p(t);
    else {
      const n = new sc(i, this), o = n.u(this.options);
      n.p(t), this.T(o), this._$AH = n;
    }
  }
  _$AC(e) {
    let t = za.get(e.strings);
    return t === void 0 && za.set(e.strings, t = new Yi(e)), t;
  }
  k(e) {
...
const lc = vo.litHtmlPolyfillSupport;
lc == null ? void 0 : lc(Yi, Ui), (vo.litHtmlVersions ?? (vo.litHtmlVersions = [])).push("3.3.2");
const rc = (r, e, t) => {
  const s = e;
  let i = s._$litPart$;
...
function zc() {
  ...
  return xn = r === "localhost" || r === "127.0.0.1" || r === "" || location.protocol === "file:" ? hc() : Promise.resolve(false), xn;
}
async function hc() {
  try {
...
function Ic(r) {
  return mc.find((e) => e.id === r) ?? null;
}
const mi = "hekatan_ai_", Es = { getKey(r) {
...
const Rc = `Eres un asistente experto en estructuras y FEM que ayuda al usuario a generar modelos
...
support 4 fixed
frameload 2 0 0 -20
solve`, An = (r) => new Promise((e) => setTimeout(e, r)), En = () => new Promise((r) => requestAnimationFrame(() => requestAnimationFrame(r))), bn = [];
function ho() {
  var _a2, _b, _c2, _d;
...
async function uo(r) {
  var _a2, _b, _c2, _d, _e, _f;
  ((_b = (_a2 = ct()).__hekatanExample) == null ? void 0 : _b.call(_a2)) === r ? (_d = (_c2 = ct()).__hekatanRebuild) == null ? void 0 : _d.call(_c2) : (_f = (_e = ct()).__hekatanLoadExampleById) == null ? void 0 : _f.call(_e, r), await En(), await An(150);
}
async function Ia(r) {
  var _a2, _b, _c2, _d;
  ...
  return (_d = (_c2 = ct()).__hekatanRebuild) == null ? void 0 : _d.call(_c2), await En(), await An(150), t;
}
function vn() {
  var _a2, _b, _c2, _d, _e;
...
async function ko(r, e) {
  ...
  switch (r) {
    case "obtener_modelo": {
      ...
      return t === "cli-modeler" ? s.script = String(ct().__hekatanCliScript ?? "").slice(0, 3e3) : t && (s.parametros = Fa()), s;
    }
    ...
    case "cargar_plantilla": {
      ...
      const s = await Ia(e == null ? void 0 : e.parametros);
      return { ok: true, plantilla: t, ...vn(), avisos: s, parametros: Fa(), siguiente: s.length ? "Hay claves que no existen. Llama cambiar_parametros usando SOLO claves de 'parametros'." : "Ajusta con cambiar_parametros si hace falta y luego llama resultados." };
    }
    case "cambiar_parametros": {
      bn.push(ho());
      const t = await Ia(e == null ? void 0 : e.parametros);
...
solve`), bn.push(ho()), ct().__hekatanCliStats = null, ct().__hekatanCliScript = t, await uo("cli-modeler");
      for (let n = 0; n < 20 && !ct().__hekatanCliStats; n++) await An(100);
    ...
    case "vista": {
      ...
      return (e == null ? void 0 : e.deformada) !== void 0 && (t == null ? void 0 : t.deformedShape) && (t.deformedShape.val = !!e.deformada), (e == null ? void 0 : e.campo_cascara) && (t == null ? void 0 : t.shellResults) && (t.shellResults.val = e.campo_cascara), (e == null ? void 0 : e.camara) && ((_r2 = (_q = ct()).__hekatanSetView) == null ? void 0 : _r2.call(_q, e.camara)), await En(), { ok: true };
    }
    case "deshacer": {
      const t = bn.pop();
      ...
      if (t.ex === "cli-modeler" && (ct().__hekatanCliScript = t.script), await uo(t.ex), t.ex !== "cli-modeler") {
        ...
        s && (Object.assign(s, t.params), (_v = (_u = ct()).__hekatanRebuild) == null ? void 0 : _v.call(_u), await En());
      }
...
async function yc(r, e, t, s) {
  ...
  const n = r.id === "ollama", o = [{ role: "system", content: gc }, ...Di], a = n ? { model: e, stream: false, options: { temperature: 0.1, num_ctx: 16384 }, tools: go.map((h) => ({ type: "function", function: h })), messages: o.map((h) => h.tool_calls ? { ...h, tool_calls: h.tool_calls.map((p) => ({ function: { name: p.function.name, arguments: typeof p.function.arguments == "string" ? JSON.parse(p.function.arguments || "{}") : p.function.arguments } })) } : h) } : { model: e, messages: o, temperature: 0.1, tool_choice: "auto", tools: go.map((h) => ({ type: "function", function: h })) }, l = await fetch(n ? "http://localhost:11434/api/chat" : r.url, { method: "POST", headers: i, signal: s, body: JSON.stringify(a) }).catch((h) => {
    if ((h == null ? void 0 : h.name) === "AbortError") throw h;
    if (r.id === "ollama") {
      const p = location.protocol !== "file:" && !/^(localhost|127\.0\.0\.1|\[::1\])$/.test(location.hostname);
      throw new Error(p ? "Ollama esta en tu PC pero RECHAZA a " + location.origin + ' (403). Dale permiso y reinicialo:  setx OLLAMA_ORIGINS "' + location.origin + '"  (o usa el agente desde la version local, que si tiene permiso).' : "Ollama no responde en localhost:11434. Abrelo o instala: ollama.com \u2192 ollama pull qwen2.5:7b");
    }
...
async function wo() {
  const r = us.value.trim();
  if (!r || ei) return;
  const e = Es.getProvider(), t = yi.find((h) => h.id === e) ?? yi[0], s = Es.getKey(t.id), i = Es.getModel(`agente_${t.id}`) || t.modelos[0];
  if (t.clave && !s) {
    _s("error", `${t.nombre} necesita clave. ${t.pista}`);
    return;
  }
  us.value = "", _s("user", r), Di.push({ role: "user", content: r }), ei = new AbortController(), Js.textContent = "\u25A0 Parar";
  const n = _s("ia", "\u2026"), o = /* @__PURE__ */ new Set(["cargar_plantilla", "cambiar_parametros", "modelar_heks"]);
  let a = false, l = false, c = 0;
  try {
    for (let h = 0; h < Ra; h++) {
      const p = await yc(t, i, s, ei.signal);
      let f = p.tool_calls ?? [];
      if (!f.length && p.content && (f = xc(p.content)), Di.push({ role: "assistant", content: f.length ? p.content ?? "" : p.content, tool_calls: f.length ? f : void 0 }), !f.length && a && !l && c < 2) {
      ...
      if (!f.length) {
        n.remove(), _s("ia", (p.content ?? "").trim() || "(sin respuesta)");
        return;
      }
      for (const x of f) {
        let b = {};
        ...
        const C = _s("paso", `\u{1F527} ${x.function.name} ${b ? bc(b) : "(argumentos inv\xE1lidos)"}`);
        si.insertBefore(C, n);
        let _;
        try {
        ...
        In(), C.textContent += `
   \u2192 ${Ja(x.function.name, _)}`, o.has(x.function.name) && (a = true, l = false), (x.function.name === "resultados" || x.function.name === "analisis_modal") && (l = true), Di.push({ role: "tool", tool_call_id: x.id, content: JSON.stringify(_).slice(0, 6e3) });
      }
    }
    n.remove(), _s("error", `Par\xE9 tras ${Ra} pasos. P\xEDdeme que siga si hace falta.`);
  } catch (h) {
    n.remove(), (h == null ? void 0 : h.name) !== "AbortError" ? _s("error", String((h == null ? void 0 : h.message) ?? h)) : _s("error", "Detenido.");
  } finally {
...
function vc() {
  ...
  for (const E of yi) n.add(new Option(E.nombre, E.id));
  ...
  const h = () => {
    const E = yi.find((R) => R.id === n.value) ?? yi[0];
    a.innerHTML = E.modelos.map((R) => `<option value="${R}">`).join(""), o.value = Es.getModel(`agente_${E.id}`) || E.modelos[0], l.style.display = E.clave ? "" : "none", l.value = Es.getKey(E.id), c.textContent = E.pista;
  }, p = Es.getProvider();
  n.value = yi.some((E) => E.id === p) ? p : "ollama", n.onchange = () => {
    Es.setProvider(n.value), h();
  }, o.onchange = () => Es.setModel(`agente_${n.value}`, o.value.trim()), l.onchange = () => Es.setKey(n.value, l.value.trim()), s.append(n, o, a, l, c), h(), si = document.createElement("div"), si.style.cssText = "flex:1;overflow-y:auto;display:flex;flex-direction:column;gap:6px;padding:10px;";
  const f = document.createElement("div");
  f.style.cssText = "display:flex;gap:6px;padding:8px 10px;border-top:1px solid #1e293b;", us = document.createElement("textarea"), us.rows = 2, us.placeholder = "Ej.: edificio de 4 pisos, 3\xD72 vanos de 5 m; dime la flecha y el periodo", us.style.cssText = i + "flex:1;resize:none;font-size:13px;", ["keydown", "keyup", "keypress"].forEach((E) => us.addEventListener(E, (R) => R.stopPropagation())), us.addEventListener("keydown", (E) => {
    E.key === "Enter" && !E.shiftKey && (E.preventDefault(), wo());
  });
  ...
  b.textContent = "\u21B6 Deshacer", b.title = "Vuelve el modelo al estado anterior al \xFAltimo cambio del agente", b.style.cssText = "background:#334155;color:#e5e7eb;border:none;border-radius:4px;padding:4px 8px;cursor:pointer;font-size:12px;", b.onclick = async () => {
    if (ei) return;
    const E = await ko("deshacer", {});
    _s("paso", `\u21B6 deshacer \u2192 ${Ja("deshacer", E)}`);
  }, x.append(Js, b), f.append(us, x), r.append(e, s, si, f), document.body.appendChild(r);
  let C = 0, _ = 0, S = false;
  return e.addEventListener("pointerdown", (E) => {
    if (E.target.tagName === "BUTTON") return;
    S = true;
    const R = r.getBoundingClientRect();
    C = E.clientX - R.left, _ = E.clientY - R.top, e.setPointerCapture(E.pointerId);
  }), e.addEventListener("pointermove", (E) => {
    S && (r.style.left = Math.max(0, E.clientX - C) + "px", r.style.top = Math.max(0, E.clientY - _) + "px", r.style.right = "auto", r.style.bottom = "auto");
  }), e.addEventListener("pointerup", () => {
    S = false;
  }), _s("ia", "Hola. P\xEDdeme una estructura y la armo en el visor, paso a paso: plantilla o .heks, c\xE1lculo, resultados y modal."), r;
}
function Co() {
  const r = !!os && os.style.display !== "none" && document.body.contains(os);
  for (const e of ["hk-agente-lanzador", "hk-caja-negra-btn"]) {
    const t = document.getElementById(e);
    t && (t.style.display = r ? "none" : "block");
  }
}
function So(r) {
  In(), os || (os = vc()), os.style.display = "flex", Co(), r && (us.value = r), us.focus();
}
function _c(r) {
  const e = () => {
...
function kc() {
  ...
  r.id = "hk-agente-explicar", r.textContent = "\u{1F4AC} Expl\xEDcame", r.title = "Que el agente lea el modelo y sus resultados y te los explique", r.style.cssText = ["position:fixed", "left:0", "top:0", "z-index:8999", "height:44px", "padding:0 14px", "border-radius:22px", "border:1px solid #22d3ee", "background:#0b1220", "color:#e2e8f0", "font:13px system-ui,sans-serif", "cursor:pointer", "box-shadow:0 4px 14px rgba(0,0,0,.4)", "display:none"].join(";"), r.onclick = () => {
    Qa("Expl\xEDcame este modelo ya calculado. Usa obtener_modelo y resultados, y dime en pocas l\xEDneas: qu\xE9 estructura es, qu\xE9 cargas y apoyos tiene, cu\xE1nto se desplaza (d\xF3nde y cu\xE1nto), si las reacciones equilibran la carga y si el resultado es razonable.");
...
async function Qa(r) {
  So(), us.value = r, await wo();
}
typeof window < "u" && (ct().__hekatanAgenteIA = So, ct().__hekatanPedirAgente = Qa, ct().__hekatanAgenteTool = ko);
export {
  qt as A,
  rc as D,
  Rc as H,
