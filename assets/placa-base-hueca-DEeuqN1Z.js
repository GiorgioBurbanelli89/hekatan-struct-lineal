...
import { v as n, b as ta, e as pe, C as ue, M as he, V as ea, B as aa, d as oa } from "./Text-BE1eWO-3.js";
import { P as sa } from "./tweakpane-BXg6ZhiP.js";
import { a as na, __tla as __tla_0 } from "./analyze-CW2pK7-V.js";
import { d as la, __tla as __tla_1 } from "./didacticCpp-Czy7NlhT.js";
import { g as ca, i as ve, j as be, a as ra, __tla as __tla_2 } from "./aiAgent-BdxJjwS0.js";
import { e as ia } from "./makeDraggable-zx2br6Yh.js";
import { g as da, __tla as __tla_3 } from "./getParameters-BfCQ31Oa.js";
...
Promise.all([
  ...
  n.derive(() => {
    ...
    for (let t = 0; t < R; t++) for (let e = 0; e < $; e++) S(j[t][e], j[t][e + 1], j[t + 1][e + 1], j[t + 1][e], L);
    function F(t, e) {
      let a = -1, s = 1 / 0;
      for (let c = 0; c <= R; c++) for (let l = 0; l <= $; l++) {
        const _ = j[c][l], T = Math.hypot(w[_][0] - t, w[_][1] - e);
        T < s && (s = T, a = _);
      }
      return a;
    }
    ...
    for (let t = 0; t <= g; t++) {
      ...
      for (let a = 0; a <= f; a++) {
        const s = -d / 2 + a * nt;
        t === 0 ? e.push(F(s, -m / 2)) : e.push(C(s, -m / 2, u + t * O));
      }
      P.push(e);
    }
    ...
    for (let t = 0; t <= g; t++) {
      ...
      for (let a = 0; a <= x; a++) {
        const s = -m / 2 + a * lt;
        t === 0 ? e.push(F(-d / 2, s)) : a === 0 ? e.push(P[t][0]) : a === x ? e.push(N[t][0]) : e.push(C(-d / 2, s, u + t * O));
      }
      z.push(e);
    }
    ...
    for (let t = 0; t <= g; t++) {
      ...
      for (let a = 0; a <= x; a++) {
        const s = -m / 2 + a * lt;
        t === 0 ? e.push(F(d / 2, s)) : a === 0 ? e.push(P[t][f]) : a === x ? e.push(N[t][f]) : e.push(C(d / 2, s, u + t * O));
      }
      k.push(e);
    }
    ...
    function A(t, e, a, s) {
      const [c, l] = t, [_, T] = e, At = a[0][s], zt = F(c + _ * Gt, l + T * Gt), me = a[Math.min(Se, a.length - 1)][s];
      S(At, zt, me, me, H);
    }
    ...
    const $t = Math.PI * y * y / 4, _t = Math.PI * y ** 4 / 64, Rt = 2 * _t, q = [], je = (o - 2 * mt) / Math.max(1, Et - 1), Fe = (p - 2 * Lt) / Math.max(1, dt - 1);
    for (let t = 0; t < Et; t++) for (let e = 0; e < dt; e++) {
      const a = -o / 2 + mt + t * je, s = -p / 2 + Lt + e * Fe;
    ...
    for (const [t, e] of Ie) {
      const a = C(t, e, u + J), s = F(t, e), c = C(t, e, u - D);
  ...
  jt.style.cssText = "position:fixed;top:8px;right:8px;width:300px;max-height:85vh;overflow-y:auto;z-index:9999;";
  const E = new sa({
    ...
    phiNn: 0,
    demandCapAnchor: 0
  }, Ft = (o) => o < 1 ? `${o.toFixed(2)} \u2713` : o < 1.2 ? `${o.toFixed(2)} \u26A0` : `${o.toFixed(2)} \u2717`, rt = E.addFolder({
