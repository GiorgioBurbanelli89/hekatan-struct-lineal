...
function Ei({ drawingObj: t, gridObj: y, scene: g, getActiveCamera: b, controls: S, gridSize: z, derivedDisplayScale: A, rendererElm: F, viewerRender: C }) {
  ...
  const ke = new E(), Be = (e, o, s, n, a, c) => {
    ...
    Te = [e, o, s], lt = [i / f, l / f, r / f], ke.set((e + n) / 2, (o + a) / 2, (s + c) / 2), ke.project(b());
  ...
  window.__hekatanRevolveSelection = (e, o, s, n = 360) => {
    ...
    const p = w > 0 ? 1 : -1, h = (Y) => {
      const ee = r[(Y - 1 + f) % f], pe = r[Y], be = r[(Y + 1) % f], Je = [pe[0] - ee[0], pe[1] - ee[1]], qe = [be[0] - pe[0], be[1] - pe[1]], je = Math.hypot(Je[0], Je[1]) || 1, Ee = Math.hypot(qe[0], qe[1]) || 1, Ge = [p * Je[1] / je, -p * Je[0] / je], De = [p * qe[1] / Ee, -p * qe[0] / Ee], ze = 1 + (Ge[0] * De[0] + Ge[1] * De[1]);
    ...
    const P = [...((_b = t.areas) == null ? void 0 : _b.rawVal) ?? []], $ = d.map((Y) => {
      ...
      return r.map((be, Je) => Xo(V, [be[0] + M[Je][0] * ee, be[1] + M[Je][1] * ee, pe]));
    });
    let T = 0;
  ...
  window.__hekatanUpdateSelectionPtScale = Es, S.addEventListener("change", () => {
    ...
    if ((!s.length || ((_c = (_b = (_a3 = window.__hekatanCadState) == null ? void 0 : _a3.get) == null ? void 0 : _b.call(_a3)) == null ? void 0 : _c.tool) !== "fillarea") && rt.visible && (rt.visible = false), s.length) {
      ...
      if (l) _o(l.type, l.x, l.y, l.z), wt.position.set(l.x, l.y, l.z), wt.visible = true, n.set(l.x, l.y, l.z), So(l.type, e.clientX, e.clientY);
  ...
  const Zs = (e, o) => {
    ...
    if (s === "medir") {
      ...
      if (!l) return;
      if (xt.length >= 2 && (xt = []), xt.push(l), xt.length === 1) at.visible = false, Ct(), ie("\u{1F4CF} Regla \u2014 1er punto puesto. Marca el 2\xBA.");
      else {
        const [r, f] = xt;
        at.geometry.setFromPoints([new E(r[0], r[1], r[2]), new E(f[0], f[1], f[2])]), at.visible = true;
        const d = Math.hypot(f[0] - r[0], f[1] - r[1], f[2] - r[2]), w = Math.hypot(f[0] - r[0], f[1] - r[1]);
        He.textContent = `${d.toFixed(3)} m`, Ct(), ie(`\u{1F4CF} Distancia ${d.toFixed(3)} m  \xB7  \u0394x ${(f[0] - r[0]).toFixed(3)}  \u0394y ${(f[1] - r[1]).toFixed(3)}  \u0394z ${(f[2] - r[2]).toFixed(3)}  \xB7  en planta ${w.toFixed(3)} m`);
      }
      C();
      return;
    }
