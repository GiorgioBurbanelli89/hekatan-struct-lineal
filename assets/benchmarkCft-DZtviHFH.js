import { a as xe, __tla as __tla_0 } from "./analyze-Bfj82fAo.js";
import { d as Ae, __tla as __tla_1 } from "./didacticCpp-BoYi16rL.js";
let Ne;
let __tla = Promise.all([
  ...
  Ne = {
    ...
    computedLabels(t, r) {
      const F = t.D_out, c = t.t_HSS, i = F - 2 * c, h = F * F - i * i, m = i * i, d = (F ** 4 - i ** 4) / 12, I = i ** 4 / 12, E = t.E_s / t.E_c, k = Math.round(t.transformMode) === 0, s = k ? h + m / E : E * h + m, a = k ? d + I / E : E * d + I, n = k ? t.E_s : t.E_c, x = (t.A_b ?? 7610) * 1e-6, A = (t.Iy_b ?? 12.9) * 1e-5, p = t.E_s / 2.6, X = 5 / 6 * x, f = (t.Lx ?? 4) / Math.round(t.nx ?? 4), g = 12 * t.E_s * A / (p * X * f * f), $ = Math.round(t.beamTheory ?? 1) === 0;
      return {
        "n = E_s/E_c": E.toFixed(2),
        "A_s (HSS)": `${(h * 1e4).toFixed(2)} cm\xB2`,
        "A_c (fill)": `${(m * 1e4).toFixed(2)} cm\xB2`,
        ...
        "\u03C6 Timoshenko viga": $ ? "\u2248 0 (Bernoulli forzado)" : `${g.toFixed(4)}  (W360x60, L=${f.toFixed(2)} m)`,
        "Factor (1+\u03C6)": $ ? "1.000 (sin shear)" : (1 + g).toFixed(4)
      };
    },
    build(t, r) {
      var _a;
      const c = [
        "cftNoSlab",
        "cftDeckSlab"
      ][Math.round(t.setup)] || "cftDeckSlab", i = c === "cftDeckSlab", h = Math.round(t.transformMode) === 0, m = Math.round(t.beamTheory) === 0, d = -1, I = t.Lx, E = t.Ly, k = t.h_story, s = Math.round(t.nx), a = Math.round(t.ny), n = s + 1, x = I / s, A = E / a, p = t.D_out, X = t.t_HSS, f = p - 2 * X, g = p * p - f * f, $ = f * f, Y = (p ** 4 - f ** 4) / 12, V = f ** 4 / 12, Q = 2 * Y, v = t.E_s / t.E_c;
      let G, B, j, N;
      h ? (G = g + $ / v, B = Y + V / v, j = Q, N = t.E_s) : (G = v * g + $, B = v * Y + V, j = v * Q, N = t.E_c);
      const ce = N / 2.6, _ = [];
      ...
      for (let e = 0; e < M.length; e++) H.set(e, t.E_c), se.set(e, t.nu_c), ae.set(e, t.t_slab), C.set(e, ie), L.set(e, 24 / 9.80665);
      const me = t.E_s / 2.6, R = /* @__PURE__ */ new Map(), w = /* @__PURE__ */ new Map(), fe = 355e-9, _e = 279e-5, de = 473e-5;
      for (let e = 0; e < oe; e++) {
        const o = M.length + e;
        H.set(o, t.E_s), C.set(o, me), J.set(o, t.A_b * 1e-6), U.set(o, t.Iy_b * 1e-5), Z.set(o, t.Iz_b * 1e-5), K.set(o, fe), L.set(o, 78.5 / 9.80665), m ? (R.set(o, d), w.set(o, d)) : (R.set(o, _e), w.set(o, de));
      }
      for (let e = oe; e < u.length; e++) {
        const o = M.length + e;
        H.set(o, N), C.set(o, ce), J.set(o, G), U.set(o, B), Z.set(o, B), K.set(o, j), L.set(o, (h ? 78.5 : 24) / 9.80665), m && (R.set(o, d), w.set(o, d));
      }
      r.nodes.val = _, r.elements.val = z, r.nodeInputs.val = {
        supports: D,
        loads: y
      }, r.elementInputs.val = {
        elasticities: H,
      ...
      try {
        r.deformOutputs.val = Ae(_, z, {
          supports: D,
          loads: y
        }, r.elementInputs.val), r.analyzeOutputs.val = xe(_, z, r.elementInputs.val, r.deformOutputs.val);
      } catch (e) {
      ...
      if (ne) {
        const e = ne[2] * 1e3, o = i ? "w_centro" : "w_midspan_beam", T = h ? "steel-eq" : "concrete-eq", b = m ? "Bernoulli" : "Timoshenko", S = m ? Me[c] : pe[c], he = m ? Ie[c] : ke[c], le = Te[c], Ee = Fe[c], P = ge[c], q = $e[c], O = ve[c];
        if (console.log(`[Benchmark CFT] ${c} (${T}, ${b}) \u2014 BASE EMPOTRADA
  ${o} = ${e.toFixed(4)} mm  (Hekatan-CLI ${b} ref: ${S == null ? void 0 : S.toFixed(4)})
  Julia ${b} ref: ${he == null ? void 0 : he.toFixed(4)}` + (m ? `  \xB7  OpenSees ref: ${le.toFixed(4)}  \xB7  PyNite ref: ${Ee.toFixed(4)}` : `  \xB7  OpenSees/PyNite usan Bernoulli (${le.toFixed(4)})`) + `
  ETABS Filled Steel Tube PIN (legacy): ${P.toFixed(4)}
  ETABS HSS+modifiers PIN (legacy):     ${q.toFixed(4)}
  \u{1F3AF} ETABS BASE EMPOTRADA (Filled Tube): ${O.toFixed(4)}`), Math.abs(O) > 1e-9) {
          const l = (e - O) / Math.abs(O) * 100, W = l >= 0 ? "+" : "";
        ...
        if (Math.abs(q) > 1e-9) {
          ...
          !m && l < 1 ? console.log(`  \u2713\u2713 \u0394 ${l.toFixed(2)}% vs ETABS HSS+modifiers (transf. simple)`) : console.log(`  \u0394 ${l.toFixed(2)}% vs ETABS HSS+modifiers`);
        }
        if (Math.abs(P) > 1e-9) {
          const l = Math.abs(e - P) / Math.abs(P) * 100;
