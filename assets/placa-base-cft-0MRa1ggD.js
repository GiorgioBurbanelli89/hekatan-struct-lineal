...
import { v as c, b as Io, e as Ut, M as Vt, C as Xt, V as So, B as Eo, d as Lo } from "./Text-BE1eWO-3.js";
import { P as Ho } from "./tweakpane-BXg6ZhiP.js";
import { a as Oo, __tla as __tla_0 } from "./analyze-BQ-8aq8N.js";
import { d as qo, __tla as __tla_1 } from "./didacticCpp-Czy7NlhT.js";
import { g as Go, i as Ye, j as De, a as Yo, __tla as __tla_2 } from "./aiAgent-Ocg1dyZM.js";
import { e as Do } from "./makeDraggable-zx2br6Yh.js";
import { g as $o, __tla as __tla_3 } from "./getParameters-Cpy8HY4C.js";
import { e as Oe } from "./materials-VwssM8Vw.js";
...
Promise.all([
  ...
  const Rt = 2e8, Zt = 0.3, qe = Rt / (2 * (1 + Zt)), Ge = 78, Jo = 25e4, Uo = 6e5, d = {
    B: {
      value: c.state(0.5),
      min: 0.3,
      max: 1.2,
  ...
  }, $e = c.state([]), Je = c.state([]), Ue = c.state({}), Ve = c.state({}), Xe = c.state({}), Re = c.state({}), Ze = c.state([]), Ke = c.state({
    vmMax: 0,
    A1: 0,
    A2: 0,
    phiPp: 0,
  ...
  c.derive(() => {
    ...
    function $(t, e, o, n, a) {
      ...
      const l = y.length - 1;
      K.set(l, a), Q.set(l, Rt), W.set(l, Zt), tt.set(l, Ge), et.set(l, qe), ot.set(l, 0), at.set(l, 0), nt.set(l, 0), st.set(l, 0);
    }
    function ne(t, e, o, n, a) {
      ...
      const l = y.length - 1;
      Q.set(l, Rt), W.set(l, Zt), tt.set(l, Ge), et.set(l, qe), ot.set(l, o), at.set(l, n), nt.set(l, n), st.set(l, a), K.set(l, 0);
    }
    const Tt = s / R, Ct = h / Z, H = [];
    for (let t = 0; t <= Z; t++) {
      const e = [];
      for (let o = 0; o <= R; o++) e.push(j(-s / 2 + o * Tt, -h / 2 + t * Ct, x));
      H.push(e);
    }
    for (let t = 0; t < Z; t++) for (let e = 0; e < R; e++) {
      const o = -s / 2 + (e + 0.5) * Tt, n = -h / 2 + (t + 0.5) * Ct;
      Math.hypot(o, n) < oe || $(H[t][e], H[t][e + 1], H[t + 1][e + 1], H[t + 1][e], L);
    }
    function O(t, e) {
      let o = -1, n = 1 / 0;
      for (let a = 0; a <= Z; a++) for (let l = 0; l <= R; l++) {
        const u = H[a][l], w = Math.hypot(_[u][0] - t, _[u][1] - e);
        w < n && (n = w, o = u);
      }
      return o;
    }
    const T = Math.max(2, Math.round(p / Tt)), C = Math.max(2, Math.round(m / Ct)), ht = p / T, xt = m / C, lt = D / A, F = [];
    for (let t = 0; t <= A; t++) {
      const e = [];
      for (let o = 0; o <= T; o++) {
        const n = -p / 2 + o * ht;
        t === 0 ? e.push(O(n, -m / 2)) : e.push(j(n, -m / 2, x + t * lt));
      }
      F.push(e);
    }
    for (let t = 0; t < A; t++) for (let e = 0; e < T; e++) $(F[t][e], F[t][e + 1], F[t + 1][e + 1], F[t + 1][e], b);
    const I = [];
    for (let t = 0; t <= A; t++) {
      const e = [];
      for (let o = 0; o <= T; o++) {
    ...
    for (let t = 0; t <= A; t++) {
      ...
      for (let o = 0; o <= C; o++) {
        const n = -m / 2 + o * xt;
        t === 0 ? e.push(O(-p / 2, n)) : o === 0 ? e.push(F[t][0]) : o === C ? e.push(I[t][0]) : e.push(j(-p / 2, n, x + t * lt));
      }
      q.push(e);
    }
    for (let t = 0; t < A; t++) for (let e = 0; e < C; e++) $(q[t][e], q[t][e + 1], q[t + 1][e + 1], q[t + 1][e], b);
    const G = [];
    for (let t = 0; t <= A; t++) {
      const e = [];
      for (let o = 0; o <= C; o++) {
        const n = -m / 2 + o * xt;
        t === 0 ? e.push(O(p / 2, n)) : o === 0 ? e.push(F[t][T]) : o === C ? e.push(I[t][T]) : e.push(j(p / 2, n, x + t * lt));
      }
      G.push(e);
    }
    for (let t = 0; t < A; t++) for (let e = 0; e < C; e++) $(G[t][e], G[t][e + 1], G[t + 1][e + 1], G[t + 1][e], b);
    const ae = Oe(V / 1e3), se = 0.2, eo = ae / (2 * (1 + se)), le = 2 * b, Nt = p - 2 * b - le, kt = m - 2 * b - le, N = 4, k = 4, S = A, zt = Nt / N, ce = kt / k, oo = D / S, ie = (t, e) => Math.hypot(t, e) < oe + zt * 0.5, i = [];
    for (let t = 0; t <= S; t++) {
      const e = [];
      for (let o = 0; o <= k; o++) {
        const n = [];
        for (let a = 0; a <= N; a++) {
          const l = -Nt / 2 + a * zt, u = -kt / 2 + o * ce, w = x + L + t * oo;
          t === 0 && !ie(l, u) ? n.push(O(l, u)) : n.push(j(l, u, w));
        }
        e.push(n);
      }
      i.push(e);
    ...
    function v(t, e, o, n) {
      ...
      const a = y.length - 1;
      K.set(a, 1e-3), Q.set(a, ae), W.set(a, se), tt.set(a, 24 / 9.80665), et.set(a, eo), ot.set(a, 0), at.set(a, 0), nt.set(a, 0), st.set(a, 0);
    }
    ...
    const no = Math.min(0.2, D * 0.4), re = Math.min(0.1, (s - p) / 2 * 0.7), ao = Math.max(1, Math.round(no / lt));
    function E(t, e, o, n) {
      const [a, l] = t, [u, w] = e, U = o[0][n], Jt = O(a + u * re, l + w * re), He = o[Math.min(ao, o.length - 1)][n];
      $(U, Jt, He, He, b);
    }
    const de = Math.max(1, Math.round(T * 0.25)), pe = Math.max(1, Math.round(C * 0.25)), jt = Math.round(T / 2) - de, Ft = Math.round(T / 2) + de, It = Math.round(C / 2) - pe, St = Math.round(C / 2) + pe, me = -p / 2 + jt * ht, ue = -p / 2 + Ft * ht;
    E([
      me,
      m / 2
    ], [
    ...
    const xe = Math.PI * z * z / 4, Et = Math.PI * z ** 4 / 64, be = 2 * Et, ct = [], so = (s - 2 * Bt) / Math.max(1, te - 1), lo = (h - 2 * ee) / Math.max(1, gt - 1);
    for (let t = 0; t < te; t++) for (let e = 0; e < gt; e++) {
      const o = -s / 2 + Bt + t * so, n = -h / 2 + ee + e * lo;
    ...
    const co = [
      ...ct
    ], _e = Oe(V / 1e3), ve = 0.2, io = _e / (2 * (1 + ve)), g = 10, B = 10, P = 6, Lt = ut / g, Me = ft / B, ro = Pt / P, po = [];
    for (let t = 0; t <= Z; t++) for (let e = 0; e <= R; e++) {
      const o = H[t][e];
      po.push({
        id: o,
    ...
    for (let t = 0; t <= P; t++) {
      ...
      for (let o = 0; o <= B; o++) {
        const n = [];
        for (let a = 0; a <= g; a++) n.push(j(-ut / 2 + a * Lt, -ft / 2 + o * Me, -Pt + t * ro));
        e.push(n);
      }
      r.push(e);
    }
    function J(t, e, o, n) {
      ...
      const a = y.length - 1;
      K.set(a, 1e-3), Q.set(a, _e), W.set(a, ve), tt.set(a, 24 / 9.80665), et.set(a, io), ot.set(a, 0), at.set(a, 0), nt.set(a, 0), st.set(a, 0);
    }
    ...
    for (let t = 0; t < B; t++) for (let e = 0; e < g; e++) {
      ...
      mo(o, n) || J(r[P][t][e], r[P][t][e + 1], r[P][t + 1][e + 1], r[P][t + 1][e]);
    }
    ...
    for (const [t, e] of co) {
      const o = j(t, e, x + mt), n = O(t, e), a = j(t, e, x - pt);
    ...
    const Ht = [];
    _.forEach((t, e) => {
      Math.abs(t[2] - (x + D)) < 1e-6 && Math.abs(t[0]) <= p / 2 + 1e-6 && Math.abs(t[1]) <= m / 2 + 1e-6 && Ht.push(e);
    });
    const Ot = Math.max(1, Ht.length), fo = -X / Ot, ho = wt / Ot, xo = At / Ot, Be = /* @__PURE__ */ new Map();
    for (const t of Ht) Be.set(t, [
      0,
      0,
      fo,
      ho,
    ...
    let Gt = {}, Yt = {};
    try {
      Gt = qo(_, y, Pe, qt), Yt = Oo(_, y, qt, Gt);
    } catch (t) {
    ...
    function rt(t, e) {
      ...
      const a = new Eo().setFromPoints(n), l = new Lo(a, bo);
      it.push(l);
    }
    ...
    for (const [t, e] of [
      ...
      const o = new Vt(new Xt(we / 2, we / 2, D, 8), _o);
    ...
    const vo = new Ut({
      ...
      metalness: 0.7,
      roughness: 0.3
    }), Te = z * 0.8, Ce = z * 0.85, yo = x + mt + Te / 2;
    for (const [t, e] of ct) {
      const o = new Xt(z / 2, z / 2, pt + mt, 12), n = new Vt(o, vo);
      n.position.set(t, e, x + (-pt + mt) / 2), n.rotation.x = Math.PI / 2, it.push(n);
      const a = new Xt(Ce, Ce, Te, 6), l = new Vt(a, Mo);
      l.position.set(t, e, yo), l.rotation.x = Math.PI / 2, it.push(l);
    }
    let Dt = 0;
    ...
    const go = 0.65, dt = s * h, ke = ut * ft, Bo = Math.min(2, Math.sqrt(ke / dt)), Po = Math.min(0.85 * V * dt * Bo, 1.7 * V * dt), ze = go * Po, wo = X / Math.max(1, ze), je = Math.max(0, (s - 0.95 * Math.max(p, m)) / 2), Ao = X / dt, Fe = je * Math.sqrt(2 * Math.max(0, Ao) / (0.9 * Jo)), To = Fe / Math.max(1e-6, L), Co = Math.max(0.05, s - 2 * Bt), No = Math.sqrt(wt * wt + At * At), Ie = Math.max(0, No / Co - X / 2) / Math.max(1, gt), Se = 0.75 * (0.75 * Math.PI * z * z / 4) * Uo, ko = Ie / Math.max(1, Se), zo = p * m, $t = (p - 2 * b) * (m - 2 * b), Ee = zo - $t, Le = 35e4 * Ee + 0.85 * V * $t, jo = 0.75 * Le, Fo = X / Math.max(1, jo);
    Ke.val = {
      vmMax: Dt,
      A1: dt,
      A2: ke,
      ...
      Pno_composite: Le,
      demandCapPno: Fo
    }, $e.val = _, Je.val = y, Ue.val = Pe, Ve.val = qt, Xe.val = Gt, Re.val = Yt, Ze.val = it;
  });
  const Qe = Go({
    mesh: {
      nodes: $e,
  ...
  const Vo = Y.addFolder({
    title: "FEM"
  });
  Vo.addBinding(f, "vmMax", {
    readonly: true,
    label: "\u03C3 vM max (kN/m\xB2)",
    format: (s) => s.toExponential(3)
  });
  const We = Y.addFolder({
    title: "Unidades",
    expanded: false
  }), to = {
    stress: De.val,
  ...
  document.body.append($o(d), Qe, Yo({
