...
import { l as $t, a0 as xi, a1 as ql, v as we, a2 as Wl, D as Pt, M as pt, B as Fe, F as Mt, a3 as Zl, q as bt, a4 as Kl, a5 as Gl, a6 as wa, h as xa, a7 as ba, Y as Hs, a8 as Ai, a9 as Ti, $ as qn, H as ft, L as is, b as mt, f as gt, p as el, c as Jl, aa as Ql, V as te, I as an, J as Bt, ab as ho, u as Hi, d as Vt, a as uo, A as tl, k as Ri, t as er, s as Xn, P as tr, ac as zi, n as po, m as sr, w as Ns, X as Pn, E as va, S as vn, ad as fo, ae as Rn, g as _a, i as ka, j as Ca, C as Sa, x as nr, y as ir, z as or, W as ar, G as Ma, af as bi, N as mo, O as lr, ag as rr, _ as cr } from "./Text-Br8EG2up.js";
import { P as sl } from "./tweakpane-BXg6ZhiP.js";
let Ut, Tc, Md, Lc, vd, xd, ur, Da, rc, cc, wd, dc, xc, Ii, Cs, Vo, $d, Cd, kd, _d, Sd, bd;
let __tla = (async () => {
  class nl {
    constructor(e, t = 32) {
  ...
  function Po(l) {
    ...
    const e = [
      ...l
    ].sort((o, a) => o - a), t = (o) => e[Math.min(e.length - 1, Math.max(0, Math.round(o * (e.length - 1))))], s = Fo.val === "robusto" && e.length >= 20;
    let n = s ? t(0.01) : e[0], i = s ? t(0.99) : e[e.length - 1];
    return n >= 0 && i > 0 && (n = 0), i <= 0 && n < 0 && (i = 0), [
      n,
      i
    ];
  }
  ...
  const gr = new $t(16746496), yr = new $t(52428), Aa = new $t(52292), Ta = new $t(3377407), za = new $t(16763904);
  function Ia(l, e) {
  ...
  function Nr({ drawingObj: l, gridObj: e, scene: t, getActiveCamera: s, controls: n, gridSize: i, derivedDisplayScale: o, rendererElm: a, viewerRender: r }) {
    ...
    const _e = () => {
      ...
      return Se = new Float32Array(v), ce.geometry.dispose(), ce.geometry = new Fe(), ce.geometry.setAttribute("position", new bt(Se, 3)), ce.visible = Se.length > 0, Se;
    };
    let ge = null, xe = null;
    const Ne = (u, g) => {
      const w = _e();
  ...
  @5<45<30  \u2192 esf\xE9rica 3D: 5m, azimuth 45\xB0, elevaci\xF3n 30\xB0`, $e.style.cssText = [
    ...
    const ti = new te(), si = new te(), Xo = new te(), _l = (u) => {
      if (!zt) return null;
    ...
    window.__hekatanRevolveSelection = (u, g, w, m = 360) => {
      ...
      var _a3, _b, _c2;
      const { segs: w, auxIds: m } = Ko(), v = Ml(w), E = (de) => de.pts.every((me) => Math.abs(me[2] - de.pts[0][2]) < 1e-6), k = v.find((de) => de.cerrada && E(de)), M = v.find((de) => !de.cerrada && de.pts.length >= 2 && !E(de));
      if (!k) return {
        contorno: 0,
        perfil: 0,
        areas: 0,
    ...
    window.__hekatanUpdateSelectionPtScale = ea, n.addEventListener("change", () => {
      ...
      if ((!w.length || ((_c2 = (_b = (_a3 = window.__hekatanCadState) == null ? void 0 : _a3.get) == null ? void 0 : _b.call(_a3)) == null ? void 0 : _c2.tool) !== "fillarea") && _t.visible && (_t.visible = false), w.length) {
        ...
        if ($ === "select" || !$) {
          const F = (window.__hekatanSnap2D ?? 0.5) * 1.5, L = kl(m.x, m.y, m.z, F), j = Ui(m.x, m.y, m.z, F), Z = jo(m.x, m.y, m.z, F);
          if (L >= 0) {
        ...
        if (T.length > 0 && N[T[T.length - 1]]) {
          ...
          if (!j && !Z && window.__hekatanAxisSnap !== false) {
            ...
            for (const [xt, St] of Re) {
              const Ot = Xe(He.clone().addScaledVector(St, -Je)), fs = Xe(He.clone().addScaledVector(St, Je)), xn = fs.x - Ot.x, Js = fs.y - Ot.y, Hl = tt - Ot.x, Xl = Le - Ot.y, Yl = xn * xn + Js * Js || 1;
              let wi = (Hl * xn + Xl * Js) / Yl;
              wi = Math.max(0, Math.min(1, wi));
              const fa = Math.hypot(tt - (Ot.x + wi * xn), Le - (Ot.y + wi * Js));
              if (Ze === null || fa < Ze.dpx) {
                const ro = c.ray, ma = He.clone().sub(ro.origin), co = St.dot(ro.direction), ga = St.dot(ma), jl = ro.direction.dot(ma), ya = 1 - co * co, Ul = Math.abs(ya) < 1e-6 ? -ga : (co * jl - ga) / ya;
                Ze = {
                  axis: xt,
                  dpx: fa,
  ...
  const Kr = 16755200, Ra = 56831, Gr = 56831, Jr = 56831, ki = 65382;
  function Qr(l) {
    const e = new ft();
    ...
    function q(Se, ce) {
      ...
      const Ke = Ur(), Ee = Wr[Ke.dispUnit] ?? 1e3, Be = qr[Ke.forceUnit] ?? 1;
      if (Qe >= 0) {
        const lt = ze[Qe];
        let et = `Nodo ${Qe}
...
nodos: [${lt.join(", ")}]`, qt === "shell" && ((_l = l.mesh) == null ? void 0 : _l.analyzeOutputs)) {
          const ht = l.mesh.analyzeOutputs.rawVal, rt = Zr[Ke.stressUnit] ?? 1, Tt = [
            [
              "bendingXX",
              "Mxx",
              Be,
...
Mz \u2248 ${Ft(un * Be)} ${Ke.forceUnit}\xB7m`);
    ...
    function D(Se, ce, _e) {
      ...
      if (Se.type === "node") {
        ...
        const Me = ge[Se.idx], Ie = V(Me[0]), Oe = V(Me[1]);
        if (Ie && Oe) {
  ...
  function oc(l, e) {
    ...
    function r() {
      ...
      const Ce = le - ie || 1, ve = pe - he || 1, Ae = 0.12 * Math.max(Ce, ve), Ve = 46, ot = Math.min((ue - 2 * Ve) / (Ce + 2 * Ae), (be - 2 * Ve) / (ve + 2 * Ae)), yt = (ue - Ce * ot) / 2, Se = (be - ve * ot) / 2, ce = (Ee) => yt + (Ee - ie) * ot, _e = (Ee) => be - (Se + (Ee - he) * ot), ge = "http://www.w3.org/2000/svg", xe = (Ee, Be, Rt) => {
      ...
      for (const Ee of ae) {
        const Be = ((_h = (_g = (_f = (_e2 = l.elementInputs) == null ? void 0 : _e2.rawVal) == null ? void 0 : _f.localAngles) == null ? void 0 : _g.get) == null ? void 0 : _h.call(_g, Ee.i)) ?? 0, Rt = Dn(ll(R ?? "normals", al(X[Q[Ee.i][0]], X[Q[Ee.i][1]], Be)), s.plano), at = Math.hypot(Rt.u, Rt.v);
        Ne.set(Ee.i, at > 0.3 ? [
          Rt.u / at,
          -Rt.v / at
        ] : null);
      ...
      const Ie = 0.12 * Math.max(Ce, ve) * ot, Oe = Me > 0 ? Ie / Me : 0, Ye = R === "bendingsY" || R === "bendingsZ", $e = (Ee) => Math.abs(Ee) >= 100 ? Ee.toFixed(1) : Math.abs(Ee) >= 10 ? Ee.toFixed(2) : Ee.toFixed(3), Qe = [];
      for (const Ee of ae) {
      ...
      const Ke = R ? sc[R] ?? R : "sin resultado";
      H.textContent = `${Ke} \xB7 ${s.plano === "XY" ? "planta" : "alzado"} ${s.plano} en ${q} = ${s.en.toFixed(2)} m`, U.textContent = R ? `${ae.length} barras en el plano \xB7 m\xE1ximo ${$e(Me)} ${nc[R] ?? ""}` + (Ye ? " \xB7 el momento va del lado de la tracci\xF3n" : "") + (ze ? ` \xB7 ${ze} barra(s) con este resultado en otro plano (sin dibujar)` : "") + " \xB7 clic en una barra: su gr\xE1fico" : "Eleg\xED un resultado en \xABFrame results\xBB (Axial, Cortante, Momento) para ver su diagrama aqu\xED.";
    }
    const c = () => {
      try {
        r();
  ...
  wd = function({ mesh: l, settingsObj: e, drawingObj: t, objects3D: s, solids: n }) {
    ...
    if (l) {
      ...
      o.add(ce);
      const _e = pc(l, _), ge = Vr(l, _, A, _e), xe = Da(_e);
    ...
    return ((n) => {
      ...
      if (l.nodes.val.forEach((xe, Ne) => {
        ...
        for (const Ie of l.elements.val) {
          ...
          if (Se === "losas" ? Oe : Se === "muros" ? Ye || $e : Se === "murosX" ? Ye : Se === "murosY" ? $e : false) for (const Ke of Ie) Ne.add(Ke);
        }
        const Me = [];
        for (const Ie of Ne) {
          const Oe = ge[Ie];
...
`;
  ...
  let Do, Va, Li, Ba, ul, Vs, pl, bc, rn, Wn, Zn, Lo, vc, _o, Nn, Ha, Xa, Qs, Ya, ja, fl, _c, Kn, Ua, nn;
  Do = globalThis;
  Va = (l) => l;
  Li = Do.trustedTypes;
  ...
  Lo = Array.isArray;
  vc = (l) => Lo(l) || typeof (l == null ? void 0 : l[Symbol.iterator]) == "function";
  _o = `[ 	
\f\r]`;
...
\f\r"'\`<>=]|("|')|))|$)`, "g");
  ...
  const kc = (l, e) => {
    ...
    for (let a = 0; a < t; a++) {
      const r = l[a];
      let c, d, p = -1, f = 0;
      for (; f < r.length && (o.lastIndex = f, d = o.exec(r), d !== null); ) f = o.lastIndex, o === Nn ? d[1] === "!--" ? o = Ha : d[1] !== void 0 ? o = Xa : d[2] !== void 0 ? (fl.test(d[2]) && (n = RegExp("</" + d[2], "g")), o = Qs) : d[3] !== void 0 && (o = Qs) : o === Qs ? d[0] === ">" ? (o = n ?? Nn, p = -1) : d[1] === void 0 ? p = -2 : (p = o.lastIndex - d[2].length, c = d[1], o = d[3] === void 0 ? Qs : d[3] === '"' ? ja : Ya) : o === ja || o === Ya ? o = Qs : o === Ha || o === Xa ? o = Nn : (o = Qs, n = void 0);
  ...
  const Ac = Do.litHtmlPolyfillSupport;
  Ac == null ? void 0 : Ac(Gn, Jn), (Do.litHtmlVersions ?? (Do.litHtmlVersions = [])).push("3.3.2");
  Tc = (l, e, t) => {
    const s = e;
    let n = s._$litPart$;
...
`).map((s) => s.trim()).find(Boolean) ?? "").replace(/[*`$]/g, "").slice(0, 60) || "Hoja \xB7 Hekatan LISP";
  ...
  Vo = function(l) {
    Yi(), as || (as = pd()), as.style.display = "flex", Oo(), setTimeout(() => {
      try {
  ...
  typeof window < "u" && (dt().__hekatanAgenteIA = Vo, dt().__hekatanPedirAgente = vl, dt().__hekatanAgenteTool = No);
})();
export {
  Ut as A,
  Tc as D,
