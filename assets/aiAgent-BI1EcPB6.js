...
import { l as $t, a0 as pn, a1 as Sl, v as xe, a2 as Ml, D as Pt, M as pt, B as ze, F as Mt, a3 as $l, q as bt, a4 as El, a5 as Al, a6 as ia, h as na, a7 as oa, Y as Vs, a8 as _n, a9 as kn, $ as Vi, H as ft, L as is, b as mt, f as gt, p as Ra, c as Tl, aa as zl, V as te, I as ti, J as Bt, ab as Jn, u as Tn, d as Vt, a as Qn, A as Da, k as Mn, t as Il, s as Li, P as Fl, ac as Cn, n as eo, m as Pl, w as Ds, X as Ti, E as aa, S as gi, ad as to, ae as zi, g as la, i as ra, j as ca, C as da, x as Rl, y as Dl, z as Ll, W as Nl, G as ha, af as fn, N as so, O as Ol, ag as Vl, _ as Bl } from "./Text-Br8EG2up.js";
...
function pa() {
  ...
  const t = new El(e, 256, 1, Al), s = bi(Ts.val);
...
function Yl(r, e, t) {
  ...
  return n.renderOrder = -1, n.frustumCulled = false, n.userData.isShellArea = true, n.name = "__hekatan_shell_colormap", xe.derive(() => {
    ...
    const o = [], a = [], l = [];
    e.val.forEach((A, F) => {
      A.length === 3 ? (o.push(A[0], A[1], A[2]), a.push(F), l.push(0)) : A.length === 4 && (o.push(A[0], A[1], A[2]), o.push(A[0], A[2], A[3]), a.push(F, F), l.push(0, 1));
...
function Ul(r, e, t) {
  ...
  const a = () => {
    ...
    x.addEventListener("mousedown", (F) => {
      b = true, C = F.clientX, _ = F.clientY;
      const X = s.getBoundingClientRect();
      S = X.left, A = X.top, s.style.left = `${S}px`, s.style.top = `${A}px`;
    }), window.addEventListener("mousemove", (F) => {
      if (!b) return;
      const X = F.clientX - C, q = F.clientY - _, G = Math.max(0, Math.min(window.innerWidth - 40, S + X)), W = Math.max(0, Math.min(window.innerHeight - 40, A + q));
  ...
  const f = (x, b) => {
    ...
    p = [];
    const C = (_, S) => {
      const A = Math.floor(Math.min(x[S], -50)), F = Math.ceil(Math.max(b[S], 50)), X = F - A > 400 ? 0.5 : 0.1;
      return c["pos" + _] = Math.max(A, Math.min(F, c["pos" + _])), l.addBinding(c, "pos" + _, { min: A, max: F, step: X, label: `  pos ${_} (m)` }).on("change", h);
    };
...
function ya(r, e, t, s, i, n, o = 8) {
  ...
  _ = [_[0] / S, _[1] / S, _[2] / S];
  const A = C(f, _), F = (ne, ae) => ne[0] * ae[0] + ne[1] * ae[1] + ne[2] * ae[2], X = (ne) => {
    const ae = ne ?? [0, 0, 0];
    return [(ae[0] || 0) * i, (ae[1] || 0) * i, (ae[2] || 0) * n];
  }, q = X(t), G = X(s), W = F(q, _), U = F(G, _), K = F(q, A), D = F(G, A), R = h ? F(h, A) * i : 0, B = p ? F(p, A) * i : 0, Q = h ? F(h, _) * i : 0, re = p ? F(p, _) * i : 0, pe = F(q, f), be = F(G, f), le = [];
...
function Gl(r, e, t) {
  ...
  return Vi((p, f) => {
    ...
    const p = e.elemFrames ? e.elemFrames.rawVal : true, f = e.elemColumns.rawVal, x = e.elemBeams.rawVal, b = e.elemZapatas ? e.elemZapatas.rawVal : true, C = e.elemLosas ? e.elemLosas.rawVal : true, _ = e.colorByType ? e.colorByType.rawVal : false, S = t.val, A = ((_e = r.elements) == null ? void 0 : _e.val) || [], F = (D) => {
      if (D.length === 2) {
        if (!p) return false;
        const R = S[D[0]], B = S[D[1]];
        return !R || !B ? true : wa(R, B) ? f : x;
      }
      if (D.length === 4) {
...
function no(r, e) {
  ...
  const c = new $t(t.grid).multiplyScalar(1.3), h = new $t(t.grid).multiplyScalar(0.8), p = (C, _, S, A) => {
    const F = [], X = C === "xy" ? (K, D) => [K, D, 0] : C === "xz" ? (K, D) => [K, 0, D] : (K, D) => [0, K, D], q = Math.floor(a / _);
    for (let K = -q; K <= q; K++) {
    ...
    const G = new ze();
    G.setAttribute("position", new Mt(F, 3));
    const W = new mt({ color: S, transparent: true, opacity: A, depthWrite: false }), U = new is(G, W);
    return U.name = `grid-${C}-${_ === o ? "minor" : "major"}`, U;
  }, f = (C, _, S) => {
    const A = C === "xy" ? (U, K) => [U, K, 0] : C === "xz" ? (U, K) => [U, 0, K] : (U, K) => [0, U, K], F = [[-a, -a], [a, -a], [a, a], [-a, a]], X = [];
    for (const [U, K] of F) X.push(...A(U, K));
    const q = new ze();
    q.setAttribute("position", new Mt(X, 3));
    const G = new mt({ color: _, transparent: true, opacity: S, depthWrite: false }), W = new Ra(q, G);
    return W.name = `grid-${C}-border`, W.renderOrder = 1, W;
  }, x = (C, _, S) => {
    const A = C === "xy" ? (G, W) => [G, W, 0] : C === "xz" ? (G, W) => [G, 0, W] : (G, W) => [0, G, W], F = _ === "u" ? [...A(-a, 0), ...A(a, 0)] : [...A(0, -a), ...A(0, a)], X = new ze();
    X.setAttribute("position", new Mt(F, 3));
...
function Ql(r, e, t, s) {
  ...
  return xe.derive(() => {
    ...
    (_b = (_a2 = r.nodeInputs) == null ? void 0 : _a2.val.supports) == null ? void 0 : _b.forEach((b, C) => {
      const _ = t.val[C];
      if (!_) return;
      const S = b ?? [], A = (S[0] ? 1 : 0) + (S[1] ? 1 : 0) + (S[2] ? 1 : 0), F = (S[3] ? 1 : 0) + (S[4] ? 1 : 0) + (S[5] ? 1 : 0);
      let X;
      A >= 3 && F >= 3 ? X = new pt(n, a) : A >= 3 && F === 0 ? X = new pt(o, l) : X = new pt(o, c), X.position.set(_[0], _[1], _[2]);
      const q = x * f();
...
function er(r, e, t, s) {
  ...
  xe.derive(() => {
    ...
    let p = h;
    if (h.length > c) {
      const C = h.map((B) => a[B][0]), _ = h.map((B) => a[B][1]), S = Math.min(...C), A = Math.max(...C), F = Math.min(..._), X = Math.max(..._), q = h.map((B) => a[B][2]), G = Math.max(1e-6, (Math.max(...q) - Math.min(...q)) / 40), W = (B) => Math.round(B / G), U = new Set(q.map(W)), K = Math.max(4, Math.floor(c / Math.max(1, U.size))), D = Math.max(2, Math.round(Math.sqrt(K))), R = /* @__PURE__ */ new Map();
      for (const B of h) {
        const Q = A - S < 1e-9 ? 0 : (a[B][0] - S) / (A - S), re = X - F < 1e-9 ? 0 : (a[B][1] - F) / (X - F), pe = Math.min(D - 1, Math.floor(Q * D)), be = Math.min(D - 1, Math.floor(re * D)), le = `${pe},${be},${W(a[B][2])}`, ne = Math.hypot(Q * D - (pe + 0.5), re * D - (be + 0.5)), ae = R.get(le);
    ...
    for (const C of p) {
      ...
      if (S) for (let A = 0; A < 3; A++) {
        const F = _[A];
        if (!(Math.abs(F) > 1e-9 * (f || 1))) continue;
        const X = new te(A === 0 ? Math.sign(F) : 0, A === 1 ? Math.sign(F) : 0, A === 2 ? Math.sign(F) : 0), q = 0.45 + 0.55 * (f ? Math.abs(F) / f : 1), G = new ti(X, new te(...S), 1, A === 2 ? 15637248 : 15022123, 0.3, 0.3);
        if (G.userData = { nudo: S, dir: X, rel: q }, i.add(G), x) {
          const W = new Bt(b(F), A === 2 ? "#f5b642" : "#ff6b5e");
...
function or(r, e, t, s) {
  ...
  return n.setAttribute("color", new Mt([...p, ...p, ...f, ...f, ...x, ...x], 3)), xe.derive(() => {
    ...
    e.deformedShape.val, e.orientations.val && (i.clear(), (_a2 = r.elements) == null ? void 0 : _a2.val.forEach((b) => {
      ...
      const F = 0.05 * e.gridSize.rawVal * 0.75 * s.rawVal;
      C.scale.set(F, F, F), i.add(C);
    }));
  }), xe.derive(() => {
...
function lr(r, e, t, s) {
  ...
  const S = new gt({ color: 52479, transparent: true, opacity: 0.35, side: Pt, depthWrite: false }), A = new mt({ color: 52479 }), F = new gt({ color: 16750848, transparent: true, opacity: 0.4, side: Pt, depthWrite: false }), X = new mt({ color: 16750848 });
  function q(G, W) {
  ...
  return xe.derive(() => {
    ...
    U.forEach((B, Q) => {
      ...
      if (le.type === "CFT") {
        ...
        const ve = new pt(he.steelFillGeom, F);
        ...
        switch (le.type) {
          ...
          case "circ":
            ...
            break;
          case "I":
            he = l(le.b, le.h, le.tf, le.tw), Se = F, ve = X;
            break;
          case "HSS":
            he = c(le.b, le.h, le.tw ?? le.b * 0.05), Se = F, ve = X;
            break;
          case "CFT":
            he = h(le.b, le.h, le.tw ?? le.b * 0.05), Se = F, ve = X;
            break;
          case "L":
            he = p(le.b ?? le.h, le.h, le.t ?? le.tw ?? 3e-3), Se = F, ve = X;
            break;
          case "2L":
            he = f(le.b ?? le.h, le.h, le.t ?? le.tw ?? 3e-3, le.dis ?? 0.01), Se = F, ve = X;
            break;
          case "C":
          case "coldC":
            he = x(le.b, le.h, le.tf ?? le.t ?? 3e-3, le.tw ?? le.t ?? 3e-3), Se = F, ve = X;
            break;
          case "2C":
            he = b(le.b, le.h, le.tf ?? 5e-3, le.tw ?? 5e-3, le.dis ?? 0.01), Se = F, ve = X;
            break;
          case "T":
            he = C(le.b, le.h, le.tf ?? 0.01, le.tw ?? 6e-3), Se = F, ve = X;
            break;
          case "pipe":
            he = _(le.d, le.tw ?? le.d * 0.05), Se = F, ve = X;
            break;
          default:
            return;
        }
  ...
  return xe.derive(() => {
    ...
    try {
      x.forEach((A, F) => {
        var _a3, _b2, _c3;
        if (A.length === 2) {
          let X = rr(C.get(F)), q = true;
          if (X || (X = cr((_a3 = b.areas) == null ? void 0 : _a3.get(F), (_b2 = b.momentsOfInertiaY) == null ? void 0 : _b2.get(F), (_c3 = b.momentsOfInertiaZ) == null ? void 0 : _c3.get(F)), q = false), !X) return;
...
class oo extends ft {
  constructor(e, t, s, i, n, o, a) {
    ...
    if (this.text = new Bt(`${n[0].toFixed(4)}`), this.text2 = new Bt(`${(n[1] * -1).toFixed(4)}`), this.normalizedResult = o, this.textPosition = po(e, t), this.text2Position = po(t, e), this.text.position.set(...this.textPosition), this.text2.position.set(...this.text2Position), this.text.rotation.setFromRotationMatrix(i), this.text2.rotation.setFromRotationMatrix(i), this.add(this.text, this.text2), c) {
      ...
      const S = new Cn(h), A = new Cn(p), F = new gt({ color: o[0] > 0 ? 24435 : 11411474, side: Pt }), X = new gt({ color: -o[1] > 0 ? 24435 : 11411474, side: Pt });
      this.mesh = new pt(S, F), this.mesh2 = new pt(A, X), this.mesh.position.set(...e), this.mesh2.position.set(...e), this.mesh.rotation.setFromRotationMatrix(i), this.mesh2.rotation.setFromRotationMatrix(i), a && this.mesh.rotateX(Math.PI / 2), a && this.mesh2.rotateX(Math.PI / 2), this.add(this.mesh, this.mesh2);
    } else {
...
function gr({ drawingObj: r, gridObj: e, scene: t, getActiveCamera: s, controls: i, gridSize: n, derivedDisplayScale: o, rendererElm: a, viewerRender: l }) {
  ...
  let _ = false, S = null, A = null, F = null;
  ...
  const q = (u, g, y) => Math.round(u * 1e3) + "," + Math.round(g * 1e3) + "," + Math.round(y * 1e3), G = (u) => {
    ...
    const P = [], L = [];
    for (let V = 1; V < T.length; V++) P.push(T[V].distanceTo(T[V - 1]));
    for (let V = 1; V < T.length - 1; V++) {
      const H = T[V].clone().sub(T[V - 1]).normalize(), ee = T[V + 1].clone().sub(T[V]).normalize();
      L.push(Math.acos(Math.max(-1, Math.min(1, H.dot(ee)))) / Math.max(1e-6, (P[V - 1] + P[V]) / 2));
    }
    const Y = L.map((V, H) => {
      let ee = 0, Z = 0;
    ...
    for (let N = 1; N < u.length - 1; N++) {
      const P = u[N].clone().sub(u[N - 1]).normalize(), L = u[N + 1].clone().sub(u[N]).normalize();
      g += Math.acos(Math.max(-1, Math.min(1, P.dot(L))));
    }
    ...
    const k = (N, P) => {
      for (let L = 1; L < u.length; L++) {
        ...
        if (Y <= P && P <= j || j <= P && P <= Y) {
          const O = Math.abs(j - Y) < 1e-12 ? 0 : (P - Y) / (j - Y);
          return u[L - 1].clone().lerp(u[L], O);
        }
      }
      return u[u.length - 1].clone();
    }, M = [], $ = v >= 0 ? u[0].getComponent(v) : 0, I = v >= 0 ? u[u.length - 1].getComponent(v) : 0, T = v >= 0 && Math.abs(I - $) > 1e-6 && u.every((N, P) => P === 0 || (N.getComponent(v) - u[P - 1].getComponent(v)) * (I - $) >= -1e-6);
    for (let N = 0; N <= y; N++) {
      const P = T ? k((L) => L.getComponent(v), $ + (I - $) * N / y) : k((L, Y) => E[Y], E[E.length - 1] * N / y);
      M.push([P.x, P.y, P.z]);
    }
    return M[0] = u[0].toArray(), M[y] = u[u.length - 1].toArray(), M;
  };
  window.__hekatanCadenaIfc = () => (F || []).map((u) => [u.x, u.y, u.z]);
  const D = /* @__PURE__ */ new Map(), R = (u) => {
    ...
    if (y) {
      ...
      const I = new te(), T = new te(), N = new te(), P = (Y) => Math.round(v[3 * Y] * 1e3) + "," + Math.round(v[3 * Y + 1] * 1e3) + "," + Math.round(v[3 * Y + 2] * 1e3), L = /* @__PURE__ */ new Map();
      for (let Y = 0; Y < m; Y++) {
        const j = 3 * Y;
        ...
        for (let O = 0; O < 3; O++) {
          const z = P(j + O), V = P(j + (O + 1) % 3), H = z < V ? z + "|" + V : V + "|" + z, ee = L.get(H);
  ...
  const re = (u, g) => {
    ...
    for (E[g] = 1; M.length && k.length < 4e4; ) {
      ...
      for (let I = 0; I < 3; I++) {
        const T = u.vec[3 * $ + I];
        if (T < 0 || E[T]) continue;
        const N = u.N[3 * $] * u.N[3 * T] + u.N[3 * $ + 1] * u.N[3 * T + 1] + u.N[3 * $ + 2] * u.N[3 * T + 2], P = v[0] * u.N[3 * T] + v[1] * u.N[3 * T + 1] + v[2] * u.N[3 * T + 2];
        N >= y && P >= m && (E[T] = 1, M.push(T));
      }
    }
    return k;
  }, pe = (u, g, y) => {
    ...
    for (const T of v.keys()) {
      ...
      let P = "", L = T;
      for (let Y = 0; Y < 1e5; Y++) {
        const j = (v.get(L) || []).find((O) => O !== P && !k.has(O));
        if (!j) break;
        N.push(j), k.add(j), P = L, L = j;
      }
      N.length > M.length && (M = N);
    }
    const $ = M.map((T) => E.get(T)), I = [];
    for (let T = 0; T < $.length; T++) {
      const N = $[(T + $.length - 1) % $.length], P = $[T], L = $[(T + 1) % $.length];
      if (P.distanceTo(N) < 1e-3) continue;
      const Y = P.clone().sub(N).normalize(), j = L.clone().sub(P).normalize();
      Y.dot(j) > Math.cos(3 * Math.PI / 180) || I.push(P);
    }
    return I;
  }, le = (u, g, y) => {
  ...
  const ue = 1, he = (u, g, y) => Math.floor(u / ue) + "," + Math.floor(g / ue) + "," + Math.floor(y / ue), Se = (u) => {
    ...
    if (y) {
      ...
      const I = (O) => Math.round(M[3 * O] * 1e3) + "," + Math.round(M[3 * O + 1] * 1e3) + "," + Math.round(M[3 * O + 2] * 1e3), T = new Float64Array(k * 3), N = new te(), P = new te(), L = new te();
      for (let O = 0; O < k; O++) {
        const z = 3 * O, V = 3 * O + 1, H = 3 * O + 2;
        N.set(M[3 * V] - M[3 * z], M[3 * V + 1] - M[3 * z + 1], M[3 * V + 2] - M[3 * z + 2]), P.set(M[3 * H] - M[3 * z], M[3 * H + 1] - M[3 * z + 1], M[3 * H + 2] - M[3 * z + 2]), L.crossVectors(N, P).normalize(), T[3 * O] = L.x, T[3 * O + 1] = L.y, T[3 * O + 2] = L.z;
      }
  ...
  const He = (u, g) => {
    ...
    let I = Fs, T = null, N = Fs, P = null, L = -1;
    ...
    for (let O = -1; O <= 1; O++) for (let z = -1; z <= 1; z++) for (let V = -1; V <= 1; V++) {
      ...
      if (H) for (const ee of H) {
        ...
        nt < N && (N = nt, P = Y.clone().lerp(j, Ae), L = ee);
      }
    }
    return L >= 0 && (m.adj || (m.adj = G(m.segs)), A = { S: m.segs, adj: m.adj, s: L }), T ? { tipo: "ifcVert", punto: T } : P ? { tipo: "ifcEdge", punto: P } : null;
  }, dt = () => {
    var _a3, _b, _c2;
      ...
      for (const k of y) {
        ...
        if (M) {
          ...
          for (let $ = 0; $ + 2 < M.count; $ += 3) {
            ...
            for (const [I, T] of g) {
              const N = [E[0].getComponent(I) - T, E[1].getComponent(I) - T, E[2].getComponent(I) - T], P = [];
              for (let L = 0; L < 3; L++) {
                const Y = E[L], j = E[(L + 1) % 3], O = N[L], z = N[(L + 1) % 3];
                (O < 0 && z >= 0 || O >= 0 && z < 0) && P.push(Y.clone().lerp(j, O / (O - z)));
              }
              P.length === 2 && v.push(P[0].x, P[0].y, P[0].z, P[1].x, P[1].y, P[1].z);
            }
          }
        }
      }
  ...
  const Le = (u, g) => {
    ...
    for (let $ = 0; $ + 5 < y.length; $ += 6) {
      ...
      if (!I || !T) continue;
      const N = T.x - I.x, P = T.y - I.y, L = N * N + P * P || 1e-9;
      let Y = ((u - I.x) * N + (g - I.y) * P) / L;
      Y = Math.max(0, Math.min(1, Y));
      const j = Math.hypot(u - (I.x + Y * N), g - (I.y + Y * P));
  ...
  const Me = () => {
    ...
    if (b.visible = !!window.__hekatanGridPlaneXZ, C.visible = !!window.__hekatanGridPlaneYZ, window.__hekatanOrthoRaycast === true && ls.visible) {
      const m = c.intersectObjects([ls, fs, ms], false);
  ...
  const Ve = new te(), Rt = (u, g, y, m, v, E) => {
    ...
    const T = a.getBoundingClientRect(), N = T.left + (Ve.x * 0.5 + 0.5) * T.width, P = T.top + (-Ve.y * 0.5 + 0.5) * T.height;
    Ee.style.left = N + "px", Ee.style.top = P + "px", Ee.style.display = "block";
  ...
  window.__hekatanTypeCoord = (u) => {
    ...
    const y = at(g);
    if (!y) return false;
    Jo(new te(y[0], y[1], y[2]), null), Ye = y, Ee.blur();
    try {
  ...
  const Wt = (u) => {
    ...
    c.setFromCamera(h, g);
    let y = null, m = null;
    const v = c.intersectObjects(t.children, true).filter((P) => P.object.isMesh && P.object !== vt && P.object !== _t && P.object.visible !== false);
    if (v.length) {
      const P = v[0], L = P.point;
      y = [L.x, L.y, L.z];
      const j = (_b = (_a3 = P.object.geometry) == null ? void 0 : _a3.attributes) == null ? void 0 : _b.position;
      j && P.face && (m = [P.face.a, P.face.b, P.face.c].map((O) => {
        const z = new te().fromBufferAttribute(j, O);
        return P.object.localToWorld(z), [z.x, z.y, z.z];
      }));
    } else {
      const P = Me();
      if (P.length) {
    ...
    if (!y) return null;
    const E = a.getBoundingClientRect(), k = (P) => {
      const L = new te(P[0], P[1], P[2]).project(g);
      return [E.left + (L.x * 0.5 + 0.5) * E.width, E.top + (-L.y * 0.5 + 0.5) * E.height];
    }, M = [u.clientX, u.clientY], $ = 14;
    let I = y, T = $;
    const N = (P) => {
      const L = k(P), Y = Math.hypot(L[0] - M[0], L[1] - M[1]);
      Y < T && (T = Y, I = P);
    };
    for (const P of m ?? []) N(P);
    for (const P of r.points.rawVal) N(P);
    return I;
  }, Dt = () => {
    if (Ke.length < 1) {
  ...
  const ks = (u) => {
    ...
    const I = ((_d = (_c2 = (_b = window.__hekatanCadState) == null ? void 0 : _b.get) == null ? void 0 : _c2.call(_b)) == null ? void 0 : _d.workPlane) ?? "xy", T = (O) => I === "xy" ? [O[0], O[1]] : I === "xz" ? [O[0], O[2]] : [O[1], O[2]], N = T(u), P = (O, z) => {
      let V = false;
    ...
    return Y;
  }, Lt = new ft(), Ht = new pt(new Ds(1, 1), new gt({ color: 2282478, transparent: true, opacity: 0.08, side: Pt, depthWrite: false })), Cs = new is(new aa(new Ds(1, 1)), new mt({ color: 2282478, transparent: true, opacity: 0.85 })), Bs = new is(new ze(), new mt({ color: 2282478, transparent: true, opacity: 0.3 })), qi = (u, g) => {
  ...
  const _i = (u) => {
    const g = new ze().setFromPoints([new te(0, 0, 0), new te(0, 0, 0), new te(0, 0, 0), new te(0, 0, 0)]), y = new mt({ color: u, transparent: true, opacity: 0.2, depthTest: false }), m = new Ra(g, y);
  ...
  }, ls = li(3462041), fs = li(16724804), ms = li(6333946);
  ps.add(ls, fs, ms);
  const Ms = (u, g, y, m) => {
  ...
  ws.id = "hk-refplane-badge", ws.style.cssText = ["position:fixed", "pointer-events:none", "z-index:99997", "padding:3px 10px", "border-radius:4px", "font-family:Consolas,monospace", "font-size:12px", "font-weight:bold", "transform:translate(20px,40px)", "white-space:nowrap", "display:none"].join(";") + ";", document.body.appendChild(ws), window.__hekatanSetOrthoPlanes = (u) => {
    ...
    if (window.__hekatanShowOrthoPlanes = u, ps.visible = u, u) {
      ...
      $s(Us, E, "xy", k), $s(oi, E, "xz", k), $s(ai, E, "yz", k), Ms(ls, E, "xy", k), Ms(fs, E, "xz", k), Ms(ms, E, "yz", k), ls.material.opacity = 0.05, fs.material.opacity = 0.05, ms.material.opacity = 0.05;
    } else {
    ...
    $s(Us, E, "xy", u), $s(oi, E, "xz", u), $s(ai, E, "yz", u), Ms(ls, E, "xy", u), Ms(fs, E, "xz", u), Ms(ms, E, "yz", u), l();
  };
  const Mo = (u) => {
    if (ls.material.opacity = u === "xy" ? 0.09 : 0.025, fs.material.opacity = u === "xz" ? 0.09 : 0.025, ms.material.opacity = u === "yz" ? 0.09 : 0.025, u) {
  ...
  const Wi = new te(), Zi = new te(), Eo = new te(), el = (u) => {
    if (!zt) return null;
    const g = u[0], y = u[1], m = u[2];
    return zt === "x" ? (Wi.set(g - 1e4, y, m), Zi.set(g + 1e4, y, m)) : zt === "y" ? (Wi.set(g, y - 1e4, m), Zi.set(g, y + 1e4, m)) : (Wi.set(g, y, m - 1e4), Zi.set(g, y, m + 1e4)), c.ray.distanceSqToSegment(Wi, Zi, null, Eo), Eo;
  };
  ...
  const rs = new pt(new gi(0.02, 12, 12), new gt({ color: 16766720, transparent: true, opacity: 0.9, depthTest: false }));
  rs.renderOrder = 998, rs.visible = false, t.add(rs);
  const Ki = (u) => {
    const g = s();
    ...
    rs.visible && rs.scale.setScalar(Ki(rs.position));
  }, xs = new ft();
  ...
  const tl = (u, g, y, m) => {
    ...
    for (const k of Ue) {
      ...
      if (M === "pt") {
        ...
        if (!T || !N) continue;
        const P = new ze().setFromPoints([new te(T[0], T[1], T[2]), new te(N[0], N[1], N[2])]), L = new Vt(P, new mt({ color: Gi, transparent: true, opacity: 0.95, depthTest: false }));
        ...
        const N = new ze().setFromPoints(T), P = new Vt(N, new mt({ color: Gi, transparent: true, opacity: 0.95, depthTest: false }));
        P.renderOrder = 999, xs.add(P);
      } else if (M === "aux") {
        const I = m[+$[0]];
  ...
  window.__hekatanRefreshSelection = cs, window.__hekatanSelectIds = (u) => {
    var _a3;
    Ue.clear();
  ...
  const ki = (u, g, y, m, v, E, k, M, $) => {
    const I = k - m, T = M - v, N = $ - E, P = I * I + T * T + N * N;
    if (P < 1e-12) return Math.hypot(u - m, g - v, y - E);
    let L = ((u - m) * I + (g - v) * T + (y - E) * N) / P;
    ...
    for (let I = 0; I < v.length; I++) {
      ...
      for (let N = 0; N < T.length - 1; N++) {
        const P = E[T[N]], L = E[T[N + 1]];
        if (!P || !L) continue;
        const Y = ki(u, g, y, P[0], P[1], P[2], L[0], L[1], L[2]);
        Y < $ && ($ = Y, k = I, M = N);
      }
    }
    ...
    for (const P of k) for (const L of P) M.add(L);
    const $ = r.points.rawVal, I = /* @__PURE__ */ new Map(), T = [];
    for (let P = 0; P < $.length; P++) M.has(P) && (I.set(P, T.length), T.push($[P]));
    const N = k.map((P) => P.map((L) => I.get(L)).filter((L) => L !== void 0));
    if (r.points.val = T, r.polylines.val = N, r.areas) {
      const P = E.length - 1;
      r.areas.val = r.areas.rawVal.map((L) => L > u ? L + P : L);
    }
  ...
  const Po = () => {
    ...
    for (const v of g) {
      ...
      const M = [0, 1, 2].map((T) => k.reduce((N, P) => N + P[T], 0) / k.length), $ = k.map((T) => Math.hypot(T[0] - M[0], T[1] - M[1], T[2] - M[2])), I = $.reduce((T, N) => T + N, 0) / $.length;
      I < 1e-9 || $.some((T) => Math.abs(T - I) > 5e-3 * I) || m.push({ c: M, r: I });
    }
    return Io = m;
  };
  window.__hekatanCentrosDeducidos = Po;
  ...
  window.__hekatanDrawCircle = (u, g, y, m, v = window.__hekatanArcSegs ?? 12, E = "xy") => {
    ...
    for (let I = 0; I < k; I++) {
      const T = 2 * Math.PI * I / k, N = m * Math.cos(T), P = m * Math.sin(T);
      let L;
      E === "xy" ? L = [u + N, g + P, y] : E === "xz" ? L = [u + N, g, y + P] : L = [u, g + N, y + P], $.push(L);
    }
    ...
    if (r.points.val = [...r.points.rawVal, ...$], r.polylines) {
      const I = [...$.map((N, P) => M + P), M], T = r.polylines.rawVal;
    ...
    let P;
    if (N < 1e-12) P = new te().addVectors(E, M).multiplyScalar(0.5);
    else {
      const ke = I.clone().multiplyScalar($.lengthSq()).sub($.clone().multiplyScalar(I.lengthSq())), Ae = new te().crossVectors(ke, T);
      P = E.clone().add(Ae.divideScalar(N));
    }
    const L = E.distanceTo(P), Y = T.lengthSq() > 1e-12 ? T.clone().normalize() : new te(0, 1, 0), j = new te().subVectors(E, P).normalize(), O = new te().crossVectors(Y, j).normalize(), z = (ke) => {
      const Ae = new te().subVectors(ke, P);
      ...
      return P.clone().add(Ae.multiplyScalar(L));
    ...
    if (Qi.push({ c: [P.x, P.y, P.z], r: L }), en()) {
      tn(se, false);
      return;
    }
    ...
    for (let Z = 0; Z <= m; Z++) {
      ...
      se[N] = J, se[P] = j(J), se[$] = u[0][$], ee.push(se);
    }
    if (ee[0] = [u[0][0], u[0][1], u[0][2]], ee[m] = [u[y - 1][0], u[y - 1][1], u[y - 1][2]], en()) return tn(ee, false), { ok: true, plano: I, coef: O, ia: N, io: P };
    ...
    return { ok: true, plano: I, coef: O, ia: N, io: P };
  };
  const Ro = () => {
    var _a3, _b;
    ...
    return [...Ue].forEach((I) => {
      ...
      if (T[0] === "aux") {
        const P = v[+T[1]];
        P && P.length === 6 && (E.push([[P[0], P[1], P[2]], [P[3], P[4], P[5]]]), k.push(I));
        return;
      }
      ...
      if (!(N < 0 || !g[N] || y.has(N))) if (T[0] === "poly") {
        if (M.has(N)) return;
        M.add(N);
        for (let P = 0; P + 1 < g[N].length; P++) E.push([$(g[N][P]), $(g[N][P + 1])]);
      } else {
        const P = g[N][+T[2]], L = g[N][+T[2] + 1];
        P != null && L != null && !M.has(N) && E.push([$(P), $(L)]);
      }
    }), { segs: E, auxIds: k };
  ...
  window.__hekatanRevolveSelection = (u, g, y, m = 360) => {
    var _a3, _b, _c2;
    const v = Math.max(3, Math.round(y || 16)), E = Math.abs(m - 360) < 1e-9, k = v, M = E ? v : v + 1, { segs: $, auxIds: I } = Ro();
    ...
    Et();
    const T = r.points.rawVal, N = ((_a3 = r.polylines) == null ? void 0 : _a3.rawVal) ?? [], P = [...T];
    let L = N.slice();
    ...
    L.push([]), r.points.val = P, r.polylines && (r.polylines.val = L), r.areas && (r.areas.val = Y);
    const J = Do(I);
    try {
      (_c2 = window.__hekatanRebuild) == null ? void 0 : _c2.call(window);
    } catch {
    ...
    var _a3, _b, _c2;
    const { segs: y, auxIds: m } = Ro(), v = nl(y), E = (de) => de.pts.every((me) => Math.abs(me[2] - de.pts[0][2]) < 1e-6), k = v.find((de) => de.cerrada && E(de)), M = v.find((de) => !de.cerrada && de.pts.length >= 2 && !E(de));
    ...
    const P = N > 0 ? 1 : -1, L = (de) => {
      const me = $[(de - 1 + I) % I], ke = $[de], Ae = $[(de + 1) % I], nt = [ke[0] - me[0], ke[1] - me[1]], je = [Ae[0] - ke[0], Ae[1] - ke[1]], et = Math.hypot(nt[0], nt[1]) || 1, De = Math.hypot(je[0], je[1]) || 1, Je = [P * nt[1] / et, -P * nt[0] / et], Be = [P * je[1] / De, -P * je[0] / De], Pe = 1 + (Je[0] * Be[0] + Je[1] * Be[1]);
    ...
    const E = Math.min(u[0], g[0]), k = Math.max(u[0], g[0]), M = Math.min(u[1], g[1]), $ = Math.max(u[1], g[1]), I = (u[2] + g[2]) / 2, T = k - E, N = $ - M, P = Math.min(y, T / 2 - 0.01, N / 2 - 0.01);
    if (P <= 0) return;
    const L = r.points.rawVal.length, Y = [], j = [], O = (z, V) => {
      Y.push([z, V, I]), j.push(L + Y.length - 1);
    };
    for (let z = 0; z <= v; z++) O(E + P + (T - 2 * P) * z / v, M);
    for (let z = 1; z <= m; z++) {
      const V = -Math.PI / 2 + Math.PI / 2 * z / m;
      O(k - P + P * Math.cos(V), M + P + P * Math.sin(V));
    }
    for (let z = 1; z <= v; z++) O(k, M + P + (N - 2 * P) * z / v);
    for (let z = 1; z <= m; z++) {
      const V = 0 + Math.PI / 2 * z / m;
      O(k - P + P * Math.cos(V), $ - P + P * Math.sin(V));
    }
    for (let z = 1; z <= v; z++) O(k - P - (T - 2 * P) * z / v, $);
    for (let z = 1; z <= m; z++) {
      const V = Math.PI / 2 + Math.PI / 2 * z / m;
      O(E + P + P * Math.cos(V), $ - P + P * Math.sin(V));
    }
    for (let z = 1; z <= v; z++) O(E, $ - P - (N - 2 * P) * z / v);
    for (let z = 1; z < m; z++) {
      const V = Math.PI + Math.PI / 2 * z / m;
      O(E + P + P * Math.cos(V), M + P + P * Math.sin(V));
    }
    if (j.push(L), en()) {
      tn(Y, true);
      return;
    ...
    let I;
    if (_ && r.gridTarget) {
      const T = r.gridTarget.rawVal, N = new zi(...T.rotation), P = new te(1, 0, 0).applyEuler(N), L = new te(0, 1, 0).applyEuler(N), Y = new te(...T.position), j = new te(m, v, E), O = new te(k, M, $), z = j.clone().sub(Y).dot(P), V = j.clone().sub(Y).dot(L), H = O.clone().sub(Y).dot(P), ee = O.clone().sub(Y).dot(L), Z = (J, se) => Y.clone().addScaledVector(P, J).addScaledVector(L, se).toArray();
    ...
    if (window.__hekatanPushUndo && window.__hekatanPushUndo(), r.points.val = [...r.points.rawVal, ...I], r.polylines) {
      const T = r.polylines.rawVal, N = T.length - 1, P = [y, y + 1, y + 2, y + 3, y];
      r.polylines.val = [...T.slice(0, -1), P, []], r.areas && (r.areas.val = [...r.areas.rawVal, N]);
    }
    try {
      (_a3 = window.__hekatanRebuild) == null ? void 0 : _a3.call(window);
    } catch {
    ...
    const P = [...((_b = r.areas) == null ? void 0 : _b.rawVal) ?? []], L = new Set(P.map((O) => [...new Set(E[O] ?? [])].sort((z, V) => z - V).join("-"))), Y = [...E];
    let j = 0;
    for (const O of T) {
      const z = O.slice().sort((V, H) => V - H).join("-");
      L.has(z) || (L.add(z), Y.push([...O, O[0]]), P.push(Y.length - 1), j++);
    }
    if (j) {
      window.__hekatanPushUndo && window.__hekatanPushUndo(), r.polylines.val = Y, r.areas && (r.areas.val = P);
      try {
        (_c2 = window.__hekatanRebuild) == null ? void 0 : _c2.call(window);
      } catch {
      }
    ...
    M /= T, $ /= T, I /= T;
    let N = v * I - E * $, P = E * M - m * I, L = m * $ - v * M;
    const Y = Math.hypot(N, P, L) || 1;
    N /= Y, P /= Y, L /= Y;
    const j = u[0], O = (Pe) => [(Pe[0] - j[0]) * M + (Pe[1] - j[1]) * $ + (Pe[2] - j[2]) * I, (Pe[0] - j[0]) * N + (Pe[1] - j[1]) * P + (Pe[2] - j[2]) * L], z = (Pe, Xe) => [j[0] + Pe * M + Xe * N, j[1] + Pe * $ + Xe * P, j[2] + Pe * I + Xe * L], V = u.map(O);
    ...
    const de = (Pe, Xe) => {
      ...
      for (let xt = 0, St = V.length - 1; xt < V.length; St = xt++) {
        const [Ot, us] = V[xt], [fi, Ks] = V[St];
        us > Xe != Ks > Xe && Pe < (fi - Ot) * (Xe - us) / (Ks - us) + Ot && (We = !We);
      }
      return We;
  ...
  const ds = new ft();
  ds.visible = false, t.add(ds), window.__hekatanShowAxes = (u, g, y = 12, m = 2) => {
    var _a3, _b;
    for (; ds.children.length; ) {
      const T = ds.children.pop();
      (_a3 = T.geometry) == null ? void 0 : _a3.dispose(), (_b = T.material) == null ? void 0 : _b.dispose();
    }
    if (!u.length || !g.length) return;
    const v = Math.min(...g) - m, E = Math.max(...g) + m, k = Math.min(...u) - m, M = Math.max(...u) + m, $ = "ABCDEFGHIJKLMNOPQRSTUVWXYZ", I = (T, N, P, L, Y) => {
      ...
      return H.position.set(N, P, L), H.scale.set(1.2, 0.6, 1), H;
    };
    u.forEach((T, N) => {
      const P = N < $.length ? $[N] : `X${N}`, L = new ze().setFromPoints([new te(T, v, 0), new te(T, E, 0), new te(T, v, 0), new te(T, v, y)]), Y = new Ti({ color: 6333946, dashSize: 0.3, gapSize: 0.15, transparent: true, opacity: 0.6 }), j = new is(L, Y);
      j.computeLineDistances(), ds.add(j), ds.add(I(P, T, v - 0.5, 0, "#60a5fa")), ds.add(I(P, T, E + 0.5, 0, "#60a5fa"));
    }), g.forEach((T, N) => {
      const P = `${N + 1}`, L = new ze().setFromPoints([new te(k, T, 0), new te(M, T, 0), new te(k, T, 0), new te(k, T, y)]), Y = new Ti({ color: 16478597, dashSize: 0.3, gapSize: 0.15, transparent: true, opacity: 0.6 }), j = new is(L, Y);
      j.computeLineDistances(), ds.add(j), ds.add(I(P, k - 0.5, T, 0, "#fb7185")), ds.add(I(P, M + 0.5, T, 0, "#fb7185"));
    }), ds.visible = true, l();
  ...
  window.__hekatanShowRefPlanes = (u = [0, 3, 6, 9, 12], g = 20, y = 0, m = 0) => {
    ...
    u.forEach((E, k) => {
      ...
      const Y = new la(P), j = new ra({ map: Y, transparent: true }), O = new ca(j);
  ...
  window.__hekatanUpdateSelectionPtScale = Oo, i.addEventListener("change", () => {
    var _a3;
    Mi(), rs.visible && Ao(), (_a3 = window.__hekatanUpdateOsnapScale) == null ? void 0 : _a3.call(window), Oo();
    ...
    if ((!y.length || ((_c2 = (_b = (_a3 = window.__hekatanCadState) == null ? void 0 : _a3.get) == null ? void 0 : _b.call(_a3)) == null ? void 0 : _c2.tool) !== "fillarea") && _t.visible && (_t.visible = false), y.length) {
      ...
      if (M) ln(M.type, M.x, M.y, M.z), vt.position.set(M.x, M.y, M.z), vt.visible = true, m.set(M.x, M.y, M.z), cn(M.type, u.clientX, u.clientY);
        ...
        dl(), rn();
        const P = !v && window.__hekatanSnapEnabled !== false, L = window.__hekatanSnap2D ?? 0.5;
        P && L > 0 && (m.x = Math.round(m.x / L) * L, m.y = Math.round(m.y / L) * L, m.z = Math.round(m.z / L) * L), vt.position.copy(m), vt.visible = true;
      }
      ...
      if ($ === "select" || !$) {
        const P = (window.__hekatanSnap2D ?? 0.5) * 1.5, L = tl(m.x, m.y, m.z, P), Y = Pn(m.x, m.y, m.z, P), j = To(m.x, m.y, m.z, P);
        if (L >= 0) {
          const H = r.points.rawVal[L];
          rs.position.set(H[0], H[1], H[2]), rs.visible = true, Ao(), ys.visible = false, bs = { kind: "pt", a: L };
        } else if (Y) {
          const H = r.points.rawVal, ee = r.polylines.rawVal[Y.polyIdx], Z = H[ee[Y.segIdx]], J = H[ee[Y.segIdx + 1]];
          ys.geometry.setFromPoints([new te(Z[0], Z[1], Z[2]), new te(J[0], J[1], J[2])]), ys.visible = true, rs.visible = false, bs = ((_l = (_k = r.areas) == null ? void 0 : _k.rawVal) == null ? void 0 : _l.includes(Y.polyIdx)) ?? false ? { kind: "poly", a: Y.polyIdx } : { kind: "seg", a: Y.polyIdx, b: Y.segIdx };
        } else if (j >= 0) {
          const ee = (((_m = window.__hekatanDrawingAuxLines) == null ? void 0 : _m.rawVal) ?? [])[j];
          ee && (ys.geometry.setFromPoints([new te(ee[0], ee[1], ee[2]), new te(ee[3], ee[4], ee[5])]), ys.visible = true, rs.visible = false, bs = { kind: "aux", a: j });
        } else ys.visible = false, rs.visible = false, bs = null;
      ...
      if ($ === "delete" || $ === "trim" || $ === "extend" || $ === "offset") {
        const P = (window.__hekatanSnap2D ?? 0.5) * 1.5, L = Pn(m.x, m.y, m.z, P), Y = To(m.x, m.y, m.z, P);
        let j = false;
        if (Y >= 0) if (!L) j = true;
        else {
        ...
        j ? (Is = Y, gs = -1, zs = -1, sl(Y)) : L ? (gs = L.polyIdx, zs = L.segIdx, Is = -1, il(L.polyIdx, L.segIdx)) : (gs = -1, zs = -1, Is = -1, Qt.visible = false), Re.visible = false, Zt.visible = false, Kt.visible = false, ot(), Fe.style.left = u.clientX + "px", Fe.style.top = u.clientY + "px", Fe.style.display = "block";
      ...
      if (T.length > 0 && N[T[T.length - 1]]) {
        ...
        if (!Y && !j && window.__hekatanAxisSnap !== false) {
          ...
          for (const [xt, St] of Pe) {
            const Ot = Xe(Be.clone().addScaledVector(St, -Je)), us = Xe(Be.clone().addScaledVector(St, Je)), fi = us.x - Ot.x, Ks = us.y - Ot.y, bl = et - Ot.x, vl = De - Ot.y, _l2 = fi * fi + Ks * Ks || 1;
        ...
        if (!Y && !j && O) {
          ...
          let Be = null;
          Je === ls ? Be = "xy" : Je === fs ? Be = "xz" : Je === ms && (Be = "yz"), Be === "xy" ? Y = je >= et ? "x" : "y" : Be === "xz" ? Y = je >= De ? "x" : "z" : Be === "yz" ? Y = et >= De ? "y" : "z" : Y = je >= et && je >= De ? "x" : et >= De ? "y" : "z";
        }
        ...
        if (!Y && !j && z) {
          ...
          if (Je > 1e-3) {
            ...
            St.sort((Ot, us) => Ot[1] - us[1]), St[0][1] <= Pe && (Y = St[0][0]);
          }
        }
        if (Y) {
          ...
          let We = null;
          Xe === ls ? We = "xy" : Xe === fs ? We = "xz" : Xe === ms && (We = "yz");
        ...
        ps.visible = ie, ie || Mo(null), ie && ($s(Us, L, "xy", oe), $s(oi, L, "xz", oe), $s(ai, L, "yz", oe), Ms(ls, L, "xy", oe), Ms(fs, L, "xz", oe), Ms(ms, L, "yz", oe));
        const de = ie ? c.intersectObjects([ls, fs, ms], false) : [];
        let me = null;
        if (de.length > 0) {
          const je = de[0].object;
          je === ls ? me = "xy" : je === fs ? me = "xz" : je === ms && (me = "yz");
        }
        ...
        const P = `X=${m.x.toFixed(2)} Y=${m.y.toFixed(2)} Z=${m.z.toFixed(2)}`;
        Fe.textContent = P;
        const L = document.getElementById("hk-coord-fixed");
        if (L && (L.textContent = P), Re.visible = false, Zt.visible = false, (/* @__PURE__ */ new Set(["line", "polyline", "area", "node", "column", "wall", "rect", "circle", "arc", "polyline-multi", "axis", "chaflan"])).has($)) {
  ...
  const Vn = (u, g, y, m, v) => {
    ...
    for (let J = 0; J < V.length; J++) {
      ...
      if (H.includes(J)) {
        ...
        !de || !me || z(P(de), P(me)) && (Ue.add(`seg:${J}:${ie}`), j++);
      }
    }
    ...
    for (let J = 0; J < Z.length; J++) {
      ...
      const oe = P([se[0], se[1], se[2]]), ie = P([se[3], se[4], se[5]]);
      z(oe, ie) && (Ue.add(`aux:${J}`), j++);
    }
    cs(), we(j === 0 && !I ? "\u{1F535} Window (izq\u2192der) \u2014 0: solo coge lo que quede ENTERO dentro. De DERECHA a IZQUIERDA (\u{1F7E2} captura) coge todo lo que toque." : `${I ? "\u{1F7E2} Crossing" : "\u{1F535} Window"} \u2014 ${j} item(s) ${v ? "agregados a" : "\u2192"} selecci\xF3n (total ${Ue.size})`), Gt.style.display = "none";
  }, on = () => {
  ...
  const Bn = () => {
    ...
    for (const Y of m) {
      const j = L.get(Y);
      j !== void 0 && j < N.length && P.push(j);
    }
    if (r.polylines && (r.polylines.val = N), r.areas && (r.areas.val = P), I.size > 0 && v) {
  ...
  const Nt = (u, g, y, m) => {
    ...
    yt = new La({ container: es, title: T });
    {
      ...
      const P = { vuelo: 1.5, losa: true, borde: true, ambos: true }, L = N.addFolder({ title: "\u2310 Volado sobre la viga designada", expanded: false });
      L.addBinding(P, "vuelo", { label: "vuelo (m)", min: 0.1, max: 6, step: 0.05 }), L.addBinding(P, "losa", { label: "con pa\xF1o de losa (si no, hueca)" }), L.addBinding(P, "borde", { label: "con viga de borde" }), L.addBinding(P, "ambos", { label: "a los dos lados" }), L.addButton({ title: "\u2310 Poner volado (VOL)" }).on("click", () => {
        var _a4;
        const j = (_a4 = window.__hekatanVoladoSelection) == null ? void 0 : _a4.call(window, P.vuelo, { losa: P.losa, vigaBorde: P.borde, lados: P.ambos ? "ambos" : "afuera" });
        we(j ? `\u2310 Volado de ${P.vuelo} m en ${j} pa\xF1o(s)` + (P.losa ? " con losa" : " hueco") : "\u26A0 Design\xE1 una VIGA (un segmento) y volv\xE9 a pulsar");
    ...
    if (E) {
      ...
      N.addButton({ title: `\u25B2 Empotrar los ${g.length} nudo(s) (6 GDL)` }).on("click", () => P([true, true, true, true, true, true], "Empotrado")), N.addButton({ title: `\u25B3 Articular los ${g.length} nudo(s) (Ux Uy Uz)` }).on("click", () => P([true, true, true, false, false, false], "Articulado"));
    ...
    if (k) {
      ...
      const P = yt.addFolder({ title: "\u{1F527} Property Modifiers", expanded: false });
      P.addBinding(fe, "A_mod", { label: "A mod", min: 0, max: 10, step: 0.1 }), P.addBinding(fe, "Iz_mod", { label: "Iz mod (fuerte)", min: 0, max: 10, step: 0.1 }), P.addBinding(fe, "Iy_mod", { label: "Iy mod (d\xE9bil)", min: 0, max: 10, step: 0.1 }), P.addBinding(fe, "J_mod", { label: "J mod", min: 0, max: 10, step: 0.1 }), yt.addFolder({ title: "\u{1F3AF} Insertion Point", expanded: false }).addBinding(fe, "insertionPoint", { label: "Cardinal", options: { "1 \u2014 Bottom Left": "1 \u2014 Bottom Left", "2 \u2014 Bottom Center": "2 \u2014 Bottom Center", "3 \u2014 Bottom Right": "3 \u2014 Bottom Right", "4 \u2014 Middle Left": "4 \u2014 Middle Left", "5 \u2014 Middle Center": "5 \u2014 Middle Center", "6 \u2014 Middle Right": "6 \u2014 Middle Right", "7 \u2014 Top Left": "7 \u2014 Top Left", "8 \u2014 Top Center": "8 \u2014 Top Center", "9 \u2014 Top Right": "9 \u2014 Top Right", "10 \u2014 Centroid": "10 \u2014 Centroid", "11 \u2014 Shear Center": "11 \u2014 Shear Center" } }), yt.addFolder({ title: "\u{1F9ED} Local Axes", expanded: false }).addBinding(fe, "beta", { label: "\u03B2 (\xB0)", min: -180, max: 180, step: 5 });
    ...
    if ($) {
      const N = yt.addFolder({ title: "\u2139 Selecci\xF3n" }), P = { msg: "Seleccion\xE1 nodos, frames o \xE1reas para editar" };
      N.addBinding(P, "msg", { readonly: true, label: "Qu\xE9 hacer" });
    }
  ...
  a.addEventListener("pointerdown", (u) => {
    ...
    if (u.button === 2) {
      ...
      if (g) {
        if (ts ? on() : window.dispatchEvent(new KeyboardEvent("keydown", { key: "Escape", bubbles: true })), Ue.size > 0 && (Ue.clear(), cs()), r.polylines) {
    ...
    var _a3, _b, _c2;
    const g = ((_c2 = (_b = (_a3 = window.__hekatanCadState) == null ? void 0 : _a3.get) == null ? void 0 : _b.call(_a3)) == null ? void 0 : _c2.tool) ?? "select";
    g !== "select" && g !== "none" && g || u.button === 0 && (window.__hekatanBloquearVentana || u.pointerType !== "touch" && (hs = null, $i = false));
  }), a.addEventListener("pointermove", (u) => {
    if (ts && u.buttons === 0) {
    ...
    if (!hs) return;
    const g = u.clientX - hs.x, y = u.clientY - hs.y, m = Math.hypot(g, y);
    if (!$i && m < 8) return;
    $i = true;
    const v = u.clientX < hs.x;
    Vn(hs.x, hs.y, u.clientX, u.clientY, v);
  }), a.addEventListener("pointerup", (u) => {
    if (!hs) return;
    if (!$i) {
      hs = null;
    ...
    Vo(hs.x, hs.y, u.clientX, u.clientY, g), hs = null, $i = false;
  ...
  const os = new ft();
  os.visible = false, os.frustumCulled = false, t.add(os);
  const Ho = { end: 16724804, mid: 16498468, node: 6333946, cen: 3462041, per: 12616956, nea: 16744118, int: 16746496, ori: 16777215, grid: 2282478, track: 16761856, ifc: 16096779, ifcAxis: 16639626, ifcSec: 16486972, ifcEdge: 16498468, ifcVert: 16724804 }, ln = (u, g, y, m) => {
    var _a3, _b, _c2, _d;
    for (; os.children.length; ) {
      const k = os.children.pop();
      (_b = (_a3 = k.geometry) == null ? void 0 : _a3.dispose) == null ? void 0 : _b.call(_a3), (_d = (_c2 = k.material) == null ? void 0 : _c2.dispose) == null ? void 0 : _d.call(_c2);
    }
    const v = Ho[u] ?? 16777215, E = new ze().setFromPoints([new te(-1, -1, 0), new te(1, -1, 0), new te(1, -1, 0), new te(1, 1, 0), new te(1, 1, 0), new te(-1, 1, 0), new te(-1, 1, 0), new te(-1, -1, 0)]);
    os.add(new is(E, new mt({ color: v, linewidth: 2 }))), os.position.set(g, y, m), os.visible = true, Xn();
  };
  let Hn = 4;
  const Xn = () => {
    os.visible && os.scale.setScalar(Hn * nn(os.position));
  };
  window.__hekatanOsnapMarkerRef = os, window.__hekatanUpdateOsnapScale = Xn, window.__hekatanOsnapPx = (u) => (typeof u == "number" && u > 0 && (Hn = u, Xn(), l()), Hn);
  const rn = () => {
    os.visible = false;
  ...
  const hl = (u, g, y, m, v) => {
    ...
    const P = window.__hekatanAxisGrids ?? [], L = window.__hekatanLevels ?? [], Y = P.filter((z) => z && z.start && z.end).map((z) => [z.start, z.end]);
    for (const [z, V] of Y) {
  ...
  window.__hekatanPushUndo = Et, window.__hekatanUndo = hn, window.__hekatanRedo = jo, document.addEventListener("keydown", (u) => {
    var _a3;
    const g = u.key.toLowerCase();
  ...
  const qn = () => {
    qe = [], Ut = null, Yn(), zt = null, $o(), Re.visible = false, Zt.visible = false, ot(), we("\u23F9 Dibujo finalizado \u2014 click para empezar otra serie"), l(), ss();
  };
  ...
  const Go = (u, g) => {
    ...
    qe = [], Re.visible = false;
    let E = 0;
    u === "move" ? E = Ko(v[0], v[1], v[2]) : (E = Zo().size, (_c2 = window.__hekatanReplicateSelection) == null ? void 0 : _c2.call(window, v[0], v[1], v[2], 1)), we(`\u2713 ${u === "move" ? "Movidos" : "Copiados"} ${E} nudo${E === 1 ? "" : "s"} \u2014 \u0394 (${v[0].toFixed(2)}, ${v[1].toFixed(2)}, ${v[2].toFixed(2)}) m.`), u === "move" && (Ue.clear(), cs()), (_e2 = (_d = window.__hekatanCadState) == null ? void 0 : _d.setTool) == null ? void 0 : _e2.call(_d, "select"), ss();
  };
  ...
  const gl = () => {
    ...
    const k = [g[0] - u[0], g[1] - u[1], g[2] - u[2]], M = [m[0] - y[0], m[1] - y[1], m[2] - y[2]], $ = [u[0] - y[0], u[1] - y[1], u[2] - y[2]], I = k[0] * k[0] + k[1] * k[1] + k[2] * k[2], T = k[0] * M[0] + k[1] * M[1] + k[2] * M[2], N = M[0] * M[0] + M[1] * M[1] + M[2] * M[2], P = k[0] * $[0] + k[1] * $[1] + k[2] * $[2], L = M[0] * $[0] + M[1] * $[1] + M[2] * $[2], Y = I * N - T * T;
    if (Y < 1e-12) return null;
    const j = (T * L - N * P) / Y, O = (I * L - T * P) / Y;
    ...
    if (u === "offset") {
      ...
      for (let De = 0; De < O.length - 1; De++) {
        const Je = m[O[De]], Be = m[O[De + 1]], Pe = [Be[0] - Je[0], Be[1] - Je[1], Be[2] - Je[2]], Xe = Math.hypot(Pe[0], Pe[1], Pe[2]) || 1, We = Pe[0] / Xe, xt = Pe[1] / Xe, St = Pe[2] / Xe, Ot = [V[1] * St - V[2] * xt, V[2] * We - V[0] * St, V[0] * xt - V[1] * We], us = Math.hypot(Ot[0], Ot[1], Ot[2]) || 1;
        H.push({ a: Je, b: Be, n: [Ot[0] / us, Ot[1] / us, Ot[2] / us] });
      }
      let ee = 0, Z = 1 / 0;
      H.forEach((De, Je) => {
    ...
    const M = y[Ut.poly], $ = m[M[Ut.seg]], I = m[M[Ut.seg + 1]], T = y[E], N = T[k], P = T[k + 1];
    if (!$ || !I || N == null || P == null) {
    ...
    if (u === "trim") {
      ...
      zo(E, V ? k : k + 1), we(`\u2713 Recortado en (${j[0].toFixed(2)}, ${j[1].toFixed(2)}, ${j[2].toFixed(2)}). Designe otro trozo o Esc.`);
    } else {
  ...
  window.__hekatanSelectionSize = () => Ue.size, window.__hekatanSelectLast = () => {
    ...
    return Ue.clear(), g >= 0 && Ue.add(`poly:${g}`), cs(), we(g >= 0 ? "SELECCI\xD3N 1 objeto (el \xFAltimo dibujado) \xB7 Esc suelta" : "No hay ning\xFAn objeto dibujado todav\xEDa."), Ue.size;
    ...
    return u.forEach((m, v) => {
      ...
      y.has(v) || Ue.add(`pt:${v}`);
    }), cs(), we(`SELECCI\xD3N ${Ue.size} objetos (todo el modelo) \xB7 Esc suelta`), Ue.size;
    ...
    for (let O = 1; O <= m; O++) {
      ...
      j.forEach((J) => {
        Z.set(J, P.length), P.push([k[J][0] + V, k[J][1] + H, k[J][2] + ee]);
      }), T.forEach((J) => {
    ...
    $.forEach((V) => {
      ...
      for (let ee = 1; ee <= m; ee++) {
        ...
        L.push(P.length), P.push([...Z, Z[0]]), z++;
      }
    }), P.push([]), r.points.val = N, r.polylines && (r.polylines.val = P), r.areas && (r.areas.val = L);
    try {
      (_d = window.__hekatanRebuild) == null ? void 0 : _d.call(window);
    } catch {
    }
    ...
    for (const [j, O] of $) {
      ...
      for (const me of de) {
        ...
        const je = N.length;
        N.push([V[0] + ke, V[1] + Ae, V[2]]), P.push([j, nt]), P.push([O, je]), v && P.push([nt, je]), m && (L.push(P.length), P.push([j, O, je, nt, j])), Y++;
      }
    }
    if (!Y) return 0;
    P.push([]), r.points.val = N, r.polylines && (r.polylines.val = P), r.areas && (r.areas.val = L);
    ...
    (u.ctrlKey || u.metaKey) && (v = new te(Math.round(m[0].point.x), Math.round(m[0].point.y), Math.round(m[0].point.z)));
    {
      ...
      if (k.length > 0) {
        ...
        if ($) {
          ...
          let T = zt;
          if (!T && I) {
            const N = Math.abs(v.x - $[0]), P = Math.abs(v.y - $[1]), L = Math.abs(v.z - $[2]);
            T = N >= P && N >= L ? "x" : P >= L ? "y" : "z";
          }
  ...
  const Jo = (u, g) => {
    ...
    if (y === "select" || y === "none" || !y) {
      if (bs) {
        ...
        !!g && (g.ctrlKey || g.metaKey || g.shiftKey) || Ue.clear(), Ue.has(I) ? Ue.delete(I) : Ue.add(I), cs(), we(`\u2713 Seleccionados ${Ue.size} elemento(s) \u2014 Ctrl+Click para multi-selecci\xF3n`);
      } else {
    ...
    if (y === "circle") {
      ...
      const [k, M] = qe, $ = Math.hypot(M[0] - k[0], M[1] - k[1], M[2] - k[2]), I = Math.abs(M[0] - k[0]), T = Math.abs(M[1] - k[1]), N = Math.abs(M[2] - k[2]), P = String(((_j = (_i2 = (_h = window.__hekatanCadState) == null ? void 0 : _h.get) == null ? void 0 : _i2.call(_h)) == null ? void 0 : _j.workPlane) ?? ""), Y = (P === "xy" ? N < 1e-3 : P === "xz" ? T < 1e-3 : P === "yz" ? I < 1e-3 : false) ? P : N < 1e-3 ? "xy" : T < 1e-3 ? "xz" : "yz", j = window.__hekatanArcSegs ?? 12;
    ...
    if (y === "ifcface") {
      ...
      const P = I ?? 0.2, L = T === "exterior" ? 0 : T === "interior" ? P : P / 2, Y = M.map((H) => H.clone().addScaledVector($, -L));
      ...
      we(`\u25A6 \xC1rea desde la cara del IFC: ${M.length} v\xE9rtices, ${j} shell(s). Espesor medido ${I ? I.toFixed(2) + " m" : "no medido (0.20 m supuesto)"}; malla en ${V}; formulaci\xF3n ${z}, t = ${P.toFixed(2)} m.`), pe(null, -1, null);
      try {
    ...
    if (y === "ifcline") {
      ...
      for (const N of k) {
        let P = M.findIndex((L) => Math.abs(L[0] - N[0]) < 1e-3 && Math.abs(L[1] - N[1]) < 1e-3 && Math.abs(L[2] - N[2]) < 1e-3);
        P < 0 && (P = M.length + I.length, I.push(N)), $.push(P);
      }
      if (r.points.val = [...M, ...I], r.polylines) {
        const N = r.polylines.rawVal, P = N.length && N[N.length - 1].length === 0 ? N.slice(0, -1) : N;
        r.polylines.val = [...P, $, []];
      }
      const T = F.reduce((N, P, L) => L ? N + P.distanceTo(F[L - 1]) : 0, 0);
    ...
    if (y === "parabola" || y === "cubica") {
      ...
      const T = "xyz"[I.ia ?? 0], N = "xyz"[I.io ?? 2], P = (I.coef ?? []).map((L, Y) => `${L >= 0 && Y ? "+" : ""}${L.toFixed(3)}${Y ? "\xB7" + T + (Y > 1 ? "^" + Y : "") : ""}`).join(" ");
      we(`\u2713 ${M} dibujada en ${String(I.plano ?? "").toUpperCase()} \u2014 ${$} tramos a \u0394 igual de ${T} \xB7 ${N} = ${P}`), qe = [];
      try {
    ...
    if (y === "fillarea") {
      ...
      for (const oe of L) for (const ie of $.get(oe)) if (!(ie < oe)) {
        for (const de of $.get(ie)) if (de !== oe) for (const me of $.get(de)) {
          ...
          N.has(ke) || (N.add(ke), P.push([oe, ie, de, me]));
        }
      }
      for (const oe of L) for (const ie of $.get(oe)) if (!(ie < oe)) for (const de of $.get(ie)) {
        ...
        N.has(me) || (N.add(me), P.push([oe, ie, de]));
      }
    ...
    if (y === "extl") {
      ...
      const $ = r.polylines.rawVal, I = r.points.rawVal, T = $[M.polyIdx], N = I[T[M.segIdx]], P = I[T[M.segIdx + 1]];
      if (!N || !P) {
      ...
      r.points.val = [...r.points.rawVal, [N[0], N[1], N[2]], [P[0], P[1], P[2]], [P[0], P[1], P[2] + L], [N[0], N[1], N[2] + L]];
    ...
    if (y === "aux") {
      ...
      const I = M[0] - k[0], T = M[1] - k[1], N = M[2] - k[2], P = Math.sqrt(I * I + T * T + N * N);
      we(`\u2713 L\xEDnea auxiliar creada \u2014 L=${P.toFixed(2)}m (cyan, no FEM)`), qe = [];
      return;
    }
      ...
      (u.ctrlKey || u.metaKey) && m.set(Math.round(m.x), Math.round(m.y), Math.round(m.z));
      {
        ...
        if (M.length > 0) {
          ...
          if (I) {
            ...
            let N = zt;
            if (!N && T) {
              const P = Math.abs(m.x - I[0]), L = Math.abs(m.y - I[1]), Y = Math.abs(m.z - I[2]);
              N = P >= L && P >= Y ? "x" : L >= Y ? "y" : "z";
            }
...
function kr(r, e, t, s) {
  ...
  return xe.derive(() => {
    ...
    const X = Math.max(A[0] - S[0], A[1] - S[1], A[2] - S[2], 1) * br, q = [], G = [], W = [];
    let U = 0;
...
const Er = 16755200, ba = 56831, Ar = 56831, Tr = 56831, wn = 65382;
function zr(r) {
  const e = new ft();
  e.name = "__hekatan_hover", e.renderOrder = 99;
  const t = new gi(1, 16, 16), s = new gt({ color: Er, transparent: true, opacity: 0.85, depthTest: false }), i = new pt(t, s);
  ...
  const h = new ze(), p = new gt({ color: Ar, transparent: true, opacity: 0.45, side: Pt, depthTest: false }), f = new pt(h, p);
  ...
  C.visible = false, C.renderOrder = 100, e.add(C);
  const _ = new gt({ color: wn, transparent: true, opacity: 0.95, depthTest: false }), S = new gt({ color: wn, transparent: true, opacity: 0.85, depthTest: false }), A = new da(1, 1, 1, 12), F = new gt({ color: wn, transparent: true, opacity: 0.55, side: Pt, depthTest: false }), X = new mt({ color: wn, linewidth: 4, transparent: true, opacity: 1, depthTest: false }), q = [];
    ...
    for (let at = 0; at < Me.length; at++) {
      ...
      if (!(!Qe || Qe.length < 2)) {
        if (Qe.length === 2) {
          ...
          const Re = Ir(ye, Le, Fe.x, Fe.y, st.x, st.y);
...
const Pr = { normals: "Axial", torsions: "Torsi\xF3n", shearsY: "Cortante 2-2", shearsZ: "Cortante 3-3", bendingsY: "Momento 2-2", bendingsZ: "Momento 3-3" }, Rr = { normals: "kN", torsions: "kN\xB7m", shearsY: "kN", shearsZ: "kN", bendingsY: "kN\xB7m", bendingsZ: "kN\xB7m" }, Ls = 1e-3;
function Ii(r, e) {
...
function Lr(r, e) {
  ...
  const i = () => {
    ...
    var _a3, _b2;
    const F = ((_a3 = r.nodes) == null ? void 0 : _a3.rawVal) ?? [], X = ((_b2 = r.elements) == null ? void 0 : _b2.rawVal) ?? [], q = /* @__PURE__ */ new Set();
    for (const G of X) {
      if (G.length !== 2) continue;
      const W = F[G[0]], U = F[G[1]];
      if (!W || !U) continue;
  ...
  function a() {
    ...
    const A = t.querySelector(".hk-d2-plano"), F = t.querySelector(".hk-d2-en");
    A.addEventListener("change", () => {
      s = { plano: A.value, en: n(A.value)[0] ?? 0 }, l();
    }), F.addEventListener("change", () => {
      s.en = Number(F.value), l();
    });
    const X = (W) => {
  ...
  function l() {
    ...
    const A = new Set(p && !p.hidden && f >= 0 ? b(f) : []), F = t.querySelector(".hk-d2-svg"), X = t.querySelector(".hk-d2-tit"), q = t.querySelector(".hk-d2-pie"), G = t.querySelector(".hk-d2-plano"), W = t.querySelector(".hk-d2-en");
    G.value = s.plano;
    ...
    F.innerHTML = "";
    const pe = F.clientWidth || 880, be = F.clientHeight || 480, le = [];
    if (Q.forEach(($e, Ve) => {
    ...
    const Se = ae - ne || 1, ve = he - ue || 1, Oe = 0.12 * Math.max(Se, ve), He = 46, dt = Math.min((pe - 2 * He) / (Se + 2 * Oe), (be - 2 * He) / (ve + 2 * Oe)), wt = (pe - Se * dt) / 2, Ce = (be - ve * dt) / 2, ce = ($e) => wt + ($e - ne) * dt, _e = ($e) => be - (Ce + ($e - ue) * dt), ge = "http://www.w3.org/2000/svg", ye = ($e, Ve, Rt) => {
      ...
      return Rt != null && (ot.textContent = Rt), F.appendChild(ot), ot;
    }, Le = /* @__PURE__ */ new Map();
    for (const $e of le) {
      const Ve = ((_h = (_g = (_f = (_e2 = r.elementInputs) == null ? void 0 : _e2.rawVal) == null ? void 0 : _f.localAngles) == null ? void 0 : _g.get) == null ? void 0 : _h.call(_g, $e.i)) ?? 0, Rt = Ii(Xa(R ?? "normals", Ha(B[Q[$e.i][0]], B[Q[$e.i][1]], Ve)), s.plano), ot = Math.hypot(Rt.u, Rt.v);
    ...
    const Ze = R ? Pr[R] ?? R : "sin resultado";
    X.textContent = `${Ze} \xB7 ${s.plano === "XY" ? "planta" : "alzado"} ${s.plano} en ${K} = ${s.en.toFixed(2)} m`, q.textContent = R ? `${le.length} barras en el plano \xB7 m\xE1ximo ${Ee(Me)} ${Rr[R] ?? ""}` + (Ge ? " \xB7 el momento va del lado de la tracci\xF3n" : "") + (Te ? ` \xB7 ${Te} barra(s) con este resultado en otro plano (sin dibujar)` : "") + " \xB7 clic en una barra: su gr\xE1fico" : "Eleg\xED un resultado en \xABFrame results\xBB (Axial, Cortante, Momento) para ver su diagrama aqu\xED.";
  }
  const c = () => {
    try {
      l();
  ...
  setInterval(() => {
    var _a3, _b2;
    const A = (_a3 = r.analyzeOutputs) == null ? void 0 : _a3.rawVal, F = (_b2 = e == null ? void 0 : e.frameResults) == null ? void 0 : _b2.rawVal, X = [A, F];
    if (!(h && h[0] === A && h[1] === F)) {
      h = X, c();
      try {
        S();
  ...
  function b(A) {
    var _a3, _b2;
    const F = ((_a3 = r.nodes) == null ? void 0 : _a3.rawVal) ?? [], X = ((_b2 = r.elements) == null ? void 0 : _b2.rawVal) ?? [], q = /* @__PURE__ */ new Map();
    X.forEach((K, D) => {
      if (K.length === 2) for (const R of K) q.has(R) || q.set(R, []), q.get(R).push(D);
    });
    const G = (K) => {
      const D = F[X[K][0]], R = F[X[K][1]], B = [R[0] - D[0], R[1] - D[1], R[2] - D[2]], Q = Math.hypot(B[0], B[1], B[2]) || 1;
  ...
  function C(A) {
    ...
    f = A, p || (p = document.createElement("div"), p.id = "hk-diagrama-barra", p.style.cssText = ["position:fixed", "right:24px", "top:90px", "width:min(620px,92vw)", "z-index:9991", "background:#0b0e14", "border:1px solid #2f3b50", "border-radius:8px", "box-shadow:0 12px 40px rgba(0,0,0,.6)", "font:12px 'Segoe UI',system-ui,sans-serif", "color:#c9d3e0"].join(";"), p.innerHTML = '<div style="display:flex;align-items:center;gap:10px;padding:7px 10px;background:#141a24;border-bottom:1px solid #2f3b50"><b style="color:#e6c463;white-space:nowrap">\u{1F4C8} Barra</b><span class="hk-b-tit" style="color:#9fb0c6;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;min-width:0;flex:1 1 auto"></span><select class="hk-b-pl" style="margin-left:auto;background:#1b2230;color:#dbe6f5;border:1px solid #33415c;border-radius:4px"><option value="12">plano 1-2 (V2 \xB7 M3)</option><option value="13">plano 1-3 (V3 \xB7 M2)</option></select><button class="hk-b-x" style="background:#7a2d2d;color:#fff;border:1px solid #b04545;border-radius:4px;cursor:pointer;padding:2px 9px">\u2715</button></div><div class="hk-b-cuerpo" style="padding:6px 10px 10px"></div>', document.body.appendChild(p), p.querySelector(".hk-b-x").addEventListener("click", () => {
      p.hidden = true, _(), l();
    }), p.querySelector(".hk-b-pl").addEventListener("change", (F) => {
      x = F.target.value, S();
    })), p.hidden = false, _(), S(), l();
  }
  function _() {
    if (!t || !p) return;
    const A = window.innerWidth, F = Math.min(560, Math.round(A * 0.4));
    p.style.width = F + "px", !p.hidden && !t.hidden ? (t.style.transform = "none", t.style.left = "12px", t.style.width = A - F - 36 + "px", p.style.top = t.getBoundingClientRect().top + "px") : t.hidden || (t.style.left = "50%", t.style.transform = "translateX(-50%)", t.style.width = "min(900px,92vw)");
  }
  function S() {
    var _a3, _b2, _c2;
    if (!p || p.hidden || f < 0) return;
    const A = ((_a3 = r.nodes) == null ? void 0 : _a3.rawVal) ?? [], F = ((_b2 = r.elements) == null ? void 0 : _b2.rawVal) ?? [], X = ((_c2 = r.analyzeOutputs) == null ? void 0 : _c2.rawVal) ?? {};
    if (!F[f]) return;
    const q = b(f), G = [];
    let W = 0, U = -1;
    q.forEach((ae, ue) => {
      const [he, Se] = F[ae], ve = ue === 0 ? q.length > 1 && F[q[1]].includes(he) : he !== U, Oe = ve ? Se : he, He = ve ? he : Se, dt = Math.hypot(A[He][0] - A[Oe][0], A[He][1] - A[Oe][1], A[He][2] - A[Oe][2]);
...
function Mc({ mesh: r, settingsObj: e, drawingObj: t, objects3D: s, solids: i }) {
  ...
  const n = document.createElement("div"), o = new Rl(), a = new Dl(45, 1, 0.1, 2 * 1e6), l = new Ll(-10, 10, 10, -10, -1e3, 2e6);
  let c = a;
  ...
  const _ = ql(e), S = xe.derive(() => Math.pow(10, _.displayScale.val / 10)), A = Or(r, _), F = () => {
    const ce = [];
    ...
    return { planes: F(), majorStep: _e, minorStep: ce };
  };
  ...
  if (r) {
    o.add(jl(_, A, S), Gl(r, _, A), tr(_, A, S), sr(r, _, A, S), Ql(r, _, A, S), er(r, _, A, S), or(r, _, A, S), lr(r, _, A, S), hr(r, _, A), mr(r, _, A, S), ur(r, _, A, S)), window.__hekatanDiagrama2D || (Lr(r, _), h.domElement.addEventListener("dblclick", () => {
      var _a2;
    ...
    const ce = zr({ scene: o, rendererElm: h.domElement, getActiveCamera: () => c, derivedNodes: A, derivedDisplayScale: S, mesh: r, settings: _, render: ae });
    o.add(ce);
    const _e = Ur(r, _), ge = yr(r, _, A, _e), ye = va(_e);
  ...
  t && gr({ drawingObj: t, gridObj: q, scene: o, getActiveCamera: () => c, controls: p, gridSize: U, derivedDisplayScale: S, rendererElm: h.domElement, viewerRender: ae }), Vi((ce, _e) => {
    ...
    }), q = no(_.gridSize.rawVal, { planes: F() }), o.add(q), n.style.setProperty("--hk-legend-color", _e.legendMarker), ae();
  });
...
function Or(r, e) {
  return xe.derive(() => {
    ...
    if (!s || t.length === 0 || !qa(r, (_e = r == null ? void 0 : r.deformOutputs) == null ? void 0 : _e.val, "la deformada") || !Ri(s, t.length, "la deformada")) return t;
  ...
  message(e, t) {
    ...
    return t.tmp || (t.tmp = { zIndex: x.css("z-index"), overflow: c.overflow }), t.html === "" && t.body === "" && t.buttons === "" ? o() : (t.msgIndex = d(e.box).find(".w2ui-message").length, t.msgIndex === 0 && typeof this.lock == "function" && (d(e.box).css("overflow", "hidden"), e.owner ? e.owner.lock(e.param) : this.lock(e.box)), d(e.box).find(".w2ui-message").css("z-index", 1390), x.css("z-index", 1501), p = `
      ...
      (_b2 = b != null ? isNaN(b) ? _.find(S).filter(b).get(0) : _.find(S).get(b) : _.find("[name=hidden-first]").get(0)) == null ? void 0 : _b2.focus(), d(e.box).find(".w2ui-message").find(S + ",[name=hidden-first],[name=hidden-last]").off(".keep-focus"), d(_).find(S + ",[name=hidden-first],[name=hidden-last]").on("blur.keep-focus", function(A) {
        setTimeout(() => {
          var _a4, _b3, _c2;
          var F = document.activeElement, X = 0 < d(_).find(S).filter(F).length, q = d(F).attr("name");
          !X && F && F !== document.body && ((_a4 = d(_).find(S).get(0)) == null ? void 0 : _a4.focus()), q == "hidden-last" && ((_b3 = d(_).find(S).get(0)) == null ? void 0 : _b3.focus()), q == "hidden-first" && ((_c2 = d(_).find(S).get(-1)) == null ? void 0 : _c2.focus());
        }, 1);
      });
    }, l;
  }
...
const _ut = class _ut {
  ...
  getPosition(e) {
    ...
    if (t && t.box) {
      ...
      let F, X, q, G, W = "", U = { offset: 0, class: "", style: `#${t.id} { --tip-size: ${S}px; }` }, K = { left: 0, top: 0 }, D = { posX: "", x: 0, posY: "", y: 0 };
      o.forEach((R) => {
      ...
      switch (U.class = _.arrow || "w2ui-arrow-" + a, a) {
        case "top":
          F = _.left + (_.width - (q ?? C.width)) / 2, X = _.top - (G ?? C.height) - S / 1.5 + 1;
          break;
        case "bottom":
          F = _.left + (_.width - (q ?? C.width)) / 2, X = _.top + _.height + S / 1.25 + 1;
          break;
        case "left":
          F = _.left - (q ?? C.width) - S / 1.2 - 1, X = _.top + (_.height - (G ?? C.height)) / 2;
          break;
        case "right":
          F = _.left + _.width + S / 1.2 + 1, X = _.top + (_.height - (G ?? C.height)) / 2;
      }
      b && (p.align == "left" && (K.left = _.left - F, F = _.left), p.align == "right" && (K.left = _.left + _.width - (q ?? C.width) - F, F = _.left + _.width - (q ?? C.width)), ["top", "bottom"].includes(W) && p.align.startsWith("both") && (n = p.align.split(":")[1] ?? 50, _.width >= n) && (F = _.left, q = _.width), p.align == "top" && (K.top = _.top - X, X = _.top), p.align == "bottom" && (K.top = _.top + _.height - (G ?? C.height) - X, X = _.top + _.height - (G ?? C.height)), ["left", "right"].includes(W) && p.align.startsWith("both") && (n = p.align.split(":")[1] ?? 50, _.height >= n) && (X = _.top, G = _.height));
      {
        let R;
        ...
        (["top", "bottom"].includes(W) || p.autoResize) && (F < l && (R = true, K.left -= F, F = l), F > h) && (R = true, K.left -= F - h, F += h - F), (["left", "right"].includes(W) || p.autoResize) && (X < c && (R = true, K.top -= X, X = c), X > s) && (R = true, K.top -= X - s, X += s - X), R && (l = b ? "left" : "top", h = b ? "width" : "height", U.offset = -K[l], c = C[h] / 2 - S, Math.abs(U.offset) > c + S && (U.class = ""), Math.abs(U.offset) > c && (U.offset = U.offset < 0 ? -c : c), U.style = w.stripSpaces(`#${t.id} .w2ui-overlay-body:after,
                            #${t.id} .w2ui-overlay-body:before {
                                --tip-size: ${S}px;
                                margin-${l}: ${U.offset}px;
                            }`));
      }
      return i = W == "top" ? -p.margin : W == "bottom" ? p.margin : 0, e = W == "left" ? -p.margin : W == "right" ? p.margin : 0, X = Math.floor(100 * (X + parseFloat(p.offsetY) + parseFloat(i))) / 100, { left: F = Math.floor(100 * (F + parseFloat(p.offsetX) + parseFloat(e))) / 100, top: X, arrow: U, adjust: K, width: q, height: G, pos: W };
    }
  }
};
...
class Wr extends ut {
  ...
  initControls(e) {
    ...
    function f(S) {
      var F = t.el, _ = S.pageX - t.x, S = S.pageY - t.y;
      let b = t.left + _, C = t.top + S;
      var _ = parseInt(F.prop("clientWidth")) / 2, S = (b < -_ && (b = -_), C < -_ && (C = -_), b > t.width - _ && (b = t.width - _), C > t.height - _ && (C = t.height - _), F.hasClass("move-x") && F.css({ left: b + "px" }), F.hasClass("move-y") && F.css({ top: C + "px" }), d(F.get(0).parentNode).attr("name")), A = parseInt(F.css("left")) + _, F = parseInt(F.css("top")) + _;
      S === "palette" && c({ s: Math.round(A / t.width * 100), v: Math.round(100 - F / t.height * 100) }), S === "rainbow" && (c({ h: Math.round(2.4 * A) }), h()), S === "alpha" && c({ a: parseFloat(Number(A / 150).toFixed(2)) });
    }
...
class Kr extends ut {
  ...
  getMonthHTML(e, t, s) {
    ...
    for (let A = 0; A < 42; A++) {
      ...
      let F = "", X = "", q, G;
      G = e.type === "datetime" ? (q = w.formatDateTime(_, e.format), w.formatDate(_, w.settings.dateFormat)) : q = w.formatDate(_, e.format), e.colored && e.colored[G] !== void 0 && (_ = e.colored[G].split("|"), X = "background-color: " + _[0] + ";", F = "color: " + _[1] + ";"), x += `<div class="w2ui-day ${this.inRange(q, e, true) ? "w2ui-date " + (G == a ? "w2ui-selected" : "") : "w2ui-blocked"} ${C.join(" ")}"
                       style="${F + X}" date="${G}" data-date="${b.getTime()}">
                            ${S}
...
class Ec extends ni {
        ...
        for (let l = this.total = 0; l < this.records.length; l++) {
          ...
          if (function c(h) {
            ...
            for (let S = 0; S < t.searchData.length; S++) {
              let A = t.searchData[S], F = t.getSearch(A.field);
              if (A != null) {
                F == null && (F = { field: A.field, type: A.type });
                let X = t.parseField(h, F.field);
                switch (f = X == null || typeof X == "object" && X.toString == o ? "" : String(X).toLowerCase(), A.value != null && (Array.isArray(A.value) ? (x = A.value[0], b = A.value[1]) : x = String(A.value).toLowerCase()), A.operator) {
                  case "=":
                  case "is":
                    t.parseField(h, F.field) == A.value ? p++ : F.type == "date" ? (C = t.parseField(h, F.field + "_") instanceof Date ? t.parseField(h, F.field + "_") : t.parseField(h, F.field), f = w.formatDate(C, "yyyy-mm-dd"), x = w.formatDate(w.isDate(x, w.settings.dateFormat, true), "yyyy-mm-dd"), f == x && p++) : F.type == "time" ? (C = t.parseField(h, F.field + "_") instanceof Date ? t.parseField(h, F.field + "_") : t.parseField(h, F.field), f = w.formatTime(C, "hh24:mi"), x = w.formatTime(x, "hh24:mi"), f == x && p++) : F.type == "datetime" && (C = t.parseField(h, F.field + "_") instanceof Date ? t.parseField(h, F.field + "_") : t.parseField(h, F.field), f = w.formatDateTime(C, "yyyy-mm-dd|hh24:mm:ss"), x = w.formatDateTime(w.isDateTime(x, w.settings.datetimeFormat, true), "yyyy-mm-dd|hh24:mm:ss"), f == x) && p++;
                    break;
                  case "between":
                    ["int", "float", "money", "currency", "percent"].indexOf(F.type) != -1 ? parseFloat(t.parseField(h, F.field)) >= parseFloat(x) && parseFloat(t.parseField(h, F.field)) <= parseFloat(b) && p++ : F.type == "date" ? (C = t.parseField(h, F.field + "_") instanceof Date ? t.parseField(h, F.field + "_") : t.parseField(h, F.field), f = w.isDate(C, w.settings.dateFormat, true), x = w.isDate(x, w.settings.dateFormat, true), (b = w.isDate(b, w.settings.dateFormat, true)) != null && (b = new Date(b.getTime() + 864e5)), f >= x && f < b && p++) : F.type == "time" ? (f = t.parseField(h, F.field + "_") instanceof Date ? t.parseField(h, F.field + "_") : t.parseField(h, F.field), x = w.isTime(x, true), b = w.isTime(b, true), x = (/* @__PURE__ */ new Date()).setHours(x.hours, x.minutes, x.seconds || 0, 0), b = (/* @__PURE__ */ new Date()).setHours(b.hours, b.minutes, b.seconds || 0, 0), f >= x && f < b && p++) : F.type == "datetime" && (f = t.parseField(h, F.field + "_") instanceof Date ? t.parseField(h, F.field + "_") : t.parseField(h, F.field), x = w.isDateTime(x, w.settings.datetimeFormat, true), b = (b = w.isDateTime(b, w.settings.datetimeFormat, true)) && new Date(b.getTime() + 864e5), f >= x) && f < b && p++;
                    break;
                  case "<=":
                    _ = true;
                  case "<":
                  case "less":
                    ["int", "float", "money", "currency", "percent"].indexOf(F.type) != -1 ? (f = parseFloat(t.parseField(h, F.field)), x = parseFloat(A.value), (f < x || _ && f === x) && p++) : F.type == "date" ? (C = t.parseField(h, F.field + "_") instanceof Date ? t.parseField(h, F.field + "_") : t.parseField(h, F.field), f = w.isDate(C, w.settings.dateFormat, true), x = w.isDate(x, w.settings.dateFormat, true), (f < x || _ && f === x) && p++) : F.type == "time" ? (C = t.parseField(h, F.field + "_") instanceof Date ? t.parseField(h, F.field + "_") : t.parseField(h, F.field), f = w.formatTime(C, "hh24:mi"), x = w.formatTime(x, "hh24:mi"), (f < x || _ && f === x) && p++) : F.type == "datetime" && (C = t.parseField(h, F.field + "_") instanceof Date ? t.parseField(h, F.field + "_") : t.parseField(h, F.field), f = w.formatDateTime(C, "yyyy-mm-dd|hh24:mm:ss"), x = w.formatDateTime(w.isDateTime(x, w.settings.datetimeFormat, true), "yyyy-mm-dd|hh24:mm:ss"), f.length == x.length) && (f < x || _ && f === x) && p++;
                    break;
                  case ">=":
                    _ = true;
                  case ">":
    ...
    if (this.ranges.length !== 0) {
      let C = function(S) {
        ...
        if (A && A.type == "expand") {
          ...
          let F, X, q = S.target;
          q.tagName.toUpperCase() != "TD" && (q = d(q).closest("td")[0]), (X = d(q).attr("col") != null ? parseInt(d(q).attr("col")) : X) != null && (q = d(q).closest("tr")[0], F = f.records[d(q).attr("index")].recid, A.newRange[1].recid != F || A.newRange[1].column != X) && (S = w.clone(A.newRange), A.newRange = [{ recid: A.recid, column: A.column }, { recid: F, column: X }], b.detail && (b.detail.newRange = w.clone(A.newRange), b.detail.originalRange = w.clone(A.originalRange)), (b = f.trigger("selectionExtend", b)).isCancelled === true ? (A.newRange = S, b.detail.newRange = S) : (f.removeRange("grid-selection-expand"), f.addRange({ name: "grid-selection-expand", range: A.newRange, style: "background-color: rgba(100,100,100,0.1); border: 2px dotted rgba(100,100,100,0.5);" })));
        }
      }, _ = function(S) {
      ...
      for (let S = 0; S < this.ranges.length; S++) {
        ...
        let A = d(this.box).find("#grid_" + this.name + "_rec_" + w.escapeId(n.recid) + ' td[col="' + n.column + '"]'), F = d(this.box).find("#grid_" + this.name + "_rec_" + w.escapeId(o.recid) + ' td[col="' + o.column + '"]'), X = d(this.box).find("#grid_" + this.name + "_frec_" + w.escapeId(n.recid) + ' td[col="' + n.column + '"]'), q = d(this.box).find("#grid_" + this.name + "_frec_" + w.escapeId(o.recid) + ' td[col="' + o.column + '"]'), G = o.column;
        n.column < this.last.colStart && o.column > this.last.colStart && (A = d(this.box).find("#grid_" + this.name + "_rec_" + w.escapeId(n.recid) + ' td[col="start"]')), n.column < this.last.colEnd && o.column > this.last.colEnd && (F = d(this.box).find("#grid_" + this.name + "_rec_" + w.escapeId(o.recid) + ' td[col="end"]'), G = '"end"');
  ...
  select() {
    ...
    if (this.selectType == "row") for (let S = 0; S < s.length; S++) {
      ...
      if (o != null) {
        let A = null, F = null;
        (this.searchData.length !== 0 || o + 1 >= this.last.range_start && o + 1 <= this.last.range_end) && (A = d(this.box).find("#grid_" + this.name + "_frec_" + w.escapeId(n)), F = d(this.box).find("#grid_" + this.name + "_rec_" + w.escapeId(n))), this.selectType == "row" && t.indexes.indexOf(o) == -1 && (t.indexes.push(o), A && F && (A.addClass("w2ui-selected").find(".w2ui-col-number").addClass("w2ui-row-selected"), F.addClass("w2ui-selected").find(".w2ui-col-number").addClass("w2ui-row-selected"), A.find(".w2ui-grid-select-check").prop("checked", true)), e++);
      }
    }
    else {
      var a = {};
      ...
      for (h in a) {
        ...
        if (f != null) {
          ...
          for (let F = 0; F < a[h].length; F++) x.indexOf(a[h][F]) == -1 && x.push(a[h][F]);
          x.sort((F, X) => F - X);
      ...
      if ((S.type == "text" || S.type == "alphanumeric" && w.isAlphaNumeric(t) || S.type == "int" && w.isInt(t) || S.type == "float" && w.isFloat(t) || S.type == "percent" && w.isFloat(t) || (S.type == "hex" || S.type == "color") && w.isHex(t) || S.type == "currency" && w.isMoney(t) || S.type == "money" && w.isMoney(t) || S.type == "date" && w.isDate(t) || S.type == "time" && w.isTime(t) || S.type == "datetime" && w.isDateTime(t) || S.type == "datetime" && w.isDate(t) || S.type == "enum" && w.isAlphaNumeric(t) || S.type == "list" && w.isAlphaNumeric(t)) && (_ = this.defaultOperator[this.operatorsMap[S.type]], _ = { field: S.field, type: S.type, operator: S.operator != null ? S.operator : _, value: t }, String(t).trim() != "") && i.push(_), ["int", "float", "money", "currency", "percent"].indexOf(S.type) != -1 && String(t).trim().split("-").length == 2 && (_ = String(t).trim().split("-"), A = { field: S.field, type: S.type, operator: S.operator != null ? S.operator : "between", value: [_[0], _[1]] }, i.push(A)), ["list", "enum"].indexOf(S.type) != -1) {
        ...
        for (let R = 0; R < S.options.items; R++) {
          ...
          try {
            ...
            q.test(X) && F.push(R), X.text && q.test(X.text) && F.push(X.id);
          } catch {
          }
        }
        0 < F.length && (A = { field: S.field, type: S.type, operator: S.operator != null ? S.operator : "in", value: F }, i.push(A));
      }
    }
        ...
        if (["enum", "file"].indexOf(h.type) != -1) console.log('ERROR: input types "enum" and "file" are not supported in inline editing.');
          ...
          if (a.isCancelled !== true) {
            ...
            i = getComputedStyle(l.find('[col="' + t + '"] > div').get(0)), o = `font-family: ${i["font-family"]}; font-size: ${i["font-size"]};`, h.type === "div" ? (x.addClass("w2ui-editable").html(w.stripSpaces(`<div id="grid_${this.name}_edit_${e}_${t}" class="w2ui-input w2ui-focus"
              ...
              this.last.inEditMode && S && (S.focus(), clearTimeout(this.last.kbd_timer), (S.resize = F)(S));
            }, 50), a.finish({ input: S });
          }
        }
      }
      ...
      if (i.isCancelled !== true) if (0 < d(this.box).find(".w2ui-message").length) e.keyCode == 27 && this.message();
        ...
        let X = function(U) {
          if (l && G(), !(_.length <= 0)) {
            ...
            if ((R = F || R != null ? R : t.searchData.length == 0 || s ? 0 : t.last.searchIds[0]) != null) {
              if (F && t.multiSelect) {
                if (W()) return;
            ...
            } else F || t.selectNone(true);
          }
        }, q = function(U) {
          if (l && G(), !(_.length <= 0)) {
            let R = t.nextRow(C, t.selectType == "row" ? 0 : h[0].column, U);
            if ((R = F || R != null ? R : t.searchData.length == 0 || s ? t.records.length - 1 : t.last.searchIds[t.last.searchIds.length - 1]) != null) {
              if (F && t.multiSelect) {
                if (W()) return;
        ...
        let S = false, A = e.keyCode, F = e.shiftKey;
        switch (A) {
          case 8:
          case 46:
            ...
            (function() {
              if (l) G();
                ...
                if (t.selectType == "row") {
                  ...
                  if (B = (B == null ? void 0 : B.index) != b ? null : B == null ? void 0 : B.colIndex, F || B != null || (t.selectNone(true), B = 0), B != null) if (F && t.multiSelect) {
                    if (W()) return;
                    var K = [], D = [], R = [];
            ...
            (function() {
              if (l) G();
                ...
                if (t.selectType == "row") {
                  ...
                  if (R = R.index != b ? null : R.colIndex, F || R != null || (t.selectNone(true), R = t.columns.length - 1), R != null) if (F && A == 39 && t.multiSelect) {
                    if (W()) return;
                    var U = [], K = [], D = [];
      ...
      if (!Array.isArray(s) || !s.includes(C) && !s.includes(_.field)) {
        ...
        if (X != null && X.isConnected || (X = n.box.querySelector(`#grid_${n.name}_data_${b}_` + C), p.w2ui._update.cells[C] = X), X != null) {
          if (t) d(X).replace(n.getCellHTML(b, C, false)), X = n.box.querySelector(`#grid_${n.name}_data_${b}_` + C), p.w2ui._update.cells[C] = X;
            ...
            var S = X.children[0], { value: b, style: A, className: F } = n.getCellValue(b, C, false, true);
            if (S.innerHTML != b && (S.innerHTML = b), A != "" && X.style.cssText != A && (X.style.cssText = A), F != "") {
              let G = ["w2ui-grid-data"], W = [];
              S = F.split(" ").filter((U) => !!U), X.classList.forEach((U) => {
                  ...
                  if (p.range != x) {
                    ...
                    if (s.selectType != "row") {
                      ...
                      for (let R = 0; R < f.length; R++) {
                        let B = false;
                        for (let Q = 0; Q < F.length; Q++) f[R].recid == F[Q].recid && f[R].column == F[Q].column && (B = true);
                        B || D.push({ recid: f[R].recid, column: f[R].column });
                      }
                      s.select(D), D = [];
                      for (let R = 0; R < F.length; R++) {
                        let B = false;
                        for (let Q = 0; Q < f.length; Q++) f[Q].recid == F[R].recid && f[Q].column == F[R].column && (B = true);
                        B || D.push({ recid: F[R].recid, column: F[R].column });
                      }
                      s.unselect(D);
...
`)), C.push({ kind: f.type == "text/html" ? "html" : "text", data: _ });
  ...
  resizeRecords() {
    ...
    let S = 8 * String(this.total).length + 10, A = (S < 34 && (S = 34), this.lineNumberWidth != null && (S = this.lineNumberWidth), false), F = false, X = 0;
    ...
    ((_a2 = b[0]) == null ? void 0 : _a2.clientWidth) < X && (A = true), ((_b = p[0]) == null ? void 0 : _b.clientHeight) - (((_c2 = f[0]) == null ? void 0 : _c2.clientHeight) ?? 0) < (((_d = d(b).find(":scope > table")[0]) == null ? void 0 : _d.clientHeight) ?? 0) + (A ? w.scrollBarSize() : 0) && (F = true), this.fixedBody ? (s = ((_e = n[0]) == null ? void 0 : _e.clientHeight) - (this.show.header ? w.getSize(o, "height") : 0) - (this.show.toolbar ? w.getSize(a, "height") : 0) - (l.css("display") != "none" ? w.getSize(l, "height") : 0) - (this.show.footer ? w.getSize(h, "height") : 0), p.css("height", s + "px")) : (o = (s = w.getSize(f, "height") + w.getSize(d(this.box).find("#grid_" + this.name + "_records table"), "height") + (A ? w.scrollBarSize() : 0)) + (this.show.header ? w.getSize(o, "height") : 0) + (this.show.toolbar ? w.getSize(a, "height") : 0) + (l.css("display") != "none" ? w.getSize(l, "height") : 0) + (this.show.footer ? w.getSize(h, "height") : 0), n.css("height", o + "px"), p.css("height", s + "px"), i.css("height", w.getSize(n, "height") + "px"));
    let q = this.records.length;
    if (a = typeof this.url != "object" ? this.url : this.url.get, this.searchData.length == 0 || a || (q = this.last.searchIds.length), this.fixedBody || (F = false), A || F ? (f.find(":scope > table > tbody > tr:nth-child(1) td.w2ui-head-last").css("width", w.scrollBarSize() + "px").show(), b.css({ top: (0 < this.columnGroups.length && this.show.columns ? 1 : 0) + w.getSize(f, "height") + "px", "-webkit-overflow-scrolling": "touch", "overflow-x": A ? "auto" : "hidden", "overflow-y": F ? "auto" : "hidden" })) : (f.find(":scope > table > tbody > tr:nth-child(1) td.w2ui-head-last").hide(), b.css({ top: (0 < this.columnGroups.length && this.show.columns ? 1 : 0) + w.getSize(f, "height") + "px", overflow: "hidden" }), 0 < b.length && (this.last.scrollTop = 0, this.last.scrollLeft = 0)), A ? (C.css("margin-bottom", w.scrollBarSize() + "px"), _.show()) : (C.css("margin-bottom", 0), _.hide()), C.css({ overflow: "hidden", top: b.css("top") }), this.show.emptyRecords && !F) {
    ...
    let W, U;
    if (0 < p.length) {
      let ne = parseInt(p[0].clientWidth) - (F ? w.scrollBarSize() : 0) - (this.show.lineNumbers ? S : 0) - (this.reorderRows ? 26 : 0) - (this.show.selectColumn ? 26 : 0) - (this.show.expandColumn ? 26 : 0) - 1, ae = (W = ne, false);
...
`;
      ...
      for (let _ = 0; _ < e.columnGroups.length; _++) {
        ...
        if (!((S = _ == e.columnGroups.length - 1 ? 100 : S) <= 0)) {
          if (x.main === true) {
            ...
            let F = "";
            b.resizable !== false && (F = `<div class="w2ui-resizer" name="${f}"></div>`);
            var C = w.lang(typeof b.text == "function" ? b.text(b) : b.text);
            h = `<td id="grid_${e.name}_column_${f}" class="w2ui-head ${A}" col="${f}"     rowspan="2" colspan="${S}">` + F + `    <div class="w2ui-col-group w2ui-col-header ${A ? "w2ui-col-sorted" : ""}">        <div class="${A}"></div>` + (C || "&#160;") + "    </div></td>";
  ...
  scroll(e) {
    ...
    e && (F = e.target.scrollTop, e = e.target.scrollLeft, this.last.scrollTop = F, this.last.scrollLeft = e, p = d(this.box).find(`#grid_${this.name}_columns`)[0], f = d(this.box).find(`#grid_${this.name}_summary`)[0], p && (p.scrollLeft = e), f && (f.scrollLeft = e), n[0]) && (n[0].scrollTop = F), this.last.bubbleEl && (rt.hide(this.name + "-bubble"), this.last.bubbleEl = null);
    let o = null, a = null;
    ...
    if (o != null && (o < 0 && (o = 0), a < 0 && (a = 0), o == a && (0 < o ? o-- : a++), o != this.last.colStart || a != this.last.colEnd)) {
      ...
      if (p < 5 && f < 5) {
        ...
        this.last.colStart = o, this.last.colEnd = a;
        var e = this.getColumnsHTML(), F = this.getRecordsHTML(), p = this.getSummaryHTML(), f = h.find(`#grid_${this.name}_columns`);
        let Q = h.find(`#grid_${this.name}_records`);
        var X = h.find(`#grid_${this.name}_frecords`);
        let re = h.find(`#grid_${this.name}_summary`);
        f.find("tbody").html(e[1]), X.html(F[0]), Q.prepend(F[1]), p != null && re.html(p[1]), setTimeout(() => {
    ...
    if (e == -1) {
      ...
      for (; ; ) {
        ...
        if (f.hidden) A++, 0 < F && F--;
          ...
          a.w2ui.colspan[this.columns[A - 1].field] = 0, F--;
        } else {
  ...
  getCellHTML(e, t, s, i) {
    ...
    if (_.indexes.indexOf(e) != -1 && (S = true), i == null && (i = ((_b = a == null ? void 0 : a.w2ui) == null ? void 0 : _b.colspan) && a.w2ui.colspan[o.field] ? a.w2ui.colspan[o.field] : 1), t === 0 && Array.isArray((_c2 = a == null ? void 0 : a.w2ui) == null ? void 0 : _c2.children)) {
      ...
      for (; K != null; ) {
        U++;
        var F = this.records[K].w2ui;
        if (F == null || F.parent_recid == null) break;
        K = this.get(F.parent_recid, true);
      }
            ...
            if (this.type === "file" && (/image/i.test(f.type) && (b = `
              ...
              rt.show({ name: C, anchor: h.get(0), html: b, hideOn: ["doc-click"], class: "" }).show((_) => {
                d(`#w2overlay-${C} img`).on("load", function(S) {
                  var A = this.clientWidth, F = this.clientHeight;
                  A < 300 & F < 300 || (F <= A && 300 < A && d(this).css("width", "300px"), A < F && 300 < F && d(this).css("height", "300px"));
                }).on("error", function(S) {
...
\f\r]`, Pi = /<(?:(!--|\/[^a-zA-Z])|(\/?[a-zA-Z][^>\s]*)|(\/?$))/g, $a = /-->/g, Ea = />/g, Gs = RegExp(`>|${co}(?:([^\\s"'>=/]+)(${co}*=${co}*(?:[^ 	
\f\r"'\`<>=]|("|')|))|$)`, "g"), Aa = /'/g, Ta = /"/g, Ka = /^(?:script|style|textarea|title)$/i, ec = (r) => (e, ...t) => ({ _$litType$: r, strings: e, values: t }), Sn = ec(1), Xi = Symbol.for("lit-noChange"), qt = Symbol.for("lit-nothing"), za = /* @__PURE__ */ new WeakMap(), Qs = ii.createTreeWalker(ii, 129);
function Ga(r, e) {
...
const tc = (r, e) => {
  ...
  for (let a = 0; a < t; a++) {
    const l = r[a];
    let c, h, p = -1, f = 0;
    for (; f < l.length && (o.lastIndex = f, h = o.exec(l), h !== null); ) f = o.lastIndex, o === Pi ? h[1] === "!--" ? o = $a : h[1] !== void 0 ? o = Ea : h[2] !== void 0 ? (Ka.test(h[2]) && (i = RegExp("</" + h[2], "g")), o = Gs) : h[3] !== void 0 && (o = Gs) : o === Gs ? h[0] === ">" ? (o = i ?? Pi, p = -1) : h[1] === void 0 ? p = -2 : (p = o.lastIndex - h[2].length, c = h[1], o = h[3] === void 0 ? Gs : h[3] === '"' ? Ta : Aa) : o === Ta || o === Aa ? o = Gs : o === $a || o === Ea ? o = Pi : (o = Gs, i = void 0);
    const x = o === Gs && r[a + 1].startsWith("/>") ? " " : "";
    n += o === Pi ? l + Jr : p >= 0 ? (s.push(c), l.slice(0, p) + Wa + l.slice(p) + Ns + x) : l + Ns + (p === -2 ? a : x);
  }
...
function Ac({ buttons: r, clickedButton: e, author: t, sourceCode: s }) {
  const i = document.createElement("div");
  function n() {
    return Bl() === "dark" ? "\u2600" : "\u263E";
  }
...
const Pc = `Eres un asistente experto en estructuras y FEM que ayuda al usuario a generar modelos
...
let ns = null, si, as, Js, ei = null;
function In() {
  ns && !document.body.contains(ns) && document.body.appendChild(ns);
}
function _s(r, e) {
  In();
...
async function wo() {
  ...
  as.value = "", _s("user", r), Di.push({ role: "user", content: r }), ei = new AbortController(), Js.textContent = "\u25A0 Parar";
  ...
  try {
   ...
   \u2192 ${Ja(x.function.name, _)}`, o.has(x.function.name) && (a = true, l = false), (x.function.name === "resultados" || x.function.name === "analisis_modal") && (l = true), Di.push({ role: "tool", tool_call_id: x.id, content: JSON.stringify(_).slice(0, 6e3) });
    ...
    n.remove(), _s("error", `Par\xE9 tras ${Pa} pasos. P\xEDdeme que siga si hace falta.`);
  } catch (h) {
...
function vc() {
  ...
  ["keydown", "keyup", "keypress", "paste"].forEach((A) => l.addEventListener(A, (F) => F.stopPropagation())), l.type = "password", l.placeholder = "API key", l.style.cssText = i + "flex:1 1 100%;";
  const c = document.createElement("div");
  c.style.cssText = "flex:1 1 100%;color:#64748b;font-size:11px;";
  const h = () => {
    const A = yi.find((F) => F.id === n.value) ?? yi[0];
    a.innerHTML = A.modelos.map((F) => `<option value="${F}">`).join(""), o.value = As.getModel(`agente_${A.id}`) || A.modelos[0], l.style.display = A.clave ? "" : "none", l.value = As.getKey(A.id), c.textContent = A.pista;
  }, p = As.getProvider();
  n.value = yi.some((A) => A.id === p) ? p : "ollama", n.onchange = () => {
    As.setProvider(n.value), h();
  }, o.onchange = () => As.setModel(`agente_${n.value}`, o.value.trim()), l.onchange = () => As.setKey(n.value, l.value.trim()), s.append(n, o, a, l, c), h(), si = document.createElement("div"), si.style.cssText = "flex:1;min-height:0;overflow-y:auto;display:flex;flex-direction:column;gap:6px;padding:10px;";
  const f = document.createElement("div");
  f.style.cssText = "display:flex;gap:6px;padding:8px 10px;border-top:1px solid #1e293b;flex-shrink:0;", as = document.createElement("textarea"), as.rows = 2, as.placeholder = "Ej.: edificio de 4 pisos, 3\xD72 vanos de 5 m; dime la flecha y el periodo", as.style.cssText = i + "flex:1;resize:none;font-size:13px;", ["keydown", "keyup", "keypress"].forEach((A) => as.addEventListener(A, (F) => F.stopPropagation())), as.addEventListener("keydown", (A) => {
  ...
  b.textContent = "\u21B6 Deshacer", b.title = "Vuelve el modelo al estado anterior al \xFAltimo cambio del agente", b.style.cssText = "background:#334155;color:#e5e7eb;border:none;border-radius:4px;padding:4px 8px;cursor:pointer;font-size:12px;", b.onclick = async () => {
    ...
    _s("paso", `\u21B6 deshacer \u2192 ${Ja("deshacer", A)}`);
  }, x.append(Js, b), f.append(as, x), r.append(e, s, si, f), document.body.appendChild(r);
  let C = 0, _ = 0, S = false;
  return e.addEventListener("pointerdown", (A) => {
    if (A.target.tagName === "BUTTON") return;
    S = true;
    const F = r.getBoundingClientRect();
    C = A.clientX - F.left, _ = A.clientY - F.top, e.setPointerCapture(A.pointerId);
...
function Co() {
  const r = !!ns && ns.style.display !== "none" && document.body.contains(ns);
...
function So(r) {
  In(), ns || (ns = vc()), ns.style.display = "flex", Co(), setTimeout(() => {
    try {
      const e = ns.querySelector('input[type="password"]');
      if (e && !e.value) {
        e.focus();
        return;
      }
      as == null ? void 0 : as.focus();
    } catch {
    }
  }, 60), r && (as.value = r), as.focus();
}
function _c(r) {
  const e = () => {
...
function Rc() {
  ...
  r.id = "hk-agente-lanzador", r.textContent = "\u{1F916}", r.title = "Agente IA: p\xEDdele una estructura y la modela", r.style.cssText = ["position:fixed", "right:16px", "bottom:100px", "z-index:9600", "width:44px", "height:44px", "border-radius:50%", "border:1px solid #22d3ee", "background:#0b1220", "font-size:22px", "cursor:pointer", "box-shadow:0 4px 14px rgba(0,0,0,.4)"].join(";"), _c(r), r.onclick = () => {
    ns && ns.style.display !== "none" && document.body.contains(ns) ? (ns.style.display = "none", Co()) : So();
