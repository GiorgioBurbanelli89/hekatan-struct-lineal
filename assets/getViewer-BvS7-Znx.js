import { u as nn, a6 as Fo, q as Wa, v as ce, a7 as Ga, D as Vt, M as lt, B as Ee, F as $t, a8 as Ha, z as ft, a9 as Ja, aa as Qa, h as Qs, ab as Os, r as Tn, ac as Io, ad as Lo, a4 as ua, _ as rt, b as ct, L as Ht, y as pa, c as Oa, ae as ja, f as pt, V, $ as Ln, af as as, K as Do, d as Et, a as is, A as fa, t as Ro, J as ei, H as io, I as ti, ag as To, w as ls, o as ni, N as _n, a2 as oo, E as js, S as Wn, m as rs, ah as so, g as ea, i as ta, j as na, C as oa, W as oi, X as si, Y as ai, Z as ii, T as Eo, P as cs, U as li } from "./theme-C-zoknmI.js";
...
class ma {
  ...
  getColor(y) {
    y = Wa.clamp(y, this.minV, this.maxV), y = (y - this.minV) / (this.maxV - this.minV);
...
function ui(t, y, v) {
  new ma();
  const b = aa(), S = new Ga({ uniforms: { cmap: { value: b }, ambient: { value: 0.95 } }, vertexShader: `
      #include <common>
...
function us(t, y) {
  ...
  const L = new nn(v.grid).multiplyScalar(1.3), N = new nn(v.grid).multiplyScalar(0.8), B = (j, U, de, Z) => {
    ...
    const H = new ct({ color: de, transparent: true, opacity: Z, depthWrite: false }), G = new Ht(J, H);
    return G.name = `grid-${j}-${U === A ? "minor" : "major"}`, G;
  }, ne = (j, U, de) => {
    const Z = j === "xy" ? (G, X) => [G, X, 0] : j === "xz" ? (G, X) => [G, 0, X] : (G, X) => [0, G, X], re = [[-E, -E], [E, -E], [E, E], [-E, E]], ae = [];
...
function wi(t, y, v, b) {
  ...
  ce.derive(() => {
    ...
    let B = N;
    if (N.length > L) {
      const j = N.map((R) => E[R][0]), U = N.map((R) => E[R][1]), de = Math.min(...j), Z = Math.max(...j), re = Math.min(...U), ae = Math.max(...U), se = N.map((R) => E[R][2]), J = Math.max(1e-6, (Math.max(...se) - Math.min(...se)) / 40), H = (R) => Math.round(R / J), G = new Set(se.map(H)), X = Math.max(4, Math.floor(L / Math.max(1, G.size))), K = Math.max(2, Math.round(Math.sqrt(X))), q = /* @__PURE__ */ new Map();
      for (const R of N) {
        const O = Z - de < 1e-9 ? 0 : (E[R][0] - de) / (Z - de), fe = ae - re < 1e-9 ? 0 : (E[R][1] - re) / (ae - re), ye = Math.min(K - 1, Math.floor(O * K)), be = Math.min(K - 1, Math.floor(fe * K)), W = `${ye},${be},${H(E[R][2])}`, _e = Math.hypot(O * K - (ye + 0.5), fe * K - (be + 0.5)), ue = q.get(W);
...
function _i(t, y, v, b) {
  ...
  S.add(z);
  function A(J, H) {
    const G = J / 2, X = H / 2, K = new Float32Array([0, -G, -X, 0, G, -X, 0, G, X, 0, -G, -X, 0, G, X, 0, -G, X]), q = new Ee();
    q.setAttribute("position", new ft(K, 3));
    const R = new Float32Array([0, -G, -X, 0, G, -X, 0, G, X, 0, -G, X, 0, -G, -X]), O = new Ee();
  ...
  function E(J, H = 24) {
    ...
    for (let O = 0; O < H; O++) {
      ...
      X[O * 9] = 0, X[O * 9 + 1] = 0, X[O * 9 + 2] = 0, X[O * 9 + 3] = 0, X[O * 9 + 4] = G * Math.cos(fe), X[O * 9 + 5] = G * Math.sin(fe), X[O * 9 + 6] = 0, X[O * 9 + 7] = G * Math.cos(ye), X[O * 9 + 8] = G * Math.sin(ye);
    }
    const K = new Ee();
    ...
    for (let O = 0; O <= H; O++) {
      ...
      q[O * 3] = 0, q[O * 3 + 1] = G * Math.cos(fe), q[O * 3 + 2] = G * Math.sin(fe);
    }
    const R = new Ee();
    return R.setAttribute("position", new ft(q, 3)), { fill: K, outline: R };
  }
  function C(J, H, G, X) {
    const K = G ?? H * 0.08, q = X ?? J * 0.07, R = J / 2, O = H / 2, fe = O - K, ye = q / 2, be = [];
    function W(he, Ie, ve, qe) {
      be.push(0, he, Ie, 0, ve, Ie, 0, ve, qe, 0, he, Ie, 0, ve, qe, 0, he, qe);
    }
    W(-R, -O, R, -fe), W(-ye, -fe, ye, fe), W(-R, fe, R, O);
    const _e = new Ee();
  ...
  function L(J, H, G) {
    ...
    const be = new Float32Array([0, -X, -K, 0, X, -K, 0, X, -K, 0, X, K, 0, X, K, 0, -X, K, 0, -X, K, 0, -X, -K, 0, -q, -R, 0, q, -R, 0, q, -R, 0, q, R, 0, q, R, 0, -q, R, 0, -q, R, 0, -q, -R]), W = new Ee();
    return W.setAttribute("position", new ft(be, 3)), { fill: ye, outline: W };
  }
  function N(J, H, G) {
    const X = J / 2, K = H / 2, q = X - G, R = K - G, O = new Ee(), fe = new Float32Array([0, -q, -R, 0, q, -R, 0, q, R, 0, -q, -R, 0, q, R, 0, -q, R]);
    ...
    const W = new Ee();
    W.setAttribute("position", new ft(new Float32Array(ye), 3));
    ...
    return ue.setAttribute("position", new ft(_e, 3)), { concFill: O, steelFillGeom: W, outline: ue };
  }
  function B(J, H, G) {
  ...
  function ne(J, H, G, X) {
    const K = X / 2, q = [], R = [[0, -J - K, -H / 2], [0, -G - K, -H / 2], [0, -G - K, H / 2 - G], [0, -K, H / 2 - G], [0, -K, H / 2], [0, -J - K, H / 2]], O = [[0, K, -H / 2], [0, K + G, -H / 2], [0, K + G, H / 2 - G], [0, J + K, H / 2 - G], [0, J + K, H / 2], [0, K, H / 2]], fe = [0, 1, 2, 0, 2, 5, 2, 3, 4, 2, 4, 5];
    ...
    const W = new Ee();
    return W.setAttribute("position", new ft(new Float32Array(be), 3)), { fill: ye, outline: W };
  }
  function pe(J, H, G, X) {
    const K = H / 2, q = J, R = [[0, -q, -K], [0, -q, -K + G], [0, -X, -K + G], [0, -X, K - G], [0, -q, K - G], [0, -q, K], [0, 0, K], [0, 0, -K]], O = [0, 1, 7, 1, 6, 7, 1, 2, 6, 2, 5, 6, 2, 3, 5, 3, 4, 5], fe = [];
  ...
  function we(J, H, G, X, K) {
    const q = H / 2, R = K / 2, O = [], fe = [[0, -J, -q], [0, -J, -q + G], [0, -R - X, -q + G], [0, -R - X, q - G], [0, -J, q - G], [0, -J, q], [0, -R, q], [0, -R, -q]], ye = fe.map((Ve) => [Ve[0], -Ve[1], Ve[2]]), be = [0, 1, 7, 1, 6, 7, 1, 2, 6, 2, 5, 6, 2, 3, 5, 3, 4, 5];
    for (const Ve of be) O.push(...fe[Ve]);
    for (const Ve of be) O.push(...ye[Ve]);
    const W = new Ee();
    W.setAttribute("position", new ft(new Float32Array(O), 3));
    const _e = [];
    ...
    const ue = new Ee();
    return ue.setAttribute("position", new ft(new Float32Array(_e), 3)), { fill: W, outline: ue };
  }
  function j(J, H, G, X) {
    const K = J / 2, q = H / 2, R = X / 2, O = [[0, -R, -q], [0, R, -q], [0, R, q - G], [0, K, q - G], [0, K, q], [0, -K, q], [0, -K, q - G], [0, -R, q - G]], fe = [0, 1, 7, 1, 2, 7, 6, 7, 5, 2, 3, 4, 2, 4, 5, 2, 5, 7], ye = [];
    ...
    const _e = new Ee();
    return _e.setAttribute("position", new ft(new Float32Array(W), 3)), { fill: be, outline: _e };
  }
  function U(J, H, G = 24) {
    const X = J / 2, K = X - H, q = [];
    for (let ye = 0; ye < G; ye++) {
      const be = ye / G * Math.PI * 2, W = (ye + 1) / G * Math.PI * 2, _e = Math.cos(be), ue = Math.sin(be), Ve = Math.cos(W), he = Math.sin(W);
    ...
    const O = [];
    for (let ye = 0; ye < G; ye++) {
      const be = ye / G * Math.PI * 2, W = (ye + 1) / G * Math.PI * 2;
      O.push(0, X * Math.cos(be), X * Math.sin(be), 0, X * Math.cos(W), X * Math.sin(W)), O.push(0, K * Math.cos(be), K * Math.sin(be), 0, K * Math.cos(W), K * Math.sin(W));
    }
    const fe = new Ee();
    return fe.setAttribute("position", new ft(new Float32Array(O), 3)), { fill: R, outline: fe };
  }
  ...
  return ce.derive(() => {
    ...
    G.forEach((R, O) => {
      ...
      if (W.type === "CFT") {
        const he = N(W.b, W.h, W.tw ?? W.b * 0.05), Ie = new lt(he.concFill, de);
        ...
        switch (W.type) {
          case "rect":
            he = A(W.b, W.h), Ie = de, ve = Z;
            break;
          case "circ":
            he = E(W.d), Ie = de, ve = Z;
            break;
          case "I":
            he = C(W.b, W.h, W.tf, W.tw), Ie = re, ve = ae;
            break;
          case "HSS":
            he = L(W.b, W.h, W.tw ?? W.b * 0.05), Ie = re, ve = ae;
            break;
          case "CFT":
            he = N(W.b, W.h, W.tw ?? W.b * 0.05), Ie = re, ve = ae;
            break;
          case "L":
            he = B(W.b ?? W.h, W.h, W.t ?? W.tw ?? 3e-3), Ie = re, ve = ae;
            break;
          case "2L":
            he = ne(W.b ?? W.h, W.h, W.t ?? W.tw ?? 3e-3, W.dis ?? 0.01), Ie = re, ve = ae;
            break;
          case "C":
          case "coldC":
            he = pe(W.b, W.h, W.tf ?? W.t ?? 3e-3, W.tw ?? W.t ?? 3e-3), Ie = re, ve = ae;
            break;
          case "2C":
            he = we(W.b, W.h, W.tf ?? 5e-3, W.tw ?? 5e-3, W.dis ?? 0.01), Ie = re, ve = ae;
            break;
          case "T":
            he = j(W.b, W.h, W.tf ?? 0.01, W.tw ?? 6e-3), Ie = re, ve = ae;
            break;
          case "pipe":
            he = U(W.d, W.tw ?? W.d * 0.05), Ie = re, ve = ae;
            break;
          default:
            return;
        }
      ...
      const Ve = bi(W);
      if (Ve) {
        const Ie = ["I", "HSS", "CFT", "L", "2L", "C", "2C", "T", "pipe", "coldC"].includes(W.type) ? "#ff9900" : "#00ccff", ve = new It(Ve, Ie, "transparent");
    ...
    try {
      pe.forEach((Z, re) => {
        ...
        if (Z.length === 2) {
          ...
          if (G < 1e-9) return;
          const X = new ei(Pi(ae), { depth: G, bevelEnabled: false, curveSegments: 4 });
        ...
        if (Z.length === 3 || Z.length === 4) {
          ...
          if (q < 1e-12) return;
          const R = [G / q, X / q, K / q], O = [], fe = (he) => se.map((Ie) => [Ie[0] + R[0] * he, Ie[1] + R[1] * he, Ie[2] + R[2] * he]), ye = Math.abs(R[2]) > 0.5, be = R[2] > 0 ? -1 : 1, W = fe(ye ? 0 : +ae / 2), _e2 = fe(ye ? be * ae : -ae / 2), ue = (he, Ie, ve) => O.push(...he, ...Ie, ...ve);
          for (const he of [W, _e2]) ue(he[0], he[1], he[2]), he.length === 4 && ue(he[0], he[2], he[3]);
          for (let he = 0; he < se.length; he++) {
            const Ie = (he + 1) % se.length;
            ue(W[he], _e2[he], _e2[Ie]), ue(W[he], _e2[Ie], W[Ie]);
          }
          const Ve = new Ee();
...
function Vi({ drawingObj: t, gridObj: y, scene: v, getActiveCamera: b, controls: S, gridSize: z, derivedDisplayScale: A, rendererElm: E, viewerRender: C }) {
  ...
  const Te = new V(), Pt = (e, o, s, n, a, c) => {
    ...
    Ze = [e, o, s], Ge = [i / p, l / p, r / p], Te.set((e + n) / 2, (o + a) / 2, (s + c) / 2), Te.project(b());
    ...
    Pe.style.display = "none", tt.style.display = "none", Ze = null, Ge = null, Me = false, document.activeElement === Pe && Pe.blur();
  }, Mt = (e) => {
    ...
    if (!Ze || !Ge || !t.polylines) return;
    let s = Ge[0], n = Ge[1], a = Ge[2];
  ...
  window.__hekatanTypeCoord = (e) => {
    ...
    if (!Ze || !Ge || document.activeElement === Pe) return;
  ...
  const _a = (e, o, s, n) => {
    ...
    for (const i of Ne) {
      ...
      if (l === "pt") {
        const p = e[+r[0]];
        if (!p) continue;
        const d = new lt(new Wn(0.025, 12, 12), new pt({ color: mo, transparent: true, opacity: 0.9, depthTest: false }));
  ...
  window.__hekatanDrawCircle = (e, o, s, n, a = window.__hekatanArcSegs ?? 12, c = "xy") => {
    ...
    if (Y >= 0) {
      ...
      for (let $e = 1; $e <= et && Ye; $e++) {
        const We = T(k * $e / et).getComponent(Y);
        (We - Qe) * (ke - me) < -1e-9 && (Ye = false), Qe = We;
      }
      if (Ye) {
        te = true;
        for (let $e = 0; $e <= a; $e++) {
          const We = me + (ke - me) * $e / a;
          let Re = 0, Ae = k;
          for (let Ue = 0; Ue < 60; Ue++) {
            const ut = (Re + Ae) / 2;
            (T(ut).getComponent(Y) - We) * (ke - me) < 0 ? Re = ut : Ae = ut;
          }
          const Be = T((Re + Ae) / 2);
          $.push([Be.x, Be.y, Be.z]);
        }
  ...
  window.__hekatanRevolveSelection = (e, o, s, n = 360) => {
    ...
    const f = m > 0 ? 1 : -1, h = (Y) => {
      const te = r[(Y - 1 + p) % p], me = r[Y], ke = r[(Y + 1) % p], et = [me[0] - te[0], me[1] - te[1]], Ye = [ke[0] - me[0], ke[1] - me[1]], Qe = Math.hypot(et[0], et[1]) || 1, $e = Math.hypot(Ye[0], Ye[1]) || 1, We = [f * et[1] / Qe, -f * et[0] / Qe], Re = [f * Ye[1] / $e, -f * Ye[0] / $e], Ae = 1 + (We[0] * Re[0] + We[1] * Re[1]);
      return [(We[0] + Re[0]) / Math.max(Ae, 1e-6), (We[1] + Re[1]) / Math.max(Ae, 1e-6)];
    ...
    const Y = (Ae, Be) => {
      ...
      return Ue;
    }, te = Math.max(1, Math.round($ / I)), me = Math.max(1, Math.round(T / I)), ke = $ / te, et = T / me, Ye = /* @__PURE__ */ new Map(), Qe = [], $e = t.points.rawVal.length, We = (Ae, Be) => {
    ...
    for (let Ae = 0; Ae < te; Ae++) for (let Be = 0; Be < me; Be++) {
      ...
      const Ue = We(Ae, Be), ut = We(Ae + 1, Be), yt = We(Ae + 1, Be + 1), Ft = We(Ae, Be + 1);
      Re.push([Ue, ut, yt, Ft]);
    }
  ...
  const ht = new rt(), za = new lt(new Wn(0.01, 12, 12), new pt({ color: 16777215, transparent: true, opacity: 0.95 })), Vs = new lt(new Wn(0.015, 12, 12), new pt({ color: 16498468, transparent: true, opacity: 0.2, depthWrite: false }));
  ...
  window.__hekatanUpdateSelectionPtScale = $s, S.addEventListener("change", () => {
    ...
    if ((!s.length || ((_c = (_b = (_a3 = window.__hekatanCadState) == null ? void 0 : _a3.get) == null ? void 0 : _b.call(_a3)) == null ? void 0 : _c.tool) !== "fillarea") && mt.visible && (mt.visible = false), s.length) {
      ...
      if (l) ko(l.type, l.x, l.y, l.z), ht.position.set(l.x, l.y, l.z), ht.visible = true, n.set(l.x, l.y, l.z), Po(l.type, e.clientX, e.clientY);
        ...
        Va(), So();
        const f = !a && window.__hekatanSnapEnabled !== false, h = window.__hekatanSnap2D ?? 0.5;
        f && h > 0 && (n.x = Math.round(n.x / h) * h, n.y = Math.round(n.y / h) * h, n.z = Math.round(n.z / h) * h), ht.position.copy(n), ht.visible = true;
      }
      jn(), G(Z && !l && (c || de) ? H(Z) : null), Tt = { p: n.clone(), x: e.clientX, y: e.clientY };
      ...
      if (r === "select" || !r) {
        ...
        if (h >= 0) {
          ...
          cn.geometry.setFromPoints([new V(k[0], k[1], k[2]), new V(P[0], P[1], P[2])]), cn.visible = true, Qt.visible = false, un = ((_l = (_k = t.areas) == null ? void 0 : _k.rawVal) == null ? void 0 : _l.includes(M.polyIdx)) ?? false ? { kind: "poly", a: M.polyIdx } : { kind: "seg", a: M.polyIdx, b: M.segIdx };
        } else if (_ >= 0) {
          const F = (((_m = window.__hekatanDrawingAuxLines) == null ? void 0 : _m.rawVal) ?? [])[_];
      ...
      if (r === "delete" || r === "trim" || r === "extend" || r === "offset") {
        ...
        let u = "";
        _ ? u = `\u{1F5D1} l\xEDnea aux #${vn + 1}` : h ? u = ((_o2 = (_n2 = t.areas) == null ? void 0 : _n2.rawVal) == null ? void 0 : _o2.includes(h.polyIdx)) ?? false ? `\u{1F5D1} \xE1rea #${h.polyIdx + 1}` : `\u{1F5D1} seg ${h.segIdx + 1} / poly #${h.polyIdx + 1}` : u = "\u{1F5D1} acerc\xE1 a l\xEDnea/\xE1rea", ze.textContent = `${w}  \xB7  ${u}`;
        const x = document.getElementById("hk-coord-fixed");
        x && (x.textContent = w), C();
        return;
      } else Zt.visible = false, ln = -1, vn = -1;
      ze.style.left = e.clientX + "px", ze.style.top = e.clientY + "px", ze.style.display = "block";
      const p = ((_p = t.polylines) == null ? void 0 : _p.rawVal) ?? [], d = p[p.length - 1] ?? [], m = t.points.rawVal ?? [];
      if (d.length > 0 && m[d[d.length - 1]]) {
        ...
        if (!M && !_ && window.__hekatanAxisSnap !== false) {
          const Ye = E.getBoundingClientRect(), Qe = e.clientX, $e = e.clientY, We = ((_q = settings.gridSize) == null ? void 0 : _q.rawVal) ?? 10, Re = new V(h[0], h[1], h[2]), Ae = [["x", new V(1, 0, 0)], ["y", new V(0, 1, 0)], ["z", new V(0, 0, 1)]], Be = (ut) => {
            const yt = ut.clone().project(o);
            return { x: (yt.x * 0.5 + 0.5) * Ye.width + Ye.left, y: (-yt.y * 0.5 + 0.5) * Ye.height + Ye.top };
          };
          let Ue = null;
          for (const [ut, yt] of Ae) {
            const Ft = Be(Re.clone().addScaledVector(yt, -We)), tn = Be(Re.clone().addScaledVector(yt, We)), Kn = tn.x - Ft.x, In = tn.y - Ft.y, Ya = Qe - Ft.x, Ua = $e - Ft.y, Za = Kn * Kn + In * In || 1;
            let Ao = (Ya * Kn + Ua * In) / Za;
            Ao = Math.max(0, Math.min(1, Ao));
            const Ws = Math.hypot(Qe - (Ft.x + Ao * Kn), $e - (Ft.y + Ao * In));
            if (Ue === null || Ws < Ue.dpx) {
              const os = L.ray, Gs = Re.clone().sub(os.origin), ss = yt.dot(os.direction), Hs = yt.dot(Gs), qa = os.direction.dot(Gs), Js = 1 - ss * ss, Ka = Math.abs(Js) < 1e-6 ? -Hs : (ss * qa - Hs) / Js;
              Ue = { axis: ut, dpx: Ws, pt: Re.clone().addScaledVector(yt, Ka) };
            }
          }
        ...
        if (!M && !_ && w) {
          const Ye = Math.abs(n.x - h[0]), Qe = Math.abs(n.y - h[1]), $e = Math.abs(n.z - h[2]), We = (_r = s[0]) == null ? void 0 : _r.object;
          let Re = null;
          We === Jt ? Re = "xy" : We === sn ? Re = "xz" : We === an && (Re = "yz"), Re === "xy" ? M = Ye >= Qe ? "x" : "y" : Re === "xz" ? M = Ye >= $e ? "x" : "z" : Re === "yz" ? M = Qe >= $e ? "y" : "z" : M = Ye >= Qe && Ye >= $e ? "x" : Qe >= $e ? "y" : "z";
        }
        const u = window.__hekatanPolarTrack !== false;
        if (!M && !_ && u) {
          const Ye = n.x - h[0], Qe = n.y - h[1], $e = n.z - h[2], We = Math.hypot(Ye, Qe, $e);
          if (We > 1e-3) {
            const Ae = Math.tan(6 * Math.PI / 180) * We, Be = Math.hypot(Qe, $e), Ue = Math.hypot(Ye, $e), ut = Math.hypot(Ye, Qe), yt = [["x", Be], ["y", Ue], ["z", ut]];
        ...
        if (M) {
          ...
          const We = !!_t, Ae = { x: "#ff3344", y: "#34d399", z: "#60a5fa" }[M];
          Rt.style.background = "rgba(15,23,42,0.92)", Rt.style.color = Ae, Rt.style.border = `1.5px solid ${Ae}`;
          const Be = (_s2 = s[0]) == null ? void 0 : _s2.object;
          let Ue = null;
          Be === Jt ? Ue = "xy" : Be === sn ? Ue = "xz" : Be === an && (Ue = "yz");
          const ut = Ue ? ` (plano ${Ue.toUpperCase()})` : "";
          Rt.textContent = We ? `\u{1F512} LOCK ${M.toUpperCase()}${ut}` : `\u22A5 ORTO ${M.toUpperCase()}${ut}`, Rt.style.left = e.clientX + 20 + "px", Rt.style.top = e.clientY + 18 + "px", Rt.style.transform = "none", Rt.style.display = "block";
        ...
        if (!a && !_ && window.__hekatanTrack !== false && window.__hekatanOsnapOn !== false) {
          ...
          for (const Re of Ye) if (!(Math.abs(Re[0] - h[0]) < 1e-9 && Math.abs(Re[1] - h[1]) < 1e-9 && Math.abs(Re[2] - h[2]) < 1e-9)) for (const Ae of Qe) {
            ...
            ut < Mn && ut < We && (We = ut, x = { q: Re, eje: Ae });
          }
        }
        x ? (x.eje === "x" ? n.x = x.q[0] : x.eje === "y" ? n.y = x.q[1] : n.z = x.q[2], Yt.geometry.setFromPoints([new V(x.q[0], x.q[1], x.q[2]), new V(n.x, n.y, n.z)]), (_t2 = Yt.computeLineDistances) == null ? void 0 : _t2.call(Yt), Yt.visible = true, ht.position.set(n.x, n.y, n.z), ht.visible = true, Po("track", e.clientX, e.clientY)) : Yt.visible = false, Tt = { p: n.clone(), x: e.clientX, y: e.clientY };
        ...
        $ && ($.textContent = k), Fe.geometry.setFromPoints([new V(h[0], h[1], h[2]), new V(n.x, n.y, n.z)]), (_u = Fe.computeLineDistances) == null ? void 0 : _u.call(Fe), Fe.visible = true, Pt(h[0], h[1], h[2], n.x, n.y, n.z);
        ...
        vs(te), te && (rn.style.left = e.clientX + "px", rn.style.top = e.clientY + "px"), Pn.geometry.setFromPoints([new V(h[0] - T, h[1], h[2]), new V(h[0] + T, h[1], h[2])]), (_v = Pn.computeLineDistances) == null ? void 0 : _v.call(Pn), Cn.geometry.setFromPoints([new V(h[0], h[1] - T, h[2]), new V(h[0], h[1] + T, h[2])]), (_w = Cn.computeLineDistances) == null ? void 0 : _w.call(Cn), zn.geometry.setFromPoints([new V(h[0], h[1], h[2] - T), new V(h[0], h[1], h[2] + T)]), (_x = zn.computeLineDistances) == null ? void 0 : _x.call(zn), Xt.visible = true;
        ...
        if (h && (h.textContent = f), Fe.visible = false, Xt.visible = false, (/* @__PURE__ */ new Set(["line", "polyline", "area", "node", "column", "wall", "rect", "circle", "arc", "polyline-multi", "axis", "chaflan"])).has(r)) {
          if (Ze = null, Ge = null, Pe.style.left = e.clientX + 20 + "px", Pe.style.top = e.clientY - 28 + "px", Pe.style.display = "block", !Me) {
    ...
    $i(y, { position: new V(...t.gridTarget.val.position), quaternion: e.clone().multiply(o) }, C);
    {
      ...
      for (const c of Gn) v.remove(c), No(c);
      if (Gn.length = 0, a) {
        ...
        for (const r of l) {
          ...
          }), p.position.set(0, 0, r), p.quaternion.copy(o), v.add(p), Gn.push(p);
        }
      }
    }
  ...
  window.__hekatanDeleteSelected = Go, window.addEventListener("keydown", (e) => {
    if (e.key !== "Delete" && e.key !== "Backspace") return;
    const o = document.activeElement, s = o && (o.id === "hk3-cmd-input" || o.id === "hk-dyn-input") && o.value === "";
    o && (o.tagName === "INPUT" || o.tagName === "TEXTAREA" || o.isContentEditable) && !s || Ne.size !== 0 && (e.preventDefault(), Go());
  });
  ...
  E.addEventListener("pointerdown", (e) => {
    ...
    if (Kt && e.buttons === 0) {
      ...
      Wo(Kt.x, Kt.y, e.clientX, e.clientY, c);
      return;
    }
    if (!en) return;
    ...
    Wo(en.x, en.y, e.clientX, e.clientY, a);
  }), E.addEventListener("pointerup", (e) => {
    if (!en) return;
    if (!eo) {
      en = null;
  ...
  const Gt = new rt();
  Gt.visible = false, Gt.frustumCulled = false, v.add(Gt);
  const Ts = { end: 16724804, mid: 16498468, node: 6333946, cen: 3462041, per: 12616956, nea: 16744118, int: 16746496, ori: 16777215, grid: 2282478, track: 16761856, ifc: 16096779, ifcAxis: 16639626, ifcSec: 16486972, ifcEdge: 16498468, ifcVert: 16724804 }, ko = (e, o, s, n) => {
    ...
    Gt.add(new Ht(c, new ct({ color: a, linewidth: 2 }))), Gt.position.set(o, s, n), Gt.visible = true, Jo();
  };
  let Ho = 4;
  const Jo = () => {
    Gt.visible && Gt.scale.setScalar(Ho * Mo(Gt.position));
  };
  window.__hekatanOsnapMarkerRef = Gt, window.__hekatanUpdateOsnapScale = Jo, window.__hekatanOsnapPx = (e) => (typeof e == "number" && e > 0 && (Ho = e, Jo(), C()), Ho);
  const So = () => {
    Gt.visible = false;
  ...
  const $a = (e, o, s, n, a) => {
    ...
    if (c.int) {
      ...
      for (let x = 0; x < u.length; x++) for (let g = x + 1; g < u.length; g++) {
        ...
        const $e = (me * Ye - ke * et) / Qe, We = (te * Ye - me * et) / Qe;
        if ($e < -1e-6 || $e > 1 + 1e-6 || We < -1e-6 || We > 1 + 1e-6) continue;
        const Re = [F[0] + $e * T[0], F[1] + $e * T[1], F[2] + $e * T[2]], Ae = [P[0] + We * I[0], P[1] + We * I[1], P[2] + We * I[2]];
  ...
  window.__hekatanCadRefreshPrompt = Wt, window.__hekatanRefreshStatus = () => {
    ...
    Xe = [], at = [], it.visible = false, Qo(), Dt = null, C(), le("\u{1F6E0} Tool cambiado \u2014 clicks pendientes limpiados"), Wt();
  };
  function Qo() {
    if (!t.polylines) return;
  ...
  const jo = () => {
    Xe = [], Dt = null, Qo(), _t = null, Ms(), Fe.visible = false, Xt.visible = false, nt(), le("\u23F9 Dibujo finalizado \u2014 click para empezar otra serie"), C(), Wt();
  };
  ...
  const qs = (e, o) => {
    ...
    if (Xe.push(o), Xe.length === 1) {
      Ze = o, le(`${e === "move" ? "MOVER" : "COPIAR"} punto base (${o[0].toFixed(2)}, ${o[1].toFixed(2)}, ${o[2].toFixed(2)}). Precise el segundo punto.`), Wt();
      return;
    }
    const [s, n] = Xe, a = [n[0] - s[0], n[1] - s[1], n[2] - s[2]];
    Xe = [], Fe.visible = false;
    let c = 0;
    e === "move" ? c = Zs(a[0], a[1], a[2]) : (c = Us().size, (_c = window.__hekatanReplicateSelection) == null ? void 0 : _c.call(window, a[0], a[1], a[2], 1)), le(`\u2713 ${e === "move" ? "Movidos" : "Copiados"} ${c} nudo${c === 1 ? "" : "s"} \u2014 \u0394 (${a[0].toFixed(2)}, ${a[1].toFixed(2)}, ${a[2].toFixed(2)}) m.`), e === "move" && (Ne.clear(), Ot()), (_e2 = (_d = window.__hekatanCadState) == null ? void 0 : _d.setTool) == null ? void 0 : _e2.call(_d, "select"), Wt();
  };
  ...
  const Da = () => {
    ...
    if (!Dt) {
      ...
      Dt = { poly: ln, seg: Math.max(0, gn) }, le(e === "offset" ? `DESFASE l\xEDnea #${Dt.poly + 1} designada \u2014 clic en el lado hacia el que va la copia${$n > 0 ? ` (${$n} m)` : ""}.` : e === "trim" ? "RECORTAR contorno designado \u2014 clic en el trozo de l\xEDnea a quitar." : "ALARGAR contorno designado \u2014 clic en la l\xEDnea a alargar, cerca del extremo libre."), Wt();
      return;
    }
    if (e === "offset") {
      ...
      for (let $e = 0; $e < w.length - 1; $e++) {
        const We = n[w[$e]], Re = n[w[$e + 1]], Ae = [Re[0] - We[0], Re[1] - We[1], Re[2] - We[2]], Be = Math.hypot(Ae[0], Ae[1], Ae[2]) || 1, Ue = Ae[0] / Be, ut = Ae[1] / Be, yt = Ae[2] / Be, Ft = [x[1] * yt - x[2] * ut, x[2] * Ue - x[0] * yt, x[0] * ut - x[1] * Ue], tn = Math.hypot(Ft[0], Ft[1], Ft[2]) || 1;
        g.push({ a: We, b: Re, n: [Ft[0] / tn, Ft[1] / tn, Ft[2] / tn] });
      }
      let F = 0, k = 1 / 0;
      g.forEach(($e, We) => {
      ...
      const I = g.map(($e) => ({ a: [$e.a[0] + $ * T * $e.n[0], $e.a[1] + $ * T * $e.n[1], $e.a[2] + $ * T * $e.n[2]], b: [$e.b[0] + $ * T * $e.n[0], $e.b[1] + $ * T * $e.n[1], $e.b[2] + $ * T * $e.n[2]] })), Y = I.length, te = ($e) => {
        const We = I[($e - 1 + Y) % Y], Re = I[$e % Y];
        return es(We.a, We.b, Re.a, Re.b, true, true) ?? Re.a;
      }, me = [], ke = u ? Y : Y + 1;
      for (let $e = 0; $e < ke; $e++) !u && $e === 0 ? me.push(I[0].a) : !u && $e === Y ? me.push(I[Y - 1].b) : me.push(te($e));
      vt();
      const et = n.length;
      t.points.val = [...n, ...me];
      const Ye = me.map(($e, We) => et + We);
      u && Ye.push(et);
      let Qe = s.slice();
  ...
  const Ks = (e, o) => {
    ...
    if (s === "select" || s === "none" || !s) {
      if (un) {
        ...
        Kt ? (Is(Kt.x, Kt.y, l, r, i), Kt = null) : i || (Kt = { x: l, y: r }, le("\u{1F5B1} Click 2 para cerrar el rect\xE1ngulo (\u2192 derecha=Window azul, \u2190izquierda=Crossing verde). Esc=cancelar."), Wo(l, r, l + 1, r + 1, false));
      }
      return;
    }
    if (s === "axis") {
    ...
    if (s === "ifcface") {
      ...
      const r = O.normal.clone(), p = W(O.m, O.punto, r);
...
function Bi(t, y, v, b) {
  ...
  return ce.derive(() => {
    ...
    for (const { idx: R, vals: O } of we) {
      ...
      if (!ye || !be) continue;
      const W = new V(be[0] - ye[0], be[1] - ye[1], be[2] - ye[2]), _e = W.length();
      if (_e < 1e-10) continue;
      W.normalize();
      const ue = Math.abs(W.y) < 0.99 ? new V(0, 1, 0) : new V(1, 0, 0), Ve = new V().crossVectors(W, ue).normalize(), he = new V().crossVectors(W, Ve).normalize(), Ie = fs + 1, ve = Li;
      for (let qe = 0; qe < Ie; qe++) {
        const Je = qe / fs, xt = ye[0] + W.x * _e * Je, gt = ye[1] + W.y * _e * Je, xe = ye[2] + W.z * _e * Je, D = O[0] + (O[1] - O[0]) * Je, oe = z.getColor(D) ?? new nn(0, 0, 0);
      ...
      for (let qe = 0; qe < fs; qe++) for (let Je = 0; Je < ve; Je++) {
        const xt = (Je + 1) % ve, gt = G + qe * ve + Je, xe = G + qe * ve + xt, D = G + (qe + 1) * ve + Je, oe = G + (qe + 1) * ve + xt;
...
function Wi(t) {
  ...
  const v = new Wn(1, 16, 16), b = new pt({ color: Zi, transparent: true, opacity: 0.85, depthTest: false }), S = new lt(v, b);
  ...
  function X(xe, D) {
    ...
    const Ge = Ni(), Me = Yi[Ge.dispUnit] ?? 1e3, Te = Xi[Ge.forceUnit] ?? 1;
    if (tt >= 0) {
      const st = Se[tt];
      let He = `Nodo ${tt}
(${st[0].toFixed(3)}, ${st[1].toFixed(3)}, ${st[2].toFixed(3)})`;
      ...
      if (ze == null ? void 0 : ze.deformations) {
        ...
        if (Oe && (He += `
\u2500\u2500\u2500\u2500 \u0394 desplaz. \u2500\u2500\u2500\u2500`, He += `
Ux = ${St(Oe[0] * Me, 3)} ${Ge.dispUnit}`, He += `
Uy = ${St(Oe[1] * Me, 3)} ${Ge.dispUnit}`, He += `
Uz = ${St(Oe[2] * Me, 3)} ${Ge.dispUnit}`, (Math.abs(Oe[3]) > 1e-9 || Math.abs(Oe[4]) > 1e-9 || Math.abs(Oe[5]) > 1e-9) && (He += `
...
\u2500\u2500\u2500\u2500 R reacciones \u2500\u2500\u2500\u2500`, He += `
Fx = ${St(Fe[0] * Te)} ${Ge.forceUnit}`, He += `
Fy = ${St(Fe[1] * Te)} ${Ge.forceUnit}`, He += `
Fz = ${St(Fe[2] * Te)} ${Ge.forceUnit}`, (Math.abs(Fe[3]) > 1e-6 || Math.abs(Fe[4]) > 1e-6 || Math.abs(Fe[5]) > 1e-6) && (He += `
Mx = ${St(Fe[3] * Te)} ${Ge.forceUnit}\xB7m`, He += `
My = ${St(Fe[4] * Te)} ${Ge.forceUnit}\xB7m`, He += `
Mz = ${St(Fe[5] * Te)} ${Ge.forceUnit}\xB7m`));
        }
      }
    ...
    for (let st = 0; st < ge.length; st++) {
      ...
      if (!(!He || He.length < 2)) {
        if (He.length === 2) {
          ...
          const Fe = Gi(ie, Ce, ze.x, ze.y, Oe.x, Oe.y);
...
nodos: [${st.join(", ")}]`, Bt === "shell" && ((_l = t.mesh) == null ? void 0 : _l.analyzeOutputs)) {
        const it = t.mesh.analyzeOutputs.rawVal, at = Ui[Ge.stressUnit] ?? 1, bt = [["bendingXX", "Mxx", Te, `${Ge.forceUnit}\xB7m/m`], ["bendingYY", "Myy", Te, `${Ge.forceUnit}\xB7m/m`], ["bendingXY", "Mxy", Te, `${Ge.forceUnit}\xB7m/m`], ["membraneXX", "Nxx", Te, `${Ge.forceUnit}/m`], ["membraneYY", "Nyy", Te, `${Ge.forceUnit}/m`], ["membraneXY", "Nxy", Te, `${Ge.forceUnit}/m`], ["shearX", "Qx", Te, `${Ge.forceUnit}/m`], ["shearY", "Qy", Te, `${Ge.forceUnit}/m`], ["vonMises", "\u03C3VM", at, Ge.stressUnit], ["pressure", "p", at, Ge.stressUnit]], Ke = [];
      ...
      if (Bt === "frame" && ((_m = t.mesh) == null ? void 0 : _m.deformOutputs) && t.mesh.elementInputs) {
        ...
        if (bt && st.length === 2) {
          ...
          if (Ke && je && Nt && Ct) {
            ...
            if (Lt > 1e-9) {
              const hn = mt / Lt, Sn = fn / Lt, co = zt / Lt, Xt = (je[0] - Ke[0]) * hn + (je[1] - Ke[1]) * Sn + (je[2] - Ke[2]) * co, mn = ((_n2 = at.elasticities) == null ? void 0 : _n2.get(nt)) ?? 0, Pn = ((_o = at.areas) == null ? void 0 : _o.get(nt)) ?? 0, Cn = ((_p = at.momentsOfInertiaY) == null ? void 0 : _p.get(nt)) ?? 0, zn = ((_q = at.momentsOfInertiaZ) == null ? void 0 : _q.get(nt)) ?? 0, Gn = ((_r = at.torsionalConstants) == null ? void 0 : _r.get(nt)) ?? 0, No = ((_s = at.shearModuli) == null ? void 0 : _s.get(nt)) ?? mn / 2.6, Yt = mn * Pn * (Xt / Lt), Hn = (je[3] - Ke[3]) * hn + (je[4] - Ke[4]) * Sn + (je[5] - Ke[5]) * co, An = No * Gn * (Hn / Lt), Rn = je[4] - Ke[4], Dn = je[5] - Ke[5], on = mn * Cn * Rn / Lt, Bn = mn * zn * Dn / Lt;
              ze += `
\u2500\u2500\u2500\u2500 frame \u2500\u2500\u2500\u2500`, ze += `
L = ${St(Lt, 3)} m`, ze += `
\u0394L = ${St(Xt * Me, 3)} ${Ge.dispUnit}`, ze += `
\u03B5 = ${St(Xt / Lt, 6)}`, Math.abs(Yt) > 1e-6 && (ze += `
N \u2248 ${St(Yt * Te)} ${Ge.forceUnit}`), Math.abs(An) > 1e-6 && (ze += `
T \u2248 ${St(An * Te)} ${Ge.forceUnit}\xB7m`), Math.abs(on) > 1e-6 && (ze += `
My \u2248 ${St(on * Te)} ${Ge.forceUnit}\xB7m`), Math.abs(Bn) > 1e-6 && (ze += `
Mz \u2248 ${St(Bn * Te)} ${Ge.forceUnit}\xB7m`);
            }
          }
        }
      }
    ...
    if (xe.type === "node") {
      ...
      if (ge) {
        ...
        if (Le.length >= 2) {
          ...
          for (const Ge of Le) for (let Me = 0; Me < 3; Me++) Ge[Me] < tt[Me] && (tt[Me] = Ge[Me]), Ge[Me] > Ze[Me] && (Ze[Me] = Ge[Me]);
      ...
      const ge = ee[xe.idx], Le = G(ge[0]), De = G(ge[1]);
      if (Le && De) {
        const ot = Le.clone().add(De).multiplyScalar(0.5), Pe = De.clone().sub(Le), tt = Pe.length(), Ze = Math.max(1e-4, 3.5 * ve(ot));
        L.position.copy(ot);
        const Ge = new V(0, 1, 0), Me = Ge.clone().cross(Pe).normalize(), Te = Ge.angleTo(Pe);
  ...
  const W = () => {
    ...
    (oe < -2 || ee < -2 || oe > D.width + 2 || ee > D.height + 2) && (be && clearTimeout(be), be = window.setTimeout(W, 200));
  }, ue = () => {
...
function ji(t, y) {
  ...
  function E() {
    ...
    const ae = (H) => {
      const G = z(b.plano), X = G.findIndex((q) => Math.abs(q - b.en) < kn), K = Math.max(0, Math.min(G.length - 1, (X < 0 ? 0 : X) + H));
      G.length && (b.en = G[K], C());
    };
    ...
    se.addEventListener("pointerdown", (H) => {
      ...
      J = { x: H.clientX, y: H.clientY, l: G.left, t: G.top }, v.style.transform = "none", v.style.left = G.left + "px", v.style.top = G.top + "px";
  ...
  function C() {
    ...
    H.innerHTML = G.map((Me, Te) => `<option value="${Me}" ${Math.abs(Me - b.en) < kn ? "selected" : ""}>${K} ${Te + 1} \xB7 ${X} = ${Me.toFixed(2)} m</option>`).join("");
    ...
    re.innerHTML = "";
    const ye = re.clientWidth || 880, be = re.clientHeight || 480, W = [];
    if (O.forEach((Me, Te) => {
      if (Me.length !== 2) return;
      const Pt = R[Me[0]], nt = R[Me[1]];
      if (!Pt || !nt) return;
      const Mt = ao(Pt, b.plano), Bt = ao(nt, b.plano);
      Math.abs(Mt.fuera - b.en) < kn && Math.abs(Bt.fuera - b.en) < kn && W.push({ i: Te, a: Mt, b: Bt });
    }), !W.length) {
      se.textContent = "No hay barras en este plano.", ae.textContent = "";
      return;
    }
    let _e = 1 / 0, ue = -1 / 0, Ve = 1 / 0, he = -1 / 0;
    for (const Me of W) for (const Te of [Me.a, Me.b]) _e = Math.min(_e, Te.u), ue = Math.max(ue, Te.u), Ve = Math.min(Ve, Te.v), he = Math.max(he, Te.v);
    ...
    const Se = W.filter((Me) => !Ce.get(Me.i)).length;
    let ge = 0;
    if (fe) for (const Me of W) {
    ...
    for (const Me of W) for (const Te of [Me.a, Me.b]) b.plano !== "XY" && Math.abs(Te.v - Ve) < kn && ie("rect", { x: D(Te.u) - 6, y: oe(Te.v), width: 12, height: 7, fill: "#b03a3a" });
    const Ze = [];
  ...
  function we(Z) {
    ...
    for (const X of [0, 1]) {
      ...
      for (let R = 0; R < 500; R++) {
        ...
        const fe = O[0];
        X === 0 ? G.unshift(fe) : G.push(fe), q = ae[fe][0] === q ? ae[fe][1] : ae[fe][0], K = fe;
      }
    }
    return G;
  }
  ...
  function de() {
    ...
    let H = 0, G = -1;
    se.forEach((ue, Ve) => {
      const [he, Ie] = re[ue], ve = Ve === 0 ? se.length > 1 && re[se[1]].includes(he) : he !== G, qe = ve ? Ie : he, Je = ve ? he : Ie, xt = Math.hypot(Z[Je][0] - Z[qe][0], Z[Je][1] - Z[qe][1], Z[Je][2] - Z[qe][2]);
      J.push({ x: H, e: ue, fin: ve ? 1 : 0 }), H += xt, J.push({ x: H, e: ue, fin: ve ? 0 : 1 }), G = Je;
    });
    const X = H, K = (ue, Ve) => {
    ...
    fe.innerHTML = "";
    const ye = Math.max(300, fe.clientWidth), be = 124, W = 46, _e = (be - 14) / 2;
    for (const [ue, Ve, he, Ie] of O) {
      const ve = J.map((ge) => K(ue, ge)), qe = Math.max(...ve), Je = Math.min(...ve), xt = Math.max(Math.abs(qe), Math.abs(Je)) || 1, gt = (ge) => W + ge / (X || 1) * (ye - 2 * W), xe = (ge) => _e + (Ie ? 1 : -1) * (ge / xt) * (_e - 16), D = (ge) => Math.abs(ge) >= 100 ? ge.toFixed(1) : Math.abs(ge) >= 10 ? ge.toFixed(2) : ge.toFixed(3);
      let oe = gt(0) + "," + _e + " ";
      J.forEach((ge, Le) => {
        oe += gt(ge.x) + "," + xe(ve[Le]) + " ";
      }), oe += gt(X) + "," + _e;
      ...
      fe.insertAdjacentHTML("beforeend", '<div style="display:flex;justify-content:space-between;margin-top:4px"><b style="color:#dbe6f5">' + Ve + ' <span style="color:#6f7d90;font-weight:400">(' + he + ')</span></b><span style="color:#9fb0c6">m\xE1x ' + D(qe) + " \xB7 m\xEDn " + D(Je) + (Ie ? " \xB7 positivo hacia abajo" : "") + '</span></div><svg width="' + ye + '" height="' + be + '" style="display:block;background:#0e131c;border-radius:4px"><line x1="' + W + '" y1="' + _e + '" x2="' + (ye - W) + '" y2="' + _e + '" stroke="#e6ecf5" stroke-width="2"/><polygon points="' + oe + '" fill="' + Se + '" fill-opacity=".35" stroke="' + Se + '" stroke-width="1.4"/>' + Ce(0, "#f2f5fa") + Ce(J.length - 1, "#f2f5fa") + (ee > 0 && ee < J.length - 1 ? Ce(ee, "#8fd3ff") : "") + (ie > 0 && ie < J.length - 1 && ie !== ee ? Ce(ie, "#ff9f9a") : "") + '<text x="' + W + '" y="' + (be - 3) + '" fill="#6f7d90" font-size="10" text-anchor="middle">0</text><text x="' + (ye - W) + '" y="' + (be - 3) + '" fill="#6f7d90" font-size="10" text-anchor="middle">' + X.toFixed(2) + " m</text></svg>");
    }
  }
...
function pl({ mesh: t, settingsObj: y, drawingObj: v, objects3D: b, solids: S }) {
  ...
  let ye = false, be = null, W = null, _e = false;
  function ue() {
  ...
  function he(D, oe) {
    ...
    if (C.left = -ge * Se, C.right = ge * Se, C.updateProjectionMatrix(), D && be) {
      if (W ? (W.object = be, W.update()) : (W = new sa(be, N.domElement), W.enableDamping = true, W.dampingFactor = 0.1, W.screenSpacePanning = true, W.zoomSpeed = 0.8, W.panSpeed = 1.2, W.rotateSpeed = 0.9, W.touches = { ONE: Eo.ROTATE, TWO: Eo.DOLLY_PAN }, W.target.copy(B.target), W.addEventListener("change", ue), W.enabled = false), !_e) {
        const Le = (De) => {
          if (!ye || !W) return;
          const ot = N.domElement.getBoundingClientRect(), Pe = De.clientX - ot.left, tt = ot.width / 2, Ze = Pe >= tt;
          B.enabled = !Ze, W.enabled = Ze;
        };
  ...
  if (t) {
    ...
    const D = Wi({ scene: A, rendererElm: N.domElement, getActiveCamera: () => L, derivedNodes: Z, derivedDisplayScale: de, mesh: t, settings: U, render: ue });
    A.add(D);
  ...
  v && Vi({ drawingObj: v, gridObj: se, scene: A, getActiveCamera: () => L, controls: B, gridSize: G, derivedDisplayScale: de, rendererElm: N.domElement, viewerRender: ue }), ua((D, oe) => {
    var _a;
