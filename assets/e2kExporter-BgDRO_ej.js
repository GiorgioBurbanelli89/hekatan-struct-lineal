...
function qt(g) {
  ...
  for (const G of A) {
    ...
    switch (W) {
      case "PROGRAM CONTROL": {
        ...
        if (o) {
          const n = o.split(",").map((s) => s.trim());
          n[0] && (P.force = n[0]), n[1] && (P.length = n[1]);
        }
        break;
      }
      case "MATERIAL PROPERTIES 01 - GENERAL": {
        const o = l.get("Material");
        o && !H.has(o) && H.set(o, { E: 0, nu: 0, G: 0 });
        break;
      }
      case "MATERIAL PROPERTIES 02 - BASIC MECHANICAL PROPERTIES": {
        const o = l.get("Material");
        if (o) {
          const n = H.get(o) || { E: 0, nu: 0, G: 0 };
          n.E = Y(l.get("E1")), n.G = Y(l.get("G12")), n.nu = Y(l.get("U12")), n.density = Y(l.get("UnitMass")), H.set(o, n);
        }
        break;
      }
      case "MATERIAL PROPERTIES 03A - STEEL DATA": {
        const o = l.get("Material");
        o && H.has(o) && (H.get(o).fy = Y(l.get("Fy")));
        break;
      }
      ...
      case "JOINT COORDINATES": {
        const o = l.get("Joint");
        if (o) {
          const n = Y(l.get("XorR")), s = Y(l.get("Y")), E = Y(l.get("Z"));
          Q.set(o, [n, s, E]);
        }
        break;
      }
      case "CONNECTIVITY - FRAME": {
        const o = l.get("Frame"), n = l.get("JointI"), s = l.get("JointJ");
        o && n && s && K.push({ name: o, j1: n, j2: s });
        break;
      }
      case "CONNECTIVITY - AREA": {
        const o = l.get("Area");
        if (o) {
          const n = parseInt(l.get("NumJoints") || "4"), s = [];
          for (let E = 1; E <= n; E++) {
      ...
      case "JOINT RESTRAINT ASSIGNMENTS": {
        const o = l.get("Joint");
        if (o) {
          const n = [((_a = l.get("U1")) == null ? void 0 : _a.toLowerCase()) === "yes", ((_b = l.get("U2")) == null ? void 0 : _b.toLowerCase()) === "yes", ((_c = l.get("U3")) == null ? void 0 : _c.toLowerCase()) === "yes", ((_d = l.get("R1")) == null ? void 0 : _d.toLowerCase()) === "yes", ((_e = l.get("R2")) == null ? void 0 : _e.toLowerCase()) === "yes", ((_f = l.get("R3")) == null ? void 0 : _f.toLowerCase()) === "yes"];
          j.set(o, n);
        }
        break;
      }
      case "JOINT SPRING ASSIGNMENTS 1 - UNCOUPLED": {
        const o = l.get("Joint");
        o && q.set(o, ["U1", "U2", "U3", "R1", "R2", "R3"].map((n) => parseFloat(l.get(n) ?? "0") || 0));
        break;
      }
      case "FRAME SECTION ASSIGNMENTS": {
        const o = l.get("Frame"), n = l.get("AnalSect");
        o && n && te.set(o, n);
        break;
      }
      case "AREA SECTION ASSIGNMENTS": {
        const o = l.get("Area"), n = l.get("Section");
        o && n && le.set(o, n);
        break;
      }
      case "FRAME LOADS - DISTRIBUTED": {
        const o = l.get("Frame"), n = l.get("Dir"), s = Y(l.get("FOverLA"));
        if (o && n && s) {
          const E = { X: 0, Y: 1, Z: 2 }[n];
          if (E !== void 0) {
      ...
      case "SOLID PROPERTY ASSIGNMENTS": {
        const o = l.get("Solid"), n = l.get("SolidProp");
        o && n && C.set(o, n);
        break;
      }
      case "AREA STIFFNESS MODIFIERS": {
        const o = l.get("Area");
        o && Ee.set(o, ["f11", "f22", "f12", "m11", "m22", "m12", "v13", "v23"].map((n) => l.has(n) ? Y(l.get(n)) : 1));
        break;
      }
  ...
  return wt(P, m, H, S, D, Q, K, X, j, te, le, ae, oe, ie, Ee, Se, i, h, C, J, re, q);
}
function Qt(g) {
  const A = { force: "KN", length: "m" };
  let R = "UX,UY,UZ,RX,RY,RZ";
  const P = /* @__PURE__ */ new Map(), m = /* @__PURE__ */ new Map(), H = /* @__PURE__ */ new Map(), J = /* @__PURE__ */ new Map(), re = [], S = [], D = /* @__PURE__ */ new Map(), Q = [], K = /* @__PURE__ */ new Map(), X = /* @__PURE__ */ new Map(), j = /* @__PURE__ */ new Map(), q = /* @__PURE__ */ new Map(), te = [], le = /* @__PURE__ */ new Map(), ae = /* @__PURE__ */ new Map();
  let oe = "", ie = "";
  for (const i of g) {
    const h = i.trim();
    ...
    switch (oe) {
      ...
      case "SHELL SECTION": {
        const G = C.get("NAME");
        G && H.set(G, { material: C.get("MAT") || "", type: C.get("TYPE") || "Shell", thickness: Y(C.get("TH")) });
        break;
      }
      case "FRAME": {
        const G = W[0], M = C.get("J");
  ...
  return wt(A, R, P, m, H, J, re, S, D, /* @__PURE__ */ new Map(), /* @__PURE__ */ new Map(), Q, K, X, j, q, te, le, ae);
}
function wt(g, A, R, P, m, H, J, re, S, D, Q, K, X = /* @__PURE__ */ new Map(), j = /* @__PURE__ */ new Map(), q = /* @__PURE__ */ new Map(), te = /* @__PURE__ */ new Map(), le = [], ae = /* @__PURE__ */ new Map(), oe = /* @__PURE__ */ new Map(), ie, Ee, Se = /* @__PURE__ */ new Map()) {
  var _a, _b;
  const i = [], h = /* @__PURE__ */ new Map(), C = [];
  for (const [T, p] of H) h.set(T, C.length), i.push(T), C.push(p);
  ...
  const o = W.length - l, n = [];
  for (const T of le) {
    const p = T.joints.map((O) => h.get(O));
    if (p.some((O) => O === void 0)) continue;
    const b = W.length;
    W.push([p[0], p[1], p[3], p[2], p[4], p[5], p[7], p[6]]), G.push(T.name), n.push(b);
...
function Bt(g, A, R, P = {}) {
  ...
  for (const D of R ?? []) {
    ...
    if (!K) continue;
    if (D.dof === -2 || D.dof === -4) {
      H.push([Q, Math.round(D.k), D.dof === -4]);
      continue;
    }
    ...
    if (K.length === 3) {
      ...
      const o = [M[1] * l[2] - M[2] * l[1], M[2] * l[0] - M[0] * l[2], M[0] * l[1] - M[1] * l[0]], n = Math.hypot(o[0], o[1], o[2]);
      for (let s = 0; s < 4; s++) Se[s] += C[s] * n;
    }
    K.forEach((i, h) => {
      for (let C = 0; C < 3; C++) Math.abs(ie[C]) > 1e-12 && re(i, C, D.k * Se[h] * ie[C] * ie[C]);
    });
  }
  return { nodales: m, colgados: H, deArea: J };
}
function cs(g) {
  var _a, _b, _c, _d, _e, _f, _g, _h, _i, _j, _k, _l, _m;
  const { nodes: A, elements: R, nodeInputs: P, elementInputs: m } = g, H = { force: "KN", length: "m" };
  ...
  const Q = [], K = (o) => {
    ...
    const n = ((_a2 = m.elasticities) == null ? void 0 : _a2.get(o)) || 0, s = (_b2 = m.poissonsRatios) == null ? void 0 : _b2.get(o), E = ((_c2 = m.shearModuli) == null ? void 0 : _c2.get(o)) || 0, N = s !== void 0 ? s : n > 0 && E > 0 ? Math.max(0, Math.min(0.5, n / (2 * E) - 1)) : 0.2, B = E > 0 ? E : n > 0 ? n / (2 * (1 + N)) : 0, T = (_d2 = m.sectionShapes) == null ? void 0 : _d2.get(o), p = (T == null ? void 0 : T.type) === "CFT" && T.steelRho > 0 ? T.steelRho : ((_e2 = m.densities) == null ? void 0 : _e2.get(o)) || 0, b = p > 0 ? `_r${+p.toPrecision(6)}` : "_r0";
    return { E: n, nu: N, G: B, rho: p, key: `MAT_${Math.round(n)}_n${N.toFixed(4)}${b}` };
  }, X = [], j = [];
  if (R.forEach((o, n) => {
    o.length === 2 ? Q.push(n) : o.length === 8 ? j.push(n) : X.push(n);
  }), Q.length > 0) {
    S('TABLE:  "CONNECTIVITY - FRAME"');
    for (const o of Q) {
      const n = R[o];
      S(`   Frame=${o + 1}   JointI=${n[0] + 1}   JointJ=${n[1] + 1}   IsCurved=No`);
    }
    D();
  }
  if (X.length > 0) {
    ...
    for (const o of X) {
      const n = R[o], s = n.map((E, N) => `Joint${N + 1}=${E + 1}`).join("   ");
      S(`   Area=${o + 1}   NumJoints=${n.length}   ${s}`);
    }
    D();
  }
  if (j.length > 0) {
    ...
    for (const o of j) {
      const n = R[o], s = [n[0], n[1], n[3], n[2], n[4], n[5], n[7], n[6]];
  ...
  for (const o of Q) {
    ...
    if (g.cftAs !== "general" && (f == null ? void 0 : f.type) === "CFT" && B > 0 && (x || f.b > 0 && f.h > 0 && f.tw > 0 && f.tw < Math.min(f.b, f.h) / 2)) {
      const k = x ? f.d - 2 * f.tw : 0, se = x ? 0 : f.b - 2 * f.tw, Z = x ? 0 : f.h - 2 * (f.tf ?? f.tw), ce = x ? Math.PI * (f.d * f.d - k * k) / 4 : f.b * f.h - se * Z, Me = x ? Math.PI * k * k / 4 : se * Z, Ae = (f.fillE > 0 ? f.fillE / B : Math.max(0.01, Math.min(1, (n - ce) / Me))) * B, Le = 0.2, ge = f.fillRho ?? 2.4, Ie = `FILL_${Math.round(Ae)}_r${ge}`;
    ...
    const ee = `A${n.toPrecision(6)}_Iz${s.toPrecision(6)}_s${p.toPrecision(6)}_${b.toPrecision(6)}${O ? O.D ? `_SDC${O.D}x${O.t}` : `_SD${O.b}x${O.h}x${O.t}` : ""}${U ? `_P${U.kind}${U.t3}x${U.t2}x${U.tf}x${U.tw}x${U.t2b ?? ""}x${U.tfb ?? ""}x${U.dis ?? ""}` : ""}`;
    if (!q.has(ee)) {
      let k = 0.3, se = 0.3;
      n > 0 && s > 0 && (k = Math.sqrt(12 * s / n), se = n / k), q.set(ee, { A: n, Iz: s, Iy: E, J: N, b: se, h: k, matKey: T, As2: p > 0 ? p : n * 5 / 6, As3: b > 0 ? b : n * 5 / 6, sd: O, param: U });
    }
  ...
  if (Q.length > 0) {
    ...
    for (const o of Q) {
      const n = le.get(o) || "SEC1";
      S(`   Frame=${o + 1}   AutoSelect=N.A.   AnalSect=${n}   MatProp=Default`);
    }
    D();
  }
  if (q.size > 0) {
    ...
    for (const [, n] of q) {
      if (o++, n.sd) {
        S(`   SectionName=SEC${o}   Material=${n.matKey}   Shape="SD Section"   Area=${$(n.A)}   TorsConst=${$(n.J)}   I33=${$(n.Iz)}   I22=${$(n.Iy)}   I23=0   AS2=${$(n.As2)}   AS3=${$(n.As3)} _`), S("        Color=Cyan   FromFile=No   AMod=1   A2Mod=1   A3Mod=1   JMod=1   I2Mod=1   I3Mod=1   MMod=1   WMod=1");
        continue;
      }
      if (n.param) {
        const s = n.param, E = s.kind === "I" ? `Shape="I/Wide Flange"   t3=${$(s.t3)}   t2=${$(s.t2)}   tf=${$(s.tf)}   tw=${$(s.tw)}   t2b=${$(s.t2b)}   tfb=${$(s.tfb)}` : s.kind === "C" ? `Shape=Channel   t3=${$(s.t3)}   t2=${$(s.t2)}   tf=${$(s.tf)}   tw=${$(s.tw)}` : s.kind === "2L" ? `Shape="Double Angle"   t3=${$(s.t3)}   t2=${$(s.t2)}   SngAngWid=${$((s.t2 - (s.dis ?? 0)) / 2)}   tf=${$(s.tf)}   tw=${$(s.tw)}   dis=${$(s.dis ?? 0)}` : `Shape=Box/Tube   t3=${$(s.t3)}   t2=${$(s.t2)}   tf=${$(s.tf)}   tw=${$(s.tw)}`;
        S(`   SectionName=SEC${o}   Material=${n.matKey}   ${E}   FilletRadius=0   Area=${$(n.A)}   TorsConst=${$(n.J)}   I33=${$(n.Iz)}   I22=${$(n.Iy)}   I23=0   AS2=${$(n.As2)}   AS3=${$(n.As3)} _`), S("        Color=Red   FromFile=No   AMod=1   A2Mod=1   A3Mod=1   JMod=1   I2Mod=1   I3Mod=1   MMod=1   WMod=1");
        continue;
      }
      S(`   SectionName=SEC${o}   Material=${n.matKey}   Shape=General   t3=${$(n.h)}   t2=${$(n.b)}   Area=${$(n.A)}   TorsConst=${$(n.J)}   I33=${$(n.Iz)}   I22=${$(n.Iy)}   I23=0   AS2=${$(n.As2)}   AS3=${$(n.As3)} _`), S("        Color=Blue   FromFile=No   AMod=1   A2Mod=1   A3Mod=1   JMod=1   I2Mod=1   I3Mod=1   MMod=1   WMod=1");
    }
    D();
  }
  const ae = [...q.values()].map((o, n) => ({ sec: o, name: `SEC${n + 1}` })).filter((o) => o.sec.sd);
  if (ae.length > 0) {
    ...
    D();
    const o = ae.filter((s) => !s.sec.sd.D), n = ae.filter((s) => s.sec.sd.D);
    if (o.length > 0) {
  ...
  }
  {
    const o = Q.filter((n) => {
      var _a2;
      const s = (_a2 = m.localAngles) == null ? void 0 : _a2.get(n);
      return s !== void 0 && isFinite(s) && Math.abs(s) > 1e-9;
    });
    if (o.length > 0) {
      S('TABLE:  "FRAME LOCAL AXES ASSIGNMENTS 1 - TYPICAL"');
      for (const n of o) S(`   Frame=${n + 1}   Angle=${$(m.localAngles.get(n))}   AdvanceAxes=No`);
      D();
    }
  }
  {
    const o = m.endOffsets, n = Q.filter((s) => {
  ...
  if (!oe) for (const o of X) {
    const n = ((_i = m.thicknesses) == null ? void 0 : _i.get(o)) || 0.1;
    (_j = m.elasticities) == null ? void 0 : _j.get(o);
    const s = K(o).key, E = W(o) ? 2 : ((_k = m.plateFormulations) == null ? void 0 : _k.get(o)) ?? 0, N = `t${n.toPrecision(6)}_f${E}`;
    Ee.has(N) || Ee.set(N, { t: n, matKey: s, formulacion: E });
  ...
  for (let o = 0; o < A.length; o++) {
    const n = A[o];
    S(`   Joint=${o + 1}   CoordSys=GLOBAL   CoordType=Cartesian   XorR=${$(n[0])}   Y=${$(n[1])}   Z=${$(n[2])}   SpecialJt=No`);
  }
  if (D(), P.supports && P.supports.size > 0) {
    S('TABLE:  "JOINT RESTRAINT ASSIGNMENTS"');
    for (const [o, n] of P.supports) {
      if (!n.some((E) => E)) continue;
      const s = (E) => E ? "Yes" : "No";
      S(`   Joint=${o + 1}   U1=${s(n[0])}   U2=${s(n[1])}   U3=${s(n[2])}   R1=${s(n[3])}   R2=${s(n[4])}   R3=${s(n[5])}`);
    }
    D();
  }
  {
    const o = g.patrones ? m.areaSpringsExport : void 0, n = Bt(A, R, P.springs, { sinArea: !!(o == null ? void 0 : o.size) }).nodales;
    ...
    if (n.size > 0) {
      ...
      for (const [s, E] of [...n].sort((N, B) => N[0] - B[0])) S(`   Joint=${s + 1}   CoordSys=Global   U1=${$(E[0])}   U2=${$(E[1])}   U3=${$(E[2])}   R1=${$(E[3])}   R2=${$(E[4])}   R3=${$(E[5])}`);
      D();
    }
  }
  ...
  if (G && G.size > 0) {
    ...
    const n = [...o].filter(([, s]) => s.length >= 2);
    if (n.length > 0) {
      S('TABLE:  "CONSTRAINT DEFINITIONS - DIAPHRAGM"');
      for (const [s] of n) S(`   Name=DIAPH${s}   CoordSys=GLOBAL   Axis=Z`);
      D(), S('TABLE:  "JOINT CONSTRAINT ASSIGNMENTS"');
      for (const [s, E] of n) for (const N of E) S(`   Joint=${N + 1}   Constraint=DIAPH${s}`);
      D();
    }
  }
  const M = P.cargasPorPatron;
  if (g.patrones && M) {
    const o = m.frameLoadsPorPatron ?? {}, n = [.../* @__PURE__ */ new Set([...Object.keys(M), ...Object.keys(o)])], s = (p) => /^dead$/i.test(p) ? "Dead" : /^(dne|sdead|scm|superdead)$/i.test(p) ? '"Super Dead"' : /^(live|viva|l)$/i.test(p) ? "Live" : "Other", E = m.selfWeight ?? 0;
    S('TABLE:  "LOAD PATTERN DEFINITIONS"');
    for (const p of n) S(`   LoadPat=${p}   DesignType=${s(p)}   SelfWtMult=${/^dead$/i.test(p) ? $(E) : 0}`);
    D(), S('TABLE:  "LOAD CASE DEFINITIONS"');
    for (const p of n) S(`   Case=${p}   Type=LinStatic   InitialCond=Zero   DesTypeOpt="Prog Det"   DesignType=${s(p)}   DesActOpt="Prog Det"   DesignAct=Non-Composite   AutoType=None   RunCase=Yes`);
    D(), S('TABLE:  "CASE - STATIC 1 - LOAD ASSIGNMENTS"');
    for (const p of n) S(`   Case=${p}   LoadType="Load pattern"   LoadName=${p}   LoadSF=1`);
    D();
    const N = [];
    for (const p of n) for (const [b, f] of M[p] ?? /* @__PURE__ */ new Map()) f.some((O) => Math.abs(O) > 1e-12) && N.push(`   Joint=${b + 1}   LoadPat=${p}   CoordSys=GLOBAL   F1=${$(f[0])}   F2=${$(f[1])}   F3=${$(f[2])}   M1=${$(f[3])}   M2=${$(f[4])}   M3=${$(f[5])}`);
    N.length && (S('TABLE:  "JOINT LOADS - FORCE"'), N.forEach(S), D());
    const B = [];
    for (const p of n) for (const [b, f] of o[p] ?? /* @__PURE__ */ new Map()) {
      const O = R[b];
    ...
    const n = m.frameLoads, s = /* @__PURE__ */ new Map();
    if ((_m = P.loads) == null ? void 0 : _m.forEach((N, B) => s.set(B, [...N])), n && n.size > 0) {
      const N = (B, T) => {
  ...
  for (let o = 0; o < R.length; o++) {
    const { E: n, nu: s, G: E, rho: N, key: B } = K(o);
    l.has(B) || l.set(B, { E: n, nu: s, G: E, rho: N });
  }
  if (j.length > 0) {
    const o = m.solidIncompatible === false ? "No" : "Yes", n = /* @__PURE__ */ new Map();
    for (const s of j) {
      const { E, nu: N, G: B, rho: T, key: p } = K(s);
      l.has(p) || l.set(p, { E, nu: N, G: B, rho: T }), n.has(p) || n.set(p, `SOL${n.size + 1}`);
    }
    S('TABLE:  "SOLID PROPERTY DEFINITIONS"');
    for (const [s, E] of n) S(`   SolidProp=${E}   Material=${s}   MatAngleA=0   MatAngleB=0   MatAngleC=0   InComp=${o}   Color=Yellow`);
    D(), S('TABLE:  "SOLID PROPERTY ASSIGNMENTS"');
    for (const s of j) S(`   Solid=${s + 1}   SolidProp=${n.get(K(s).key)}`);
    D();
  }
  for (const [o, n] of te) l.has(o) || l.set(o, n);
  ...
  for (const [o, n] of l) S(`   Material=${o}   UnitWeight=${$(n.rho * 9.80665)}   UnitMass=${$(n.rho)}   E1=${$(n.E)}   G12=${$(n.G)}   U12=${$(n.nu)}   A1=9.9E-06`);
  ...
  return D(), S('TABLE:  "PROGRAM CONTROL"'), S(`   ProgramName=SAP2000   Version=24.1.0   CurrUnits="${H.force}, ${H.length}, C"   SteelCode="AISC 360-16"   ConcCode="ACI 318-19"   AlumCode="AA 2015"   ColdCode=AISI-16   RegenHinge=Yes`), D(), S("END TABLE DATA"), S(""), re.join(`\r
`);
}
function $(g) {
...
function es(g, A, R = 0.05) {
  const P = A.map(([m, H]) => `${(+m).toFixed(4)} ${(+H).toFixed(5)}`).join("  ");
  return [`  FUNCTION "${g}"  FUNCTYPE "SPECTRUM"  DAMPRATIO ${R}  SPECTYPE "USER"  `, `  FUNCTION "${g}"  TIMEVAL "${P}"  `];
}
function ts(g) {
  const { name: A, func: R, modalCase: P = "Modal", sfX: m = 9.81, sfY: H = 9.81 } = g, J = [`  LOADCASE "${A}"  TYPE  "Response Spectrum"  MODALCASE  "${P}"  `];
  return m && J.push(`  LOADCASE "${A}"  ACCEL  "U1"  FUNC  "${R}"  SF  ${m}  `), H && J.push(`  LOADCASE "${A}"  ACCEL  "U2"  FUNC  "${R}"  SF  ${H}  `), J;
}
function Pt(g) {
...
`) ? `\r
` : `
`, P = g.split(/\r?\n/), m = A.name ?? "NEC", H = es(m, A.points, A.dampRatio ?? 0.05), J = A.modalCase ?? "Modal", re = ts({ name: A.caseName ?? "Sismo NEC", func: m, modalCase: J, sfX: A.sfX, sfY: A.sfY });
  let S = [];
  ...
  return Ft(P, "FUNCTIONS", H), Ft(P, "LOAD CASES", [...S, ...re]), P.join(R);
}
function Ft(g, A, R) {
  ...
  const m = g.findIndex((J) => J.trim() === "END"), H = m >= 0 ? m : g.length;
  g.splice(H, 0, `$ ${A}`, ...R, "");
}
function os(g, A) {
  ...
  for (const m of P) {
    ...
    if (!(!H || H.length === 0)) {
      R.push(`$ ${m}`);
      for (const J of H) R.push(J);
      R.push("");
    }
  }
  for (const [m, H] of g) if (!P.includes(m) && H.length !== 0) {
    R.push(`$ ${m}`);
    for (const J of H) R.push(J);
    R.push("");
  }
...
function ns(g) {
  ...
  const { nodes: A, elements: R, nodeInputs: P, elementInputs: m, title: H, units: J } = g, re = g.shellLoads ?? m.shellSurfaceLoads;
  let S;
  ...
  const D = g.shellAngles ?? m.shellAngles, Q = m.cargaDeArea, K = !!(S && S.size > 0), X = m.selfWeight, j = m.frameLoads, q = (g.weightMode ?? "auto") === "auto" && X !== void 0, te = /* @__PURE__ */ new Map(), le = (e, t) => {
    ...
    te.set(e, a.map((c, r) => c + t[r]));
  }, ae = /* @__PURE__ */ new Set();
  if (q) {
    if (j) for (const [e, t] of j) {
      const a = R[e];
      if (!a || a.length !== 2) continue;
      const c = A[a[0]], r = A[a[1]], d = [r[0] - c[0], r[1] - c[1], r[2] - c[2]], I = Math.hypot(d[0], d[1], d[2]);
      if (I < 1e-9) continue;
      const u = [d[0] / I, d[1] / I, d[2] / I], F = I * I / 12, v = [u[1] * t[2] - u[2] * t[1], u[2] * t[0] - u[0] * t[2], u[0] * t[1] - u[1] * t[0]];
      le(a[0], [t[0] * I / 2, t[1] * I / 2, t[2] * I / 2, F * v[0], F * v[1], F * v[2]]), le(a[1], [t[0] * I / 2, t[1] * I / 2, t[2] * I / 2, -F * v[0], -F * v[1], -F * v[2]]), ae.add(e);
    }
    if (X && X > 0) {
      ...
      R.forEach((a, c) => {
        ...
        if (r) {
          if (a.length === 2) {
            const d = ((_b2 = m.areas) == null ? void 0 : _b2.get(c)) ?? 0, I = A[a[0]], u = A[a[1]], F = [u[0] - I[0], u[1] - I[1], u[2] - I[2]];
            let v = Math.hypot(F[0], F[1], F[2]);
            const L = t == null ? void 0 : t.get(c);
            if (L) {
              const y = Math.hypot(F[0], F[1]);
              y > 1e-9 && Math.abs(Math.atan2(Math.abs(F[2]), y)) * 180 / Math.PI < 20 && (v = Math.max(v - L[0] - L[1], 0));
            }
            const w = d * v * r * 9.80665 * X;
            le(a[0], [0, 0, -w / 2, 0, 0, 0]), le(a[1], [0, 0, -w / 2, 0, 0, 0]);
          } else if (a.length === 4) {
            const d = ((_c2 = m.thicknesses) == null ? void 0 : _c2.get(c)) ?? 0, I = a.map((y) => A[y]);
            let u = 0, F = 0, v = 0;
            for (let y = 0; y < 4; y++) {
              const V = I[y], z = I[(y + 1) % 4];
              u += V[1] * z[2] - V[2] * z[1], F += V[2] * z[0] - V[0] * z[2], v += V[0] * z[1] - V[1] * z[0];
            }
            const L = Math.hypot(u, F, v) / 2, w = d * L * r * 9.80665 * X;
            for (const y of a) le(y, [0, 0, -w / 4, 0, 0, 0]);
          }
        }
      });
    }
  ...
  const oe = (e, t) => {
    const a = te.get(e);
    return [t[0] - ((a == null ? void 0 : a[0]) ?? 0), t[1] - ((a == null ? void 0 : a[1]) ?? 0), t[2] - (K ? (Q == null ? void 0 : Q.get(e)) ?? 0 : 0) - ((a == null ? void 0 : a[2]) ?? 0)];
  }, ie = (e, t) => {
    const a = te.get(e);
    return [(t[3] ?? 0) - ((a == null ? void 0 : a[3]) ?? 0), (t[4] ?? 0) - ((a == null ? void 0 : a[4]) ?? 0), (t[5] ?? 0) - ((a == null ? void 0 : a[5]) ?? 0)];
  }, Ee = "N", Se = "MM", i = [], h = (e) => Math.round(e * 1e4) / 1e4, C = (e) => !isFinite(e) || e === 0 ? "0" : Number(e.toPrecision(10)).toString(), W = 1e3, G = 1e3, M = (e) => e * G, l = (e) => e * W, o = (e) => e * W, n = (e) => e * W * G, s = (e) => e * W / G ** 2, E = (e) => e * W / G ** 3, N = /* @__PURE__ */ new Date(), B = `${N.getMonth() + 1}/${N.getDate()}/${N.getFullYear()}  ${N.getHours()}:${String(N.getMinutes()).padStart(2, "0")}:${String(N.getSeconds()).padStart(2, "0")}`;
  i.push(`$ File   "Hekatan_export.e2k"  saved ${B} in ETABS 22.6.0`), i.push(""), i.push("$ PROGRAM INFORMATION"), i.push('  PROGRAM  "ETABS"  VERSION "22.6.0"  '), i.push(""), i.push("$ CONTROLS"), i.push(`  UNITS  "${Ee}"  "${Se}"  "C"  `), i.push('  TITLE1  "Hekatan Struct Lineal export"  '), H && i.push(`  TITLE2  "${H}"  `), i.push("  PREFERENCE  MERGETOL 0.001"), i.push('  RLLF  METHOD "ASCE7-10"  USEDEFAULTMIN "YES"  '), i.push("");
  ...
  const Me = 980665e-8, pe = (e) => {
    ...
    const a = (_b2 = m.densities) == null ? void 0 : _b2.get(e);
    if (a === void 0) return;
    const c = a > 100 ? a * Me : a * 9.80665, r = (_c2 = m.deckSections) == null ? void 0 : _c2.get(e);
    if (r && r.tc > 0) {
    ...
    var _a2;
    const t = ((_a2 = m.elasticities) == null ? void 0 : _a2.get(e)) ?? 0, a = pe(e);
  ...
  let Gt = 0, Yt = 0;
  for (const e of Le) {
    const t = parseFloat(e.split("|")[0]), a = e.split("|")[1], c = t >= 1e8, r = c ? `Steel_${++Gt}` : `Conc_${++Yt}`;
    ge.set(e, r), Ie.set(e, c);
    const d = a !== "-" ? parseFloat(a) : c ? 76.97 : 24, I = s(t), u = E(d), F = (() => {
      const w = g.elementInputs.poissonsRatios;
      if (w) {
        for (const [y, V] of w) if (Ae(y) === e) return V;
      }
    })(), v = F !== void 0 ? F : c ? 0.3 : 0.2, L = c ? 117e-7 : 1e-5;
    if (c) {
      i.push(`  MATERIAL  "${r}"    TYPE "Steel"    GRADE "Grade 50"    WEIGHTPERVOLUME ${C(u)}`), i.push(`  MATERIAL  "${r}"    SYMTYPE "Isotropic"  E ${h(I)}  U ${v}  A ${L}`);
      const w = 345e3, y = 45e4;
      i.push(`  MATERIAL  "${r}"  FY ${h(s(w))}  FU ${h(s(y))}  FYE ${h(s(w * 1.1))}  FUE ${h(s(y * 1.1))}`);
    } else i.push(`  MATERIAL  "${r}"    TYPE "Concrete"    WEIGHTPERVOLUME ${C(u)}`), i.push(`  MATERIAL  "${r}"    SYMTYPE "Isotropic"  E ${h(I)}  U ${v}  A ${L}`), i.push(`  MATERIAL  "${r}"    FC ${h(s(24e3))}`);
  }
  const rt = /* @__PURE__ */ new Map();
  {
    ...
    (_b = m.sectionShapes) == null ? void 0 : _b.forEach((a, c) => {
      var _a2;
      if ((a == null ? void 0 : a.type) !== "CFT" || !(a.fillE > 0) || !((((_a2 = m.elasticities) == null ? void 0 : _a2.get(c)) ?? 0) > 0)) return;
      const d = (a.fillRho ?? 2.4) * 9.80665, I = `${a.fillE}|${d.toFixed(4)}`;
      let u = e.get(I);
      u || (u = `ConcFill_${e.size + 1}`, e.set(I, u), i.push(`  MATERIAL  "${u}"    TYPE "Concrete"    WEIGHTPERVOLUME ${C(E(d))}`), i.push(`  MATERIAL  "${u}"    SYMTYPE "Isotropic"  E ${h(s(a.fillE))}  U 0.2  A 1.0e-5`), i.push(`  MATERIAL  "${u}"    FC ${h(s(24e3))}`)), rt.set(c, u);
    });
  }
  ...
  R.forEach((e, t) => {
    ...
    const a = (_a2 = m.sectionShapes) == null ? void 0 : _a2.get(t), c = ((_b2 = m.elasticities) == null ? void 0 : _b2.get(t)) ?? 0, r = ge.get(Ae(t)) || "Conc_1", d = Ie.get(Ae(t)) ?? c >= 1e8, I = ((_c2 = m.areas) == null ? void 0 : _c2.get(t)) ?? 0, u = ((_d2 = m.momentsOfInertiaZ) == null ? void 0 : _d2.get(t)) ?? 0, F = ((_e3 = m.momentsOfInertiaY) == null ? void 0 : _e3.get(t)) ?? 0, v = ((_f2 = m.torsionalConstants) == null ? void 0 : _f2.get(t)) ?? 0;
    let L = (a == null ? void 0 : a.type) || "rect", w = (a == null ? void 0 : a.h) ?? 0, y = (a == null ? void 0 : a.b) ?? 0, V = (a == null ? void 0 : a.d) ?? 0;
    const z = (a == null ? void 0 : a.tf) ?? 0, ne = (a == null ? void 0 : a.tw) ?? 0;
    if (!a && w <= 0 && y <= 0 && V <= 0 && I > 0 && u > 0 && F > 0) {
    ...
    const ze = (a == null ? void 0 : a.name) ? `NAME_${a.name}` : `${L}_${h(w)}_${h(y)}_${h(V)}_${h(z)}_${h(ne)}_${r}`;
    (a == null ? void 0 : a.name) && !Ye.has(ze) && Ye.set(ze, a.name);
    let fe = Ye.get(ze);
    if (!fe) {
      const de = d ? "S" : "C";
    ...
    const Je = a, Kt = !((Je == null ? void 0 : Je.t2b) > 0) || Math.abs(Je.t2b - y) < 1e-9 && Math.abs((Je.tfb ?? z) - z) < 1e-9;
    ...
    if ($e === "General") {
      ...
      Oe += `  D ${h(M(w))} B ${h(M(y))} AREA ${C(I * 1e6)} AS2 ${C(de * 1e6)} AS3 ${C(Be * 1e6)} I33 ${C(u * 1e12)} I22 ${C(F * 1e12)} TORSION ${C((v || u + F) * 1e12)} S33POS ${C(2 * u / w * 1e9)} S33NEG ${C(2 * u / w * 1e9)} S22POS ${C(2 * F / y * 1e9)} S22NEG ${C(2 * F / y * 1e9)} Z33 ${C(2 * u / w * 1e9)} Z22 ${C(2 * F / y * 1e9)} R33 ${C(Math.sqrt(u / I) * 1e3)} R22 ${C(Math.sqrt(F / I) * 1e3)} `, i.push(Oe);
      return;
    }
  ...
  let kt = 0;
  A.forEach((e) => {
    const { dz: t } = ce(e[2]), a = `${h(e[0])},${h(e[1])},${t}`;
    ke.has(a) || ke.set(a, `${++kt}`);
  });
  const xe = /* @__PURE__ */ new Map(), Ze = [];
  {
    ...
    for (const [a, c] of e) {
      ...
      if (!I) {
        ...
        const u = ["UX", "UY", "UZ", "RX", "RY", "RZ"], F = r.map((v, L) => `${u[L]}  ${+v.toPrecision(12)}`);
        Ze.push(`  POINTSPRING  "${I}"  NONLINEARSPECOPTION  "LINKS"  ${F.join(" ")} `);
      }
      xe.set(a, I);
    }
    Ze.length && (i.push("$ POINT SPRING PROPERTIES"), Ze.forEach((a) => i.push(a)), i.push(""));
  }
  i.push("$ POINT COORDINATES");
  for (const [e, t] of ke) {
    const [a, c, r] = e.split(",").map(Number);
    i.push(r ? `  POINT "${t}"  ${h(M(a))} ${h(M(c))} ${h(M(r))} ` : `  POINT "${t}"  ${h(M(a))} ${h(M(c))} `);
  }
  i.push("");
  const Te = (e) => {
    const t = A[e], { story: a, dz: c } = ce(t[2]), r = `${h(t[0])},${h(t[1])},${c}`;
    return { pt: ke.get(r) || "1", story: a };
  }, lt = (e) => {
    var _a2, _b2, _c2, _d2, _e3, _f2;
    const t = [], a = (_a2 = g.propertyModifiers) == null ? void 0 : _a2.get(e);
    a && a.some((L) => Math.abs(L - 1) > 1e-9) && t.push(`PROPMODIFIERS "${a.map((L) => h(L)).join(" ")}"`);
    ...
    const I = (_e3 = m.rigidOffsets) == null ? void 0 : _e3.get(e), u = (_f2 = m.endOffsets) == null ? void 0 : _f2.get(e), F = u ? [u[0], u[1]] : I, v = u && u.length > 2 ? u[2] : 0;
    return F && (Math.abs(F[0]) > 1e-9 || Math.abs(F[1]) > 1e-9) && t.push(`LENGTHOFFI ${h(M(F[0]))} LENGTHOFFJ ${h(M(F[1]))} RIGIDZONE ${h(v)}`), t.length > 0 ? ` ${t.join(" ")} ` : "";
  ...
  R.forEach((e, t) => {
    ...
    const d = Te(c), I = _e.get(t) || `Sec_${t}`, u = `${d.pt}_${I}_${a}`;
    Ue.has(u) || Ue.set(u, []), Ue.get(u).push({ i: t, bot: c, top: r, zBot: h(A[c][2]), zTop: h(A[r][2]), planPt: d.pt, secName: I, type: a });
  }), Ue.forEach((e, t) => {
    e.sort((c, r) => c.zBot - r.zBot);
    let a = 0;
    for (let c = 1; c <= e.length; c++) if (c === e.length || Math.abs(e[c].zBot - e[c - 1].zTop) > 1e-6) {
      const d = e.slice(a, c);
      d.length >= 1 && (Xe.push({ elemIndices: d.map((I) => I.i), planPt: d[0].planPt, bottomNodeIdx: d[0].bot, topNodeIdx: d[d.length - 1].top, secName: d[0].secName, type: d[0].type, nSegments: d.length }), d.forEach((I) => ft.add(I.i))), a = c;
    }
  }), i.push("$ LINE CONNECTIVITIES");
  const Ke = [], He = (e) => se.indexOf(e), Et = /* @__PURE__ */ new Map(), St = (e, t, a, c, r, d, I, u) => {
    const F = Te(c), v = Te(a);
    u !== void 0 && Et.set(u, { name: e, story: F.story });
    const L = He(F.story) - He(v.story);
    L <= 0 ? i.push(`  LINE  "${e}"  BEAM  "${v.pt}"  "${F.pt}"  0`) : i.push(`  LINE  "${e}"  ${t}  "${v.pt}"  "${F.pt}"  ${L}`);
  ...
  Xe.forEach((e, t) => {
    const a = lt(e.elemIndices[0]), c = [];
    let r = [];
    e.elemIndices.forEach((d, I) => {
      r.push(d);
      const [u, F] = R[d], v = A[u][2] >= A[F][2] ? u : F;
      (ce(A[v][2]).dz === 0 || I === e.elemIndices.length - 1) && (c.push(r), r = []);
    }), c.forEach((d) => {
      const [I, u] = R[d[0]], F = A[I][2] <= A[u][2] ? I : u, [v, L] = R[d[d.length - 1]], w = A[v][2] >= A[L][2] ? v : L;
      He(Te(w).story) - He(Te(F).story);
      let y = `C${t + 1}`;
      for (let V = 1; ; V++) {
        const z = i.length;
        St(y, e.type, F, w, e.secName, a, d.length);
    ...
    const a = bt(A, e), c = _e.get(t) || `Sec_${t}`, r = lt(t), d = A[e[0]][2] <= A[e[1]][2] ? e[0] : e[1], I = A[e[0]][2] <= A[e[1]][2] ? e[1] : e[0];
    St(`E${t + 1}`, a === "BEAM" ? "BRACE" : a, d, I, c, r, 3, t);
  }), i.push("");
  ...
  i.push("$ POINT ASSIGNS"), (_c = P.supports) == null ? void 0 : _c.forEach((e, t) => {
    ...
    if (e[0] && a.push("UX"), e[1] && a.push("UY"), e[2] && a.push("UZ"), e[3] && a.push("RX"), e[4] && a.push("RY"), e[5] && a.push("RZ"), a.length > 0) {
      ...
      i.push(`  POINTASSIGN  "${c.pt}"  "${c.story}"  RESTRAINT "${a.join(" ")}" ${r}${d} `), me.add(`${c.pt}@${c.story}`);
    }
  });
  for (const [e, t] of xe) {
    const a = Te(e);
    me.has(`${a.pt}@${a.story}`) || (i.push(`  POINTASSIGN  "${a.pt}"  "${a.story}"  SPRINGPROP "${t}" `), me.add(`${a.pt}@${a.story}`));
  }
  ...
  P.diaphragms && P.diaphragms.forEach((e, t) => {
    ...
    const t = Te(e), a = `${t.pt}@${t.story}`;
    !me.has(a) && t.story !== "Base" && (i.push(`  POINTASSIGN  "${t.pt}"  "${t.story}"  DIAPH "D1"  `), me.add(a));
  }) : Ve && Xe.forEach((e) => {
    for (const t of e.elemIndices) {
      const [a, c] = R[t], r = A[a][2] >= A[c][2] ? a : c, d = Te(r), I = `${d.pt}@${d.story}`;
  ...
  At == null ? void 0 : At.forEach((e) => e.cells.forEach((t) => Tt.add(t))), R.forEach((e, t) => {
    if (e.length === 4 || e.length === 3) {
      const a = A[e[0]], c = A[e[1]], r = A[e[2]], d = [c[0] - a[0], c[1] - a[1], c[2] - a[2]], I = [r[0] - a[0], r[1] - a[1], r[2] - a[2]], u = d[1] * I[2] - d[2] * I[1], F = d[2] * I[0] - d[0] * I[2], v = d[0] * I[1] - d[1] * I[0], L = Math.sqrt(u * u + F * F + v * v), w = L > 1e-10 && Math.abs(v) / L < 0.5;
  ...
  let qe = false;
  const Qe = [], Mt = (e) => {
    const t = g.elementInputs.plateFormulations, a = he.find((r) => r.isWall === e), c = t && a ? t.get(a.idx) : void 0;
    return c === 2 ? "Membrane" : c === 1 ? "ShellThin" : "ShellThick";
  }, It = (e, t) => {
    const a = g.elementInputs.thicknesses, c = he.find((r) => r.isWall === e);
    return (c ? a == null ? void 0 : a.get(c.idx) : void 0) ?? (a == null ? void 0 : a.values().next().value) ?? t;
  }, mt = ["F11MOD", "F22MOD", "F12MOD", "M11MOD", "M22MOD", "M12MOD", "V13MOD", "V23MOD"], ve = (e) => {
    var _a2;
    ...
    const a = he.filter((I) => I.isWall === t), c = /* @__PURE__ */ new Map();
    for (const I of a) {
      const u = ve(I.idx) ?? [1, 1, 1, 1, 1, 1, 1, 1];
    ...
    var _a2;
    const t = gt == null ? void 0 : gt.get(e.idx), a = Nt == null ? void 0 : Nt.get(e.idx), c = ve(e.idx), r = (_a2 = g.elementInputs.deckSections) == null ? void 0 : _a2.get(e.idx), d = r ? [r.tc, r.hr, r.wrt, r.wrb, r.sr, r.w].map((I) => h(I)).join(",") : "-";
    return `${e.isWall ? "W" : "F"}|${t ?? "-"}|${a ?? "-"}|${c ? c.map((I) => h(I)).join(",") : "-"}|${Ae(e.idx)}|${d}`;
  }, et = (e) => {
    var _a2;
  ...
  for (const e of he) {
    ...
    const a = e.isWall, c = !a && et(e.idx), r = a ? ++Ht : c ? ++vt : ++Ut, d = Ae(e.idx);
    ye.set(t, { nombre: (a ? "Muro" : c ? De : "Losa") + (r === 1 ? "" : String(r)), isWall: a, mem: c, t: gt == null ? void 0 : gt.get(e.idx), pf: Nt == null ? void 0 : Nt.get(e.idx), idx: e.idx, mat: ge.get(d) ?? ue, acero: Ie.get(d) ?? false });
  }
  const We = (e) => {
    var _a2;
    return ((_a2 = ye.get(be(e))) == null ? void 0 : _a2.nombre) ?? (e.isWall ? "Muro" : "Losa");
  }, Ot = (e) => e === 2 ? "Membrane" : e === 1 ? "ShellThin" : "ShellThick", Wt = (e, t) => {
    const a = he.find((d) => be(d) === t), c = a ? ve(a.idx) ?? null : null;
    if (!c) return "";
  ...
  const Rt = [...ye.entries()].filter(([e]) => !tt.has(e)), st = (e) => {
    ...
    const a = (c) => C(M(c));
    return `  SHELLPROP  "${e}"  PROPTYPE  "Deck"  DECKTYPE "Filled"  CONCMATERIAL "${ue}"  DECKMATERIAL "${ue}"  DECKSLABDEPTH ${a(t.tc)} DECKRIBDEPTH ${a(t.hr)} DECKRIBWIDTHTOP ${a(t.wrt)} DECKRIBWIDTHBOTTOM ${a(t.wrb)} DECKRIBSPACING ${a(t.sr)} DECKSHEARTHICKNESS ${a(76e-5)} DECKUNITWEIGHT ${C(zt(t.w))} SHEARSTUDDIAM ${a(0.019)} SHEARSTUDHEIGHT ${a(0.1)} SHEARSTUDFU 400 `;
  };
  if (he.some((e) => !e.isWall)) {
    ...
    if (qe) {
      ...
      const a = [...ye.values()].find((r) => r.nombre === De), c = st(we == null ? void 0 : we.idx);
      (a == null ? void 0 : a.acero) ? i.push(`  SHELLPROP  "${De}"  PROPTYPE  "Slab"  MATERIAL "${a.mat}"  MODELINGTYPE "Membrane"  SLABTYPE "Slab"  SLABTHICKNESS ${h(M(e))} `) : c ? i.push(Dt(De, c)) : i.push(`  SHELLPROP  "${De}"  PROPTYPE  "Slab"  MATERIAL "${ue}"  MODELINGTYPE "Membrane"  SLABTYPE "Slab"  SLABTHICKNESS ${h(M(e))} `);
  ...
  if (he.some((e) => e.isWall)) {
    ...
    const a = ut("Muro", true);
    a && i.push(a), i.push("");
  }
  if (Rt.length) {
    i.push("$ OTRAS SECCIONES DE CASCARA");
    for (const [e, t] of Rt) {
      const a = t.t ?? (t.isWall ? 0.2 : 0.15);
      i.push(t.isWall ? `  SHELLPROP  "${t.nombre}"  PROPTYPE  "Wall"  MATERIAL "${t.mat ?? ue}"  MODELINGTYPE "${Ot(t.pf)}"  WALLTHICKNESS ${h(M(a))} ` : t.mem && t.acero ? `  SHELLPROP  "${t.nombre}"  PROPTYPE  "Slab"  MATERIAL "${t.mat}"  MODELINGTYPE "Membrane"  SLABTYPE "Slab"  SLABTHICKNESS ${h(M(a))} ` : t.mem && st(t.idx) ? Dt(t.nombre, st(t.idx)) : t.mem ? `  SHELLPROP  "${t.nombre}"  PROPTYPE  "Slab"  MATERIAL "${ue}"  MODELINGTYPE "Membrane"  SLABTYPE "Slab"  SLABTHICKNESS ${h(M(a))} ` : `  SHELLPROP  "${t.nombre}"  PROPTYPE  "Slab"  MATERIAL "${ue}"  MODELINGTYPE "${Ot(t.pf)}"  SLABTYPE "Slab"  SLABTHICKNESS ${h(M(a))} `);
  ...
  if (he.length > 0) {
    ...
    const e = [];
    he.forEach((t, a) => {
      const { el: c, isWall: r } = t, d = r ? `W${a + 1}` : `F${a + 1}`, I = r ? "PANEL" : "FLOOR", u = c.map((F) => Te(F));
      if (r) {
        ...
        const F = u.length, v = (z) => se.indexOf(z), L = Math.max(...u.map((z) => v(z.story))), w = u.map((z) => L - v(z.story)), y = se[L] ?? u[0].story;
  ...
  for (const e of Re) {
    let t;
    e.type === "Dead" ? t = Pe === "manual" ? 0 : e.selfWeightMultiplier ?? X ?? 1 : (t = 0, (e.selfWeightMultiplier ?? 0) !== 0 && console.warn(`[e2k] El patron "${e.name}" (tipo ${e.type ?? "Other"}) pedia SELFWEIGHT ${e.selfWeightMultiplier}. Se exporta 0: el peso propio va solo en Dead.`)), i.push(`  LOADPATTERN "${e.name}"  TYPE  "${e.type ?? "Other"}"  SELFWEIGHT  ${t}`);
  }
  i.push("");
  ...
  if (P.loads && P.loads.size > 0 && P.loads.forEach((e, t) => {
    const [a, c, r] = oe(t, e), [d, I, u] = ie(t, e);
    Ct(t, [a, c, _t ? r : 0, d, I, u]);
  }), P.moments && P.moments.size > 0 && P.moments.forEach((e, t) => {
    Ct(t, [0, 0, 0, e[0] ?? 0, e[1] ?? 0, e[2] ?? 0]);
  }), nt.forEach((e, t) => {
    if (e.every((c) => Math.abs(c) <= 1e-10)) return;
    const a = Te(t);
    ot.push(`  POINTLOAD  "${a.pt}"  "${a.story}"  TYPE "FORCE"  LC "${Ce}"  FX ${C(o(e[0]))}  FY ${C(o(e[1]))}  FZ ${C(o(e[2]))}  MX ${C(n(e[3]))}  MY ${C(n(e[4]))}  MZ ${C(n(e[5]))}`);
  }), ot.length > 0 && (i.push("$ POINT OBJECT LOADS"), ot.forEach((e) => i.push(e)), i.push("")), q && ae.size > 0) {
    const e = [];
    for (const t of ae) {
      const a = j.get(t), c = Et.get(t);
      if (!c) continue;
      const r = (d) => C(l(d) / G);
      Math.abs(a[2]) > 1e-12 && e.push(`  LINELOAD  "${c.name}"  "${c.story}"  TYPE "UNIFF"  DIR "${a[2] < 0 ? "GRAV" : "Z"}"  LC "${Ce}"  FVAL ${r(Math.abs(a[2]))}`), Math.abs(a[0]) > 1e-12 && e.push(`  LINELOAD  "${c.name}"  "${c.story}"  TYPE "UNIFF"  DIR "X"  LC "${Ce}"  FVAL ${r(a[0])}`), Math.abs(a[1]) > 1e-12 && e.push(`  LINELOAD  "${c.name}"  "${c.story}"  TYPE "UNIFF"  DIR "Y"  LC "${Ce}"  FVAL ${r(a[1])}`);
    }
  ...
  if (S && S.size > 0 && Qe.length > 0) {
    const e = [];
    for (const t of Qe) {
      const a = $t.get(t.idx), c = a !== void 0 ? { value: a } : S.get(t.idx);
...
function bt(g, A) {
  const R = g[A[0]], P = g[A[1]], m = Math.abs(P[2] - R[2]), H = Math.sqrt((P[0] - R[0]) ** 2 + (P[1] - R[1]) ** 2), J = m > H * 0.5;
