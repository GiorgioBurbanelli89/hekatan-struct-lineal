import { p as co, q as eo, __tla as __tla_0 } from "./aiAgent-CXo11zjw.js";
...
let __tla = Promise.all([
  ...
  function ao(t, n, s, e, r, i) {
    const o = e * i * i * i / (12 * (1 - r * r)), a = [
      ...
      const u = w * _ - y * x, f = _ / u, S = -y / u, D = -x / u, Q = w / u;
      let A = 0, L = 0, E = 0;
      for (let T = 0; T < 4; T++) {
        const j = f * h[T] + S * M[T], p = D * h[T] + Q * M[T], J = s[3 * T + 1], O = s[3 * T + 2];
        A += j * O, L += -p * J, E += p * O - j * J;
      }
      const W = [
        A,
        L,
    ...
    return Y.map(([l, g]) => {
      ...
      return [
        ...
        1,
        2
      ].map((y) => w.reduce((x, _, u) => x + _ * m[u][y], 0));
    });
  }
  ...
  function io(t, n, s, e, r, i) {
    ...
    let M = [], w = [], y = [], x = [], _ = [], u = 0, f = 0, S = 0;
    for (let j = 0; j < 3; j++) for (let p = 0; p < 3; p++) {
      const J = l[j], O = l[p], N = g[j] * g[p], { N: d, dNdxi: Z, dNdeta: R } = $t(J, O);
      let P = 0, I = 0, q = 0, B = 0;
      ...
      for (let v = 0; v < 4; v++) V.push(tt * lt[v] + st * ut[v]), gt.push(C * lt[v] + at * ut[v]);
      const bt = -2 * J * (1 - O * O), pt = -2 * O * (1 - J * J), vt = tt * bt + st * pt, Mt = C * bt + at * pt, St = [], Xt = [], dt = [], ht = [];
      for (let v = 0; v < 4; v++) {
        const et = Y[v];
      ...
      j === 1 && p === 1 && (M = d.slice(), w = it.slice(), y = ct.slice(), x = Xt.slice(), _ = dt.slice(), u = vt, f = Mt, S = Math.abs($));
    }
    const D = s / (2 * (1 + e)), Q = new Array(14).fill(0);
    for (let j = 0; j < 4; j++) Q[3 * j] = -0.5 * y[j], Q[3 * j + 1] = 0.5 * w[j], Q[3 * j + 2] = 0.5 * (_[j] - x[j]) - M[j];
    Q[12] = -0.5 * f, Q[13] = 0.5 * u;
    const A = i * D * r * 4 * S;
    for (let j = 0; j < 14; j++) for (let p = 0; p < 14; p++) h[j][p] += A * Q[j] * Q[p];
    const L = [
      [
        h[12][12],
        h[12][13]
    ...
    for (let j = 0; j < 12; j++) for (let p = 0; p < 12; p++) W[j][p] = h[j][p];
    ...
    for (let j = 0; j < 12; j++) for (let p = 0; p < 12; p++) {
      let J = 0;
      for (let O = 0; O < 2; O++) for (let N = 0; N < 2; N++) J += h[j][12 + O] * T[O][N] * h[12 + N][p];
      W[j][p] -= J;
    }
    return W;
  }
  function fo(t, n, s, e, r) {
    ...
    for (const l of c) {
      ...
      const [u, f, S, D] = x, Q = Jt(2, 12);
      for (let A = 0; A < 12; A++) Q[0][A] = u * _[0][A] + f * _[1][A], Q[1][A] = S * _[0][A] + D * _[1][A];
      m.push(Q);
    }
    for (const [l, g] of Y) {
      const { dNdxi: h, dNdeta: M } = $t(l, g), { dNdx: w, dNdy: y, detJ: x, J: _ } = Ht(h, M, t, n), u = Jt(3, 12);
      for (let p = 0; p < 4; p++) u[0][p * 3 + 1] = w[p], u[1][p * 3 + 2] = y[p], u[2][p * 3 + 1] = y[p], u[2][p * 3 + 2] = w[p];
      for (let p = 0; p < 12; p++) for (let J = 0; J < 12; J++) {
        let O = 0;
        O += o * (u[0][p] * u[0][J] + e * u[0][p] * u[1][J] + e * u[1][p] * u[0][J] + u[1][p] * u[1][J]), O += o * (1 - e) / 2 * u[2][p] * u[2][J], i[p][J] += O * Math.abs(x);
      }
      const f = Jt(2, 12), S = 0.5 * (1 - g), D = 0.5 * (1 + g), Q = 0.5 * (1 - l), A = 0.5 * (1 + l), [L, E, W, T] = _, j = 1 / x;
      for (let p = 0; p < 12; p++) {
        const J = S * m[0][0][p] + D * m[1][0][p], O = Q * m[2][1][p] + A * m[3][1][p];
        f[0][p] = j * (T * J - E * O), f[1][p] = j * (-W * J + L * O);
      }
      for (let p = 0; p < 12; p++) for (let J = 0; J < 12; J++) i[p][J] += b * (f[0][p] * f[0][J] + f[1][p] * f[1][J]) * Math.abs(x);
    }
    return i;
  }
  function lo(t, n, s) {
    ...
    for (let y = 0; y < 12; y++) for (let x = 0; x < 12; x++) {
      let _ = 0;
      const u = y / 3 | 0, f = y % 3, S = x / 3 | 0, D = x % 3;
      for (let Q = 0; Q < 3; Q++) {
        const A = h[Q][f];
        if (A !== 0) for (let L = 0; L < 3; L++) {
          const E = h[L][D];
          E !== 0 && (_ += A * Y[u * 3 + Q][S * 3 + L] * E);
        }
      }
      M[y][x] = _;
    }
  ...
  function Mo(t) {
    ...
    for (let f = 0; f < 3; f++) {
      const S = f * 6, D = S + 3;
      for (let Q = 0; Q < 3; Q++) for (let A = 0; A < 3; A++) u[S + Q][S + A] = _[Q][A], u[D + Q][D + A] = _[Q][A];
    }
    return u;
  }
  yo = function(t, n, s) {
  ...
  function Ao(t, n, s) {
    ...
    const h = g > 0 && a > 0 ? 12 * i * e / (a * g * m ** 2) : 0, M = l > 0 && a > 0 ? 12 * i * r / (a * l * m ** 2) : 0, w = i * o / Y, y = a * b / Y, x = 12 * i * e / m ** 3 / (1 + h), _ = 6 * i * e / m ** 2 / (1 + h), u = 4 * i * e / m * (1 + h / 4) / (1 + h), f = 2 * i * e / m * (1 - h / 2) / (1 + h), S = 12 * i * r / m ** 3 / (1 + M), D = 6 * i * r / m ** 2 / (1 + M), Q = 4 * i * r / m * (1 + M / 4) / (1 + M), A = 2 * i * r / m * (1 - M / 2) / (1 + M);
    return [
      [
        w,
        0,
  ...
  function Xo(t, n, s) {
    ...
    const e = ((_a = n.elasticities) == null ? void 0 : _a.get(s)) ?? 0, r = ((_b = n.elasticitiesOrthogonal) == null ? void 0 : _b.get(s)) ?? 0, i = ((_c = n.poissonsRatios) == null ? void 0 : _c.get(s)) ?? 0, o = ((_d = n.shearModuli) == null ? void 0 : _d.get(s)) ?? 0, a = ((_e = n.thicknesses) == null ? void 0 : _e.get(s)) ?? 0, b = r > 0, Y = b ? W(e, r, o, i, a) : L(e, i, a), c = b ? T(o, a) : E(e, i, a), m = b ? ro(e, r, o, i) : so(e, i), l = t.map(([N, d]) => [
      N,
      d
    ]), g = l[1][0] - l[0][0], h = l[2][0] - l[0][0], M = l[0][1] - l[1][1], w = l[2][1] - l[0][1], y = 0.5 * (g * w - h * -M), x = j(l), _ = J(l), u = O(l, m, a), f = U(U(Kt(x), c), x), S = U(U(Kt(_), Y), _), D = ot(18, 18).toArray(), Q = U(Bt(f, S), y), A = [
      [
        0,
        1,
        5
    ...
    function j(N) {
      const d = ot(2, 18).toArray(), [Z, R] = N[0], [P, I] = N[1], [q, B] = N[2], $ = 0.5 * ((P - Z) * (B - R) - (q - Z) * -(R - I)), tt = (Z + P + q) / 3, st = (R + I + B) / 3, C = [
        ...
        B,
        R
      ], V = 1 / 3, [gt, bt, pt, vt] = p(C, at), [Mt, St, Xt, dt] = p(it, ct), [ht, rt, zt, v] = p(lt, ut), et = ot(2, 18).toArray(), mt = ot(2, 18).toArray(), yt = ot(2, 18).toArray();
      for (let z = 0; z < 2; z++) for (let F = 0; F < 6; F++) et[z][F] = V * gt[z][F] + bt[z][F], et[z][F + 6] = V * gt[z][F] + pt[z][F], et[z][F + 12] = V * gt[z][F], mt[z][F] = V * Mt[z][F], mt[z][F + 6] = V * Mt[z][F] + St[z][F], mt[z][F + 12] = V * Mt[z][F] + Xt[z][F], yt[z][F] = V * ht[z][F] + zt[z][F], yt[z][F + 6] = V * ht[z][F], yt[z][F + 12] = V * ht[z][F] + rt[z][F];
    ...
    function J(N) {
      const d = ot(3, 18).toArray(), [Z, R] = N[0], [P, I] = N[1], [q, B] = N[2], $ = P - Z, tt = q - Z, st = q - P, C = I - B, at = B - R, it = R - I, ct = 0.5 * ($ * at - tt * -it), lt = C / (2 * ct), ut = st / (2 * ct), V = at / (2 * ct), gt = -tt / (2 * ct), bt = it / (2 * ct), pt = $ / (2 * ct);
      return d[0][4] = lt, d[0][10] = V, d[0][16] = bt, d[1][3] = -ut, d[1][9] = -gt, d[1][15] = -pt, d[2][3] = -lt, d[2][4] = ut, d[2][9] = -V, d[2][10] = gt, d[2][15] = -bt, d[2][16] = pt, d;
    }
    function O(N, d, Z) {
      ...
      const ut = 1 / 8, V = ut / 6, gt = ut ** 2 / 4, bt = 1, pt = 2, vt = 1, Mt = 0, St = 1, Xt = -1, dt = -1, ht = -1, rt = -2, zt = N[0][0], v = N[0][1], et = N[1][0], mt = N[1][1], yt = N[2][0], z = N[2][1], F = zt - et, Lt = et - yt, Qt = yt - zt, kt = v - mt, Tt = mt - z, qt = z - v, jt = -F, wt = -Lt, xt = -Qt, Nt = -kt, _t = -Tt, Dt = -qt, X = 0.5 * (jt * qt - Qt * -kt), G = 2 * X, K = 4 * X, k = 0.5 * Z, Ft = X * Z, Yt = jt ** 2 + Nt ** 2, ft = wt ** 2 + _t ** 2, At = xt ** 2 + Dt ** 2;
  ...
  ko = function(t, n, s, e) {
    ...
    n.forEach((Y, c) => {
      ...
      if (Y.length === 2) {
        ...
        U(g, l);
        const h = xo(s, c), M = No(m), w = vo(l), y = So(m), _ = U(1 / (2 * y), U(U(h, M), w)).toArray(), u = ((_d = s.thicknesses) == null ? void 0 : _d.get(c)) ?? 1, f = _[0][0] * u, S = _[1][0] * u, D = _[2][0] * u, Q = _[0][1] * (u ** 3 / 12), A = _[1][1] * (u ** 3 / 12), L = _[2][1] * (u ** 3 / 12);
      ...
      if (n.forEach((l, g) => {
        ...
        for (const [g, h] of m) {
          ...
          const y = c.get(g), x = (S) => {
            ...
            const W = S.get(g) ?? 0;
            for (const j of w) {
              const p = c.get(j), J = p[0] - y[0], O = p[1] - y[1], N = (S.get(j) ?? 0) - W;
          ...
          }, _ = x(a.bendingXX), u = x(a.bendingYY), f = x(a.bendingXY);
          a.tranverseShearX.set(g, _[0] + f[1]), a.tranverseShearY.set(g, u[1] + f[0]);
        }
      }
    }
    return n.forEach((Y, c) => {
      if (Y.length !== 3 && Y.length !== 4) return;
      const m = Y.length, l = new Array(m).fill(0), g = new Array(m).fill(0), h = new Array(m).fill(0), M = new Array(m).fill(0), w = new Array(m).fill(0), y = new Array(m).fill(0), x = new Array(m).fill(0), _ = new Array(m).fill(0), u = new Array(m).fill(0);
      Y.forEach((A, L) => {
        const E = (b.get(A) || []).filter((p) => n[p].length === 3 || n[p].length === 4), W = (p) => Pt(E.map((J) => p.get(J) ?? 0)), T = (p, J) => Pt(E.map((O) => {
          const N = o.get(O), d = N ? n[O].indexOf(A) : -1;
          return N && d >= 0 ? N[d][p] : J.get(O) ?? 0;
        }));
        l[L] = T(0, a.membraneXX), g[L] = T(1, a.membraneYY), h[L] = T(2, a.membraneXY);
        const j = (p, J) => Pt(E.map((O) => {
          const N = i.get(O), d = N ? n[O].indexOf(A) : -1;
          return N && d >= 0 ? N[d][p] : J.get(O) ?? 0;
        }));
        M[L] = j(0, a.bendingXX), w[L] = j(1, a.bendingYY), y[L] = j(2, a.bendingXY), x[L] = W(a.tranverseShearX), _[L] = W(a.tranverseShearY), u[L] = W(a.vonMises);
  ...
  function wo(t, n, s, e) {
    ...
    const ut = p[0][0] * it + p[0][1] * ct, V = p[1][0] * it + p[1][1] * ct, gt = p[2][2] * lt;
    let bt = 0, pt = 0, vt = 0;
    for (let X = 0; X < 4; X++) {
      const G = T[X * 6 + 3], K = T[X * 6 + 4];
      bt += C[X] * K, pt += -at[X] * G, vt += at[X] * K - C[X] * G;
    }
    const Mt = -1, St = Mt * (O[0][0] * bt + O[0][1] * pt), Xt = Mt * (O[1][0] * bt + O[1][1] * pt), dt = Mt * (O[2][2] * vt);
    let ht = null;
    if (Math.abs(q) > 1e-20) {
      const X = [];
    ...
    const Lt = mt * yt, Qt = mt * z, kt = ut / o + 6 * St / (o * o), Tt = V / o + 6 * Xt / (o * o), qt = gt / o + 6 * dt / (o * o), jt = Math.sqrt(kt * kt - kt * Tt + Tt * Tt + 3 * qt * qt), wt = ut / o - 6 * St / (o * o), xt = V / o - 6 * Xt / (o * o), Nt = gt / o - 6 * dt / (o * o), _t = Math.sqrt(wt * wt - wt * xt + xt * xt + 3 * Nt * Nt), Dt = Math.max(jt, _t);
    return {
      Nx: ut,
      Ny: V,
      Nxy: gt,
