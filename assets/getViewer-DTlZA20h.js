import { N as Ut, a6 as In, q as bs, v as q, a7 as _s, D as Pt, M as Oe, B as be, F as bt, a8 as Ss, x as pt, a9 as ks, aa as Ps, h as Io, ab as Ro, r as an, ac as Xn, ad as Nn, a4 as Oo, _ as tt, a as ut, L as qt, w as Qo, b as Cs, ae as zs, f as st, V as S, $ as sn, af as io, H as wo, d as zt, c as lo, Y as jo, Z as Un, G as Fs, z as Pn, A as As, ag as Yn, t as Es, o as Ts, I as Qt, a2 as Sn, E as Do, S as hn, m as ro, ah as kn, g as Bo, i as Xo, j as No, C as Yo, K as Vs, U as $s, W as Ls, X as Is, T as Rn, P as co, O as Rs } from "./theme-U-6D_qyI.js";
...
class ts {
  ...
  setColorMap(l, u = 32) {
    ...
    for (let p = 1; p < u; p++) {
      ...
      for (let g = 0; g < this.map.length - 1; g++) if (x > this.map[g][0] && x <= this.map[g + 1][0]) {
        ...
        d.setHex(this.map[g][1], In), w.setHex(this.map[g + 1][1], In);
  ...
  updateCanvas(l) {
    ...
    for (let C = 1; C >= 0; C -= p) for (let v = this.map.length - 1; v >= 0; v--) if (C < this.map[v][0] && C >= this.map[v - 1][0]) {
      ...
      x.setHex(this.map[v - 1][1], In), g.setHex(this.map[v][1], In), k.lerpColors(x, g, (C - Z) / (ye - Z)), d[w * 4] = Math.round(k.r * 255), d[w * 4 + 1] = Math.round(k.g * 255), d[w * 4 + 2] = Math.round(k.b * 255), d[w * 4 + 3] = 255, w += 1;
    }
...
function Zo() {
  ...
  return u.minFilter = Io, u.magFilter = Io, u.wrapS = Ro, u.wrapT = Ro, u.needsUpdate = true, u;
}
function Xs() {
  const l = [];
  for (let u = 0; u <= 12; u++) {
...
function uo(e, l) {
  ...
  const k = new Ut(u.grid), C = new Ut(u.grid).multiplyScalar(0.45), v = (te, z, ne, W) => {
    ...
    const R = new be();
    R.setAttribute("position", new bt(ve, 3));
    const U = new ut({ color: ne, transparent: true, opacity: W, depthWrite: false }), $ = new qt(R, U);
    ...
    const R = new ut({ color: z, transparent: true, opacity: ne, depthWrite: false }), U = new Qo(B, R);
    return U.name = `grid-${te}-border`, U.renderOrder = 1, U;
  }, ye = (te, z, ne) => {
    const W = te === "xy" ? (R, U) => [R, U, 0] : te === "xz" ? (R, U) => [R, 0, U] : (R, U) => [0, R, U], ve = z === "u" ? [...W(-x, 0), ...W(x, 0)] : [...W(0, -x), ...W(0, x)], ge = new be();
...
function Ks(e, l, u, h) {
  ...
  return q.derive(() => {
    ...
    let C = k;
    if (k.length > g) {
      const v = k.map(($) => p[$][0]), Z = k.map(($) => p[$][1]), ye = Math.min(...v), le = Math.max(...v), te = Math.min(...Z), z = Math.max(...Z), ne = k.map(($) => p[$][2]), W = Math.max(1e-6, (Math.max(...ne) - Math.min(...ne)) / 40), ve = ($) => Math.round($ / W), ge = new Set(ne.map(ve)), B = Math.max(4, Math.floor(g / Math.max(1, ge.size))), R = Math.max(2, Math.round(Math.sqrt(B))), U = /* @__PURE__ */ new Map();
      for (const $ of k) {
        const A = le - ye < 1e-9 ? 0 : (p[$][0] - ye) / (le - ye), X = z - te < 1e-9 ? 0 : (p[$][1] - te) / (z - te), D = Math.min(R - 1, Math.floor(A * R)), L = Math.min(R - 1, Math.floor(X * R)), Q = `${D},${L},${ve(p[$][2])}`, ce = Math.hypot(A * R - (D + 0.5), X * R - (L + 0.5)), se = U.get(Q);
        (!se || ce < se.d) && U.set(Q, { i: $, d: ce });
      }
      C = [...U.values()].map(($) => $.i);
    }
    for (const v of C) {
...
function js(e, l, u, h) {
  ...
  d.add(w);
  function p(R, U) {
    const $ = R / 2, A = U / 2, X = new Float32Array([0, -$, -A, 0, $, -A, 0, $, A, 0, -$, -A, 0, $, A, 0, -$, A]), D = new be();
  ...
  function x(R, U = 24) {
    const $ = R / 2, A = new Float32Array(U * 9);
    for (let Q = 0; Q < U; Q++) {
      const ce = Q / U * Math.PI * 2, se = (Q + 1) / U * Math.PI * 2;
      A[Q * 9] = 0, A[Q * 9 + 1] = 0, A[Q * 9 + 2] = 0, A[Q * 9 + 3] = 0, A[Q * 9 + 4] = $ * Math.cos(ce), A[Q * 9 + 5] = $ * Math.sin(ce), A[Q * 9 + 6] = 0, A[Q * 9 + 7] = $ * Math.cos(se), A[Q * 9 + 8] = $ * Math.sin(se);
    }
    const X = new be();
    X.setAttribute("position", new pt(A, 3));
    const D = new Float32Array((U + 1) * 3);
    for (let Q = 0; Q <= U; Q++) {
      const ce = Q / U * Math.PI * 2;
      D[Q * 3] = 0, D[Q * 3 + 1] = $ * Math.cos(ce), D[Q * 3 + 2] = $ * Math.sin(ce);
    }
    const L = new be();
    return L.setAttribute("position", new pt(D, 3)), { fill: X, outline: L };
  }
  function g(R, U, $, A) {
    const X = $ ?? U * 0.08, D = A ?? R * 0.07, L = R / 2, Q = U / 2, ce = Q - X, se = D / 2, ee = [];
    function I(me, Ve, Me, Le) {
      ee.push(0, me, Ve, 0, Me, Ve, 0, Me, Le, 0, me, Ve, 0, Me, Le, 0, me, Le);
    }
    I(-L, -Q, L, -ce), I(-se, -ce, se, ce), I(-L, ce, L, Q);
    const de = new be();
    de.setAttribute("position", new pt(new Float32Array(ee), 3));
    const G = new Float32Array([0, -L, -Q, 0, L, -Q, 0, L, -ce, 0, se, -ce, 0, se, ce, 0, L, ce, 0, L, Q, 0, -L, Q, 0, -L, ce, 0, -se, ce, 0, -se, -ce, 0, -L, -ce, 0, -L, -Q]), fe = new be();
    return fe.setAttribute("position", new pt(G, 3)), { fill: de, outline: fe };
  }
  function k(R, U, $) {
    const A = R / 2, X = U / 2, D = A - $, L = X - $, Q = [];
    function ce(de, G, fe, me) {
      Q.push(0, de, G, 0, fe, G, 0, fe, me, 0, de, G, 0, fe, me, 0, de, me);
    }
    ce(-A, -X, A, -L), ce(-A, L, A, X), ce(-A, -L, -D, L), ce(D, -L, A, L);
    const se = new be();
    se.setAttribute("position", new pt(new Float32Array(Q), 3));
    const ee = new Float32Array([0, -A, -X, 0, A, -X, 0, A, -X, 0, A, X, 0, A, X, 0, -A, X, 0, -A, X, 0, -A, -X, 0, -D, -L, 0, D, -L, 0, D, -L, 0, D, L, 0, D, L, 0, -D, L, 0, -D, L, 0, -D, -L]), I = new be();
    return I.setAttribute("position", new pt(ee, 3)), { fill: se, outline: I };
  }
  function C(R, U, $) {
    const A = R / 2, X = U / 2, D = A - $, L = X - $, Q = new be(), ce = new Float32Array([0, -D, -L, 0, D, -L, 0, D, L, 0, -D, -L, 0, D, L, 0, -D, L]);
    Q.setAttribute("position", new pt(ce, 3));
    const se = [];
    function ee(fe, me, Ve, Me) {
      se.push(0, fe, me, 0, Ve, me, 0, Ve, Me, 0, fe, me, 0, Ve, Me, 0, fe, Me);
    }
    ee(-A, -X, A, -L), ee(-A, L, A, X), ee(-A, -L, -D, L), ee(D, -L, A, L);
    const I = new be();
    I.setAttribute("position", new pt(new Float32Array(se), 3));
    const de = new Float32Array([0, -A, -X, 0, A, -X, 0, A, -X, 0, A, X, 0, A, X, 0, -A, X, 0, -A, X, 0, -A, -X, 0, -D, -L, 0, D, -L, 0, D, -L, 0, D, L, 0, D, L, 0, -D, L, 0, -D, L, 0, -D, -L]), G = new be();
    return G.setAttribute("position", new pt(de, 3)), { concFill: Q, steelFillGeom: I, outline: G };
  }
  function v(R, U, $) {
    const A = [], X = [[0, -R / 2, -U / 2], [0, -R / 2 + $, -U / 2], [0, -R / 2 + $, U / 2 - $], [0, R / 2, U / 2 - $], [0, R / 2, U / 2], [0, -R / 2, U / 2]], D = [0, 1, 2, 0, 2, 5, 2, 3, 4, 2, 4, 5];
    for (const se of D) A.push(...X[se]);
    const L = new be();
    L.setAttribute("position", new pt(new Float32Array(A), 3));
    const Q = [];
    for (let se = 0; se < X.length; se++) {
      const ee = (se + 1) % X.length;
      Q.push(...X[se], ...X[ee]);
    }
    const ce = new be();
    return ce.setAttribute("position", new pt(new Float32Array(Q), 3)), { fill: L, outline: ce };
  }
  function Z(R, U, $, A) {
    const X = A / 2, D = [], L = [[0, -R - X, -U / 2], [0, -$ - X, -U / 2], [0, -$ - X, U / 2 - $], [0, -X, U / 2 - $], [0, -X, U / 2], [0, -R - X, U / 2]], Q = [[0, X, -U / 2], [0, X + $, -U / 2], [0, X + $, U / 2 - $], [0, R + X, U / 2 - $], [0, R + X, U / 2], [0, X, U / 2]], ce = [0, 1, 2, 0, 2, 5, 2, 3, 4, 2, 4, 5];
    for (const de of ce) D.push(...L[de]);
    for (const de of ce) D.push(...Q[de]);
    const se = new be();
    se.setAttribute("position", new pt(new Float32Array(D), 3));
    const ee = [];
    for (const de of [L, Q]) for (let G = 0; G < de.length; G++) {
      const fe = (G + 1) % de.length;
      ee.push(...de[G], ...de[fe]);
    }
    const I = new be();
    return I.setAttribute("position", new pt(new Float32Array(ee), 3)), { fill: se, outline: I };
  }
  function ye(R, U, $, A) {
    const X = U / 2, D = R, L = [[0, -D, -X], [0, -D, -X + $], [0, -A, -X + $], [0, -A, X - $], [0, -D, X - $], [0, -D, X], [0, 0, X], [0, 0, -X]], Q = [0, 1, 7, 1, 6, 7, 1, 2, 6, 2, 5, 6, 2, 3, 5, 3, 4, 5], ce = [];
    for (const de of Q) ce.push(...L[de]);
    const se = new be();
    se.setAttribute("position", new pt(new Float32Array(ce), 3));
    const ee = [];
    for (let de = 0; de < L.length; de++) {
      const G = (de + 1) % L.length;
      ee.push(...L[de], ...L[G]);
    }
    const I = new be();
    return I.setAttribute("position", new pt(new Float32Array(ee), 3)), { fill: se, outline: I };
  ...
  function le(R, U, $, A, X) {
    const D = U / 2, L = X / 2, Q = [], ce = [[0, -R, -D], [0, -R, -D + $], [0, -L - A, -D + $], [0, -L - A, D - $], [0, -R, D - $], [0, -R, D], [0, -L, D], [0, -L, -D]], se = ce.map((fe) => [fe[0], -fe[1], fe[2]]), ee = [0, 1, 7, 1, 6, 7, 1, 2, 6, 2, 5, 6, 2, 3, 5, 3, 4, 5];
    for (const fe of ee) Q.push(...ce[fe]);
    for (const fe of ee) Q.push(...se[fe]);
    const I = new be();
    I.setAttribute("position", new pt(new Float32Array(Q), 3));
    const de = [];
    for (const fe of [ce, se]) for (let me = 0; me < fe.length; me++) {
      const Ve = (me + 1) % fe.length;
      de.push(...fe[me], ...fe[Ve]);
    }
    const G = new be();
    return G.setAttribute("position", new pt(new Float32Array(de), 3)), { fill: I, outline: G };
  }
  function te(R, U, $, A) {
    const X = R / 2, D = U / 2, L = A / 2, Q = [[0, -L, -D], [0, L, -D], [0, L, D - $], [0, X, D - $], [0, X, D], [0, -X, D], [0, -X, D - $], [0, -L, D - $]], ce = [0, 1, 7, 1, 2, 7, 6, 7, 5, 2, 3, 4, 2, 4, 5, 2, 5, 7], se = [];
    for (const G of ce) se.push(...Q[G]);
    const ee = new be();
    ee.setAttribute("position", new pt(new Float32Array(se), 3));
    const I = [];
    for (let G = 0; G < Q.length; G++) {
      const fe = (G + 1) % Q.length;
      I.push(...Q[G], ...Q[fe]);
    }
    const de = new be();
    return de.setAttribute("position", new pt(new Float32Array(I), 3)), { fill: ee, outline: de };
  }
  function z(R, U, $ = 24) {
    const A = R / 2, X = A - U, D = [];
    for (let se = 0; se < $; se++) {
      const ee = se / $ * Math.PI * 2, I = (se + 1) / $ * Math.PI * 2, de = Math.cos(ee), G = Math.sin(ee), fe = Math.cos(I), me = Math.sin(I);
      D.push(0, A * de, A * G, 0, A * fe, A * me, 0, X * fe, X * me), D.push(0, A * de, A * G, 0, X * fe, X * me, 0, X * de, X * G);
    }
    const L = new be();
    L.setAttribute("position", new pt(new Float32Array(D), 3));
    const Q = [];
    for (let se = 0; se < $; se++) {
      const ee = se / $ * Math.PI * 2, I = (se + 1) / $ * Math.PI * 2;
      Q.push(0, A * Math.cos(ee), A * Math.sin(ee), 0, A * Math.cos(I), A * Math.sin(I)), Q.push(0, X * Math.cos(ee), X * Math.sin(ee), 0, X * Math.cos(I), X * Math.sin(I));
    }
    const ce = new be();
    return ce.setAttribute("position", new pt(new Float32Array(Q), 3)), { fill: L, outline: ce };
  }
  ...
  function B(R, U) {
    const $ = Math.abs(U[0] - R[0]), A = Math.abs(U[1] - R[1]), X = Math.abs(U[2] - R[2]);
    return X > $ && X > A || A > $ && A > X;
  }
  return q.derive(() => {
    var _a2, _b;
    l.deformedShape.val, l.secColumns.val, l.secBeams.val, l.secFloor.val;
    const R = l.secColumns.rawVal, U = l.secBeams.rawVal;
    if (!R && !U) {
    ...
    $.forEach((L, Q) => {
      ...
      const ce = u.rawVal[L[0]], se = u.rawVal[L[1]];
      if (!ce || !se) return;
      const ee = B(ce, se);
      if (ee && !R || !ee && !U) return;
      if (D >= 0) {
        const me = Math.min(ce[1], se[1]);
        Math.max(ce[1], se[1]);
        const Ve = l.gridSize.rawVal || 3;
        if (Math.floor(me / Ve + 0.01) !== D) return;
      }
      const I = X == null ? void 0 : X.get(Q);
      if (!I) return;
      const de = [(ce[0] + se[0]) / 2, (ce[1] + se[1]) / 2, (ce[2] + se[2]) / 2], G = qn(ce, se);
      if (I.type === "CFT") {
        const me = C(I.b, I.h, I.tw ?? I.b * 0.05), Ve = new Oe(me.concFill, ne);
        Ve.position.set(...de), Ve.rotation.setFromRotationMatrix(G), d.add(Ve);
        const Me = new Oe(me.steelFillGeom, ve);
        Me.position.set(...de), Me.rotation.setFromRotationMatrix(G), d.add(Me);
        const Le = new zt(me.outline, ge);
        Le.position.set(...de), Le.rotation.setFromRotationMatrix(G), d.add(Le);
      } else {
        let me, Ve, Me;
        switch (I.type) {
          case "rect":
            me = p(I.b, I.h), Ve = ne, Me = W;
            break;
          case "circ":
            me = x(I.d), Ve = ne, Me = W;
            break;
          case "I":
            me = g(I.b, I.h, I.tf, I.tw), Ve = ve, Me = ge;
            break;
          case "HSS":
            me = k(I.b, I.h, I.tw ?? I.b * 0.05), Ve = ve, Me = ge;
            break;
          case "CFT":
            me = C(I.b, I.h, I.tw ?? I.b * 0.05), Ve = ve, Me = ge;
            break;
          case "L":
            me = v(I.b ?? I.h, I.h, I.t ?? I.tw ?? 3e-3), Ve = ve, Me = ge;
            break;
          case "2L":
            me = Z(I.b ?? I.h, I.h, I.t ?? I.tw ?? 3e-3, I.dis ?? 0.01), Ve = ve, Me = ge;
            break;
          case "C":
          case "coldC":
            me = ye(I.b, I.h, I.tf ?? I.t ?? 3e-3, I.tw ?? I.t ?? 3e-3), Ve = ve, Me = ge;
            break;
          case "2C":
            me = le(I.b, I.h, I.tf ?? 5e-3, I.tw ?? 5e-3, I.dis ?? 0.01), Ve = ve, Me = ge;
            break;
          case "T":
            me = te(I.b, I.h, I.tf ?? 0.01, I.tw ?? 6e-3), Ve = ve, Me = ge;
            break;
          case "pipe":
            me = z(I.d, I.tw ?? I.d * 0.05), Ve = ve, Me = ge;
            break;
          default:
            return;
        }
        const Le = new Oe(me.fill, Ve);
        Le.position.set(...de), Le.rotation.setFromRotationMatrix(G), d.add(Le);
        const Qe = new zt(me.outline, Me);
        Qe.position.set(...de), Qe.rotation.setFromRotationMatrix(G), d.add(Qe);
      }
      const fe = Qs(I);
      if (fe) {
        const Ve = ["I", "HSS", "CFT", "L", "2L", "C", "2C", "T", "pipe", "coldC"].includes(I.type) ? "#ff9900" : "#00ccff", Me = new kt(fe, Ve, "transparent");
        Me.position.set(de[0], de[1], de[2]);
    ...
    w.children.forEach((U) => {
      U instanceof kt && U.updateScale(R * h.rawVal);
    });
  }), q.derive(() => {
    ...
    try {
      ye.forEach((W, ve) => {
        ...
        if (W.length === 2) {
          ...
          const R = Z[W[0]], U = Z[W[1]];
          if (!R || !U) return;
          const $ = Math.hypot(U[0] - R[0], U[1] - R[1], U[2] - R[2]);
          if ($ < 1e-9) return;
          ...
          X.position.set(R[0], R[1], R[2]), X.rotation.setFromRotationMatrix(qn(R, U)), h.add(X);
          return;
        }
        if (W.length === 3 || W.length === 4) {
          ...
          const R = [B[1][0] - B[0][0], B[1][1] - B[0][1], B[1][2] - B[0][2]], U = [B[2][0] - B[0][0], B[2][1] - B[0][1], B[2][2] - B[0][2]], $ = R[1] * U[2] - R[2] * U[1], A = R[2] * U[0] - R[0] * U[2], X = R[0] * U[1] - R[1] * U[0], D = Math.hypot($, A, X);
          if (D < 1e-12) return;
          const L = [$ / D, A / D, X / D], Q = [], ce = (G) => B.map((fe) => [fe[0] + L[0] * G, fe[1] + L[1] * G, fe[2] + L[2] * G]), se = ce(+ge / 2), ee = ce(-ge / 2), I = (G, fe, me) => Q.push(...G, ...fe, ...me);
          for (const G of [se, ee]) I(G[0], G[1], G[2]), G.length === 4 && I(G[0], G[2], G[3]);
          for (let G = 0; G < B.length; G++) {
            const fe = (G + 1) % B.length;
            I(se[G], ee[G], ee[fe]), I(se[G], ee[fe], se[fe]);
          }
          const de = new be();
          de.setAttribute("position", new bt(Q, 3)), de.computeVertexNormals(), h.add(new Oe(de, p));
        }
      });
    } catch (W) {
...
function ra({ drawingObj: e, gridObj: l, scene: u, getActiveCamera: h, controls: d, gridSize: w, derivedDisplayScale: p, rendererElm: x, viewerRender: g }) {
  ...
  const A = new S(), X = (n, o, a, t, s, i) => {
    ...
    R = [n, o, a], U = [c / _, m / _, y / _], A.set((n + t) / 2, (o + s) / 2, (a + i) / 2), A.project(h());
    ...
    B.style.display = "none", R = null, U = null, $ = false, document.activeElement === B && B.blur();
  }, L = (n) => {
    var _a2, _b, _c, _d, _e, _f, _g, _h;
    const o = ((_c = (_b = (_a2 = window.__hekatanCadState) == null ? void 0 : _a2.get) == null ? void 0 : _b.call(_a2)) == null ? void 0 : _c.tool) ?? "select";
    if (o === "offset") {
      on = n, j(`\u21C9 DESFASE distancia ${n} m \u2014 designe la l\xEDnea y luego el lado.`), B.blur();
      try {
    ...
    if (o === "circle" && Ae.length === 1) {
      const M = Ae[0];
      Ae = [], (_e = window.__hekatanDrawCircle) == null ? void 0 : _e.call(window, M[0], M[1], M[2], n), j(`\u2713 C\xEDrculo r=${n} m en (${M[0].toFixed(2)}, ${M[1].toFixed(2)}, ${M[2].toFixed(2)}).`);
      try {
    ...
    if (o === "col" || o === "wall" || o === "extp" || o === "extl") {
      wt = n, j(`\u{1F4D0} Altura ${n}m memorizada \u2014 hac\xE9 el click para crear ${{ col: "columna", wall: "pared", extp: "extrusi\xF3n punto\u2192l\xEDnea", extl: "extrusi\xF3n l\xEDnea\u2192\xE1rea" }[o]}.`), B.blur();
      return;
    }
    if (!R || !U || !e.polylines) return;
    let a = U[0], t = U[1], s = U[2];
    at === "x" ? (a = Math.sign(a) || 1, t = 0, s = 0) : at === "y" ? (a = 0, t = Math.sign(t) || 1, s = 0) : at === "z" && (a = 0, t = 0, s = Math.sign(s) || 1);
    const i = R[0] + a * n, c = R[1] + t * n, m = R[2] + s * n;
    ...
    e.polylines.val = [...o.slice(0, -1), [...a, e.points.rawVal.length - 1]], R = n, B.blur();
    try {
  ...
  window.__hekatanTypeCoord = (n) => {
    ...
    const a = ce(o);
    if (!a) return false;
    To(new S(a[0], a[1], a[2]), null), R = a, B.blur();
    try {
    ...
    if (n.key === "Enter") {
      ...
      const a = Q(B.value);
      if (!a) return;
      if ($ = false, a.kind === "length") L(a.L), j(`\u270F DDE ${a.L}m aplicado en direcci\xF3n actual`);
      else {
        const t = ce(a);
        if (!t) return;
        se(t);
        const s = a.kind;
        j(`\u270F ${s} \u2192 (${t[0].toFixed(2)}, ${t[1].toFixed(2)}, ${t[2].toFixed(2)})`);
      }
      return;
    }
    if (n.key === "Escape") {
    ...
    if (!R || !U || document.activeElement === B) return;
  ...
  const de = new zt(new be().setFromPoints([new S(0, 0, 0), new S(0, 0, 0)]), new Sn({ color: 2282478, dashSize: 0.2, gapSize: 0.1, transparent: true, opacity: 0.85, linewidth: 2 }));
  de.frustumCulled = false, de.visible = false, u.add(de);
  const G = new zt(new be(), new ut({ color: 2282478, transparent: true, opacity: 0.9 }));
  G.frustumCulled = false, G.visible = false, u.add(G);
  let fe = [];
  const me = new tt(), Ve = new Oe(new Qt(1, 1), new st({ color: 2282478, transparent: true, opacity: 0.08, side: Pt, depthWrite: false })), Me = new qt(new Do(new Qt(1, 1)), new ut({ color: 2282478, transparent: true, opacity: 0.85 })), Le = new qt(new be(), new ut({ color: 2282478, transparent: true, opacity: 0.3 })), Qe = (n, o) => {
  ...
  me.add(Ve, Me, Le), me.visible = false, me.frustumCulled = false, u.add(me);
  const ct = new tt();
  ...
  }, ie = O(3462041), he = O(16724804), ue = O(6333946), ke = new tt();
  ke.frustumCulled = false, ke.visible = false, u.add(ke), ke.add(ie, he, ue);
  const Fe = (n) => {
  ...
  $e.id = "hk-refplane-badge", $e.style.cssText = ["position:fixed", "pointer-events:none", "z-index:99997", "padding:3px 10px", "border-radius:4px", "font-family:Consolas,monospace", "font-size:12px", "font-weight:bold", "transform:translate(20px,40px)", "white-space:nowrap", "display:none"].join(";") + ";", document.body.appendChild($e), window.__hekatanSetOrthoPlanes = (n) => {
    ...
    if (window.__hekatanShowOrthoPlanes = n, ke.visible = n, n) {
      ...
      Ge(ie, i, "xy", c), Ge(he, i, "xz", c), Ge(ue, i, "yz", c), Ke(Ee, i, "xy", c), Ke(De, i, "xz", c), Ke(Be, i, "yz", c), Ee.material.opacity = 0.05, De.material.opacity = 0.05, Be.material.opacity = 0.05;
    } else {
    ...
    Ge(ie, i, "xy", n), Ge(he, i, "xz", n), Ge(ue, i, "yz", n), Ke(Ee, i, "xy", n), Ke(De, i, "xz", n), Ke(Be, i, "yz", n), g();
  };
  const mt = (n) => {
  ...
  window.addEventListener("keydown", (n) => {
    ...
    if (n.key === "Enter" && t === "polyarea" && fe.length >= 3) {
      const s = yn();
      j(`\u2713 \xC1rea libre mallada \u2014 ${s} shells Q4 creados.`), n.preventDefault();
      return;
    }
    ...
    window.__hekatanOsnapOn = n, n || Vn(), j(`\u{1F9F2} OSNAP ${n ? "ON" : "OFF"} (F3)`);
  }, window.__hekatanTogglePolar = () => {
    const n = window.__hekatanPolarTrack === false;
    window.__hekatanPolarTrack = n, n || (ct.visible = false), j(`\u25C8 POLAR ${n ? "ON" : "OFF"} (F10)`);
  ...
  const Ne = new S(), Ie = new S(), ze = new S(), qe = (n) => {
    if (!at) return null;
    const o = n[0], a = n[1], t = n[2];
    return at === "x" ? (Ne.set(o - 1e4, a, t), Ie.set(o + 1e4, a, t)) : at === "y" ? (Ne.set(o, a - 1e4, t), Ie.set(o, a + 1e4, t)) : (Ne.set(o, a, t - 1e4), Ie.set(o, a, t + 1e4)), k.ray.distanceSqToSegment(Ne, Ie, null, ze), ze;
  };
  ...
  window.__hekatanCentrosDeducidos = wn, window.__hekatanDrawCircle = (n, o, a, t, s = window.__hekatanArcSegs ?? 12, i = "xy") => {
    ...
    const Y = i.distanceTo(P), H = new S().subVectors(i, P), oe = new S().subVectors(m, P), Pe = Math.acos(Math.max(-1, Math.min(1, H.dot(oe) / (Y * Y)))), pe = e.points.rawVal.length, we = [], We = M.clone();
    for (let Te = 0; Te <= s; Te++) {
      const Se = Te / s, Ye = Pe * Se, Ue = new ro().setFromAxisAngle(We, Ye), Je = H.clone().applyQuaternion(Ue).add(P);
      we.push([Je.x, Je.y, Je.z]);
    }
    if (e.points.val = [...e.points.rawVal, ...we], rn.push({ c: [P.x, P.y, P.z], r: Y }), e.polylines) {
      const Te = we.map((Ye, Ue) => pe + Ue), Se = e.polylines.rawVal;
    ...
    let _;
    if (z && e.gridTarget) {
      const M = e.gridTarget.rawVal, r = new kn(...M.rotation), f = new S(1, 0, 0).applyEuler(r), b = new S(0, 1, 0).applyEuler(r), E = new S(...M.position), T = new S(t, s, i), V = new S(c, m, y), P = T.clone().sub(E).dot(f), Y = T.clone().sub(E).dot(b), H = V.clone().sub(E).dot(f), oe = V.clone().sub(E).dot(b), Pe = (pe, we) => E.clone().addScaledVector(f, pe).addScaledVector(b, we).toArray();
    ...
    r /= E, f /= E, b /= E;
    const T = n[0], V = (_e) => [(_e[0] - T[0]) * m + (_e[1] - T[1]) * y + (_e[2] - T[2]) * _, (_e[0] - T[0]) * r + (_e[1] - T[1]) * f + (_e[2] - T[2]) * b], P = (_e, Re) => [T[0] + _e * m + Re * r, T[1] + _e * y + Re * f, T[2] + _e * _ + Re * b], Y = n.map(V);
    let H = 1 / 0, oe = -1 / 0, Pe = 1 / 0, pe = -1 / 0;
    for (const [_e, Re] of Y) _e < H && (H = _e), _e > oe && (oe = _e), Re < Pe && (Pe = Re), Re > pe && (pe = Re);
    const we = oe - H, We = pe - Pe;
    if (we < 1e-6 || We < 1e-6) return 0;
    let Te = o && o > 0 ? o : 0.5;
    for (; we / Te * (We / Te) > 2500; ) Te *= 2;
    Te = Math.min(Te, Math.min(we, We));
    const Se = (_e, Re) => {
      let et = false;
      for (let At = 0, Yt = Y.length - 1; At < Y.length; Yt = At++) {
        const [Bt, Wt] = Y[At], [fn, _n] = Y[Yt];
        Wt > Re != _n > Re && _e < (fn - Bt) * (Re - Wt) / (_n - Wt) + Bt && (et = !et);
      }
      return et;
    }, Ye = Math.max(1, Math.round(we / Te)), Ue = Math.max(1, Math.round(We / Te)), Je = we / Ye, ft = We / Ue, Dt = /* @__PURE__ */ new Map(), ht = [], Ce = e.points.rawVal.length, rt = (_e, Re) => {
      const et = _e + "," + Re, At = Dt.get(et);
      if (At !== void 0) return At;
      const Yt = Ce + ht.length;
      return ht.push(P(H + _e * Je, Pe + Re * ft)), Dt.set(et, Yt), Yt;
    }, je = [];
    for (let _e = 0; _e < Ye; _e++) for (let Re = 0; Re < Ue; Re++) {
      if (!Se(H + (_e + 0.5) * Je, Pe + (Re + 0.5) * ft)) continue;
      const et = rt(_e, Re), At = rt(_e + 1, Re), Yt = rt(_e + 1, Re + 1), Bt = rt(_e, Re + 1);
      je.push([et, At, Yt, Bt]);
    }
    ...
    if (window.__hekatanPushUndo && window.__hekatanPushUndo(), e.points.val = [...e.points.rawVal, ...ht], e.polylines && e.areas) {
      ...
      const Re = [];
      for (const et of je) Re.push(_e.length), _e.push([et[0], et[1], et[2], et[3], et[0]]);
  ...
  const yn = () => {
    if (fe.length < 3) return fe = [], G.visible = false, g(), 0;
    const n = window.__hekatanMeshPolyArea(fe.slice());
    return fe = [], G.visible = false, g(), n;
  };
  window.__hekatanFinalizePolyArea = yn, window.__hekatanSetInclinedPlaneFrom3 = (n, o, a) => {
    ...
    Ve.geometry.dispose(), Ve.geometry = new Qt(M, M), Me.geometry.dispose(), Me.geometry = new Do(new Qt(M, M)), Qe(r, 1), me.position.copy(_), me.quaternion.copy(m), me.scale.set(1, 1, 1), me.visible = true;
    try {
      (_a2 = window.__hekatanRefreshStatus) == null ? void 0 : _a2.call(window);
    } catch {
    }
    ...
    e.gridTarget && (e.gridTarget.val = { position: [0, 0, 0], rotation: [0, 0, 0] }), z = false, me.visible = false, g();
  };
  const $t = new tt();
  $t.visible = false, u.add($t), window.__hekatanShowAxes = (n, o, a = 12, t = 2) => {
    var _a2, _b;
    for (; $t.children.length; ) {
      const M = $t.children.pop();
    ...
    n.forEach((M, r) => {
      ...
      T.computeLineDistances(), $t.add(T), $t.add(_(f, M, s - 0.5, 0, "#60a5fa")), $t.add(_(f, M, i + 0.5, 0, "#60a5fa"));
    }), o.forEach((M, r) => {
      ...
      T.computeLineDistances(), $t.add(T), $t.add(_(f, c - 0.5, M, 0, "#fb7185")), $t.add(_(f, m + 0.5, M, 0, "#fb7185"));
    }), $t.visible = true, g();
  ...
  window.__hekatanUpdateSelectionPtScale = Mo, d.addEventListener("change", () => {
    ...
    if (a.length) {
      ...
      if (c === "select" || !c) {
        ...
        ee.style.left = n.clientX + "px", ee.style.top = n.clientY + "px", ee.style.display = "block";
        let E = t;
        ...
        if (window.__hekatanCursorXYZ = [E.x, E.y, E.z], lt) {
          ...
          ee.textContent = `${T}  \xB7  \u{1F5B1} Click \u2192 ${P[lt.kind]}`;
        } else ee.textContent = T;
        const V = document.getElementById("hk-coord-fixed");
        V && (V.textContent = T), de.visible = false, ct.visible = false, g();
        return;
      }
      if (c === "delete" || c === "trim" || c === "extend" || c === "offset") {
        ...
        b ? (xt = f, Ze = -1, He = -1, Kn(f)) : r ? (Ze = r.polyIdx, He = r.segIdx, xt = -1, Gn(r.polyIdx, r.segIdx)) : (Ze = -1, He = -1, xt = -1, xe.visible = false), de.visible = false, ct.visible = false, D(), ee.style.left = n.clientX + "px", ee.style.top = n.clientY + "px", ee.style.display = "block";
        const E = `X=${t.x.toFixed(2)} Y=${t.y.toFixed(2)} Z=${t.z.toFixed(2)}`;
        let T = "";
        b ? T = `\u{1F5D1} l\xEDnea aux #${xt + 1}` : r ? T = ((_i = (_h = e.areas) == null ? void 0 : _h.rawVal) == null ? void 0 : _i.includes(r.polyIdx)) ?? false ? `\u{1F5D1} \xE1rea #${r.polyIdx + 1}` : `\u{1F5D1} seg ${r.segIdx + 1} / poly #${r.polyIdx + 1}` : T = "\u{1F5D1} acerc\xE1 a l\xEDnea/\xE1rea", ee.textContent = `${E}  \xB7  ${T}`;
        const V = document.getElementById("hk-coord-fixed");
        V && (V.textContent = E), g();
        return;
      ...
      if (y.length > 0 && _[y[y.length - 1]]) {
        ...
        if (dt = null, !f && window.__hekatanAxisSnap !== false) {
          ...
          for (const [rt, je] of Dt) {
            const _e2 = ht(ft.clone().addScaledVector(je, -Je)), Re = ht(ft.clone().addScaledVector(je, Je)), et = Re.x - _e2.x, At = Re.y - _e2.y, Yt = Ye - _e2.x, Bt = Ue - _e2.y, Wt = et * et + At * At || 1;
        ...
        if (!f && E) {
          ...
          if (Je > 1e-3) {
            ...
            je.sort((_e2, Re) => _e2[1] - Re[1]), je[0][1] <= Dt && (f = je[0][0]);
          }
        }
        if (f) {
        ...
        ee.textContent = `${P} | \u0394L=${T.toFixed(2)}m ${V.toFixed(0)}\xB0`;
        const Y = document.getElementById("hk-coord-fixed");
        Y && (Y.textContent = P), de.geometry.setFromPoints([new S(r[0], r[1], r[2]), new S(t.x, t.y, t.z)]), (_n = de.computeLineDistances) == null ? void 0 : _n.call(de), de.visible = true, X(r[0], r[1], r[2], t.x, t.y, t.z);
        const H = window.__hekatanOrthoExt ?? 8, oe = window.__hekatanShowOrthoPlanes !== false;
        ke.visible = oe, oe || mt(null), oe && (Ge(ie, r, "xy", H), Ge(he, r, "xz", H), Ge(ue, r, "yz", H), Ke(Ee, r, "xy", H), Ke(De, r, "xz", H), Ke(Be, r, "yz", H));
        const Pe = oe ? k.intersectObjects([Ee, De, Be], false) : [];
        let pe = null;
        if (Pe.length > 0) {
          const Se = Pe[0].object;
          Se === Ee ? pe = "xy" : Se === De ? pe = "xz" : Se === Be && (pe = "yz");
        }
        mt(pe), pe && ($e.style.left = n.clientX + "px", $e.style.top = n.clientY + "px"), F.geometry.setFromPoints([new S(r[0] - H, r[1], r[2]), new S(r[0] + H, r[1], r[2])]), (_o2 = F.computeLineDistances) == null ? void 0 : _o2.call(F), K.geometry.setFromPoints([new S(r[0], r[1] - H, r[2]), new S(r[0], r[1] + H, r[2])]), (_p = K.computeLineDistances) == null ? void 0 : _p.call(K), N.geometry.setFromPoints([new S(r[0], r[1], r[2] - H), new S(r[0], r[1], r[2] + H)]), (_q = N.computeLineDistances) == null ? void 0 : _q.call(N), ct.visible = true;
        const we = F.material, We = K.material, Te = N.material;
        f === "x" ? (we.opacity = 0.95, We.opacity = 0.1, Te.opacity = 0.1) : f === "y" ? (we.opacity = 0.1, We.opacity = 0.95, Te.opacity = 0.1) : f === "z" ? (we.opacity = 0.1, We.opacity = 0.1, Te.opacity = 0.95) : (we.opacity = 0.5, We.opacity = 0.5, Te.opacity = 0.5);
      } else {
        const M = `X=${t.x.toFixed(2)} Y=${t.y.toFixed(2)} Z=${t.z.toFixed(2)}`;
        ee.textContent = M;
        const r = document.getElementById("hk-coord-fixed");
        if (r && (r.textContent = M), de.visible = false, ct.visible = false, (/* @__PURE__ */ new Set(["line", "polyline", "area", "node", "column", "wall", "rect", "circle", "arc", "polyline-multi", "axis", "chaflan"])).has(c)) {
          if (R = null, U = null, B.style.left = n.clientX + 20 + "px", B.style.top = n.clientY - 28 + "px", B.style.display = "block", !$) {
      ...
      g();
    } else Vn(), ee.style.display = "none", vt.visible = false, de.visible = false, ct.visible = false, D(), g();
  }), q.derive(() => {
  ...
  const On = (n, o, a, t, s) => {
    ...
    r.updateMatrixWorld();
    const f = (pe) => {
      const we = new S(pe[0], pe[1], pe[2]);
      return we.project(r), { x: M.left + (we.x * 0.5 + 0.5) * M.width, y: M.top + (-we.y * 0.5 + 0.5) * M.height };
    }, b = (pe) => pe.x >= i && pe.x <= c && pe.y >= m && pe.y <= y, E = (pe, we) => !(pe.x < i && we.x < i || pe.x > c && we.x > c || pe.y < m && we.y < m || pe.y > y && we.y > y);
    s || re.clear();
    let T = 0;
    const V = ((_a2 = e.points) == null ? void 0 : _a2.rawVal) ?? [];
    for (let pe = 0; pe < V.length; pe++) {
      const we = V[pe];
      we && b(f(we)) && (re.add(`pt:${pe}`), T++);
    }
    const P = (pe, we) => _ ? b(pe) || b(we) || E(pe, we) : b(pe) && b(we), Y = ((_b = e.polylines) == null ? void 0 : _b.rawVal) ?? [], H = ((_c = e.areas) == null ? void 0 : _c.rawVal) ?? [];
    for (let pe = 0; pe < Y.length; pe++) {
      ...
      if (H.includes(pe)) {
        ...
        if (!_) Te = we.every((Se) => {
          ...
          Te = false;
          for (let Se = 0; Se < we.length - 1; Se++) {
            const Ye = V[we[Se]], Ue = V[we[Se + 1]];
        ...
        Te && (re.add(`poly:${pe}`), T++);
      } else for (let Te = 0; Te < we.length - 1; Te++) {
        const Se = V[we[Te]], Ye = V[we[Te + 1]];
        !Se || !Ye || P(f(Se), f(Ye)) && (re.add(`seg:${pe}:${Te}`), T++);
      }
    }
    const Pe = ((_d = window.__hekatanDrawingAuxLines) == null ? void 0 : _d.rawVal) ?? [];
    for (let pe = 0; pe < Pe.length; pe++) {
      const we = Pe[pe];
      if (!we || we.length !== 6) continue;
      const We = f([we[0], we[1], we[2]]), Te = f([we[3], we[4], we[5]]);
      P(We, Te) && (re.add(`aux:${pe}`), T++);
    }
    Tt(), j(`${_ ? "\u{1F7E2} Crossing" : "\u{1F535} Window"} \u2014 ${T} item(s) ${s ? "agregados a" : "\u2192"} selecci\xF3n (total ${re.size})`), Ft.style.display = "none";
  }, En = () => {
    Lt && (Lt = null, Ft.style.display = "none", j("Selecci\xF3n cancelada"));
  };
  ...
  const _o = () => {
    ...
    const n = [...re], o = ((_a2 = e.points) == null ? void 0 : _a2.rawVal) ?? [], a = ((_b = e.polylines) == null ? void 0 : _b.rawVal) ?? [], t = ((_c = e.areas) == null ? void 0 : _c.rawVal) ?? [], s = window.__hekatanDrawingAuxLines, i = (s == null ? void 0 : s.rawVal) ?? [], c = /* @__PURE__ */ new Set(), m = /* @__PURE__ */ new Set(), y = /* @__PURE__ */ new Map(), _ = /* @__PURE__ */ new Set();
    for (const E of n) {
      const [T, ...V] = E.split(":");
      if (T === "pt") c.add(+V[0]);
    ...
    return j(`\u{1F5D1} ${M} item(s) borrado(s)`), true;
  };
  window.__hekatanDeleteSelected = _o, window.addEventListener("keydown", (n) => {
    ...
    o && (o.tagName === "INPUT" || o.tagName === "TEXTAREA" || o.isContentEditable) && !a || re.size !== 0 && (n.preventDefault(), _o());
  });
  ...
  const cs = () => {
    ...
    n.addEventListener("mousedown", (c) => {
      ...
      const m = Vt.getBoundingClientRect();
      s = m.left, i = m.top, Vt.style.transform = "none", Vt.style.left = `${s}px`, Vt.style.top = `${i}px`, c.preventDefault();
      ...
      Vt.style.left = `${_}px`, Vt.style.top = `${M}px`;
    }), window.addEventListener("mouseup", () => {
      if (o) {
        o = false;
        try {
          localStorage.setItem(So, JSON.stringify({ left: parseFloat(Vt.style.left), top: parseFloat(Vt.style.top) }));
        } catch {
        }
      }
    });
  ...
  const Mt = (n, o, a, t) => {
    ...
    if (ot && (ot.dispose(), ot = null), re.size === 0) {
      Vt.style.display = "none";
      return;
    }
    const n = [...re], o = n.filter((r) => r.startsWith("pt:")), a = n.filter((r) => r.startsWith("seg:")), t = n.filter((r) => r.startsWith("poly:")), s = n.filter((r) => r.startsWith("aux:")), i = o.length > 0, c = a.length > 0, m = t.length > 0, y = !i && !c && !m, _ = [];
    ...
    const M = `\u{1F3AF} ${re.size} item(s) \u2014 ${_.join(", ")}`;
    ot = new es({ container: Vt, title: M });
    {
      ...
      r.addBinding(Ct, "dx", { label: "\u0394x (m)", step: 0.1 }), r.addBinding(Ct, "dy", { label: "\u0394y (m)", step: 0.1 }), r.addBinding(Ct, "dz", { label: "\u0394z (m)", step: 0.1 }), r.addBinding(Ct, "copias", { label: "Copias", min: 1, max: 50, step: 1 }), r.addButton({ title: "\u29C9 Replicar selecci\xF3n" }).on("click", () => {
        ...
        j(b ? `\u29C9 Replicado \xD7${b} (\u0394 ${Ct.dx},${Ct.dy},${Ct.dz} m)` : "\u26A0 Nada que replicar \u2014 seleccion\xE1 nodos/frames/\xE1reas");
        ...
        j(b ? `\u2192 Copia desplazada \u0394 ${Ct.dx},${Ct.dy},${Ct.dz} m` : "\u26A0 Nada seleccionado");
      });
      ...
      f.addButton({ title: "Snap a grilla ON/OFF (F9)" }).on("click", () => {
        ...
        window.__hekatanOsnapOn = !(window.__hekatanOsnapOn ?? true), j(`\u{1F9F2} OSNAP ${window.__hekatanOsnapOn ? "ON" : "OFF"}`);
      });
    }
    if (i) {
      ...
      b.addBinding(J, "Fx", { step: 0.1 }), b.addBinding(J, "Fy", { step: 0.1 }), b.addBinding(J, "Fz", { step: 0.1 }), b.addBinding(J, "Mx", { step: 0.1 }), b.addBinding(J, "My", { step: 0.1 }), b.addBinding(J, "Mz", { step: 0.1 }), ot.addFolder({ title: "\u2696 Additional Mass (kg)", expanded: false }).addBinding(J, "mass", { label: "m", min: 0, step: 1 }), ot.addFolder({ title: "\u{1F517} Diaphragm (rigid link)", expanded: false }).addBinding(J, "diaphragm", { label: "Diafragma", options: { Ninguno: "Ninguno", "D1 (rigid)": "D1 (rigid)", "D2 (rigid)": "D2 (rigid)", "D3 (rigid)": "D3 (rigid)" } }), ot.addButton({ title: `\u2713 Aplicar a ${o.length} nodo(s) seleccionado(s)` }).on("click", () => {
        ...
        if (H.some((oe) => oe !== 0) && (Mt("nodes", o, "springs", H), V++), J.mass !== 0 && (Mt("nodes", o, "mass", J.mass), V++), J.diaphragm !== "Ninguno" && (Mt("nodes", o, "diaphragm", J.diaphragm), V++), V === 0) {
          j("\u26A0 Nada que aplicar \u2014 marc\xE1 un DOF (Ux\u2026Rz) para apoyo, o un valor de carga/resorte/masa, y volv\xE9 a aplicar.");
        ...
        } else j(`\u2713 Propiedades aplicadas a ${o.length} nodo(s)`);
      });
    }
    if (c) {
      ...
      H.addBinding(J, "qx", { step: 0.1 }), H.addBinding(J, "qy", { step: 0.1 }), H.addBinding(J, "qz", { step: 0.1 }), ot.addFolder({ title: "\u2696 Additional Mass (kg/m)", expanded: false }).addBinding(J, "massPerM", { label: "m/L", min: 0, step: 1 }), ot.addButton({ title: "\u2713 Aplicar a segmentos seleccionados" }).on("click", () => {
        ...
        const pe = [J.relMxI, J.relMyI, J.relMzI], we = [J.relMxJ, J.relMyJ, J.relMzJ];
        (pe.some((Se) => Se) || we.some((Se) => Se)) && Mt("segs", a, "releases", { i: pe, j: we }), J.hinges !== "None" && Mt("segs", a, "hinges", J.hinges);
        const We = [J.LKx, J.LKy, J.LKz];
        We.some((Se) => Se !== 0) && Mt("segs", a, "lineSprings", We);
        const Te = [J.qx, J.qy, J.qz];
        Te.some((Se) => Se !== 0) && Mt("segs", a, "distLoad", Te), J.massPerM !== 0 && Mt("segs", a, "massPerM", J.massPerM), j(`\u2713 Propiedades aplicadas a ${a.length} segmento(s)`);
      });
    }
    if (m) {
      ...
      r.addBinding(J, "shellType", { label: "Tipo", options: { "Mindlin (FSDT)": "Mindlin (FSDT)", "Kirchhoff (CPT)": "Kirchhoff (CPT)", "Plane stress": "Plane stress" } }), r.addBinding(J, "thickness", { label: "Espesor (m)", min: 0.01, step: 0.01 }), r.addBinding(J, "material_shell", { label: "Material", options: { "Concreto C20": "Concreto C20", "Concreto C25": "Concreto C25", "Concreto C30": "Concreto C30", "Acero A36": "Acero A36" } }), ot.addFolder({ title: "\u2B07 Carga superficial (kN/m\xB2)" }).addBinding(J, "surfLoad", { label: "q", step: 0.1 }), ot.addButton({ title: "\u2713 Aplicar a \xE1reas seleccionadas" }).on("click", () => {
        Mt("areas", t, "shellType", J.shellType), Mt("areas", t, "thickness", J.thickness), Mt("areas", t, "material", J.material_shell), J.surfLoad !== 0 && Mt("areas", t, "surfLoad", J.surfLoad), j(`\u2713 Propiedades aplicadas a ${t.length} \xE1rea(s)/shell(s)`);
      });
    }
    if (y) {
  ...
  x.addEventListener("pointerdown", (n) => {
    ...
    if (n.button === 2) {
      ...
      if (o) {
        if (Lt ? En() : window.dispatchEvent(new KeyboardEvent("keydown", { key: "Escape", bubbles: true })), re.size > 0 && (re.clear(), Tt()), e.polylines) {
        ...
        s && s !== "select" && s !== "none" ? ((_c = t == null ? void 0 : t.setTool) == null ? void 0 : _c.call(t, "select"), j(`\u238B Cancelado \u2014 tool '${s}' cerrado, volv\xE9s a Seleccionar`)) : j("\u238B Cancelado (click derecho)");
      }
    }
    ...
    if (Lt && n.buttons === 0) {
      ...
      On(Lt.x, Lt.y, n.clientX, n.clientY, i);
      return;
    }
    if (!Nt) return;
  ...
  const ps = { end: 16724804, mid: 16498468, node: 6333946, cen: 3462041, per: 12616956, nea: 16744118, int: 16746496 }, ko = (n, o, a, t) => {
    ...
    for (const r of c) if (!(r.length < 2)) for (let f = 0; f < r.length - 1; f++) {
      ...
      if (!(!b || !E) && (s.end && (y("end", b[0], b[1], b[2]), y("end", E[0], E[1], E[2])), s.mid && y("mid", (b[0] + E[0]) / 2, (b[1] + E[1]) / 2, (b[2] + E[2]) / 2), s.nea || s.per)) {
        ...
        if (Y < 1e-12) continue;
        const H = Math.max(0, Math.min(1, ((n - b[0]) * T + (o - b[1]) * V + (a - b[2]) * P) / Y)), oe = b[0] + H * T, Pe = b[1] + H * V, pe = b[2] + H * P;
        s.nea && y("nea", oe, Pe, pe), s.per && y("per", oe, Pe, pe);
      }
    }
    if (s.cen) {
      const r = wn(), f = [...rn];
    ...
    if (s.int) {
      ...
      for (let f = 0; f < r.length; f++) for (let b = f + 1; b < r.length; b++) {
        const [E, T] = r[f], [V, P] = r[b], Y = [T[0] - E[0], T[1] - E[1], T[2] - E[2]], H = [P[0] - V[0], P[1] - V[1], P[2] - V[2]], oe = [E[0] - V[0], E[1] - V[1], E[2] - V[2]], Pe = Y[0] * Y[0] + Y[1] * Y[1] + Y[2] * Y[2], pe = Y[0] * H[0] + Y[1] * H[1] + Y[2] * H[2], we = H[0] * H[0] + H[1] * H[1] + H[2] * H[2], We = Y[0] * oe[0] + Y[1] * oe[1] + Y[2] * oe[2], Te = H[0] * oe[0] + H[1] * oe[1] + H[2] * oe[2], Se = Pe * we - pe * pe;
        if (Se < 1e-12) continue;
        const Ye = (pe * Te - we * We) / Se, Ue = (Pe * Te - pe * We) / Se;
  ...
  const fs = () => {
    ...
    switch (n) {
      ...
      case "move":
        return re.size ? s(t ? "MOVER Precise segundo punto (o teclee @dx,dy,dz):" : "MOVER Precise punto base:") : s("MOVER Designe objetos (S o ventana) y vuelva a M:");
      case "copy":
        return re.size ? s(t ? "COPIAR Precise segundo punto (o teclee @dx,dy,dz):" : "COPIAR Precise punto base:") : s("COPIAR Designe objetos (S o ventana) y vuelva a CO:");
      case "select":
        return re.size ? s(`SELECCI\xD3N ${re.size} objeto${re.size === 1 ? "" : "s"} \xB7 Supr borra \xB7 M mueve \xB7 CO copia \xB7 Esc suelta:`) : s("Designe objetos (clic-clic: ventana izq\u2192der, captura der\u2192izq) o teclee un comando:");
      default:
        return s(hs);
    }
  }, It = () => {
  ...
  window.__hekatanCadRefreshPrompt = It, window.__hekatanRefreshStatus = () => {
    ...
    Ae = [], fe = [], G.visible = false, Qn(), St = null, g(), j("\u{1F6E0} Tool cambiado \u2014 clicks pendientes limpiados"), It();
  };
  function Qn() {
    if (!e.polylines) return;
  ...
  const un = [], $n = [], jn = () => {
    ...
    var _a2;
    e.points.val = n.p, e.polylines && (e.polylines.val = n.l), e.areas && (e.areas.val = n.a), Ae = [], de.visible = false, ct.visible = false, D();
    try {
      (_a2 = window.__hekatanRebuild) == null ? void 0 : _a2.call(window);
    } catch {
    }
    ...
    const n = un.pop();
    if (!n) {
      j("\u21B6 Nada para deshacer");
      return;
    }
    $n.push(jn()), Po(n), j(`\u21B6 Deshacer \u2014 quedan ${un.length}`);
  }, Co = () => {
    const n = $n.pop();
    if (!n) {
      j("\u21B7 Nada para rehacer");
    ...
    un.push(jn()), Po(n), j(`\u21B7 Rehacer \u2014 quedan ${$n.length}`);
  };
  window.__hekatanPushUndo = Rt, window.__hekatanUndo = Ln, window.__hekatanRedo = Co, document.addEventListener("keydown", (n) => {
    ...
    if (o === "c" || o === "cerrar" || o === "close") {
      if (s.length < 3) return j("Cerrar necesita al menos tres puntos."), true;
      Rt(), e.polylines.val = [...t.slice(0, -1), [...s, s[0]], []];
      try {
        (_d = window.__hekatanRebuild) == null ? void 0 : _d.call(window);
      } catch {
      }
      return eo(), j(`\u2713 Polil\xEDnea cerrada \u2014 ${s.length} tramos.`), true;
    }
    if (o === "u" || o === "deshacer" || o === "undo") {
      ...
      return g(), j(`\u21B6 \xDAltimo punto quitado \u2014 quedan ${c.length}.`), It(), true;
    }
    return false;
  ...
  const eo = () => {
    Ae = [], St = null, Qn(), at = null, Zt(), de.visible = false, ct.visible = false, D(), j("\u23F9 Dibujo finalizado \u2014 click para empezar otra serie"), g(), It();
  };
  window.__hekatanFinalizeDraw = eo;
  const zo = () => {
    var _a2, _b, _c;
    Ae = [], fe = [], G.visible = false;
    let n = false;
    re.size && (re.clear(), Tt(), n = true), eo();
    try {
    ...
    j(n ? "\u238B Selecci\xF3n cancelada" : "\u238B Sin herramienta \u2014 clic-clic para seleccionar por ventana"), g(), It();
  };
  ...
  const Eo = (n, o) => {
    var _a2, _b, _c, _d, _e;
    if (!re.size) {
      j(`${n === "move" ? "MOVER" : "COPIAR"}: primero designe objetos (S, o ventana), luego vuelva al comando.`), (_b = (_a2 = window.__hekatanCadState) == null ? void 0 : _a2.setTool) == null ? void 0 : _b.call(_a2, "select"), It();
      return;
    }
    if (Ae.push(o), Ae.length === 1) {
      R = o, j(`${n === "move" ? "MOVER" : "COPIAR"} punto base (${o[0].toFixed(2)}, ${o[1].toFixed(2)}, ${o[2].toFixed(2)}). Precise el segundo punto.`), It();
      return;
    }
    const [a, t] = Ae, s = [t[0] - a[0], t[1] - a[1], t[2] - a[2]];
    Ae = [], de.visible = false;
    let i = 0;
    n === "move" ? i = Ao(s[0], s[1], s[2]) : (i = Fo().size, (_c = window.__hekatanReplicateSelection) == null ? void 0 : _c.call(window, s[0], s[1], s[2], 1)), j(`\u2713 ${n === "move" ? "Movidos" : "Copiados"} ${i} nudo${i === 1 ? "" : "s"} \u2014 \u0394 (${s[0].toFixed(2)}, ${s[1].toFixed(2)}, ${s[2].toFixed(2)}) m.`), n === "move" && (re.clear(), Tt()), (_e = (_d = window.__hekatanCadState) == null ? void 0 : _d.setTool) == null ? void 0 : _e.call(_d, "select"), It();
  };
  ...
  const ws = () => {
    ...
    if (!St) {
      if (Ze < 0) {
        j(`${s}: pase el cursor por una l\xEDnea (se pone roja) y haga clic.`);
        return;
      }
      St = { poly: Ze, seg: Math.max(0, He) }, j(n === "offset" ? `DESFASE l\xEDnea #${St.poly + 1} designada \u2014 clic en el lado hacia el que va la copia${on > 0 ? ` (${on} m)` : ""}.` : n === "trim" ? "RECORTAR contorno designado \u2014 clic en el trozo de l\xEDnea a quitar." : "ALARGAR contorno designado \u2014 clic en la l\xEDnea a alargar, cerca del extremo libre."), It();
      return;
    }
    if (n === "offset") {
      const T = St.poly, V = a[T];
      if (!V || V.length < 2) {
        St = null, j("DESFASE: esa polil\xEDnea no tiene tramos."), It();
        return;
      }
      const P = V.length > 2 && V[0] === V[V.length - 1], Y = ws(), H = [];
      for (let Ce = 0; Ce < V.length - 1; Ce++) {
        const rt = t[V[Ce]], je = t[V[Ce + 1]], _e = [je[0] - rt[0], je[1] - rt[1], je[2] - rt[2]], Re = Math.hypot(_e[0], _e[1], _e[2]) || 1, et = _e[0] / Re, At = _e[1] / Re, Yt = _e[2] / Re, Bt = [Y[1] * Yt - Y[2] * At, Y[2] * et - Y[0] * Yt, Y[0] * At - Y[1] * et], Wt = Math.hypot(Bt[0], Bt[1], Bt[2]) || 1;
      ...
      const pe = H[oe], we = Math.sign((o[0] - pe.a[0]) * pe.n[0] + (o[1] - pe.a[1]) * pe.n[1] + (o[2] - pe.a[2]) * pe.n[2]) || 1, We = on > 0 ? on : Pe;
      if (We < 1e-6) {
        j("DESFASE: distancia nula \u2014 teclee una distancia o clique m\xE1s lejos.");
        return;
      }
      const Te = H.map((Ce) => ({ a: [Ce.a[0] + we * We * Ce.n[0], Ce.a[1] + we * We * Ce.n[1], Ce.a[2] + we * We * Ce.n[2]], b: [Ce.b[0] + we * We * Ce.n[0], Ce.b[1] + we * We * Ce.n[1], Ce.b[2] + we * We * Ce.n[2]] })), Se = Te.length, Ye = (Ce) => {
      ...
      P && Dt.push(ft);
      let ht = a.slice();
      ht.length && ht[ht.length - 1].length === 0 && (ht = ht.slice(0, -1)), e.polylines.val = [...ht, Dt, []], St = null, j(`\u2713 Desfase a ${We.toFixed(2)} m \u2014 ${Se} tramo${Se === 1 ? "" : "s"} nuevo${Se === 1 ? "" : "s"}. Designe otra l\xEDnea o Esc.`);
      try {
        (_a2 = window.__hekatanRebuild) == null ? void 0 : _a2.call(window);
      } catch {
      }
    ...
    if (i < 0 || i === St.poly && c === St.seg) {
      ...
      if (i = -1, a.forEach((P, Y) => {
        for (let H = 0; H < P.length - 1; H++) {
          ...
          if (!oe || !Pe) continue;
          const pe = Jt(o[0], o[1], o[2], oe[0], oe[1], oe[2], Pe[0], Pe[1], Pe[2]);
          pe < V && (V = pe, i = Y, c = H);
        }
      }), i < 0) {
        j(`${s}: pase el cursor por OTRA l\xEDnea y haga clic.`);
        return;
      }
    }
    const m = a[St.poly], y = t[m[St.seg]], _ = t[m[St.seg + 1]], M = a[i], r = M[c], f = M[c + 1];
    if (!y || !_ || r == null || f == null) {
      j(`${s}: no se pudo leer el tramo.`);
      return;
    }
    const b = t[r], E = t[f];
    if (n === "trim") {
      ...
      if (!T) {
        j("RECORTAR: esa l\xEDnea no cruza el contorno designado.");
        return;
      }
      Rt();
      const V = t.length;
      ...
      Fn(i, Y ? c : c + 1), j(`\u2713 Recortado en (${T[0].toFixed(2)}, ${T[1].toFixed(2)}, ${T[2].toFixed(2)}). Designe otro trozo o Esc.`);
    } else {
      const T = to(b, E, y, _, true, false);
      if (!T) {
        j("ALARGAR: ni prolongada llega esa l\xEDnea al contorno.");
        return;
      }
      const P = Ot(o, b) < Ot(o, E) ? c : c + 1;
      if (P !== 0 && P !== M.length - 1) {
        j("ALARGAR: solo se alarga un extremo libre de la polil\xEDnea.");
        return;
      }
      const Y = M[P];
      if (Ot(T, b) + Ot(T, E) < Ot(b, E) + 1e-6) {
        j("ALARGAR: el contorno corta el tramo por dentro; use RECORTAR.");
        return;
      }
      if (Rt(), ys(Y) > 1) {
        const oe = t.length;
        e.points.val = [...t, T];
        const Pe = M.slice();
        Pe[P] = oe, e.polylines.val = a.map((pe, we) => we === i ? Pe : pe);
      } else e.points.val = t.map((oe, Pe) => Pe === Y ? T : oe);
      j(`\u2713 Alargada hasta (${T[0].toFixed(2)}, ${T[1].toFixed(2)}, ${T[2].toFixed(2)}). Designe otra l\xEDnea o Esc.`);
    }
    try {
  ...
  window.__hekatanSelectionSize = () => re.size, window.__hekatanSelectLast = () => {
    var _a2;
    const n = ((_a2 = e.polylines) == null ? void 0 : _a2.rawVal) ?? [];
    let o = n.length - 1;
    for (; o >= 0 && (!n[o] || n[o].length < 2); ) o--;
    return re.clear(), o >= 0 && re.add(`poly:${o}`), Tt(), j(o >= 0 ? "SELECCI\xD3N 1 objeto (el \xFAltimo dibujado) \xB7 Esc suelta" : "No hay ning\xFAn objeto dibujado todav\xEDa."), re.size;
    ...
    return n.forEach((t, s) => {
      !t || t.length < 2 || (re.add(`poly:${s}`), t.forEach((i) => a.add(i)));
    }), o.forEach((t, s) => {
      a.has(s) || re.add(`pt:${s}`);
    }), Tt(), j(`SELECCI\xD3N ${re.size} objetos (todo el modelo) \xB7 Esc suelta`), re.size;
  }, window.__hekatanReplicateSelection = (n, o, a, t) => {
    var _a2, _b, _c, _d;
    t = Math.max(1, Math.round(t || 1));
    const s = [...re], i = e.points.rawVal, c = ((_a2 = e.polylines) == null ? void 0 : _a2.rawVal) ?? [], m = new Set(((_b = e.areas) == null ? void 0 : _b.rawVal) ?? []), y = /* @__PURE__ */ new Set(), _ = /* @__PURE__ */ new Set(), M = [];
    if (s.forEach((T) => {
    ...
    for (let T = 1; T <= t; T++) {
      ...
      E.forEach((oe) => {
        ...
        const Pe = c[oe].map((we) => H.has(we) ? H.get(we) : we), pe = f.length;
        f.push(Pe), m.has(oe) && b.push(pe);
      }), M.forEach(([oe, Pe]) => {
    ...
    if (!a.length) return;
    {
      ...
      if (!isFinite(c.x) || !isFinite(c.y) || !isFinite(c.z) || i > Math.max(s * 12, 300)) {
        j("\u26A0 Click rasante descartado \u2014 cay\xF3 demasiado lejos. Acerc\xE1 la vista o clicke\xE1 sobre la grilla.");
        return;
      }
    }
    let t = a[0].point;
    ...
    if (dt) t = dt.clone(), j(`\u{1F4D0} Eje \u2192 (${t.x.toFixed(2)}, ${t.y.toFixed(2)}, ${t.z.toFixed(2)})`);
    else {
      const s = (window.__hekatanSnap2D ?? 0.5) * 1.2, i = (_b = window.__hekatanOsnapCompute) == null ? void 0 : _b.call(window, t.x, t.y, t.z, s);
      if (i) t = new S(i.x, i.y, i.z), j(`\u{1F3AF} Snap [${i.type.toUpperCase()}] \u2192 (${t.x.toFixed(2)}, ${t.y.toFixed(2)}, ${t.z.toFixed(2)})`);
      else {
  ...
  const To = (n, o) => {
    ...
    if (a === "select" || a === "none" || !a) {
      if (lt) {
        ...
        !!o && (o.ctrlKey || o.metaKey || o.shiftKey) || re.clear(), re.has(c) ? re.delete(c) : re.add(c), Tt(), j(`\u2713 Seleccionados ${re.size} elemento(s) \u2014 Ctrl+Click para multi-selecci\xF3n`);
      } else {
        const t = !!o && (o.ctrlKey || o.metaKey || o.shiftKey), s = (o == null ? void 0 : o.clientX) ?? 0, i = (o == null ? void 0 : o.clientY) ?? 0;
        Lt ? (bo(Lt.x, Lt.y, s, i, t), Lt = null) : t || (Lt = { x: s, y: i }, j("\u{1F5B1} Click 2 para cerrar el rect\xE1ngulo (\u2192 derecha=Window azul, \u2190izquierda=Crossing verde). Esc=cancelar."), On(s, i, s + 1, i + 1, false));
      }
      return;
    }
    if (a === "axis") {
      ...
      if (!t) return;
      if (!t.pendingStart) {
        t.pendingStart = [n.x, n.y, n.z], j(`\u{1F4CD} Eje \u2014 click 1 OK en (${n.x.toFixed(2)}, ${n.y.toFixed(2)}, ${n.z.toFixed(2)}). Click 2=fin.`);
        return;
      }
      const s = t.mode === "number", i = (_d = window.__hekatanAxisCommit) == null ? void 0 : _d.call(window, t.pendingStart, [n.x, n.y, n.z], s);
      j(`\u2713 Eje "${i}" creado. Click 1=nuevo eje, o cambia tool.`);
      return;
    }
    if (a === "move" || a === "copy") {
      Eo(a, [n.x, n.y, n.z]);
    ...
    if (a === "delete") {
      if (xt >= 0) {
        ...
        if (i >= 0 && i < s.length) {
          ...
          t && typeof t == "object" && "val" in t ? t.val = c : window.__hekatanDrawingAuxLines = c, j(`\u{1F5D1} L\xEDnea auxiliar #${i + 1} borrada`), xt = -1, xe.visible = false;
          try {
            (_e = window.__hekatanRebuild) == null ? void 0 : _e.call(window);
          } catch {
          }
        ...
        const t = Ze, s = He;
        ((_g = (_f = e.areas) == null ? void 0 : _f.rawVal) == null ? void 0 : _g.includes(t)) ?? false ? (ln(t), j(`\u{1F5D1} \xC1rea #${t + 1} (shell Q4) borrada`)) : s >= 0 ? (Fn(t, s), j(`\u{1F5D1} Segmento ${s + 1} de polil\xEDnea #${t + 1} borrado`)) : (ln(t), j(`\u{1F5D1} Polil\xEDnea #${t + 1} borrada`));
      } else j("\u{1F5D1} Acerc\xE1 el cursor a una l\xEDnea/\xE1rea/aux para borrarla");
      return;
    }
    if (a === "circle") {
      if (Ae.push([n.x, n.y, n.z]), Ae.length === 1) {
        j("\u25CB C\xEDrculo \u2014 click 1/2 OK (centro). Ahora marc\xE1 el radio.");
        return;
      }
      ...
      (_h = window.__hekatanDrawCircle) == null ? void 0 : _h.call(window, t[0], t[1], t[2], i, _, y), j(`\u2713 C\xEDrculo dibujado en ${y.toUpperCase()} \u2014 r=${i.toFixed(2)}m, ${_} segmentos`), Ae = [];
      try {
    ...
    if (a === "arc") {
      if (Ae.push([n.x, n.y, n.z]), Ae.length === 1) {
        j("\u2312 Arco \u2014 click 1/3 OK (inicio). Marc\xE1 el punto medio.");
        return;
      }
      if (Ae.length === 2) {
        j("\u2312 Arco \u2014 click 2/3 OK (medio). Marc\xE1 el final.");
        return;
      }
      const [t, s, i] = Ae, c = window.__hekatanArcSegs ?? 12;
      (_j = window.__hekatanDrawArc) == null ? void 0 : _j.call(window, t, s, i, c), j(`\u2713 Arco dibujado \u2014 ${c} segmentos`), Ae = [];
      try {
    ...
    if (a === "rect") {
      if (Ae.push([n.x, n.y, n.z]), Ae.length === 1) {
        j("\u25AD Rect\xE1ngulo \u2014 click 1/2 OK (esquina). Marc\xE1 la esquina opuesta.");
        return;
      }
      const [t, s] = Ae;
      (_l = window.__hekatanDrawRect) == null ? void 0 : _l.call(window, t, s), j(`\u2713 Rect\xE1ngulo dibujado \u2014 (${t[0].toFixed(1)},${t[1].toFixed(1)}) \u2192 (${s[0].toFixed(1)},${s[1].toFixed(1)})`), Ae = [];
      try {
    ...
    if (a === "rectarea") {
      if (Ae.push([n.x, n.y, n.z]), Ae.length === 1) {
        j("\u25AD \xC1rea rectangular \u2014 click 1/2 OK (esquina). Marc\xE1 la esquina opuesta.");
        return;
      }
      const [t, s] = Ae;
      (_n = window.__hekatanDrawRectArea) == null ? void 0 : _n.call(window, t, s), j(`\u2713 \xC1rea rectangular (shell Q4) creada \u2014 (${t[0].toFixed(1)},${t[1].toFixed(1)}) \u2192 (${s[0].toFixed(1)},${s[1].toFixed(1)})`), Ae = [];
      return;
    }
    if (a === "polyarea") {
      fe.push([n.x, n.y, n.z]), G.geometry.setFromPoints(fe.map((t) => new S(t[0], t[1], t[2]))), G.visible = fe.length >= 1, j(`\u25B0 \xC1rea libre \u2014 ${fe.length} punto(s). Click m\xE1s v\xE9rtices, o Enter / click-derecho para cerrar y mallar (m\xEDn. 3).`), g();
      return;
    }
    if (a === "plane3") {
      if (Ae.push([n.x, n.y, n.z]), Ae.length < 3) {
        j(`\u25E3 Plano inclinado \u2014 punto ${Ae.length}/3. Tip: cambi\xE1 la Cota Z (o enganch\xE1 un nodo) entre clicks para darle inclinaci\xF3n.`);
        return;
      }
      const [t, s, i] = Ae, c = (_o2 = window.__hekatanSetInclinedPlaneFrom3) == null ? void 0 : _o2.call(window, t, s, i);
      j(c ? "\u2713 Plano de trabajo INCLINADO activo. Dibuj\xE1 el \xE1rea (\u25AD/\u2B21) sobre \xE9l. (XY para resetear)" : "\u26A0 Los 3 puntos son colineales \u2014 no definen un plano. Reintent\xE1."), Ae = [];
      return;
    }
    if (a === "col") {
      Rt();
      ...
      e.polylines.val = [...i.slice(0, -1), ...i[i.length - 1].length > 0 ? [i[i.length - 1]] : [], [c - 2, c - 1], []], wt = 0, j(`\u258C Columna creada \u2014 h=${s.toFixed(2)}m. Tipe\xE1 altura + Enter para custom.`);
      try {
    ...
    if (a === "wall") {
      if (Ae.push([n.x, n.y, n.z]), Ae.length === 1) {
        j("\u25A5 Pared Q4 \u2014 click 1/2 OK (esquina base 1). Marc\xE1 la otra esquina base.");
        return;
      }
      ...
      j(`\u25A5 Pared Q4 creada \u2014 h=${i.toFixed(2)}m. Tipe\xE1 altura + Enter para custom.`), Ae = [], wt = 0;
      try {
    ...
    if (a === "extp") {
      ...
      const i = e.polylines.rawVal, c = e.points.rawVal.length;
      e.polylines.val = [...i.slice(0, -1), ...i[i.length - 1].length > 0 ? [i[i.length - 1]] : [], [c - 2, c - 1], []], wt = 0, j(`\u2B06 Extrusi\xF3n punto\u2192l\xEDnea \u2014 h=${t.toFixed(2)}m`);
      try {
    ...
    if (a === "extl") {
      ...
      if (!s) {
        j("\u2B06 Extruir l\xEDnea \u2014 acerc\xE1 el cursor a una l\xEDnea existente y volv\xE9 a clickear.");
        return;
      }
      const i = e.polylines.rawVal, c = e.points.rawVal, m = i[s.polyIdx], y = c[m[s.segIdx]], _ = c[m[s.segIdx + 1]];
      if (!y || !_) {
        j("\u2B06 Extruir l\xEDnea \u2014 segmento no v\xE1lido.");
        return;
      }
      ...
      wt = 0, j(`\u2B06 Extrusi\xF3n l\xEDnea\u2192\xE1rea Q4 \u2014 h=${M.toFixed(2)}m`);
      try {
    ...
    if (a === "auxp") {
      ...
      j(`\u2726 Punto auxiliar agregado en (${n.x.toFixed(2)}, ${n.y.toFixed(2)}, ${n.z.toFixed(2)})`);
      return;
    }
    if (a === "aux") {
      if (Ae.push([n.x, n.y, n.z]), Ae.length === 1) {
        j("\u250A L\xEDnea auxiliar \u2014 click 1/2 OK. Marc\xE1 el punto final.");
        return;
      }
      ...
      j(`\u2713 L\xEDnea auxiliar creada \u2014 L=${_.toFixed(2)}m (cyan, no FEM)`), Ae = [];
      return;
    }
    ...
    if (a === "chaflan") {
      if (Ae.push([n.x, n.y, n.z]), Ae.length === 1) {
        j("\u25B1 Losa con chaflanes \u2014 click 1/2 OK (esquina). Marc\xE1 la esquina opuesta.");
        return;
      }
      ...
      j(`\u2713 Losa con chaflanes dibujada \u2014 ${m}\xD7${y}m, r=${i}m, ${c} seg/chafl\xE1n`), Ae = [];
      try {
    ...
    if ($ = false, Rt(), e.points.val = [...e.points.rawVal, n.toArray()], e.polylines && (e.polylines.val = [...e.polylines.rawVal.slice(0, -1), [...e.polylines.rawVal.length ? e.polylines.rawVal.pop() : [], e.points.rawVal.length - 1]]), e.polylines) {
      ...
      if (a === "line" && i.length >= 2) {
        j(`\uFF0F L\xEDnea \u2014 ${i.length - 1} tramo${i.length === 2 ? "" : "s"}. Segu\xED marcando puntos; Esc o clic derecho para terminar.`);
        try {
      ...
      if (a === "area" && i.length === 4) {
        e.polylines.val = [...t.slice(0, -1), [...i, i[0]], []], e.areas && (e.areas.val = [...e.areas.rawVal, s]), j("\u25A6 \xC1rea (shell Q4) creada \u2014 4 v\xE9rtices marcados.");
        try {
    ...
    if (a === "node") j(`\u25CF Nodo creado en (${n.x.toFixed(2)}, ${n.y.toFixed(2)}, ${n.z.toFixed(2)})`);
    else if (a === "line") j("\uFF0F L\xEDnea \u2014 click 1/2 OK. Marc\xE1 el segundo punto para crear el frame.");
    else if (a === "polyline") j("\u2310 Polil\xEDnea \u2014 punto agregado. Continu\xE1 clickeando, right-click para terminar.");
    else if (a === "area") {
      const t = ((_x = e.polylines) == null ? void 0 : _x.rawVal[e.polylines.rawVal.length - 1]) ?? [];
      j(`\u25A6 \xC1rea \u2014 click ${t.length}/4. Marc\xE1 ${4 - t.length} v\xE9rtice${4 - t.length === 1 ? "" : "s"} m\xE1s.`);
    }
  };
  x.addEventListener("click", () => It()), x.addEventListener("contextmenu", (n) => {
    ...
    if (((_c = (_b = (_a2 = window.__hekatanCadState) == null ? void 0 : _a2.get) == null ? void 0 : _b.call(_a2)) == null ? void 0 : _c.tool) === "polyarea" && fe.length >= 3) {
      n.preventDefault();
      const a = yn();
      j(`\u2713 \xC1rea libre mallada \u2014 ${a} shells Q4 creados.`);
      return;
    }
...
function ma(e, l, u, h) {
  ...
  return q.derive(() => {
    ...
    for (let L = 0; L < k.length; L++) {
      ...
      const ce = ha(C, L, v, Z);
      ce && (ye.push(ce[0], ce[1]), le.push({ idx: L, vals: ce }));
    }
    if (ye.length === 0) {
      x.val = [];
      return;
    ...
    const ge = Math.max(W[0] - ne[0], W[1] - ne[1], W[2] - ne[2], 1) * ua, B = [], R = [], U = [];
    let $ = 0;
    for (const { idx: L, vals: Q } of le) {
      const ce = k[L], se = g[ce[0]], ee = g[ce[1]];
      if (!se || !ee) continue;
      const I = new S(ee[0] - se[0], ee[1] - se[1], ee[2] - se[2]), de = I.length();
      if (de < 1e-10) continue;
      I.normalize();
      const G = Math.abs(I.y) < 0.99 ? new S(0, 1, 0) : new S(1, 0, 0), fe = new S().crossVectors(I, G).normalize(), me = new S().crossVectors(I, fe).normalize(), Ve = fo + 1, Me = pa;
      for (let Le = 0; Le < Ve; Le++) {
        const Qe = Le / fo, ct = se[0] + I.x * de * Qe, ae = se[1] + I.y * de * Qe, F = se[2] + I.z * de * Qe, K = Q[0] + (Q[1] - Q[0]) * Qe, N = w.getColor(K) ?? new Ut(0, 0, 0);
        p.copy(N).convertSRGBToLinear();
        for (let O = 0; O < Me; O++) {
          const ie = O / Me * Math.PI * 2, he = Math.cos(ie), ue = Math.sin(ie);
          B.push(ct + (fe.x * he + me.x * ue) * ge, ae + (fe.y * he + me.y * ue) * ge, F + (fe.z * he + me.z * ue) * ge), R.push(p.r, p.g, p.b);
        }
      }
    ...
    const A = new be();
    A.setAttribute("position", new bt(B, 3)), A.setAttribute("color", new bt(R, 3)), A.setIndex(U), A.computeVertexNormals();
...
function _a(e) {
  ...
  function A(ae, F) {
    ...
    const N = e.rendererElm.getBoundingClientRect(), O = ae - N.left, ie = F - N.top, he = e.derivedNodes.rawVal, ue = (_a2 = e.mesh.elements) == null ? void 0 : _a2.rawVal;
    if (!he || !ue) return null;
    const ke = /* @__PURE__ */ new Map(), Fe = (Ne) => {
      if (ke.has(Ne)) return ke.get(Ne);
      const Ie = $(Ne);
      if (!Ie) return ke.set(Ne, null), null;
      const ze = Ie.clone().project(K), qe = (ze.x * 0.5 + 0.5) * N.width, xe = (-ze.y * 0.5 + 0.5) * N.height, Ze = { x: qe, y: xe, z: ze.z };
      return ke.set(Ne, Ze), Ze;
    }, Ee = /* @__PURE__ */ new Set();
    for (const Ne of ue) if (Ne) for (const Ie of Ne) Ee.add(Ie);
    const De = 8;
    let Be = -1, Ke = De;
    for (let Ne = 0; Ne < he.length; Ne++) {
      ...
      const ze = Ie.x - O, qe = Ie.y - ie, xe = Math.sqrt(ze * ze + qe * qe);
      xe < Ke && (Ke = xe, Be = Ne);
    }
...
(${Ne[0].toFixed(3)}, ${Ne[1].toFixed(3)}, ${Ne[2].toFixed(3)})`;
      ...
      if (ze == null ? void 0 : ze.deformations) {
        ...
        if (qe && (Ie += `
\u2500\u2500\u2500\u2500 \u0394 desplaz. \u2500\u2500\u2500\u2500`, Ie += `
Ux = ${yt(qe[0] * mt, 3)} ${$e.dispUnit}`, Ie += `
Uy = ${yt(qe[1] * mt, 3)} ${$e.dispUnit}`, Ie += `
Uz = ${yt(qe[2] * mt, 3)} ${$e.dispUnit}`, (Math.abs(qe[3]) > 1e-9 || Math.abs(qe[4]) > 1e-9 || Math.abs(qe[5]) > 1e-9) && (Ie += `
Rx = ${yt(qe[3] * 1e3, 3)} mrad`, Ie += `
Ry = ${yt(qe[4] * 1e3, 3)} mrad`, Ie += `
Rz = ${yt(qe[5] * 1e3, 3)} mrad`)), ze.reactions) {
          const xe = ze.reactions.get(Be);
          xe && (Math.abs(xe[0]) > 1e-9 || Math.abs(xe[1]) > 1e-9 || Math.abs(xe[2]) > 1e-9 || Math.abs(xe[3]) > 1e-6 || Math.abs(xe[4]) > 1e-6 || Math.abs(xe[5]) > 1e-6) && (Ie += `
\u2500\u2500\u2500\u2500 R reacciones \u2500\u2500\u2500\u2500`, Ie += `
Fx = ${yt(xe[0] * Ge)} ${$e.forceUnit}`, Ie += `
Fy = ${yt(xe[1] * Ge)} ${$e.forceUnit}`, Ie += `
Fz = ${yt(xe[2] * Ge)} ${$e.forceUnit}`, (Math.abs(xe[3]) > 1e-6 || Math.abs(xe[4]) > 1e-6 || Math.abs(xe[5]) > 1e-6) && (Ie += `
Mx = ${yt(xe[3] * Ge)} ${$e.forceUnit}\xB7m`, Ie += `
My = ${yt(xe[4] * Ge)} ${$e.forceUnit}\xB7m`, Ie += `
Mz = ${yt(xe[5] * Ge)} ${$e.forceUnit}\xB7m`));
        }
      }
      return { type: "node", idx: Be, info: Ie };
    }
    const at = 5;
    ...
    for (let Ne = 0; Ne < ue.length; Ne++) {
      ...
      if (!(!Ie || Ie.length < 2)) {
        if (Ie.length === 2) {
          ...
          if (!qe) continue;
          const xe = Math.min(...ze.map((re) => re.x)), Ze = Math.max(...ze.map((re) => re.x)), He = Math.min(...ze.map((re) => re.y)), xt = Math.max(...ze.map((re) => re.y));
  ...
  Shape: ${xe.shape}`);
        const Ze = /concrete|hormig|rect.*sólida/i.test(xe.shape || ""), He = Ze ? 100 : 1e3, xt = Ze ? "cm" : "mm", re = (nt) => {
          const gt = nt * He;
          return Math.abs(gt - Math.round(gt)) < 0.05 ? `${Math.round(gt)}` : `${gt.toFixed(1)}`;
        }, Xe = [];
        if (xe.D != null && Xe.push(`D=${re(xe.D)}`), xe.B != null && Xe.push(`B=${re(xe.B)}`), xe.TF != null && Xe.push(`TF=${re(xe.TF)}`), xe.TW != null && Xe.push(`TW=${re(xe.TW)}`), xe.t != null && Xe.push(`t=${re(xe.t)}`), Xe.length && (ze += `
...
`));
      ...
      if (Zt === "frame" && ((_m = e.mesh) == null ? void 0 : _m.deformOutputs) && e.mesh.elementInputs) {
        ...
        if (xt && Ne.length === 2) {
          const re = xt.get(Ne[0]), Xe = xt.get(Ne[1]), nt = he[Ne[0]], gt = he[Ne[1]];
          if (re && Xe && nt && gt) {
            const Kt = gt[0] - nt[0], _t = gt[1] - nt[1], Et = gt[2] - nt[2], lt = Math.sqrt(Kt * Kt + _t * _t + Et * Et);
            if (lt > 1e-9) {
              const jt = Kt / lt, Tt = _t / lt, Jt = Et / lt, en = (Xe[0] - re[0]) * jt + (Xe[1] - re[1]) * Tt + (Xe[2] - re[2]) * Jt, tn = ((_n = He.elasticities) == null ? void 0 : _n.get(dt)) ?? 0, Kn = ((_o = He.areas) == null ? void 0 : _o.get(dt)) ?? 0, Gn = ((_p = He.momentsOfInertiaY) == null ? void 0 : _p.get(dt)) ?? 0, ln = ((_q = He.momentsOfInertiaZ) == null ? void 0 : _q.get(dt)) ?? 0, Fn = ((_r = He.torsionalConstants) == null ? void 0 : _r.get(dt)) ?? 0, rn = ((_s2 = He.shearModuli) == null ? void 0 : _s2.get(dt)) ?? tn / 2.6, mn = tn * Kn * (en / lt), An = (Xe[3] - re[3]) * jt + (Xe[4] - re[4]) * Tt + (Xe[5] - re[5]) * Jt, wn = rn * Fn * (An / lt), yn = Xe[4] - re[4], $t = Xe[5] - re[5], Xt = tn * Gn * yn / lt, Gt = tn * ln * $t / lt;
              ze += `
\u2500\u2500\u2500\u2500 frame \u2500\u2500\u2500\u2500`, ze += `
L = ${yt(lt, 3)} m`, ze += `
...
Mz \u2248 ${yt(Gt * Ge)} ${$e.forceUnit}\xB7m`);
  ...
  function X(ae, F, K) {
    ...
    if (ae.type === "node") {
      ...
      if (ue) {
        ...
        d.position.copy(ue), d.scale.setScalar(De), d.visible = true;
      }
    } else if (ae.type === "frame" && N) {
      const ue = N[ae.idx], ke = $(ue[0]), Fe = $(ue[1]);
      if (ke && Fe) {
      ...
      const ue = N[ae.idx], ke = [], Fe = [];
      for (const Ee of ue) {
        const De = $(Ee);
        if (!De) return;
        ke.push(De.x, De.y, De.z);
      }
      ue.length === 4 ? Fe.push(0, 1, 2, 0, 2, 3) : ue.length === 3 && Fe.push(0, 1, 2), C.setAttribute("position", new bt(ke, 3)), C.setIndex(Fe), C.computeVertexNormals(), Z.visible = true;
    } else if (ae.type === "solid" && N) {
      const ue = N[ae.idx], ke = [[0, 1], [1, 2], [2, 3], [3, 0], [4, 5], [5, 6], [6, 7], [7, 4], [0, 4], [1, 5], [2, 6], [3, 7]], Fe = [];
      for (const [Ee, De] of ke) {
        const Be = $(ue[Ee]), Ke = $(ue[De]);
    ...
    const ie = e.rendererElm.getBoundingClientRect(), he = ((_c = e.rendererElm.parentElement) == null ? void 0 : _c.getBoundingClientRect()) ?? ie;
    U.style.left = `${F - he.left}px`, U.style.top = `${K - he.top}px`, e.render();
  }
  let D = "", L = 0, Q = 0;
  const ce = window.__hekatanHoverDebug ?? false, se = (ae) => {
  ...
  const I = () => {
    ...
    (K < -2 || N < -2 || K > F.width + 2 || N > F.height + 2) && (ee && clearTimeout(ee), ee = window.setTimeout(I, 200));
  }, G = () => {
    ee && (clearTimeout(ee), ee = null);
  };
  e.rendererElm.addEventListener("pointermove", se), e.rendererElm.addEventListener("pointerleave", de), e.rendererElm.addEventListener("pointerenter", G);
  function fe() {
    var _a2, _b, _c;
  ...
  e.rendererElm.addEventListener("pointerdown", (ae) => {
    ae.button === 0 && (me = { x: ae.clientX, y: ae.clientY });
  }), e.rendererElm.addEventListener("pointerup", (ae) => {
    if (ae.button !== 0 || !me) return;
    const F = ae.clientX - me.x, K = ae.clientY - me.y;
    if (me = null, F * F + K * K > 9 || !fe()) return;
  ...
  function Ve() {
    for (const ae of R.children.slice()) {
      R.remove(ae);
      const F = ae.geometry;
      F && F !== u && F !== W && F.dispose();
    }
  ...
  function Me(ae, F) {
    ...
    if (ae.type === "node") {
      ...
      ie.position.copy(N), ie.scale.setScalar(0.025 * F * O), ie.renderOrder = 101, R.add(ie);
    } else if (ae.type === "frame" && K) {
      const N = K[ae.idx], O = $(N[0]), ie = $(N[1]);
      if (!O || !ie) return;
      const he = O.clone().add(ie).multiplyScalar(0.5), ue = ie.clone().sub(O), ke = ue.length(), Fe = e.getActiveCamera().position.distanceTo(he), Ee = new Oe(W, ne);
      Ee.position.copy(he);
      const De = new S(0, 1, 0);
      Ee.quaternion.setFromAxisAngle(De.clone().cross(ue).normalize(), De.angleTo(ue)), Ee.scale.set(Fe * 35e-4, ke, Fe * 35e-4), Ee.renderOrder = 101, R.add(Ee);
      ...
      const he = new be();
      he.setAttribute("position", new bt(O, 3)), he.setIndex(ie), he.computeVertexNormals();
      const ue = new Oe(he, ve);
      ue.renderOrder = 101, R.add(ue);
      ...
      const he = new be();
      he.setAttribute("position", new bt(ie, 3));
      const ue = new qt(he, ge);
      ue.renderOrder = 101, R.add(ue);
    }
  }
  function Le() {
...
function Da({ mesh: e, settingsObj: l, drawingObj: u, objects3D: h, solids: d }) {
  ...
  v.enableDamping = true, v.dampingFactor = 0.1, v.screenSpacePanning = true, v.zoomSpeed = 0.8, v.panSpeed = 1.2, v.rotateSpeed = 0.9, v.keyPanSpeed = 12, v.listenToKeyEvents(window), v.touches = { ONE: Rn.ROTATE, TWO: Rn.DOLLY_PAN }, C.domElement.addEventListener("wheel", (F) => {
    if (!F.ctrlKey && Math.abs(F.deltaX) > Math.abs(F.deltaY) * 1.5) {
      ...
      const he = N.length() * 1e-3 * v.panSpeed;
      K.addScaledVector(O, F.deltaX * he), x.position.addScaledVector(O, F.deltaX * he), v.update();
    }
  }, { passive: false });
  ...
  function te() {
    ...
    F.enableX && (Z.normal.set(F.invertX ? 1 : -1, 0, 0), Z.constant = F.invertX ? -F.posX : F.posX, K.push(Z)), F.enableY && (ye.normal.set(0, F.invertY ? 1 : -1, 0), ye.constant = F.invertY ? -F.posY : F.posY, K.push(ye)), F.enableZ && (le.normal.set(0, 0, F.invertZ ? 1 : -1), le.constant = F.invertZ ? -F.posZ : F.posZ, K.push(le)), C.clippingPlanes = K, p.traverse((O) => {
      const ie = O;
      if (ie.material) {
        const he = Array.isArray(ie.material) ? ie.material : [ie.material];
        for (const ue of he) ue.clippingPlanes = K, ue.needsUpdate = true;
      }
    });
  ...
  R(), w.appendChild(Ys(z, e, d)), w.setAttribute("id", "viewer"), w.appendChild(C.domElement), C.setPixelRatio(window.devicePixelRatio);
  const U = an();
  ...
  q.derive(() => {
    ...
    }), B = uo(F, ge()), B.visible = z.gridVisible.rawVal, p.add(B), R(), p.remove(X), X.traverse((ie) => {
      var _a3, _b, _c, _d;
      (_b = (_a3 = ie.geometry) == null ? void 0 : _a3.dispose) == null ? void 0 : _b.call(_a3), (_d = (_c = ie.material) == null ? void 0 : _c.dispose) == null ? void 0 : _d.call(_c);
    }), X = qo(F, K), p.add(X);
    ...
    for (const K of F) {
      ...
      const ue = g.top;
      if (g.left = -ue * he, g.right = ue * he, g.updateProjectionMatrix(), ee && ee.isPerspectiveCamera) ee.aspect = he, ee.updateProjectionMatrix();
      else if (ee && ee.isOrthographicCamera) {
        const ke = ee, Fe = ke.top;
        ke.left = -Fe * he, ke.right = Fe * he, ke.updateProjectionMatrix();
      }
      C.setSize(N, O), G();
    }
  ...
  let se = false, ee = null, I = null, de = false;
  function G() {
    ...
    const N = F / 2;
    C.setScissorTest(true), C.setViewport(0, 0, N, K), C.setScissor(0, 0, N, K), C.render(p, k), C.setViewport(N, 0, N, K), C.setScissor(N, 0, N, K), C.render(p, ee), C.setScissorTest(false);
  }
  function fe(F) {
  ...
  function me(F, K) {
    se = F, K && (ee = K);
    const N = w.clientWidth || 1, O = w.clientHeight || 1, he = (F ? N / 2 : N) / O;
    x.isPerspectiveCamera && (x.aspect = he, x.updateProjectionMatrix());
    const ue = g.top;
    if (g.left = -ue * he, g.right = ue * he, g.updateProjectionMatrix(), F && ee) {
      if (I ? (I.object = ee, I.update()) : (I = new Uo(ee, C.domElement), I.enableDamping = true, I.dampingFactor = 0.1, I.screenSpacePanning = true, I.zoomSpeed = 0.8, I.panSpeed = 1.2, I.rotateSpeed = 0.9, I.touches = { ONE: Rn.ROTATE, TWO: Rn.DOLLY_PAN }, I.target.copy(v.target), I.addEventListener("change", G), I.enabled = false), !de) {
        const ke = (Fe) => {
          if (!se || !I) return;
          const Ee = C.domElement.getBoundingClientRect(), De = Fe.clientX - Ee.left, Be = Ee.width / 2, Ke = De >= Be;
          v.enabled = !Ke, I.enabled = Ke;
        };
        C.domElement.addEventListener("pointerdown", ke, true), C.domElement.addEventListener("wheel", ke, { capture: true, passive: true }), de = true;
      }
    } else F || (v.enabled = true, I && (I.enabled = false));
    w.__splitMode = F, window.__hekatanSplitMode = F, window.__hekatanSplitCamera = F ? ee : null, G();
  }
  if (e) {
    ...
    const ie = ma(e, z, W);
    p.add(ie);
    const he = ie.__colorMapValues, ue = Ho(he);
    ue.id = "frame-legend", w.appendChild(ue), q.derive(() => {
      var _a2;
      ...
      O.hidden = !Ee || !Be, N.visible = Ee, ue.hidden = !De;
    });
  }
  if (d) {
  ...
  const Ve = { scene: p, perspCamera: x, orthoCamera: g, get camera() {
    return k;
  }, controls: v, renderer: C, rendererElm: C.domElement, render: G, setActiveCamera: fe, setSplitMode: me, get splitMode() {
    return se;
  }, get splitCamera() {
    return ee;
  }, settings: z };
  ...
  const Le = (F, K, N) => {
    ...
    const N = v.target, O = new S().subVectors(k.position, N), ie = O.length(), he = new S(), ue = new S();
    he.crossVectors(k.up, O).normalize(), ue.copy(k.up).normalize();
    const ke = ie * 0.05;
    N.addScaledVector(he, -F * ke), N.addScaledVector(ue, K * ke), k.position.addScaledVector(he, -F * ke), k.position.addScaledVector(ue, K * ke), v.update(), G();
  }, ct = (F) => {
  ...
  return ((d) => {
    ...
    const d = /* @__PURE__ */ new Map(), w = /* @__PURE__ */ new Map(), p = /* @__PURE__ */ new Map(), x = /* @__PURE__ */ new Map(), g = /* @__PURE__ */ new Map(), k = /* @__PURE__ */ new Map(), C = /* @__PURE__ */ new Map(), v = /* @__PURE__ */ new Map(), Z = /* @__PURE__ */ new Map(), ye = /* @__PURE__ */ new Map(), le = (K, N) => {
      K == null ? void 0 : K.forEach((O, ie) => {
        const he = e.elements.val[ie];
        if (he) for (let ue = 0; ue < he.length; ue++) N.set(he[ue], [O[ue] ?? O[0]]);
      });
    };
    ...
    const te = /* @__PURE__ */ new Map(), z = /* @__PURE__ */ new Map(), ne = /* @__PURE__ */ new Map(), W = /* @__PURE__ */ new Map(), ve = /* @__PURE__ */ new Map(), ge = (K, N, O, ie, he) => {
      K.forEach((ue, ke) => {
        var _a3, _b2;
        const Fe = ue[0] ?? 0, Ee = ((_a3 = N.get(ke)) == null ? void 0 : _a3[0]) ?? 0, De = ((_b2 = O.get(ke)) == null ? void 0 : _b2[0]) ?? 0, Be = (Fe + Ee) / 2, Ke = Math.hypot((Fe - Ee) / 2, De);
        ie.set(ke, [Be + Ke]), he.set(ke, [Be - Ke]);
      });
    };
    ...
    const B = (_v = (_u = e.analyzeOutputs) == null ? void 0 : _u.val) == null ? void 0 : _v.colorMapRanges, R = (_w = l.solidResults) == null ? void 0 : _w.val, $ = R && R !== "none" ? R : l.shellResults.val, A = B == null ? void 0 : B[$], X = { bendingXX: [d, 0], bendingYY: [w, 0], bendingXY: [p, 0], membraneXX: [x, 0], membraneYY: [g, 0], membraneXY: [k, 0], tranverseShearX: [C, 0], tranverseShearY: [v, 0], membranePrincipalMax: [te, 0], membranePrincipalMin: [z, 0], bendingPrincipalMax: [ne, 0], bendingPrincipalMin: [W, 0], transverseShearMax: [ve, 0], vonMises: [Z, 0], pressure: [ye, 0], displacementX: [(_y = (_x = e.deformOutputs) == null ? void 0 : _x.val) == null ? void 0 : _y.deformations, 0], displacementY: [(_A = (_z = e.deformOutputs) == null ? void 0 : _z.val) == null ? void 0 : _A.deformations, 1], displacementZ: [(_C = (_B = e.deformOutputs) == null ? void 0 : _B.val) == null ? void 0 : _C.deformations, 2] }, D = l.shellResults.val, L = za.val, Q = Fa.val, ce = D === "displacementX" || D === "displacementY" || D === "displacementZ", se = D === "bendingXX" || D === "bendingYY" || D === "bendingXY" || D === "bendingPrincipalMax" || D === "bendingPrincipalMin", ee = D === "membraneXX" || D === "membraneYY" || D === "membraneXY" || D === "membranePrincipalMax" || D === "membranePrincipalMin", I = D === "vonMises" || D === "pressure", de = D === "tranverseShearX" || D === "tranverseShearY" || D === "transverseShearMax", G = (_D = l.solidResults) == null ? void 0 : _D.val, fe = G === "vonMises" || G === "sigmaXX" || G === "sigmaYY" || G === "sigmaZZ" || G === "tauXY" || G === "tauYZ" || G === "tauXZ", me = G === "ux" || G === "uy" || G === "uz", Ve = Aa.val, Me = fe ? Ta[Ve] : me || ce ? Jo[Q] : se || ee || I || de ? 1 / Ea[L] : 1, Le = fe ? Ve : me || ce ? Q : se ? `${L}\xB7m/m` : ee ? `${L}/m\xB2` : I ? `${L}/m\xB2` : de ? `${L}/m` : "";
    ...
    if (e.nodes.val.forEach((K, N) => {
      ...
      const ie = O[0].get(N), he = ie ? ie[O[1]] ?? 0 : 0;
      F.push(he * Me);
    }), !zn.val && Qe !== "auto") {
      const K = e.nodes.val, N = /* @__PURE__ */ new Set(), O = (he, ue) => {
        var _a3;
        const ke = (_a3 = K[he[0]]) == null ? void 0 : _a3[ue];
        return he.every((Fe) => {
          var _a4;
          return Math.abs((((_a4 = K[Fe]) == null ? void 0 : _a4[ue]) ?? NaN) - ke) < 1e-6;
        });
      };
      for (const he of e.elements.val) {
        if (he.length !== 4) continue;
        const ue = O(he, 2), ke = !ue && O(he, 0), Fe = !ue && O(he, 1);
        if (Qe === "losas" ? ue : Qe === "muros" ? ke || Fe : Qe === "murosX" ? ke : Qe === "murosY" ? Fe : false) for (const Be of he) N.add(Be);
      }
      const ie = [];
      for (const he of N) {
        const ue = F[he];
        Number.isFinite(ue) && ie.push(ue);
      }
