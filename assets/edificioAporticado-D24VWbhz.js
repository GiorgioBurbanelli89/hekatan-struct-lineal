import { a as ss, __tla as __tla_0 } from "./analyze-h3F5Phoi.js";
import { m as ns, d as Ie, __tla as __tla_1 } from "./didacticCpp-Czy7NlhT.js";
import { b as as, t as is, i as cs } from "./cadSections-Cc6U2MoW.js";
import { b as rs, a as to } from "./cotas3D-DoMwrmzt.js";
import { S as ls, f as ds, M as Go, e as ae, b as jt, B as uo, V as G, d as Oe, c as fs, L as Co, E as hs, N as ms, D as us } from "./theme-C-zoknmI.js";
let ks, As;
let __tla = Promise.all([
  ...
  function ps(t, a, D = {}) {
    ...
    }, T = 1e8, K = 1e4, j = 1e4, U = 2 * j, at = T / (2 * (1 + 0.3));
    for (const gt of a) {
      const W = [];
      let Mt = 0, tt = 0;
      ...
      for (const m of W) {
        ...
        const $ = k + _.length - 1;
        P.areas.set($, K), P.momentsOfInertiaY.set($, j), P.momentsOfInertiaZ.set($, j), P.torsionalConstants.set($, U), P.elasticities.set($, T), P.shearModuli.set($, at), P.densities.set($, 0);
      }
    }
  ...
  function ys(t, a, D, y, k, p, _, P = {}) {
    var _a, _b;
    const T = P.Fy_steel ?? 345e3;
    P.fc_concrete;
    const K = P.Fy_rebar ?? 42e4, j = P.omega ?? 0.15, U = P.phi ?? 0.9, at = k < 0.5 ? U * j * K * (1 - 0.59 * j) : U * T, gt = p < 0.5 ? U * j * K * (1 - 0.59 * j) : U * T, W = D.frameBendingMoments, Mt = [];
    for (let tt = 0; tt < a.length; tt++) {
      ...
      $ && (ct = $.Mi, _t = $.Mj);
      const M = ((_a = y.areas) == null ? void 0 : _a.get(tt)) ?? 0.16, B = ((_b = y.momentsOfInertiaZ) == null ? void 0 : _b.get(tt)) ?? 213e-5, rt = vs(M, B, m ? at : gt), $t = ct / rt, yt = _t / rt;
      Mt.push({
        nodeIdx: x,
        elementIdx: tt,
        end: "i",
  ...
  function bs(t, a, D, y = {}) {
    ...
    for (const T of t) {
      if (!k && T.classification.state === "Elastic") continue;
      const K = a[T.nodeIdx];
      if (!K) continue;
      const j = new ds({
      ...
      U.position.set(K[0], K[1], K[2]), U.userData = {
        hingeState: T.classification.state,
        ratio: T.classification.ratio.toFixed(3),
        element: T.elementIdx,
        end: T.end
      }, _.push(U);
    }
    return _;
  ...
  const ie = 9.80665;
  function Le(t, a, D, y, k = 0.01) {
    const p = Math.abs(t) < k, _ = Math.abs(t - D) < k, P = Math.abs(a) < k, T = Math.abs(a - y) < k, K = [
      p,
      _,
      P,
      T
  ...
  function Ee(t) {
    const { P_kN: a, Mx_kN: D, My_kN: y, tipo: k, q_adm_tonf: p, ks: _ } = t, P = t.Lz_min ?? 1, T = t.Lz_max ?? 4, K = t.t_min ?? 0.3;
    if (a <= 0) return {
      tipo: k,
      Lz: P,
      Bz: P,
  ...
  ks = {
    ...
    params: {
      ...
      slabDisc: J("Avanzado", "Discretizaci\xF3n losa", 0.5, Ms),
    ...
    dynamicParams(t) {
      ...
      for (let p = 1; p <= D; p++) a[`hP_${p}`] = u("Alturas por piso", `h Piso ${p} (m)`, 0, 0, 6, 0.1), a[`colB_p${p}`] = u("Secciones por piso", `b col P${p} (m)`, 0, 0, 1, 0.05), a[`colH_p${p}`] = u("Secciones por piso", `h col P${p} (m)`, 0, 0, 1, 0.05), a[`vigaB_p${p}`] = u("Secciones por piso", `b viga P${p} (m)`, 0, 0, 0.8, 0.05), a[`vigaH_p${p}`] = u("Secciones por piso", `h viga P${p} (m)`, 0, 0, 1, 0.05);
      for (let p = 1; p <= y; p++) a[`svX_${p}`] = u("Luces por vano", `svX #${p} (m)`, 0, 0, 12, 0.5);
      for (let p = 1; p <= k; p++) a[`svY_${p}`] = u("Luces por vano", `svY #${p} (m)`, 0, 0, 12, 0.5);
      return a;
    },
    computedLabels(t, a) {
      var _a;
      ...
      y.forEach((ct, _t) => {
        ...
        Math.abs(M) > Math.abs(p) && (p = M, T = _t, $[0], $[1]), M > 0 && M > Math.abs(K) && (K = M, j = _t), Math.abs(B) > Math.abs(_) && (_ = B), Math.abs(z) > Math.abs(P) && (P = z), U.push({
          idx: _t,
          x: $[0],
          y: $[1],
          P_kN: Math.abs(M),
      ...
      const W = Math.abs(p) / 9.80665, Mt = Math.abs(_) / 9.80665, tt = Math.abs(P) / 9.80665, it = K / 9.80665, x = Math.round(t.nPisos), f = {
        ...
        "P (compresi\xF3n)": `${W.toFixed(2)} tonf (nodo ${T})`,
    ...
    build(t, a) {
      var _a, _b;
      const D = Math.round(t.nVanosX), y = Math.round(t.nVanosY), k = Math.round(t.nPisos), p = Math.max(1, Math.round(t.nSubViga)), _ = Math.max(1, Math.round(t.nSubCol)), P = t.fcConcr * 0.0981, T = 4700 * Math.sqrt(P) * 1e3, K = 2e8, j = 0.2, U = 0.3, at = T / (2 * (1 + j)), gt = K / (2 * (1 + U)), W = (o, s, e) => Array.from({
        length: s
      }, (i, n) => {
        const h = t[`${o}${n + 1}`];
      ...
      const z = [], rt = /* @__PURE__ */ new Set(), $t = /* @__PURE__ */ new Set(), yt = /* @__PURE__ */ new Set(), O = /* @__PURE__ */ new Set(), q = /* @__PURE__ */ new Map(), Et = (o, s, e, i, n) => {
        ...
        for (let e = z.length - 1; e >= 0; e--) {
          ...
          z.push([
            o,
            h
          ]), $t.has(e) && $t.add(N), yt.has(e) && yt.add(N), rt.has(e) && rt.add(N), q.has(e) && q.set(N, q.get(e));
        }
      };
            ...
            for (let lt = 0; lt <= g; lt++) {
              ...
              if (St !== void 0) N.push(St);
                ...
                M.push([
                  ...
                  wt,
                  s
                ]), pt.set(xt, kt), N.push(kt), eo(kt);
              }
            }
            w.push(N);
          }
          for (let Y = 0; Y < c; Y++) for (let N = 0; N < g; N++) O.add(z.length), z.push([
            w[Y][N],
            w[Y][N + 1],
            w[Y + 1][N + 1],
            w[Y + 1][N]
      ...
      if (lo > 0) {
        const o = lo === 1 || lo === 3, s = lo === 2 || lo === 3, e = (g, c, w) => {
          ...
          for (let St = 0; St <= wt; St++) {
            const kt = [];
            for (let yo = 0; yo <= X; yo++) kt.push(e(g + yo / X * (w - g), c + yo / X * (Y - c), N + St / wt * (lt - N)));
            xt.push(kt);
          }
      ...
      const Uo = [
        ...
        t.vigaH_7,
        t.vigaH_8
      ].map((o) => o > 0 ? o : t.vigaH), Lo = Math.round(t.matCol) === 2, Wo = Math.round(t.matCol) === 3, te = Math.round(t.matViga) === 2, me = (o) => {
        const s = Uo[o] ?? t.colSize, e = Qo[o] ?? t.colSize;
        if (Lo) {
          const i = Math.min(t.tCft, Math.min(s, e) / 2 - 1e-3), n = as(s, e, i, K, U, T, j);
          return {
            A: n.A,
            Iz: n.Iz,
            Iy: n.Iy,
      ...
      }, Ye = t.matCol < 0.5 ? T : K, He = t.matCol < 0.5 ? at : gt, qe = t.matCol < 0.5 ? j : U, Re = t.matCol < 0.5 ? oo : jo, Xe = t.matViga < 0.5 ? T : K, Ze = t.matViga < 0.5 ? at : gt, je = t.matViga < 0.5 ? j : U, Ge = t.matViga < 0.5 ? oo : jo, po = /* @__PURE__ */ new Map(), xo = /* @__PURE__ */ new Map(), Eo = /* @__PURE__ */ new Map(), Po = /* @__PURE__ */ new Map(), Vo = /* @__PURE__ */ new Map(), No = /* @__PURE__ */ new Map(), vo = /* @__PURE__ */ new Map(), zo = /* @__PURE__ */ new Map(), oe = /* @__PURE__ */ new Map(), To = /* @__PURE__ */ new Map(), Bo = /* @__PURE__ */ new Map(), Do = /* @__PURE__ */ new Map(), ue = /* @__PURE__ */ new Map(), ge = /* @__PURE__ */ new Map(), Me = /* @__PURE__ */ new Map(), pe = Math.round(t.slabType), Je = pe === 2 ? 0 : 1, Ke = pe === 1 ? 0 : 1, Yo = t.crackedSections > 0.5, xe = t.matCol < 0.5 && Yo ? 0.7 : 1, ve = t.matViga < 0.5 && Yo ? 0.35 : 1, Ue = Yo ? 0.25 : 1, Qe = 1, Ho = t.massSource > 0.5, We = t.qDead + 0.25 * t.qLive, ts = Ho ? We / ce / Math.max(t.slabT, 0.05) : oo, ee = /* @__PURE__ */ new Map();
      ...
      for (let o = 0; o < z.length; o++) {
        const s = q.get(o) ?? 0;
        if (O.has(o)) po.set(o, T), xo.set(o, at), zo.set(o, j), oe.set(o, t.slabT), ge.set(o, Je * Qe), Me.set(o, Ke * Ue), ue.set(o, Math.round(t.slabForm ?? 1)), vo.set(o, ts);
        else if (go.has(o)) po.set(o, T), xo.set(o, at), zo.set(o, j), oe.set(o, t.tMuro ?? 0.25), vo.set(o, Ho ? 0 : oo);
        else if (rt.has(o)) {
          const e = me(Math.min(s, 7));
          po.set(o, Ye), xo.set(o, He), zo.set(o, qe), Eo.set(o, e.A), Po.set(o, e.Iz * xe), Vo.set(o, e.Iy * xe), No.set(o, e.J), Wo && (Bo.set(o, e.As2), To.set(o, e.As3), Do.set(o, {
            type: "HSS",
            b: e.b,
            h: e.h,
            tf: e.tf,
          ...
          })), Lo && (Bo.set(o, e.As2), To.set(o, e.As3), Do.set(o, {
            type: "CFT",
            b: e.b,
            h: e.h,
            tw: e.t,
          ...
          const i = Lo ? (jo * (e.b * e.h - (e.b - 2 * e.t) * (e.h - 2 * e.t)) + oo * (e.b - 2 * e.t) * (e.h - 2 * e.t)) / e.A : Re;
          vo.set(o, Ho ? 0 : i);
        } else {
          const e = De(Math.min(s, 7), yt.has(o));
          if (po.set(o, Xe), xo.set(o, Ze), zo.set(o, je), Eo.set(o, e.A), Po.set(o, e.Iz * ve), Vo.set(o, e.Iy * ve), No.set(o, e.J), te) {
            Bo.set(o, e.As2), To.set(o, e.As3);
      ...
      try {
        ...
        const d = Math.sqrt((n - s) ** 2 + (h - e) ** 2 + (l - i) ** 2) || 1, g = bs(o, M, d, {
          showElastic: false,
          radiusFactor: 0.015
        });
        se.push(...g), a.__plasticHinges = _s(o);
      } catch (o) {
      ...
      if ((t.mostrarZapatas ?? 1) >= 0.5) try {
        ...
        if (o) {
          ...
          if (o.forEach((n, h) => {
            ...
            const n = t.q_adm_zapata ?? 10, h = t.ks_zapata ?? 1030, l = Math.max(0, t.Hf_pedestal ?? 0.5), d = Math.max(0.1, t.t_zapata ?? 0.3), g = Math.max(2, Math.round(t.nSubZapata ?? 4)), c = Math.max(0, t.voladoExtra ?? 0.3), w = Math.round(t.tipoZapataOverride ?? 0) | 0, Y = Math.round(t.estiloZapata ?? 1), N = Jo(s, e, i, n, h), lt = [
              "central",
              "lindero",
              "esquinera"
            ];
            for (const L of N) w > 0 && (L.tipo = lt[w - 1]), L.t = d;
            const X = [], wt = (L) => new ae({
              color: L,
              transparent: true,
              opacity: 0.55,
            ...
            for (const L of N) {
              const Z = L.Lz, st = L.Bz, Zt = L.t;
              let qt = 0, Rt = 0;
              L.tipo === "esquinera" ? (qt = L.x < e / 2 ? -(Z / 2 - c) : Z / 2 - c, Rt = L.y < i / 2 ? -(st / 2 - c) : st / 2 - c) : L.tipo === "lindero" && (Math.abs(L.x) < 1e-3 || Math.abs(L.x - e) < 1e-3 ? qt = L.x < e / 2 ? -(Z / 2 - c) : Z / 2 - c : (Math.abs(L.y) < 1e-3 || Math.abs(L.y - i) < 1e-3) && (Rt = L.y < i / 2 ? -(st / 2 - c) : st / 2 - c));
              const Q = L.x - qt, I = L.y - Rt, V = -l, Vt = V - Zt / 2, Yt = V - Zt, so = L.ratio;
              let no = 4906624;
              if (so > 1.5 ? no = 15680580 : so > 1 ? no = 16096779 : so > 0.8 && (no = 16498468), l > 1e-3) {
                ...
                X.push(new Oe(bt, new jt({
                  color: 6333946,
                  linewidth: 4
                }))), X.push(to(`Df=${l.toFixed(2)}m`, L.x + 0.1, L.y + 0.1, -l / 2, "#60a5fa"));
              }
              if (Y === 0) {
              ...
              (t.mostrarLabelsZapatas ?? 1) >= 0.5 && X.push(to(`${L.tipo[0].toUpperCase()} ${Z.toFixed(2)}\xD7${st.toFixed(2)}\xD7${Zt.toFixed(2)}m \u03C3/q=${L.ratio.toFixed(2)}`, Q, I, Yt - 0.2, so <= 1 ? "#4ade80" : so <= 1.5 ? "#f59e0b" : "#ef4444"));
            }
            if (Math.round(t.sistemaCimentacion ?? 0) === 1) {
              const L = Math.round(t.vigaAmarre_pos ?? 0), Z = L === 0 ? -l : -l / 2, st = t.vigaAmarre_b ?? 0.25, Zt = t.vigaAmarre_h ?? 0.4, qt = /* @__PURE__ */ new Map(), Rt = /* @__PURE__ */ new Map();
              for (const I of s) {
              ...
              Q.length > 0 && (X.push(new Co(new uo().setFromPoints(Q), new jt({
                color: 2282478,
                linewidth: 3
              }))), X.push(to(`Vigas amarre ${(st * 100).toFixed(0)}\xD7${(Zt * 100).toFixed(0)} cm @ ${L === 0 ? "zapatas" : "pedestales"}`, e / 2, i / 2, Z + 0.2, "#22d3ee")));
            }
            se.push(...X);
          }
        }
      ...
      if ((t.modoCimentacion ?? 0) >= 0.5) try {
        ...
        if (s && s.size > 0) {
          ...
          if (s.forEach((h, l) => {
            ...
            const h = t.q_adm_zapata ?? 10, l = t.ks_zapata ?? 1030, d = Math.max(0, t.Hf_pedestal ?? 0.5), g = Math.max(0.1, t.t_zapata ?? 0.3), c = Math.max(2, Math.round(t.nSubZapata ?? 4)), w = Math.max(0, t.voladoExtra ?? 0.3), Y = Math.round(t.tipoZapataOverride ?? 0) | 0, N = Jo(e, i, n, h, l), lt = [
              "central",
              "lindero",
              "esquinera"
            ];
            for (const r of N) Y > 0 && (r.tipo = lt[Y - 1]), r.t = g;
            const X = Uo[0] ?? t.colSize, wt = Qo[0] ?? t.colSize, xt = X * wt, St = X * wt ** 3 / 12, kt = wt * X ** 3 / 12, yo = 0.14 * Math.pow(Math.min(X, wt), 4), _e = t.matCol < 0.5 ? T : K, we = t.matCol < 0.5 ? at : gt, Ce = t.matCol < 0.5 ? j : U, L = [], Z = [], st = /* @__PURE__ */ new Map(), Zt = /* @__PURE__ */ new Map(), qt = /* @__PURE__ */ new Map(), Rt = /* @__PURE__ */ new Map(), Q = /* @__PURE__ */ new Map(), I = /* @__PURE__ */ new Map(), V = /* @__PURE__ */ new Map(), Vt = /* @__PURE__ */ new Map(), Yt = /* @__PURE__ */ new Map(), so = /* @__PURE__ */ new Map(), no = /* @__PURE__ */ new Map(), $o = [], bt = [], Kt = (r, S, v) => `${Math.round(r * 1e4)},${Math.round(S * 1e4)},${Math.round(v * 1e4)}`, Ut = /* @__PURE__ */ new Map(), ao = (r, S, v) => {
              const F = Kt(r, S, v), H = Ut.get(F);
            ...
            for (const r of N) {
              ...
              const Nt = r.x - H, Ft = r.y - nt, Qt = -d, Tt = S / c, Wt = v / c, dt = [];
              for (let R = 0; R <= c; R++) {
                const ot = [];
                for (let ht = 0; ht <= c; ht++) {
                  const Dt = Nt - S / 2 + ht * Tt, zt = Ft - v / 2 + R * Wt;
                  ot.push(ao(Dt, zt, Qt));
                }
                dt.push(ot);
              }
              for (let R = 0; R < c; R++) for (let ot = 0; ot < c; ot++) {
                ...
                Z.push([
                  ...
                  dt[R + 1][ot + 1],
                  dt[R + 1][ot]
                ]), Yt.set(ht, F), st.set(ht, T), Vt.set(ht, j), Zt.set(ht, at), V.set(ht, oo);
              }
              const io = 0.5;
              for (let R = 0; R <= c; R++) for (let ot = 0; ot <= c; ot++) {
                const ht = Tt * Wt * (ot === 0 || ot === c ? 0.5 : 1) * (R === 0 || R === c ? 0.5 : 1), Dt = l * ht, zt = Dt * io, A = dt[R][ot];
                $o.push({
                  node: A,
                  dof: 0,
                  k: zt
              ...
              let C = 0, E = 0, b = 1 / 0;
              for (let R = 0; R <= c; R++) for (let ot = 0; ot <= c; ot++) {
                const ht = dt[R][ot], Dt = L[ht][0], zt = L[ht][1], A = Math.sqrt((Dt - r.x) ** 2 + (zt - r.y) ** 2);
                A < b && (b = A, C = R, E = ot);
              }
              ...
              }
              {
                ...
                for (let zt = 0; zt <= c; zt++) {
                  const A = -S / 2 + zt * ot;
                  Dt.push(new G(Nt + A, Ft - v / 2, -d), new G(Nt + A, Ft + v / 2, -d));
                }
                for (let zt = 0; zt <= c; zt++) {
                  const A = -v / 2 + zt * ht;
                  Dt.push(new G(Nt - S / 2, Ft + A, -d), new G(Nt + S / 2, Ft + A, -d));
                }
              ...
              if ((t.mostrarLabelsZapatas ?? 1) >= 0.5) {
                ...
                bt.push(to(`P=${R.toFixed(2)} tonf`, r.x, r.y, 0.3, "#fbbf24")), bt.push(to(`Mx=${ot.toFixed(2)}  My=${ht.toFixed(2)} tonf\xB7m`, r.x, r.y, 0.1, "#fbbf24")), bt.push(to(`${r.tipo[0].toUpperCase()} ${S.toFixed(2)}\xD7${v.toFixed(2)}\xD7${F.toFixed(2)}m \u03C3/q=${co.toFixed(2)}`, Nt, Ft, -d - F - 0.2, co <= 1 ? "#4ade80" : co <= 1.5 ? "#f59e0b" : "#ef4444"));
              }
            }
            const Xt = Math.round(t.sistemaCimentacion ?? 0);
            if (Xt === 1) {
              const r = Math.round(t.vigaAmarre_pos ?? 0), S = t.vigaAmarre_h ?? 0.4, v = t.vigaAmarre_b ?? 0.25, F = v * S, H = v * S ** 3 / 12, nt = S * v ** 3 / 12, Nt = 0.21 * Math.pow(Math.min(v, S), 3) * Math.max(v, S), Ft = /* @__PURE__ */ new Map();
              for (const C of N) {
                let E;
                r === 0 ? E = -d : E = -d / 2;
                const b = ao(C.x, C.y, E);
                if (Ft.set(C.idx, b), r === 1 && d > 1e-3) {
                  const ft = ao(C.x, C.y, -d / 2), Ht = ao(C.x, C.y, 0), It = ao(C.x, C.y, -d);
                }
              }
              const Qt = /* @__PURE__ */ new Map(), Tt = /* @__PURE__ */ new Map();
              for (const C of e) {
                const E = C.y.toFixed(4), b = C.x.toFixed(4);
                Qt.has(E) || Qt.set(E, []), Tt.has(b) || Tt.set(b, []), Qt.get(E).push(C), Tt.get(b).push(C);
              }
              const Wt = (C, E) => {
                const b = Z.length;
                Z.push([
                  C,
                  E
                ]), st.set(b, _e), Zt.set(b, we), Vt.set(b, Ce), qt.set(b, F), Rt.set(b, nt), Q.set(b, H), I.set(b, Nt), V.set(b, oo);
              };
              let dt = 0;
              for (const C of Qt.values()) {
                C.sort((E, b) => E.x - b.x);
                for (let E = 0; E < C.length - 1; E++) {
                  const b = Ft.get(C[E].idx), ft = Ft.get(C[E + 1].idx);
            ...
            if (So === 1) {
              ...
              for (const F of e) {
                const H = F.y.toFixed(4), nt = F.x.toFixed(4);
                r.has(H) || r.set(H, []), S.has(nt) || S.set(nt, []), r.get(H).push(F), S.get(nt).push(F);
              }
              const v = (F) => _o.set(F, (_o.get(F) ?? 0) + 1);
              for (const F of r.values()) {
                F.sort((H, nt) => H.x - nt.x);
                for (let H = 0; H < F.length - 1; H++) v(F[H].idx), v(F[H + 1].idx);
              }
              for (const F of S.values()) {
                F.sort((H, nt) => H.y - nt.y);
                for (let H = 0; H < F.length - 1; H++) v(F[H].idx), v(F[H + 1].idx);
              }
              console.log(`[Cimentaci\xF3n] Vigas de amarre activas \u2014 momentos en zapatas reducidos por factor (1 - ${bo} \xB7 n_vigas/4):`), _o.forEach((F, H) => {
                const nt = (bo * F / 4 * 100).toFixed(0);
                console.log(`   Zapata ${H}: ${F} vigas conectadas \u2192 momento reducido ${nt}%`);
            ...
            for (const r of N) {
              const S = e.find((A) => A.idx === r.idx), v = r.Lz, F = r.Bz, H = r.t;
              let nt = 0, Nt = 0;
              r.tipo === "esquinera" ? (nt = r.x < i / 2 ? -(v / 2 - w) : v / 2 - w, Nt = r.y < n / 2 ? -(F / 2 - w) : F / 2 - w) : r.tipo === "lindero" && (Math.abs(r.x) < 1e-3 || Math.abs(r.x - i) < 1e-3 ? nt = r.x < i / 2 ? -(v / 2 - w) : v / 2 - w : (Math.abs(r.y) < 1e-3 || Math.abs(r.y - n) < 1e-3) && (Nt = r.y < n / 2 ? -(F / 2 - w) : F / 2 - w));
              const Ft = r.x - nt, Qt = r.y - Nt, Tt = [], Wt = [], dt = {
                elasticities: /* @__PURE__ */ new Map(),
                shearModuli: /* @__PURE__ */ new Map(),
              ...
              for (let A = 0; A <= c; A++) {
                ...
                for (let mt = 0; mt <= c; mt++) {
                  const Lt = -v / 2 + mt * io, Ot = -F / 2 + A * Bt;
                  et.push(Tt.length), Tt.push([
                    Lt,
                    Ot,
                    0
                  ]);
                  const Ro = Ft + Lt, Xo = Qt + Ot, Zo = Kt(Ro, Xo, -d), ho = Ut.get(Zo);
              ...
              for (let A = 0; A < c; A++) for (let et = 0; et < c; et++) {
                ...
                Wt.push([
                  ...
                  C[A + 1][et + 1],
                  C[A + 1][et]
                ]), dt.thicknesses.set(mt, H), dt.elasticities.set(mt, T), dt.poissonsRatios.set(mt, j), dt.shearModuli.set(mt, at), dt.densities.set(mt, oo);
              }
              const b = [], ft = 0.5;
              for (let A = 0; A <= c; A++) for (let et = 0; et <= c; et++) {
                const mt = io * Bt * (et === 0 || et === c ? 0.5 : 1) * (A === 0 || A === c ? 0.5 : 1), Lt = l * mt, Ot = C[A][et];
                b.push({
                  node: Ot,
                  dof: 0,
                  k: Lt * ft
              ...
              if (So === 1 && fo === 0) {
                const A = t.vigaAmarre_b ?? 0.25, et = r.idx, mt = e.filter((vt) => Math.abs(vt.y - r.y) < 1e-3 && vt.idx !== et).sort((vt, Ao) => vt.x - Ao.x), Lt = e.filter((vt) => Math.abs(vt.x - r.x) < 1e-3 && vt.idx !== et).sort((vt, Ao) => vt.y - Ao.y), Ot = mt.find((vt) => vt.x > r.x), Ro = [
                  ...mt
                ].reverse().find((vt) => vt.x < r.x), Xo = Lt.find((vt) => vt.y > r.y), Zo = [
                  ...Lt
              ...
              let wo = 0, ne = 0, R = 1 / 0;
              for (let A = 0; A <= c; A++) for (let et = 0; et <= c; et++) {
                const mt = -v / 2 + et * io, Lt = -F / 2 + A * Bt, Ot = (mt - It) ** 2 + (Lt - co) ** 2;
            ...
            a.nodes.val = L, a.elements.val = Z, a.nodeInputs.val = {
              supports: so,
              loads: no
            }, a.elementInputs.val = {
              etabsWallJoint: Math.round(t.comparar ?? 1) === 1,
    ...
    runModal(t, a, D) {
      ...
      if (!(!y.length || !k.length || !((_a = p.supports) == null ? void 0 : _a.size) || !((_b = _.densities) == null ? void 0 : _b.size))) try {
        const P = [], T = /* @__PURE__ */ new Map(), K = /* @__PURE__ */ new Map(), j = /* @__PURE__ */ new Map(), U = /* @__PURE__ */ new Map(), at = /* @__PURE__ */ new Map(), gt = /* @__PURE__ */ new Map(), W = /* @__PURE__ */ new Map(), Mt = /* @__PURE__ */ new Map(), tt = /* @__PURE__ */ new Map();
        let it = 0, x = 0;
        const f = [];
        let m = 0;
        for (let O = 0; O < k.length; O++) {
          ...
          Et || (P.push(q), ((_g = _.areas) == null ? void 0 : _g.has(O)) && T.set(m, _.areas.get(O)), ((_h = _.momentsOfInertiaY) == null ? void 0 : _h.has(O)) && K.set(m, _.momentsOfInertiaY.get(O)), ((_i = _.momentsOfInertiaZ) == null ? void 0 : _i.has(O)) && j.set(m, _.momentsOfInertiaZ.get(O)), ((_j = _.torsionalConstants) == null ? void 0 : _j.has(O)) && U.set(m, _.torsionalConstants.get(O)), ((_k = _.elasticities) == null ? void 0 : _k.has(O)) && at.set(m, _.elasticities.get(O)), ((_l = _.shearModuli) == null ? void 0 : _l.has(O)) && gt.set(m, _.shearModuli.get(O)), ((_m = _.densities) == null ? void 0 : _m.has(O)) && W.set(m, _.densities.get(O)), ((_n = _.thicknesses) == null ? void 0 : _n.has(O)) && Mt.set(m, _.thicknesses.get(O)), ((_o = _.poissonsRatios) == null ? void 0 : _o.has(O)) && tt.set(m, _.poissonsRatios.get(O)), eo && f.push(m), m++);
        }
