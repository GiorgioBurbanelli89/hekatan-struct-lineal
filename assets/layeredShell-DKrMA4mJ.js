import { __tla as __tla_0 } from "./didacticCpp-iMwzdM-v.js";
...
let __tla = Promise.all([
  ...
  const p = "\u{1F7E6} Iso (1 capa)", M = "\u{1F7E7} CLT 3 [0/90/0]", _ = "\u{1F7E8} CLT 5 [0/90/0/90/0]", f = "\u{1F7EA} Sandwich [face/core/face]", b = "\u{1F7E5} Bimet\xE1lico [E1/E2]", u = "\u{1F4D0} Geometr\xEDa / Mesh / Carga";
  function K(e) {
    ...
    if (o === "sandwich") {
      const s = Math.min(0.45, Math.max(0.01, e.sw_face_pct / 100)), a = 1 - 2 * s, c = e.sw_t;
      return {
        layers: [
          {
            E: e.sw_E_face,
  ...
  q = {
    ...
    build(e, d) {
      var _a, _b;
      const { layers: m, presetKey: o } = K(e), r = m.reduce((t, n) => t + n.thickness, 0), i = Math.round(e.bcType) === 1 ? "clamped" : "simply-supported", s = Math.round(e.stressMode) === 1 ? "plane-strain" : "plane-stress", a = O({
        layers: m,
        meshLx: e.Lx,
        meshLy: e.Ly,
      ...
      const l = m.reduce((t, n) => t + n.E * n.thickness, 0) / r, h = ((_a = m[0]) == null ? void 0 : _a.nu) ?? 0.3, F = m.reduce((t, n) => t + (n.density ?? 0) * n.thickness, 0) / r, E = /* @__PURE__ */ new Map(), k = /* @__PURE__ */ new Map(), g = /* @__PURE__ */ new Map(), w = /* @__PURE__ */ new Map();
      x.forEach((t, n) => {
        E.set(n, r), k.set(n, l), g.set(n, h), w.set(n, F / 9.80665);
      }), d.elementInputs.val = {
        thicknesses: E,
        elasticities: k,
        poissonsRatios: g,
      ...
      const y = /* @__PURE__ */ new Map(), I = /* @__PURE__ */ new Map(), P = e.Lx / Math.round(e.nx) * (e.Ly / Math.round(e.ny));
      c.forEach((t, n) => {
        const C = Math.abs(t[0]) < 1e-6 || Math.abs(t[0] - e.Lx) < 1e-6 || Math.abs(t[1]) < 1e-6 || Math.abs(t[1] - e.Ly) < 1e-6;
        C && (i === "clamped" ? y.set(n, [
          true,
          true,
          true,
        ...
        const D = (Math.abs(t[0]) < 1e-6 || Math.abs(t[0] - e.Lx) < 1e-6) && (Math.abs(t[1]) < 1e-6 || Math.abs(t[1] - e.Ly) < 1e-6) ? 0.25 : C ? 0.5 : 1;
        I.set(n, [
          0,
          0,
          -e.q * P * D,
      ...
      const N = /* @__PURE__ */ new Map(), v = /* @__PURE__ */ new Map(), S = /* @__PURE__ */ new Map(), B = /* @__PURE__ */ new Map(), X = /* @__PURE__ */ new Map();
      a.elementResults.forEach((t, n) => {
        N.set(n, [
          t.Mxx,
          t.Mxx,
          t.Mxx,
      ...
      const Y = a.abbd.A[0][0], T = a.abbd.D[0][0], $ = a.abbd.B[0][0], A = ((_b = Object.entries(q.params.preset.options ?? {}).find(([, t]) => t === Math.round(e.preset))) == null ? void 0 : _b[0]) ?? "?";
      console.log(`[Layered Shell] ${A} | ${m.length} capas | t=${r.toFixed(3)}m | BC=${i} | mode=${s} | mesh=${Math.round(e.nx)}\xD7${Math.round(e.ny)}
  ABBD: A11=${Y.toExponential(3)}  B11=${$.toExponential(3)}  D11=${T.toExponential(3)}
  maxW=${a.maxW.toExponential(3)} m | maxMxx=${a.maxMxx.toFixed(2)} kN\xB7m/m | maxMyy=${a.maxMyy.toFixed(2)} kN\xB7m/m`);
    ...
    computedLabels(e, d) {
      ...
      const s = d.analyzeOutputs.val;
      let a = 0, c = 0, x = 0;
      return (_b = s == null ? void 0 : s.bendingXX) == null ? void 0 : _b.forEach((l) => {
        for (const h of l) Math.abs(h) > Math.abs(a) && (a = h);
      }), (_c = s == null ? void 0 : s.bendingYY) == null ? void 0 : _c.forEach((l) => {
        for (const h of l) Math.abs(h) > Math.abs(c) && (c = h);
      }), (_d = s == null ? void 0 : s.membraneXX) == null ? void 0 : _d.forEach((l) => {
