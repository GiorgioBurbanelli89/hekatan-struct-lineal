(async ()=>{
    ...
    async function Ce(R = {}) {
        ...
        var d = R, h = !!globalThis.window, x = !!globalThis.WorkerGlobalScope, H = globalThis.process?.versions?.node && globalThis.process?.type != "renderer", j = !h && !H && !x;
        if (H) {
        ...
        var de = console.log.bind(console), N = console.error.bind(console);
        ...
        function F(e) {
            d.onAbort?.(e), e = "Aborted(" + e + ")", N(e), ue = !0;
        ...
        var we;
        function gr() {
            return d.locateFile ? Q("deform.wasm") : new URL("/hekatan-struct-lineal/assets/deform-BLaVg75W.wasm", import.meta.url).href;
        }
        function lr(e) {
        ...
        async function He(e, r) {
            try {
                ...
                N(`failed to asynchronously prepare wasm: ${i}`), b(e) && N(`warning: Loading from a file URI (${e}) is not supported in most browsers. See https://emscripten.org/docs/getting_started/FAQ.html#how-do-i-run-a-local-webserver-for-testing-why-does-my-program-stall-in-downloading-or-preparing`), F(i);
            }
        }
        ...
        var ur = (e)=>{
            ...
            ne.shown ||= {}, ne.shown[e] || (ne.shown[e] = 1, H && (e = "warning: " + e), N(e));
...
`));
            ...
            pe++, d.monitorRunDependencies?.(pe), c(e, "addRunDependency requires an ID"), c(!Oe[e]), Oe[e] = 1, se === null && globalThis.setInterval && (se = setInterval(()=>{
                ...
                var r = !1;
                for(var t in Oe)r || (r = !0, N("still waiting on run dependencies:")), N(`dependency: ${t}`);
                r && N("(end of list)");
            }, 1e4), se.unref?.());
        }, Pr = [], xr = async (e, r)=>{
            ...
            writeFile (e, r, t = {}) {
                ...
                typeof r == "string" && (r = new Uint8Array(Re(r))), ArrayBuffer.isView(r) ? n.write(o, r, 0, r.byteLength, void 0, t.canOwn) : F("Unsupported data type"), n.close(o);
            },
            cwd: ()=>n.currentPath,
            chdir (e) {
                var r = n.lookupPath(e, {
    ...
    function rt(R) {
        ...
        if (!R) return O;
        if (R.length >= 12) {
            for(let h = 0; h < 12; h++)O[h] = R[h] ? 1 : 0;
            return O;
        }
        const d = [
            3,
        ...
        for(let h = 0; h < 6 && h < R.length; h++)R[h] && (O[d[h]] = 1);
        return O;
    }
    await Ce();
    const f = await Ce();
    function tt(R, O, d, h, x = 10, H = 0, j = 0, Y = 1, Z, _e) {
        if (R.length === 0) return {
            frequencies: [],
            modeShapes: [],
            massParticipation: []
        };
        const p = [], Q = U(R.flat(), Float64Array, f.HEAPF64);
        p.push(Q);
        const le = O.flat(), ee = U(le, Uint32Array, f.HEAPU32);
        p.push(ee);
        const Ue = O.map((P)=>P.length), de = U(Ue, Uint32Array, f.HEAPU32);
        p.push(de);
        const N = d.supports ? Array.from(d.supports.keys()) : [], ce = d.supports ? Array.from(d.supports.values()).flat().map((P)=>P ? 1 : 0) : [], ue = U(N, Uint32Array, f.HEAPU32);
        p.push(ue);
        const c = U(ce, Uint8Array, f.HEAPU8);
        p.push(c);
        const b = (P)=>{
            const M = P ? Array.from(P.keys()) : [], ae = P ? Array.from(P.values()) : [], Ne = U(M, Uint32Array, f.HEAPU32);
            p.push(Ne);
            const Re = U(ae, Float64Array, f.HEAPF64);
            return p.push(Re), {
                keysPtr: Ne,
                valuesPtr: Re,
                size: M.length
    ...
    function U(R, O, d) {
        const h = new O(R), x = f._malloc(h.length * h.BYTES_PER_ELEMENT);
    ...
    self.onmessage = async (R)=>{
        const { nodes: O, elements: d, nodeInputs: h, elementInputs: x, nModes: H } = R.data ?? {};
        try {
            const j = {};
