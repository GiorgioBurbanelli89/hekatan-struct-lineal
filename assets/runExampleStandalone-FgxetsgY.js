...
import { j as Lt, l as Et, g as zt, c as Pt, __tla as __tla_1 } from "./aiAgent-CDkRN2K5.js";
import { c as Tt } from "./modeScale-sgWZ-KrB.js";
import { f as nt, d as it, e as Ft, g as It, h as St, i as st, j as Dt, s as lt, k as ct, l as dt, m as pt, n as Nt, o as Ct, p as At, q as Bt, t as Ot, r as jt } from "./units-BjHPBmMt.js";
let Gt;
let __tla = Promise.all([
  ...
  function Yt(w, C) {
    ...
    `;
      if (p) {
        ...
        <div class="section">
          ...
          <div class="prop-row"><span class="prop-key">EA/L (axial)</span><span class="prop-val">${f(X)}</span></div>
    ...
    U.addEventListener("click", () => {
      V = !V, U.classList.toggle("active", V), U.textContent = V ? "\u{1F50D} Inspect (ON)" : "\u{1F50D} Inspect", V ? setTimeout(at, 50) : et();
    });
  }
  K.derive(() => {
    Lt.val = nt.val;
  ...
  Gt = function(w) {
    const C = K.state([]), Y = K.state([]), V = K.state({}), P = K.state({}), R = K.state({}), U = K.state({}), W = K.state([]), Q = {
      nodes: C,
      elements: Y,
      nodeInputs: V,
      elementInputs: P,
...
Ejecutar: python -X utf8 <archivo>.py`);
    ...
    try {
      ...
      if (l == null ? void 0 : l.addFolder) {
        ...
        y.on("click", async () => {
          var _a, _b;
          const d = C.rawVal || [], a = Y.rawVal || [], e = V.rawVal || {}, r = P.rawVal || {};
              ...
              if (!c) {
                ...
                try {
                  const { modalAnalysis: N } = await ot(async () => {
                    const { modalAnalysis: I } = await import("./index-CionLQQD.js").then(async (m2) => {
                      await m2.__tla;
                      return m2;
                    });
                    return {
        ...
        p.addBinding(h, "limite", {
          ...
          for (let r = 0; r < x.length; r++) {
            ...
            for (let T = 0; T < 2; T++) {
              let I = i[T];
              for (; ; ) {
                const X = (z.get(I) || []).filter((J) => !d.has(J)).find((J) => {
                  const G = y(x[J]);
            ...
            for (const T of t) for (const I of x[T]) {
              const X = b[I], H = m < 1e-9 ? 0 : ((X[0] - c[0]) * (o[0] - c[0]) + (X[1] - c[1]) * (o[1] - c[1]) + (X[2] - c[2]) * (o[2] - c[2])) / (m * m), J = L + (F - L) * H, G = Math.abs((k.get(I) || [
                0,
                0,
                0
              ])[2] - J) * 1e3;
