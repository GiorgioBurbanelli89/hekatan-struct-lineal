...
import { v as n, b as ta, e as pe, C as ue, M as he, V as ea, B as aa, d as oa } from "./Text-C1TX4d8g.js";
import { P as sa } from "./tweakpane-BXg6ZhiP.js";
import { a as na, __tla as __tla_0 } from "./analyze-DW0aWqsq.js";
import { d as la, __tla as __tla_1 } from "./didacticCpp-BoYi16rL.js";
import { g as ca, l as ve, m as be, c as ra, __tla as __tla_2 } from "./aiAgent-D29HUSaa.js";
import { e as ia } from "./makeDraggable-zx2br6Yh.js";
import { g as da, __tla as __tla_3 } from "./getParameters-8h1v8PIV.js";
import { e as ma } from "./materials-VwssM8Vw.js";
...
Promise.all([
  ...
  n.derive(() => {
    ...
    const f = Math.max(2, Math.round(d / Ot)), x = Math.max(2, Math.round(m / qt)), nt = d / f, lt = m / x, O = it / g, P = [];
    for (let t = 0; t <= g; t++) {
      const e = [];
      for (let a = 0; a <= f; a++) {
        const s = -d / 2 + a * nt;
        t === 0 ? e.push(I(s, -m / 2)) : e.push(C(s, -m / 2, u + t * O));
      }
      P.push(e);
    }
    ...
    for (let t = 0; t <= g; t++) {
      ...
      for (let a = 0; a <= x; a++) {
        const s = -m / 2 + a * lt;
        t === 0 ? e.push(I(-d / 2, s)) : a === 0 ? e.push(P[t][0]) : a === x ? e.push(N[t][0]) : e.push(C(-d / 2, s, u + t * O));
      }
      z.push(e);
    }
    ...
    for (let t = 0; t <= g; t++) {
      ...
      for (let a = 0; a <= x; a++) {
        const s = -m / 2 + a * lt;
        t === 0 ? e.push(I(d / 2, s)) : a === 0 ? e.push(P[t][f]) : a === x ? e.push(N[t][f]) : e.push(C(d / 2, s, u + t * O));
      }
      k.push(e);
    }
    for (let t = 0; t < g; t++) for (let e = 0; e < x; e++) S(k[t][e], k[t][e + 1], k[t + 1][e + 1], k[t + 1][e], H);
    const ke = Math.min(0.2, it * 0.4), Gt = Math.min(0.1, (o - d) / 2 * 0.7), Se = Math.max(1, Math.round(ke / O));
    function A(t, e, a, s) {
      const [c, l] = t, [_, T] = e, At = a[0][s], zt = I(c + _ * Gt, l + T * Gt), me = a[Math.min(Se, a.length - 1)][s];
      S(At, zt, me, me, H);
    }
    ...
    const $t = Math.PI * y * y / 4, _t = Math.PI * y ** 4 / 64, Rt = 2 * _t, q = [], Fe = (o - 2 * mt) / Math.max(1, Et - 1), Ie = (p - 2 * Lt) / Math.max(1, dt - 1);
    for (let t = 0; t < Et; t++) for (let e = 0; e < dt; e++) {
      const a = -o / 2 + mt + t * Fe, s = -p / 2 + Lt + e * Ie;
      Math.abs(a) < d / 2 + 5e-3 && Math.abs(s) < m / 2 + 5e-3 || q.push([
        a,
        s
      ]);
    }
    const je = [
      ...q
    ], Zt = ma(ut / 1e3), Kt = 0.2, Te = Zt / (2 * (1 + Kt)), v = 10, b = 10, M = 6, yt = U / v, Qt = V / b, Ee = pt / M, r = [];
    for (let t = 0; t <= M; t++) {
      const e = [];
      for (let a = 0; a <= b; a++) {
        const s = [];
  ...
  document.body.append(da(i), Ce, ra({
