import { p as co, q as eo, __tla as __tla_0 } from "./aiAgent-86HwhYYU.js";
...
let __tla = Promise.all([
  ...
  function ao(t, n, s, e, c, i) {
    const o = e * i * i * i / (12 * (1 - c * c)), a = [
      ...
      const m = d * _ - y * N, l = _ / m, X = -y / m, D = -N / m, T = d / m;
      let Y = 0, L = 0, E = 0;
      for (let J = 0; J < 4; J++) {
        const j = l * h[J] + X * M[J], p = D * h[J] + T * M[J], k = s[3 * J + 1], F = s[3 * J + 2];
        Y += j * F, L += -p * k, E += p * F - j * k;
      }
      const H = [
        Y,
        L,
    ...
    return A.map(([f, g]) => {
      ...
      return [
        ...
        1,
        2
      ].map((y) => d.reduce((N, _, m) => N + _ * u[m][y], 0));
    });
  }
  ...
  function io(t, n, s, e, c, i) {
    ...
    let M = [], d = [], y = [], N = [], _ = [], m = 0, l = 0, X = 0;
    for (let j = 0; j < 3; j++) for (let p = 0; p < 3; p++) {
      const k = f[j], F = f[p], v = g[j] * g[p], { N: x, dNdxi: Z, dNdeta: R } = $t(k, F);
      let G = 0, I = 0, q = 0, B = 0;
      ...
      for (let S = 0; S < 4; S++) V.push(tt * lt[S] + st * mt[S]), gt.push(C * lt[S] + at * mt[S]);
      const bt = -2 * k * (1 - F * F), pt = -2 * F * (1 - k * k), vt = tt * bt + st * pt, Mt = C * bt + at * pt, St = [], Xt = [], dt = [], ht = [];
      for (let S = 0; S < 4; S++) {
        const et = A[S];
      ...
      j === 1 && p === 1 && (M = x.slice(), d = it.slice(), y = ct.slice(), N = Xt.slice(), _ = dt.slice(), m = vt, l = Mt, X = Math.abs($));
    }
    ...
    const Y = i * D * c * 4 * X;
    for (let j = 0; j < 14; j++) for (let p = 0; p < 14; p++) h[j][p] += Y * T[j] * T[p];
    const L = [
      [
        h[12][12],
        h[12][13]
    ...
    for (let j = 0; j < 12; j++) for (let p = 0; p < 12; p++) H[j][p] = h[j][p];
    ...
    for (let j = 0; j < 12; j++) for (let p = 0; p < 12; p++) {
      let k = 0;
      for (let F = 0; F < 2; F++) for (let v = 0; v < 2; v++) k += h[j][12 + F] * J[F][v] * h[12 + v][p];
      H[j][p] -= k;
    }
    return H;
  }
  function fo(t, n, s, e, c) {
    ...
    for (const f of r) {
      ...
      const [m, l, X, D] = N, T = Jt(2, 12);
      for (let Y = 0; Y < 12; Y++) T[0][Y] = m * _[0][Y] + l * _[1][Y], T[1][Y] = X * _[0][Y] + D * _[1][Y];
      u.push(T);
    }
    for (const [f, g] of A) {
      const { dNdxi: h, dNdeta: M } = $t(f, g), { dNdx: d, dNdy: y, detJ: N, J: _ } = Ht(h, M, t, n), m = Jt(3, 12);
      for (let p = 0; p < 4; p++) m[0][p * 3 + 1] = d[p], m[1][p * 3 + 2] = y[p], m[2][p * 3 + 1] = y[p], m[2][p * 3 + 2] = d[p];
      for (let p = 0; p < 12; p++) for (let k = 0; k < 12; k++) {
        let F = 0;
        F += o * (m[0][p] * m[0][k] + e * m[0][p] * m[1][k] + e * m[1][p] * m[0][k] + m[1][p] * m[1][k]), F += o * (1 - e) / 2 * m[2][p] * m[2][k], i[p][k] += F * Math.abs(N);
      }
      const l = Jt(2, 12), X = 0.5 * (1 - g), D = 0.5 * (1 + g), T = 0.5 * (1 - f), Y = 0.5 * (1 + f), [L, E, H, J] = _, j = 1 / N;
      for (let p = 0; p < 12; p++) {
        const k = X * u[0][0][p] + D * u[1][0][p], F = T * u[2][1][p] + Y * u[3][1][p];
        l[0][p] = j * (J * k - E * F), l[1][p] = j * (-H * k + L * F);
      }
      for (let p = 0; p < 12; p++) for (let k = 0; k < 12; k++) i[p][k] += b * (l[0][p] * l[0][k] + l[1][p] * l[1][k]) * Math.abs(N);
    }
    return i;
  }
  function lo(t, n, s) {
    ...
    for (let y = 0; y < 12; y++) for (let N = 0; N < 12; N++) {
      let _ = 0;
      const m = y / 3 | 0, l = y % 3, X = N / 3 | 0, D = N % 3;
      for (let T = 0; T < 3; T++) {
        const Y = h[T][l];
        if (Y !== 0) for (let L = 0; L < 3; L++) {
          const E = h[L][D];
          E !== 0 && (_ += Y * A[m * 3 + T][X * 3 + L] * E);
        }
      }
      M[y][N] = _;
    }
  ...
  function Mo(t) {
    ...
    for (let l = 0; l < 3; l++) {
      const X = l * 6, D = X + 3;
      for (let T = 0; T < 3; T++) for (let Y = 0; Y < 3; Y++) m[X + T][X + Y] = _[T][Y], m[D + T][D + Y] = _[T][Y];
    }
    return m;
  }
  yo = function(t, n, s) {
  ...
  function Ao(t, n, s) {
    ...
    const h = g > 0 && a > 0 ? 12 * i * e / (a * g * u ** 2) : 0, M = f > 0 && a > 0 ? 12 * i * c / (a * f * u ** 2) : 0, d = i * o / A, y = a * b / A, N = 12 * i * e / u ** 3 / (1 + h), _ = 6 * i * e / u ** 2 / (1 + h), m = 4 * i * e / u * (1 + h / 4) / (1 + h), l = 2 * i * e / u * (1 - h / 2) / (1 + h), X = 12 * i * c / u ** 3 / (1 + M), D = 6 * i * c / u ** 2 / (1 + M), T = 4 * i * c / u * (1 + M / 4) / (1 + M), Y = 2 * i * c / u * (1 - M / 2) / (1 + M);
    return [
      [
        d,
        0,
  ...
  function Xo(t, n, s) {
    ...
    const e = ((_a = n.elasticities) == null ? void 0 : _a.get(s)) ?? 0, c = ((_b = n.elasticitiesOrthogonal) == null ? void 0 : _b.get(s)) ?? 0, i = ((_c = n.poissonsRatios) == null ? void 0 : _c.get(s)) ?? 0, o = ((_d = n.shearModuli) == null ? void 0 : _d.get(s)) ?? 0, a = ((_e = n.thicknesses) == null ? void 0 : _e.get(s)) ?? 0, b = c > 0, A = b ? H(e, c, o, i, a) : L(e, i, a), r = b ? J(o, a) : E(e, i, a), u = b ? ro(e, c, o, i) : so(e, i), f = t.map(([v, x]) => [
      v,
      x
    ]), g = f[1][0] - f[0][0], h = f[2][0] - f[0][0], M = f[0][1] - f[1][1], d = f[2][1] - f[0][1], y = 0.5 * (g * d - h * -M), N = j(f), _ = k(f), m = F(f, u, a), l = W(W(Kt(N), r), N), X = W(W(Kt(_), A), _), D = ot(18, 18).toArray(), T = W(Bt(l, X), y), Y = [
      [
        0,
        1,
        5
    ...
    function j(v) {
      const x = ot(2, 18).toArray(), [Z, R] = v[0], [G, I] = v[1], [q, B] = v[2], $ = 0.5 * ((G - Z) * (B - R) - (q - Z) * -(R - I)), tt = (Z + G + q) / 3, st = (R + I + B) / 3, C = [
        ...
        B,
        R
      ], V = 1 / 3, [gt, bt, pt, vt] = p(C, at), [Mt, St, Xt, dt] = p(it, ct), [ht, rt, Ot, S] = p(lt, mt), et = ot(2, 18).toArray(), ut = ot(2, 18).toArray(), yt = ot(2, 18).toArray();
      for (let O = 0; O < 2; O++) for (let z = 0; z < 6; z++) et[O][z] = V * gt[O][z] + bt[O][z], et[O][z + 6] = V * gt[O][z] + pt[O][z], et[O][z + 12] = V * gt[O][z], ut[O][z] = V * Mt[O][z], ut[O][z + 6] = V * Mt[O][z] + St[O][z], ut[O][z + 12] = V * Mt[O][z] + Xt[O][z], yt[O][z] = V * ht[O][z] + Ot[O][z], yt[O][z + 6] = V * ht[O][z], yt[O][z + 12] = V * ht[O][z] + rt[O][z];
    ...
    function k(v) {
      const x = ot(3, 18).toArray(), [Z, R] = v[0], [G, I] = v[1], [q, B] = v[2], $ = G - Z, tt = q - Z, st = q - G, C = I - B, at = B - R, it = R - I, ct = 0.5 * ($ * at - tt * -it), lt = C / (2 * ct), mt = st / (2 * ct), V = at / (2 * ct), gt = -tt / (2 * ct), bt = it / (2 * ct), pt = $ / (2 * ct);
      return x[0][4] = lt, x[0][10] = V, x[0][16] = bt, x[1][3] = -mt, x[1][9] = -gt, x[1][15] = -pt, x[2][3] = -lt, x[2][4] = mt, x[2][9] = -V, x[2][10] = gt, x[2][15] = -bt, x[2][16] = pt, x;
    }
    function F(v, x, Z) {
      ...
      const mt = 1 / 8, V = mt / 6, gt = mt ** 2 / 4, bt = 1, pt = 2, vt = 1, Mt = 0, St = 1, Xt = -1, dt = -1, ht = -1, rt = -2, Ot = v[0][0], S = v[0][1], et = v[1][0], ut = v[1][1], yt = v[2][0], O = v[2][1], z = Ot - et, Lt = et - yt, Qt = yt - Ot, kt = S - ut, Tt = ut - O, qt = O - S, jt = -z, wt = -Lt, xt = -Qt, Nt = -kt, _t = -Tt, Dt = -qt, w = 0.5 * (jt * qt - Qt * -kt), P = 2 * w, K = 4 * w, Q = 0.5 * Z, zt = w * Z, Yt = jt ** 2 + Nt ** 2, ft = wt ** 2 + _t ** 2, At = xt ** 2 + Dt ** 2;
      ...
      const nt = P / 3;
      tt[0][0] = nt * bt / Yt, tt[0][1] = nt * pt / Yt, tt[0][2] = nt * vt / Yt, tt[1][0] = nt * Mt / ft, tt[1][1] = nt * St / ft, tt[1][2] = nt * Xt / ft, tt[2][0] = nt * dt / At, tt[2][1] = nt * ht / At, tt[2][2] = nt * rt / At, st[0][0] = nt * rt / Yt, st[0][1] = nt * dt / Yt, st[0][2] = nt * ht / Yt, st[1][0] = nt * vt / ft, st[1][1] = nt * bt / ft, st[1][2] = nt * pt / ft, st[2][0] = nt * Xt / At, st[2][1] = nt * Mt / At, st[2][2] = nt * St / At, C[0][0] = nt * St / Yt, C[0][1] = nt * Xt / Yt, C[0][2] = nt * Mt / Yt, C[1][0] = nt * ht / ft, C[1][1] = nt * rt / ft, C[1][2] = nt * dt / ft, C[2][0] = nt * pt / At, C[2][1] = nt * vt / At, C[2][2] = nt * bt / At, at = W(Bt(U(tt), U(st)), 0.5).toArray(), it = W(Bt(U(st), U(C)), 0.5).toArray(), ct = W(Bt(U(C), U(tt)), 0.5).toArray();
  ...
  ko = function(t, n, s, e) {
    ...
    n.forEach((A, r) => {
      ...
      if (A.length === 2) {
        ...
        W(g, f);
        const h = xo(s, r), M = No(u), d = vo(f), y = So(u), _ = W(1 / (2 * y), W(W(h, M), d)).toArray(), m = ((_e = s.thicknesses) == null ? void 0 : _e.get(r)) ?? 1, l = _[0][0] * m, X = _[1][0] * m, D = _[2][0] * m, T = _[0][1] * (m ** 3 / 12), Y = _[1][1] * (m ** 3 / 12), L = _[2][1] * (m ** 3 / 12);
      ...
      if (n.forEach((f, g) => {
        ...
        for (const [g, h] of u) {
          ...
          const y = r.get(g), N = (X) => {
            ...
            const H = X.get(g) ?? 0;
            for (const j of d) {
              const p = r.get(j), k = p[0] - y[0], F = p[1] - y[1], v = (X.get(j) ?? 0) - H;
          ...
          }, _ = N(a.bendingXX), m = N(a.bendingYY), l = N(a.bendingXY);
          a.tranverseShearX.set(g, _[0] + l[1]), a.tranverseShearY.set(g, m[1] + l[0]);
        }
      }
    }
    return n.forEach((A, r) => {
      if (A.length !== 3 && A.length !== 4) return;
      const u = A.length, f = new Array(u).fill(0), g = new Array(u).fill(0), h = new Array(u).fill(0), M = new Array(u).fill(0), d = new Array(u).fill(0), y = new Array(u).fill(0), N = new Array(u).fill(0), _ = new Array(u).fill(0), m = new Array(u).fill(0);
      A.forEach((Y, L) => {
        const E = (b.get(Y) || []).filter((p) => n[p].length === 3 || n[p].length === 4), H = (p) => Gt(E.map((k) => p.get(k) ?? 0)), J = (p, k) => Gt(E.map((F) => {
          const v = o.get(F), x = v ? n[F].indexOf(Y) : -1;
          return v && x >= 0 ? v[x][p] : k.get(F) ?? 0;
        }));
        f[L] = J(0, a.membraneXX), g[L] = J(1, a.membraneYY), h[L] = J(2, a.membraneXY);
        const j = (p, k) => Gt(E.map((F) => {
          const v = i.get(F), x = v ? n[F].indexOf(Y) : -1;
          return v && x >= 0 ? v[x][p] : k.get(F) ?? 0;
        }));
        M[L] = j(0, a.bendingXX), d[L] = j(1, a.bendingYY), y[L] = j(2, a.bendingXY), N[L] = H(a.tranverseShearX), _[L] = H(a.tranverseShearY), m[L] = H(a.vonMises);
  ...
  function wo(t, n, s, e) {
    ...
    const mt = p[0][0] * it + p[0][1] * ct, V = p[1][0] * it + p[1][1] * ct, gt = p[2][2] * lt;
    let bt = 0, pt = 0, vt = 0;
    for (let w = 0; w < 4; w++) {
      const P = J[w * 6 + 3], K = J[w * 6 + 4];
      bt += C[w] * K, pt += -at[w] * P, vt += at[w] * K - C[w] * P;
    }
    const Mt = -1, St = Mt * (F[0][0] * bt + F[0][1] * pt), Xt = Mt * (F[1][0] * bt + F[1][1] * pt), dt = Mt * (F[2][2] * vt);
    let ht = null;
    if (Math.abs(q) > 1e-20) {
      const w = [];
    ...
    const Lt = ut * yt, Qt = ut * O, kt = mt / o + 6 * St / (o * o), Tt = V / o + 6 * Xt / (o * o), qt = gt / o + 6 * dt / (o * o), jt = Math.sqrt(kt * kt - kt * Tt + Tt * Tt + 3 * qt * qt), wt = mt / o - 6 * St / (o * o), xt = V / o - 6 * Xt / (o * o), Nt = gt / o - 6 * dt / (o * o), _t = Math.sqrt(wt * wt - wt * xt + xt * xt + 3 * Nt * Nt), Dt = Math.max(jt, _t);
    return {
      Nx: mt,
      Ny: V,
      Nxy: gt,
