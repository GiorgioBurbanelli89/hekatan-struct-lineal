import { _ as ba, m as La, d as Ca, __tla as __tla_0 } from "./didacticCpp-ClOTguHC.js";
import { b as qe, v as Pa, V as $, d as Te, B as Ze } from "./Text-C1TX4d8g.js";
import { a as za, __tla as __tla_1 } from "./analyze-DtswgObf.js";
...
let __tla = Promise.all([
  ...
  let W, oe, He, ya, ke, ka, Ia, Xe, Sa, wa, Na;
  W = 2004e4;
  oe = 0.2;
  He = W / (2 * (1 + oe));
  ya = 9.81;
  ...
  Va = {
    ...
    build(e, s) {
      ...
      e.Hp;
      const I = 9.80665, S = (e.useDead ?? 1) >= 0.5 ? 1 : 0, B = (e.useLive ?? 1) >= 0.5 ? 1 : 0, ne = (S * (e.P1 ?? 0) + B * (e.P1_L ?? 0)) * I, te = (S * (e.P2 ?? 0) + B * (e.P2_L ?? 0)) * I, G = e.ks, se = (S * (e.M1x ?? 0) + B * (e.M1x_L ?? 0)) * I, re = (S * (e.M1y ?? 0) + B * (e.M1y_L ?? 0)) * I, de = (S * (e.M2x ?? 0) + B * (e.M2x_L ?? 0)) * I, ce = (S * (e.M2y ?? 0) + B * (e.M2y_L ?? 0)) * I, T = Math.round(e.nSubX), _ = Math.round(e.nSubY), w = (u - y) / 2, Z = k / 2, O = u / 2, b = r + f + x / 2, Y = y / 2 + w, le = O;
      function H(a, o, n, t) {
        const d = [
          a,
          ...n.filter((l) => l > a && l < o),
      ...
      const Ie = K(Z, O, 0), Se = K(b, Y, 0), ee = [];
      for (const a of R) a >= Z - 1e-6 && ee.push(K(a, O, 0));
      for (const a of j) a <= b + 1e-6 && ee.push(K(a, Y, 0));
      let we = 0;
      for (let a = 0; a < ee.length - 1; a++) {
        ...
        V.push([
          o,
          n
        ]), U.set(t, W), J.set(t, oe), he.set(t, He), fe.set(t, M * h), ge.set(t, M * h ** 3 / 12), ve.set(t, h * M ** 3 / 12), Me.set(t, 0.28 * M * h ** 3), Q.set(t, ke), xe.set(t, {
          type: "rect",
          b: M,
          h
        }), we++;
      }
      console.log(`[viga amarre] ${we} segmentos col-a-col compartiendo nodos con slab`);
      const Ne = 0.5, We = W * 1e3, Ue = He * 1e3, Je = k * k * 100, De = k ** 4 / 12 * 100, Qe = 0.14 * k ** 4 * 100;
      function Ee(a, o, n, t, d, c) {
        let C = 0;
        for (let l = 0; l < d.length; l++) for (let m = 0; m < t.length; m++) {
          const D = t[m], P = d[l];
          if (Math.abs(D - o) > Ne / 2 + 1e-6 || Math.abs(P - n) > Ne / 2 + 1e-6) continue;
          const v = c[l][m];
          if (v === a) continue;
          const i = V.length;
          V.push([
      ...
      const ea = Ee(Ie, Z, O, R, X, p), aa = Ee(Se, b, Y, j, q, N);
      ...
      const Fe = r / T, Ae = u / _, oa = x / T, na = y / _, Be = 0.5, L = [], ae = [];
      ...
      try {
        s.deformOutputs.val = Ca(s.nodes.val, s.elements.val, s.nodeInputs.val, s.elementInputs.val, L);
        const a = za(s.nodes.val, s.elements.val, s.elementInputs.val, s.deformOutputs.val), o = s.deformOutputs.rawVal.deformations, n = /* @__PURE__ */ new Map();
        let t = 0, d = 0;
      ...
      for (const a of ae) {
        ...
        o && Number.isFinite(o[2]) && (Le = Math.max(Le, Math.abs(o[2])));
      }
      const ta = new Set(ae), Ce = Xe * 12, ie = (_a = document.querySelector("#viewer")) == null ? void 0 : _a.__settings, Ve = (a, o, n = 1) => {
        const t = a ? o : 0, c = -(Le * Math.max(t, 1) + ka), C = n > 0 ? n : n < 0 ? -1 / n : 1, l = Ia * C, m = Sa * C, D = [];
        for (const P of ae) {
          ...
          ye.push(new $($e, Oe, Re)), D.push(new Te(new Ze().setFromPoints(ye), wa));
          const F = m, ga = [
            new $(i - F, E - F, c),
            new $(i + F, E - F, c),
            new $(i + F, E + F, c),
    ...
    async exportF2k(e) {
      const { downloadEdificioCimentacionF2k: s } = await ba(async () => {
        const { downloadEdificioCimentacionF2k: b } = await import("./f2kCimentacionCompleta-Ct_BuFMO.js");
        return {
          downloadEdificioCimentacionF2k: b
        };
      }, [], import.meta.url), r = 9.80665, u = e.Lz1, f = e.Bz1, M = e.Lv, h = e.Bv, x = e.Hv, y = e.Lz2, A = e.Bz2, k = e.tz, I = e.bc, S = e.ks, B = (f - A) / 2, ne = I / 2, te = f / 2, G = u + M + y / 2, se = A / 2 + B, re = u / 2, de = f / 2, ce = u + M + y / 2, T = B + A / 2, _ = (e.useDead ?? 1) >= 0.5 ? 1 : 0, w = (e.useLive ?? 1) >= 0.5 ? 1 : 0, Z = [
        {
          xC: re,
          yC: de,
          xCol: ne,
          ...
          P_live_kN: w * (e.P1_L ?? 0) * r,
          Mx_live_kNm: w * (e.M1x_L ?? 0) * r,
          My_live_kNm: w * (e.M1y_L ?? 0) * r,
          label: 1
        },
        {
          xC: ce,
          ...
          P_live_kN: w * (e.P2_L ?? 0) * r,
          Mx_live_kNm: w * (e.M2x_L ?? 0) * r,
          My_live_kNm: w * (e.M2y_L ?? 0) * r,
          label: 2
        }
      ], O = [
        {
      ...
      try {
        ...
        const b = (e.P1 ?? 0) + (e.P1_L ?? 0), Y = (e.P2 ?? 0) + (e.P2_L ?? 0), le = 1.4 * (e.P1 ?? 0) + 1.7 * (e.P1_L ?? 0), H = 1.4 * (e.P2 ?? 0) + 1.7 * (e.P2_L ?? 0);
        alert(`\u2705 F2K exportado con 2 zapatas + viga de amarre:

\u2022 Z1 ${u}\xD7${f} m \u2014 PD=${e.P1} + PL=${e.P1_L ?? 0} = ${b} tonf | Pu=${le.toFixed(1)} tonf
\u2022 Z2 ${y}\xD7${A} m \u2014 PD=${e.P2} + PL=${e.P2_L ?? 0} = ${Y} tonf | Pu=${H.toFixed(1)} tonf
\u2022 Viga amarre ${h}\xD7${x} m, Lv=${M} m
\u2022 ks = ${S.toFixed(0)} kN/m\xB3

Patrones de carga incluidos: Dead (+peso propio), Live
Combinaci\xF3n: Pu = 1.4D + 1.7L (ACI 318 nominal Guerra MDI)

Abrilo en SAFE 20.x: File \u2192 Import \u2192 SAFE Text File (.f2k)`), console.log(`[F2K Zapata+Viga] exportado: Z1=${u}\xD7${f}, Z2=${y}\xD7${A}, ks=${S} kN/m\xB3`);
      } catch (b) {
        alert(`\u274C Error exportando F2K: ${b.message}`), console.error(b);
      }
    }
  };
});
