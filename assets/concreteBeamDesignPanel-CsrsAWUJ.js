...
let __tla = Promise.all([
  ...
  oe = function(t) {
    const { mesh: e, viewerElm: o, onStatusChange: a } = t, i = t.scalePercent ?? Ft, [s, d] = t.visFrequencyRange ?? [
      0.5,
      3
    ];
    let n = null, l = 0, m = 0, u = [], r = [], c = null;
    function p() {
      var _a;
    ...
    function S() {
      ...
      const h = n.frequencies[l] ?? 0, x = h > 0 ? 1 / h : 0, y = [
        "Ux",
        "Uy",
        "Uz",
        "Rx",
    ...
    function w(h) {
      ...
      if (m && (cancelAnimationFrame(m), m = 0), h) {
        const x = p();
        (x == null ? void 0 : x.deformedShape) && c !== null && (x.deformedShape.val = c, c = null);
        const y = g(u) ? u : g(r) ? r : [];
    ...
    function v() {
      ...
      w(false);
      const h = p();
      (h == null ? void 0 : h.deformedShape) && (c === null && (c = h.deformedShape.val), h.deformedShape.val = false);
      const x = n.modeShapes[l], y = ((_a = n.frequencies) == null ? void 0 : _a[l]) || 1, k = ((_b = n.frequencies) == null ? void 0 : _b[0]) || 1, T = t.velocidadPorFrecuencia ? Math.max(s, Math.min(d, y / k)) : 1 / (t.periodoVisible ?? It);
      ...
      const b = C > 1e-12 ? E * i / 100 / C : 1, F = performance.now(), z = A > 4e3 ? 100 : A > 1500 ? 66 : 0;
      let D = -1 / 0;
      const L = () => {
        var _a2;
    ...
    function V(h) {
      ...
      w(false);
      const x = p();
      (x == null ? void 0 : x.deformedShape) && (c === null && (c = x.deformedShape.val), x.deformedShape.val = false), l = Math.max(0, Math.min((((_a = n.frequencies) == null ? void 0 : _a.length) ?? 1) - 1, h));
      ...
      const E = $ > 1e-12 ? A * i / 100 / $ : 1, C = new Array(T);
      for (let b = 0; b < T; b++) {
        const F = k[b];
        C[b] = [
          F[0] + (y[b * 6] || 0) * E,
    ...
    return {
      setResults(h) {
        var _a;
        n = h, l >= (((_a = h == null ? void 0 : h.frequencies) == null ? void 0 : _a.length) ?? 0) && (l = 0), u = e.nodes.rawVal.map((x) => [
          ...x
        ]), M();
      },
      setMode(h) {
        ...
        l = Math.max(0, Math.min(x - 1, h)), m !== 0 ? v() : M();
      },
      showStatic(h) {
        V(h);
      },
  ...
  function Q(t, e) {
    const a = Array.from(document.querySelectorAll("button,.tp-btnv_b")).find((i) => {
      const s = (i.textContent || "").replace(/\s+/g, " ").trim();
...
la zapata (SAFE y OpenSees).`);
  ...
  function dt() {
    ...
    const i = (r) => {
      ...
      if (!c) return false;
      const p = c.style.display !== "none";
      return c.removeAttribute("style"), c.className = "piel", p || (c.style.display = "none"), a.appendChild(c), true;
    };
    let s = 0;
    const d = () => {
      const r = i("hk-home-btn"), c = i("hk-back-btn");
      (!r || !c) && ++s < 20 && setTimeout(d, 400);
    };
    (() => {
      for (const r of [
        "analisis",
        ...
        c.id = `hk-${r}-btn`, c.className = "piel", c.textContent = Ot[r] + " \u25BE", c.onclick = (p) => {
          p.stopPropagation(), jt(r, c);
        }, a.appendChild(c);
      }
    })(), d();
    const l = () => {
    ...
    m.__hekatanActualizarBotonVolver = (r) => {
      u == null ? void 0 : u(r);
      const c = document.getElementById("hk-back-btn");
      c && (c.style.display = r ? "" : "none");
  ...
  function jt(t, e) {
    ...
    a.innerHTML = `<div style="padding:2px 6px 6px;color:#9cc">${Bt[t]}</div>` + i + nt[t].map((d) => `<div data-id="${d.id}" style="padding:6px 8px;border-radius:4px;cursor:pointer"><b>${d.icono} ${d.titulo}</b><div style="opacity:.75;margin-top:2px">${d.detalle}</div></div>`).join(""), a.querySelectorAll("[data-id]").forEach((d) => {
  ...
  he = function(t) {
    ...
    t.addEventListener("pointerdown", (n) => {
      ...
      i(n.target) && !n.target.closest("[data-plegar]") && s();
    });
    const d = () => {
  ...
  function zt(t) {
    const e = Math.max(0, K(t.coverToBarCenter, _t)), o = Math.max(0, K(t.compressionCoverToBarCenter, e)), a = t.h - e, i = t.b, s = Math.max(1e-9, i - 2 * e), d = Math.max(1e-9, t.h - 2 * e), n = s * d;
    return {
      b: i,
      h: t.h,
      d: a,
  ...
  function Gt(t, e, o, a, i, s, d, n) {
    const l = t / ot, m = t / 1e3, u = e / 1e3, r = Kt(l), c = 3e-3 / (3e-3 + 5e-3) * i, p = r * c, M = i * i - 2 * d / (0.85 * t * n * a), S = M <= 0 ? i : i - Math.sqrt(M), _ = Math.max(3 * Math.sqrt(m) / u * a * i, 200 / u * a * i), g = 0.04 * a * i;
    let w = 0, v = 0, V = false;
    if (d > 0) if (S <= p) w = d / Math.max(1e-12, n * e * Math.max(1e-9, i - S / 2));
    else {
      V = true;
      const A = 0.85 * t * a * p * (i - p / 2) * n, $ = Math.max(0, d - A), C = (Math.min(e, o * 3e-3 * Math.max(0, c - s) / Math.max(1e-12, c)) - 0.85 * t) * (i - s) * n;
      v = C > 1e-9 ? $ / C : 0;
      const b = A / Math.max(1e-12, e * (i - p / 2) * n), F = $ / Math.max(1e-12, e * (i - s) * n);
      w = b + F;
    }
    const h = Math.min(g, Math.max(_, w)), x = Math.min(g, Math.max(_, v)), y = n * h * e * Math.max(0, i - S / 2), k = d <= 1e-12 ? 0 : d / Math.max(1e-12, y);
    return {
      Mu: d,
      beta1: r,
      cMax: c,
  ...
  function Xt(t, e, o, a, i, s) {
    const d = t / 6.894757293, n = e / (0.0254 * 0.0254), l = o / 0.0254, r = 1 + I(i) * 224.808943 / Math.max(1e-12, 4 * n * Math.sqrt(d)), c = s * Math.sqrt(d) * n * n / Math.max(1e-12, l) * 112984829e-12;
    return {
      Tth: 2 * c * r,
      Tcr: 4 * c * r
    };
  ...
  function Wt(t) {
    const e = zt(t.section), o = t.material, a = t.demand, i = t.code ?? "ACI 318-19", s = I(o.fc), d = I(o.fy), n = I(o.fys ?? d), l = I(o.lambda ?? 1), m = I(o.Es ?? 2e8), u = I(o.phiFlexure ?? 0.9), r = I(o.phiShearTorsion ?? 0.75), c = I(o.phiCriticalTorsion ?? r), p = K(a.Mu), M = Math.abs(K(a.Vu)), S = Math.abs(K(a.Tu)), _ = K(a.Pu ?? 0), g = s / ot, w = s / 1e3, v = Gt(s, d, m, e.b, e.d, e.dp, Math.abs(p), u);
    ...
    const V = o.concreteShearCapacity ?? 0.53 * l * Math.sqrt(g) * ot * e.bw * e.d, h = I(V), x = h + 0.66 * Math.sqrt(w) * 1e3 * e.bw * e.d, y = r * h, k = r * x, T = I((M - y) / Math.max(1e-12, r * n * e.d)), A = I(t.stirrupSpacing ?? 0.3), $ = Math.max(0.075 * Math.sqrt(w) * 1e3 * e.bw / Math.max(1, d), 0.35 * e.bw / Math.max(1, d * A)), E = Math.max(T, $), C = M <= 1e-12 ? 0 : M / Math.max(1e-12, k), b = M <= k + 1e-9, F = Xt(s, e.Acp, e.pcp, e.b * e.h, _, l), z = c * F.Tth, D = c * F.Tcr, L = S <= 1e-12 ? 0 : D / S, q = S <= D + 1e-9, U = S > z + 1e-9, O = U ? S / Math.max(1e-12, 2 * r * e.A0 * n) : 0, j = U ? S * e.ph / Math.max(1e-12, 2 * r * e.A0 * d) : 0, P = I(0.5 * Math.sqrt(w) * 1e3 * e.Acp / Math.max(1, d) - O * e.ph), W = U ? Math.max(j, P) : 0, pt = S <= D + 1e-9, ft = Math.sqrt(Math.pow(M / Math.max(1e-12, e.bw * e.d), 2) + Math.pow(S * e.ph / Math.max(1e-12, 1.7 * e.Aoh * e.Aoh), 2)), Ct = o.combinedAdditiveCoefficient ?? 2, ht = r * (h / Math.max(1e-12, e.bw * e.d) + Ct * Math.sqrt(g) * ot), bt = ft / Math.max(1e-12, ht), Z = bt <= 1 + 1e-9;
    let J = "OK", xt;
  ...
  function Et(t, e) {
    ...
    if (!o || !a) return false;
    const i = Math.hypot(a[0] - o[0], a[1] - o[1], a[2] - o[2]);
    return i > 1e-9 && Math.abs(a[2] - o[2]) <= Math.max(0.03, i * 0.02);
  }
  function wt(t) {
  ...
  function Ht(t, e, o) {
    ...
    const a = e.map((u, r) => ({
      ...
      const r = t[u[0]], c = t[u[1]];
      if (!r || !c) return null;
      const p = Math.hypot(c[0] - r[0], c[1] - r[1], c[2] - r[2]);
      return p <= 1e-9 ? null : [
        (c[0] - r[0]) / p,
        (c[1] - r[1]) / p,
        (c[2] - r[2]) / p
    ...
    for (let u = 0; u < a.length; u++) for (let r = u + 1; r < a.length; r++) {
      const c = a[u], p = a[r];
      if (c.element[0] !== p.element[0] && c.element[0] !== p.element[1] && c.element[1] !== p.element[0] && c.element[1] !== p.element[1]) continue;
      const M = n(c.element), S = n(p.element), _ = (_a = o == null ? void 0 : o.get) == null ? void 0 : _a.call(o, c.index), g = (_b = o == null ? void 0 : o.get) == null ? void 0 : _b.call(o, p.index);
      !M || !S || !_ || !g || _.b !== g.b || _.h !== g.h || Math.abs(M[0] * S[0] + M[1] * S[1] + M[2] * S[2]) > 0.999 && d(c.index, p.index);
    }
  ...
  function Yt(t, e) {
    const o = t.states, a = et(o.nodes), i = et(o.elements), s = et(o.elementInputs) ?? {}, d = et(o.analyzeOutputs) ?? {}, n = s.sectionShapes, l = d.shearsY, m = d.torsions, u = d.bendingsZ, r = d.bendingsY, c = d.normals, p = [], M = [
      0,
      0.25,
      0.5,
      0.75,
    ...
    return i.forEach((_, g) => {
      var _a;
      if (!Et(a, _)) return;
      const w = wt((_a = n == null ? void 0 : n.get) == null ? void 0 : _a.call(n, g));
      if (!w) return;
      const v = H(l, g), V = H(m, g), h = H(u, g), x = H(r, g), y = H(c, g), k = a[_[0]], T = a[_[1]], A = Math.hypot(T[0] - k[0], T[1] - k[1], T[2] - k[2]);
      for (const $ of M) {
  ...
  function Zt(t, e, o) {
    const a = e.filter((m) => m.beam === t), i = mt(e).get(t) ?? a[0];
    if (!i) return `# Reporte RC \xB7 ${t}

No hay datos para esta viga.`;
    const s = i.result, d = s.interaction.pass ? ut(s) : "O/S #45", n = a.map((m) => {
      const u = m.result;
      return `#| ${f(m.station, 3)} m | ${f(u.flexure.Mu)} kN*m | ${f(u.shear.Vu)} kN | ${f(u.torsion.Tu)} kN*m | ${f(u.interaction.ratio, 4)} | ${u.interaction.pass ? "OK" : "O/S #45"} |`;
    }).join(`
`);
    return [
      "```lisp",
      [
        ...
        `#: Estaci\xF3n control: ${f(i.station, 3)} m \xB7 Norma: ${o.code} \xB7 Estado: ${d}`,
        "",
...
`);
  ...
  function Jt(t) {
    ...
    const e = mt(t), o = t.map((a, i) => {
      const s = a.result, d = e.get(a.beam) === a ? ut(s) : "", n = s.interaction.pass ? d : "See ErrMsg", l = s.interaction.error ?? "";
      return `<tr data-beam="${a.beam}" data-row="${i}" style="cursor:pointer" title="Abrir reporte de ${a.beam}">
      <td>${a.beam}</td>
      <td>${f(a.station)}</td>
      <td>${f(a.Mu)}</td>
      <td>${f(a.Vu)}</td>
...
`)
    ...
    }), s = URL.createObjectURL(i), d = document.createElement("a");
  ...
  be = function(t) {
    ...
    o("hkdv-res").onclick = (n) => {
      ...
      m && qt(`Reporte RC \xB7 ${m}`, Zt(m, i, a()));
    };
    let i = [];
    const s = () => {
      const n = t ?? {
      ...
      try {
        i = Yt(n, a());
        const l = i.filter((r) => !r.result.interaction.pass).length, m = i.filter((r) => !r.result.torsion.criticalPass).length, u = new Set(i.map((r) => r.beam)).size;
        o("hkdv-info").textContent = `${u} barras \xB7 ${l} estaciones O/S #45 \xB7 ${m} estaciones fuera de \u03C6Tcr`, o("hkdv-res").innerHTML = Jt(i);
      } catch (l) {
    ...
    }, o("hkdv-run").onclick = s, o("hkdv-csv").onclick = () => Qt(i);
  };
});
export {
  __tla,
