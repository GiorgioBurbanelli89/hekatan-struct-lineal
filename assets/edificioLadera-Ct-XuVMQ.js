import { a as mo, __tla as __tla_0 } from "./analyze-Bun5MfUS.js";
import { m as uo, d as fo, __tla as __tla_1 } from "./didacticCpp-BoYi16rL.js";
import { e as Co } from "./materials-VwssM8Vw.js";
let xo;
  ...
  xo = {
    ...
    computedLabels(t, n) {
      ...
      r.forEach((i, l) => {
        const m = C[l];
        if (!m) return;
        const M = Math.round(m[2] * 100) / 100;
        let F = true;
        ...
        if (!F) return;
        const I = i[2], N = i[0], L = i[1], b = Math.abs(I), w = Math.sqrt(N * N + L * L), h = x.get(M) ?? {
          count: 0,
          maxP: 0,
          sumP: 0,
          maxV: 0
        };
        h.count += 1, h.sumP += b, b > h.maxP && (h.maxP = b), w > h.maxV && (h.maxV = w), x.set(M, h);
      });
      const v = {
        "\u2500\u2500 Reacciones por cota \u2500\u2500": ""
      }, _ = Array.from(x.keys()).sort((i, l) => i - l);
      for (const i of _) {
        const l = x.get(i), m = l.maxP / 9.80665, M = l.maxV / 9.80665;
        v[`z=${i.toFixed(2)} m`] = `${l.count} cols, P_max=${m.toFixed(1)} tonf, V_max=${M.toFixed(2)} tonf`;
      }
      let p = 1 / 0, E = -1 / 0;
      ...
      if (P > 0.1) {
        ...
        v["\u2500\u2500 Asimetr\xEDa ladera \u2500\u2500"] = "", v["Desnivel cim."] = `${P.toFixed(2)} m`, v["Col. m\xE1s corta"] = `~${l.toFixed(1)} pisos`, v["Col. m\xE1s larga"] = `~${m.toFixed(1)} pisos`;
        const M = Math.pow(m / Math.max(l, 0.5), 3);
        v["k_corta/k_larga"] = `${M.toFixed(1)}\xD7 \u26A0 torsi\xF3n`;
      }
      return v;
    },
    build(t, n) {
      const d = Math.round(t.nVanosX), r = Math.round(t.nVanosY), C = Math.round(t.nPisos), y = t.spanX, x = t.spanY, v = t.hPiso, _ = t.cotaCubierta, p = [];
      for (let e = 0; e <= d; e++) {
        const a = `cotaPortico${e + 1}`;
        p.push(t[a] ?? 0);
      }
      const E = t.fcConcr * 0.0981, P = Co(E), i = 0.2, l = P / (2 * (1 + i)), m = t.colSize * t.colSize, M = t.vigaB * t.vigaH, F = t.colSize ** 4 / 12, I = t.colSize ** 4 / 12, N = 0.14 * t.colSize ** 4, L = t.vigaB * t.vigaH ** 3 / 12, b = t.vigaH * t.vigaB ** 3 / 12, w = 0.14 * Math.min(t.vigaB, t.vigaH) ** 3 * Math.max(t.vigaB, t.vigaH), h = [], g = /* @__PURE__ */ new Map(), V = (e, a, o) => {
      ...
      for (let e = 0; e <= C; e++) O.push(_ - e * v);
      const X = /* @__PURE__ */ new Map();
      for (let e = 0; e <= d; e++) for (let a = 0; a <= r; a++) {
        const o = e * y, c = a * x, u = p[e], S = [];
        S.push(V(o, c, u));
        const ro = O.filter((z) => z > u + 1e-3).sort((z, lo) => z - lo);
        for (const z of ro) S.push(V(o, c, z));
        X.set(`${e},${a}`, S);
      }
      const $ = [], B = /* @__PURE__ */ new Map(), A = /* @__PURE__ */ new Map(), G = /* @__PURE__ */ new Map(), Y = /* @__PURE__ */ new Map(), j = /* @__PURE__ */ new Map(), H = /* @__PURE__ */ new Map(), T = /* @__PURE__ */ new Map(), R = /* @__PURE__ */ new Map(), D = /* @__PURE__ */ new Map(), W = (e, a) => {
        const o = $.length;
        $.push([
          e,
          a
        ]), B.set(o, P), A.set(o, i), G.set(o, m), j.set(o, F), Y.set(o, I), H.set(o, N), T.set(o, l), R.set(o, Q), D.set(o, {
          type: "rect",
          b: t.colSize,
          h: t.colSize
        });
        ...
        $.push([
          e,
          a
        ]), B.set(o, P), A.set(o, i), G.set(o, M), j.set(o, L), Y.set(o, b), H.set(o, w), T.set(o, l), R.set(o, Q), D.set(o, {
          type: "rect",
          b: t.vigaB,
          h: t.vigaH
        });
      ...
      for (let e = 0; e <= d; e++) for (let a = 0; a <= r; a++) {
        const c = X.get(`${e},${a}`)[0];
        (Math.round(t.apoyo) | 0) === 0 ? q.set(c, [
          true,
          true,
          true,
          true,
