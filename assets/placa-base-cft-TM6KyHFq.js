...
import { a as Oo, __tla as __tla_0 } from "./analyze-DW0aWqsq.js";
import { d as qo, __tla as __tla_1 } from "./didacticCpp-BoYi16rL.js";
import { g as Go, l as Ye, m as De, c as Yo, __tla as __tla_2 } from "./aiAgent-D29HUSaa.js";
import { e as Do } from "./makeDraggable-zx2br6Yh.js";
import { g as $o, __tla as __tla_3 } from "./getParameters-8h1v8PIV.js";
...
Promise.all([
  ...
  c.derive(() => {
    ...
    for (let t = 0; t <= P; t++) {
      ...
      for (let o = 0; o <= B; o++) {
        const n = [];
        for (let a = 0; a <= g; a++) n.push(F(-ut / 2 + a * Lt, -ft / 2 + o * Me, -Pt + t * ro));
        e.push(n);
      }
      r.push(e);
    }
    ...
    const go = 0.65, dt = s * h, ke = ut * ft, Bo = Math.min(2, Math.sqrt(ke / dt)), Po = Math.min(0.85 * V * dt * Bo, 1.7 * V * dt), ze = go * Po, wo = X / Math.max(1, ze), Fe = Math.max(0, (s - 0.95 * Math.max(p, m)) / 2), Ao = X / dt, je = Fe * Math.sqrt(2 * Math.max(0, Ao) / (0.9 * Jo)), To = je / Math.max(1e-6, L), Co = Math.max(0.05, s - 2 * Bt), No = Math.sqrt(wt * wt + At * At), Ie = Math.max(0, No / Co - X / 2) / Math.max(1, gt), Se = 0.75 * (0.75 * Math.PI * z * z / 4) * Uo, ko = Ie / Math.max(1, Se), zo = p * m, $t = (p - 2 * b) * (m - 2 * b), Ee = zo - $t, Le = 35e4 * Ee + 0.85 * V * $t, Fo = 0.75 * Le, jo = X / Math.max(1, Fo);
    Ke.val = {
      vmMax: Dt,
      A1: dt,
      A2: ke,
