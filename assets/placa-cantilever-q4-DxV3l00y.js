...
import { a as C } from "./analyze-CWJH9Nzr.js";
import { d as N, __tla as __tla_0 } from "./didacticCpp-iMwzdM-v.js";
import { g as q, a as G, __tla as __tla_1 } from "./aiAgent-BbDbzZHq.js";
import { g as Q } from "./getParameters-Dgz5Nzxm.js";
...
Promise.all([
  ...
  a.derive(() => {
    const y = l.Lx.value.val, m = l.Ly.value.val, c = l.t.value.val, s = Math.round(l.nx.value.val), n = Math.round(l.ny.value.val), u = l.E.value.val, i = l.nu.value.val, v = l.P.value.val, r = u / (2 * (1 + i)), I = y / s, P = m / n, d = [], o = [];
    ...
    const h = [];
    for (let t = 0; t <= n; t++) h.push(t * p + s);
    ...
    try {
      ...
      const t = (n / 2 | 0) * p + s, e = x.deformations.get(t), k = e ? e[1] : 0;
  ...
  setTimeout(() => {
    ...
    ].find((r) => r.__ctx)) == null ? void 0 : _a.__ctx, c = M.val;
    if (!(m == null ? void 0 : m.camera) || !(c == null ? void 0 : c.length)) return;
    const s = c.map((r) => r[0]), n = c.map((r) => r[2]), u = (Math.min(...s) + Math.max(...s)) / 2, i = (Math.min(...n) + Math.max(...n)) / 2, v = Math.max(Math.max(...s) - Math.min(...s), Math.max(...n) - Math.min(...n), 1);
