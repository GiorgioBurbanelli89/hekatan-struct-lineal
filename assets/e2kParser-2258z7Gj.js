import { d as es, b as os, a as ns, c as as, t as cs, i as is } from "./cadSections-Cc6U2MoW.js";
...
function fs(h, A = wt) {
  return h.tipo === "tube" ? bt({ tipo: "rect", d: h.d, b: h.b }) : h.tipo === "pipe" ? bt({ tipo: "circle", d: h.d }, A) : bt(h, A);
}
function ps(h) {
  let A = 0, i = 0, r = 0, p = 0, S = 0, y = 0;
  for (let P = 0; P < h.length; P++) {
    const [L, w] = h[P], [T, G] = h[(P + 1) % h.length], k = L * G - T * w;
    A += k, i += (L + T) * k, r += (w + G) * k, p += (w * w + w * G + G * G) * k, S += (L * L + L * T + T * T) * k, y += (L * G + 2 * L * w + 2 * T * G + T * w) * k;
  }
  if (A /= 2, Math.abs(A) < 1e-18) return { A: 0, cx: 0, cy: 0, Ixx: 0, Iyy: 0, Ixy: 0 };
  const F = i / (6 * A), N = r / (6 * A);
  return { A, cx: F, cy: N, Ixx: p / 12 - A * N * N, Iyy: S / 12 - A * F * F, Ixy: y / 24 - A * F * N };
}
function ds(h, A) {
  const i = rs(A.rot ?? 0), r = Math.cos(i), p = Math.sin(i), S = A.mirror ? -1 : 1, y = h.map(([F, N]) => {
    const P = F * S;
    return [P * r - N * p + (A.xc ?? 0), P * p + N * r + (A.yc ?? 0)];
  });
  return A.mirror ? y.reverse() : y;
}
function Es(h, A) {
  let i = 0, r = 0, p = 0;
  const S = [];
  for (const T of h) {
    const G = A > 0 && T.E ? T.E / A : 1;
...
function hs(h, A, i, r, p) {
  switch ((h || "").toUpperCase()) {
    case "CONCRETE RECTANGULAR":
    case "SOLID RECT":
    case "RECTANGLE":
      return { tipo: "rect", d: A, b: i };
    case "CONCRETE CIRCLE":
    case "SOLID CIRCLE":
    case "CIRCLE":
      return { tipo: "circle", d: A };
    case "STEEL ANGLE":
    case "ANGLE":
      return { tipo: "angle", d: A, b: i, tf: r, tw: p };
    case "STEEL CHANNEL":
    case "CHANNEL":
      return { tipo: "channel", d: A, b: i, tf: r, tw: p };
    case "STEEL TEE":
    case "CONCRETE TEE":
    case "TEE":
      return { tipo: "tee", d: A, b: i, tf: r, tw: p };
    case "STEEL I/WIDE FLANGE":
    case "I SECTION":
    case "ISECTION":
      return { tipo: "isection", d: A, b: i, tf: r, tw: p };
    case "STEEL TUBE":
    case "TUBE":
      return { tipo: "tube", d: A, b: i, tf: r, tw: p };
    case "STEEL PIPE":
    case "PIPE":
      return { tipo: "pipe", d: A, t: r || p };
    default:
      return A > 0 && i > 0 ? { tipo: "rect", d: A, b: i } : null;
  }
}
function Is(h) {
  ...
  for (const n of A) {
    ...
    if (x === "FRAME SECTIONS") {
      ...
      if (t) {
        ...
        const g = s.match(/FILLMATERIAL\s+"([^"]+)"/);
  ...
  for (const n of F) for (const [s, t] of G) {
    ...
    for (let l = 0; l < c.length - 1; l++) {
      ...
      if (K.push([d, m]), st.push(c.length > 2 ? `${n.name}-${l + 1}` : n.name), Tt.push(n.type), it.push(o), dt.set(I, t.section), t.spring && Mt.set(I, t.spring), t.mallaEnCruces && xt.set(I, true), t.rigidZone > 0 && Et.set(I, [t.rigidZone, t.rigidZone]), t.angle && Gt.set(I, t.angle), t.offsets && rt.set(I, [t.offsets[0], t.offsets[1], t.rigidZone]), t.releases.length > 0) {
        const g = new Array(12).fill(false);
        for (const R of t.releases) {
          const O = f[R];
          O !== void 0 && (O < 6 && l !== 0 || O >= 6 && l !== c.length - 2 || (g[O] = true));
        }
        g.some(Boolean) && kt.set(I, g);
      }
    }
  }
  ...
  for (const [n, s] of dt) {
    ...
    let l = 0, d = 0, m = 0, I = 0, g = 0, R = 0, O = "rect", D = 0, W = false, $ = false;
    const U = w.get(s);
    if (t.shape === "SD Section" && (U == null ? void 0 : U.length)) {
      ...
      if (q.length) {
        const v = Es(q, E);
        v.A > 0 && (l = v.A, d = v.Iz, m = v.Iy, I = v.J, g = v.As2, R = v.As3, O = "rect", $ = true, $t++);
      }
    }
    if (!$) switch (t.shape) {
      case "Concrete Rectangular":
        l = e * a, d = a * e ** 3 / 12, m = e * a ** 3 / 12, I = a * e ** 3 * (1 / 3 - 0.21 * (e / a) * (1 - e ** 4 / (12 * a ** 4))), g = R = 5 / 6 * l, O = "rect";
        break;
      case "Concrete Circle":
        l = Math.PI * e ** 2 / 4, d = m = Math.PI * e ** 4 / 64, I = Math.PI * e ** 4 / 32, g = R = 0.9 * l, O = "circ";
        break;
      case "Steel I/Wide Flange": {
        const E = is(e, a, c, f);
        l = E.A, d = E.Iz, m = E.Iy, I = E.J, R = E.As2, g = E.As3, O = "I";
        break;
      }
      case "Steel Tube": {
        const E = cs(a, e, c || f, f || c);
        l = E.A, d = E.Iz, m = E.Iy, I = E.J, R = E.As2, g = E.As3, O = "HSS";
        break;
      }
      case "Filled Steel Pipe": {
        ...
        l = V.A, d = V.Iz, m = V.Iy, I = V.J, R = V.As2, g = V.As3, D = v, W = true, O = "CFT";
        break;
      }
      case "Filled Steel Tube": {
        ...
        l = V.A, d = V.Iz, m = V.Iy, I = V.J, R = V.As2, g = V.As3, D = v, O = "CFT";
        break;
      }
      case "Steel Angle": {
        const E = c || f;
        l = E * (e + a - E), d = E * (e ** 3 + a * E ** 2 + E ** 2 * (e - E)) / 12, m = E * (a ** 3 + e * E ** 2 + E ** 2 * (a - E)) / 12, I = (e + a - E) * E ** 3 / 3, g = e * E, R = a * E, O = "L";
        break;
      }
      case "Steel Channel": {
        const E = as(e, a, c, f);
        l = E.A, d = E.Iz, m = E.Iy, I = E.J, R = E.As2, g = E.As3, O = "C";
        break;
      }
      case "Steel Double Angle": {
        const E = es(e, a, c, f, t.DIS || 0);
        l = E.A, d = E.Iz, m = E.Iy, I = E.J, R = E.As2, g = E.As3, O = "2L";
        break;
      }
      case "Cold Formed C":
        l = 2 * a * c + (e - 2 * c) * f, d = (f * e ** 3 + 2 * a * c * (e - c) ** 2) / 12, m = (2 * c * a ** 3 + (e - 2 * c) * f ** 3) / 12, I = (2 * a * c ** 3 + (e - 2 * c) * f ** 3) / 3, g = (e - 2 * c) * f, R = 2 * a * c * 5 / 6, O = t.shape === "Cold Formed C" ? "coldC" : "C";
        break;
      case "Steel Double Channel":
        l = 2 * (2 * a * c + (e - 2 * c) * f), d = 2 * (f * e ** 3 + 2 * a * c * (e - c) ** 2) / 12, m = 2 * (2 * c * a ** 3 + (e - 2 * c) * f ** 3) / 12, I = 2 * (2 * a * c ** 3 + (e - 2 * c) * f ** 3) / 3, g = 2 * (e - 2 * c) * f, R = 4 * a * c * 5 / 6, O = "2C";
        break;
      case "General":
        if (t.AREA && t.AREA > 0) {
          l = t.AREA, d = t.I33 ?? 0, m = t.I22 ?? 0, I = t.TORSION ?? 0, R = t.AS2 ?? 0, g = t.AS3 ?? 0, O = "general";
          break;
        }
      default:
        e > 0 && a > 0 && (l = e * a, d = a * e ** 3 / 12, m = e * a ** 3 / 12, I = Math.min(e, a) * Math.max(e, a) ** 3 / 3 * 0.3, g = R = 5 / 6 * l);
        break;
    }
    t.modI2 && (m *= t.modI2), t.modI3 && (d *= t.modI3), ut.set(n, l), Ot.set(n, d), yt.set(n, m), Lt.set(n, I), g > 0 && Nt.set(n, g), R > 0 && Rt.set(n, R), It.set(n, { type: O, ...D > 0 ? { fillE: D } : {}, b: W ? void 0 : a || void 0, h: W ? void 0 : e || void 0, d: O === "circ" || O === "pipe" || W ? e : void 0, tw: f || void 0, tf: c || void 0, r: t.R, name: s });
  }
  ...
  for (const n of C) {
    ...
    const l = [(a[0] - e[0]) / c, (a[1] - e[1]) / c, (a[2] - e[2]) / c], d = c * c / 12, m = [l[1] * f[2] - l[2] * f[1], l[2] * f[0] - l[0] * f[2], l[0] * f[1] - l[1] * f[0]], I = (g, R) => {
      const O = X.get(g) || [0, 0, 0, 0, 0, 0];
  ...
  const gt = /* @__PURE__ */ new Map(), Ut = /* @__PURE__ */ new Map(), Yt = /* @__PURE__ */ new Map(), zt = /* @__PURE__ */ new Map(), Ft = [], B = { sinAssign: 0, sinNudo: 0, colapsada: 0, poligono: 0 };
  for (const n of N) {
    const s = P.get(n.name);
    if (!s) {
      B.sinAssign++;
    ...
    for (const c of a) {
      ...
      if (l) {
        ...
        if ((d == null ? void 0 : d.E) && ht.set(f, d.E), (d == null ? void 0 : d.G) && mt.set(f, d.G), (d == null ? void 0 : d.nu) !== void 0 && Ut.set(f, d.nu), (d == null ? void 0 : d.density) && at.set(f, d.density), l.deck && l.deck.tc > 0) {
          const g = l.deck, R = g.tc + (g.sr > 0 ? g.hr * (g.wrt + g.wrb) / 2 / g.sr : 0);
          at.set(f, (((d == null ? void 0 : d.density) ?? 0) * R + g.w) / g.tc), Ct.set(f, { ...g });
        }
  ...
  for (const n of b) {
    ...
    for (const e of t) {
      ...
      let g = 0;
      for (const $ of I) g += $.val;
      if (!g) {
        o || Vt++, o = true;
        break;
      }
      o || Jt++, o = true;
      const R = g * m / c.length;
      Ht += g * m;
      let O = 0, D = 0, W = 0;
  ...
  const ts = () => {
    ...
    return K.forEach((t, o) => {
      ...
      const a = t;
      if (a.length === 2) {
        const c = ut.get(o) ?? 0, f = H[a[0]], l = H[a[1]], d = rt.get(o), m = l[0] - f[0], I = l[1] - f[1], g = l[2] - f[2], R = Math.hypot(m, I), O = R > 1e-9 && Math.atan2(Math.abs(g), R) * 180 / Math.PI < 20, D = Math.max(0, Math.hypot(m, I, g) - (d && O ? d[0] + d[1] : 0)), W = e * c * D * Q;
        s(a[0], -W / 2), s(a[1], -W / 2);
      } else if (a.length >= 3) {
        const c = gt.get(o) ?? 0, f = a.map((R) => H[R]);
        let l = 0, d = 0, m = 0;
        for (let R = 0; R < f.length; R++) {
          const O = f[R], D = f[(R + 1) % f.length];
          l += O[1] * D[2] - O[2] * D[1], d += O[2] * D[0] - O[0] * D[2], m += O[0] * D[1] - O[1] * D[0];
        }
        const I = Math.hypot(l, d, m) / 2, g = e * c * I * Q;
        for (const R of a) s(R, -g / a.length);
      }
  ...
  return { units: i, stories: r.reverse(), materials: p, frameSections: S, nodes: H, nodeNames: Dt, nodeNameToIdx: nt, elements: K, elementNames: st, elementTypes: Tt, elementStories: it, elementSections: dt, nodeInputs: { supports: At, loads: ts(), springNames: St, springs: ss() }, elementInputs: { elasticities: ht, shearModuli: mt, areas: ut, momentsOfInertiaZ: Ot, momentsOfInertiaY: yt, torsionalConstants: Lt, shearAreasY: Nt, shearAreasZ: Rt, rigidOffsets: Et, momentReleases: kt, localAngles: Gt, endOffsets: rt, densities: new Map([...at].map(([n, s]) => [n, s / 9.80665])), deckSections: Ct, sectionShapes: It, thicknesses: gt, poissonsRatios: Ut, plateFormulations: Yt, shellModifiers: zt, springNames: Mt, mallaEnCruces: xt }, sectionShapes: It, grids: et, planosRef: ot, springProps: k, info: { ...Kt(K, At), nNodes: H.length, nFrames: K.length - Ft.length, nAreas: N.length, nAreasMontadas: Ft.length, nSDCompuestas: $t, nSDLeidas: w.size, title: tt }, rawSections: ft };
}
function ms(h, A) {
  const i = h.length;
  if (i < 3) return [];
  const r = [0, 0, 0];
  for (let M = 0; M < i; M++) {
    const C = A[h[M]], b = A[h[(M + 1) % i]];
    r[0] += (C[1] - b[1]) * (C[2] + b[2]), r[1] += (C[2] - b[2]) * (C[0] + b[0]), r[2] += (C[0] - b[0]) * (C[1] + b[1]);
  }
  const p = [Math.abs(r[0]), Math.abs(r[1]), Math.abs(r[2])], S = p[2] >= p[0] && p[2] >= p[1] ? 2 : p[1] >= p[0] ? 1 : 0, [y, F] = S === 2 ? [0, 1] : S === 1 ? [2, 0] : [1, 2], N = h.map((M) => [A[M][y], A[M][F]]);
  let P = 0;
...
function Kt(h, A) {
  ...
  const p = new Set([...A ?? /* @__PURE__ */ new Map()].map(([N]) => N)), S = /* @__PURE__ */ new Set();
  let y = 0, F = 0;
  for (const N of r) {
    if (S.has(N)) continue;
    const P = [N], L = [];
