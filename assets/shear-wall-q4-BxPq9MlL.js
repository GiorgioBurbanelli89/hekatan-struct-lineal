...
import { a as U } from "./analyze-C-HJ03ae.js";
import { d as j, __tla as __tla_0 } from "./didacticCpp-iMwzdM-v.js";
import { g as Q, a as q, __tla as __tla_1 } from "./aiAgent-BRmWiiEZ.js";
import { g as V } from "./getParameters-cA6pUV9s.js";
...
Promise.all([
  ...
  a.derive(() => {
    const n = s.W.value.val, d = s.H.value.val, v = s.t.value.val, l = Math.round(s.nx.value.val), r = Math.round(s.ny.value.val), E = s.E.value.val, S = s.nu.value.val, k = s.P.value.val, H = E / (2 * (1 + S)), C = n / l, F = d / r, i = [], o = [];
    ...
    const p = l + 1;
    for (let t = 0; t < r; t++) for (let e = 0; e < l; e++) o.push([
      t * p + e,
      t * p + e + 1,
      (t + 1) * p + e + 1,
      (t + 1) * p + e
    ...
    let x = {}, P = {};
    try {
      x = j(i, o, O, h), P = U(i, o, h, x);
      const t = r * p + Math.floor(l / 2), e = x.deformations.get(t), w = e ? e[0] : 0, m = Math.abs(w), L = Math.abs(m - c) / c * 100, W = Math.abs(m - u) / u * 100, G = Math.abs(m - g) / g * 100;
    ...
    $.val = i, T.val = o, _.val = O, B.val = h, R.val = x, z.val = P;
  });
