...
function cn(t, n, c, e, r, i) {
  const s = e * i * i * i / (12 * (1 - r * r)), a = [[s, s * r, 0], [s * r, s, 0], [0, 0, s * (1 - r) / 2]], y = (g, h) => {
    const f = [-0.25 * (1 - h), 0.25 * (1 - h), 0.25 * (1 + h), -0.25 * (1 + h)], m = [-0.25 * (1 - g), -0.25 * (1 + g), 0.25 * (1 + g), 0.25 * (1 - g)];
    let _ = 0, u = 0, S = 0, B = 0;
    for (let q = 0; q < 4; q++) _ += f[q] * t[q], u += f[q] * n[q], S += m[q] * t[q], B += m[q] * n[q];
    const N = _ * B - u * S, p = B / N, k = -u / N, L = -S / N, O = _ / N;
    let j = 0, z = 0, Y = 0;
    for (let q = 0; q < 4; q++) {
  ...
  return d.map(([g, h]) => {
    const f = g * Math.sqrt(3), m = h * Math.sqrt(3), _ = d.map(([u, S]) => (1 + u * f) * (1 + S * m) / 4);
    return [0, 1, 2].map((u) => _.reduce((S, B, N) => S + B * X[N][u], 0));
  });
}
function en(t, n) {
  ...
  for (let a = 0; a < 4; a++) {
    ...
    c[a] = 0.25 * (1 + y) * (1 + d) * (y + d - 1), e[a] = 0.25 * i[a] * (1 + d) * (2 * y + d), r[a] = 0.25 * s[a] * (1 + y) * (y + 2 * d);
  }
...
function an(t, n, c, e) {
  ...
  for (let Y = 0; Y < 4; Y++) {
    ...
    r.push(-w / Q), i.push(0.75 * w * M / Q), s.push((0.25 * w * w - 0.5 * M * M) / Q), a.push(-M / Q), y.push((0.25 * M * M - 0.5 * w * w) / Q);
  }
  ...
  const u = h * _ - f * m, S = _ / u, B = -f / u, N = -m / u, p = h / u, k = new Array(12).fill(0), L = new Array(12).fill(0), O = new Array(12).fill(0), j = new Array(12).fill(0);
  for (let Y = 0; Y < 4; Y++) {
    ...
    const I = 1.5 * (a[q] * d[M] - a[P] * d[w]), nt = 1.5 * (a[q] * o[M] - a[P] * o[w]), l = -d[Y] + y[q] * d[M] + y[P] * d[w], b = -o[Y] + y[q] * o[M] + y[P] * o[w];
...
function fn(t, n, c, e, r, i, s = "esquinas") {
  const a = e * i * i * i / (12 * (1 - r * r)), y = [[a, a * r, 0], [a * r, a, 0], [0, 0, a * (1 - r) / 2]], d = (h, f) => {
    const m = an(t, n, h, f), _ = [0, 1, 2].map((u) => m[u].reduce((S, B, N) => S + B * c[N], 0));
    return [0, 1, 2].map((u) => y[u][0] * _[0] + y[u][1] * _[1] + y[u][2] * _[2]);
  }, o = [[-1, -1], [1, -1], [1, 1], [-1, 1]];
  if (s === "esquinas") return o.map(([h, f]) => d(h, f));
  const X = 1 / Math.sqrt(3), g = o.map(([h, f]) => d(h * X, f * X));
  return o.map(([h, f]) => {
    const m = h * Math.sqrt(3), _ = f * Math.sqrt(3), u = o.map(([S, B]) => (1 + S * m) * (1 + B * _) / 4);
    return [0, 1, 2].map((S) => u.reduce((B, N, p) => B + N * g[p][S], 0));
  });
}
function ln(t, n) {
...
function gn(t, n, c, e) {
  ...
  const d = 1 / y;
  return { det: y, Ji: [[a * d, -i * d], [-s * d, r * d]] };
}
function hn(t, n, c, e, r, i, s = {}) {
  const a = s.tipo ?? 8, y = s.gammaFac ?? 0.4, d = s.mod ?? null;
  let o, X, g;
  ...
  const m = [1, 2, 3, 0], _ = [3, 0, 1, 2], u = [], S = [];
  for (let l = 0; l < 4; l++) u.push((n[m[l]] - n[l]) / 8), S.push(-(t[m[l]] - t[l]) / 8);
  ...
  const j = (l, b) => {
    ...
    for (let G = 0; G < 4; G++) {
      const mt = _[G];
      wt.push(tt[mt] * u[mt] - tt[G] * u[G]), bt.push(ft[mt] * u[mt] - ft[G] * u[G]), dt.push(tt[mt] * S[mt] - tt[G] * S[G]), gt.push(ft[mt] * S[mt] - ft[G] * S[G]);
    }
  ...
  {
    const l = j(0, 0), b = e / (2 * (1 + r)), T = y * b, J = new Array(14).fill(0);
...
function Wt(t, n, c, e) {
  ...
  const y = r * a - i * s, d = 1 / y, o = [], X = [];
  for (let g = 0; g < 4; g++) o.push(d * (a * t[g] - i * n[g])), X.push(d * (-s * t[g] + r * n[g]));
  return { dNdx: o, dNdy: X, detJ: y, J: [r, i, s, a] };
}
function yn(t, n, c, e, r, i) {
  const s = c * r / (1 - e * e), a = [[s, s * e, 0], [s * e, s, 0], [0, 0, s * (1 - e) / 2]], y = [1, 2, 3, 0], d = [3, 0, 1, 2], o = [], X = [];
  for (let w = 0; w < 4; w++) o.push((n[y[w]] - n[w]) / 8), X.push(-(t[y[w]] - t[w]) / 8);
  const g = [-Math.sqrt(3 / 5), 0, Math.sqrt(3 / 5)], h = [5 / 9, 8 / 9, 5 / 9], f = Dt(14, 14);
  let m = [], _ = [], u = [], S = [], B = [], N = 0, p = 0, k = 0;
  for (let w = 0; w < 3; w++) for (let M = 0; M < 3; M++) {
    ...
    const yt = -2 * Q * (1 - C * C), ut = -2 * C * (1 - Q * Q), Yt = J * yt + v * ut, pt = A * yt + K * ut, wt = [], bt = [], dt = [], gt = [];
    for (let R = 0; R < 4; R++) {
      const lt = d[R];
    ...
    w === 1 && M === 1 && (m = x.slice(), _ = st.slice(), u = $.slice(), S = bt.slice(), B = dt.slice(), N = Yt, p = pt, k = Math.abs(T));
  }
  const L = c / (2 * (1 + e)), O = new Array(14).fill(0);
  for (let w = 0; w < 4; w++) O[3 * w] = -0.5 * u[w], O[3 * w + 1] = 0.5 * _[w], O[3 * w + 2] = 0.5 * (B[w] - S[w]) - m[w];
...
function un(t, n, c, e, r) {
  const i = Dt(12, 12), s = c * r * r * r / (12 * (1 - e * e)), y = 5 / 6 * c / (2 * (1 + e)) * r, d = [[-zt, -zt], [zt, -zt], [zt, zt], [-zt, zt]], o = [{ xi: 0, eta: -1 }, { xi: 0, eta: 1 }, { xi: -1, eta: 0 }, { xi: 1, eta: 0 }], X = [];
  for (const g of o) {
    const { N: h, dNdxi: f, dNdeta: m } = $t(g.xi, g.eta), { dNdx: _, dNdy: u, J: S } = Wt(f, m, t, n), B = Dt(2, 12);
    for (let j = 0; j < 4; j++) B[0][j * 3] = _[j], B[0][j * 3 + 1] = -h[j], B[1][j * 3] = u[j], B[1][j * 3 + 2] = -h[j];
    const [N, p, k, L] = S, O = Dt(2, 12);
    for (let j = 0; j < 12; j++) O[0][j] = N * B[0][j] + p * B[1][j], O[1][j] = k * B[0][j] + L * B[1][j];
    X.push(O);
  }
  for (const [g, h] of d) {
    const { dNdxi: f, dNdeta: m } = $t(g, h), { dNdx: _, dNdy: u, detJ: S, J: B } = Wt(f, m, t, n), N = Dt(3, 12);
    for (let M = 0; M < 4; M++) N[0][M * 3 + 1] = _[M], N[1][M * 3 + 2] = u[M], N[2][M * 3 + 1] = u[M], N[2][M * 3 + 2] = _[M];
    ...
    for (let M = 0; M < 12; M++) for (let Q = 0; Q < 12; Q++) i[M][Q] += y * (p[0][M] * p[0][Q] + p[1][M] * p[1][Q]) * Math.abs(S);
  }
  return i;
}
function pn(t, n, c) {
  ...
  const { localCoords: s } = on(t), a = s.map((u) => u[0]), y = s.map((u) => u[1]), d = un(a, y, e, r, i), X = yn(a, y, e, r, i, 0.4), g = Dt(24, 24), h = [2, 3, 4, 8, 9, 10, 14, 15, 16, 20, 21, 22], f = [[1, 0, 0], [0, 0, -1], [0, 1, 0]], m = Dt(12, 12);
  for (let u = 0; u < 12; u++) for (let S = 0; S < 12; S++) {
    let B = 0;
    const N = u / 3 | 0, p = u % 3, k = S / 3 | 0, L = S % 3;
    for (let O = 0; O < 3; O++) {
      const j = f[O][p];
  ...
  for (let u = 0; u < 12; u++) for (let S = 0; S < 12; S++) g[h[u]][h[S]] += m[u][S];
  const _ = [0, 1, 5, 6, 7, 11, 12, 13, 17, 18, 19, 23];
  for (let u = 0; u < 12; u++) for (let S = 0; S < 12; S++) g[_[u]][_[S]] += X[u][S];
  return g;
}
function Mn(t) {
  ...
  for (let s = 0; s < 4; s++) for (let a = 0; a < 2; a++) {
    const y = s * 6 + a * 3;
    for (let d = 0; d < 3; d++) for (let o = 0; o < 3; o++) i[y + d][y + o] = r[d][o];
  }
  return i;
}
function on(t) {
  const n = [t[2][0] - t[0][0], t[2][1] - t[0][1], t[2][2] - t[0][2]], c = [t[3][0] - t[1][0], t[3][1] - t[1][1], t[3][2] - t[1][2]], e = Ut(n, c), r = Math.sqrt(e[0] ** 2 + e[1] ** 2 + e[2] ** 2), i = e.map((f) => f / r), s = [t[1][0] - t[0][0], t[1][1] - t[0][1], t[1][2] - t[0][2]], a = Math.sqrt(s[0] ** 2 + s[1] ** 2 + s[2] ** 2), y = s.map((f) => f / a), d = Ut(i, y), o = t.map((f) => f[0]).reduce((f, m) => f + m) / 4, X = t.map((f) => f[1]).reduce((f, m) => f + m) / 4, g = t.map((f) => f[2]).reduce((f, m) => f + m) / 4, h = t.map((f) => {
    const m = f[0] - o, _ = f[1] - X, u = f[2] - g;
    return [m * y[0] + _ * y[1] + u * y[2], m * d[0] + _ * d[1] + u * d[2]];
  });
...
function mn(t, n = 0) {
  ...
  }, e = nn(t[1], t[0]), r = Ct(e), i = Pt(e, [1, 0, 0]) / r, s = Pt(e, [0, 1, 0]) / r, a = Pt(e, [0, 0, 1]) / r, y = Math.sqrt(i ** 2 + s ** 2);
  if (y < 1e-9) {
...
function bn(t) {
  ...
  for (let p = 0; p < 3; p++) for (let k = 0; k < 3; k++) d[p][0] += s[p][k] * a[k], d[p][1] += s[p][k] * y[k];
  ...
  if (Math.abs(_) > 1 - 1e-10) {
    const p = g[2];
    u = m.map((k, L) => k[2] - p * g[L]);
  } else u = m.map((p, k) => p[0] - _ * g[k]);
  ...
  S = S.map((p) => p / h);
  const B = [u, S, f], N = at(18, 18).toArray();
  for (let p = 0; p < 3; p++) {
    const k = p * 6, L = k + 3;
...
function wn(t, n) {
  ...
  const i = r.length, s = c.length, a = Array.from({ length: s }, (f, m) => Array.from({ length: s }, (_, u) => t[c[m]][c[u]])), y = Array.from({ length: i }, (f, m) => Array.from({ length: s }, (_, u) => t[r[m]][c[u]])), d = Array.from({ length: s }, (f, m) => Array.from({ length: i }, (_, u) => t[c[m]][r[u]])), o = Yn(a);
  if (!o) return t;
...
function tn(t, n) {
  ...
  for (let s = 0; s < c; s++) for (let a = 0; a < e; a++) for (let y = 0; y < r; y++) i[s][a] += t[s][y] * n[y][a];
  return i;
}
function Yn(t) {
  ...
  for (let e = 0; e < n; e++) {
    ...
    for (let s = 0; s < n; s++) {
      if (s === e) continue;
      const a = c[s][e];
      for (let y = 0; y < 2 * n; y++) c[s][y] -= a * c[e][y];
    }
  }
...
function Nn(t, n, c) {
  ...
  for (let i = 0; i < 12; i++) for (let s = 0; s < 12; s++) {
    let a = 0;
    for (let y = 0; y < 12; y++) a += e[i][y] * c[y][s];
    r[i][s] = a;
  }
  return r;
}
function Sn(t, n, c) {
  ...
  const e = ((_a = n == null ? void 0 : n.momentsOfInertiaZ) == null ? void 0 : _a.get(c)) ?? 0, r = ((_b = n == null ? void 0 : n.momentsOfInertiaY) == null ? void 0 : _b.get(c)) ?? 0, i = ((_c = n == null ? void 0 : n.elasticities) == null ? void 0 : _c.get(c)) ?? 0, s = ((_d = n == null ? void 0 : n.areas) == null ? void 0 : _d.get(c)) ?? 0, a = ((_e = n == null ? void 0 : n.shearModuli) == null ? void 0 : _e.get(c)) ?? 0, y = ((_f = n == null ? void 0 : n.torsionalConstants) == null ? void 0 : _f.get(c)) ?? 0, d = Ct(nn(t[0], t[1]));
  ...
  const f = h > 0 && a > 0 ? 12 * i * e / (a * h * X ** 2) : 0, m = g > 0 && a > 0 ? 12 * i * r / (a * g * X ** 2) : 0, _ = i * s / d, u = a * y / d, S = 12 * i * e / X ** 3 / (1 + f), B = 6 * i * e / X ** 2 / (1 + f), N = 4 * i * e / X * (1 + f / 4) / (1 + f), p = 2 * i * e / X * (1 - f / 2) / (1 + f), k = 12 * i * r / X ** 3 / (1 + m), L = 6 * i * r / X ** 2 / (1 + m), O = 4 * i * r / X * (1 + m / 4) / (1 + m), j = 2 * i * r / X * (1 - m / 2) / (1 + m);
  return [[_, 0, 0, 0, 0, 0, -_, 0, 0, 0, 0, 0], [0, S, 0, 0, 0, B, 0, -S, 0, 0, 0, B], [0, 0, k, 0, -L, 0, 0, 0, -k, 0, -L, 0], [0, 0, 0, u, 0, 0, 0, 0, 0, -u, 0, 0], [0, 0, -L, 0, O, 0, 0, 0, L, 0, j, 0], [0, B, 0, 0, 0, N, 0, -B, 0, 0, 0, p], [-_, 0, 0, 0, 0, 0, _, 0, 0, 0, 0, 0], [0, -S, 0, 0, 0, -B, 0, S, 0, 0, 0, -B], [0, 0, -k, 0, L, 0, 0, 0, k, 0, L, 0], [0, 0, 0, -u, 0, 0, 0, 0, 0, u, 0, 0], [0, 0, -L, 0, j, 0, 0, 0, L, 0, O, 0], [0, B, 0, 0, 0, p, 0, -B, 0, 0, 0, N]];
}
function jn(t, n, c) {
  var _a, _b, _c, _d, _e;
  const e = ((_a = n.elasticities) == null ? void 0 : _a.get(c)) ?? 0, r = ((_b = n.elasticitiesOrthogonal) == null ? void 0 : _b.get(c)) ?? 0, i = ((_c = n.poissonsRatios) == null ? void 0 : _c.get(c)) ?? 0, s = ((_d = n.shearModuli) == null ? void 0 : _d.get(c)) ?? 0, a = ((_e = n.thicknesses) == null ? void 0 : _e.get(c)) ?? 0, y = r > 0, d = y ? P(e, r, s, i, a) : z(e, i, a), o = y ? q(s, a) : Y(e, i, a), X = y ? rn(e, r, s, i) : sn(e, i), g = t.map(([D, x]) => [D, x]), h = g[1][0] - g[0][0], f = g[2][0] - g[0][0], m = g[0][1] - g[1][1], _ = g[2][1] - g[0][1], u = 0.5 * (h * _ - f * -m), S = w(g), B = Q(g), N = C(g, X, a), p = ct(ct(Ot(S), o), S), k = ct(ct(Ot(B), d), B), L = at(18, 18).toArray(), O = ct(Kt(p, k), u), j = [[0, 1, 5], [6, 7, 11], [12, 13, 17]];
  ...
  function w(D) {
    const x = at(2, 18).toArray(), [U, E] = D[0], [I, nt] = D[1], [l, b] = D[2], T = 0.5 * ((I - U) * (b - E) - (l - U) * -(E - nt)), J = (U + I + l) / 3, v = (E + nt + b) / 3, A = [J, U, I], K = [v, E, nt], st = [J, I, l], $ = [v, nt, b], et = [J, l, U], Mt = [v, b, E], tt = 1 / 3, [ft, yt, ut, Yt] = M(A, K), [pt, wt, bt, dt] = M(st, $), [gt, G, mt, R] = M(et, Mt), lt = at(2, 18).toArray(), Xt = at(2, 18).toArray(), At = at(2, 18).toArray();
  ...
  function Q(D) {
    const x = at(3, 18).toArray(), [U, E] = D[0], [I, nt] = D[1], [l, b] = D[2], T = I - U, J = l - U, v = l - I, A = nt - b, K = b - E, st = E - nt, $ = 0.5 * (T * K - J * -st), et = A / (2 * $), Mt = v / (2 * $), tt = K / (2 * $), ft = -J / (2 * $), yt = st / (2 * $), ut = T / (2 * $);
    return x[0][4] = et, x[0][10] = tt, x[0][16] = yt, x[1][3] = -Mt, x[1][9] = -ft, x[1][15] = -ut, x[2][3] = -et, x[2][4] = Mt, x[2][9] = -tt, x[2][10] = ft, x[2][15] = -yt, x[2][16] = ut, x;
  }
  function C(D, x, U) {
    ...
    const Mt = 1 / 8, tt = Mt / 6, ft = Mt ** 2 / 4, yt = 1, ut = 2, Yt = 1, pt = 0, wt = 1, bt = -1, dt = -1, gt = -1, G = -2, mt = D[0][0], R = D[0][1], lt = D[1][0], Xt = D[1][1], At = D[2][0], H = D[2][1], V = mt - lt, Ft = lt - At, Bt = At - mt, qt = R - Xt, Qt = Xt - H, Tt = H - R, xt = -V, jt = -Ft, _t = -Bt, vt = -qt, Jt = -Qt, kt = -Tt, F = 0.5 * (xt * Tt - Bt * -qt), ot = 2 * F, W = 4 * F, Z = 0.5 * U, Lt = F * U, Nt = xt ** 2 + vt ** 2, ht = jt ** 2 + Jt ** 2, St = _t ** 2 + kt ** 2;
...
function rn(t, n, c, e) {
  const r = n * e / t, i = 1 - e * r, s = t / i, a = n / i, y = e * n / i;
  return rt([[s, y, 0], [y, a, 0], [0, 0, c]]);
}
function Qn(t, n, c, e) {
  ...
  n.forEach((d, o) => {
    ...
    if (d.length === 2) {
      ...
      let _ = ct(m, f);
      const u = (_b = c == null ? void 0 : c.frameLoads) == null ? void 0 : _b.get(o);
      if (u && (u[0] || u[1] || u[2])) {
        const S = X[0], B = X[1], N = [B[0] - S[0], B[1] - S[1], B[2] - S[2]], p = Math.hypot(N[0], N[1], N[2]);
        if (p > 1e-9) {
          const k = [N[0] / p, N[1] / p, N[2] / p], L = p * p / 12, O = [k[1] * u[2] - k[2] * u[1], k[2] * u[0] - k[0] * u[2], k[0] * u[1] - k[1] * u[0]], j = [-u[0] * p / 2, -u[1] * p / 2, -u[2] * p / 2, -L * O[0], -L * O[1], -L * O[2], -u[0] * p / 2, -u[1] * p / 2, -u[2] * p / 2, +L * O[0], +L * O[1], +L * O[2]], z = ct(h, j);
          _ = _.map((Y, P) => Y + z[P]);
        }
      }
      ...
      ct(h, g);
      const f = vn(c, o), m = xn(X), _ = Jn(g), u = kn(X), B = ct(1 / (2 * u), ct(ct(f, m), _)).toArray(), N = ((_d = c.thicknesses) == null ? void 0 : _d.get(o)) ?? 1, p = B[0][0] * N, k = B[1][0] * N, L = B[2][0] * N, O = B[0][1] * (N ** 3 / 12), j = B[1][1] * (N ** 3 / 12), z = B[2][1] * (N ** 3 / 12);
  ...
  const { nodeToCentroidElementIndiciesMap: y } = Dn(t, n);
  {
    ...
    if (n.forEach((g, h) => {
      ...
      const f = g.map((m) => t[m]);
      o.set(h, [0, 1, 2].map((m) => f.reduce((_, u) => _ + u[m], 0) / 4)), X.set(h, g);
    }), [...X.keys()].some(d)) {
      ...
      for (const [h, f] of X) {
        ...
        const u = o.get(h), S = (k) => {
          ...
          const P = k.get(h) ?? 0;
          for (const w of _) {
            const M = o.get(w), Q = M[0] - u[0], C = M[1] - u[1], D = (k.get(w) ?? 0) - P;
  ...
  return n.forEach((d, o) => {
    ...
    const X = d.length, g = new Array(X).fill(0), h = new Array(X).fill(0), f = new Array(X).fill(0), m = new Array(X).fill(0), _ = new Array(X).fill(0), u = new Array(X).fill(0), S = new Array(X).fill(0), B = new Array(X).fill(0), N = new Array(X).fill(0);
    d.forEach((j, z) => {
      const Y = (y.get(j) || []).filter((M) => n[M].length === 3 || n[M].length === 4), P = (M) => Et(Y.map((Q) => M.get(Q) ?? 0)), q = (M, Q) => Et(Y.map((C) => {
      ...
      m[z] = w(0, a.bendingXX), _[z] = w(1, a.bendingYY), u[z] = w(2, a.bendingXY), S[z] = P(a.tranverseShearX), B[z] = P(a.tranverseShearY), N[z] = P(a.vonMises);
    }), r.membraneXX.set(o, g), r.membraneYY.set(o, h), r.membraneXY.set(o, f), r.bendingXX.set(o, m), r.bendingYY.set(o, _), r.bendingXY.set(o, u);
...
function _n(t, n, c, e) {
  ...
  const r = ((_a = c.elasticities) == null ? void 0 : _a.get(e)) ?? 0, i = ((_b = c.poissonsRatios) == null ? void 0 : _b.get(e)) ?? 0, s = ((_c = c.thicknesses) == null ? void 0 : _c.get(e)) ?? 1, a = t[0], y = t[1], d = t[2], o = t[3], X = [y[0] - a[0], y[1] - a[1], y[2] - a[2]], g = [d[0] - o[0], d[1] - o[1], d[2] - o[2]];
  let h = [X[0] + g[0], X[1] + g[1], X[2] + g[2]], f = Math.sqrt(h[0] * h[0] + h[1] * h[1] + h[2] * h[2]);
  f < 1e-14 && (f = 1);
  let m = [h[0] / f, h[1] / f, h[2] / f];
  const _ = [d[0] - a[0], d[1] - a[1], d[2] - a[2]], u = [o[0] - y[0], o[1] - y[1], o[2] - y[2]];
  let S = [_[1] * u[2] - _[2] * u[1], _[2] * u[0] - _[0] * u[2], _[0] * u[1] - _[1] * u[0]], B = Math.sqrt(S[0] * S[0] + S[1] * S[1] + S[2] * S[2]);
  B < 1e-14 && (B = 1);
  ...
  const L = 0.25 * (a[0] + y[0] + d[0] + o[0]), O = 0.25 * (a[1] + y[1] + d[1] + o[1]), j = 0.25 * (a[2] + y[2] + d[2] + o[2]), z = [], Y = [];
  for (let F = 0; F < 4; F++) {
...
function xn(t) {
  const [n, c] = t[0], [e, r] = t[1], [i, s] = t[2], a = r - s, y = s - c, d = c - r, o = i - e, X = n - i, g = e - n;
  return rt([[a, y, d, 0, 0, 0], [0, 0, 0, o, X, g], [o, X, g, a, y, d]]);
}
function Jn(t) {
  const [n, c, e] = [t[0], t[6], t[12]], [r, i, s] = [t[1], t[7], t[13]], [a, y, d] = [t[4], t[10], t[16]], [o, X, g] = [t[3], t[9], t[15]];
  return rt([[n, -a], [c, -y], [e, -d], [r, o], [i, X], [s, g]]);
}
function kn(t) {
  const [n, c] = t[0], [e, r] = t[1], [i, s] = t[2], a = e - n, y = i - n, d = s - c, o = c - r;
  return 0.5 * (a * d - y * -o);
}
function Dn(t, n) {
  const c = /* @__PURE__ */ new Map(), e = /* @__PURE__ */ new Map();
  return n.forEach((r, i) => {
    const s = r.map((y) => t[y]), a = qn(s);
    r.forEach((y) => {
      var _a, _b;
      c.has(y) || c.set(y, []), (_a = c.get(y)) == null ? void 0 : _a.push(a), e.has(y) || e.set(y, []), (_b = e.get(y)) == null ? void 0 : _b.push(i);
    });
