...
function Eo(e) {
  ...
  const l = [...e].sort((w, p) => w - p), d = (w) => l[Math.min(l.length - 1, Math.max(0, Math.round(w * (l.length - 1))))];
...
function la(e, l, d) {
  ...
  return w.renderOrder = -1, w.frustumCulled = false, w.userData.isShellArea = true, w.name = "__hekatan_shell_colormap", W.derive(() => {
    ...
    const le = H && H[0] > H[1], ue = Math.min(b, z), ae = Math.max(b, z), E = ae - ue, ie = new Float32Array(d.val.length);
...
function ra(e, l, d) {
  ...
  const g = () => {
    ...
    let H = false, le = 0, ue = 0, ae = 0, E = 0;
    b.addEventListener("mousedown", (ie) => {
      H = true, le = ie.clientX, ue = ie.clientY;
      ...
      if (!H) return;
      const ee = ie.clientX - le, be = ie.clientY - ue, ge = Math.max(0, Math.min(window.innerWidth - 40, ae + ee)), U = Math.max(0, Math.min(window.innerHeight - 40, E + be));
...
function Po(e, l) {
  ...
  }, ue = { xy: [14042459, 5155178], xz: [14042459, 4882390], yz: [5155178, 4882390] };
  for (const ae of c) {
...
function pa(e, l, d, h) {
  ...
  const g = new pt({ color: 10166822 }), x = new pt({ color: 2792847 }), k = new pt({ color: 3835647 }), z = () => {
    ...
    let ue = [1 / 0, 1 / 0, 1 / 0], ae = [-1 / 0, -1 / 0, -1 / 0];
    for (const E of le) for (let ie = 0; ie < 3; ie++) E[ie] < ue[ie] && (ue[ie] = E[ie]), E[ie] > ae[ie] && (ae[ie] = E[ie]);
    return Math.max(ae[0] - ue[0], ae[1] - ue[1], ae[2] - ue[2], 0.1);
  }, b = () => 0.08 * z(), H = () => h.rawVal;
  return W.derive(() => {
    var _a2, _b;
    ...
    c.clear();
    const le = b();
    (_b = (_a2 = e.nodeInputs) == null ? void 0 : _a2.val.supports) == null ? void 0 : _b.forEach((ue, ae) => {
      const E = d.val[ae];
      if (!E) return;
      const ie = ue ?? [], ee = (ie[0] ? 1 : 0) + (ie[1] ? 1 : 0) + (ie[2] ? 1 : 0), be = (ie[3] ? 1 : 0) + (ie[4] ? 1 : 0) + (ie[5] ? 1 : 0);
      let ge;
    ...
    const ue = b() * H();
    c.children.forEach((ae) => ae.scale.set(ue, ue, ue));
  }), W.derive(() => {
...
function ua(e, l, d, h) {
  ...
  return W.derive(() => {
    ...
    (_c = (_b = (_a2 = e.nodeInputs) == null ? void 0 : _a2.val) == null ? void 0 : _b.loads) == null ? void 0 : _c.forEach((b, H) => {
      p[H] && b.slice(0, 3).some((le) => Math.abs(le) > 1e-15) && k.push(H);
    });
    let z = k;
    if (k.length > x) {
      const b = k.map((T) => p[T][0]), H = k.map((T) => p[T][1]), le = Math.min(...b), ue = Math.max(...b), ae = Math.min(...H), E = Math.max(...H), ie = k.map((T) => p[T][2]), ee = Math.max(1e-6, (Math.max(...ie) - Math.min(...ie)) / 40), be = (T) => Math.round(T / ee), ge = new Set(ie.map(be)), U = Math.max(4, Math.floor(x / Math.max(1, ge.size))), B = Math.max(2, Math.round(Math.sqrt(U))), K = /* @__PURE__ */ new Map();
      for (const T of k) {
        const V = ue - le < 1e-9 ? 0 : (p[T][0] - le) / (ue - le), Z = E - ae < 1e-9 ? 0 : (p[T][1] - ae) / (E - ae), Y = Math.min(B - 1, Math.floor(V * B)), $ = Math.min(B - 1, Math.floor(Z * B)), ne = `${Y},${$},${be(p[T][2])}`, he = Math.hypot(V * B - (Y + 0.5), Z * B - ($ + 0.5)), ce = K.get(ne);
        (!ce || he < ce.d) && K.set(ne, { i: T, d: he });
      }
      z = [...K.values()].map((T) => T.i);
    }
    for (const b of z) {
      const H = e.nodeInputs.val.loads.get(b), le = p[b];
      if (!le) continue;
      const ue = new _(...H.slice(0, 3));
      if (ue.lengthSq() < 1e-30) continue;
      ue.normalize();
      const ae = new ln(ue, new _(...le), 1, 15637248, 0.3, 0.3), E = g * h.rawVal;
...
function ds(e, l) {
  const d = new tt(), h = Math.min(0.05 * e, 0.6), c = rn(), w = new kt("X", "red", "transparent"), p = new kt(l ? "Z" : "Y", "green", "transparent"), g = new kt(l ? "Y" : "Z", "blue", "transparent"), x = new ln(new _(1, 0, 0), new _(0, 0, 0), 1, c.axisArrow, 0.2, 0.2), k = new ln(new _(0, 1, 0), new _(0, 0, 0), 1, c.axisArrow, 0.2, 0.2), z = new ln(new _(0, 0, 1), new _(0, 0, 0), 1, c.axisArrow, 0.2, 0.2);
...
function to(e, l) {
  const d = new _(...e), c = new _(...l).clone().sub(d), w = c.length(), p = c.dot(new _(1, 0, 0)) / w, g = c.dot(new _(0, 1, 0)) / w, x = c.dot(new _(0, 0, 1)) / w, k = Math.sqrt(p ** 2 + g ** 2);
...
function wa(e, l, d) {
  const h = En([l, d]), c = En([e, d]), w = En([e, l]), p = new _(...h).sub(new _(...c)).normalize(), g = new _(...d).sub(new _(...w)).normalize(), x = p.clone().cross(g).normalize(), k = x.clone().cross(p).normalize();
...
function ya(e, l, d, h) {
  ...
  return w.setAttribute("color", new bt([...b, ...b, ...H, ...H, ...le, ...le], 3)), W.derive(() => {
    var _a2;
    l.deformedShape.val, l.orientations.val && (c.clear(), (_a2 = e.elements) == null ? void 0 : _a2.val.forEach((ue) => {
      const ae = new Ht(w, p), E = d.rawVal[ue[0]], ie = d.rawVal[ue[1]];
      if (ue.length === 2 && (ae.position.set(...zo(E, ie)), ae.rotation.setFromRotationMatrix(to(E, ie))), ue.length === 3) {
        const ge = d.rawVal[ue[2]];
...
function ga(e, l, d, h) {
  ...
  function g(B, K = 24) {
    ...
    for (let ne = 0; ne < K; ne++) {
      const he = ne / K * Math.PI * 2, ce = (ne + 1) / K * Math.PI * 2;
      V[ne * 9] = 0, V[ne * 9 + 1] = 0, V[ne * 9 + 2] = 0, V[ne * 9 + 3] = 0, V[ne * 9 + 4] = T * Math.cos(he), V[ne * 9 + 5] = T * Math.sin(he), V[ne * 9 + 6] = 0, V[ne * 9 + 7] = T * Math.cos(ce), V[ne * 9 + 8] = T * Math.sin(ce);
    }
    const Z = new Me();
    ...
    for (let ne = 0; ne <= K; ne++) {
      ...
      Y[ne * 3] = 0, Y[ne * 3 + 1] = T * Math.cos(he), Y[ne * 3 + 2] = T * Math.sin(he);
    }
    const $ = new Me();
    return $.setAttribute("position", new ft(Y, 3)), { fill: Z, outline: $ };
  }
  function x(B, K, T, V) {
    const Z = T ?? K * 0.08, Y = V ?? B * 0.07, $ = B / 2, ne = K / 2, he = ne - Z, ce = Y / 2, se = [];
    function R(ye, Ie, _e, Xe) {
      se.push(0, ye, Ie, 0, _e, Ie, 0, _e, Xe, 0, ye, Ie, 0, _e, Xe, 0, ye, Xe);
    }
    R(-$, -ne, $, -he), R(-ce, -he, ce, he), R(-$, he, $, ne);
    const fe = new Me();
    fe.setAttribute("position", new ft(new Float32Array(se), 3));
    const O = new Float32Array([0, -$, -ne, 0, $, -ne, 0, $, -he, 0, ce, -he, 0, ce, he, 0, $, he, 0, $, ne, 0, -$, ne, 0, -$, he, 0, -ce, he, 0, -ce, -he, 0, -$, -he, 0, -$, -ne]), we = new Me();
    return we.setAttribute("position", new ft(O, 3)), { fill: fe, outline: we };
  }
  function k(B, K, T) {
    const V = B / 2, Z = K / 2, Y = V - T, $ = Z - T, ne = [];
    function he(fe, O, we, ye) {
      ne.push(0, fe, O, 0, we, O, 0, we, ye, 0, fe, O, 0, we, ye, 0, fe, ye);
    }
    he(-V, -Z, V, -$), he(-V, $, V, Z), he(-V, -$, -Y, $), he(Y, -$, V, $);
    const ce = new Me();
    ce.setAttribute("position", new ft(new Float32Array(ne), 3));
    const se = new Float32Array([0, -V, -Z, 0, V, -Z, 0, V, -Z, 0, V, Z, 0, V, Z, 0, -V, Z, 0, -V, Z, 0, -V, -Z, 0, -Y, -$, 0, Y, -$, 0, Y, -$, 0, Y, $, 0, Y, $, 0, -Y, $, 0, -Y, $, 0, -Y, -$]), R = new Me();
    return R.setAttribute("position", new ft(se, 3)), { fill: ce, outline: R };
  }
  function z(B, K, T) {
    const V = B / 2, Z = K / 2, Y = V - T, $ = Z - T, ne = new Me(), he = new Float32Array([0, -Y, -$, 0, Y, -$, 0, Y, $, 0, -Y, -$, 0, Y, $, 0, -Y, $]);
    ne.setAttribute("position", new ft(he, 3));
    const ce = [];
    function se(we, ye, Ie, _e) {
      ce.push(0, we, ye, 0, Ie, ye, 0, Ie, _e, 0, we, ye, 0, Ie, _e, 0, we, _e);
    }
    se(-V, -Z, V, -$), se(-V, $, V, Z), se(-V, -$, -Y, $), se(Y, -$, V, $);
    const R = new Me();
    R.setAttribute("position", new ft(new Float32Array(ce), 3));
    const fe = new Float32Array([0, -V, -Z, 0, V, -Z, 0, V, -Z, 0, V, Z, 0, V, Z, 0, -V, Z, 0, -V, Z, 0, -V, -Z, 0, -Y, -$, 0, Y, -$, 0, Y, -$, 0, Y, $, 0, Y, $, 0, -Y, $, 0, -Y, $, 0, -Y, -$]), O = new Me();
    return O.setAttribute("position", new ft(fe, 3)), { concFill: ne, steelFillGeom: R, outline: O };
  }
  function b(B, K, T) {
    const V = [], Z = [[0, -B / 2, -K / 2], [0, -B / 2 + T, -K / 2], [0, -B / 2 + T, K / 2 - T], [0, B / 2, K / 2 - T], [0, B / 2, K / 2], [0, -B / 2, K / 2]], Y = [0, 1, 2, 0, 2, 5, 2, 3, 4, 2, 4, 5];
    for (const ce of Y) V.push(...Z[ce]);
    const $ = new Me();
    $.setAttribute("position", new ft(new Float32Array(V), 3));
    const ne = [];
    for (let ce = 0; ce < Z.length; ce++) {
      const se = (ce + 1) % Z.length;
      ne.push(...Z[ce], ...Z[se]);
    }
    const he = new Me();
    return he.setAttribute("position", new ft(new Float32Array(ne), 3)), { fill: $, outline: he };
  }
  function H(B, K, T, V) {
    const Z = V / 2, Y = [], $ = [[0, -B - Z, -K / 2], [0, -T - Z, -K / 2], [0, -T - Z, K / 2 - T], [0, -Z, K / 2 - T], [0, -Z, K / 2], [0, -B - Z, K / 2]], ne = [[0, Z, -K / 2], [0, Z + T, -K / 2], [0, Z + T, K / 2 - T], [0, B + Z, K / 2 - T], [0, B + Z, K / 2], [0, Z, K / 2]], he = [0, 1, 2, 0, 2, 5, 2, 3, 4, 2, 4, 5];
    for (const fe of he) Y.push(...$[fe]);
    for (const fe of he) Y.push(...ne[fe]);
    const ce = new Me();
    ce.setAttribute("position", new ft(new Float32Array(Y), 3));
    const se = [];
    for (const fe of [$, ne]) for (let O = 0; O < fe.length; O++) {
      const we = (O + 1) % fe.length;
      se.push(...fe[O], ...fe[we]);
    }
    const R = new Me();
    return R.setAttribute("position", new ft(new Float32Array(se), 3)), { fill: ce, outline: R };
  }
  function le(B, K, T, V) {
    const Z = K / 2, Y = B, $ = [[0, -Y, -Z], [0, -Y, -Z + T], [0, -V, -Z + T], [0, -V, Z - T], [0, -Y, Z - T], [0, -Y, Z], [0, 0, Z], [0, 0, -Z]], ne = [0, 1, 7, 1, 6, 7, 1, 2, 6, 2, 5, 6, 2, 3, 5, 3, 4, 5], he = [];
    for (const fe of ne) he.push(...$[fe]);
    const ce = new Me();
    ce.setAttribute("position", new ft(new Float32Array(he), 3));
    const se = [];
  ...
  function ue(B, K, T, V, Z) {
    const Y = K / 2, $ = Z / 2, ne = [], he = [[0, -B, -Y], [0, -B, -Y + T], [0, -$ - V, -Y + T], [0, -$ - V, Y - T], [0, -B, Y - T], [0, -B, Y], [0, -$, Y], [0, -$, -Y]], ce = he.map((we) => [we[0], -we[1], we[2]]), se = [0, 1, 7, 1, 6, 7, 1, 2, 6, 2, 5, 6, 2, 3, 5, 3, 4, 5];
    for (const we of se) ne.push(...he[we]);
    for (const we of se) ne.push(...ce[we]);
    const R = new Me();
    R.setAttribute("position", new ft(new Float32Array(ne), 3));
    const fe = [];
    for (const we of [he, ce]) for (let ye = 0; ye < we.length; ye++) {
      const Ie = (ye + 1) % we.length;
      fe.push(...we[ye], ...we[Ie]);
    }
    const O = new Me();
    return O.setAttribute("position", new ft(new Float32Array(fe), 3)), { fill: R, outline: O };
  }
  function ae(B, K, T, V) {
    const Z = B / 2, Y = K / 2, $ = V / 2, ne = [[0, -$, -Y], [0, $, -Y], [0, $, Y - T], [0, Z, Y - T], [0, Z, Y], [0, -Z, Y], [0, -Z, Y - T], [0, -$, Y - T]], he = [0, 1, 7, 1, 2, 7, 6, 7, 5, 2, 3, 4, 2, 4, 5, 2, 5, 7], ce = [];
    for (const O of he) ce.push(...ne[O]);
    const se = new Me();
    se.setAttribute("position", new ft(new Float32Array(ce), 3));
    const R = [];
    ...
    const fe = new Me();
    return fe.setAttribute("position", new ft(new Float32Array(R), 3)), { fill: se, outline: fe };
  }
  function E(B, K, T = 24) {
    const V = B / 2, Z = V - K, Y = [];
    for (let ce = 0; ce < T; ce++) {
      const se = ce / T * Math.PI * 2, R = (ce + 1) / T * Math.PI * 2, fe = Math.cos(se), O = Math.sin(se), we = Math.cos(R), ye = Math.sin(R);
      Y.push(0, V * fe, V * O, 0, V * we, V * ye, 0, Z * we, Z * ye), Y.push(0, V * fe, V * O, 0, Z * we, Z * ye, 0, Z * fe, Z * O);
    }
    const $ = new Me();
    $.setAttribute("position", new ft(new Float32Array(Y), 3));
    const ne = [];
    for (let ce = 0; ce < T; ce++) {
      const se = ce / T * Math.PI * 2, R = (ce + 1) / T * Math.PI * 2;
  ...
  return W.derive(() => {
    ...
    T.forEach(($, ne) => {
      if ($.length !== 2) return;
      const he = d.rawVal[$[0]], ce = d.rawVal[$[1]];
      if (!he || !ce) return;
      const se = U(he, ce);
      if (se && !B || !se && !K) return;
      if (Y >= 0) {
        const ye = Math.min(he[1], ce[1]);
        Math.max(he[1], ce[1]);
      ...
      if (!R) return;
      const fe = [(he[0] + ce[0]) / 2, (he[1] + ce[1]) / 2, (he[2] + ce[2]) / 2], O = to(he, ce);
      if (R.type === "CFT") {
        const ye = z(R.b, R.h, R.tw ?? R.b * 0.05), Ie = new nt(ye.concFill, ie);
        Ie.position.set(...fe), Ie.rotation.setFromRotationMatrix(O), c.add(Ie);
        const _e = new nt(ye.steelFillGeom, be);
        _e.position.set(...fe), _e.rotation.setFromRotationMatrix(O), c.add(_e);
        const Xe = new St(ye.outline, ge);
        Xe.position.set(...fe), Xe.rotation.setFromRotationMatrix(O), c.add(Xe);
      } else {
        let ye, Ie, _e;
        switch (R.type) {
          case "rect":
          ...
          case "coldC":
            ...
            break;
          case "2C":
            ye = ue(R.b, R.h, R.tf ?? 5e-3, R.tw ?? 5e-3, R.dis ?? 0.01), Ie = be, _e = ge;
            break;
          case "T":
...
function va(e) {
  if (!e) return null;
  const l = e.type, d = (z, b) => [z, b], h = (z, b) => [d(-z / 2, -b / 2), d(z / 2, -b / 2), d(z / 2, b / 2), d(-z / 2, b / 2)], c = (z, b = 24) => {
    const H = z / 2, le = [];
    for (let ue = 0; ue < b; ue++) {
      const ae = 2 * Math.PI * ue / b;
      le.push(d(H * Math.cos(ae), H * Math.sin(ae)));
    }
    return le;
  }, w = e.b ?? 0, p = e.h ?? 0, g = e.d ?? 0, x = e.tw ?? e.t ?? 0, k = e.tf ?? e.t ?? 0;
  switch (l) {
    case "rect":
...
function Ma(e, l, d) {
  ...
  const h = Math.sqrt(Math.sqrt(d / l)), c = Math.sqrt(e / h), w = e / c;
...
function _a(e, l, d) {
  ...
  return W.derive(() => {
    ...
    const H = d.val ?? [], le = ((_d = e.elements) == null ? void 0 : _d.val) ?? [], ue = ((_e = e.elementInputs) == null ? void 0 : _e.val) ?? {}, ae = ue.sectionShapes ?? /* @__PURE__ */ new Map(), E = ue.thicknesses ?? /* @__PURE__ */ new Map();
    let ie = "";
    try {
      le.forEach((ee, be) => {
        var _a3, _b2, _c2;
        if (ee.length === 2) {
          ...
          if (ge || (ge = Ma((_a3 = ue.areas) == null ? void 0 : _a3.get(be), (_b2 = ue.momentsOfInertiaY) == null ? void 0 : _b2.get(be), (_c2 = ue.momentsOfInertiaZ) == null ? void 0 : _c2.get(be)), U = false), !ge) return;
        ...
        if (ee.length === 3 || ee.length === 4) {
          ...
          if (Y < 1e-12) return;
          const $ = [T / Y, V / Y, Z / Y], ne = [], he = (O) => U.map((we) => [we[0] + $[0] * O, we[1] + $[1] * O, we[2] + $[2] * O]), ce = he(+ge / 2), se = he(-ge / 2), R = (O, we, ye) => ne.push(...O, ...we, ...ye);
          for (const O of [ce, se]) R(O[0], O[1], O[2]), O.length === 4 && R(O[0], O[2], O[3]);
          for (let O = 0; O < U.length; O++) {
            const we = (O + 1) % U.length;
            R(ce[O], se[O], se[we]), R(ce[O], se[we], ce[we]);
          }
          const fe = new Me();
          fe.setAttribute("position", new bt(ne, 3)), fe.computeVertexNormals(), h.add(new nt(fe, p));
        }
      });
    } catch (ee) {
...
class ps extends tt {
  constructor(l, d, h, c, w, p, g) {
    ...
    if (this.text = new kt(`${w[0].toFixed(4)}`), this.text2 = new kt(`${(w[1] * -1).toFixed(4)}`), this.normalizedResult = p, this.textPosition = zo(l, d), this.text2Position = zo(d, l), this.text.position.set(...this.textPosition), this.text2.position.set(...this.text2Position), this.text.rotation.setFromRotationMatrix(c), this.text2.rotation.setFromRotationMatrix(c), this.add(this.text, this.text2), k) {
      const z = new An().moveTo(0, 0).lineTo(0, p[0]).lineTo(x, 0).lineTo(0, 0), b = new An().moveTo(x, 0).lineTo(h, -p[1]).lineTo(h, 0).lineTo(x, 0), H = z.getPoints(), le = b.getPoints(), ue = new Me().setFromPoints(H), ae = new Me().setFromPoints(le), E = new ut({ color: rn().resultOutline });
      this.lines = new St(ue, E), this.lines2 = new St(ae, E), this.lines.position.set(...l), this.lines2.position.set(...l), this.lines.rotation.setFromRotationMatrix(c), this.lines2.rotation.setFromRotationMatrix(c), g && this.lines.rotateX(Math.PI / 2), g && this.lines2.rotateX(Math.PI / 2), this.add(this.lines, this.lines2);
      ...
      const le = new Qn(z), ue = new pt({ color: p[0] > 0 ? 24435 : 11411474, side: Pt });
      this.mesh = new nt(le, ue), this.mesh.position.set(...l), this.mesh.rotation.setFromRotationMatrix(c), g && this.mesh.rotateX(Math.PI / 2), this.add(this.mesh);
    }
  }
  updateScale(l) {
    var _a2, _b;
...
var Ss = ((e) => (e.normals = "normals", e.shearsY = "shearsY", e.shearsZ = "shearsZ", e.torsions = "torsions", e.bendingsY = "bendingsY", e.bendingsZ = "bendingsZ", e))(Ss || {});
function Sa(e, l, d, h) {
  const c = () => {
    const g = d.rawVal;
  ...
  return W.derive(() => {
    ...
    (_b = (_a2 = e.analyzeOutputs) == null ? void 0 : _a2.rawVal[g]) == null ? void 0 : _b.forEach((x, k) => {
      var _a3, _b2;
      const z = ((_a3 = e.elements) == null ? void 0 : _a3.rawVal[k]) ?? [0, 1], b = d.rawVal[z[0]], H = d.rawVal[z[1]];
      if (!b || !H) return;
      const le = new _(...H).distanceTo(new _(...b)), ue = ka((_b2 = e.analyzeOutputs) == null ? void 0 : _b2.rawVal[g]), ae = x == null ? void 0 : x.map((ee) => ee / (ue === 0 ? 1 : ue)), E = to(b, H), ie = new p[g](b, H, le, E, x ?? [0, 0], ae ?? [0, 0], !!["normals", "shearsZ", "torsions", "bendingsY"].includes(g));
...
var Vo = ((e) => (e.deformations = "deformations", e.reactions = "reactions", e))(Vo || {});
function Ca(e, l, d, h) {
  const c = new tt();
  return W.derive(() => {
    var _a2, _b;
...
function za({ drawingObj: e, gridObj: l, scene: d, getActiveCamera: h, controls: c, gridSize: w, derivedDisplayScale: p, rendererElm: g, viewerRender: x }) {
  const k = new Os(), z = new Qs(), b = (t) => {
  ...
  }, ue = le(Math.PI / 2, 0, 0), ae = le(0, Math.PI / 2, 0);
  let E = false;
  const ie = () => {
    if (E) return k.intersectObjects([H], false);
    if (ue.visible = !!window.__hekatanGridPlaneXZ, ae.visible = !!window.__hekatanGridPlaneYZ, window.__hekatanOrthoRaycast === true && Ye.visible) {
      const s = k.intersectObjects([Ye, rt, Le], false);
      if (s.length > 0) return s;
    }
    const o = [H];
    return ue.visible && o.push(ue), ae.visible && o.push(ae), Jt.visible && pn.length > 0 && o.push(...pn), k.intersectObjects(o, false);
  ...
  const V = new _(), Z = (t, o, s, n, a, i) => {
    const r = n - t, f = a - o, m = i - s, y = Math.hypot(r, f, m);
    if (y < 0.01) {
      U.style.display = "none";
      return;
    }
    B = [t, o, s], K = [r / y, f / y, m / y], V.set((t + n) / 2, (o + a) / 2, (s + i) / 2), V.project(h());
    ...
    if (o === "offset") {
      an = t, oe(`\u21C9 DESFASE distancia ${t} m \u2014 designe la l\xEDnea y luego el lado.`), U.blur();
      try {
    ...
    if (o === "circle" && $e.length === 1) {
      const M = $e[0];
      $e = [], (_e2 = window.__hekatanDrawCircle) == null ? void 0 : _e2.call(window, M[0], M[1], M[2], t), oe(`\u2713 C\xEDrculo r=${t} m en (${M[0].toFixed(2)}, ${M[1].toFixed(2)}, ${M[2].toFixed(2)}).`);
      try {
    ...
    if (o === "col" || o === "wall" || o === "extp" || o === "extl") {
      xt = t, oe(`\u{1F4D0} Altura ${t}m memorizada \u2014 hac\xE9 el click para crear ${{ col: "columna", wall: "pared", extp: "extrusi\xF3n punto\u2192l\xEDnea", extl: "extrusi\xF3n l\xEDnea\u2192\xE1rea" }[o]}.`), U.blur();
      return;
    }
    if (!B || !K || !e.polylines) return;
    let s = K[0], n = K[1], a = K[2];
    at === "x" ? (s = Math.sign(s) || 1, n = 0, a = 0) : at === "y" ? (s = 0, n = Math.sign(n) || 1, a = 0) : at === "z" && (s = 0, n = 0, a = Math.sign(a) || 1);
    const i = B[0] + s * t, r = B[1] + n * t, f = B[2] + a * t;
    window.__hekatanPushUndo && window.__hekatanPushUndo(), e.points.val = [...e.points.rawVal, [i, r, f]];
    const m = e.polylines.rawVal, y = m.length ? m[m.length - 1] : [];
    e.polylines.val = [...m.slice(0, -1), [...y, e.points.rawVal.length - 1]], U.blur();
    try {
    ...
    if (t.kind === "relSpherical") {
      ...
      return [B[0] + n * Math.cos(o), B[1] + n * Math.sin(o), B[2] + t.L * Math.sin(s)];
    }
    return null;
  }, ce = (t) => {
    var _a2, _b;
    ...
    e.polylines.val = [...o.slice(0, -1), [...s, e.points.rawVal.length - 1]], B = t, U.blur();
    try {
  ...
  const fe = new St(new Me().setFromPoints([new _(0, 0, 0), new _(0, 0, 0)]), new zn({ color: 2282478, dashSize: 0.2, gapSize: 0.1, transparent: true, opacity: 0.85, linewidth: 2 }));
  ...
  let we = [];
  const ye = new tt(), Ie = new nt(new jt(1, 1), new pt({ color: 2282478, transparent: true, opacity: 0.08, side: Pt, depthWrite: false })), _e = new Ht(new os(new jt(1, 1)), new ut({ color: 2282478, transparent: true, opacity: 0.85 })), Xe = new Ht(new Me(), new ut({ color: 2282478, transparent: true, opacity: 0.3 })), Qe = (t, o) => {
  ...
  const Ct = (t) => {
    const o = new Me().setFromPoints([new _(0, 0, 0), new _(0, 0, 0)]), s = new zn({ color: t, dashSize: 0.15, gapSize: 0.08, transparent: true, opacity: 0.5, linewidth: 1 });
    return new St(o, s);
  }, P = Ct(16711680), L = Ct(65280), Q = Ct(35071);
  ht.add(P, L, Q);
  const N = [], me = (t) => t.traverse((o) => {
    var _a2, _b, _c, _d;
  ...
  re.material.dashSize = 0.28, re.material.gapSize = 0.16, re.material.opacity = 0.9, re.frustumCulled = false, re.visible = false, re.renderOrder = 98, d.add(re);
  const Se = (t) => {
    const o = new Me().setFromPoints([new _(0, 0, 0), new _(0, 0, 0), new _(0, 0, 0), new _(0, 0, 0)]), s = new ut({ color: t, transparent: true, opacity: 0.2, depthTest: false }), n = new ys(o, s);
  ...
  const He = (t) => {
    const o = new jt(1, 1), s = new pt({ color: t, transparent: true, opacity: 0.06, side: Pt, depthWrite: false }), n = new nt(o, s);
  ...
  Oe.id = "hk-refplane-badge", Oe.style.cssText = ["position:fixed", "pointer-events:none", "z-index:99997", "padding:3px 10px", "border-radius:4px", "font-family:Consolas,monospace", "font-size:12px", "font-weight:bold", "transform:translate(20px,40px)", "white-space:nowrap", "display:none"].join(";") + ";", document.body.appendChild(Oe), window.__hekatanSetOrthoPlanes = (t) => {
    var _a2;
    if (window.__hekatanShowOrthoPlanes = t, Ze.visible = t, t) {
    ...
    var _a2;
    if (window.__hekatanOrthoExt = t, !Ze.visible) {
      x();
      return;
    }
    ...
    ot(Pe, i, "xy", t), ot(Be, i, "xz", t), ot(Re, i, "yz", t), st(Ye, i, "xy", t), st(rt, i, "xz", t), st(Le, i, "yz", t), x();
  };
  const cn = (t) => {
    if (Ye.material.opacity = t === "xy" ? 0.09 : 0.025, rt.material.opacity = t === "xz" ? 0.09 : 0.025, Le.material.opacity = t === "yz" ? 0.09 : 0.025, t) {
      ...
      Oe.style.background = a.bg, Oe.style.color = a.text, Oe.textContent = `\u25A6 Plano ${t.toUpperCase()}`, Oe.style.display = "block";
    } else Oe.style.display = "none";
  }, ot = (t, o, s, n) => {
    let a;
    s === "xy" ? a = [new _(o[0] - n, o[1] - n, o[2]), new _(o[0] + n, o[1] - n, o[2]), new _(o[0] + n, o[1] + n, o[2]), new _(o[0] - n, o[1] + n, o[2]), new _(o[0] - n, o[1] - n, o[2])] : s === "xz" ? a = [new _(o[0] - n, o[1], o[2] - n), new _(o[0] + n, o[1], o[2] - n), new _(o[0] + n, o[1], o[2] + n), new _(o[0] - n, o[1], o[2] + n), new _(o[0] - n, o[1], o[2] - n)] : a = [new _(o[0], o[1] - n, o[2] - n), new _(o[0], o[1] + n, o[2] - n), new _(o[0], o[1] + n, o[2] + n), new _(o[0], o[1] - n, o[2] + n), new _(o[0], o[1] - n, o[2] - n)], t.geometry.setFromPoints(a);
  };
  let at = null;
  ...
  const Ae = () => {
    ...
    Ce.style.background = "rgba(15,23,42,0.92)", Ce.style.color = t[at], Ce.style.border = `1.5px solid ${t[at]}`, Ce.textContent = `\u{1F512} LOCK ${at.toUpperCase()}`, Ce.style.display = "block";
  };
  window.addEventListener("keydown", (t) => {
    ...
    if (s === "x" || s === "y" || s === "z") at = at === s ? null : s, Ae(), t.preventDefault();
    else if (t.key === "Escape") {
      const a = document.activeElement;
      a && (a.tagName === "INPUT" || a.tagName === "TEXTAREA") && a.blur(), Ko(), t.preventDefault();
    ...
    window.__hekatanOsnapOn = t, t || Xn(), oe(`\u{1F9F2} OSNAP ${t ? "ON" : "OFF"} (F3)`);
  }, window.__hekatanTogglePolar = () => {
    const t = window.__hekatanPolarTrack === false;
    window.__hekatanPolarTrack = t, t || (ht.visible = false), oe(`\u25C8 POLAR ${t ? "ON" : "OFF"} (F10)`);
  ...
  const Ke = new _(), ze = new _(), ct = new _(), mt = (t) => {
    if (!at) return null;
  ...
  const xn = (t, o, s, n) => {
    ...
    for (let f = 0; f < a.length; f++) {
      const m = a[f];
      if (!m) continue;
      const y = Math.hypot(t - m[0], o - m[1], s - m[2]);
      y < r && (r = y, i = f);
    }
    return i;
  }, At = () => {
    ...
    for (const r of ke) {
      ...
      if (f === "pt") {
        ...
        const y = o[+m[0]], M = t[y == null ? void 0 : y[+m[1]]], S = t[y == null ? void 0 : y[+m[1] + 1]];
        if (!M || !S) continue;
  ...
  const tn = (t, o, s, n, a, i, r, f, m) => {
    ...
    for (let y = 0; y < a.length; y++) {
      ...
      for (let S = 0; S < M.length - 1; S++) {
        ...
        if (!u || !F) continue;
        const j = tn(t, o, s, u[0], u[1], u[2], F[0], F[1], F[2]);
        j < m && (m = j, r = y, f = S);
      }
    }
    ...
    for (let m = 0; m < i.length; m++) {
      const y = i[m];
      if (!y || y.length !== 6) continue;
      const M = tn(t, o, s, y[0], y[1], y[2], y[3], y[4], y[5]);
      M < f && (f = M, r = m);
    }
    return r;
  }, no = (t) => {
    ...
    const s = o.filter((m, y) => y !== t), n = /* @__PURE__ */ new Set();
    ...
    e.points.val = r, e.polylines.val = f, e.areas && (e.areas.val = e.areas.rawVal.filter((m) => m !== t).map((m) => m > t ? m - 1 : m)), it.visible = false, De = -1, Ge = -1;
    try {
      (_a2 = window.__hekatanRebuild) == null ? void 0 : _a2.call(window);
    } catch {
    }
  ...
  ee.geometry.setAttribute("position", new bt(e.points.rawVal.flat(), 3)), ee.geometry.computeBoundingSphere(), ee.frustumCulled = false, be.frustumCulled = false, d.add(be), H.position.set(0, 0, 0), H.rotateX(Math.PI / 2), H.geometry.rotateX(Math.PI / 2), H.updateMatrixWorld(), e.polylines && (e.polylines.val = [...e.polylines.rawVal, []]), window.__hekatanDrawAt = (t, o, s) => {
  ...
  window.__hekatanCentrosDeducidos = vn, window.__hekatanDrawCircle = (t, o, s, n, a = window.__hekatanArcSegs ?? 12, i = "xy") => {
    var _a2;
    const r = Math.max(4, Math.round(a)), f = e.points.rawVal.length, m = [];
    for (let y = 0; y < r; y++) {
      const M = 2 * Math.PI * y / r, S = n * Math.cos(M), u = n * Math.sin(M);
      let F;
      i === "xy" ? F = [t + S, o + u, s] : i === "xz" ? F = [t + S, o, s + u] : F = [t, o + S, s + u], m.push(F);
    }
    if (e.points.val = [...e.points.rawVal, ...m], on.push({ c: [t, o, s], r: n }), e.polylines) {
      const y = [...m.map((S, u) => f + u), f], M = e.polylines.rawVal;
      ((_a2 = M[M.length - 1]) == null ? void 0 : _a2.length) > 0 ? e.polylines.val = [...M, y, []] : e.polylines.val = [...M.slice(0, -1), y, []];
    }
  }, window.__hekatanDrawArc = (t, o, s, n = window.__hekatanArcSegs ?? 12) => {
    const a = Math.max(4, Math.round(n)), i = new _(...t), r = new _(...o), f = new _(...s), m = new _().subVectors(r, i), y = new _().subVectors(f, i), M = new _().crossVectors(m, y).normalize(), S = new _().addVectors(i, r).multiplyScalar(0.5), u = new _().addVectors(r, f).multiplyScalar(0.5), F = new _().crossVectors(m, M).normalize(), j = new _().crossVectors(new _().subVectors(f, r), M).normalize(), J = new _().subVectors(u, S), X = F.x * j.y - F.y * j.x;
    let v;
    if (Math.abs(X) > 1e-9) {
      const pe = (J.x * j.y - J.y * j.x) / X;
      v = new _().addVectors(S, F.clone().multiplyScalar(pe));
    } else v = S.clone();
    const A = i.distanceTo(v), C = new _().subVectors(i, v), D = new _().subVectors(f, v), q = Math.acos(Math.max(-1, Math.min(1, C.dot(D) / (A * A)))), I = e.points.rawVal.length, G = [], de = M.clone();
    for (let pe = 0; pe <= a; pe++) {
      const ve = pe / a, Je = q * ve, Ve = new Kn().setFromAxisAngle(de, Je), je = C.clone().applyQuaternion(Ve).add(v);
      G.push([je.x, je.y, je.z]);
    }
    if (e.points.val = [...e.points.rawVal, ...G], on.push({ c: [v.x, v.y, v.z], r: A }), e.polylines) {
      const pe = G.map((Je, Ve) => I + Ve), ve = e.polylines.rawVal;
      e.polylines.val = [...ve.slice(0, -1), pe, []];
    }
  }, window.__hekatanDrawSlabChaflan = (t, o, s = 1, n = 6, a = 6) => {
    const i = Math.min(t[0], o[0]), r = Math.max(t[0], o[0]), f = Math.min(t[1], o[1]), m = Math.max(t[1], o[1]), y = (t[2] + o[2]) / 2, M = r - i, S = m - f, u = Math.min(s, M / 2 - 0.01, S / 2 - 0.01);
    if (u <= 0) return;
    const F = e.points.rawVal.length, j = [], J = [], X = (v, A) => {
    ...
    if (J.push(F), e.points.val = [...e.points.rawVal, ...j], e.polylines) {
      const v = e.polylines.rawVal;
      e.polylines.val = [...v.slice(0, -1), J, []];
    }
  }, window.__hekatanDrawRect = (t, o) => {
    const s = e.points.rawVal.length, n = t[0], a = t[1], i = t[2], r = o[0], f = o[1], m = o[2];
    let y;
    if (Math.abs(i - m) < 1e-6 ? y = [[n, a, i], [r, a, i], [r, f, i], [n, f, i]] : Math.abs(a - f) < 1e-6 ? y = [[n, a, i], [r, a, i], [r, a, m], [n, a, m]] : y = [[n, a, i], [n, f, i], [n, f, m], [n, a, m]], e.points.val = [...e.points.rawVal, ...y], e.polylines) {
      const M = [s, s + 1, s + 2, s + 3, s], S = e.polylines.rawVal;
      e.polylines.val = [...S.slice(0, -1), M, []];
    }
    ...
    let y;
    if (E && e.gridTarget) {
      const M = e.gridTarget.rawVal, S = new Fn(...M.rotation), u = new _(1, 0, 0).applyEuler(S), F = new _(0, 1, 0).applyEuler(S), j = new _(...M.position), J = new _(n, a, i), X = new _(r, f, m), v = J.clone().sub(j).dot(u), A = J.clone().sub(j).dot(F), C = X.clone().sub(j).dot(u), D = X.clone().sub(j).dot(F), q = (I, G) => j.clone().addScaledVector(u, I).addScaledVector(F, G).toArray();
    ...
    if (window.__hekatanPushUndo && window.__hekatanPushUndo(), e.points.val = [...e.points.rawVal, ...y], e.polylines) {
      const M = e.polylines.rawVal, S = M.length - 1, u = [s, s + 1, s + 2, s + 3, s];
      e.polylines.val = [...M.slice(0, -1), u, []], e.areas && (e.areas.val = [...e.areas.rawVal, S]);
    }
    try {
      (_a2 = window.__hekatanRebuild) == null ? void 0 : _a2.call(window);
    } catch {
    ...
    for (let xe = 0; xe < s; xe++) {
      const Ee = t[xe], qe = t[(xe + 1) % s];
      n += (Ee[1] - qe[1]) * (Ee[2] + qe[2]), a += (Ee[2] - qe[2]) * (Ee[0] + qe[0]), i += (Ee[0] - qe[0]) * (Ee[1] + qe[1]);
    }
    const r = Math.hypot(n, a, i) || 1;
    n /= r, a /= r, i /= r;
    let f = t[1][0] - t[0][0], m = t[1][1] - t[0][1], y = t[1][2] - t[0][2];
    const M = Math.hypot(f, m, y) || 1;
    f /= M, m /= M, y /= M;
    let S = a * y - i * m, u = i * f - n * y, F = n * m - a * f;
    const j = Math.hypot(S, u, F) || 1;
    S /= j, u /= j, F /= j;
    const J = t[0], X = (xe) => [(xe[0] - J[0]) * f + (xe[1] - J[1]) * m + (xe[2] - J[2]) * y, (xe[0] - J[0]) * S + (xe[1] - J[1]) * u + (xe[2] - J[2]) * F], v = (xe, Ee) => [J[0] + xe * f + Ee * S, J[1] + xe * m + Ee * u, J[2] + xe * y + Ee * F], A = t.map(X);
    let C = 1 / 0, D = -1 / 0, q = 1 / 0, I = -1 / 0;
    for (const [xe, Ee] of A) xe < C && (C = xe), xe > D && (D = xe), Ee < q && (q = Ee), Ee > I && (I = Ee);
    const G = D - C, de = I - q;
    if (G < 1e-6 || de < 1e-6) return 0;
    let pe = o && o > 0 ? o : 0.5;
    for (; G / pe * (de / pe) > 2500; ) pe *= 2;
    pe = Math.min(pe, Math.min(G, de));
    const ve = (xe, Ee) => {
      let qe = false;
      ...
      return qe;
    }, Je = Math.max(1, Math.round(G / pe)), Ve = Math.max(1, Math.round(de / pe)), je = G / Je, et = de / Ve, dt = /* @__PURE__ */ new Map(), Ue = [], Fe = e.points.rawVal.length, We = (xe, Ee) => {
    ...
    if (window.__hekatanPushUndo && window.__hekatanPushUndo(), e.points.val = [...e.points.rawVal, ...Ue], e.polylines && e.areas) {
      ...
      xe.push([]), e.polylines.val = xe, e.areas.val = [...e.areas.rawVal, ...Ee];
    }
    try {
      (_a2 = window.__hekatanRebuild) == null ? void 0 : _a2.call(window);
    } catch {
  ...
  const dn = () => {
    ...
    return we = [], O.visible = false, x(), t;
  };
  window.__hekatanFinalizePolyArea = dn, window.__hekatanSetInclinedPlaneFrom3 = (t, o, s) => {
    var _a2;
    const n = new _(t[0], t[1], t[2]), a = new _(o[0], o[1], o[2]), i = new _(s[0], s[1], s[2]), r = new _().subVectors(a, n).cross(new _().subVectors(i, n));
    if (r.lengthSq() < 1e-9) return false;
    r.normalize();
    const f = new Kn().setFromUnitVectors(new _(0, 0, 1), r), m = new Fn().setFromQuaternion(f);
    e.gridTarget && (e.gridTarget.val = { position: [n.x, n.y, n.z], rotation: [m.x, m.y, m.z] }), E = true;
    ...
    e.gridTarget && (e.gridTarget.val = { position: [0, 0, 0], rotation: [Math.PI / 2, 0, 0] }), E = false, ye.visible = false, x();
  };
  const qt = new tt();
  qt.visible = false, d.add(qt), window.__hekatanShowAxes = (t, o, s = 12, n = 2) => {
    var _a2, _b;
    ...
    const a = Math.min(...o) - n, i = Math.max(...o) + n, r = Math.min(...t) - n, f = Math.max(...t) + n, m = "ABCDEFGHIJKLMNOPQRSTUVWXYZ", y = (M, S, u, F, j) => {
  ...
  W.derive(() => {
    ...
    (t == null ? void 0 : t.val) && (t.val, ks(), x());
  });
  const un = new tt();
  ...
  W.derive(() => {
    ...
    (t == null ? void 0 : t.val) !== void 0 && (t.val, To(), x());
  }), c.addEventListener("change", () => {
    un.children.forEach((t) => {
  ...
  const Rn = (t) => {
    ...
    const t = ao * Rn(yt.position) / 0.015;
    yt.scale.setScalar(Math.max(1e-4, Math.min(1e5, t)));
  };
  let _n = 10;
  const io = (t) => Math.max(1e-4, _n * Rn(t));
  window.__hekatanAperturaPx = (t) => (typeof t == "number" && t > 0 && (_n = t), _n), window.__hekatanUpdateSnapScale = bn, window.__hekatanSnapMarker = yt, window.__hekatanMetrosPorPixel = Rn, window.__hekatanSnapPx = (t) => (typeof t == "number" && t > 0 && (ao = t, bn(), x()), ao);
  const $o = () => {
  ...
  window.__hekatanUpdateSelectionPtScale = $o, c.addEventListener("change", () => {
    ...
    yt.position.set(t, o, s), yt.visible = true, bn(), x();
    ...
    const s = ie();
    if (s.length) {
      const n = s[0].point, a = t.altKey, i = io(n), r = a ? null : (_a2 = window.__hekatanOsnapCompute) == null ? void 0 : _a2.call(window, n.x, n.y, n.z, i, { x: t.clientX, y: t.clientY });
      if (r) Bo(r.type, r.x, r.y, r.z), yt.position.set(r.x, r.y, r.z), yt.visible = true, n.set(r.x, r.y, r.z), Xo(r.type, t.clientX, t.clientY);
      else {
        Es(), Xn();
      ...
      if (f === "select" || !f) {
        ...
        if (u >= 0) {
          ...
          const A = e.points.rawVal, C = e.polylines.rawVal[F.polyIdx], D = A[C[F.segIdx]], q = A[C[F.segIdx + 1]];
          zt.geometry.setFromPoints([new _(D[0], D[1], D[2]), new _(q[0], q[1], q[2])]), zt.visible = true, vt.visible = false, Ut = ((_f = (_e2 = e.areas) == null ? void 0 : _e2.rawVal) == null ? void 0 : _f.includes(F.polyIdx)) ?? false ? { kind: "poly", a: F.polyIdx } : { kind: "seg", a: F.polyIdx, b: F.segIdx };
        } else if (j >= 0) {
        ...
        v && (v.textContent = X), Ne = { p: J.clone(), x: t.clientX, y: t.clientY }, fe.visible = false, ht.visible = false, re.visible = false, x();
        return;
      }
      if (f === "delete" || f === "trim" || f === "extend" || f === "offset") {
        ...
        j ? (wt = F, De = -1, Ge = -1, no(F)) : u ? (De = u.polyIdx, Ge = u.segIdx, wt = -1, oo(u.polyIdx, u.segIdx)) : (De = -1, Ge = -1, wt = -1, it.visible = false), fe.visible = false, ht.visible = false, re.visible = false, Y(), se.style.left = t.clientX + "px", se.style.top = t.clientY + "px", se.style.display = "block";
        const J = `X=${n.x.toFixed(2)} Y=${n.y.toFixed(2)} Z=${n.z.toFixed(2)}`;
        let X = "";
        j ? X = `\u{1F5D1} l\xEDnea aux #${wt + 1}` : u ? X = ((_i = (_h = e.areas) == null ? void 0 : _h.rawVal) == null ? void 0 : _i.includes(u.polyIdx)) ?? false ? `\u{1F5D1} \xE1rea #${u.polyIdx + 1}` : `\u{1F5D1} seg ${u.segIdx + 1} / poly #${u.polyIdx + 1}` : X = "\u{1F5D1} acerc\xE1 a l\xEDnea/\xE1rea", se.textContent = `${J}  \xB7  ${X}`;
        const v = document.getElementById("hk-coord-fixed");
        v && (v.textContent = J), x();
        return;
      ...
      if (y.length > 0 && M[y[y.length - 1]]) {
        ...
        if (Nt = null, !F && window.__hekatanAxisSnap !== false) {
          const Ve = g.getBoundingClientRect(), je = t.clientX, et = t.clientY, dt = ((_k = settings.gridSize) == null ? void 0 : _k.rawVal) ?? 10, Ue = new _(u[0], u[1], u[2]), Fe = [["x", new _(1, 0, 0)], ["y", new _(0, 1, 0)], ["z", new _(0, 0, 1)]], We = (xe) => {
        ...
        if (F) {
          ...
          Ce.textContent = dt ? `\u{1F512} LOCK ${F.toUpperCase()}${xe}` : `\u22A5 ORTO ${F.toUpperCase()}${xe}`, Ce.style.left = t.clientX + 20 + "px", Ce.style.top = t.clientY + 18 + "px", Ce.style.transform = "none", Ce.style.display = "block";
        } else at || (Ce.style.display = "none");
        let X = null;
        if (!a && window.__hekatanTrack !== false && window.__hekatanOsnapOn !== false) {
          const Ve = e.points.rawVal, je = F ? [F] : ["z", "x", "y"], et = { x: t.clientX, y: t.clientY };
          let dt = 1 / 0;
        ...
        X ? (X.eje === "x" ? n.x = X.q[0] : X.eje === "y" ? n.y = X.q[1] : n.z = X.q[2], re.geometry.setFromPoints([new _(X.q[0], X.q[1], X.q[2]), new _(n.x, n.y, n.z)]), (_n2 = re.computeLineDistances) == null ? void 0 : _n2.call(re), re.visible = true, yt.position.set(n.x, n.y, n.z), yt.visible = true, Xo("track", t.clientX, t.clientY)) : re.visible = false, Ne = { p: n.clone(), x: t.clientX, y: t.clientY };
        ...
        D && (D.textContent = C), fe.geometry.setFromPoints([new _(u[0], u[1], u[2]), new _(n.x, n.y, n.z)]), (_o2 = fe.computeLineDistances) == null ? void 0 : _o2.call(fe), fe.visible = true, Z(u[0], u[1], u[2], n.x, n.y, n.z);
        ...
        if (G.length > 0) {
          const Ve = G[0].object;
          Ve === Ye ? de = "xy" : Ve === rt ? de = "xz" : Ve === Le && (de = "yz");
        }
        cn(de), de && (Oe.style.left = t.clientX + "px", Oe.style.top = t.clientY + "px"), P.geometry.setFromPoints([new _(u[0] - q, u[1], u[2]), new _(u[0] + q, u[1], u[2])]), (_p = P.computeLineDistances) == null ? void 0 : _p.call(P), L.geometry.setFromPoints([new _(u[0], u[1] - q, u[2]), new _(u[0], u[1] + q, u[2])]), (_q = L.computeLineDistances) == null ? void 0 : _q.call(L), Q.geometry.setFromPoints([new _(u[0], u[1], u[2] - q), new _(u[0], u[1], u[2] + q)]), (_r = Q.computeLineDistances) == null ? void 0 : _r.call(Q), ht.visible = true;
        const pe = P.material, ve = L.material, Je = Q.material;
        F === "x" ? (pe.opacity = 0.95, ve.opacity = 0.1, Je.opacity = 0.1) : F === "y" ? (pe.opacity = 0.1, ve.opacity = 0.95, Je.opacity = 0.1) : F === "z" ? (pe.opacity = 0.1, ve.opacity = 0.1, Je.opacity = 0.95) : (pe.opacity = 0.5, ve.opacity = 0.5, Je.opacity = 0.5);
      } else {
        const S = `X=${n.x.toFixed(2)} Y=${n.y.toFixed(2)} Z=${n.z.toFixed(2)}`;
        se.textContent = S;
        const u = document.getElementById("hk-coord-fixed");
        if (u && (u.textContent = S), fe.visible = false, ht.visible = false, (/* @__PURE__ */ new Set(["line", "polyline", "area", "node", "column", "wall", "rect", "circle", "arc", "polyline-multi", "axis", "chaflan"])).has(f)) {
          if (B = null, K = null, U.style.left = t.clientX + 20 + "px", U.style.top = t.clientY - 28 + "px", U.style.display = "block", !T) {
      ...
      x();
    } else Xn(), se.style.display = "none", yt.visible = false, fe.visible = false, ht.visible = false, Y(), x();
  }), W.derive(() => {
    var _a2;
    if (!e.gridTarget) return;
    const t = new Kn().setFromEuler(new Fn(...e.gridTarget.val.rotation)), o = new Kn().setFromAxisAngle(new _(1, 0, 0), Math.PI / 2);
    Fa(l, { position: new _(...e.gridTarget.val.position), quaternion: t.clone().multiply(o) }, x);
    {
      const n = e.gridTarget.val.position[2], a = Math.abs(t.x - Math.sin(Math.PI / 4)) < 1e-3;
      for (const i of N) d.remove(i), me(i);
      if (N.length = 0, a) {
    ...
    H.position.set(...e.gridTarget.val.position), H.quaternion.setFromEuler(new Fn(...e.gridTarget.val.rotation)), H.updateMatrixWorld();
    const s = new _(0, 0, 1).applyEuler(new Fn(...e.gridTarget.val.rotation));
    E = !(Math.abs(s.x) > 0.999 || Math.abs(s.y) > 0.999 || Math.abs(s.z) > 0.999);
  }), W.derive(() => {
    ee.geometry.setAttribute("position", new bt(e.points.val.flat(), 3)), ee.geometry.computeBoundingSphere();
  }), W.derive(() => {
  ...
  const ro = (t, o, s, n, a) => {
    ...
    var _a2, _b, _c, _d;
    const i = Math.min(t, s), r = Math.max(t, s), f = Math.min(o, n), m = Math.max(o, n), y = s < t, M = g.getBoundingClientRect(), S = h();
    S.updateMatrixWorld();
    const u = (I) => {
    ...
    a || ke.clear();
    let J = 0;
    const X = ((_a2 = e.points) == null ? void 0 : _a2.rawVal) ?? [];
    for (let I = 0; I < X.length; I++) {
      const G = X[I];
      G && F(u(G)) && (ke.add(`pt:${I}`), J++);
    }
    const v = (I, G) => y ? F(I) || F(G) || j(I, G) : F(I) && F(G), A = ((_b = e.polylines) == null ? void 0 : _b.rawVal) ?? [], C = ((_c = e.areas) == null ? void 0 : _c.rawVal) ?? [];
    for (let I = 0; I < A.length; I++) {
      ...
      if (C.includes(I)) {
        ...
        pe && (ke.add(`poly:${I}`), J++);
      } else for (let pe = 0; pe < G.length - 1; pe++) {
        const ve = X[G[pe]], Je = X[G[pe + 1]];
        !ve || !Je || v(u(ve), u(Je)) && (ke.add(`seg:${I}:${pe}`), J++);
      }
    }
    ...
    for (let I = 0; I < q.length; I++) {
      ...
      const de = u([G[0], G[1], G[2]]), pe = u([G[3], G[4], G[5]]);
      v(de, pe) && (ke.add(`aux:${I}`), J++);
    }
  ...
  window.__hekatanCancelClickClickRect = Dn, window.addEventListener("keydown", (t) => {
    t.key === "Escape" && Bt && Dn();
  });
  const Io = () => {
    var _a2, _b, _c, _d;
    if (ke.size === 0) return false;
    const t = [...ke], o = ((_a2 = e.points) == null ? void 0 : _a2.rawVal) ?? [], s = ((_b = e.polylines) == null ? void 0 : _b.rawVal) ?? [], n = ((_c = e.areas) == null ? void 0 : _c.rawVal) ?? [], a = window.__hekatanDrawingAuxLines, i = (a == null ? void 0 : a.rawVal) ?? [], r = /* @__PURE__ */ new Set(), f = /* @__PURE__ */ new Set(), m = /* @__PURE__ */ new Map(), y = /* @__PURE__ */ new Set();
    for (const j of t) {
      const [J, ...X] = j.split(":");
      if (J === "pt") r.add(+X[0]);
    ...
    if (e.polylines && (e.polylines.val = S), e.areas && (e.areas.val = u), y.size > 0 && a) {
  ...
  window.__hekatanDeleteSelected = Io, window.addEventListener("keydown", (t) => {
    if (t.key !== "Delete" && t.key !== "Backspace") return;
    const o = document.activeElement, s = o && (o.id === "hk3-cmd-input" || o.id === "hk-dyn-input") && o.value === "";
    o && (o.tagName === "INPUT" || o.tagName === "TEXTAREA" || o.isContentEditable) && !s || ke.size !== 0 && (t.preventDefault(), Io());
  });
  ...
  const Mt = (t, o, s, n) => {
    ...
    const t = [...ke], o = t.filter((S) => S.startsWith("pt:")), s = t.filter((S) => S.startsWith("seg:")), n = t.filter((S) => S.startsWith("poly:")), a = t.filter((S) => S.startsWith("aux:")), i = o.length > 0, r = s.length > 0, f = n.length > 0, m = !i && !r && !f, y = [];
    ...
    if (r) {
      ...
      C.addBinding(te, "qx", { step: 0.1 }), C.addBinding(te, "qy", { step: 0.1 }), C.addBinding(te, "qz", { step: 0.1 }), lt.addFolder({ title: "\u2696 Additional Mass (kg/m)", expanded: false }).addBinding(te, "massPerM", { label: "m/L", min: 0, step: 1 }), lt.addButton({ title: "\u2713 Aplicar a segmentos seleccionados" }).on("click", () => {
        ...
        const de = [te.LKx, te.LKy, te.LKz];
        de.some((ve) => ve !== 0) && Mt("segs", s, "lineSprings", de);
        const pe = [te.qx, te.qy, te.qz];
        pe.some((ve) => ve !== 0) && Mt("segs", s, "distLoad", pe), te.massPerM !== 0 && Mt("segs", s, "massPerM", te.massPerM), oe(`\u2713 Propiedades aplicadas a ${s.length} segmento(s)`);
      });
    }
    if (f) {
  ...
  g.addEventListener("pointerdown", (t) => {
    t.button === 2 && (hn = { x: t.clientX, y: t.clientY }, Bn = false);
  }), g.addEventListener("pointermove", (t) => {
    if (hn && t.buttons & 2 && !Bn) {
      const o = t.clientX - hn.x, s = t.clientY - hn.y;
    ...
    if (t.button === 2) {
      ...
      if (o) {
        if (Bt ? Dn() : window.dispatchEvent(new KeyboardEvent("keydown", { key: "Escape", bubbles: true })), ke.size > 0 && (ke.clear(), At()), e.polylines) {
    ...
    if (Bt && t.buttons === 0) {
      ...
      ro(Bt.x, Bt.y, t.clientX, t.clientY, i);
      return;
    }
    if (!Kt) return;
    ...
    ro(Kt.x, Kt.y, t.clientX, t.clientY, a);
  }), g.addEventListener("pointerup", (t) => {
    if (!Kt) return;
    if (!Sn) {
      Kt = null;
    ...
    Lo(Kt.x, Kt.y, t.clientX, t.clientY, o), Kt = null, Sn = false;
  ...
  const Do = { end: 16724804, mid: 16498468, node: 6333946, cen: 3462041, per: 12616956, nea: 16744118, int: 16746496, ori: 16777215, grid: 2282478, track: 16761856 }, Bo = (t, o, s, n) => {
    var _a2, _b, _c, _d;
    for (; Zt.children.length; ) {
      const r = Zt.children.pop();
      (_b = (_a2 = r.geometry) == null ? void 0 : _a2.dispose) == null ? void 0 : _b.call(_a2), (_d = (_c = r.material) == null ? void 0 : _c.dispose) == null ? void 0 : _d.call(_c);
    }
    const a = Do[t] ?? 16777215, i = new Me().setFromPoints([new _(-1, -1, 0), new _(1, -1, 0), new _(1, -1, 0), new _(1, 1, 0), new _(1, 1, 0), new _(-1, 1, 0), new _(-1, 1, 0), new _(-1, -1, 0)]);
  ...
  window.__hekatanOsnapMarkerRef = Zt, window.__hekatanUpdateOsnapScale = po, window.__hekatanOsnapPx = (t) => (typeof t == "number" && t > 0 && (co = t, po(), x()), co);
  const Xn = () => {
    Zt.visible = false;
  ...
  const Xo = (t, o, s) => {
    ...
    Wt.textContent = n, Wt.style.color = "#" + (Do[t] ?? 16777215).toString(16).padStart(6, "0"), Wt.style.left = o + 18 + "px", Wt.style.top = s - 26 + "px", Wt.style.display = "block";
  }, Es = () => {
    Wt.style.display = "none";
  }, Pn = new _(), uo = (t, o, s) => {
    const n = h();
    ...
    return Pn.set(t, o, s).project(n), !isFinite(Pn.x) || !isFinite(Pn.y) ? null : { x: a.left + (Pn.x * 0.5 + 0.5) * a.width, y: a.top + (-Pn.y * 0.5 + 0.5) * a.height };
  };
  ...
  const Vs = (t, o, s, n, a) => {
    ...
    const y = { ori: 0, end: 0, node: 0, int: 1, grid: 2, mid: 2, cen: 3, per: 4, nea: 5 }, M = a, S = (v, A, C, D) => {
      ...
      } else if (q = Math.hypot(A - t, C - o, D - s), q > n) return;
      const I = y[v] ?? 9;
      (!m || I < m.r || I === m.r && q < m.d) && (m = { type: v, x: A, y: C, z: D, d: q, r: I });
    };
    if (i.ori !== false && S("ori", 0, 0, 0), i.grid !== false && window.__hekatanSnapEnabled === true) {
      const v = window.__hekatanGridConfig, A = (v == null ? void 0 : v.minorStep) && v.minorStep > 0 ? v.minorStep : 1, C = ((v == null ? void 0 : v.gridSize) ?? 30) / 2, D = ((_d = (_c = (_b = window.__hekatanCadState) == null ? void 0 : _b.get) == null ? void 0 : _c.call(_b)) == null ? void 0 : _d.workPlane) ?? "xy", q = (G) => Math.round(G / A) * A, I = (G, de) => Math.abs(G) <= C + 1e-9 && Math.abs(de) <= C + 1e-9;
      if (D === "xz") {
        const G = q(t), de = q(s);
        I(G, de) && S("grid", G, o, de);
      } else if (D === "yz") {
        const G = q(o), de = q(s);
        I(G, de) && S("grid", t, G, de);
      } else {
        const G = q(t), de = q(o);
        I(G, de) && S("grid", G, de, s);
      }
    }
    ...
    for (const v of f) if (!(v.length < 2)) for (let A = 0; A < v.length - 1; A++) {
      ...
      if (!(!C || !D) && (i.end && (S("end", C[0], C[1], C[2]), S("end", D[0], D[1], D[2])), i.mid && S("mid", (C[0] + D[0]) / 2, (C[1] + D[1]) / 2, (C[2] + D[2]) / 2), i.nea || i.per)) {
        const q = D[0] - C[0], I = D[1] - C[1], G = D[2] - C[2], de = q * q + I * I + G * G;
        if (de < 1e-12) continue;
        const pe = Math.max(0, Math.min(1, ((t - C[0]) * q + (o - C[1]) * I + (s - C[2]) * G) / de)), ve = C[0] + pe * q, Je = C[1] + pe * I, Ve = C[2] + pe * G;
    ...
    if (i.cen) {
      ...
      for (const A of v) {
        ...
        let q = 0, I = 0, G = 0, de = 0;
        for (const pe of D) {
          const ve = r[pe];
          ve && (q += ve[0], I += ve[1], G += ve[2], de++);
        }
        de >= 3 && S("cen", q / de, I / de, G / de);
      }
    }
    if (i.cen) {
      const v = vn(), A = [...on];
    ...
    if (i.int) {
      ...
      for (const A of f) for (let C = 0; C < A.length - 1; C++) {
        ...
        if (!D || !q) continue;
        const I = q[0] - D[0], G = q[1] - D[1], de = q[2] - D[2], pe = I * I + G * G + de * de;
        if (pe < 1e-12) continue;
        const ve = Math.max(0, Math.min(1, ((t - D[0]) * I + (o - D[1]) * G + (s - D[2]) * de) / pe));
        Math.hypot(D[0] + ve * I - t, D[1] + ve * G - o, D[2] + ve * de - s) < 3 * n && v.push([D, q]);
      }
      for (let A = 0; A < v.length; A++) for (let C = A + 1; C < v.length; C++) {
        const [D, q] = v[A], [I, G] = v[C], de = [q[0] - D[0], q[1] - D[1], q[2] - D[2]], pe = [G[0] - I[0], G[1] - I[1], G[2] - I[2]], ve = [D[0] - I[0], D[1] - I[1], D[2] - I[2]], Je = de[0] * de[0] + de[1] * de[1] + de[2] * de[2], Ve = de[0] * pe[0] + de[1] * pe[1] + de[2] * pe[2], je = pe[0] * pe[0] + pe[1] * pe[1] + pe[2] * pe[2], et = de[0] * ve[0] + de[1] * ve[1] + de[2] * ve[2], dt = pe[0] * ve[0] + pe[1] * ve[1] + pe[2] * ve[2], Ue = Je * je - Ve * Ve;
        if (Ue < 1e-12) continue;
        const Fe = (Ve * dt - je * et) / Ue, We = (Je * dt - Ve * et) / Ue;
        if (Fe < -1e-6 || Fe > 1 + 1e-6 || We < -1e-6 || We > 1 + 1e-6) continue;
        const Te = [D[0] + Fe * de[0], D[1] + Fe * de[1], D[2] + Fe * de[2]], xe = [I[0] + We * pe[0], I[1] + We * pe[1], I[2] + We * pe[2]];
    ...
    for (const [v, A] of j) {
      ...
      if (I < 1e-12) continue;
      const G = Math.max(0, Math.min(1, ((t - v[0]) * C + (o - v[1]) * D + (s - v[2]) * q) / I));
      if (i.nea && S("nea", v[0] + G * C, v[1] + G * D, v[2] + G * q), i.int && Math.abs(q) > 1e-9) for (const de of F) {
        const pe = (de.z - v[2]) / q;
        pe < -1e-6 || pe > 1 + 1e-6 || S("int", v[0] + pe * C, v[1] + pe * D, de.z);
      }
    }
    if (i.int || i.node) for (let v = 0; v < j.length; v++) for (let A = v + 1; A < j.length; A++) {
      const [C, D] = j[v], [q, I] = j[A], G = D[0] - C[0], de = D[1] - C[1], pe = I[0] - q[0], ve = I[1] - q[1], Je = G * ve - de * pe;
      if (Math.abs(Je) < 1e-12) continue;
      const Ve = C[0] - q[0], je = C[1] - q[1], et = (pe * je - ve * Ve) / Je, dt = (G * je - de * Ve) / Je;
      ...
      S("int", C[0] + et * G, C[1] + et * de, typeof Ue == "number" ? Ue : s);
    }
    ...
    for (const v of X) {
      ...
      if (i.end && (S("end", A[0], A[1], A[2]), S("end", C[0], C[1], C[2])), i.mid && S("mid", (A[0] + C[0]) / 2, (A[1] + C[1]) / 2, (A[2] + C[2]) / 2), i.nea || i.per) {
        ...
        if (G < 1e-12) continue;
        const de = Math.max(0, Math.min(1, ((t - A[0]) * D + (o - A[1]) * q + (s - A[2]) * I) / G)), pe = A[0] + de * D, ve = A[1] + de * q, Je = A[2] + de * I;
        i.nea && S("nea", pe, ve, Je), i.per && S("per", pe, ve, Je);
      }
    }
  ...
  const Uo = () => {
    var _a2, _b;
    for (const t of mn.children.slice()) mn.remove(t), (_b = (_a2 = t.geometry) == null ? void 0 : _a2.dispose) == null ? void 0 : _b.call(_a2);
  };
  ...
  const wn = [], Yn = [], ho = () => {
    ...
    var _a2;
    e.points.val = t.p, e.polylines && (e.polylines.val = t.l), e.areas && (e.areas.val = t.a), $e = [], fe.visible = false, ht.visible = false, Y();
    try {
      (_a2 = window.__hekatanRebuild) == null ? void 0 : _a2.call(window);
    } catch {
    }
    ...
    Yn.push(ho()), Zo(t), oe(`\u21B6 Deshacer \u2014 quedan ${wn.length}`);
  }, qo = () => {
    const t = Yn.pop();
    if (!t) {
    ...
    wn.push(ho()), Zo(t), oe(`\u21B7 Rehacer \u2014 quedan ${Yn.length}`);
  };
  window.__hekatanPushUndo = Yt, window.__hekatanUndo = Nn, window.__hekatanRedo = qo, document.addEventListener("keydown", (t) => {
    var _a2;
    const o = t.key.toLowerCase();
    if (!((t.ctrlKey || t.metaKey) && (o === "y" || o === "z" && t.shiftKey))) return;
    const n = t.target;
    n && (n.tagName === "INPUT" || n.tagName === "TEXTAREA") && n.type !== "checkbox" && n.type !== "range" && (((_a2 = n.value) == null ? void 0 : _a2.length) ?? 0) > 0 || (t.preventDefault(), t.stopPropagation(), qo());
    ...
    if (o === "u" || o === "deshacer" || o === "undo") {
      ...
      let m = e.points.rawVal, y = [...n.slice(0, -1), r];
      if (!f && i === m.length - 1 && (m = m.slice(0, -1), e.points.val = m), e.polylines.val = y, r.length) {
        const M = m[r[r.length - 1]];
        M && (B = [M[0], M[1], M[2]]);
      } else B = null, fe.visible = false;
      try {
    ...
    if ((t.ctrlKey || t.metaKey) && t.key.toLowerCase() === "z" && !t.shiftKey) {
      ...
      t.preventDefault(), t.stopPropagation(), Nn();
    }
  }, { capture: true });
  const mo = () => {
    $e = [], _t = null, fo(), at = null, Ae(), fe.visible = false, ht.visible = false, Y(), oe("\u23F9 Dibujo finalizado \u2014 click para empezar otra serie"), x(), Xt();
  };
  ...
  const Ko = () => {
    ...
    oe(t ? "\u238B Selecci\xF3n cancelada" : "\u238B Sin herramienta \u2014 clic-clic para seleccionar por ventana"), x(), Xt();
  };
  ...
  const Wo = (t, o) => {
    ...
    if ($e.push(o), $e.length === 1) {
      B = o, oe(`${t === "move" ? "MOVER" : "COPIAR"} punto base (${o[0].toFixed(2)}, ${o[1].toFixed(2)}, ${o[2].toFixed(2)}). Precise el segundo punto.`), Xt();
      return;
    }
    const [s, n] = $e, a = [n[0] - s[0], n[1] - s[1], n[2] - s[2]];
    $e = [], fe.visible = false;
    let i = 0;
    t === "move" ? i = Ho(a[0], a[1], a[2]) : (i = Go().size, (_c = window.__hekatanReplicateSelection) == null ? void 0 : _c.call(window, a[0], a[1], a[2], 1)), oe(`\u2713 ${t === "move" ? "Movidos" : "Copiados"} ${i} nudo${i === 1 ? "" : "s"} \u2014 \u0394 (${a[0].toFixed(2)}, ${a[1].toFixed(2)}, ${a[2].toFixed(2)}) m.`), t === "move" && (ke.clear(), At()), (_e2 = (_d = window.__hekatanCadState) == null ? void 0 : _d.setTool) == null ? void 0 : _e2.call(_d, "select"), Xt();
  };
  ...
  const Is = () => {
  ...
  }, Qt = (t, o) => Math.hypot(t[0] - o[0], t[1] - o[1], t[2] - o[2]), wo = (t, o, s, n, a, i) => {
    const r = [o[0] - t[0], o[1] - t[1], o[2] - t[2]], f = [n[0] - s[0], n[1] - s[1], n[2] - s[2]], m = [t[0] - s[0], t[1] - s[1], t[2] - s[2]], y = r[0] * r[0] + r[1] * r[1] + r[2] * r[2], M = r[0] * f[0] + r[1] * f[1] + r[2] * f[2], S = f[0] * f[0] + f[1] * f[1] + f[2] * f[2], u = r[0] * m[0] + r[1] * m[1] + r[2] * m[2], F = f[0] * m[0] + f[1] * m[1] + f[2] * m[2], j = y * S - M * M;
    if (j < 1e-12) return null;
    const J = (M * F - S * u) / j, X = (y * F - M * u) / j;
    if (!a && (J < -1e-6 || J > 1 + 1e-6) || !i && (X < -1e-6 || X > 1 + 1e-6)) return null;
    const v = [t[0] + J * r[0], t[1] + J * r[1], t[2] + J * r[2]], A = [s[0] + X * f[0], s[1] + X * f[1], s[2] + X * f[2]];
    return Qt(v, A) > 1e-4 ? null : v;
  }, Rs = (t) => {
    var _a2;
    return (((_a2 = e.polylines) == null ? void 0 : _a2.rawVal) ?? []).reduce((o, s) => o + s.filter((n) => n === t).length, 0);
  }, Ds = { offset: "DESFASE", trim: "RECORTAR", extend: "ALARGAR" }, Bs = (t, o) => {
    var _a2, _b;
    if (!e.polylines) return;
    const s = e.polylines.rawVal, n = e.points.rawVal, a = Ds[t];
    if (!_t) {
      if (De < 0) {
        oe(`${a}: pase el cursor por una l\xEDnea (se pone roja) y haga clic.`);
        return;
      }
      _t = { poly: De, seg: Math.max(0, Ge) }, oe(t === "offset" ? `DESFASE l\xEDnea #${_t.poly + 1} designada \u2014 clic en el lado hacia el que va la copia${an > 0 ? ` (${an} m)` : ""}.` : t === "trim" ? "RECORTAR contorno designado \u2014 clic en el trozo de l\xEDnea a quitar." : "ALARGAR contorno designado \u2014 clic en la l\xEDnea a alargar, cerca del extremo libre."), Xt();
      return;
    }
    if (t === "offset") {
      ...
      const I = C[D], G = Math.sign((o[0] - I.a[0]) * I.n[0] + (o[1] - I.a[1]) * I.n[1] + (o[2] - I.a[2]) * I.n[2]) || 1, de = an > 0 ? an : q;
      if (de < 1e-6) {
        oe("DESFASE: distancia nula \u2014 teclee una distancia o clique m\xE1s lejos.");
        return;
      }
      const pe = C.map((Fe) => ({ a: [Fe.a[0] + G * de * Fe.n[0], Fe.a[1] + G * de * Fe.n[1], Fe.a[2] + G * de * Fe.n[2]], b: [Fe.b[0] + G * de * Fe.n[0], Fe.b[1] + G * de * Fe.n[1], Fe.b[2] + G * de * Fe.n[2]] })), ve = pe.length, Je = (Fe) => {
        const We = pe[(Fe - 1 + ve) % ve], Te = pe[Fe % ve];
        return wo(We.a, We.b, Te.a, Te.b, true, true) ?? Te.a;
      }, Ve = [], je = v ? ve : ve + 1;
      for (let Fe = 0; Fe < je; Fe++) !v && Fe === 0 ? Ve.push(pe[0].a) : !v && Fe === ve ? Ve.push(pe[ve - 1].b) : Ve.push(Je(Fe));
      Yt();
      const et = n.length;
      e.points.val = [...n, ...Ve];
      ...
      v && dt.push(et);
      let Ue = s.slice();
      Ue.length && Ue[Ue.length - 1].length === 0 && (Ue = Ue.slice(0, -1)), e.polylines.val = [...Ue, dt, []], _t = null, oe(`\u2713 Desfase a ${de.toFixed(2)} m \u2014 ${ve} tramo${ve === 1 ? "" : "s"} nuevo${ve === 1 ? "" : "s"}. Designe otra l\xEDnea o Esc.`);
      try {
        (_a2 = window.__hekatanRebuild) == null ? void 0 : _a2.call(window);
      } catch {
      }
    ...
    if (t === "trim") {
      ...
      e.polylines.val = s.map((C, D) => D === i ? v : C);
      ...
      if (Yt(), Rs(A) > 1) {
        ...
        e.points.val = [...n, J];
        const q = M.slice();
        q[v] = D, e.polylines.val = s.map((I, G) => G === i ? q : I);
      } else e.points.val = n.map((D, q) => q === A ? J : D);
  ...
  window.__hekatanSelectionSize = () => ke.size, window.__hekatanSelectLast = () => {
    ...
    const i = [...ke], r = e.points.rawVal, f = ((_a2 = e.polylines) == null ? void 0 : _a2.rawVal) ?? [], m = new Set(((_b = e.areas) == null ? void 0 : _b.rawVal) ?? []), y = /* @__PURE__ */ new Set(), M = /* @__PURE__ */ new Set(), S = [];
    if (i.forEach((X) => {
      if (X.startsWith("pt:")) {
        const v = +X.slice(3);
        r[v] && y.add(v);
    ...
    const j = [...((_c = e.areas) == null ? void 0 : _c.rawVal) ?? []], J = [...y];
    for (let X = 1; X <= n; X++) {
      const v = a + X, A = t * v, C = o * v, D = s * v, q = /* @__PURE__ */ new Map();
      J.forEach((I) => {
        q.set(I, u.length), u.push([r[I][0] + A, r[I][1] + C, r[I][2] + D]);
      }), M.forEach((I) => {
        const G = f[I].map((pe) => q.has(pe) ? q.get(pe) : pe), de = F.length;
        F.push(G), m.has(I) && j.push(de);
      }), S.forEach(([I, G]) => {
    ...
    var _a2, _b;
    if (window.__hekatanCursorPx = { x: t.clientX, y: t.clientY }, sn > 5) {
      sn = 0;
      return;
    }
    sn = 0;
    ...
    let n = s[0].point;
    (t.ctrlKey || t.metaKey) && (n = new _(Math.round(s[0].point.x), Math.round(s[0].point.y), Math.round(s[0].point.z)));
    {
    ...
    if (Ne && Math.abs(t.clientX - Ne.x) <= 3 && Math.abs(t.clientY - Ne.y) <= 3) n = Ne.p.clone();
    else if (Nt) n = Nt.clone(), oe(`\u{1F4D0} Eje \u2192 (${n.x.toFixed(2)}, ${n.y.toFixed(2)}, ${n.z.toFixed(2)})`);
    else {
      const a = io(n), i = (_b = window.__hekatanOsnapCompute) == null ? void 0 : _b.call(window, n.x, n.y, n.z, a, { x: t.clientX, y: t.clientY });
  ...
  const Jo = (t, o) => {
    ...
    if (s === "delete") {
      if (wt >= 0) {
        ...
        const n = De, a = Ge;
        ((_g = (_f = e.areas) == null ? void 0 : _f.rawVal) == null ? void 0 : _g.includes(n)) ?? false ? (nn(n), oe(`\u{1F5D1} \xC1rea #${n + 1} (shell Q4) borrada`)) : a >= 0 ? ($n(n, a), oe(`\u{1F5D1} Segmento ${a + 1} de polil\xEDnea #${n + 1} borrado`)) : (nn(n), oe(`\u{1F5D1} Polil\xEDnea #${n + 1} borrada`));
    ...
    if (s === "polyarea") {
      we.push([t.x, t.y, t.z]), O.geometry.setFromPoints(we.map((n) => new _(n[0], n[1], n[2]))), O.visible = we.length >= 1, oe(`\u25B0 \xC1rea libre \u2014 ${we.length} punto(s). Click m\xE1s v\xE9rtices, o Enter / click-derecho para cerrar y mallar (m\xEDn. 3).`), x();
      return;
    }
    if (s === "plane3") {
    ...
    if (s === "col") {
      Yt();
      const n = t.z, a = xt && xt > 0 ? xt : 3;
      e.points.val = [...e.points.rawVal, [t.x, t.y, n], [t.x, t.y, n + a]];
      const i = e.polylines.rawVal, r = e.points.rawVal.length;
      e.polylines.val = [...i.slice(0, -1), ...i[i.length - 1].length > 0 ? [i[i.length - 1]] : [], [r - 2, r - 1], []], xt = 0, oe(`\u258C Columna creada \u2014 h=${a.toFixed(2)}m. Tipe\xE1 altura + Enter para custom.`);
      try {
    ...
    if (s === "extp") {
      Yt();
      const n = xt && xt > 0 ? xt : 3, a = t.z;
      e.points.val = [...e.points.rawVal, [t.x, t.y, a], [t.x, t.y, a + n]];
      const i = e.polylines.rawVal, r = e.points.rawVal.length;
      e.polylines.val = [...i.slice(0, -1), ...i[i.length - 1].length > 0 ? [i[i.length - 1]] : [], [r - 2, r - 1], []], xt = 0, oe(`\u2B06 Extrusi\xF3n punto\u2192l\xEDnea \u2014 h=${n.toFixed(2)}m`);
      try {
    ...
    if (s === "extl") {
      const n = (window.__hekatanSnap2D ?? 0.5) * 1.5, a = gn(t.x, t.y, t.z, n);
      if (!a) {
        oe("\u2B06 Extruir l\xEDnea \u2014 acerc\xE1 el cursor a una l\xEDnea existente y volv\xE9 a clickear.");
        return;
      }
      const i = e.polylines.rawVal, r = e.points.rawVal, f = i[a.polyIdx], m = r[f[a.segIdx]], y = r[f[a.segIdx + 1]];
      if (!m || !y) {
      ...
      e.points.val = [...e.points.rawVal, [m[0], m[1], m[2]], [y[0], y[1], y[2]], [y[0], y[1], y[2] + M], [m[0], m[1], m[2] + M]];
      const u = e.polylines.rawVal;
      if (e.polylines.val = [...u.slice(0, -1), ...u[u.length - 1].length > 0 ? [u[u.length - 1]] : [], [S, S + 1, S + 2, S + 3, S], []], e.areas) {
  ...
  g.addEventListener("click", () => Xt()), g.addEventListener("contextmenu", (t) => {
    ...
    if (be.geometry.deleteAttribute("position"), s.length) {
      ...
      {
        const r = ((_a2 = e.polylines) == null ? void 0 : _a2.rawVal) ?? [], f = r[r.length - 1] ?? [], m = e.points.rawVal ?? [];
        if (f.length > 0) {
    ...
    if (n.length && a.length) {
      const i = new _(...e.points.rawVal[n[0].index]), r = new _(...a[0].point), f = i.sub(r), m = (_a2 = a[0].face) == null ? void 0 : _a2.normal;
  ...
  g.addEventListener("pointermove", (t) => {
    ...
    const n = k.intersectObject(ee), a = ie();
    if (n.length && a.length) {
      const r = new _(...e.points.rawVal[n[0].index]), f = new _(...a[0].point), m = r.sub(f), y = (_a2 = a[0].face) == null ? void 0 : _a2.normal;
    ...
    if (xo !== void 0) {
      let r = a[0].point;
      (t.ctrlKey || t.metaKey) && (r = new _(Math.round(r.x), Math.round(r.y), Math.round(r.z))), i[xo] = r.toArray();
    }
    e.points.val = i;
  }), g.addEventListener("pointerup", () => {
    ...
    const n = k.intersectObject(ee), a = ie();
    if (n.length && a.length) {
      const f = new _(...e.points.rawVal[n[0].index]), m = new _(...a[0].point), y = f.sub(m), M = (_a2 = a[0].face) == null ? void 0 : _a2.normal;
      M.transformDirection(H.matrixWorld), Math.abs(y.dot(M)) < 1e-4 && (s = true);
    }
    if (!s) return;
    const i = [...e.points.rawVal];
    if (i.splice(n[0].index, 1), e.points.val = i, !e.polylines) return;
    const r = e.polylines.rawVal.map((f) => f.filter((m) => m !== n[0].index)).map((f) => f.map((m) => m > n[0].index ? m - 1 : m)).filter((f) => f.length);
    r.push([]), e.polylines.val = r;
  });
}
function Fa(e, l, d) {
  const w = Math.round(14.999999999999998), p = { position: e.position.clone(), quaternion: e.quaternion.clone() }, g = setInterval(k, 1e3 / 30);
  let x = 0;
  function k() {
    x++;
    const z = x / w;
    e.position.lerpVectors(p.position, l.position, z), e.quaternion.slerpQuaternions(p.quaternion, l.quaternion, z), d && d(), x == w && clearInterval(g);
  }
}
function Aa(e, l, d, h) {
  const c = la(d, e.elements, h);
  return W.derive(() => {
...
function La(e, l, d, h) {
  ...
  return W.derive(() => {
    var _a2, _b, _c;
    l.deformedShape.val;
    const x = d.val, k = ((_a2 = e.elements) == null ? void 0 : _a2.val) ?? [], z = Ta(l.frameResults.val);
    ...
    const b = (_b = e.analyzeOutputs) == null ? void 0 : _b.val, H = (_c = e.deformOutputs) == null ? void 0 : _c.val, le = [], ue = [];
    for (let $ = 0; $ < k.length; $++) {
      if (k[$].length !== 2) continue;
      const he = $a(z, $, b, H);
      he && (le.push(he[0], he[1]), ue.push({ idx: $, vals: he }));
    }
    if (le.length === 0) {
      g.val = [];
      return;
    ...
    let T = 0;
    for (const { idx: $, vals: ne } of ue) {
      const he = k[$], ce = x[he[0]], se = x[he[1]];
      if (!ce || !se) continue;
      const R = new _(se[0] - ce[0], se[1] - ce[1], se[2] - ce[2]), fe = R.length();
      ...
      for (let Xe = 0; Xe < Ie; Xe++) {
        const Qe = Xe / Co, ht = ce[0] + R.x * fe * Qe, Ct = ce[1] + R.y * fe * Qe, P = ce[2] + R.z * fe * Qe, L = ne[0] + (ne[1] - ne[0]) * Qe, Q = w.getColor(L) ?? new Gt(0, 0, 0);
        p.copy(Q).convertSRGBToLinear();
        for (let N = 0; N < _e; N++) {
          const me = N / _e * Math.PI * 2, re = Math.cos(me), Se = Math.sin(me);
          U.push(ht + (we.x * re + ye.x * Se) * ge, Ct + (we.y * re + ye.y * Se) * ge, P + (we.z * re + ye.z * Se) * ge), B.push(p.r, p.g, p.b);
        }
      }
...
function Ua(e) {
  ...
  const le = new Me(), ue = new ut({ color: Na, linewidth: 3, transparent: true, opacity: 0.95, depthTest: false }), ae = new Ht(le, ue);
  ...
  function V(P, L) {
    ...
    const N = e.rendererElm.getBoundingClientRect(), me = P - N.left, re = L - N.top, Se = e.derivedNodes.rawVal, Pe = (_a2 = e.mesh.elements) == null ? void 0 : _a2.rawVal;
    ...
    for (let Ne = 0; Ne < Se.length; Ne++) {
      ...
      const Ae = Ce.x - me, Ke = Ce.y - re, ze = Math.sqrt(Ae * Ae + Ke * Ke);
      ze < rt && (rt = ze, Ye = Ne);
    }
...
(${Ne[0].toFixed(3)}, ${Ne[1].toFixed(3)}, ${Ne[2].toFixed(3)})`;
      const Ae = (_c = (_b = e.mesh) == null ? void 0 : _b.deformOutputs) == null ? void 0 : _c.rawVal;
...
Mz = ${gt(ze[5] * Oe)} ${Le.forceUnit}\xB7m`));
    ...
    for (let Ne = 0; Ne < Pe.length; Ne++) {
      ...
      if (!(!Ce || Ce.length < 2)) {
        if (Ce.length === 2) {
          ...
          const ze = Za(me, re, Ae.x, Ae.y, Ke.x, Ke.y);
    ...
    if (ot >= 0) {
      ...
      const Ke = (_e2 = (_d = e.mesh) == null ? void 0 : _d.elementInputs) == null ? void 0 : _e2.rawVal, ze = (_g = (_f = Ke == null ? void 0 : Ke.sectionInfo) == null ? void 0 : _f.get) == null ? void 0 : _g.call(_f, ot);
      if (ze) {
        ze.name && (Ae += `
  ...
  Material: ${mt}`);
      ...
      if (Ae += `
nodos: [${Ne.join(", ")}]`, Nt === "shell" && ((_l = e.mesh) == null ? void 0 : _l.analyzeOutputs)) {
...
`));
      ...
      if (Nt === "frame" && ((_m = e.mesh) == null ? void 0 : _m.deformOutputs) && e.mesh.elementInputs) {
        const ct = e.mesh.deformOutputs.rawVal, mt = e.mesh.elementInputs.rawVal, it = ct == null ? void 0 : ct.deformations;
        if (it && Ne.length === 2) {
...
Mz \u2248 ${gt(dn * Oe)} ${Le.forceUnit}\xB7m`);
  ...
  function Z(P, L, Q) {
    ...
    if (P.type === "node") {
      ...
      if (Pe) {
        ...
        const Ze = ((_b = e.derivedDisplayScale) == null ? void 0 : _b.rawVal) ?? 1, He = 0.021 * Re * Ze;
        c.position.copy(Pe), c.scale.setScalar(He), c.visible = true;
      }
    } else if (P.type === "frame" && N) {
      const Pe = N[P.idx], Be = T(Pe[0]), Re = T(Pe[1]);
      if (Be && Re) {
        const Ze = Be.clone().add(Re).multiplyScalar(0.5), He = Re.clone().sub(Be), Ye = He.length(), st = e.getActiveCamera().position.distanceTo(Ze) * 35e-4;
        k.position.copy(Ze);
    ...
    const re = e.rendererElm.getBoundingClientRect(), Se = ((_c = e.rendererElm.parentElement) == null ? void 0 : _c.getBoundingClientRect()) ?? re;
    K.style.left = `${L - Se.left}px`, K.style.top = `${Q - Se.top}px`, e.render();
  }
  let Y = "", $ = 0, ne = 0;
  const he = window.__hekatanHoverDebug ?? false, ce = (P) => {
    $ && cancelAnimationFrame($), $ = requestAnimationFrame(() => {
      var _a2, _b, _c;
      const L = V(P.clientX, P.clientY);
      if (he && ne < 5) {
        const N = e.derivedNodes.rawVal, me = (_b = (_a2 = e.mesh) == null ? void 0 : _a2.elements) == null ? void 0 : _b.rawVal;
        console.log(`[hover] pointer (${P.clientX}, ${P.clientY}) nodes=${(N == null ? void 0 : N.length) ?? 0} elems=${(me == null ? void 0 : me.length) ?? 0} hover=`, L), ne++;
      }
  ...
  let se = null;
  const R = () => {
    Y = "", c.visible = false, g.visible = false, k.visible = false, H.visible = false, ae.visible = false, K.style.display = "none", e.render();
  }, fe = (P) => {
    const L = e.rendererElm.getBoundingClientRect(), Q = P.clientX - L.left, N = P.clientY - L.top;
    (Q < -2 || N < -2 || Q > L.width + 2 || N > L.height + 2) && (se && clearTimeout(se), se = window.setTimeout(R, 200));
  }, O = () => {
    se && (clearTimeout(se), se = null);
  };
  e.rendererElm.addEventListener("pointermove", ce), e.rendererElm.addEventListener("pointerleave", fe), e.rendererElm.addEventListener("pointerenter", O);
  function we() {
    var _a2, _b, _c;
  ...
  function Xe(P, L) {
    ...
    if (P.type === "node") {
      ...
      if (!N) return;
      const me = new nt(d, E);
      me.position.copy(N), me.scale.setScalar(Math.max(1e-4, 7 * _e(N))), me.renderOrder = 101, B.add(me);
    } else if (P.type === "frame" && Q) {
      const N = Q[P.idx], me = T(N[0]), re = T(N[1]);
      if (!me || !re) return;
      const Se = me.clone().add(re).multiplyScalar(0.5), Pe = re.clone().sub(me), Be = Pe.length(), Re = e.getActiveCamera().position.distanceTo(Se), Ze = new nt(ee, ie);
      Ze.position.copy(Se);
      ...
      const N = Q[P.idx], me = [], re = [];
      for (const Be of N) {
        const Re = T(Be);
        if (!Re) return;
        me.push(Re.x, Re.y, Re.z);
      }
      N.length === 4 ? re.push(0, 1, 2, 0, 2, 3) : N.length === 3 && re.push(0, 1, 2);
      const Se = new Me();
      Se.setAttribute("position", new bt(me, 3)), Se.setIndex(re), Se.computeVertexNormals();
      const Pe = new nt(Se, be);
      Pe.renderOrder = 101, B.add(Pe);
    } else if (P.type === "solid" && Q) {
      const N = Q[P.idx], me = [[0, 1], [1, 2], [2, 3], [3, 0], [4, 5], [5, 6], [6, 7], [7, 4], [0, 4], [1, 5], [2, 6], [3, 7]], re = [];
      for (const [Be, Re] of me) {
        const Ze = T(N[Be]), He = T(N[Re]);
        Ze && He && re.push(Ze.x, Ze.y, Ze.z, He.x, He.y, He.z);
      }
      const Se = new Me();
      Se.setAttribute("position", new bt(re, 3));
      const Pe = new Ht(Se, ge);
      Pe.renderOrder = 101, B.add(Pe);
    }
  }
  function Qe() {
    if (Ie(), !U.length || !e.mesh) {
      e.render();
      return;
    }
    ...
    if (P.length >= 2) {
      ...
      for (const N of P) for (let me = 0; me < 3; me++) N[me] < L[me] && (L[me] = N[me]), N[me] > Q[me] && (Q[me] = N[me]);
...
function Za(e, l, d, h, c, w) {
  const p = c - d, g = w - h, x = p * p + g * g;
  if (x < 1e-9) {
    const ue = e - d, ae = l - h;
    return Math.sqrt(ue * ue + ae * ae);
  }
  let k = ((e - d) * p + (l - h) * g) / x;
  k = Math.max(0, Math.min(1, k));
  const z = d + k * p, b = h + k * g, H = e - z, le = l - b;
...
function qa(e, l, d) {
  ...
  for (let c = 0, w = d.length - 1; c < d.length; w = c++) {
    ...
    g > l != k > l && e < (x - p) * (l - g) / (k - g + 1e-12) + p && (h = !h);
  }
  return h;
}
function fs(e, l = 8) {
  ...
  return d.querySelectorAll("p").forEach((x) => g.push(x)), setTimeout(() => {
    W.derive(() => {
      c.forEach((x, k) => {
        const z = g[k];
        z && (z.innerText = Ka(e.val, x).toString());
      });
    });
  }), d;
}
...
function si({ mesh: e, settingsObj: l, drawingObj: d, objects3D: h, solids: c }) {
  ...
  b.enableDamping = true, b.dampingFactor = 0.1, b.screenSpacePanning = true, b.zoomSpeed = 0.8, b.panSpeed = 1.2, b.rotateSpeed = 0.9, b.keyPanSpeed = 12, b.listenToKeyEvents(window), b.touches = { ONE: Gn.ROTATE, TWO: Gn.DOLLY_PAN }, z.domElement.addEventListener("wheel", (P) => {
    if (!P.ctrlKey && Math.abs(P.deltaX) > Math.abs(P.deltaY) * 1.5) {
      ...
      const re = Q.length() * 1e-3 * b.panSpeed;
      L.addScaledVector(N, P.deltaX * re), g.position.addScaledVector(N, P.deltaX * re), b.update();
    }
  }, { passive: false });
  const H = new So(new _(-1, 0, 0), 0), le = new So(new _(0, -1, 0), 0), ue = new So(new _(0, 0, -1), 0);
  window.__hekatanClip = window.__hekatanClip ?? { enableX: false, enableY: false, enableZ: false, posX: 0, posY: 0, posZ: 0, invertX: false, invertY: false, invertZ: false };
  function ae() {
    const P = window.__hekatanClip, L = [];
    P.enableX && (H.normal.set(P.invertX ? 1 : -1, 0, 0), H.constant = P.invertX ? -P.posX : P.posX, L.push(H)), P.enableY && (le.normal.set(0, P.invertY ? 1 : -1, 0), le.constant = P.invertY ? -P.posY : P.posY, L.push(le)), P.enableZ && (ue.normal.set(0, 0, P.invertZ ? 1 : -1), ue.constant = P.invertZ ? -P.posZ : P.posZ, L.push(ue)), z.clippingPlanes = L, p.traverse((N) => {
      const me = N;
      if (me.material) {
        const re = Array.isArray(me.material) ? me.material : [me.material];
        for (const Se of re) Se.clippingPlanes = L, Se.needsUpdate = true;
      }
    });
  ...
  const E = ca(l), ie = W.derive(() => Math.pow(10, E.displayScale.val / 10)), ee = Ga(e, E), be = () => {
    const P = [];
  ...
  const B = () => {
    ...
    U.traverse((L) => {
      ...
      const N = L.name ?? "";
      let me = 0.55;
      N.includes("border") ? me = 1 : N.includes("major") && (me = 0.95), Q.opacity = P * me;
    });
  };
  B(), w.appendChild(ra(E, e, c)), w.setAttribute("id", "viewer"), w.appendChild(z.domElement), z.setPixelRatio(window.devicePixelRatio);
  const K = rn();
  ...
  W.derive(() => {
    ...
    p.remove(U), (_a2 = U.traverse) == null ? void 0 : _a2.call(U, (re) => {
      var _a3, _b2, _c2, _d;
    ...
    }), U = Po(P, ge()), U.visible = E.gridVisible.rawVal, p.add(U), B(), p.remove(Z), Z.traverse((re) => {
      var _a3, _b2, _c2, _d;
      (_b2 = (_a3 = re.geometry) == null ? void 0 : _a3.dispose) == null ? void 0 : _b2.call(_a3), (_d = (_c2 = re.material) == null ? void 0 : _c2.dispose) == null ? void 0 : _d.call(_c2);
    }), Z = ds(P, L), p.add(Z);
    const Q = P * 0.5 + P * 0.5 / Math.tan(45 * 0.5);
    g.position.distanceTo(b.target);
    const N = Math.abs(g.position.x) < 0.1 && Math.abs(g.position.y) < 0.1 && g.position.z > 0;
    (((_b = e == null ? void 0 : e.nodes) == null ? void 0 : _b.rawVal) ?? ((_c = e == null ? void 0 : e.nodes) == null ? void 0 : _c.val) ?? []).length > 0 || (N ? g.position.set(0, 0, Q) : g.position.set(0.5 * P, -Q, 0.5 * P), b.target.set(0, 0, 0)), b.minDistance = Math.max(0.05, P * 0.01), b.maxDistance = Math.max(50, P * 50), b.update(), O();
    ...
    for (const L of P) {
      ...
      if (Q === 0 || N === 0) continue;
      const re = (ce ? Q / 2 : Q) / N;
      g.aspect = re, g.updateProjectionMatrix();
      const Se = x.top;
      if (x.left = -Se * re, x.right = Se * re, x.updateProjectionMatrix(), se && se.isPerspectiveCamera) se.aspect = re, se.updateProjectionMatrix();
      else if (se && se.isOrthographicCamera) {
        const Pe = se, Be = Pe.top;
        Pe.left = -Be * re, Pe.right = Be * re, Pe.updateProjectionMatrix();
      }
      z.setSize(Q, N), O();
    }
  ...
  let ce = false, se = null, R = null, fe = false;
  function O() {
  ...
  function ye(P, L) {
    ce = P, L && (se = L);
    const Q = w.clientWidth || 1, N = w.clientHeight || 1, re = (P ? Q / 2 : Q) / N;
    g.isPerspectiveCamera && (g.aspect = re, g.updateProjectionMatrix());
    const Se = x.top;
    if (x.left = -Se * re, x.right = Se * re, x.updateProjectionMatrix(), P && se) {
      if (R ? (R.object = se, R.update()) : (R = new rs(se, z.domElement), R.enableDamping = true, R.dampingFactor = 0.1, R.screenSpacePanning = true, R.zoomSpeed = 0.8, R.panSpeed = 1.2, R.rotateSpeed = 0.9, R.touches = { ONE: Gn.ROTATE, TWO: Gn.DOLLY_PAN }, R.target.copy(b.target), R.addEventListener("change", O), R.enabled = false), !fe) {
        const Pe = (Be) => {
          if (!ce || !R) return;
  ...
  if (e) {
    p.add(da(E, ee, ie), sa(e, E, ee), fa(E, ee, ie), ha(e, E, ee, ie), pa(e, E, ee, ie), ua(e, E, ee, ie), ya(e, E, ee, ie), ga(e, E, ee, ie), _a(e, E, ee), Ca(e, E, ee, ie), Sa(e, E, ee, ie));
    const P = Ua({ scene: p, rendererElm: z.domElement, getActiveCamera: () => k, derivedNodes: ee, derivedDisplayScale: ie, mesh: e, settings: E, render: O });
    p.add(P);
    const L = ja(e, E), Q = Aa(e, E, ee, L), N = fs(L);
    p.add(Q), w.appendChild(N);
    const me = La(e, E, ee);
    p.add(me);
    const re = me.__colorMapValues, Se = fs(re);
  ...
  if (c) {
    ...
    N.color.setHSL(11, 43, 96), N.position.set(-10, 0, 30), p.add(N), W.derive(() => {
      ...
      c.rawVal.forEach((me) => me.visible = E.solids.val), O();
    });
  }
  if (h) {
    const P = [], L = (N) => {
  ...
  const Xe = (P, L, Q) => {
    ...
    const Q = b.target, N = new _().subVectors(k.position, Q), me = N.length(), re = new _(), Se = new _();
    re.crossVectors(k.up, N).normalize(), Se.copy(k.up).normalize();
    const Pe = me * 0.05;
    Q.addScaledVector(re, -P * Pe), Q.addScaledVector(Se, L * Pe), k.position.addScaledVector(re, -P * Pe), k.position.addScaledVector(Se, L * Pe), b.update(), O();
  }, ht = (P) => {
...
function ja(e, l) {
  ...
  return ((c) => {
    ...
    const c = /* @__PURE__ */ new Map(), w = /* @__PURE__ */ new Map(), p = /* @__PURE__ */ new Map(), g = /* @__PURE__ */ new Map(), x = /* @__PURE__ */ new Map(), k = /* @__PURE__ */ new Map(), z = /* @__PURE__ */ new Map(), b = /* @__PURE__ */ new Map(), H = /* @__PURE__ */ new Map(), le = /* @__PURE__ */ new Map(), ue = (L, Q) => {
      L == null ? void 0 : L.forEach((N, me) => {
        const re = e.elements.val[me];
        if (re) for (let Se = 0; Se < re.length; Se++) Q.set(re[Se], [N[Se] ?? N[0]]);
      });
    };
    ue((_b = (_a2 = e.analyzeOutputs) == null ? void 0 : _a2.val) == null ? void 0 : _b.bendingXX, c), ue((_d = (_c = e.analyzeOutputs) == null ? void 0 : _c.val) == null ? void 0 : _d.bendingYY, w), ue((_f = (_e2 = e.analyzeOutputs) == null ? void 0 : _e2.val) == null ? void 0 : _f.bendingXY, p), ue((_h = (_g = e.analyzeOutputs) == null ? void 0 : _g.val) == null ? void 0 : _h.membraneXX, g), ue((_j = (_i = e.analyzeOutputs) == null ? void 0 : _i.val) == null ? void 0 : _j.membraneYY, x), ue((_l = (_k = e.analyzeOutputs) == null ? void 0 : _k.val) == null ? void 0 : _l.membraneXY, k), ue((_n = (_m = e.analyzeOutputs) == null ? void 0 : _m.val) == null ? void 0 : _n.tranverseShearX, z), ue((_p = (_o2 = e.analyzeOutputs) == null ? void 0 : _o2.val) == null ? void 0 : _p.tranverseShearY, b), ue((_r = (_q = e.analyzeOutputs) == null ? void 0 : _q.val) == null ? void 0 : _r.vonMises, H), ue((_t = (_s2 = e.analyzeOutputs) == null ? void 0 : _s2.val) == null ? void 0 : _t.pressure, le);
    const ae = /* @__PURE__ */ new Map(), E = /* @__PURE__ */ new Map(), ie = /* @__PURE__ */ new Map(), ee = /* @__PURE__ */ new Map(), be = /* @__PURE__ */ new Map(), ge = (L, Q, N, me, re) => {
      L.forEach((Se, Pe) => {
        var _a3, _b2;
        ...
        me.set(Pe, [He + Ye]), re.set(Pe, [He - Ye]);
      });
    };
    ...
    const U = (_v = (_u = e.analyzeOutputs) == null ? void 0 : _u.val) == null ? void 0 : _v.colorMapRanges, B = (_w = l.solidResults) == null ? void 0 : _w.val, T = B && B !== "none" ? B : l.shellResults.val, V = U == null ? void 0 : U[T], Z = { bendingXX: [c, 0], bendingYY: [w, 0], bendingXY: [p, 0], membraneXX: [g, 0], membraneYY: [x, 0], membraneXY: [k, 0], tranverseShearX: [z, 0], tranverseShearY: [b, 0], membranePrincipalMax: [ae, 0], membranePrincipalMin: [E, 0], bendingPrincipalMax: [ie, 0], bendingPrincipalMin: [ee, 0], transverseShearMax: [be, 0], vonMises: [H, 0], pressure: [le, 0], displacementX: [(_y = (_x = e.deformOutputs) == null ? void 0 : _x.val) == null ? void 0 : _y.deformations, 0], displacementY: [(_A = (_z = e.deformOutputs) == null ? void 0 : _z.val) == null ? void 0 : _A.deformations, 1], displacementZ: [(_C = (_B = e.deformOutputs) == null ? void 0 : _B.val) == null ? void 0 : _C.deformations, 2] }, Y = l.shellResults.val, $ = Ha.val, ne = Wa.val, he = Y === "displacementX" || Y === "displacementY" || Y === "displacementZ", ce = Y === "bendingXX" || Y === "bendingYY" || Y === "bendingXY" || Y === "bendingPrincipalMax" || Y === "bendingPrincipalMin", se = Y === "membraneXX" || Y === "membraneYY" || Y === "membraneXY" || Y === "membranePrincipalMax" || Y === "membranePrincipalMin", R = Y === "vonMises" || Y === "pressure", fe = Y === "tranverseShearX" || Y === "tranverseShearY" || Y === "transverseShearMax", O = (_D = l.solidResults) == null ? void 0 : _D.val, we = O === "vonMises" || O === "sigmaXX" || O === "sigmaYY" || O === "sigmaZZ" || O === "tauXY" || O === "tauYZ" || O === "tauXZ", ye = O === "ux" || O === "uy" || O === "uz", Ie = Ja.val, _e = we ? Qa[Ie] : ye || he ? ms[ne] : ce || se || R || fe ? 1 / Oa[$] : 1, Xe = we ? Ie : ye || he ? ne : ce ? `${$}\xB7m/m` : se ? `${$}/m\xB2` : R ? `${$}/m\xB2` : fe ? `${$}/m` : "";
    ...
    if (e.nodes.val.forEach((L, Q) => {
      ...
      const me = N[0].get(Q), re = me ? me[N[1]] ?? 0 : 0;
      P.push(re * _e);
    }), !Vn.val && Qe !== "auto") {
      const L = e.nodes.val, Q = /* @__PURE__ */ new Set(), N = (re, Se) => {
        var _a3;
        const Pe = (_a3 = L[re[0]]) == null ? void 0 : _a3[Se];
        return re.every((Be) => {
          var _a4;
      ...
      for (const re of e.elements.val) {
        ...
        const Se = N(re, 2), Pe = !Se && N(re, 0), Be = !Se && N(re, 1);
        if (Qe === "losas" ? Se : Qe === "muros" ? Pe || Be : Qe === "murosX" ? Pe : Qe === "murosY" ? Be : false) for (const He of re) Q.add(He);
      }
      const me = [];
      for (const re of Q) {
        const Se = P[re];
        Number.isFinite(Se) && me.push(Se);
      }
      me.length && (Vn.val = Eo(me));
    }
    d.val = P;
  }), d;
}
