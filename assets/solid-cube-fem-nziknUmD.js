...
import { v as s } from "./Text-C1TX4d8g.js";
import { P as De } from "./tweakpane-BXg6ZhiP.js";
import { h as Ue, __tla as __tla_0 } from "./h8-GvZshkge.js";
import { g as We, j as ye, l as Le, m as Fe, c as Ge, __tla as __tla_1 } from "./aiAgent-86HwhYYU.js";
import { e as Ve } from "./makeDraggable-zx2br6Yh.js";
import { g as Ke, __tla as __tla_2 } from "./getParameters-vOEFDHQf.js";
import { __tla as __tla_3 } from "./didacticCpp-ClOTguHC.js";
Promise.all([
  (() => {
    try {
      return __tla_0;
  ...
  s.derive(() => {
    ...
    let R = Math.max(2, Math.round(j / te));
    R & 1 && (R = Math.max(2, R - 1));
    const Z = z * me, F = R * te, ue = Math.round(O / 2) * te, pe = ue - F / 2, ve = ue + F / 2, _e = -a / 2, fe = a / 2, he = -M / 2, X = M / 2;
    de(_e, he, 0, fe, X, w, C, J, O);
    const be = -Z / 2, ge = Z / 2, je = X, Me = X + p;
    de(be, je, pe, ge, Me, ve, z, Q, R);
    ...
    function Xe(e) {
      D.push(e);
      const t = D.length - 1;
      E.elasticities.set(t, B), E.poissonsRatios.set(t, N), E.thicknesses.set(t, 1e-3), E.shearModuli.set(t, B / (2 * (1 + N))), E.densities.set(t, 24 / 9.80665), E.areas.set(t, 0), E.momentsOfInertiaZ.set(t, 0), E.momentsOfInertiaY.set(t, 0), E.torsionalConstants.set(t, 0);
    }
    ...
    if (h) {
      ...
      const o = Z * F * F * F / 12, l = Z * F, c = B / (2 * (1 + N)), b = 5 / 6, i = A * p * p * p / (3 * B * o), g = A * p / (b * c * l), d = a * M * M * M / 12, x = A * p * w / (16 * B * d) * p, m = i + g + x, _ = Math.abs(t - i) / Math.abs(i || 1) * 100, f = Math.abs(t - m) / Math.abs(m || 1) * 100;
    ...
    $ && console.error("Solver H8 fall\xF3:", $), Se.val = Ne, Te.val = D, He.val = {
      supports: Be,
      loads: ke
    }, Ce.val = E, Oe.val = Pe;
  });
...
\xB7 vs CalculiX, Code Aster, FEniCS, SAP2000`;
  ...
  document.body.append(Ke(u), ne, Ge({
