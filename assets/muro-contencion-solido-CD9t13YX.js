...
import { v as n } from "./Text-C1TX4d8g.js";
import { P as se } from "./tweakpane-BXg6ZhiP.js";
import { h as oe, __tla as __tla_0 } from "./h8-Dq5Es-cV.js";
import { g as ne, i as A, j as Z, a as le, __tla as __tla_1 } from "./aiAgent-BdFWxvrn.js";
import { e as ie } from "./makeDraggable-zx2br6Yh.js";
import { g as me, __tla as __tla_2 } from "./getParameters-B8Tz82H4.js";
...
Promise.all([
  ...
  function re(e) {
    const D = e.toe + e.t + e.heel, P = e.tf + e.H, i = Math.max(1, Math.round(D / e.ms)), x = Math.max(1, Math.round(e.L / e.ms)), d = Math.max(1, Math.round(P / e.ms)), y = D / i, h = e.L / x, r = P / d, v = Math.round(e.toe / y), w = Math.round((e.toe + e.t) / y), _ = Math.round(e.tf / r), z = (a, s) => s < _ || a >= v && a < w, b = /* @__PURE__ */ new Map(), T = [], H = e.tTop !== void 0 && e.tTop > 0 && e.tTop < e.t ? e.tTop : e.t, I = (a, s) => {
      if (H === e.t || s < _ || a < v || a > w) return a * y;
    ...
    const j = Math.round(x / 2), N = b.get(`${w},${j},${d}`);
    return {
      nodes: T,
      elements: u,
      supports: M,
  ...
  n.derive(() => {
    ...
    if (d) {
      ...
      i.elements.forEach((M, c) => {
        ...
        let f = 0, S = 0;
        if (P === 2) for (const j of l) f += j, S++;
        else for (const j of O) f += j[P === 0 ? 0 : 2], S++;
        f = S ? f / S : 0;
        for (const j of M) {
          const N = t.get(j) ?? {
            s: 0,
