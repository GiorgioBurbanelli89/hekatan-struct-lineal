...
function Ea(e, l, c, p) {
  ...
  function x(I, K, X, T) {
    ...
    function Y(ve, Pe, me, Re) {
      ae.push(0, ve, Pe, 0, me, Pe, 0, me, Re, 0, ve, Pe, 0, me, Re, 0, ve, Re);
    }
  ...
  function $(I, K, X) {
    ...
    const fe = [];
    function ae(xe, ve, Pe, me) {
      fe.push(0, xe, ve, 0, Pe, ve, 0, Pe, me, 0, xe, ve, 0, Pe, me, 0, xe, me);
    }
  ...
  return te.derive(() => {
    ...
    X.forEach((L, ne) => {
      ...
      if (Y.type === "CFT") {
        ...
        me.position.set(...pe), me.rotation.setFromRotationMatrix(Q), f.add(me);
        ...
        switch (Y.type) {
          ...
          case "HSS":
            ...
            break;
          case "CFT":
            ve = $(Y.b, Y.h, Y.tw ?? Y.b * 0.05), Pe = ie, me = le;
            break;
          case "L":
            ve = _(Y.b ?? Y.h, Y.h, Y.t ?? Y.tw ?? 3e-3), Pe = ie, me = le;
            break;
          case "2L":
            ve = O(Y.b ?? Y.h, Y.h, Y.t ?? Y.tw ?? 3e-3, Y.dis ?? 0.01), Pe = ie, me = le;
            break;
          case "C":
          case "coldC":
            ve = ye(Y.b, Y.h, Y.tf ?? Y.t ?? 3e-3, Y.tw ?? Y.t ?? 3e-3), Pe = ie, me = le;
            break;
          case "2C":
        ...
        Re.position.set(...pe), Re.rotation.setFromRotationMatrix(Q), f.add(Re);
        const Ue = new zt(ve.outline, me);
        Ue.position.set(...pe), Ue.rotation.setFromRotationMatrix(Q), f.add(Ue);
      }
      const xe = Aa(Y);
      if (xe) {
        const Pe = ["I", "HSS", "CFT", "L", "2L", "C", "2C", "T", "pipe", "coldC"].includes(Y.type) ? "#ff9900" : "#00ccff", me = new At(xe, Pe, "transparent");
        me.position.set(pe[0], pe[1], pe[2]);
        const Re = 0.05 * l.gridSize.rawVal * 0.5;
        me.updateScale(Re * ((p == null ? void 0 : p.rawVal) ?? 1)), y.add(me);
      }
    });
  }), p && te.derive(() => {
...
function Na({ drawingObj: e, gridObj: l, scene: c, getActiveCamera: p, controls: f, gridSize: y, derivedDisplayScale: v, rendererElm: M, viewerRender: x }) {
  ...
  window.__hekatanMeasureRefresh = Ue, window.__hekatanClearMeasure = () => {
  ...
  const ke = (n) => {
    ...
    return new zt(o, a);
  }, we = ke(16711680), De = ke(65280), Ve = ke(35071);
  he.add(we, De, Ve);
  ...
  window.__hekatanUpdateSelectionPtScale = qo, f.addEventListener("change", () => {
    ...
    if ((!a.length || ((_c = (_b = (_a3 = window.__hekatanCadState) == null ? void 0 : _a3.get) == null ? void 0 : _b.call(_a3)) == null ? void 0 : _c.tool) !== "fillarea") && et.visible && (et.visible = false), a.length) {
      ...
      if (g.length > 0 && k[g[g.length - 1]]) {
        ...
        We(ee), ee && (ze.style.left = n.clientX + "px", ze.style.top = n.clientY + "px"), we.geometry.setFromPoints([new F(r[0] - z, r[1], r[2]), new F(r[0] + z, r[1], r[2])]), (_v = we.computeLineDistances) == null ? void 0 : _v.call(we), De.geometry.setFromPoints([new F(r[0], r[1] - z, r[2]), new F(r[0], r[1] + z, r[2])]), (_w = De.computeLineDistances) == null ? void 0 : _w.call(De), Ve.geometry.setFromPoints([new F(r[0], r[1], r[2] - z), new F(r[0], r[1], r[2] + z)]), (_x = Ve.computeLineDistances) == null ? void 0 : _x.call(Ve), he.visible = true;
        const ue = we.material, _e2 = De.material, Ke = Ve.material;
  ...
  const ls = (n, o) => {
    ...
    if (a === "medir") {
      ...
      if (Pe.length >= 2 && (Pe = []), Pe.push(t), Pe.length === 1) ve.visible = false, Ue(), ce("\u{1F4CF} Regla \u2014 1er punto puesto. Marca el 2\xBA.");
        ...
        me.textContent = `${d.toFixed(3)} m`, Ue(), ce(`\u{1F4CF} Distancia ${d.toFixed(3)} m  \xB7  \u0394x ${(i[0] - s[0]).toFixed(3)}  \u0394y ${(i[1] - s[1]).toFixed(3)}  \u0394z ${(i[2] - s[2]).toFixed(3)}  \xB7  en planta ${h.toFixed(3)} m`);
      }
      x();
      return;
    }
...
function Ga(e, l, c, p) {
  ...
  return te.derive(() => {
    ...
    for (const { idx: L, vals: ne } of be) {
      ...
      Y.normalize();
      const Q = Math.abs(Y.y) < 0.99 ? new F(0, 1, 0) : new F(1, 0, 0), xe = new F().crossVectors(Y, Q).normalize(), ve = new F().crossVectors(Y, xe).normalize(), Pe = $o + 1, me = Za;
      for (let Re = 0; Re < Pe; Re++) {
        ...
        for (let J = 0; J < me; J++) {
          const ge = J / me * Math.PI * 2, he = Math.cos(ge), ke = Math.sin(ge);
          N.push(et + (xe.x * he + ve.x * ke) * le, gt + (xe.y * he + ve.y * ke) * le, V + (xe.z * he + ve.z * ke) * le), I.push(v.r, v.g, v.b);
        }
      }
      for (let Re = 0; Re < $o; Re++) for (let Ue = 0; Ue < me; Ue++) {
        const et = (Ue + 1) % me, gt = X + Re * me + Ue, V = X + Re * me + et, H = X + (Re + 1) * me + Ue, j = X + (Re + 1) * me + et;
...
function ti(e) {
  ...
  function T(V, H) {
    ...
    const J = e.rendererElm.getBoundingClientRect(), ge = V - J.left, he = H - J.top, ke = e.derivedNodes.rawVal, we = (_a2 = e.mesh.elements) == null ? void 0 : _a2.rawVal;
    if (!ke || !we) return null;
    ...
    for (const Be of we) if (Be) for (const Le of Be) tt.add(Le);
    const Oe = 8;
    let Ye = -1, vt = Oe;
...
Mz \u2248 ${Mt(Nn * Ae)} ${He.forceUnit}\xB7m`);
  ...
  function q(V, H, j) {
    ...
    if (V.type === "node") {
      ...
      if (we) {
        ...
        f.position.copy(we), f.scale.setScalar(Oe), f.visible = true;
      }
    } else if (V.type === "frame" && J) {
      const we = J[V.idx], De = X(we[0]), Ve = X(we[1]);
      if (De && Ve) {
      ...
      const we = J[V.idx], De = [], Ve = [];
      for (const tt of we) {
        const Oe = X(tt);
        if (!Oe) return;
        De.push(Oe.x, Oe.y, Oe.z);
      }
      we.length === 4 ? Ve.push(0, 1, 2, 0, 2, 3) : we.length === 3 && Ve.push(0, 1, 2), $.setAttribute("position", new St(De, 3)), $.setIndex(Ve), $.computeVertexNormals(), O.visible = true;
    } else if (V.type === "solid" && J) {
      const we = J[V.idx], De = [[0, 1], [1, 2], [2, 3], [3, 0], [4, 5], [5, 6], [6, 7], [7, 4], [0, 4], [1, 5], [2, 6], [3, 7]], Ve = [];
      for (const [tt, Oe] of De) {
        const Ye = X(we[tt]), vt = X(we[Oe]);
  ...
  function Re(V, H) {
    ...
    if (V.type === "node") {
      ...
      if (!J) return;
      const ge = new it(c, D);
      ge.position.copy(J), ge.scale.setScalar(Math.max(1e-4, 7 * me(J))), ge.renderOrder = 101, I.add(ge);
    } else if (V.type === "frame" && j) {
      const J = j[V.idx], ge = X(J[0]), he = X(J[1]);
      if (!ge || !he) return;
      const ke = ge.clone().add(he).multiplyScalar(0.5), we = he.clone().sub(ge), De = we.length(), Ve = e.getActiveCamera().position.distanceTo(ke), tt = new it(B, oe);
      tt.position.copy(ke);
      const Oe = new F(0, 1, 0);
      tt.quaternion.setFromAxisAngle(Oe.clone().cross(we).normalize(), Oe.angleTo(we)), tt.scale.set(Ve * 35e-4, De, Ve * 35e-4), tt.renderOrder = 101, I.add(tt);
    } else if (V.type === "shell" && j) {
      ...
      const we = new it(ke, ie);
      we.renderOrder = 101, I.add(we);
    } else if (V.type === "solid" && j) {
...
function li(e, l) {
  ...
  function x() {
    ...
    const Pe = Q - pe || 1, me = ve - xe || 1, Re = 0.12 * Math.max(Pe, me), Ue = 46, et = Math.min((fe - 2 * Ue) / (Pe + 2 * Re), (ae - 2 * Ue) / (me + 2 * Re)), gt = (fe - Pe * et) / 2, V = (ae - me * et) / 2, H = (Me) => gt + (Me - pe) * et, j = (Me) => ae - (V + (Me - xe) * et), J = "http://www.w3.org/2000/svg", ge = (Me, Ae, ct) => {
    ...
    if (de) for (const Me of Y) {
      ...
      Ae && (we = Math.max(we, Math.abs(Ae[0] ?? 0), Math.abs(Ae[1] ?? 0)));
    }
    const De = 0.12 * Math.max(Pe, me) * et, Ve = we > 0 ? De / we : 0, tt = R === "bendingsY" || R === "bendingsZ", Oe = (Me) => Math.abs(Me) >= 100 ? Me.toFixed(1) : Math.abs(Me) >= 10 ? Me.toFixed(2) : Me.toFixed(3), Ye = [];
    for (const Me of Y) {
    ...
    for (const Me of Ye) Me.peso < 0.02 * we || vt.some((Ae) => Math.hypot(Ae.x - Me.x, Ae.y - Me.y) < 34) || (vt.push(Me), ge("text", { x: Me.x, y: Me.y + 4, "text-anchor": "middle", fill: "#f2f5fa", "font-size": 12, "font-weight": 600, "paint-order": "stroke", stroke: "#0b0e14", "stroke-width": 3 }, Me.t));
    const He = R ? si[R] ?? R : "sin resultado";
    le.textContent = `${He} \xB7 ${p.plano === "XY" ? "planta" : "alzado"} ${p.plano} en ${T} = ${p.en.toFixed(2)} m`, N.textContent = R ? `${Y.length} barras en el plano \xB7 m\xE1ximo ${Oe(we)} ${ai[R] ?? ""}` + (tt ? " \xB7 el momento va del lado de la tracci\xF3n" : "") + (ke ? ` \xB7 ${ke} barra(s) con este resultado en otro plano (sin dibujar)` : "") + " \xB7 clic en una barra: su gr\xE1fico" : "Eleg\xED un resultado en \xABFrame results\xBB (Axial, Cortante, Momento) para ver su diagrama aqu\xED.";
  }
  const P = () => {
    try {
      x();
  ...
  function oe() {
    ...
    let K = 0, X = -1;
    N.forEach((Q, xe) => {
      const [ve, Pe] = ie[Q], me = xe === 0 ? N.length > 1 && ie[N[1]].includes(ve) : ve !== X, Re = me ? Pe : ve, Ue = me ? ve : Pe, et = Math.hypot(B[Ue][0] - B[Re][0], B[Ue][1] - B[Re][1], B[Ue][2] - B[Re][2]);
      I.push({ x: K, e: Q, fin: me ? 1 : 0 }), K += et, I.push({ x: K, e: Q, fin: me ? 0 : 1 }), X = Ue;
    });
    const T = K, q = (Q, xe) => {
    ...
    for (const [Q, xe, ve, Pe] of ne) {
      const me = I.map((we) => q(Q, we)), Re = Math.max(...me), Ue = Math.min(...me), et = Math.max(Math.abs(Re), Math.abs(Ue)) || 1, gt = (we) => Y + we / (T || 1) * (fe - 2 * Y), V = (we) => pe + (Pe ? 1 : -1) * (we / et) * (pe - 16), H = (we) => Math.abs(we) >= 100 ? we.toFixed(1) : Math.abs(we) >= 10 ? we.toFixed(2) : we.toFixed(3);
      let j = gt(0) + "," + pe + " ";
      I.forEach((we, De) => {
        j += gt(we.x) + "," + V(me[De]) + " ";
      }), j += gt(T) + "," + pe;
      const J = me.indexOf(Re), ge = me.indexOf(Ue), he = (we, De) => {
        const Ve = V(me[we]) + (V(me[we]) < pe ? -5 : 13);
        return '<text x="' + gt(I[we].x) + '" y="' + Ve + '" text-anchor="middle" fill="' + De + '" font-size="11" font-weight="700" paint-order="stroke" stroke="#0b0e14" stroke-width="3">' + H(me[we]) + "</text>";
      }, ke = Pe ? "#d9534f" : "#3fa7d6";
...
function vi({ mesh: e, settingsObj: l, drawingObj: c, objects3D: p, solids: f }) {
  ...
  te.derive(() => {
    ...
    for (const H of V) {
      ...
      if (x.left = -ke * he, x.right = ke * he, x.updateProjectionMatrix(), ae && ae.isPerspectiveCamera) ae.aspect = he, ae.updateProjectionMatrix();
        ...
        const we = ae, De = we.top;
        we.left = -De * he, we.right = De * he, we.updateProjectionMatrix();
      }
      $.setSize(j, J), Q();
    }
  ...
  function ve(V, H) {
    ...
    if (x.left = -ke * he, x.right = ke * he, x.updateProjectionMatrix(), V && ae) {
      if (Y ? (Y.object = ae, Y.update()) : (Y = new gs(ae, $.domElement), Y.enableDamping = true, Y.dampingFactor = 0.1, Y.screenSpacePanning = true, Y.zoomSpeed = 0.8, Y.panSpeed = 1.2, Y.rotateSpeed = 0.9, Y.touches = { ONE: jn.ROTATE, TWO: jn.DOLLY_PAN }, Y.target.copy(_.target), Y.addEventListener("change", Q), Y.enabled = false), !pe) {
        ...
        $.domElement.addEventListener("pointerdown", we, true), $.domElement.addEventListener("wheel", we, { capture: true, passive: true }), pe = true;
      }
  ...
  if (e) {
    v.add(Ma(D, B, oe), wa(e, D, B), Sa(D, B, oe), Pa(e, D, B, oe), _a(e, D, B, oe), ka(e, D, B, oe), Fa(e, D, B, oe), Ea(e, D, B, oe), La(e, D, B), Ba(e, D, B, oe), Ia(e, D, B, oe)), window.__hekatanDiagrama2D || (li(e, D), $.domElement.addEventListener("dblclick", () => {
      var _a2;
      const we = (_a2 = D.frameResults) == null ? void 0 : _a2.rawVal;
      !we || we === "none" || !(window.__hekatanModelSelection ?? []).some((Ve) => Ve.type === "frame") || setTimeout(() => {
        var _a3;
    ...
    ke.id = "frame-legend", y.appendChild(ke), te.derive(() => {
      var _a2;
      const we = D.shellResults.val != "none", De = (((_a2 = D.solidResults) == null ? void 0 : _a2.val) ?? "none") !== "none", Ve = we || De, tt = D.frameResults.val.startsWith("contour:"), Oe = H.val.some((Ye) => Number.isFinite(Ye));
  ...
  const Re = (V, H, j) => {
    ...
    const we = ge * 0.05;
    j.addScaledVector(he, -V * we), j.addScaledVector(ke, H * we), P.position.addScaledVector(he, -V * we), P.position.addScaledVector(ke, H * we), _.update(), Q();
  }, et = (V) => {
  ...
  return me.append(gt()), me.append(Re("\u2191", "Pan arriba", () => Ue(0, 1))), me.append(Re("\u2295", "Zoom in", () => et(0.85))), me.append(Re("\u2190", "Pan izquierda", () => Ue(-1, 0))), me.append(Re("\u2302", "Reset vista", () => {
    _.reset(), Q();
  })), me.append(Re("\u2192", "Pan derecha", () => Ue(1, 0))), me.append(Re("\u2296", "Zoom out", () => et(1.18))), me.append(Re("\u2193", "Pan abajo", () => Ue(0, -1))), me.append(gt()), getComputedStyle(y).position === "static" && (y.style.position = "relative"), y.appendChild(me), y;
}
function ci(e, l) {
  return te.derive(() => {
    var _a2, _b, _c, _d;
...
function mi(e, l) {
  ...
  return ((f) => {
    ...
    const se = /* @__PURE__ */ new Map(), D = /* @__PURE__ */ new Map(), oe = /* @__PURE__ */ new Map(), B = /* @__PURE__ */ new Map(), ie = /* @__PURE__ */ new Map(), le = (H, j, J, ge, he) => {
      H.forEach((ke, we) => {
        var _a3, _b2;
        const De = ke[0] ?? 0, Ve = ((_a3 = j.get(we)) == null ? void 0 : _a3[0]) ?? 0, tt = ((_b2 = J.get(we)) == null ? void 0 : _b2[0]) ?? 0, Oe = (De + Ve) / 2, Ye = Math.hypot((De - Ve) / 2, tt);
        ge.set(we, [Oe + Ye]), he.set(we, [Oe - Ye]);
      });
    };
    ...
    if (e.nodes.val.forEach((H, j) => {
      ...
      const H = e.nodes.val, j = /* @__PURE__ */ new Set(), J = (he, ke) => {
        var _a3;
        const we = (_a3 = H[he[0]]) == null ? void 0 : _a3[ke];
        return he.every((De) => {
          var _a4;
          return Math.abs((((_a4 = H[De]) == null ? void 0 : _a4[ke]) ?? NaN) - we) < 1e-6;
        });
      };
      for (const he of e.elements.val) {
        if (he.length !== 4) continue;
        const ke = J(he, 2), we = !ke && J(he, 0), De = !ke && J(he, 1);
        if (Ue === "losas" ? ke : Ue === "muros" ? we || De : Ue === "murosX" ? we : Ue === "murosY" ? De : false) for (const Oe of he) j.add(Oe);
      }
      const ge = [];
      for (const he of j) {
        const ke = V[he];
