/**
 * Ejercicio 4 del libro Guerra MDI (pag.74-90).
 *
 * ZAPATA COMBINADA RECTANGULAR (2 columnas).
 * L=7.50m, B=2.50m, h=0.50m.
 * Columna 1 a x=1.00m (45×45cm): P_D=90, M_D=7, P_L=23, M_L=3
 * Columna 2 a x=6.50m (45×45cm): P_D=100, M_D=14, P_L=30, M_L=4
 * Material: f'c=240 kg/cm², q_adm=18 t/m²
 * Libro pag.76: σ_max=16.018 t/m², σ_min=9.901 t/m² (cumple q_adm).
 */
...
export const guerraEj4ZapataCombinada: ExampleDef = {
  ...
  guide: [
    ...
    "L=7.50m, B=2.50m, h=0.50m. Col 45×45cm en x=1.00m y x=6.50m.",
    "Col 1: P_D=90, M_D=7, P_L=23, M_L=3 (tonf, tonf·m)",
    "Col 2: P_D=100, M_D=14, P_L=30, M_L=4",
    "Libro pag.76: σ_max=16.018, σ_min=9.901 t/m² (cumple q_adm=18).",
  ],
  params: {
    L:           { default: 7.50, min: 5.00, max: 10.00, step: 0.10, label: "L (m)" },
    B:           { default: 2.50, min: 1.50, max: 4.00, step: 0.05, label: "B (m)" },
    h:           { default: 0.50, min: 0.30, max: 0.90, step: 0.05, label: "h espesor (m)" },
    col1_x:      { default: 1.00, min: 0.30, max: 3.00, step: 0.05, label: "col1 x (m)" },
    col2_x:      { default: 6.50, min: 3.00, max: 9.00, step: 0.05, label: "col2 x (m)" },
    col_size:    { default: 0.45, min: 0.20, max: 1.00, step: 0.05, label: "col lado (m)" },
    ks_tm3:      { default: 3640, min: 500, max: 8000, step: 50, label: "ks (tonf/m³)" },
    P_dead_c1:   { default: 90.0, min: 0, max: 300, step: 1, label: "P_D col1 (tonf)" },
    M_dead_c1:   { default: 7.0,  min: -50, max: 50, step: 0.5, label: "M_D col1 (tonf·m)" },
    P_live_c1:   { default: 23.0, min: 0, max: 100, step: 1, label: "P_L col1 (tonf)" },
    M_live_c1:   { default: 3.0,  min: -30, max: 30, step: 0.5, label: "M_L col1 (tonf·m)" },
    P_dead_c2:   { default: 100.0, min: 0, max: 300, step: 1, label: "P_D col2 (tonf)" },
    M_dead_c2:   { default: 14.0, min: -50, max: 50, step: 0.5, label: "M_D col2 (tonf·m)" },
    P_live_c2:   { default: 30.0, min: 0, max: 100, step: 1, label: "P_L col2 (tonf)" },
    M_live_c2:   { default: 4.0,  min: -30, max: 30, step: 0.5, label: "M_L col2 (tonf·m)" },
    fc_kgcm2:    { default: 240, min: 175, max: 600, step: 5, label: "f'c (kg/cm²)" },
    nx:          { default: 30, min: 12, max: 48, step: 2, label: "nx mesh" },
    ny:          { default: 10, min: 6, max: 24, step: 2, label: "ny mesh" },
  ...
  build(p, states) {
    ...
    const P_kN_c1 = (p.P_dead_c1 + p.P_live_c1) * TONF_TO_KN;
    const M_kNm_c1 = (p.M_dead_c1 + p.M_live_c1) * TONF_TO_KN;
    const P_kN_c2 = (p.P_dead_c2 + p.P_live_c2) * TONF_TO_KN;
    const M_kNm_c2 = (p.M_dead_c2 + p.M_live_c2) * TONF_TO_KN;
    ...
    // Distribuir cargas sobre cada columna
    const cy = Bz / 2;
    const findColNodes = (cxCol: number) => {
      ...
      for (let n = 0; n < nodes.length; n++) {
        ...
        if (Math.abs(xn - cxCol) <= p.col_size/2 + 1e-6 &&
            Math.abs(yn - cy) <= p.col_size/2 + 1e-6) {
          r.push(n);
        }
      }
      return r;
    ...
    const P1_pn = P_kN_c1 / col1Nodes.length;
    const M1_pn = M_kNm_c1 / col1Nodes.length;
    const P2_pn = P_kN_c2 / col2Nodes.length;
  ...
  computedLabels(_p, states) {
    ...
    const sMaxLib = ref?.manual_libro?.sigma_max_servicio_tm2;
    const sMinLib = ref?.manual_libro?.sigma_min_servicio_tm2;
    ...
    return {
      ...
      "📘 σ_max libro p.76":   sMaxLib ? `${sMaxLib.toFixed(3)} t/m²` : "—",
      "📘 σ_min libro p.76":   sMinLib ? `${sMinLib.toFixed(3)} t/m²` : "—",
      "Δ σ_max vs libro":      diffPct(sMax, sMaxLib),
      "Δ σ_min vs libro":      diffPct(sMin, sMinLib),
    };
  },
};
