...
export function exportS2k(input: S2kExportInput): string {
  ...
  const frameSecs = new Map<string, { A: number; Iz: number; Iy: number; J: number; b: number; h: number; matKey: string; As2: number; As3: number; sd?: SdCft;
    param?: { kind: "I" | "Box" | "C" | "2L"; t3: number; t2: number; tf: number; tw: number; t2b?: number; tfb?: number; dis?: number } }>();
  ...
  for (const i of frameIdx) {
    ...
    let param: { kind: "I" | "Box" | "C" | "2L"; t3: number; t2: number; tf: number; tw: number; t2b?: number; tfb?: number; dis?: number } | undefined;
    ...
    else if (!sd && shp?.type === "C" && shp.h > 0 && shp.b > 0 && shp.tf > 0 && shp.tw > 0)
  ...
  if (frameSecs.size > 0) {
    ...
    for (const [, sec] of frameSecs) {
      ...
      if (sec.param) {
        const p = sec.param;
        // (14-sep-2026) Channel y "Double Angle": filas copiadas del .$2k de SAP2000 24 (cli/_csi_secciones_c2l.py);
        // en el doble ángulo t2 es el ancho TOTAL y dis la separación (SngAngWid la calcula SAP).
