# -*- coding: utf-8 -*-
"""El Test II del paper (voladizo corto de Ibrahimbegovic, Taylor y Wilson 1990) en SAP2000,
importando el .s2k que escribe Hekatan Struct: NO se dibuja nada a mano, que es justo lo que
tarda (Jorge, 22-sep-2026: «solo importar el e2k s2k de Hekatan Struct para no demorarnos
haciendo geometria»).

Para cada malla: abre el .s2k, resuelve, lee Uz del nudo de la PUNTA (el del centro del borde
derecho) y guarda un fotograma de la ventana de SAP2000.

    python sap_itw.py            -> ITW_T2.json + fotogramas sap_*.png
"""
...
MALLAS = [("4x1", "ITW_T2.s2k"), ("8x2", "ITW_T2_8x2.s2k"), ("16x4", "ITW_T2_16x4.s2k")]
PAPER = {"4x1": 0.3445, "8x2": 0.3504, "16x4": 0.3543}
EXACTO = 0.3553


def foto(nombre):
    """Fotograma de la ventana de SAP2000 (PrintWindow, aunque esté tapada)."""
    try:
        ...
        def cb(w, _):
            nonlocal h
            if win32gui.IsWindowVisible(w) and "SAP2000" in win32gui.GetWindowText(w):
                h = h or w
            return True
        ...
        Image.frombuffer("RGB", (info["bmWidth"], info["bmHeight"]), bits, "raw", "BGRX", 0, 1).save(
            os.path.join(AQUI, f"sap_{nombre}.png"))
...
helper = comtypes.client.CreateObject("SAP2000v1.Helper").QueryInterface(
...
for nombre, fich in MALLAS:
    ...
    foto(nombre)
json.dump({"exacto": EXACTO, "mallas": res}, open(os.path.join(AQUI, "ITW_T2_sap.json"), "w"), indent=1)
