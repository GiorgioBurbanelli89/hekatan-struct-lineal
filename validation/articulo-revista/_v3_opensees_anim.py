# -*- coding: utf-8 -*-
"""v3 — OpenSees: ANIMACIÓN de los modos 1, 2 y 3 del mismo dual 2x2x4
(545 nudos, malla 1.0 m).  Mismo montaje que _cap_opensees.py.

    py _v3_opensees_anim.py <carpeta_salida>

Deja  <salida>/anim/opensees_m{1,2,3}_###.png  a 1920x1080.
"""
...
for m in (1, 2, 3):
    ...
    for f in range(NF):
        ...
        ax.set_title("OpenSees 3.x — Test M Dual · MODO %d ·  T = %.4f s   (animado, escala ×%.0f)"
                     % (m, T[m - 1], esc), fontsize=20, weight="bold")
        fig.tight_layout()
        fig.savefig(os.path.join(ANIM, "opensees_m%d_%03d.png" % (m, f)), facecolor="white")
        plt.close(fig)
    print("modo %d: %d frames" % (m, NF), flush=True)

json.dump({"T": T}, open(os.path.join(AQUI, "opensees_dual_v3.json"), "w"), indent=1)
print("FIN ->", ANIM)
