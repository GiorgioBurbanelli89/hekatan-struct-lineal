...
export function scriptKLocalBarra(mesh: MallaK, idx: number): { nombre: string; texto: string } {
  ...
  p(`% ============================================================`);
  p(`%  MATRIZ DE RIGIDEZ LOCAL 12x12 - barra ${idx + 1} (indice ${idx} del motor)`);
  p(`%  Generado por Hekatan Struct con los datos que recibe el motor.`);
  p(`%  Formula = hekatan-fem/src/cpp/utils/getLocalStiffnessMatrix.cpp`);
  ...
  p(`E  = ${num(val("elasticities"))};      % modulo de elasticidad`);
  p(`G  = ${num(val("shearModuli"))};      % modulo de cortante`);
  p(`A  = ${num(val("areas"))};      % area`);
  p(`Iz = ${num(val("momentsOfInertiaZ"))};      % I33: flexion en el plano 1-2 (V2, M3)`);
  p(`Iy = ${num(val("momentsOfInertiaY"))};      % I22: flexion en el plano 1-3 (V3, M2)`);
  p(`J  = ${num(val("torsionalConstants"))};      % constante de torsion`);
  p(`AsY = ${num(val("shearAreasY"))};     % area de cortante asociada a Iy (0 = 5/6*A, <0 = Bernoulli)`);
  p(`AsZ = ${num(val("shearAreasZ"))};     % area de cortante asociada a Iz (0 = 5/6*A, <0 = Bernoulli)`);
  if (ang) p(`% ang = ${num(ang)} grados: gira la seccion en T, NO cambia esta matriz local.`);
  p();
  ...
  p();
  p(`% --- Matriz local (misma disposicion que el C++) ----------------------`);
  ...
  if (rel && rel.some(Boolean)) {
    ...
    p();
    p(`% --- Liberaciones: condensacion estatica  Kc = Krr - Krf*inv(Kff)*Kfr --`);
